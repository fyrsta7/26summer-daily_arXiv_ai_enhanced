...
- [BoostAPR: Boosting Automated Program Repair via Execution-Grounded Reinforcement Learning with Dual Reward Models](http://arxiv.org/abs/2605.09134v3) — 相关度 0.90：基于执行反馈的强化学习与行级信用分配，直接用于自动程序修复，与coding agent高度相关。
- [Debugging the Debuggers: Failure-Anchored Structured Recovery for Software Engineering Agents](http://arxiv.org/abs/2605.08717v2) — 相关度 0.90：失败锚定的结构化恢复框架，显著提升SE agent的诊断准确率与恢复率。
- [Semantic Voting: Execution-Grounded Consensus for LLM Code Generation](http://arxiv.org/abs/2605.08680v1) — 相关度 0.90：执行指纹聚类的语义投票，改进无oracle条件下的代码生成选择方法。
- [EvidenT: An Evidence-Preserving Framework for Iterative System-Level Package Repair](http://arxiv.org/abs/2605.08621v1) — 相关度 0.85：证据保留的系统级包修复框架，闭循环验证，大幅提升agent修复成功率。
- [Sketch-and-Verify: Structured Inference-Time Scaling via Program Sketching](http://arxiv.org/abs/2605.08658v1) — 相关度 0.85：草图化归纳推理时扩展，以结构化方式提升小模型代码生成的性价比。
- [Skill Drift Is Contract Violation: Proactive Maintenance for LLM Agent Skill Libraries](http://arxiv.org/abs/2605.10990v1) — 相关度 0.85：将技能漂移视为契约违反，实现精确的agent技能库维护与修复。
- [Using Semantic Distance to Estimate Uncertainty in LLM-Based Code Generation](http://arxiv.org/abs/2605.09023v1) — 相关度 0.85：基于语义距离的不确定性估计直接提升LLM代码生成的正确性验证与可靠性。
- [A Learning Method for Symbolic Systems Using Large Language Models](http://arxiv.org/abs/2605.08694v1) — 相关度 0.70：LLM挖掘可复用证明策略并集成到符号证明器，可配合agent形式化验证。
- [Containment Verification: AI Safety Guarantees Independent of Alignment](http://arxiv.org/abs/2605.09045v2) — 相关度 0.70：对agentic框架进行形式化验证，为代码agent的安全性提供可迁移的验证方法论。
- [Evaluating LLM-Generated Code: A Benchmark and Developer Study](http://arxiv.org/abs/2605.09059v2) — 相关度 0.70：提出多层级代码生成评估方法与开发者调研，弥补正确性之外的代码质量评测空缺。
- [ParityFuzz: Finding Inconsistencies across Solidity Compilers via Fine-Grained Mutation and Differential Analysis](http://arxiv.org/abs/2605.09051v1) — 相关度 0.70：面向Solidity编译器的差分模糊测试与交叉编译器不一致检测，属于软件测试与程序分析方法。
- [Detect, Localize, and Explain: Interactive Hierarchical Log Anomaly Analytics with LLM Augmentation](http://arxiv.org/abs/2605.09222v1) — 相关度 0.60：层次化日志异常检测结合LLM解释，对agent调试与可观测性有参考价值。
