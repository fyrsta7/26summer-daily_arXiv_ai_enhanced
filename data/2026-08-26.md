...
### [88] [Fairness Hazard Analysis for Socio-Technical Processes: A Multiple-Case Study in Bias-sensitive Organisational Settings](https://arxiv.org/abs/2608.22978)
...
TL;DR: 提出公平性危害分析（FHA）方法，用于在需求工程中系统识别和缓解社会技术流程中的公平性危害，并通过概念验证和多案例研究验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 公平性在社会技术流程中日益重要，但缺乏系统性方法在需求工程阶段识别公平性危害并推导缓解措施，导致公平性问题可能被忽视并加剧系统性偏见。

Method: 引入公平性危害分析（FHA）方法论，通过两个焦点小组进行概念验证，并在两个组织中开展定性多案例研究评估其实际适用性。

Result: 概念验证显示结构化方法的益处，并建议引入迭代对话式反思；多案例研究中，从业者确认了公平性危害的相关性（覆盖高达27%的流程元素）和大多数缓解措施的适当性，但指出情境因素可能阻碍实施，同时识别了可转移的缓解模式如独立审查和集体决策。

Conclusion: FHA是一种结构化且经验验证的方法论，能在需求工程中整合公平性考量，预防社会技术流程中的系统性偏见，并提供了可迁移的缓解模式。

中文摘要：公平性日益被认可是社会技术流程中的一项首要要求，其中人类行为者、软件系统和AI技术之间的交互可能导致决策工作流程中的不公平结果。如果不去解决，公平性危害可能会累积并加剧系统性偏见，凸显了主动设计公平性的必要性。尽管人们对公平性感知系统越来越感兴趣，但在社会技术流程中系统性地识别公平性危害并推导需求级缓解措施的方法仍然有限。为了在需求工程（RE）期间支持公平性设计，公平性危害分析（FHA）被引入作为一种系统性识别、分析和缓解公平性危害的方法。FHA首先通过两个焦点小组进行概念验证评估。然后，一项涉及两个组织的定性多案例研究检验了其在实际环境中的适用性。概念验证验证凸显了该方法结构化性质带来的好处，并建议需要与领域专家进行迭代的、对话式的反思。在应用FHA的多案例研究中，参与的从业者对其结果印象深刻，并确认了所识别的公平性危害的相关性（覆盖高达27%的流程元素），以及大多数拟议缓解措施的适当性，同时指出情境因素可能阻碍其实施。评估还强调了缓解模式，如独立审查和集体决策，这些可以转移到不同的组织。本文贡献了一种结构化且经验验证的方法论，用于在RE中整合公平性考量并预防社会技术流程中的系统性偏见。
...
### [89] [ARGUS: MCP-Grounded Root Cause Analysis for Kubernetes Incidents](https://arxiv.org/abs/2608.23084)
...
### [90] [A Multi-Viewpoint Modeling Framework for Digital Twin Integration and Reuse with LLM-Assisted Compatibility Analysis](https://arxiv.org/abs/2608.23115)
...
### [92] [An AI-Assisted Migration Framework for Transforming Legacy Scientific Applications into Reusable Cloud-Based Workflows](https://arxiv.org/abs/2608.23146)
...
### [93] [LLMCrater: Lifecycle-Aware FAIR Metadata Generation using Large Language Models](https://arxiv.org/abs/2608.23158)
...
### [95] [An Empirical Study of the TianoCore Community](https://arxiv.org/abs/2608.23280)
...
TL;DR: 通过对TianoCore社区利益相关者的调查和访谈，识别了UEFI固件开发中安全与维护实践的差距，并提出了改进方向。


<details>
  <summary>Details</summary>
Motivation: UEFI固件是系统安全的基础，但TianoCore社区中的开发实践可能存在安全与维护方面的不足，需要系统性地识别问题并寻找改进机会。

Method: 对独立固件供应商、OEM、安全专家、开发者和学术研究者等利益相关者进行了问卷调查和有限的访谈研究。

Result: 发现了当前固件开发中的关键差距，包括安全实践不足、内存安全技术采用有限以及自动化程度低等问题。

Conclusion: 通过改进安全实践、采用内存安全技术和增加自动化，可以加强TianoCore生态系统中UEFI固件的维护与安全性。

中文摘要：我们调查了TianoCore社区中利益相关者所采用的软件安全与维护实践，并识别了改进固件开发工作流程的机会。我们开展了一项调查和一项有限的访谈研究，参与者包括独立固件供应商、原始设备制造商、安全专家、固件开发人员以及学术研究人员。该开源开发社区维护着UEFI固件核心的参考实现。我们强调了TianoCore生态系统中当前固件开发状态的重要差距，并确定了在改进安全实践、更广泛地采用内存安全技术以及增加手动流程自动化方面能够加强UEFI固件维护和安全性的关键领域。

Original abstract: We investigate the software security and maintenance practices adopted by stakeholders in the TianoCore community and identify opportunities to improve firmware development workflows. We conduct a survey and a limited interview study with participants representing independent firmware vendors, original equipment manufacturers, security experts, firmware developers, and academic researchers. This open-source development community maintains a reference implementation for the core of the UEFI firmware. We highlight important gaps in the current state of firmware development within the TianoCore ecosystem and identify key areas in which improved security practices, greater adoption of memory-safe technologies, and increased automation of manual processes could strengthen the maintenance and security of the UEFI firmware.

</details>


### [96] [From Natural Language Policies to Executable Obligations: A Verification Harness for Dependable In-Car LLM Agents](https://arxiv.org/abs/2608.23282)
...
### [97] [DPIAgent: Divide, Protocol, Isolate for Agentic Reproduction Test Generation](https://arxiv.org/abs/2608.23341)
...
### [98] [Formalizing and Automating Fine-Grained Move Refactorings Across Methods](https://arxiv.org/abs/2608.23377)
...
### [99] [An Interactive Agent for Requirement-Driven Candidate Sourcing](https://arxiv.org/abs/2608.23501)
...
### [100] [Architecture as Capability Equalizer for Coding Agents](https://arxiv.org/abs/2608.21747)
*Arquimedes Canedo*
...
### [101] [Learning from the Test: Self-Referential Differential Testing for Deep RL Agents](https://arxiv.org/abs/2608.22284)
...
- [AutoSaddler: Automatic Harness Optimization with Durable Updates from Agent Execution Traces](https://arxiv.org/abs/2608.23041) — 相关度 0.95：自动优化 agent harness，通过失败轨迹诊断和结构化补丁更新在 SWE-Bench 等任务上显著提升性能，直接相关。
- [Disagree to Explore, Agree to Commit: Routing-Guided Test-Time Scaling for Software Agents](https://arxiv.org/abs/2608.22191) — 相关度 0.95：利用 MoE 路由信息引导软件 agent 探索与选择，在 SWE-bench Verified 上验证，直接提升编码 agent 效果。
- [DPIAgent: Divide, Protocol, Isolate for Agentic Reproduction Test Generation](https://arxiv.org/abs/2608.23341) — 相关度 0.90：提出 Divide, Protocol, Isolate 结构的代理框架用于复现测试生成，在 SWT-Bench 上超越基线，对代码代理的测试生成直接相关。
- [Evaluating Inference-Time Defenses Against Package Hallucination in LLM-Generated Code](https://arxiv.org/abs/2608.22652) — 相关度 0.90：系统评估 LLM 代码生成中包幻觉的推理时防御，并提出评测基准与指标，对代码 agent 安全性直接相关。
- [Repo2Skill-Evo: Repository Skills Go Stale in Silence](https://arxiv.org/abs/2608.21964) — 相关度 0.90：研究仓库技能随版本更新失效的问题并提出维护基准，直接面向仓库级编码 agent 的技能维护与评测。
- [Architecture as Capability Equalizer for Coding Agents](https://arxiv.org/abs/2608.21747) — 相关度 0.88：系统比较不同架构说明格式对编码 agent 生成质量的影响，揭示能力均衡器效应，对 coding agent 设计有直接指导。
- [Prime Agent: A Self-Improving RLM Harness](https://arxiv.org/abs/2608.23552) — 相关度 0.88：面向长程 coding-agent 工作流的开源 harness，提供持久 REPL、技能记忆和子 agent 协调，工程化迁移价值明确。
- [Execution-Anchored Hallucination Calibration Reranking for Verilog Code Generation](https://arxiv.org/abs/2608.22938) — 相关度 0.85：提出执行锚定的幻觉校准重排框架 EAHC，结合执行与推理信号提升 Verilog 代码生成选择，对代码生成验证有直接价值。
- [ARGUS: MCP-Grounded Root Cause Analysis for Kubernetes Incidents](https://arxiv.org/abs/2608.23084) — 相关度 0.70：提出 MCP 驱动的 Kubernetes 事故根因分析助手 ARGUS，在真实故障注入中验证，对运维 agent 和工具反馈机制有迁移价值。
- [From Metrics to Improvement: A Lifecycle-Aware LLM Feedback Framework for Research Software Quality](https://arxiv.org/abs/2608.23118) — 相关度 0.70：提出生命周期感知的 LLM 反馈框架，基于质量指标迭代优化研究软件，对代码质量改进和 agent 反馈循环有参考价值。
- [From Natural Language Policies to Executable Obligations: A Verification Harness for Dependable In-Car LLM Agents](https://arxiv.org/abs/2608.23282) — 相关度 0.70：提出将自然语言策略编译为可执行规则的验证 harness，用于车内 LLM 代理的可依赖验证，其验证与修订循环可迁移到代码代理。
- [An Interactive Agent for Requirement-Driven Candidate Sourcing](https://arxiv.org/abs/2608.23501) — 相关度 0.60：交互式需求采集、双阶段提交协议和终止守卫等通用 agent 工作流机制可迁移到编码 agent 的需求澄清与验证流程。
- [Concepts for Securing Agentic AI Coding and the Terok Environment](https://arxiv.org/abs/2608.22930) — 相关度 0.60：针对 Agentic AI 编码的安全风险与隔离环境设计，对构建安全可靠的编码 agent 有参考价值。
- [TianoForge: An Automated Bug Triage Approach for the TianoCore UEFI Firmware Development Community](https://arxiv.org/abs/2608.23259) — 相关度 0.60：提出基于 LLM 的 bug 分类方法 TianoForge，减少分类时间，对软件工程代理的自动化维护有参考价值。
- [PatchWrite: One Line, Not One Section -- Compile-Gated, Validity-Preserving Editing for AI-Drafted Manuscripts](https://arxiv.org/abs/2608.23001) — 相关度 0.55：编译门控与证据锁定的编辑提交机制可迁移到代码 agent 的局部修改与验证流程。
- [An AI-Assisted Migration Framework for Transforming Legacy Scientific Applications into Reusable Cloud-Based Workflows](https://arxiv.org/abs/2608.23146) — 相关度 0.50：AI 辅助迁移遗留科学应用为云工作流，虽涉及 LLM 代码生成，但属垂直应用，迁移价值有限。
- [Formalizing and Automating Fine-Grained Move Refactorings Across Methods](https://arxiv.org/abs/2608.23377) — 相关度 0.50：形式化并自动化跨方法的 Move 重构，提供静态检查前提，可作为代码代理重构工具的基础。
