...
- [SWE-Marathon: Can Agents Autonomously Complete Ultra-Long-Horizon Software Work?](http://arxiv.org/abs/2606.07682v1) — 相关度 0.95：超长时程软件任务基准，揭示 agent 自我验证与提前终止问题，对 coding agent 评测极具价值。
- [Contract2Tool: Learning Preconditions and Effects for Reliable Tool-Augmented LLM Agents](http://arxiv.org/abs/2606.07904v1) — 相关度 0.90：从元数据/文档/轨迹学习工具契约并做因果工具过滤，直接提升tool-augmented agent的可靠性与效率。
- [SWE-Explore: Benchmarking How Coding Agents Explore Repositories](http://arxiv.org/abs/2606.07297v1) — 相关度 0.90：SWE-Explore 基准专门评测 coding agent 的仓库探索能力，行级覆盖与排序指标直接关联下游修复，核心相关。
- [Socratic-SWE: Self-Evolving Coding Agents via Trace-Derived Agent Skills](http://arxiv.org/abs/2606.07412v1) — 相关度 0.90：Socratic-SWE 从历史轨迹提炼技能并自进化生成新任务，闭环训练显著提升 SWE-bench 性能，强迁移价值。
- [Chiseling Out Efficiency: Structured Skeleton Supervision for Efficient Code Generation](http://arxiv.org/abs/2606.06821v1) — 相关度 0.86：显式提取效率骨架并以多任务学习提升 LLM 生成代码的运行效率，直接对应程序性能优化。
- [SkelDPO: A Skeleton-Guided Direct Preference Optimization Framework for Efficient Code Generation](http://arxiv.org/abs/2606.06826v1) — 相关度 0.86：骨架引导的直接偏好优化提升代码执行效率，与传统 correctness/efficiency 对齐形成互补。
- [Declarative Skills for AI Agents in Knowledge-Grounded Tool-Use Workflows](http://arxiv.org/abs/2606.06923v1) — 相关度 0.80：对比声明式与命令式 agent 编排，声明式技能文件对工具调用 agent 的设计有直接借鉴意义。
- [Jas: AI-Paired Engineering as a Revival of N-Version Programming](http://arxiv.org/abs/2606.07828v1) — 相关度 0.80：用精确规范与差分测试支撑 AI 多语言移植，验证了 AI-paired engineering 的可靠性工作流，对代码生成 agent 有启发。
- [MalSkillBench: A Runtime-Verified Benchmark of Malicious Agent Skills](http://arxiv.org/abs/2606.07131v3) — 相关度 0.80：首个运行时验证的恶意 skill 基准，覆盖提示注入与代码注入，对 coding agent 供应链安全评测有直接价值。
- [QBugLM: An Agentic Benchmarking Framework for LLM-based Quantum Software Debugging](http://arxiv.org/abs/2606.07314v1) — 相关度 0.80：面向量子程序调试的 agentic 框架与基准，迭代反馈显著提升修复成功率，对调试类代码 agent 有参考价值。
- [Agentic Very Much! Adoption of Coding Agent in New GitHub Projects](http://arxiv.org/abs/2606.07448v1) — 相关度 0.70：实证显示新 GitHub 项目中 coding agent 采用率翻倍且强度更高，为 agent 影响力评估提供数据。
- [Empirical Study on the Characteristics and Evolution of AI-usage in GitHub Repositories: Evidence from Code Comments](http://arxiv.org/abs/2606.06843v1) — 相关度 0.70：实证分析 3.5 万条引用 AI 的代码注释，揭示开发者真实使用模式与代码演化，对理解 coding agent 落地有参考价值。
- [From Custom Logic to APIs: Understanding and Recommending API Replacement Refactorings](http://arxiv.org/abs/2606.06912v1) — 相关度 0.70：提出混合静态模式匹配与语义推理的 API 替换重构推荐框架，可迁移至代码 agent 的自动化重构能力。
- [On the Shoulders of Giants: Empowering Automated Smart Contract Auditing via the GiAnt Corpus](http://arxiv.org/abs/2606.07363v1) — 相关度 0.70：大规模智能合约审计数据集，涵盖漏洞检测与 gas 优化任务，可用于评测安全与性能 agent。
- [Porting Declarative UI to HarmonyOS: A Heuristic-guided LLM Approach](http://arxiv.org/abs/2606.07085v1) — 相关度 0.70：用启发式骨架引导 LLM 翻译加规则修复，在 UI 迁移任务上大幅提升可编译率，此受限生成策略可迁移到代码 agent。
- [LLM Agent-Assisted Reverse Engineering with Quantitative Readability Metrics](http://arxiv.org/abs/2606.06838v1) — 相关度 0.68：LLM agent 辅助反编译代码可读性改进，结合定量指标与正确性验证，可迁移到 agent 驱动的代码重构。
- [Cherry-pick Override: Unsafe Directional Commitment in LLM Judges under Mixed Evidence](http://arxiv.org/abs/2606.07834v1) — 相关度 0.65：研究LLM裁判在混合证据下的定向承诺风险并设计外部承诺控制层，可迁移到代码agent的验证与评测可靠性。
- [Are We Lost in the Woods? Detecting Silent Semantic Faults for Random Forest Classifiers with Data-informed Static Analysis](http://arxiv.org/abs/2606.07709v1) — 相关度 0.60：用数据感知静态分析检测 ML 脚本中的静默语义错误，可辅助验证 agent 生成的 ML 代码质量。
- [MOLOT System Card: Malicious Operational Logic Observation Transformer](http://arxiv.org/abs/2606.07792v1) — 相关度 0.60：静态行为序列建模检测恶意包并发布基准，可迁移到代码安全 agent 的检测与评测。
- [Review the Code, Not the Story: A Vision and Protocol for Code-First Peer Review](http://arxiv.org/abs/2606.07683v1) — 相关度 0.60：面向可执行证据的 code-first 评审协议，AI 构建环境并审计代码路径，对 agent 评测与代码审查有参考。
- [The Windows IOCTL Census: A Corpus-Scale, Multi-Architecture Database of the Driver Control-Code Surface](http://arxiv.org/abs/2606.07732v1) — 相关度 0.60：大规模 Windows 驱动 IOCTL 数据库，作为程序分析/安全 agent 测试语料有参考价值。
- [DD-GEPA: Prompt Optimization for Dialogue Disentanglement Focusing on Task Instruction and Utterance Representation](http://arxiv.org/abs/2606.07894v1) — 相关度 0.55：用GEPA自动优化prompt解决对话解缠任务，优化方法通用但应用为NLP任务，迁移价值有限。
