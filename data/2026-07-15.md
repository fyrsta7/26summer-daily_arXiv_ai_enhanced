...
DOI: [10.1007/978-3-032-18474-0_1](https://doi.org/10.1007/978-3-032-18474-0_1)

Author affiliations: Shelley M Cazares: Google (United States)
...
Journal reference: FSE Companion '26: Companion Proceedings of the 34th ACM Joint European Software Engineering Conference and Symposium on the Foundations of Software Engineering, Montreal, QC, Canada, 2026
...
- [AutoTrace: From Patches to Triggers via Agentic Interprocedural Exploration](https://arxiv.org/abs/2607.12058) — 相关度 0.95：agentic跨过程漏洞触发定位，融合LLM决策与可验证证据，对代码agent安全分析有直接价值。
- [Code-MUE: Measuring Code LLMs' Uncertainty through Execution-based Semantic Interaction Graphs](https://arxiv.org/abs/2607.12273) — 相关度 0.95：基于执行语义交互图的代码LLM不确定性估计，黑盒可用，对agent选择性预测有直接价值。
- [Do AI Agents Know When a Task Is Simple? Toward Complexity-Aware Reasoning and Execution](https://arxiv.org/abs/2607.13034) — 相关度 0.95：提出任务感知执行范围估计与 ACRR，并给出低成本高成功率 agent 策略和基准，直接相关。
- [Multi-Perspective Agentic Program Repair via Code Property Graphs and Temporal Execution Graphs](https://arxiv.org/abs/2607.12605) — 相关度 0.95：基于 CPG/TEG 的多视角 agentic 程序修复，直接相关且评测于 Defects4J。
- [Beyond Test Presence: Assessing the Quality and Robustness of Agent-Generated Tests in Open-Source Projects](https://arxiv.org/abs/2607.12068) — 相关度 0.90：大规模评估agent生成测试的断言强度、边界覆盖与脆弱性，揭示stealth debt，对agent测试评估有直接借鉴。
- [Skills That Don't Exist: A Large-Scale Study of Hallucinated Skill Recommendation in LLM Agents](https://arxiv.org/abs/2607.12340) — 相关度 0.90：测量LLM agent技能名称幻觉并验证供应链攻击风险，提出检索缓解与结构性防御，对agent工具生态安全重要。
- [Line-Anchored Feedback Cuts Token Costs and Improves Correctness in AI Code Editing](https://arxiv.org/abs/2607.12713) — 相关度 0.85：研究反馈格式对 AI 代码编辑的正确性和 token 成本影响，可直接转化为 agent 输入优化。
- [Predicting Acceptance and Review Effort in Human and Agent Pull Requests](https://arxiv.org/abs/2607.12057) — 相关度 0.85：从提交时特征预测agent/人类PR接受度与评审工作量，可用于triage与评估。
- [Form, Not Content? A Preregistered, Placebo-Controlled Evaluation of Learned Error-Conditioned Self-Repair Through Prompts and Weights in Frozen Small Code Models](https://arxiv.org/abs/2607.12962) — 相关度 0.80：用安慰剂对照评估代码模型自修复中的错误条件反馈，方法论严谨，对修复 agent 有直接参考。
- [Taming the Drift: Context-aware Repair of Dockerfile Drift during Software Evolution](https://arxiv.org/abs/2607.12541) — 相关度 0.80：上下文感知的 Dockerfile drift 修复框架，结合图分析和 LLM 补丁生成，可直接用于代码修复 agent。
- [XScientist: A Git-Like Research Protocol for Long-Running Autonomous Scientific Discovery](https://arxiv.org/abs/2607.12301) — 相关度 0.80：提出git式长时运行自主研究协议，包含审计、门控与复现，可迁移到长程代码agent工作流。
- [Agent-Safety Evaluations as Load-Bearing Evidence: A Vendor-Neutral, Cross-Harness Reconstructability Metric](https://arxiv.org/abs/2607.12469) — 相关度 0.75：提出 agent 安全评测的可重建性度量，可迁移到 coding agent 评测体系。
- [Software Supply Chains are Dead: Use-Case-Oriented Regeneration](https://arxiv.org/abs/2607.13021) — 相关度 0.72：用 agentic 合成替代外部依赖并验证局部代码生成可行性，对代码 agent 的按需生成与供应链安全有直接价值。
- [Understanding before Naming! Enhancing LLM-based Method Name Prediction with Code Summarization](https://arxiv.org/abs/2607.12467) — 相关度 0.70：面向 LLM 的方法名预测，提出摘要+精炼策略和评估方法，可用于代码理解与生成 agent。
- [Deep4ge: DNN Training Trajectories for Fault Detection and Diagnosis](https://arxiv.org/abs/2607.12868) — 相关度 0.60：提供 DNN 训练故障检测数据集与方法，可支撑 ML 系统测试 agent 的研究。
- [Fault Injection in OpenAPI Specifications for Evaluating Black-Box Testing Effectiveness](https://arxiv.org/abs/2607.12101) — 相关度 0.60：对OpenAPI规范注入故障评估黑盒测试工具，提供规范缺陷分类与评测方法，可借鉴于API测试agent验证。
- [Hierarchical Fault Localization for Autonomous Driving Systems with Hypothesis Validation and Intent Analysis](https://arxiv.org/abs/2607.12598) — 相关度 0.60：面向 ADS 的分层故障定位框架，其假设验证和意图分析可迁移到代码级缺陷定位 agent。
- [Win by Silence: Deletion Non-Monotonicity, Autonomous Exploitation, and Typed-State Gating in LLM Plan Evaluation](https://arxiv.org/abs/2607.12986) — 相关度 0.60：揭示 LLM 计划评估中的删除非单调漏洞并设计门控机制，可迁移到 agent 评测与可靠性保障。
- [Detecting Rendering Bugs in Imperative Data Visualization Libraries via Equivalent Mutations](https://arxiv.org/abs/2607.12363) — 相关度 0.55：用等价突变检测渲染库bug，方法有通用性，但领域偏向可视化，对agent测试设计有参考。
- [TestMiner: Software Testing Analysis for GitHub Repositories](https://arxiv.org/abs/2607.12223) — 相关度 0.55：提供GitHub项目测试实践的多维概览工具，可用于评估agent测试习惯，但迁移价值有限。
