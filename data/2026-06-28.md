...
- [Stop Hand-Holding Your Coding Agent: Engineering the Loops that Replace Step-by-Step Prompting](http://arxiv.org/abs/2607.00038v1) — 相关度 0.95：系统定义 coding agent 的 loop specification（触发、目标、验证、停止、记忆），与 agent 自动化工作流高度直接相关。
- [RESOURCE2SKILL: Distilling Executable Agent Skills from Human-Created Multimodal Resources](http://arxiv.org/abs/2606.29538v3) — 相关度 0.90：将多模态人类资源蒸馏为可执行技能库并支持检索组合，直接增强软件代理的技能复用与在线获取。
- [A Multi-Dataset Benchmark for Evaluating LLM Agents in Microservice Failure Diagnosis](http://arxiv.org/abs/2606.29193v1) — 相关度 0.85：面向微服务故障诊断的LLM agent基准，提供推理过程评估，对运维与调试agent有直接价值。
- [Citation Discipline in Spec-Driven Development: A Cross-Model Empirical Study of Output Determinism and Automated Hallucination Detection in LLM-Generated Code](http://arxiv.org/abs/2606.30689v1) — 相关度 0.85：实验度量 spec-driven 代码生成的可复现性与幻觉检测，为智能体输出验证提供可迁移方法。
- [OASIF: An Efficient Obfuscation-Aware Self-Improving Framework for LLM-Based Assembly Code Instruction Following and Comprehension](http://arxiv.org/abs/2606.29155v1) — 相关度 0.80：混淆汇编代码理解的LLM自进化框架，涉及指令遵循与RL，对二进制分析agent有直接价值。
- [SAKE: Software Architectural Knowledge Evaluation Benchmark for Large Language Models](http://arxiv.org/abs/2606.29520v1) — 相关度 0.75：软件架构知识评估基准，提供LLM架构推理评测，对代码agent的规划与审查能力评估有参考。
- [Agent Security Meets Regulatory Reality -- A Practitioner Systematization of Autonomous-Agent Threats and Controls in Regulated Financial Systems](http://arxiv.org/abs/2606.29142v1) — 相关度 0.65：系统化自主agent威胁与控制，虽面向金融合规，但可迁移到代码agent的安全与可靠性设计。
- [Deforking the World of Code: A Project-Provenance Map that Recovers Cross-Forge Fork Families that Platform Graphs Cannot See](http://arxiv.org/abs/2606.29550v3) — 相关度 0.55：提供跨平台 fork 去重映射与数据集，可改善代码库度量与智能体评测中的数据膨胀和重复污染问题。
- [Connecting the Models: A Global Mega-model of MDE Projects on GitHub](http://arxiv.org/abs/2606.29606v1) — 相关度 0.50：构建 MDE 工程数据集与全局 mega-model，可作为代码仓库和模型工件分析的参考资源。
