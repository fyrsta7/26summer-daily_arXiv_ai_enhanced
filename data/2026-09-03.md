...
Selection reason: 提出端到端可靠评测pipeline，用清单+集成聚合提高LLM评判一致性并提供不确定性，适用于agent输出验证。
...
Selection reason: 面向云工程的agentic框架，明确覆盖长程工作流、图/循环工程与zero-trust harness，且包含验证与受限恢复机制
...
Selection reason: 审计MCP工具失败结果的可操作性，研究agent在失败后如何做恢复决策，可直接迁移到工具反馈与重试设计
...
Selection reason: 提出代码驱动的agentic GUI测试范式并发布带有标注bug的benchmark，直接命中agent测试与评测
...
Selection reason: 深入分析LLM judge易被游戏及commit-first防御的代价，对agent评测稳健性有重要迁移价值
...
Selection reason: 分离语义决策与语法编辑，用确定性pipeline实现LLM配置修复的fail-closed应用，提升agent修复可靠性
...
Selection reason: 系统化提出Spec-Driven Development与agent harness概念，为agentic SE提供方法论蓝图
...
Selection reason: 复现并扩展Python与Java上LLM代码修复的反馈有效性研究，为feedback-driven agent repair提供实证
...
Selection reason: 提出audit-first rollback语义保障部署时的审计/活性一致性，可迁移到agentic安全部署与验证机制
...
Selection reason: 系统评估35篇LLMSE技术是否被单提示新模型替代，为agent/工具方法研究投资与评测提供关键视角
...
Selection reason: 提出runtime-independent持久agent的连续性协议和不变式，可迁移到跨模型/跨harness的长程agent记忆与身份设计
...
Selection reason: WiseSpec将需求工程引入agent代码生成，通过结构化需求构建与执行评估迭代提高resolve率，直接相关
...
Selection reason: 用结构化记忆与多排列聚合替代listwise选择微调，减少token开销，可迁移到agent生成结果的候选选择
...
Selection reason: 用概率模型检验验证自回归模型的约束违反概率，其抽象与CEGAR思路可迁移到coding agent生成安全/正确性验证
...
Selection reason: 受控实验证明盲重新采样优于基于故障定位的修复，对code repair agent的设计和治疗意义重大
...
Selection reason: 用软件知识图谱+混合检索+执行修复接地LLM生成，显著提升成功率且成本增量小，证明显式软件知识对agent有效
...
Selection reason: 对代码生成中提示效果与技能迁移的系统评估，利用可执行评测和机制分析，对理解agent反馈价值有直接参考。
...
Selection reason: 用知识图谱补全和LLM重排序识别漏洞影响库，属于软件安全分析方法，可迁移到代码依赖风险评估场景。
...
Selection reason: 提出AI驱动连续自主重构的研究路线图，明确多目标优化、质量评估与agent集成问题，对软件工程agent设计有参考价值。
...
Selection reason: 提出SNC画像刻画SWE基准任务真实需求并分析agent轨迹，直接改进agentic基准测评方法论。
...
Selection reason: 构建评估LLM自主创建和进化agent harness的基准，关注模型能力与基础设施交付，对agent评估范式有迁移价值。
...
Selection reason: 系统审计漏洞分析数据的类别、质量与使用方式，对构建可靠的代码安全评测数据集有参考意义。
...
Selection reason: 提出关键token感知的自适应检索用于仓库级代码生成，显著提升生成一致性，可直接用于检索增强代码agent。
...
Selection reason: 利用轨迹信息进行SWE agent高效评测，融合过程与结果信号，对降低agent基准测试成本有直接价值。
