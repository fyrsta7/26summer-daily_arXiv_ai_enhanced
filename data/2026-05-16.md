...
- [Beyond Execution: Static-Analysis Rewards and Hint-Conditioned Diffusion RL for Code Generation](http://arxiv.org/abs/2605.17174v1) — 相关度 0.85：系统研究代码生成扩散模型 RL 的奖励设计与提示条件，验证静态检查奖励的有效性，对代码生成 agent 后训练有直接价值。
- [Reconsidering "Reconsidering Custom Memory Allocation"](http://arxiv.org/abs/2605.17119v1) — 相关度 0.85：重新评估自定义内存分配对执行性能的影响，包含现代平台基准与碎片/局部性方法，是 SemOpt 直接相关研究。
- [Task Abstention for Large Language Models in Code Generation](http://arxiv.org/abs/2605.17029v1) — 相关度 0.85：面向代码生成的 LLM 任务弃权机制，基于多假设检验与执行一致性提供可靠性保证，可迁移到 coding agent 的安全决策。
- [Stop Starving or Stuffing Me: Boosting Firmware Fuzzing Efficiency with On-demand Input Delivery](http://arxiv.org/abs/2605.16798v1) — 相关度 0.75：面向固件模糊测试的输入时序与数量优化，通过静态/动态分析识别输入处理路径，可迁移到 agent 测试与覆盖率提升。
- [The Range Shrinks, the Threat Remains: Re-evaluating LLM Package Hallucinations on the 2026 Frontier-Model Cohort](http://arxiv.org/abs/2605.17062v2) — 相关度 0.70：系统评估前沿模型包名幻觉，揭示模型无关的供应链攻击面，为代码 agent 依赖安全提供评测基准与风险数据。
