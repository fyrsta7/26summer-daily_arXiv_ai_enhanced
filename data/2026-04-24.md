...
- [From Natural Language to Verified Code: Toward AI Assisted Problem-to-Code Generation with Dafny-Based Formal Verification](http://arxiv.org/abs/2604.22601v1) — 相关度 0.90：面向自然语言到Dafny验证代码生成的prompt策略与自修复流程，直接相关代码生成与形式化验证agent。
- [How Do AI Agents Spend Your Money? Analyzing and Predicting Token Consumption in Agentic Coding Tasks](http://arxiv.org/abs/2604.22750v2) — 相关度 0.90：系统分析agentic coding任务的token消耗模式与可预测性，直接关乎coding agent成本优化。
- [RealBench: A Repo-Level Code Generation Benchmark Aligned with Real-World Software Development Practices](http://arxiv.org/abs/2604.22659v2) — 相关度 0.85：提出仓库级代码生成基准RealBench，贴近真实开发实践，直接用于评估coding agent代码生成能力。
- [RAG-Reflect: Agentic Retrieval-Augmented Generation with Reflections for Comment-Driven Code Maintenance on Stack Overflow](http://arxiv.org/abs/2604.22217v1) — 相关度 0.80：面向代码维护的agentic RAG框架，集成检索、推理与自反思，可迁移到代码注释驱动的自动编辑。
- [Operationalizing Reconstructive Authority: Runtime Construction, Dependency Resolution, and Execution Gating in Autonomous Agent Systems](http://arxiv.org/abs/2605.23935v1) — 相关度 0.75：提出运行时权威重构与执行门控模型，为自主代码agent提供了动态依赖解析和安全性保障机制。
- [R2Code: A Self-Reflective LLM Framework for Requirements-to-Code Traceability](http://arxiv.org/abs/2604.22432v1) — 相关度 0.75：需求到代码追踪的LLM框架，包含自反思验证与动态上下文检索，可增强代码agent的需求理解和链路一致性。
- [AutoINV: Automated Invariant Generation Framework for Formal Verification on High-Level Synthesis Designs](http://arxiv.org/abs/2604.22285v1) — 相关度 0.70：自动生成辅助断言加速HLS设计形式验证，其不变量生成与迭代选择机制可迁移到程序验证agent。
- [Inferring Equivalence Classes from Legacy Undocumented Embedded Binaries for ISO 26262-Compliant Testing](http://arxiv.org/abs/2604.22673v1) — 相关度 0.70：二进制级等价类推断结合符号执行，为无文档固件自动生成测试分区，方法可迁移到agent测试设计。
- [Evaluating LLM-Based Goal Extraction in Requirements Engineering: Prompting Strategies and Their Limitations](http://arxiv.org/abs/2604.22207v1) — 相关度 0.65：评估LLM在需求工程中的目标提取与反馈循环机制，其生成-批评工作流可迁移到软件agent的任务规划。
- [Test Design and Review Argumentation in AI-Assisted Test Generation](http://arxiv.org/abs/2604.22473v1) — 相关度 0.65：提出AI辅助测试生成中的论证结构化模板，可提高测试agent生成物件的可解释性与审查质量。
- [Train in Vain: Functionality-Preserving Poisoning to Prevent Unauthorized Use of Code Datasets](http://arxiv.org/abs/2604.22291v1) — 相关度 0.50：面向代码数据集的功能保持投毒方法，涉及编译性和静态分析规避，对代码agent数据安全有参考价值。
