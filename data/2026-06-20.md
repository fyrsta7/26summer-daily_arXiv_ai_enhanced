...
- [Revelio: Cost-Efficient Agentic Memory Safety Vulnerability Detection For Repository-Scale Codebases](http://arxiv.org/abs/2606.22263v1) — 相关度 0.95：agentic框架用可执行PoV和sanitizer验证内存安全漏洞，低成本高可信，直接面向代码安全agent。
- [Steer, Don't Solve: Training Small Critic Models for Large Code Agents](http://arxiv.org/abs/2606.21811v1) — 相关度 0.95：训练小规模critic模型在轨迹内引导代码agent，显著提升SWE-bench性能并降低成本，直接可迁移。
- [CodeTeam: An LLM-Powered Multi-Agent Framework for Repository-Level Code Generation](http://arxiv.org/abs/2606.22082v1) — 相关度 0.90：面向仓库级代码生成的多agent框架，规划/决策/实现分离并有依赖调度与QA迭代，直接相关。
- [Holmes: Multimodal Agentic Diagnosis for Mixed-Language Mobile Crashes at Industrial Scale](http://arxiv.org/abs/2606.21963v1) — 相关度 0.90：工业级多agent系统用多模态运行信号做移动崩溃根因分析，检索-探索-推理架构可迁移到代码诊断。
- [A11YRepair: Bridging Web Accessibility Barriers via Knowledge-Enhanced Divide-and-Conquer Repair](http://arxiv.org/abs/2606.21926v1) — 相关度 0.80：LLM驱动的Web可访问性缺陷自动修复，用分治工作流和领域知识，对程序修复和多缺陷协调有迁移价值。
- [CNnotator: LLM-Guided Memory Safety Annotation Synthesis](http://arxiv.org/abs/2606.21822v1) — 相关度 0.75：用LLM自动生成并验证内存安全CN规约，结合测试/验证工具，对程序分析和agent验证有参考价值。
- [TraceView: Interactive Visualization of Agentic Program Repair Trajectories](http://arxiv.org/abs/2606.22110v1) — 相关度 0.75：提供APR agent轨迹的可视化标注与诊断工具，帮助理解agent行为，对调试和分析agent有价值。
- [CFAgentBench: A Reproducible Environment and Benchmark for Autonomous Construction-Finance Agents](http://arxiv.org/abs/2606.22000v1) — 相关度 0.70：面向领域agent的可复现环境与benchmark，含功能正确性评估和风险护栏设计，评测方法论可迁移。
- [Natural Language-Focused Software Engineering via Code-Documentation Equivalence](http://arxiv.org/abs/2606.22247v1) — 相关度 0.70：提出代码-文档等价性并自动生成等价文档，实验表明能提升LLM代码理解与编辑效果，可增强agent上下文。
- [GitReq: A Gold Standard Dataset for Software Quality Requirements](http://arxiv.org/abs/2606.21810v1) — 相关度 0.65：构建软件质量需求（性能、安全等）标注数据集，为代码质量分析与需求分类提供可用benchmark。
- [AgentRiskBOM: A Risk-Scoping Security Bill of Materials for Agentic AI Systems](http://arxiv.org/abs/2606.21877v1) — 相关度 0.60：提出agent系统安全BOM，用结构化权限与风险评估增强agent透明性，部分机制可迁移到agent可靠性工程。
