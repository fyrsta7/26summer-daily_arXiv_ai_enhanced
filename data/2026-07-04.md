...
- [Don't Blame the Large Language Model: How Scaffolding Evolution Shapes Coding Agent Quality](http://arxiv.org/abs/2607.03691v1) — 相关度 0.95：系统研究coding agent脚手架演化对质量的影响，揭示agent质量波动的关键因素，对agent工程化有直接指导。
- [Semantic-aware and Self-improving Program Reduction via Agentic Large Language Models](http://arxiv.org/abs/2607.03766v1) — 相关度 0.95：利用agentic LLM进行语义感知且自改进的程序化简，将化简转化为推理任务，显著优于现有方法。
- [Kaizen: Metamorphic Fuzzing and Differential Testing for LLM-Translated HPC Applications](http://arxiv.org/abs/2607.04058v1) — 相关度 0.90：对 LLM 翻译的 HPC 代码进行形变模糊与差分测试，解决正确性验证关键问题。
- [Refused in Chat, Written in Code: Workflow-Level Jailbreak Construction in IDE Coding Agents](http://arxiv.org/abs/2607.03968v2) — 相关度 0.90：IDE 编码智能体的多轮工作流级越狱构造，对智能体安全评测有重要意义。
- [The "I Don't Know" Filter: Enhancing Agentic Reliability in Function Calling](http://arxiv.org/abs/2607.04034v1) — 相关度 0.90：提出量化 LLM 不确定性并过滤错误函数调用的机制，增强智能体可靠性。
- [AutoCedar: An Agentic Framework for Verifier-Guided Access Control Policy Synthesis](http://arxiv.org/abs/2607.03656v1) — 相关度 0.85：利用验证器反馈引导LLM合成访问控制策略，体现了agentic验证闭环，可迁移到代码生成与修复。
- [Graph-Aware Fuzzing for Graph Database Management Systems](http://arxiv.org/abs/2607.03741v1) — 相关度 0.85：面向图数据库的上下文感知模糊测试，结合LLM生成和状态反馈，属于高效的自动化测试方法。
- [Neuro-Symbolic Reasoning for Vulnerability Detection](http://arxiv.org/abs/2607.03963v1) — 相关度 0.85：神经符号结合的漏洞检测，LLM 与 Lean 验证器协作，直接适用于代码分析。
- [Inferring the Shape of Data Frames in R Programs using Abstract Interpretation](http://arxiv.org/abs/2607.03889v1) — 相关度 0.80：用抽象解释静态推断R程序中数据框形状，属于通用程序分析技术，可辅助代码理解与检测潜在错误。
- [SkillFab: An Agent-Native Skill Production Platform](http://arxiv.org/abs/2607.03780v1) — 相关度 0.80：提出agent原生技能生产平台，支持技能需求发现、审查与发布，促进可复用技能管理。
- [LogNLQ: Natural-Language Log Querying with Parser-Induced and Semantically Grounded Schemas](http://arxiv.org/abs/2607.03884v1) — 相关度 0.70：将自然语言日志查询转化为SQL生成，提供schema构建与执行验证，有助于日志分析agent。
- [Why3-py: A Tool for Formal Verification of Hypothesis Testing and Meta-Analysis in Python](http://arxiv.org/abs/2607.03951v1) — 相关度 0.60：将 Python 程序转换为 WhyML 进行形式化验证，对程序分析验证有参考价值。
