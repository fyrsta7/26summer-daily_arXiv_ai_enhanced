...
- [Code-MUE: Measuring Code LLMs' Uncertainty through Execution-based Semantic Interaction Graphs](http://arxiv.org/abs/2607.12273v1) — 相关度 0.95：基于执行语义交互图的代码LLM不确定性估计，黑盒可用，对agent选择性预测有直接价值。
- [Do AI Agents Know When a Task Is Simple? Toward Complexity-Aware Reasoning and Execution](http://arxiv.org/abs/2607.13034v1) — 相关度 0.95：提出任务感知执行范围估计与 ACRR，并给出低成本高成功率 agent 策略和基准，直接相关。
- [Multi-Perspective Agentic Program Repair via Code Property Graphs and Temporal Execution Graphs](http://arxiv.org/abs/2607.12605v1) — 相关度 0.95：基于 CPG/TEG 的多视角 agentic 程序修复，直接相关且评测于 Defects4J。
- [Harness Handbook: Making Evolving Agent Harnesses Readable,Navigable, and Editable](http://arxiv.org/abs/2607.13285v1) — 相关度 0.90：为 agent harness 提供行为定位与渐进式披露机制，直接改善 coding agent 的编辑规划与代码定位。
- [Skills That Don't Exist: A Large-Scale Study of Hallucinated Skill Recommendation in LLM Agents](http://arxiv.org/abs/2607.12340v1) — 相关度 0.90：测量LLM agent技能名称幻觉并验证供应链攻击风险，提出检索缓解与结构性防御，对agent工具生态安全重要。
- [Line-Anchored Feedback Cuts Token Costs and Improves Correctness in AI Code Editing](http://arxiv.org/abs/2607.12713v1) — 相关度 0.85：研究反馈格式对 AI 代码编辑的正确性和 token 成本影响，可直接转化为 agent 输入优化。
- [Bulkhead: Automated Semantic Detection and Remediation of Container Escape Vulnerabilities](http://arxiv.org/abs/2607.12723v1) — 相关度 0.80：集成多 agent 与形式方法的容器逃逸漏洞检测修复框架，对安全代码 agent 有迁移价值。
- [Form, Not Content? A Preregistered, Placebo-Controlled Evaluation of Learned Error-Conditioned Self-Repair Through Prompts and Weights in Frozen Small Code Models](http://arxiv.org/abs/2607.12962v1) — 相关度 0.80：用安慰剂对照评估代码模型自修复中的错误条件反馈，方法论严谨，对修复 agent 有直接参考。
- [Microflow: Microarchitectural Causal Observability for Deep Cross-Layer Analysis and Optimization](http://arxiv.org/abs/2607.13184v1) — 相关度 0.80：将微架构因果可观测性引入性能分析，支撑跨层根因定位与优化，符合 profiling 主题。
- [Taming the Drift: Context-aware Repair of Dockerfile Drift during Software Evolution](http://arxiv.org/abs/2607.12541v1) — 相关度 0.80：上下文感知的 Dockerfile drift 修复框架，结合图分析和 LLM 补丁生成，可直接用于代码修复 agent。
- [XScientist: A Git-Like Research Protocol for Long-Running Autonomous Scientific Discovery](http://arxiv.org/abs/2607.12301v1) — 相关度 0.80：提出git式长时运行自主研究协议，包含审计、门控与复现，可迁移到长程代码agent工作流。
- [Beyond Object Validation: Relational Conformance in Multi-Artifact Agent Releases](http://arxiv.org/abs/2607.14155v1) — 相关度 0.78：面向多工件 agent 发布的关系一致性校验，以发布图为 agent 输出验证提供可迁移机制。
- [Faithful Autoformalization of Natural Language Assertions](http://arxiv.org/abs/2607.13303v1) — 相关度 0.78：基于一致性分数和测试验证的断言自动形式化，提升 LLM 生成可执行规格的可靠性，可直接用于测试 agent。
- [Agent-Safety Evaluations as Load-Bearing Evidence: A Vendor-Neutral, Cross-Harness Reconstructability Metric](http://arxiv.org/abs/2607.12469v1) — 相关度 0.75：提出 agent 安全评测的可重建性度量，可迁移到 coding agent 评测体系。
- [SemaDiff: Identifying Semantic-Changing Commits with Generated Code and Tests](http://arxiv.org/abs/2607.13111v1) — 相关度 0.75：通过生成测试和对比执行识别语义变更提交，对 agent 驱动的测试与变更分析有直接价值。
- [From Human-Centric to Agentic Code Review: The Impact of Different Generations of Generative AI Technology on Review Quality](http://arxiv.org/abs/2607.13196v1) — 相关度 0.72：百万级 PR 研究人类/LLM/agent 评审协作模式，为 agentic code review 提供重要实证。
- [Natural-Language to SysMLv2 Translation via Conformance-Driven Iterative Refinement](http://arxiv.org/abs/2607.14162v1) — 相关度 0.72：用生产级 conformance checker 驱动 generate-check-repair 循环，机制可迁移到代码生成 agent 的验证与修复。
- [Software Supply Chains are Dead: Use-Case-Oriented Regeneration](http://arxiv.org/abs/2607.13021v1) — 相关度 0.72：用 agentic 合成替代外部依赖并验证局部代码生成可行性，对代码 agent 的按需生成与供应链安全有直接价值。
- [Mystra: Declarative Dynamic Taint Analysis via Shadow Virtual Machine](http://arxiv.org/abs/2607.12308v2) — 相关度 0.70：声明式动态污点分析，支持语言模型友好规则合成，可移植到agent驱动的污点追踪与安全验证。
- [Open-Source Intelligence for Code Provenance and the Security Patterns that Separate Human and Large-Language-Model Implementations of Common Programming Tasks](http://arxiv.org/abs/2607.12524v1) — 相关度 0.70：利用 OSINT 和分类器区分人机代码来源并分析安全模式，对代码安全分析与 agent 有参考价值。
- [Understanding before Naming! Enhancing LLM-based Method Name Prediction with Code Summarization](http://arxiv.org/abs/2607.12467v1) — 相关度 0.70：面向 LLM 的方法名预测，提出摘要+精炼策略和评估方法，可用于代码理解与生成 agent。
- [Why Not Fix It Once and for All? An Empirical Study of Multiple Patches for Vulnerability Fixes in Open-Source Software](http://arxiv.org/abs/2607.13206v1) — 相关度 0.68：多补丁漏洞修复的根因分类与完整修复验证方法，对 agent 修复可靠性与验证有参考。
- [Deep4ge: DNN Training Trajectories for Fault Detection and Diagnosis](http://arxiv.org/abs/2607.12868v1) — 相关度 0.60：提供 DNN 训练故障检测数据集与方法，可支撑 ML 系统测试 agent 的研究。
- [Foundational Constraint Solving for Expressive Refinement Typing](http://arxiv.org/abs/2607.12226v1) — 相关度 0.60：提供可证明的CHC求解后端，可增强agent或验证器的正确性保证，但应用面较窄。
- [Hierarchical Fault Localization for Autonomous Driving Systems with Hypothesis Validation and Intent Analysis](http://arxiv.org/abs/2607.12598v1) — 相关度 0.60：面向 ADS 的分层故障定位框架，其假设验证和意图分析可迁移到代码级缺陷定位 agent。
- [MetaInfer: A Knowledge Only LLM Inference Engine Generator SKILL Toolbox](http://arxiv.org/abs/2607.12875v1) — 相关度 0.60：用 LLM 多 agent 自动生成定制推理引擎，涉及编译器/运行时优化和代码生成，有参考价值。
- [Win by Silence: Deletion Non-Monotonicity, Autonomous Exploitation, and Typed-State Gating in LLM Plan Evaluation](http://arxiv.org/abs/2607.12986v1) — 相关度 0.60：揭示 LLM 计划评估中的删除非单调漏洞并设计门控机制，可迁移到 agent 评测与可靠性保障。
- [Can LLMs Learn and Apply Multi-Level Modelling Semantics? A First Empirical Study](http://arxiv.org/abs/2607.13257v1) — 相关度 0.55：LLM 生成多级建模语义的实证与提示策略对比，对生成结构化工件的 agent 有参考价值。
- [Design-System-Aware Development with AI: Evaluating Productivity and Design Consistency](http://arxiv.org/abs/2607.13156v1) — 相关度 0.55：DS-aware AI 前端开发生产力实验，对前端代码 agent 效果评测有参考价值。
- [Detecting Rendering Bugs in Imperative Data Visualization Libraries via Equivalent Mutations](http://arxiv.org/abs/2607.12363v3) — 相关度 0.55：用等价突变检测渲染库bug，方法有通用性，但领域偏向可视化，对agent测试设计有参考。
- [Evidence-Grounded Verified Agentic Reasoning: A Path Toward Eliminating LLM Hallucination in Empirical Inference via Tool-Attested Kernel Proofs](http://arxiv.org/abs/2607.12650v1) — 相关度 0.55：基于 Lean 内核验证的 agent 推理可靠性机制，可迁移到需要可审计证据的代码 agent。
