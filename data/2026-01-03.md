...
TL;DR: 该研究提出了一种基于Depth Anything V2架构的单目深度估计方法，通过DV-LORA高效适应医疗领域，在具有挑战性的内窥镜手术环境中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统自监督单目深度估计方法在具有高反射、充满液体的内窥镜手术环境中表现脆弱，特别是在薄手术器械和透明表面边界处容易崩溃。现有方法通常依赖带噪声真实世界伪标签训练的基础模型，难以准确捕捉手术场景的几何细节。

Method: 1. 利用Depth Anything V2架构的高保真合成先验，该架构能固有地捕捉薄结构的精确几何细节；2. 使用动态向量低秩适应（DV-LORA）高效地将这些先验适应到医疗领域，最小化参数预算同时弥合合成到真实的差距；3. 在SCARED数据集上引入物理分层评估协议，严格量化高反射区域的性能。

Result: 在SCARED数据集上建立了新的最先进水平：准确度（<1.25）达到98.1%，平方相对误差相比现有基线降低了超过17%，在不利手术光照条件下表现出卓越的鲁棒性。

Conclusion: 通过结合Depth Anything V2的合成先验和DV-LORA的高效领域适应，该方法显著提升了内窥镜手术环境中单目深度估计的准确性和鲁棒性，特别是在处理薄手术器械和高反射表面时表现优异。
...
Journal reference: SIGGRAPH Asia 2025 Conference Papers, Article 49, 1-11

DOI: [10.1145/3757377.3763931](https://doi.org/10.1145/3757377.3763931)

Author affiliations: Andrew Tinits: University of Waterloo; Stephen Mann: University of Waterloo
...
<details>
  <summary>Details</summary>
Motivation: Noise2Noise方法虽然无需干净目标图像，但对噪声目标应用非线性函数会产生偏差，限制了预处理选择。特别是在HDR图像去噪中，异常值问题严重，而常用的色调映射非线性函数被认为与Noise2Noise训练不兼容。

Method: 建立理论框架分析非线性函数对Noise2Noise训练的影响，识别出具有最小偏差的非线性函数类别。针对HDR图像去噪，研究损失函数与色调映射函数的特定组合，在减少异常值影响的同时最小化偏差。

Result: 提出的方法在蒙特卡洛渲染生成的HDR图像去噪中表现良好，仅使用噪声训练数据就能接近原始使用高样本参考图像训练实现的性能。

Conclusion: 某些非线性函数可以安全地应用于Noise2Noise训练中的噪声目标图像，这扩展了预处理选项，特别解决了HDR图像去噪中的异常值问题，使仅使用噪声数据训练高质量去噪器成为可能。
...
TL;DR: 提出基于视频分析的自动评估管道，用于城市作战训练中的"进入并清理房间"演练，通过计算机视觉提取骨骼、视线和轨迹数据，开发特定任务指标评估心理运动流畅性、情境意识和团队协调能力。


<details>
  <summary>Details</summary>
Motivation: 城市作战训练需要情境意识和肌肉记忆，但传统评估方法依赖昂贵传感器或主观观察，难以实现客观、可扩展的性能评估。需要自动化、非侵入式的评估系统来支持合成训练环境中的重复练习。

Method: 开发视频分析管道，使用计算机视觉模型从训练视频中提取2D骨骼、视线向量和运动轨迹。基于这些数据开发特定任务指标，测量心理运动流畅性、情境意识和团队协调。通过扩展的认知任务分析层次结构，使用加权组合生成团队合作和认知能力的总体性能评分。

Result: 通过真实世界ECR演练的案例研究，展示了该方法能够提供可操作的、领域特定的指标，捕捉个人和团队表现。这些见解支持行动后审查，通过交互式仪表板在Gamemaster和GIFT框架中提供直观反馈。

Conclusion: 视频分析为合成训练环境中的可扩展评估提供了可行方法，但存在跟踪困难、地面真实验证等局限性。未来工作包括扩展到3D视频数据，利用视频分析实现STE中的可扩展评估。
...
TL;DR: 提出终身域自适应3D人体姿态估计新任务，通过GAN框架整合3D姿态生成器、2D姿态判别器和3D姿态估计器，解决非平稳目标域数据适应和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法依赖受控环境标注数据，难以泛化到多样化的真实场景。传统域自适应方法（通用DA和源自由DA）忽略了目标域姿态数据的非平稳性问题，无法处理连续变化的域适应需求。

Method: 提出终身域自适应3D HPE任务框架，包含：1）GAN架构整合3D姿态生成器、2D姿态判别器和3D姿态估计器；2）新颖的3D姿态生成器范式，融合姿态感知、时序感知和域感知知识；3）在适应当前域时无法访问源域和先前目标域，通过生成对抗机制缓解域偏移和对齐原始与增强姿态。

Result: 在多样化域自适应3D HPE数据集上进行广泛实验，证明了方法的优越性能，有效缓解了域偏移问题并减轻了灾难性遗忘。

Conclusion: 首次将终身域自适应引入3D HPE任务，提出的GAN框架和3D姿态生成器范式能够有效适应当前域姿态并保留先前域知识，为解决非平稳目标域数据适应问题提供了创新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: CT虽能评估颅骨和缝合线畸形，但具有电离辐射风险，不适用于无明显异常的儿童。MRI无辐射且软组织对比度好，但无法显示颅骨缝合线、估计骨密度或评估颅顶生长。需要一种非侵入性方法来量化儿科颅骨发育和缝合线骨化。

Method: 开发深度学习驱动的管道，将T1加权MRI转换为合成CT。使用领域特定的变分自编码器，预测详细的颅骨分割，生成缝合线概率热图，并从热图推导出直接缝合线分割。

Result: 合成CT与真实CT的结构相似度达99%，Fréchet inception距离为1.01。颅骨分割在七块颅骨上平均Dice系数达85%，缝合线分割Dice系数达80%。通过TOST检验(p<0.05)确认了合成CT与真实CT在颅骨和缝合线分割上的等效性。

Conclusion: 这是首个能从MRI衍生的合成CT实现缝合线分割的儿科颅骨CT合成框架，尽管MRI对骨和缝合线的描绘有限。该方法通过结合领域特定的变分自编码器，从常规儿科MRI生成感知上无法区分的颅骨合成CT，填补了非侵入性颅骨评估的关键空白。
...
Method: 提出元引导多模态学习框架，包含元参数化自适应模态融合和一致性正则化模块。Meta-AMF根据可用模态生成自适应软标签监督信号，促进更一致的多模态融合；一致性正则化模块增强分割性能并提高框架鲁棒性和泛化能力

Result: 在BraTS2020和BraTS2023数据集上实验，相比多种SOTA方法表现更优。在BraTS2020上，15种缺失模态组合的平均Dice分数：WT 87.55、TC 79.36、ET 62.67
...
TL;DR: 提出基于AI的微血管吻合术自动动作分割与技能评估框架，包含目标跟踪、动作分割和技能分类三个模块，在边缘计算平台上实现实时性能评估。


<details>
  <summary>Details</summary>
Motivation: 神经外科微血管吻合术需要精细操作技能，传统评估依赖专家主观评分，存在评分者间变异、不一致性和时间成本高的问题，需要自动化、可扩展的客观评估方案。
...
Conclusion: 该方法能够为显微外科教育提供客观、实时的反馈，实现更标准化、数据驱动的训练协议，推动高风险手术环境中的能力评估进步。
...
TL;DR: T2VAttack：针对文本到视频扩散模型的对抗攻击研究，从语义和时间两个角度评估模型脆弱性，通过替换或插入关键词实现高效攻击。
...
Method: 提出两种攻击方法：T2VAttack-S通过贪婪搜索识别语义或时间关键词语并用同义词替换；T2VAttack-I迭代插入优化词语以最小化提示扰动。结合语义目标（评估视频-文本对齐）和时间目标（评估时间动态）进行综合攻击。

Result: 在多个先进T2V模型（ModelScope、CogVideoX、Open-Sora、HunyuanVideo）上的实验表明，即使单个词语的替换或插入也会导致语义保真度和时间动态的显著退化，揭示了当前模型的严重脆弱性。

Conclusion: 当前文本到视频扩散模型对对抗攻击高度敏感，即使是微小的提示修改也会严重影响生成质量，这突显了模型鲁棒性方面的重要漏洞，需要进一步研究防御机制。
...
TL;DR: 提出了一种仅使用图像和可选相机位姿的自驾驶场景视图外推方法，通过4D高斯模型和视频扩散模型的迭代优化，无需昂贵传感器或标注
...
Method: 首先从图像估计全局静态点云和逐帧动态点云，融合为统一表示；采用可变形4D高斯框架重建场景；用初始训练的4D高斯模型渲染退化伪图像训练视频扩散模型；通过扩散模型迭代优化逐步偏移的高斯渲染结果，并将增强结果反馈给4DGS作为训练数据，直到外推到目标视点

Result: 与基线方法相比，该方法在新外推视点上生成更高质量的图像

Conclusion: 该方法仅需图像和可选相机位姿，无需昂贵传感器或标注，通过4D高斯模型与视频扩散模型的协同迭代优化，实现了高质量的自驾驶场景视图外推
...
TL;DR: 基于SATLAS基础模型的深度学习时间序列异常检测方法，通过预测卫星影像变化前状态来高灵敏度识别地表异常


<details>
  <summary>Details</summary>
Motivation: 卫星影像地表变化检测对灾害响应和环境监测至关重要，但传统方法受大气噪声、季节变化和传感器伪影影响，难以实现高灵敏度检测

Method: 训练基于SATLAS基础模型的修复模型，利用Sentinel-2时间序列数据，从前序影像重建最后一帧的无变化状态，通过预测与观测差异检测异常

Result: 在2023年土耳其-叙利亚地震序列中成功检测到Tepehan地区的断裂特征，检测灵敏度比传统方法高约3倍，优于时间中值和Reed-Xiaoli异常检测器

Conclusion: 该方法利用深度学习的时间序列冗余性，实现了前所未有的异常检测灵敏度，为基于免费多光谱卫星数据的全球尺度地表变化自动监测提供了可行路径
...
TL;DR: GASeg是一个自监督语义分割框架，通过引入可微盒计数模块和拓扑增强策略，利用稳定的拓扑信息来桥接几何和外观特征，解决外观模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 自监督语义分割方法在面对外观模糊性（如阴影、眩光、局部纹理）时经常失败，这是因为过度依赖不稳定的外观特征。需要利用更稳定的拓扑信息来桥接几何和外观特征。

Method: 提出GASeg框架，核心是可微盒计数模块，从几何特征流和外观特征流中量化多尺度拓扑统计信息。引入拓扑增强策略，通过形态学操作模拟真实世界模糊性。使用多目标损失函数GALoss强制跨模态对齐。

Result: 在四个基准测试（包括COCO-Stuff、Cityscapes和PASCAL）上实现了最先进的性能，验证了通过拓扑信息桥接几何和外观的方法有效性。

Conclusion: 通过利用稳定的拓扑信息桥接几何和外观特征，GASeg能够有效解决自监督语义分割中的外观模糊性问题，在多个基准测试上表现出优越性能。
...
Journal reference: 2025 International Conference on Space Robotics (iSpaRo)

DOI: [10.1109/iSpaRo66239.2025.11437016](https://doi.org/10.1109/iSpaRo66239.2025.11437016)

Author affiliations: Tae Ha Park: Korean Association of Space & Environment Research; Simone D’Amico: Vaughn College of Aeronautics and Technology / American Institute of Aeronautics and Astronautics
...
<details>
  <summary>Details</summary>
Motivation: 在太空交会对接任务中，从图像序列恢复目标航天器3D结构时，传统3DGS需要静态场景假设，但太空图像存在动态光照条件。同时，3DGS模型的光度精度对后续姿态估计任务至关重要。

Method: 提出将服务航天器估计的太阳位置先验知识融入3DGS训练流程，改进3DGS渲染的光度质量。通过结合太阳位置信息，使模型能够适应太空快速变化的光照条件。

Result: 实验研究表明，该方法能有效提升3DGS模型在太空图像序列上的训练效果，使模型学会适应快速变化的光照条件，并反映全局阴影和自遮挡效应。

Conclusion: 通过整合太阳位置先验知识，显著改善了3DGS在动态太空光照条件下的光度渲染质量，为RPO任务中的3D重建和姿态估计提供了更可靠的解决方案。
...
TL;DR: 提出CVC框架，通过双视角速度转换机制和基于经验贝叶斯推断的后验一致性更新，解决流式扩散编辑中的速度误差累积问题，实现稳定、保真且语义一致的图像编辑。


<details>
  <summary>Details</summary>
Motivation: 现有流式扩散编辑方法在源图像和目标图像分布间直接转换时，潜在轨迹存在累积速度误差，导致语义不一致和结构保真度损失。需要一种能纠正速度误差、保持结构一致性的编辑框架。

Method: 提出条件速度校正（CVC）框架：1）双视角速度转换机制，将潜在演化分解为结构保持分支（与源轨迹一致）和语义引导分支（向目标分布受控偏离）；2）基于经验贝叶斯推断和Tweedie校正的后验一致性更新，对条件速度场进行数学基础误差补偿。

Result: CVC在多样化任务中实现稳定可解释的潜在动态，获得忠实重建和平滑局部语义转换。实验表明CVC在保真度、语义对齐和编辑可靠性方面均优于现有方法。

Conclusion: CVC通过理论驱动的速度校正机制，有效解决了流式扩散编辑中的轨迹漂移问题，为分布转换提供了稳定、保真且语义一致的编辑框架。
...
TL;DR: 提出FitControler，一种可学习的插件，能够集成到现代虚拟试穿模型中，实现定制化的服装合身度控制，解决了现有方法忽视服装合身度这一关键风格因素的问题。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法主要关注服装细节的忠实渲染，但忽视了塑造整体风格的关键因素——服装合身度。合身度定义了服装与穿着者身体的贴合方式，是时尚设计的基本要素。因此需要开发能够控制服装合身度的虚拟试穿系统。

Method: 提出FitControler插件，包含两个核心组件：1) 合身度感知布局生成器，基于一组精心处理的服装无关表示重新绘制身体-服装布局；2) 多尺度合身度注入器，将布局线索传递给VTON模型实现布局驱动的虚拟试穿。构建了Fit4Men数据集，包含13,000个不同合身度的身体-服装对，涵盖上衣和下装，具有不同的相机距离和身体姿态。

Result: FitControler能够与各种VTON模型协同工作，实现精确的合身度控制。引入了两种合身度一致性指标来评估生成结果。广泛的实验验证了方法的有效性。

Conclusion: 提出了首个合身度感知的虚拟试穿系统FitControler，通过可学习的插件架构解决了服装合身度控制的关键问题，为虚拟试穿领域引入了重要的风格控制维度。
...
TL;DR: 提出结构引导的2D高斯泼溅分配原则，通过结构引导初始化、自适应比特量化、几何一致性正则化，提升2DGS的率失真性能，同时保持毫秒级解码速度。


<details>
  <summary>Details</summary>
Motivation: 现有2DGS方法在分配表示容量和参数精度时忽略了图像结构，导致在低比特率下的率失真效率受限。需要一种能显式耦合图像结构与表示容量/量化精度的方法，同时保持原生解码速度。

Method: 1. 结构引导初始化：根据自然图像的空间结构先验分配2D高斯分布，获得局部化且语义有意义的分布。2. 自适应比特量化：在量化感知微调中，对协方差参数进行自适应比特宽度量化，为复杂区域的小尺度高斯分配更高精度。3. 几何一致性正则化：对齐高斯方向与局部梯度方向以更好保留结构细节。

Result: 方法显著提升了2DGS的表示能力和率失真性能，同时保持超过1000 FPS的解码速度。相比基线GSImage，在Kodak数据集上BD-rate降低43.44%，在DIV2K数据集上降低29.91%。

Conclusion: 提出的结构引导分配原则有效解决了2DGS在低比特率下率失真效率不足的问题，通过显式耦合图像结构与表示容量/量化精度，实现了更好的压缩性能，同时保持了实时解码能力。
...
TL;DR: MF-RSVLM是一个多特征融合的遥感视觉语言模型，通过多尺度特征提取和循环视觉特征注入机制，解决了现有遥感VLM在细粒度特征提取和视觉遗忘方面的问题，在遥感分类、图像描述和视觉问答任务上取得了先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在遥感领域面临挑战：遥感图像与自然图像存在本质差异，现有遥感VLM难以提取细粒度视觉特征，并且在深度语言处理过程中容易出现视觉遗忘现象。

Method: 提出MF-RSVLM模型，采用多尺度视觉表示学习，结合全局上下文和局部细节，改进对遥感场景中复杂小结构的捕捉。使用循环视觉特征注入方案，确保语言模型在生成过程中保持对视觉证据的依赖，减少视觉遗忘。

Result: 在多个遥感基准测试上的广泛实验表明，MF-RSVLM在遥感分类、图像描述和视觉问答任务上取得了最先进或极具竞争力的性能。

Conclusion: MF-RSVLM通过有效的多特征融合和循环视觉特征注入机制，显著提升了遥感视觉语言模型的性能，解决了细粒度特征提取和视觉遗忘问题，为遥感理解任务提供了有效的解决方案。
...
TL;DR: RSAgent：一种基于多模态大语言模型的智能体，通过多轮工具调用实现文本引导的目标分割，结合推理与行动迭代优化分割结果


<details>
  <summary>Details</summary>
Motivation: 现有文本引导分割方法通常将问题视为一次性定位，在初始定位错误时缺乏验证、重新聚焦和细化的能力。需要一种能够进行多轮推理和行动交互的方法来改进分割性能。

Method: 提出RSAgent智能体，通过多轮工具调用交替进行推理和行动：查询分割工具箱、观察视觉反馈、基于历史观察修正空间假设、重新定位目标并迭代优化掩码。构建数据管道合成多轮推理分割轨迹，采用两阶段训练框架：冷启动监督微调 + 基于细粒度任务特定奖励的智能体强化学习。

Result: 在ReasonSeg测试集上达到66.5% gIoU的零样本性能，比Seg-Zero-7B提升9%；在RefCOCOg上达到81.5% cIoU，在领域内和领域外基准测试中均达到最先进性能。

Conclusion: RSAgent通过多轮推理-行动交互机制有效解决了文本引导分割中的初始定位错误问题，实现了更准确、可验证和可细化的分割结果，为智能体驱动的视觉理解任务提供了新范式。
...
TL;DR: 提出基于强化学习的可训练各向异性扩散框架，通过深度Q学习选择扩散动作序列，构建自适应图像结构的随机扩散过程，在去噪性能上超越传统扩散方法并与深度CNN方法竞争。


<details>
  <summary>Details</summary>
Motivation: 传统各向异性扩散方法使用显式扩散算子，难以适应复杂图像结构，性能受限；而学习基方法虽有效但缺乏扩散方法的理论优势。需要结合两者优势，开发自适应性强且性能优越的扩散去噪方法。

Method: 将去噪过程建模为一系列朴素扩散动作序列，通过深度Q学习学习动作顺序；构建基于强化学习的可训练各向异性扩散框架，形成自适应不同图像结构的随机扩散过程。

Result: 该方法在去除三种常见噪声类型上表现出色，超越了现有扩散基方法，并与代表性深度CNN基方法具有竞争力。

Conclusion: 提出的基于强化学习的可训练各向异性扩散框架成功结合了扩散方法的理论优势和学习方法的适应性，为图像去噪提供了有效的新途径。
...
<details>
  <summary>Details</summary>
Motivation: 跨模态检索中，大规模标注数据收集耗时费力，多模态数据标注不可避免地包含噪声，这会降低模型检索性能。现有鲁棒CMR方法往往无法同时满足模型性能上限、校准可靠性和数据利用率的要求。

Method: 提出Neighbor-aware Instance Refining with Noisy Labels (NIRNL)框架：1) 跨模态边界保持(CMP)调整正负样本对的相对距离，增强样本对区分度；2) 邻居感知实例精炼(NIR)通过跨模态邻居共识识别纯子集、困难子集和噪声子集；3) 针对细粒度划分构建不同的定制优化策略，最大化利用所有可用数据同时减轻误差传播。

Result: 在三个基准数据集上的大量实验表明，NIRNL实现了最先进的性能，表现出显著的鲁棒性，特别是在高噪声率下。

Conclusion: NIRNL框架通过跨模态边界保持和邻居感知实例精炼，有效解决了带噪声标签的跨模态检索问题，在保持模型性能的同时提高了数据利用率和校准可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 细粒度遥感数据集通常采用层次标签结构进行粗到细的对象区分，但将这种语义层次嵌入表示学习空间以提升检测性能仍具挑战。现有研究在不同层次应用监督对比学习，但忽略了两个关键问题：1) 标签层次中数据分布不平衡导致高频类别主导学习过程；2) 类别间语义关系学习干扰类别无关的定位任务。

Method: 提出平衡层次对比损失结合解耦学习策略，在DETR框架内实现。平衡损失引入可学习的类别原型，在每一层次均衡不同类别的梯度贡献，确保每个层次类别在每个小批量中对损失计算贡献相等。解耦策略将DETR的对象查询分为分类和定位两组，实现任务特定的特征提取和优化。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)模型因注意力机制计算成本极高，限制了实际应用。现有稀疏注意力方法存在两个关键限制：稀疏模式预测的开销和缺乏硬件通用性（多数仅针对GPU设计）。

Method: 采用块级均值作为代表性标记进行稀疏掩码预测；实现时空感知的标记排列；针对视频生成场景引入首帧下沉机制。

Result: RainFusion2.0能够实现80%的稀疏度，在不影响视频质量的情况下获得1.5-1.8倍的端到端加速。在各种生成模型和不同硬件平台上均表现出有效性。

Conclusion: RainFusion2.0成功解决了现有稀疏注意力方法的局限性，为视频和图像生成模型提供了硬件高效、低开销的加速方案，具有良好的跨硬件平台泛化能力。
...
TL;DR: D²VLM：通过解耦学习和因子化偏好优化提升视频语言模型的时序定位与文本响应能力


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型在事件级感知的准确时序定位方面存在困难。作者观察到时序定位和文本响应这两个主要因素形成逻辑层次结构：准确的时序证据定位是可靠文本响应的基础。然而，现有工作通常以耦合方式处理这两个任务，缺乏清晰的逻辑结构，导致目标函数次优。

Method: 提出D²VLM框架，采用解耦学习方法：1) 采用"先定位，后基于证据引用回答"范式；2) 引入证据标记进行证据定位，强调超越时间戳表示的事件级视觉语义捕捉；3) 提出因子化偏好优化(FPO)算法，将概率时序定位建模明确纳入优化目标，实现对时序定位和文本响应的偏好学习；4) 构建合成数据集以支持具有显式时序定位的因子化偏好学习。

Result: 在多个任务上的实验表明，该方法具有明显优势。源代码已公开。

Conclusion: 通过解耦学习和因子化偏好优化，D²VLM框架有效提升了视频语言模型在时序定位和文本响应方面的性能，解决了现有方法中任务耦合导致的次优问题。
...
TL;DR: 提出Latent Motion Reasoning (LMR)框架，通过两阶段"思考-行动"决策过程解决文本到动作生成中的语义-运动学阻抗不匹配问题，在语义对齐和物理合理性方面取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成方法将问题视为直接翻译，面临语义-运动学阻抗不匹配的根本理论瓶颈：难以在单次映射中将语义密集的离散语言意图落地到运动学密集的高频运动数据中。

Method: 提出Latent Motion Reasoning (LMR)框架，受层次运动控制启发，将生成重构为两阶段"思考-行动"决策过程。核心是新型双粒度分词器，将运动解耦为两个不同流形：用于规划全局拓扑的压缩语义丰富推理潜空间，以及保持物理保真度的高频执行潜空间。

Result: 在T2M-GPT（离散）和MotionStreamer（连续）两个代表性基线上实现LMR，实验表明在语义对齐和物理合理性方面均获得非平凡改进，验证了运动规划的最佳基底不是自然语言而是学习的运动对齐概念空间。

Conclusion: 通过架构转向潜在系统2推理，采用两阶段"思考-行动"决策过程，有效弥合语言与物理之间的不可言说性鸿沟，为文本到动作生成提供了更优的理论框架。
...
Method: 提出免训练生成式对抗攻击框架，包含显著区域选择模块识别对深度估计影响最大的区域，以及雅可比向量积引导机制将对抗梯度导向预训练扩散模型支持的更新方向，通过扩散条件生成过程生成自然对抗物体。

Result: 大量数字和物理实验表明，该方法在有效性、隐蔽性和物理部署能力方面显著优于现有攻击方法，能够诱导显著的对抗深度偏移，对自动驾驶安全评估具有重要实际意义。

Conclusion: 该工作提出的生成式对抗攻击框架克服了传统纹理贴片攻击的限制，生成物理上合理的对抗物体，为自动驾驶系统的安全评估提供了更有效的测试工具。
...
Method: 提出GeoBench层次化基准，包含四个推理级别：视觉感知、目标导向规划、严格定理应用、自我反思回溯。通过TrustGeoGen生成六个形式化验证任务，系统评估不同能力。实验比较推理模型与通用MLLMs的性能。

Result: 实验显示：1) 推理模型如OpenAI-o3优于通用MLLMs；2) 性能随任务复杂度增加显著下降；3) 子目标分解和无关前提过滤对最终准确性有重要影响；4) 思维链提示在某些任务中意外降低性能。
...
TL;DR: 论文提出D²-Align框架，通过方向性解耦对齐缓解文本到图像扩散模型在人类反馈强化学习中的偏好模式崩溃问题


<details>
  <summary>Details</summary>
Motivation: 现有基于人类反馈强化学习的文本到图像扩散模型对齐方法虽然能在自动化奖励指标上获得高分，但往往导致偏好模式崩溃（PMC）——模型收敛到狭窄的高分输出模式（如单一风格或普遍过曝），严重损害生成多样性

Method: 提出方向性解耦对齐（D²-Align）框架：1）在奖励模型的嵌入空间中学习方向性校正（保持模型冻结）；2）在优化过程中将校正应用于奖励信号，防止模型坍缩到特定模式

Result: D²-Align在质量和多样性定量指标上均表现优异，实现了与人类偏好的更好对齐，同时避免了偏好模式崩溃

Conclusion: 方向性解耦对齐能有效缓解文本到图像扩散模型在人类反馈强化学习中的偏好模式崩溃问题，在保持生成多样性的同时实现更好的人类偏好对齐
...
TL;DR: IMDD-1M是首个大规模工业多模态缺陷数据集，包含100万对齐的图像-文本对，用于制造和质量检测的多模态学习。基于该数据集训练了面向工业场景的扩散式视觉语言基础模型，通过轻量微调即可高效适应特定领域。


<details>
  <summary>Details</summary>
Motivation: 工业制造领域缺乏大规模、高质量的多模态缺陷数据集，限制了多模态学习在质量检测和制造智能中的应用。现有数据集规模有限，覆盖缺陷类型和材料类别不足，难以支持基础模型的训练。

Method: 构建了IMDD-1M数据集，包含100万高分辨率真实缺陷图像，覆盖60+材料类别和400+缺陷类型，每张图像配有专家验证的细粒度文本描述。基于该数据集从头训练扩散式视觉语言基础模型，专门针对工业场景设计。

Result: IMDD-1M数据集支持分类、分割、检索、描述和生成等多种应用。训练的基础模型可作为通用化基础，通过轻量微调高效适应特定领域。仅需不到5%的任务特定数据即可达到专用专家模型的性能水平。

Conclusion: IMDD-1M数据集和相应的基础模型为工业检测和生成任务提供了数据高效的基础模型适应方案，为实现可扩展、领域自适应和知识驱动的制造智能铺平了道路。
...
TL;DR: DiffThinker：一种基于扩散的生成式多模态推理框架，将多模态推理重新定义为原生的图像到图像生成任务，在视觉中心任务中实现卓越的逻辑一致性和空间精度。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）在多模态推理方面取得显著进展，但其推理过程仍以文本为中心，导致在复杂长视野、视觉中心任务中表现不佳。需要一种更有效的视觉中心推理范式。

Method: 提出DiffThinker，一种基于扩散的推理框架，将多模态推理重新定义为原生的图像到图像生成任务。该框架具有四个核心特性：效率、可控性、原生并行性和协作性。
...
Conclusion: 生成式多模态推理是视觉中心推理的有前景方法，DiffThinker通过将推理重新定义为图像到图像生成任务，在逻辑一致性和空间精度方面实现了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱图像分析面临数据量大、内存需求高的计算瓶颈，现有基于大规模遥感数据集预训练的基础模型难以迁移到近距离农业监测等特定领域应用，因为光谱特征、空间尺度和语义目标存在根本差异。

Method: 提出Deep Global Clustering (DGC)框架，通过重叠区域的小补丁操作学习全局聚类结构，强制一致性约束，实现恒定内存使用，无需预训练。

Result: 在叶片病害数据集上，DGC实现背景-组织分离（平均IoU 0.925），并通过可导航的语义粒度展示无监督病害检测能力，但存在优化不稳定性和聚类过度合并问题。
...
TL;DR: 提出内部引导（IG）策略，通过在训练过程中引入中间层辅助监督，在采样过程中外推中间层和深层输出来提升扩散模型生成质量，显著优于现有引导方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型难以覆盖低概率区域，标准分类器自由引导（CFG）会导致样本过度简化或失真，而基于坏版本的引导方法需要精心设计的退化策略、额外训练和采样步骤。

Method: 提出内部引导（IG）策略：1）训练阶段在中间层引入辅助监督；2）采样阶段通过外推中间层和深层输出获得生成结果。该方法简单有效，无需额外训练或复杂采样步骤。

Result: 在ImageNet 256×256上，SiT-XL/2+IG在80和800轮分别达到FID=5.31和1.75；LightningDiT-XL/1+IG达到FID=1.34；结合CFG后达到当前最优FID=1.19，显著优于所有基线方法。

Conclusion: 内部引导（IG）是一种简单有效的策略，通过引入中间层监督和外推机制，显著提升了扩散模型的训练效率和生成质量，在多个基准上实现了大幅性能提升。
...
TL;DR: CorGi是一种无需训练、基于贡献度引导的块级间隔缓存框架，用于加速扩散变换器(DiT)推理，通过选择性重用低贡献度块的输出来减少冗余计算，在保持生成质量的同时实现最高2.0倍加速。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)在视觉生成方面表现出色，但其迭代去噪过程结合大模型容量导致推理成本高昂。现有研究表明DiT模型的迭代去噪过程在多个步骤间存在大量冗余计算，需要有效减少这些冗余。

Method: 提出CorGi框架：1) 基于贡献度评估识别低贡献度的transformer块；2) 在去噪步骤间隔内缓存这些块的输出并在后续步骤中重用；3) 对于文本到图像任务，提出CorGi+变体，利用跨注意力图识别显著token并应用部分注意力更新以保护重要物体细节。

Result: 在最先进的DiT模型上评估显示，CorGi和CorGi+平均实现最高2.0倍的加速，同时保持高生成质量。

Conclusion: CorGi和CorGi+是无需训练的DiT推理加速框架，通过选择性重用低贡献度块的输出有效减少冗余计算，在保持生成质量的同时显著提升推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有训练免费方法存在两个问题：要么依赖昂贵的外部基础模型（如SAM、DINO），要么对CLIP内部特征使用静态手工启发式方法。这些方法要么计算成本高，要么效果次优。CLIP的图像级表示缺乏精确的像素级细节，这是开放词汇语义分割的根本障碍。

Method: 提出注意力细化模块（ARM），包含语义引导的交叉注意力块和自注意力块。交叉注意力块使用鲁棒的深层特征（K，V）来选择和细化细节丰富的浅层特征（Q）。采用"一次训练，随处使用"的范式，在通用数据集（如COCO-Stuff）上训练一次后，ARM可作为各种训练免费框架的通用即插即用后处理器。

Result: 在多个基准测试中，ARM能持续提升基线性能，且推理开销可忽略不计。实验表明ARM为训练免费的开放词汇语义分割建立了高效有效的范式。

Conclusion: ARM是一个轻量级可学习模块，通过自适应融合CLIP的层次特征，有效解锁和细化了CLIP的内部潜力。与静态融合方法不同，ARM学习自适应特征融合，为训练免费的开放词汇语义分割提供了高效有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基于视觉的自动驾驶系统需要多样化和可扩展的训练数据，但现有的视频对象编辑方法往往难以同时保持高视觉保真度和时间一致性，这限制了数据增强的效果。

Method: 1. 基于文本到视频扩散先验确保时间一致性；2. 将预训练2D编码器的时序无关潜在特征注入3D解码器以恢复细节并保持因果结构；3. 引入两阶段数据对齐策略（粗粒度3D对齐和细粒度2D精炼）解决场景对象与插入资产之间的分布不匹配问题。

Result: 大量实验表明，Mirage在各种编辑场景中实现了高真实感和时间一致性。除了资产编辑，该模型还能泛化到其他视频到视频转换任务，为未来研究提供了可靠的基线。

Conclusion: Mirage通过创新的特征注入机制和对齐策略，成功解决了驾驶场景中视频资产编辑的保真度和一致性挑战，为自动驾驶数据增强提供了有效的解决方案，并展示了在更广泛视频处理任务中的应用潜力。
...
TL;DR: MambaSeg：一种新颖的双分支语义分割框架，使用并行Mamba编码器高效建模RGB图像和事件流，通过双维交互模块（DDIM）在空间和时间维度进行细粒度融合，在DDD17和DSEC数据集上实现最先进的性能并显著降低计算成本。
...
Method: 提出MambaSeg框架：1）使用并行Mamba编码器分别处理RGB图像和事件流；2）引入双维交互模块（DDIM），包含跨空间交互模块（CSIM）和跨时间交互模块（CTIM），在空间和时间维度进行细粒度融合；3）通过跨模态对齐减少模糊性并利用模态互补特性。

Result: 在DDD17和DSEC数据集上的实验表明，MambaSeg实现了最先进的语义分割性能，同时显著降低了计算成本，展示了其在高效、可扩展和鲁棒的多模态感知方面的潜力。

Conclusion: MambaSeg通过并行Mamba编码器和双维交互模块，有效解决了RGB-事件融合中的计算效率和时空融合问题，为高效、鲁棒的多模态语义分割提供了有前景的解决方案。
...
TL;DR: 提出PGMP框架解决牙科CBCT金属伪影问题，结合物理模拟生成高质量训练数据，使用确定性流形投影实现单次前向推理，并通过医学基础模型确保临床合理性
...
Method: 提出Physically-Grounded Manifold Projection (PGMP)框架：1) Anatomically-Adaptive Physics Simulation (AAPS)通过蒙特卡洛光谱建模和患者特异性数字孪生合成高保真训练对；2) DMP-Former采用Direct x-Prediction范式，将恢复重新表述为确定性流形投影，实现单次前向推理；3) Semantic-Structural Alignment (SSA)模块利用医学基础模型(MedDINOv3)的先验锚定解决方案，确保临床合理性

Result: 在合成和多中心临床数据集上的实验表明，PGMP在未见解剖结构上优于最先进方法，在效率和诊断可靠性方面设定了新基准

Conclusion: PGMP框架通过物理模拟、确定性流形投影和医学先验的整合，有效解决了牙科CBCT金属伪影减少问题，实现了高效且临床可靠的解决方案
...
TL;DR: DualityForge：一种通过可控扩散视频编辑合成反事实视频数据的框架，用于减少多模态大语言模型的视觉未接地幻觉问题


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频理解方面取得了显著进展，但存在严重缺陷：过度依赖语言先验，导致在处理违反常识的反事实视频时出现视觉未接地幻觉。这一局限性源于文本和视频之间的内在数据不平衡，且收集和标注反事实数据的成本高昂。

Method: 提出DualityForge框架，利用可控的基于扩散的视频编辑技术将真实视频转换为反事实场景。通过将结构化上下文信息嵌入视频编辑和QA生成过程，自动生成高质量的QA对以及原始-编辑视频对用于对比训练。基于此构建DualityVidQA大规模视频数据集。此外，提出Duality-Normalized Advantage Training（DNA-Train），一种两阶段SFT-RL训练机制，在RL阶段应用成对ℓ₁优势归一化，实现更稳定高效的政策优化。

Result: 在DualityVidQA-Test上的实验表明，该方法显著减少了模型在反事实视频上的幻觉，相对于Qwen2.5-VL-7B基线实现了24.0%的相对改进。此外，在幻觉和通用基准测试中都取得了显著增益，显示出强大的泛化能力。

Conclusion: DualityForge框架通过合成高质量反事实数据有效解决了MLLM的视觉未接地幻觉问题，提出的DNA-Train训练机制进一步提升了模型性能。该方法具有强大的泛化能力，作者承诺开源数据集和代码。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于2D单应性变换和网格扭曲的图像拼接方法在处理具有多个深度层和遮挡的真实3D场景时，会在非重叠区域产生重影、结构弯曲和拉伸失真，特别是在多视角累积和360°闭环拼接场景中问题更加严重

Method: 1) 将输入图像提升为统一坐标系中的密集3D点表示，通过置信度指标增强全局跨视图融合；2) 在3D空间中建立统一投影中心，采用等距圆柱投影将融合数据映射到单一全景流形；3) 在画布域进行空洞填充以处理视角转换揭示的未知区域，恢复连续纹理和语义一致性

Result: 实验评估表明，该方法在涉及显著视差和复杂遮挡的场景中，大幅减少了几何失真和重影伪影，生成了更加自然和一致的全景结果

Conclusion: 该框架将拼接从2D扭曲范式重新概念化为3D一致性范式，能够灵活整合各种3D提升和补全模块，为处理复杂3D场景的全景拼接提供了有效解决方案
...
Method: 提出UniAct两阶段框架：1) 集成微调的多模态大语言模型(MLLM)用于高层指令理解；2) 因果流式处理管道用于实时动作生成。通过FSQ共享离散码本统一多模态输入，确保跨模态对齐同时将动作约束在物理可行的流形上。

Result: 在UniMoCap（20小时人形动作基准）上验证，实现低于500ms的延迟，零样本跟踪不完美参考动作的成功率提升19%，在多样化现实场景中展现出鲁棒的泛化能力。

Conclusion: UniAct框架是实现响应式通用人形助手的关键进展，通过统一感知与控制实现了多模态指令的无缝交互执行，为人形机器人的实际应用奠定了基础。
...
TL;DR: CERES是一个因果推理框架，通过双模态因果干预（后门调整解决语言偏差，前门调整解决视觉混淆）来提升第一人称视频中基于语言查询的物体分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有Ego-RVOS方法存在两个主要问题：1）从训练数据中学习到虚假的相关性（对象-动作配对偏差）；2）第一人称视角固有的视觉混淆因素（快速运动、频繁遮挡）。这些限制导致模型在真实场景中表现不佳。

Method: CERES是一个插件式因果框架，包含双模态因果干预：1）后门调整：对抗从数据集统计中学习的语言表示偏差；2）前门调整：通过因果原则指导，将语义视觉特征与几何深度信息智能整合，创建对第一人称失真更鲁棒的表示。

Result: 在Ego-RVOS基准测试中，CERES实现了最先进的性能，证明了因果推理在构建更可靠的自我中心视频理解模型方面的潜力。

Conclusion: CERES通过因果干预有效解决了Ego-RVOS中的语言偏差和视觉混淆问题，为更广泛的自我中心视频理解任务提供了更可靠的建模方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于图像的视觉语言模型在自动驾驶中存在瓶颈，主要问题是缺乏准确的度量空间推理和几何推断能力，导致驾驶策略不可靠，需要引入3D度量数据来构建可信赖的自动驾驶系统。

Method: 提出LVLDrive框架，通过渐进融合Q-Former逐步注入LiDAR特征，确保预训练VLM知识库的稳定性；同时开发空间感知问答数据集来显式教授模型高级3D感知和推理能力。

Result: 在驾驶基准测试中，LVLDrive在场景理解、度量空间感知和可靠驾驶决策方面均优于纯视觉方法，证明了3D度量数据对构建可信VLM自动驾驶系统的必要性。

Conclusion: LVLDrive通过整合LiDAR点云成功增强了VLM的3D度量空间理解能力，为构建更安全可靠的基于VLM的自动驾驶系统提供了有效解决方案，强调了显式3D度量数据的重要性。
...
### [41] [The Mechanics of CNN Filtering with Rectification](https://arxiv.org/abs/2512.24338)
*Liam Frija-Altrac,Matthew Toews*

Main category: cs.CV

TL;DR: 该论文提出"基本信息力学"模型，将卷积滤波的机械特性与相对论和量子力学原理联系起来，分析偶数和奇数核在信息传播中的作用。


<details>
  <summary>Details</summary>
Motivation: 受狭义相对论和量子力学物理理论的启发，建立理解卷积滤波机械特性的新模型，探索CNN中信息处理与物理能量-动量关系之间的联系。

Method: 将卷积核分解为正交的偶数和奇数分量，分析它们在图像处理中的不同作用；在谱域通过离散余弦变换(DCT)分析偶-奇特性；研究小卷积滤波器(如3×3像素)的结构，重点关注低频基函数(DC Σ和梯度分量∇)。

Result: 偶数核导致图像内容各向同性扩散并保持质心，类比零净动量的静止或势能；奇数核导致质心方向性位移，类比非零动量的动能；信息位移速度与奇数核能量占总能量的比例呈线性关系；小卷积滤波器的结构由低频基函数主导，定义了信息传播的基本模式。

Conclusion: 首次证明通用CNN中信息处理与相对论物理基石——能量-动量关系之间的联系，为理解卷积滤波的机械特性提供了新的物理类比框架。
...
Result: 分割任务中，不同提示策略影响性能：默认提示在Mean-of-Max和Mean-of-Mean评估中表现最佳；增强提示在多数投票评估中表现最好，Jaccard指数0.395，Dice分数0.566。封闭式问答中，o3模型准确率最高(0.798)，GPT-4.1(0.796)紧随其后。
...
TL;DR: 该论文提出了一个用于多模态预训练的综合框架，旨在从摄像头和LiDAR等车载传感器数据中实现真正的空间智能，建立了从单模态基线到统一框架的预训练范式分类体系，并探讨了文本输入和占据表示在开放世界感知中的应用。
...
Method: 论文提出了一个多模态预训练的综合框架，分析了基础传感器特性与学习策略之间的相互作用，评估了平台特定数据集的作用。核心贡献是建立了统一的预训练范式分类体系：从单模态基线到学习整体表示的统一框架，用于3D目标检测和语义占据预测等高级任务。还研究了文本输入和占据表示的整合以促进开放世界感知和规划。

Result: 论文识别了推动多模态预训练进展的核心技术集，分析了传感器特性与学习策略的关系，建立了预训练范式的统一分类体系，并探讨了文本和占据表示在开放世界感知中的应用潜力。

Conclusion: 论文识别了计算效率和模型可扩展性等关键瓶颈，并提出了实现通用多模态基础模型的路线图，这些模型能够为实际部署实现稳健的空间智能。该框架为构建能够统一理解多模态传感器数据的自主系统提供了理论基础和方法指导。
...
TL;DR: DyStream：基于流匹配的自回归模型，用于实时生成双人对话头部视频，延迟低于100ms，实现高质量唇部同步


<details>
  <summary>Details</summary>
Motivation: 现有基于分块的方法需要完整的非因果上下文窗口，引入显著延迟，无法满足真实对话中即时非语言反馈的需求。高延迟阻碍了真实听众体验的实现。

Method: 提出DyStream方法，包含两个关键设计：(1) 采用流友好的自回归框架，配备流匹配头部进行概率建模；(2) 提出由前瞻模块增强的因果编码器，可纳入短期未来上下文（如60ms）以提高质量，同时保持低延迟。

Result: DyStream每帧生成时间仅34ms，确保整个系统延迟低于100ms。在HDTF数据集上，离线和在线LipSync Confidence分数分别达到8.13和7.61，实现最先进的唇部同步质量。

Conclusion: DyStream通过流匹配自回归框架和因果编码器增强设计，成功解决了实时双人对话头部视频生成的低延迟需求，在保持高质量的同时实现了超低延迟，为实时交互应用提供了可行方案。
...
TL;DR: 提出基于AI的微血管吻合术性能自动评估框架，使用改进的TimeSformer视频Transformer和YOLO目标检测，实现手术视频中的动作识别和运动特征分析，为外科培训提供客观、一致的评估。


<details>
  <summary>Details</summary>
Motivation: 传统外科技能评估依赖专家监督，存在主观性、评估者间变异大、耗时耗力等问题，在低收入和中等收入国家难以规模化实施。需要开发自动化、客观的评估方法来提高外科培训的可扩展性和一致性。

Method: 提出AI驱动框架：1) 基于TimeSformer的视频Transformer架构，改进为分层时间注意力和加权空间注意力机制，用于手术视频中的动作识别；2) 使用YOLO目标检测和跟踪方法提取精细运动特征，分析器械运动学；3) 从五个方面评估微血管吻合术技能：整体动作执行、关键操作中的运动质量、一般器械操作等。

Result: 在58个专家标注视频数据集上验证：动作分割帧级准确率达到87.7%，后处理后提升至93.62%；在复制专家评估的所有技能方面，平均分类准确率为76%。系统能够提供客观、一致且可解释的反馈。

Conclusion: 该AI驱动框架能够为微血管吻合术提供自动化、客观的技能评估，有助于实现更标准化、数据驱动的外科教育和培训，特别是在资源有限的环境中具有重要应用价值。
...
TL;DR: 提出基于特征到图像扩散基础模型（F2IDiff FM）的单图像超分辨率方法，使用DINOv2特征作为低层次特征条件，解决传统文本到图像扩散模型在智能手机摄影中过度生成和幻觉问题


<details>
  <summary>Details</summary>
Motivation: 传统基于文本到图像扩散基础模型（T2IDiff FM）的单图像超分辨率方法在学术研究中表现良好，但在智能手机摄影应用中存在两个主要问题：1）文本特征作为高层次特征无法准确描述图像中的细微纹理；2）智能手机LR图像分辨率高（≥12MP），而T2IDiff FM设计用于小图像（<1MP），导致需要在补丁上推理，而文本特征难以准确描述小补丁内容

Method: 提出特征到图像扩散基础模型（F2IDiff FM），使用DINOv2特征作为低层次特征条件。低层次特征提供更严格的约束条件，同时能够丰富描述小图像补丁，实现最小化幻觉的生成

Result: 该方法理论上能够解决智能手机摄影中超分辨率应用中的关键问题：1）避免过度生成和幻觉；2）适应高分辨率LR图像处理；3）准确恢复细微纹理

Conclusion: 通过使用低层次特征（DINOv2特征）替代文本特征作为扩散基础模型的条件，可以更好地控制生成过程，在保持图像保真度的同时实现高质量超分辨率，特别适用于智能手机摄影等需要最小化幻觉的应用场景
...
Method: 引入一个框架，类比先前表示学习中测量组合性的工作，使用离散小波变换获取视觉设置中依赖于输入的基元，通过检查组合表示重构原始图像表示的能力来实证测试表示空间中的组合性程度。

Result: 研究发现，单层离散小波变换分解得到的基元能够产生在潜在空间中近似组合的编码器表示，这为理解ViT如何结构化信息提供了新视角。

Conclusion: Vision Transformer编码器的表示展现出一定程度的组合性，离散小波变换基元在潜在空间中的近似组合特性揭示了ViT结构化视觉信息的新机制。
...
Journal reference: Proceedings of the 2024 International Conference on Machine Learning and Applications (ICMLA), 2025, pp. 607-614

DOI: [10.1109/ICMLA66185.2025.00089](https://doi.org/10.1109/ICMLA66185.2025.00089)

Author affiliations: Prasiddha Siwakoti: West Virginia University; Atefeh Khoshkhahtinat: West Virginia University; Piyush M. Mehta: West Virginia University; B. J. Thompson: Goddard Space Flight Center; Michael S. Kirk: Goddard Space Flight Center; Daniel Oliveira da Silva: Goddard Space Flight Center
...
Method: 提出双模块框架：1) iSWGE模块通过图节点表示光谱通道并学习边缘特征来显式建模波段间关系；2) WSGA-C模块结合稀疏图注意力与卷积注意力来减少空间冗余并强调精细结构

Result: 在SDOML数据集的六个极紫外通道上评估，相比强基线方法，MSID降低20.15%，PSNR提升最高1.09%，log变换MS-SSIM增益1.62%，在相同比特率下提供更清晰且光谱保真的重建

Conclusion: 该学习型图像压缩框架能有效压缩多光谱太阳图像，在保持光谱和空间细节方面优于现有方法，适用于带宽受限的空间任务
...
TL;DR: 该研究提出了一种结合YOLO目标检测模型和大型语言模型的管道，用于从胸部X光图像生成放射学报告，比较了YOLOv5和YOLOv8的性能，并评估了GPT-4生成的报告质量。


<details>
  <summary>Details</summary>
Motivation: 虽然AI在医学影像分类和检测任务中表现出色，但现有系统通常只输出结构化预测，需要放射科医生将其转换为完整的叙述性报告。大型语言模型的出现为弥合这一差距提供了新机会，能够从结构化发现生成诊断性叙述。

Method: 研究引入了一个集成YOLOv5和YOLOv8进行胸部X光图像异常检测的管道，然后将检测结果（边界框预测和类别标签）传递给大型语言模型（如GPT-4）生成描述性发现和临床总结。比较了两种YOLO模型在检测精度、推理延迟和生成文本质量方面的表现。

Result: 结果显示AI生成报告与人工报告具有强语义相似性。人工评估表明GPT-4在清晰度方面表现出色（4.88/5），但在自然写作流畅性方面得分较低（2.81/5）。这表明当前系统能达到临床准确性，但在文体上与放射科医生撰写的文本仍有区别。
...
<details>
  <summary>Details</summary>
Motivation: 现有推理分割方法存在严重缺陷：监督微调（SFT）存在灾难性遗忘和领域依赖问题；强化学习（RL）训练不稳定且依赖预定义奖励函数；训练免费方法受限于静态推理范式，采用单次"生成-分割"链，推理深度不足且无法自我纠正语言幻觉或空间误解。

Method: 提出EVOL-SAM3零样本框架，将推理分割重构为推理时进化搜索过程。维护提示假设种群，通过"生成-评估-进化"循环迭代优化：1）视觉竞技场通过无参考成对竞赛评估提示适应性；2）语义变异算子注入多样性并纠正语义错误；3）异构竞技场模块整合几何先验与语义推理以确保鲁棒最终选择。

Result: 在挑战性ReasonSeg基准测试中，EVOL-SAM3不仅显著超越静态基线方法，在零样本设置下还显著超过完全监督的最先进方法，证明了其优越性能。

Conclusion: EVOL-SAM3通过推理时进化搜索范式成功解决了现有推理分割方法的局限性，提供了一种无需训练、能自我纠正错误、具有深度推理能力的强大框架，为复杂语言查询的像素级定位开辟了新方向。
...
Method: 1) 提出物理增强视频数据管道PhyAugPipe，利用视觉语言模型和思维链推理构建大规模数据集PhyVidGen-135K；2) 设计物理感知分组直接偏好优化框架PhyGDPO，基于Plackett-Luce概率模型捕获整体偏好；3) 引入物理引导奖励方案PGR，嵌入VLM物理奖励引导优化；4) 提出LoRA-Switch Reference方案消除内存密集型参考复制

Result: 在PhyGenBench和VideoPhy2基准测试中显著优于现有开源方法，有效提升了视频生成的物理一致性

Conclusion: 通过构建大规模物理增强数据集和设计物理感知优化框架，成功解决了文本到视频生成中的物理一致性挑战，为物理可信的视频生成提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 针对深度学习中的大规模优化问题，传统优化算法在收敛速度、训练稳定性和噪声鲁棒性方面存在不足，需要更高效稳定的二阶优化方法。

Method: 提出新颖的二阶优化算法，结合OCP方法，对Hessian矩阵的对角元素进行适当近似，以降低计算复杂度同时保持优化性能。

Result: 在多个标准视觉定位基准测试中，该方法相比传统优化算法具有显著优势，达到竞争性的定位精度，同时表现出更快的收敛速度、增强的训练稳定性和改进的噪声干扰鲁棒性。

Conclusion: 所提出的二阶优化算法通过OCP方法和Hessian对角近似，有效解决了深度学习大规模优化问题，在视觉定位任务中展现出优越性能，为相关领域提供了有效的优化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 跨模态系统在2D视觉输入上训练，但在处理3D场景时面临维度差异。场景内相机可以弥合这一差距，但需要学习控制模块。现有方法需要预训练或微调，限制了在3D多物体场景中的适应性。

Method: 通过基于遗憾最小化和无导数优化的多元互信息估计改进方法。该方法利用表达性度量和基于值的优化，控制场景内相机直接从视觉语言模型的噪声输出中学习。使现成的2D视觉跨模态系统能够在线适应物体遮挡和区分特征。

Result: 提出的管道在多物体3D场景的跨模态任务中提高了性能，无需依赖预训练或微调。系统能够有效处理物体遮挡并区分特征，增强了在3D环境中的适应性。

Conclusion: 通过改进的多元互信息估计和基于遗憾最小化的优化，实现了2D视觉跨模态系统向3D场景的有效迁移，解决了维度差异问题，为跨模态系统在复杂3D环境中的应用提供了新途径。
...
TL;DR: 提出DARFT框架，通过决策模糊性引导的强化微调解决CDVQA中的决策模糊样本问题，提升模型判别能力
...
Method: 提出DARFT框架：1) 使用SFT训练的策略挖掘决策模糊样本(DAS)；2) 在挖掘的子集上应用组相对策略优化，利用多样本解码和组内相对优势抑制强干扰项

Result: 实验表明DARFT在SFT基线上获得一致提升，特别是在少样本设置下表现更优，有效提高了模型的判别能力和鲁棒性

Conclusion: 决策模糊样本是CDVQA性能提升的关键瓶颈，DARFT通过强化微调框架有效解决了这一问题，无需额外监督即可锐化决策边界
...
<details>
  <summary>Details</summary>
Motivation: 现有切片发现方法主要针对图像分类，不适用于检测、分割、姿态估计等多实例任务；真实场景中的错误切片常涉及复杂视觉关系，现有实例级方法缺乏细粒度推理能力；当前基准测试偏向特定算法或图像分类，人工标注无法反映真实模型失败

Method: 提出SliceLens框架：利用LLM和VLM生成和验证多样化失败假设，通过基于视觉的推理实现细粒度、可解释错误切片的可靠识别；引入FeSD基准：首个专门评估实例级视觉任务细粒度错误切片发现的基准，包含专家标注的精确到局部错误区域的切片

Result: 在现有基准和FeSD上的广泛实验表明，SliceLens达到最先进性能：在FeSD上Precision@10提升0.42（0.73 vs. 0.31）；识别出的可解释切片有助于可操作的模型改进，模型修复实验验证了其有效性

Conclusion: SliceLens通过LLM/VLM驱动的假设生成和验证，有效解决了实例级视觉任务的细粒度错误切片发现问题；FeSD基准为评估此类方法提供了标准化测试平台；该方法在识别可解释切片和促进模型改进方面表现出显著优势
...
TL;DR: 构建了首个针对飓风灾害后环境的3D语义分割数据集，评估了现有SOTA模型在该场景下的性能，发现现有方法在灾害区域存在显著局限性。
...
Method: 使用无人机采集飓风伊恩（2022）灾区的航拍影像，通过运动恢复结构（SfM）和多视角立体（MVS）技术重建3D点云，构建了专门的3D数据集。在此基础上评估了Fast Point Transformer（FPT）、Point Transformer v3（PTv3）和OA-CNNs等SOTA 3D语义分割模型。

Result: 评估发现现有SOTA模型在灾害区域表现出显著局限性，暴露了当前方法在灾后场景理解方面的不足。
...
Conclusion: CPJ框架无需微调即可实现稳健、可解释的农业诊断，通过结构化描述显著提升小样本VQA性能，为农业决策提供透明推理。
...
Method: 提出协作低秩适应(CLoRA)，包含两个核心组件：1) 基础空间共享：所有低秩模块共享一组下/上投影空间，从共享空间协作构建每个低秩模块；2) 样本无关多样性增强(SADE)：正则化表示之间的相似性，鼓励训练过程中的多样性表示。

Result: 在广泛使用的图像和点云数据集上的实验表明，CLoRA在学习性能和参数效率之间取得了更好的平衡，在点云分析中需要最少的GFLOPs，优于现有最先进方法。

Conclusion: CLoRA通过创新的基础空间共享和多样性增强机制，成功解决了LoRA微调中参数效率与学习性能的权衡问题，为视觉Transformer的高效微调提供了有效解决方案。
...
TL;DR: HaineiFRDM是一个基于扩散模型的电影修复框架，采用分块训练策略处理高分辨率影片，通过全局提示、帧融合和全局-局部频率模块提升修复质量，并构建了包含真实退化影片的修复数据集。


<details>
  <summary>Details</summary>
Motivation: 现有开源电影修复方法性能有限，主要原因是使用低质量合成数据训练和噪声光流估计，且未探索高分辨率影片修复。需要开发更强大的开源解决方案来帮助专家修复难以识别的影片缺陷。

Method: 提出HaineiFRDM扩散模型框架：1)采用分块训练和测试策略，使24GB显存GPU能处理高分辨率影片；2)设计位置感知的全局提示和帧融合模块；3)引入全局-局部频率模块重建不同分块间的一致纹理；4)先修复低分辨率结果作为全局残差缓解分块伪影；5)构建包含真实退化影片和合成数据的修复数据集。

Result: 综合实验结果表明，该模型在缺陷修复能力上显著优于现有开源方法。代码和数据集将公开发布。

Conclusion: HaineiFRDM通过扩散模型的内容理解能力，结合创新的分块策略和模块设计，成功实现了高质量高分辨率电影修复，为开源电影修复领域提供了有效的解决方案。
...
TL;DR: 该论文提出了面向户外监控场景的3D视觉定位新任务，构建了首个真实世界大规模多模态数据集MoniRefer，并提出了端到端方法Moni3DVG。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位研究主要关注室内和室外驾驶场景，缺乏针对路边基础设施监控场景的研究，这是由于缺乏由路边基础设施传感器采集的配对点云-文本数据。基础设施级别的交通场景理解对智能交通系统至关重要。

Method: 提出了Moni3DVG方法，该方法利用图像提供的丰富外观信息以及点云的几何和光学信息进行多模态特征学习和3D物体定位。这是一个端到端的框架。

Result: 构建了MoniRefer数据集，包含约136,018个物体和411,128个自然语言描述，采集自真实世界复杂交通路口。在提出的基准测试上进行了广泛实验和消融研究，证明了方法的优越性和有效性。

Conclusion: 该工作填补了户外监控场景3D视觉定位的研究空白，提出的数据集和方法为基础设施级别的交通场景理解提供了重要基础，数据集和代码将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有消防目标检测研究存在两个主要局限：1）主要关注山区或森林环境，对更频繁且结构复杂的城市救援场景关注不足；2）现有检测系统类别有限（如火焰和烟雾），缺乏覆盖消防车、消防员等指挥决策关键目标的全面系统。

Method: 1）构建FireRescue数据集，涵盖城市、山区、森林和水域多种救援场景，包含消防车、消防员等8个关键类别，共15,980张图像和32,000个边界框；2）提出FRS-YOLO改进模型，引入即插即用的多维协同增强注意力模块通过跨维度特征交互增强易混淆类别的判别表示，并集成动态特征采样器强化高响应前景特征以减轻烟雾遮挡和背景干扰。

Result: 实验结果表明，消防救援场景中的目标检测具有高度挑战性，所提方法有效提升了YOLO系列模型在此场景下的检测性能。

Conclusion: 本文通过构建全面数据集和改进检测模型，解决了消防救援场景目标检测中的场景覆盖不足和类别有限问题，为消防指挥决策提供了更有效的技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要在理想光照条件下评估视觉语言模型，但实际应用需要模型在低光照等视觉退化条件下保持性能，这一需求尚未得到充分研究。

Method: 开发DarkEQA基准测试，在RAW线性空间模拟物理真实的照明衰减和传感器噪声，采用ISP渲染流程生成多级低光照条件下的视觉退化数据，评估模型在自我中心视角下的问答能力。
...
Method: 提出径向拓扑生成框架：1）选择初始token作为中心点；2）根据空间距离将所有token分组为多个同心环；3）按从内到外的环状顺序并行预测同一环内的所有token；4）引入嵌套注意力机制动态修正不一致预测，防止误差累积。

Result: RadAR在保持自回归模型表征能力的同时，显著提高了生成效率。通过环状并行预测实现了更高的并行化程度，嵌套注意力机制有效缓解了有限上下文下同时生成token可能导致的不一致性问题。

Conclusion: RadAR通过创新的径向拓扑设计和动态输出修正机制，成功解决了传统自回归视觉生成中的效率瓶颈问题，为高效并行化视觉生成提供了新思路。
...
TL;DR: SpaceTimePilot是一个视频扩散模型，能够解耦空间和时间进行可控生成渲染，允许独立控制摄像机视角和运动序列


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型难以实现对动态场景中空间（摄像机视角）和时间（运动序列）的独立精确控制，缺乏能够同时处理空间和时间变化的可控生成方法

Method: 1) 引入动画时间嵌入机制在扩散过程中显式控制输出视频的运动序列；2) 提出时间扭曲训练方案，利用现有多视角数据集模拟时间差异；3) 改进摄像机条件机制，允许从第一帧开始改变摄像机；4) 创建CamxTime合成数据集，提供完全自由的空间-时间视频轨迹

Result: 在真实世界和合成数据上评估显示，SpaceTimePilot实现了清晰的空间-时间解耦，相比先前工作取得了显著改进，能够生成连续且任意探索空间和时间的重新渲染场景

Conclusion: SpaceTimePilot通过创新的时间嵌入机制、训练策略和数据集构建，成功实现了视频生成中空间和时间的解耦控制，为可控生成渲染提供了新方法
...
TL;DR: Splatwizard是一个专门用于评估3D高斯泼溅压缩模型的统一基准工具包，解决了现有评估工具在渲染速度、率失真权衡、内存效率和几何精度等特定指标方面的不足。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术的快速发展导致了对标准化、全面评估工具的迫切需求，特别是在压缩任务方面。现有基准往往缺乏评估不同方法独特特征所需的特定指标。

Method: 开发了Splatwizard工具包，提供了一个易于使用的框架来实施新的3DGS压缩模型并利用现有技术。该框架包含一个集成管道，可自动计算关键性能指标。

Result: Splatwizard提供了统一的评估框架，能够自动计算图像质量指标、重建网格的倒角距离、渲染帧率和计算资源消耗等关键性能指标。

Conclusion: Splatwizard填补了3DGS压缩模型评估领域的空白，为研究人员提供了一个标准化、全面的基准工具包，有助于推动该领域的发展。
...
TL;DR: 提出统一框架，将3D高斯泼溅与可学习特征嵌入结合，通过"嵌入到标签"过程实现实例分割，并引入边界硬挖掘和线性层稳定训练，在多个数据集上超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D实例分割方法面临两个主要挑战：1）多视图2D实例标签的不一致性导致3D预测质量差；2）现有方法采用两阶段流程，要么依赖超参数敏感的对比学习聚类，要么需要预处理标签以确保一致性。这些方法训练时间长且性能受限。

Method: 1）提出统一框架，将特征学习和标签预测合并为单阶段过程；2）为高斯基元引入可学习特征嵌入进行分割；3）设计"嵌入到标签"过程，高效解码嵌入为实例标签；4）针对边界伪影问题，提出边界硬挖掘策略；5）在计算三元组损失前，对光栅化特征嵌入应用线性层以稳定训练。

Result: 在ScanNet、Replica3D和Messy-Rooms数据集上，该方法在定性和定量评估中均优于基线方法。统一框架减少了训练时间，边界硬挖掘和线性层稳定策略显著提升了性能，特别是在对象边界区域。

Conclusion: 该方法成功解决了多视图2D实例标签不一致性问题，通过统一框架简化了训练流程，提高了3D实例分割的准确性和效率。边界优化策略有效改善了对象边界的分割质量，为3D场景理解提供了更鲁棒的解决方案。
...
TL;DR: CropTrack是一个用于农业环境多目标跟踪的新框架，通过结合外观和运动信息解决重复模式、相似外观和频繁遮挡问题，在身份保持方面优于传统运动跟踪方法。


<details>
  <summary>Details</summary>
Motivation: 农业环境中的多目标跟踪面临重复模式、相似外观、突然光照变化和频繁遮挡等挑战。现有跟踪器主要依赖运动信息进行关联，但在目标频繁强遮挡时难以保持身份。由于目标外观高度相似，将外观关联整合到农业场景中具有挑战性。

Method: 提出CropTrack框架，结合外观和运动信息：1）重新排序增强的外观关联；2）一对多关联配合基于外观的冲突解决策略；3）指数移动平均原型特征库以改进基于外观的关联。

Result: 在公开农业MOT数据集上的评估显示，CropTrack在身份保持方面表现一致，优于传统基于运动的跟踪方法。与最先进方法相比，CropTrack在识别F1分数和关联准确率方面取得显著提升，同时身份切换次数更低。

Conclusion: CropTrack通过有效整合外观和运动信息，解决了农业环境中多目标跟踪的关键挑战，特别是在频繁遮挡情况下保持目标身份的能力，为农业MOT提供了更鲁棒的解决方案。
...
TL;DR: 本文提出了VLN-MME评估框架，用于在视觉语言导航任务中系统评估多模态大语言模型作为零样本智能体的能力，发现增强推理链和自我反思反而导致性能下降，揭示了MLLM在具身导航中上下文意识和3D空间推理的局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在各种视觉语言任务中表现出色，但其作为具身智能体（需要多轮对话空间推理和序列动作预测）的性能仍需探索。本文旨在通过构建统一评估框架，系统评估MLLM在视觉语言导航任务中的零样本能力。

Method: 提出VLN-MME评估框架，将传统导航数据集标准化为统一基准，采用高度模块化和可访问的设计。通过该框架对不同MLLM架构、智能体设计和导航任务进行结构化比较和组件级消融实验，特别测试了推理链和自我反思增强对基线智能体的影响。

Result: 研究发现，增强基线智能体（添加推理链和自我反思）反而导致性能下降，这表明MLLM在具身导航任务中表现出较差的上下文意识。虽然MLLM能够遵循指令并结构化输出，但其3D空间推理保真度较低。

Conclusion: VLN-MME为系统评估通用MLLM在具身导航场景中的能力奠定了基础，揭示了其在序列决策能力方面的局限性。这些发现为MLLM作为具身智能体的后训练提供了重要指导。
...
Journal reference: Proc. AAAI 2026 (Vol. 40, No. 30), pages 25858-25866

DOI: [10.1609/aaai.v40i30.39785](https://doi.org/10.1609/aaai.v40i30.39785)

Author affiliations: Zichen Tang: Beijing University of Posts and Telecommunications; Haihong E: Beijing University of Posts and Telecommunications; Rongjin Li: Beijing University of Posts and Telecommunications; Jiacheng Liu: Beijing University of Posts and Telecommunications; Linwei Jia: Beijing University of Posts and Telecommunications; Zhuodi Hao: Beijing University of Posts and Telecommunications; Zhongjun Yang: Beijing University of Posts and Telecommunications; Yuanze Li: Beijing University of Posts and Telecommunications; Haolin Tian: Beijing University of Posts and Telecommunications; Xinyi Hu: Beijing University of Posts and Telecommunications; Peizhi Zhao: Beijing University of Posts and Telecommunications; Yuan Liu: Beijing University of Posts and Telecommunications; Zhengyu Wang: Beijing University of Posts and Telecommunications; Xianghe Wang: Beijing University of Posts and Telecommunications; Yiling Huang: Beijing University of Posts and Telecommunications; Ruofei Bai: Beijing University of Posts and Telecommunications; Zijian Xie: Beijing University of Posts and Telecommunications; Qian Huang: Beijing University of Posts and Telecommunications; Ruining Cao: Beijing University of Posts and Telecommunications; Haocheng Gao: Beijing University of Posts and Telecommunications

TL;DR: FinMMDocR是一个双语多模态金融数值推理基准，包含1200个专家标注问题、837份中英文金融文档，要求模型进行平均11步推理，最佳MLLM准确率仅58.0%


<details>
  <summary>Details</summary>
Motivation: 现有基准在金融领域的多模态推理评估存在不足，需要创建更贴近真实金融场景的评估基准，包含复杂的金融场景、丰富的文档类型和多步骤推理需求

Method: 构建包含1200个专家标注问题的双语多模态基准，其中57.9%问题融入12种隐含金融场景；使用837份中英文金融文档（平均50.8页），涵盖9种文档类型；设计需要平均11步推理（5.3步信息提取+5.7步计算）的问题

Result: 最佳表现的多模态大语言模型准确率仅为58.0%，不同检索增强生成方法在该任务上表现出显著性能差异，65.0%的问题需要跨页证据（平均2.4页）

Conclusion: FinMMDocR基准填补了金融领域多模态数值推理评估的空白，能够有效评估MLLMs在复杂真实场景下的推理能力，预期将推动MLLMs和推理增强方法在复杂多模态推理任务上的改进
...
<details>
  <summary>Details</summary>
Motivation: 3D目标检测器在自动驾驶感知系统中至关重要，但在标准基准上表现优异的模型往往难以跨域泛化（如美国训练的模型在亚洲或欧洲表现不佳）。现有方法需要大量目标域标注数据，成本高昂。

Method: 基于神经元激活模式选择目标域中少量代表性且多样化的样本进行标注，结合持续学习启发的后训练技术防止权重漂移，实现高效域自适应。

Result: 实验评估表明，该方法在少量标注预算下超越了线性探测和现有SOTA域自适应技术，实现了优异的跨域性能。

Conclusion: 通过精心选择少量代表性样本并采用持续学习技术，可以在极低标注成本下实现有效的LiDAR域自适应，为自动驾驶系统的跨域部署提供实用解决方案。
...
TL;DR: ProDM：一种基于扩散模型的生成框架，用于从非门控胸部CT中恢复无运动伪影的钙化病灶，提高冠状动脉钙化评分准确性


<details>
  <summary>Details</summary>
Motivation: 冠状动脉钙化评分是心血管疾病风险评估的重要工具，但非门控胸部CT存在严重运动伪影，影响钙化病灶准确量化。虽然心电门控心脏CT能减少伪影，但由于门控要求和保险覆盖限制，在人群筛查和常规成像中应用有限。需要开发能够从非门控CT中恢复无运动钙化病灶的方法。

Method: 提出ProDM（Property-aware Progressive Correction Diffusion Model）生成扩散框架，包含三个关键组件：1）CAC运动模拟数据引擎，从心脏门控CT合成具有多样运动轨迹的现实非门控采集数据，实现无配对数据的监督训练；2）属性感知学习策略，通过可微钙化一致性损失融入钙化特异性先验，保持病灶完整性；3）渐进校正方案，在扩散步骤中逐步减少伪影，增强稳定性和钙化保真度。

Result: 在真实患者数据集上的实验表明，ProDM相比多个基线方法显著提高了CAC评分准确性、空间病灶保真度和风险分层性能。对真实非门控扫描的读者研究进一步证实，ProDM能有效抑制运动伪影并提高临床可用性。

Conclusion: ProDM框架展示了渐进、属性感知方法在从常规胸部CT成像中进行可靠CAC量化方面的潜力，为非门控CT中的冠状动脉钙化评估提供了有效解决方案。
...
TL;DR: VIPER：首个面向生成式视频推理的过程感知评估基准，包含16个任务和新的POC@r指标，揭示当前视频模型存在约20%的过程-结果一致性，存在显著的结果黑客问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型展现出链式帧推理能力，但评估框架多依赖单帧评估，导致模型可能通过错误过程得出正确结论（结果黑客），缺乏对推理过程的系统性评估。

Method: 提出过程感知评估范式，构建VIPER基准（16个任务覆盖时间、结构、符号、空间、物理和规划推理），设计过程-结果一致性指标POC@r，使用VLM-as-Judge和分层评分标准评估中间步骤有效性和最终结果。

Result: 实验表明当前最先进的视频模型仅达到约20%的POC@1.0一致性，存在显著的结果黑客现象；测试时缩放和采样鲁棒性分析揭示了当前视频生成与真正广义视觉推理之间的巨大差距。

Conclusion: 需要过程感知评估来准确衡量生成式视频推理能力，VIPER基准为系统性评估提供了必要工具，揭示了当前模型的局限性，为未来研究指明了方向。
...
TL;DR: 论文提出了一种无需重新索引历史图库图像的双向连续兼容表示（Bi-C2R）框架，用于解决终身行人重识别中的特征兼容性问题。
...
Method: 提出双向连续兼容表示（Bi-C2R）框架，通过持续更新旧模型提取的图库特征，实现新旧模型特征之间的兼容性，从而在无需重新索引历史图库的情况下进行高效的终身行人重识别。

Result: 通过理论分析和多个基准测试的广泛实验验证，Bi-C2R方法在提出的RFL-ReID任务和传统L-ReID任务上均取得了领先性能。

Conclusion: 该研究提出了无需重新索引的终身行人重识别新任务（RFL-ReID），并开发了有效的Bi-C2R框架解决特征兼容性问题，为实际应用中的终身学习系统提供了实用解决方案。
...
TL;DR: 提出自引导框架将视觉配音从不适定修复任务重构为条件良好的视频编辑问题，使用DiT生成理想训练数据，训练音频驱动编辑器实现精确唇形同步和身份保持


<details>
  <summary>Details</summary>
Motivation: 音频驱动视觉配音面临理想训练数据缺失的根本挑战：现有基于掩码修复的方法需要同时生成缺失内容和同步唇形，导致视觉伪影、身份漂移和同步性差

Method: 1) 使用Diffusion Transformer作为数据生成器合成理想训练数据（唇形改变的伴生视频）；2) 在生成的对齐视频对上端到端训练DiT基音频驱动编辑器；3) 引入时间步自适应多阶段学习策略解耦编辑目标；4) 提出ContextDubBench基准数据集

Result: 方法实现了高度准确的唇形同步、忠实的身份保持，并在具有挑战性的野外场景中表现出卓越的鲁棒性，通过完整对齐的输入条件提供了丰富的视觉上下文

Conclusion: 将视觉配音重构为条件良好的视频编辑问题，通过自引导框架生成理想训练数据，结合多阶段学习策略，显著提升了唇形同步精度、身份保持和视觉保真度
...
TL;DR: FineTec：一种用于时间损坏骨架序列细粒度动作识别的统一框架，通过上下文感知补全、语义区域分解和物理驱动加速度估计来恢复丢失的时空信息


<details>
  <summary>Details</summary>
Motivation: 解决现实场景中在线姿态估计常产生大量缺失数据的问题，现有方法难以准确恢复时间动态和细粒度空间结构，导致丢失区分相似动作的关键细微运动线索

Method: 1) 上下文感知补全：通过多样化时间掩码从损坏输入恢复基础骨架序列；2) 骨架空间分解：将骨架分为五个语义区域，基于运动方差进一步划分为动态和静态子组，通过针对性扰动生成两个增强骨架序列；3) 物理驱动估计：利用拉格朗日动力学估计关节加速度；4) 融合骨架位置序列和加速度序列输入基于GCN的动作识别头

Result: 在粗粒度(NTU-60, NTU-120)和细粒度(Gym99, Gym288)基准测试中显著优于先前方法，在挑战性的Gym99-severe和Gym288-severe设置下分别达到89.1%和78.1%的top-1准确率

Conclusion: FineTec通过统一框架有效处理时间损坏的骨架序列，在细粒度动作识别中表现出优异的鲁棒性和泛化能力，为现实场景中的动作识别提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的方法在稀疏视图3D重建中存在三个关键局限：已知视图外围覆盖不足、生成视图间几何不一致、计算流程昂贵。需要一种能保持几何一致性同时扩展场景覆盖的高效方法。

Method: GaMO框架将稀疏视图重建重新定义为多视角外绘问题，从现有相机位姿扩展视野而非生成新视角。采用多视角条件化和几何感知去噪策略，以零样本方式运行无需训练。

Result: 在Replica和ScanNet++数据集上，GaMO在3、6、9个输入视图下均达到最先进的重建质量，PSNR和LPIPS指标优于先前方法，同时比SOTA扩散方法快25倍，处理时间低于10分钟。

Conclusion: GaMO通过多视角外绘范式有效解决了稀疏视图重建中的几何一致性和覆盖范围问题，在保持高质量的同时显著提升计算效率，为实际应用提供了可行方案。
...
### [77] [PyBangla at BLP-2025 Task 2: Enhancing Bangla-to-Python Code Generation with Iterative Self-Correction and Multilingual Agents](https://arxiv.org/abs/2512.23713)
...
TL;DR: BanglaCodeAct：基于多智能体提示和迭代自校正的孟加拉语到Python代码生成框架，在低资源语言代码生成中取得显著效果


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语提示的代码生成方面表现出色，但这种进展尚未扩展到低资源语言。针对孟加拉语到Python代码生成的需求，需要开发专门的方法来解决这一语言资源不足的问题。

Method: 提出BanglaCodeAct框架，采用基于智能体的方法，利用多智能体提示和迭代自校正。该方法不依赖任务特定的微调，而是在Thought-Code-Observation循环中使用开源多语言LLM，实现从孟加拉语指令动态生成、测试和优化代码。

Result: 在mHumanEval数据集上评估多个小型开源LLM，结果显示Qwen3-8B在BanglaCodeAct框架下表现最佳：开发集pass@1准确率达到94.0%，盲测集达到71.6%，为孟加拉语到Python翻译建立了新的基准。

Conclusion: 该研究为孟加拉语到Python代码生成设立了新的性能基准，证明了基于智能体的推理方法在低资源语言可靠代码生成方面的潜力，为其他低资源语言的代码生成提供了参考框架。
...
TL;DR: PharmaShip是一个用于测试预训练文本布局模型的中文药品运输单据数据集，包含序列实体识别、关系抽取和阅读顺序预测三个任务，通过实体中心评估协议和标准化基准测试，发现像素和几何信息具有互补性，而阅读顺序正则化能提升性能。


<details>
  <summary>Details</summary>
Motivation: 该研究的动机是创建一个真实世界的中文药品运输单据数据集，用于在噪声OCR和异构模板条件下压力测试预训练文本布局模型，为安全关键领域的文档理解建立可控、可复现的基准。

Method: 构建PharmaShip数据集，包含扫描的药品运输单据，涵盖序列实体识别、关系抽取和阅读顺序预测三个任务。采用实体中心评估协议，对五个代表性基线模型（LiLT、LayoutLMv3-base、GeoLayoutLM及其RORE增强变体）进行标准化预处理、数据划分和优化。通过实验分析像素信息和几何信息的互补性，以及阅读顺序正则化的效果。

Result: 实验表明：1）像素信息和显式几何信息提供互补的归纳偏置，但单独使用都不足够；2）注入阅读顺序导向的正则化能持续提升序列实体识别和实体链接性能，形成最鲁棒的配置；3）更长的位置覆盖能稳定页面后期预测并减少截断伪影；4）阅读顺序预测在词级别准确但在片段级别具有挑战性，反映了边界模糊性和长距离跨越问题。

Conclusion: PharmaShip为药品领域的安全关键文档理解建立了可控、可复现的基准，并强调序列感知约束作为结构建模的可迁移偏置的重要性。数据集已公开发布，为未来研究提供了标准化测试平台。
...
Method: 引入Luca-Noise Reflex Protocol (LN-RP)框架，通过向初始生成状态注入随机噪声种子，在152个生成周期中观察语言行为的非线性转变。

Result: 发现了三种具有不同熵特征的稳定人格模式，外部噪声源能够可靠地诱导反射性生成动态中的相变，定量评估确认了人格保持的一致性以及模式间的显著差异(p < 0.01)。
...
TL;DR: HarmTransform：一个多智能体辩论框架，用于将有害查询系统性地转化为更隐蔽的形式，以改进LLM安全对齐训练数据
...
Method: 引入HarmTransform多智能体辩论框架，通过多个智能体之间的迭代批评和精炼，系统地将有害查询转化为隐蔽形式

Result: 实验表明HarmTransform在生成有效查询转换方面显著优于标准基线；辩论具有双重作用：能提升转换质量和隐蔽性，但也可能引入主题偏移和不必要的复杂性

Conclusion: 多智能体辩论在生成全面安全训练数据方面既有潜力也有局限性，为改进LLM安全对齐提供了新方法
...
TL;DR: LLMs在德州扑克中学习到了不完全信息环境下的世界模型，包括确定性的手牌等级和随机性的胜率特征，其内部表征与理论信念状态相关。


<details>
  <summary>Details</summary>
Motivation: 扩展LLMs在完美信息游戏中涌现世界模型的研究到不完全信息领域，以德州扑克作为典型的POMDP环境，探究LLMs是否能在不完全信息游戏中学习环境表征。

Method: 在扑克手牌历史数据上预训练GPT风格模型，使用非线性探针分析其内部激活，解码模型学习到的表征并与理论信念状态进行相关性分析。

Result: 模型在没有明确指导的情况下学习了确定性的手牌等级结构和随机性的胜率特征，这些表征可通过非线性探针解码，且与理论信念状态显著相关。

Conclusion: LLMs能够在德州扑克这样的不完全信息随机环境中学习到自己的环境表征，这表明LLMs具备在复杂POMDP中构建世界模型的能力。
...
TL;DR: 提出两阶段框架应对微妙性别歧视内容检测的三大挑战：数据不足、噪声和概念模糊性，通过针对性训练和基于推理的选择性推断实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 在线性别歧视内容日益微妙且依赖上下文，传统检测方法难以应对。其解释涉及语言、心理、法律和文化等多维度重叠，产生混合甚至矛盾信号，加上标签稀缺和类别不平衡，导致决策边界不稳定，模型忽视微妙、代表性不足的伤害形式

Method: 提出两阶段框架：1) 针对性训练程序：使用类别平衡焦点损失、类别感知批处理和事后阈值校准来缓解标签不平衡和噪声监督；2) 选择性推理推断：动态路由机制直接分类高置信度案例，将不确定实例升级到新颖的"协作专家判断"模块，该模块提示多个人格并通过法官模型整合其推理

Result: 在多个基准测试中取得最先进结果：EXIST 2025 Task 1.1上F1提升+2.72%，EDOS Tasks A和B分别提升+4.48%和+1.30%

Conclusion: 该框架通过统一针对性训练和选择性推理推断，有效解决了微妙性别歧视内容检测中的数据不足、噪声和概念模糊性三大挑战，显著提升了检测性能
...
<details>
  <summary>Details</summary>
Motivation: 传统推测解码中草稿模型与目标模型过度对齐，限制了性能只能达到目标模型水平。需要一种方法既能加速推理，又能超越目标模型固有性能

Method: 在标准推测解码基础上引入动态熵惩罚机制：1) 计算采样分布的熵量化模型不确定性；2) 当两个模型都呈现高熵且top-N预测有显著重叠时，拒绝相应token并由目标LLM重新采样；3) 通过草稿模型验证实现性能超越

Result: 在多个推理基准测试中，EASD始终优于现有SD方法，在大多数情况下甚至超越目标LLM本身。效率与标准SD相当

Conclusion: EASD是一种无需训练的方法，通过熵感知机制改进推测解码，不仅加速推理还能超越目标模型性能，为LLM推理优化提供了新思路
...
TL;DR: MiMo-Audio通过将音频预训练数据扩展到超过1亿小时，实现了音频任务的少样本学习能力，在多个音频理解基准测试中达到开源SOTA，并能泛化到训练数据中未包含的任务。


<details>
  <summary>Details</summary>
Motivation: 现有音频语言模型通常需要针对特定任务进行微调，而人类能够通过少量示例或简单指令泛化到新的音频任务。GPT-3表明扩展下一个标记预测预训练能够在文本领域实现强大的泛化能力，作者认为这一范式同样适用于音频领域。

Method: 1. 将MiMo-Audio的预训练数据扩展到超过1亿小时音频数据；2. 开发系统评估框架来评估少样本学习能力；3. 在后期训练阶段，策划多样化的指令调优语料库；4. 在音频理解和生成中引入思考机制。

Result: 1. MiMo-Audio-7B-Base在语音智能和音频理解基准测试中达到开源SOTA；2. 能够泛化到训练数据中未包含的任务（语音转换、风格迁移、语音编辑）；3. 展现出强大的语音延续能力，能生成高度逼真的谈话节目、朗诵、直播和辩论；4. MiMo-Audio-7B-Instruct在音频理解基准（MMSU、MMAU、MMAR、MMAU-Pro）、口语对话基准（Big Bench Audio、MultiChallenge Audio）和指令TTS评估中达到开源SOTA，接近或超越闭源模型。

Conclusion: 通过大规模音频预训练数据扩展，MiMo-Audio实现了类似GPT-3的少样本学习能力，在音频领域展现出强大的泛化性能，为通用音频智能系统的发展提供了重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 虽然自动偏见检测在新闻分析和媒体问责中广泛应用，但人们对偏见检测模型如何做出决策以及为何失败知之甚少，需要理解不同模型架构如何操作化语言偏见

Method: 使用SHAP解释方法对两种基于Transformer的偏见检测模型进行可解释性比较研究：一个是在BABE数据集上微调的偏见检测器，另一个是在BABE数据集上微调的领域自适应预训练RoBERTa模型。分析正确和错误预测中的词级归因，以表征不同模型架构如何操作化语言偏见

Result: 两种模型都关注相似的评价语言类别，但在如何将这些信号整合到预测中存在显著差异。偏见检测器模型为假阳性分配比真阳性更强的内部证据，表明归因强度与预测正确性不一致，导致系统性地过度标记中性新闻内容。相比之下，领域自适应模型的归因模式与预测结果更一致，产生63%更少的假阳性。模型错误源于不同的语言机制，假阳性由话语层面的模糊性而非显性偏见线索驱动

Conclusion: 研究强调了可解释性感知评估对偏见检测系统的重要性，表明架构和训练选择对模型可靠性及在新闻背景下的部署适用性具有关键影响
...
TL;DR: 自适应分块提示策略在检索增强问答中平衡相关性与无关信息，减少token使用同时保持性能


<details>
  <summary>Details</summary>
Motivation: 大语言模型扩展上下文窗口虽然便于融入目标知识，但会引入更多无关信息，干扰生成过程并降低性能

Method: 设计自适应提示策略：将检索信息分割成较小块，顺序提示LLM使用每个块回答问题，通过调整块大小平衡相关信息与无关信息

Result: 在三个开放域问答数据集上的实验表明，自适应策略在使用更少token的同时，性能与标准提示相当；分析发现LLM在信息不足时倾向于生成错误答案而非拒绝回答，这是主要错误来源

Conclusion: 自适应分块提示策略能有效平衡检索信息的相关性与无关性，同时揭示了需要进一步研究增强LLM在信息不足时有效拒绝回答的能力
...
<details>
  <summary>Details</summary>
Motivation: 基于机制可解释性研究发现中间注意力层编码token级假设并迭代优化，探索利用注意力层token分布生成对抗样本，相比提示或梯度攻击更符合模型内部生成过程

Method: 从中间注意力层提取token分布作为对抗扰动，在ArgQuality数据集上使用LLaMA-3.1-Instruct-8B作为生成器和评估器进行实验

Result: 注意力层对抗样本能显著降低评估性能且保持语义相似性，但某些层和位置的替换会导致语法退化，限制实际应用效果

Conclusion: 中间层表示作为对抗样本来源具有潜力但存在局限性，可用于压力测试LLM评估流程，但需解决语法退化问题以提高实用性
...
TL;DR: 该研究通过多检索器RAG系统结合最新LLM解决金融数值推理QA任务中的领域知识缺失问题，在FinQA基准上实现了SOTA性能，但仍有改进空间。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型取得进展，金融数值问题仍然具有挑战性，因为它们需要特定的金融领域知识和复杂的多步数值推理。现有模型因缺乏领域知识而在金融数值推理QA任务中表现不佳。

Method: 采用多检索器检索增强生成(RAG)系统，同时检索外部领域知识和内部问题上下文，并利用最新的大型语言模型处理这些任务。通过SecBERT编码器进行领域特定训练，构建神经符号模型。

Result: 领域特定训练的神经符号模型超越了FinQA论文的顶级模型（作为基线）。最佳基于提示的LLM生成器实现了最先进的性能，显著改进超过7%，但仍低于人类专家水平。

Conclusion: 领域特定训练具有潜在优越性能；较小模型存在幻觉损失与外部知识增益的权衡，而较大模型的外部事实增益通常超过幻觉损失；最新LLM在少样本学习优化的数值推理能力得到增强。
...
DOI: [10.1145/3785022.3785042](https://doi.org/10.1145/3785022.3785042)

Author affiliations: Conrad Borchers: Carnegie Mellon University; Manit Rajendrakumar Patel: Carnegie Mellon University; Seiyon M. Lee: University of Florida; Anthony F. Botelho: University of Florida

TL;DR: 论文提出分析优先框架，分离学生回答内容信号与教师评分倾向，通过动态先验和文本嵌入建模，量化各信号贡献并可视化模型分歧，提升评分透明度和可审计性。


<details>
  <summary>Details</summary>
Motivation: 开放式回答是学习的核心，但自动评分常混淆学生回答内容与教师评分倾向。需要分离内容信号与评分者倾向，使评分判断可见且可审计。

Method: 使用ASSISTments数学回答数据，将教师历史建模为动态先验，从句子嵌入获取文本表示，采用中心化和残差化处理提示和教师混杂因素。使用时序验证线性模型量化各信号贡献，通过投影可视化模型分歧。

Result: 教师先验对评分预测影响显著；先验与内容嵌入结合效果最佳(AUC~0.815)，纯内容模型虽高于随机但较弱(AUC~0.626)。调整评分者效应可锐化残差内容表示，保留更多信息维度，揭示语义证据支持理解而非表面回答差异的情况。

Conclusion: 该框架将嵌入从特征转化为反思性学习分析工具，使教师和研究者能够检查评分实践与学生推理证据的一致性或冲突，提升评分透明度和教育分析质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统中的记忆模块主要作为被动存储，仅积累孤立事实用于输入压缩和子查询生成，忽视了原始事实间的高阶关联。这种静态设计限制了表示能力和对多步推理的影响，导致碎片化推理和弱全局理解能力。

Method: 提出HGMem超图记忆机制，将记忆表示为超图结构，其中超边对应不同的记忆单元。该机制能够渐进形成记忆内的高阶交互，连接围绕核心问题的事实和思考，演化成集成的、情境化的知识结构，为后续步骤的深度推理提供有力支持。

Result: 在多个为全局理解设计的挑战性数据集上进行评估，广泛实验和深入分析表明，HGMem方法持续改进多步RAG性能，在多样化任务上显著优于强基线系统。

Conclusion: HGMem将记忆概念从简单存储扩展到动态、表达性强的结构，通过超图表示捕捉高阶关联，有效提升多步检索增强生成系统的复杂推理和全局理解能力。
...
### [91] [Efficient Context Scaling with LongCat ZigZag Attention](https://arxiv.org/abs/2512.23966)
...
<details>
  <summary>Details</summary>
Motivation: 解决长上下文场景中全注意力模型计算成本高的问题，为检索增强生成和工具集成推理等应用提供高效的长序列处理能力

Method: 提出LongCat ZigZag Attention (LoZA)稀疏注意力方案，通过特定模式减少注意力计算复杂度，应用于LongCat-Flash模型进行中期训练

Result: LoZA在预填充密集和解码密集场景均实现显著加速，LongCat-Flash-Exp模型可高效处理长达100万token的序列，支持长时推理和长视野智能体能力

Conclusion: LoZA为长上下文模型提供了一种有效的稀疏化方案，在有限计算预算下实现了长序列处理的高效加速，推动了长时推理和智能体能力的发展
...
### [92] [CEC-Zero: Zero-Supervision Character Error Correction with Self-Generated Rewards](https://arxiv.org/abs/2512.23971)
...
TL;DR: CEC-Zero：一种无需监督的强化学习框架，通过让大语言模型自我纠错来解决中文拼写纠错问题，在9个基准测试中显著超越监督基线
...
Method: CEC-Zero采用零监督强化学习框架：1) 从干净文本合成错误输入；2) 通过语义相似度和候选一致性计算聚类共识奖励；3) 使用PPO优化策略。框架具有无偏奖励和收敛的理论保证。

Result: 在9个基准测试中，CEC-Zero比监督基线高出10-13个F1分数，比强LLM微调方法高出5-8个F1分数，表现出卓越性能。

Conclusion: CEC-Zero建立了一种无需标注的范式，用于鲁棒、可扩展的中文拼写纠错，释放了大语言模型在噪声文本处理中的潜力。
...
### [93] [Fantastic Reasoning Behaviors and Where to Find Them: Unsupervised Discovery of the Reasoning Process](https://arxiv.org/abs/2512.23988)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工定义概念（如过度思考、反思）在词级别监督分析推理，但难以捕捉完整的推理行为谱系，许多行为在token空间中难以定义

Method: 提出RISE框架：将思维链轨迹分割为句子级"步骤"，在步骤级激活上训练稀疏自编码器（SAEs），发现编码不同推理行为的推理向量

Result: SAEs发现了解耦特征对应可解释行为（如反思、回溯），这些行为在解码器列空间中占据可分离区域；对SAE派生向量进行干预可可控地放大或抑制特定推理行为；SAEs还能捕获结构特性（如响应长度）并发现超越人类监督的新行为

Conclusion: 无监督潜在发现方法在解释和可控引导LLM推理方面具有潜力，SAEs能够发现人类未定义的新行为并实现可控干预
...
### [94] [WISE: Web Information Satire and Fakeness Evaluation](https://arxiv.org/abs/2512.24000)
...
TL;DR: WISE框架评估8个轻量级Transformer模型在区分假新闻与讽刺内容上的性能，发现MiniLM准确率最高(87.58%)，RoBERTa-base ROC-AUC最佳(95.42%)，DistilBERT在效率与准确率间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 区分假新闻与讽刺/幽默内容具有挑战性，因为两者语言特征重叠但意图不同。需要开发能在资源受限环境中部署的高效检测系统。

Method: 开发WISE框架，在平衡的20,000样本数据集(Fakeddit)上评估8个轻量级Transformer模型和2个基线模型。使用分层5折交叉验证，评估准确率、精确率、召回率、F1分数、ROC-AUC、PR-AUC、MCC、Brier分数和预期校准误差等指标。通过配对t检验和McNemar检验进行统计显著性比较。

Result: MiniLM准确率最高(87.58%)，RoBERTa-base ROC-AUC最佳(95.42%)且准确率接近(87.36%)。DistilBERT在效率与准确率间平衡良好(86.28%准确率，93.90% ROC-AUC)。统计检验证实模型间存在显著性能差异。轻量级模型可匹配或超越基线性能。

Conclusion: 轻量级Transformer模型在假新闻与讽刺内容检测任务中表现出色，为资源受限环境中的错误信息检测系统部署提供了可行方案。模型选择需权衡准确率、计算效率和特定应用需求。
...
### [95] [iCLP: Large Language Model Reasoning with Implicit Cognition Latent Planning](https://arxiv.org/abs/2512.24014)
*Sijia Chen,Di Niu*
...
TL;DR: iCLP框架让LLM在潜在空间进行隐式规划，在语言空间进行推理，通过向量量化自编码器学习紧凑的潜在计划表示，显著提升数学推理和代码生成任务的准确性和效率


<details>
  <summary>Details</summary>
Motivation: 现有基于显式文本计划的LLM推理面临两个主要挑战：1）LLM幻觉导致计划不准确；2）任务特定问题的高度多样性使得生成有效计划困难。受人类潜意识认知启发，需要更紧凑、泛化的规划方式

Method: iCLP框架包含三个步骤：1）从现有逐步推理轨迹中蒸馏显式计划；2）通过向量量化自编码器与码本学习这些计划的离散表示（潜在计划）；3）在成对的潜在计划和对应推理步骤上微调LLM，使模型学会在推理过程中进行隐式规划

Result: 在数学推理和代码生成任务上的实验表明，iCLP使LLM能够在潜在空间规划同时在语言空间推理，显著提高了准确性和效率，并展现出强大的跨领域泛化能力，同时保持了思维链推理的可解释性

Conclusion: iCLP通过将人类潜意识认知原理应用于LLM规划，实现了更高效、准确的推理，解决了显式文本规划的局限性，为LLM推理提供了新的隐式规划范式
...
### [96] [Beyond Hallucinations: A Composite Score for Measuring Reliability in Open-Source Large Language Models](https://arxiv.org/abs/2512.24058)
...
### [97] [HY-MT1.5 Technical Report](https://arxiv.org/abs/2512.24092)
*Mao Zheng,Zheng Li,Tao Chen,Mingyang Song,Di Wang*

Main category: cs.CL

TL;DR: HY-MT1.5系列翻译模型（1.8B和7B参数）通过多阶段训练框架实现高性能机器翻译，在参数效率方面超越大型开源模型和商业API，接近甚至超越Gemini-3.0-Pro等超大私有模型性能。


<details>
  <summary>Details</summary>
Motivation: 开发参数高效的高性能机器翻译模型，在较小参数规模下实现与超大私有模型相媲美的翻译质量，同时支持高级翻译约束功能。

Method: 采用多阶段训练框架：通用预训练与MT导向预训练结合、监督微调、策略蒸馏和强化学习。支持术语干预、上下文感知翻译和格式保持等高级约束功能。

Result: HY-MT1.5-1.8B在标准中-外和英-外任务中全面超越大型开源基线（如Tower-Plus-72B、Qwen3-32B）和主流商业API，达到Gemini-3.0-Pro约90%性能。HY-MT1.5-7B在其规模类别中建立新SOTA，在Flores-200上达到Gemini-3.0-Pro的95%性能，在WMT25和普通话-少数民族语言测试集上超越Gemini-3.0-Pro。

Conclusion: HY-MT1.5系列模型在各自参数规模内为通用和专业化翻译任务提供了高度竞争力和鲁棒的解决方案，实现了卓越的参数效率与性能平衡。
...
### [98] [Training a Huggingface Model on AWS Sagemaker (Without Tears)](https://arxiv.org/abs/2512.24098)
*Liling Tan*

Main category: cs.CL

TL;DR: 本文旨在通过提供一站式指南，帮助研究人员在AWS SageMaker上从零开始训练第一个Hugging Face模型，降低云平台使用门槛。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的开发主要由资源丰富的研究团体和行业伙伴推动。由于缺乏训练日益复杂模型所需的本地计算资源，许多研究人员转向AWS SageMaker等云服务来训练Hugging Face模型。然而，云平台的陡峭学习曲线对习惯本地环境的研究人员构成了障碍。现有文档经常存在知识缺口，迫使用户在网上寻找碎片化信息。

Method: 本文通过集中整理研究人员在AWS SageMaker上成功训练第一个Hugging Face模型所需的基本信息，创建一个一站式指南。该方法旨在填补现有文档的知识缺口，提供系统化的操作流程。

Result: 本文提供了一个演示性指南，将分散的云平台使用知识整合为完整的操作流程，帮助研究人员克服云平台的学习障碍。

Conclusion: 通过提供集中的、从零开始的AWS SageMaker训练指南，本文旨在民主化云平台的采用，使更多研究人员能够利用云资源进行Hugging Face模型训练，从而促进更广泛的研究参与。
...
### [99] [Activation Steering for Masked Diffusion Language Models](https://arxiv.org/abs/2512.24143)
...
<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型通过迭代去噪过程生成文本，具有掩码并行解码优势且性能可与自回归大语言模型竞争，但其推理时控制和引导机制尚未充分探索。

Method: 提出激活引导框架：使用对比示例通过单次前向传播计算层间引导向量，在反向扩散的每一步应用这些方向，实现高效推理时控制。

Result: 在LLaDA-8B-Instruct上的实验表明能够可靠调节高层次属性，消融实验检验了引导在Transformer子模块和token范围（提示vs响应）上的效果。

Conclusion: 该框架为掩码扩散语言模型提供了有效的推理时控制机制，无需模拟去噪轨迹即可实现高效属性调控，为模型可控性研究提供了新方向。
...
### [100] [Large Emotional World Model](https://arxiv.org/abs/2512.24149)
*Changhao Song,Yazhou Zhang,Hui Gao,Chang Yang,Peng Zhang*

Main category: cs.CL

TL;DR: 本文提出大型情感世界模型（LEWM），通过构建情感-原因-方法（EWH）数据集，将情感因素系统整合到世界模型中，以更准确地预测情感驱动的社会行为。
...
Method: 基于心智理论，提出大型情感世界模型（LEWM）。构建EWH数据集，将情感整合到因果关系中，支持推理行为原因和情感如何驱动未来世界状态。模型同时建模情感状态、视觉观察和动作，预测未来状态和情感转变。

Result: 实验结果显示，LEWM能更准确地预测情感驱动的社会行为，同时在基本任务上保持与通用世界模型相当的性能。

Conclusion: 情感是世界理解的关键因素，LEWM通过系统整合情感建模，在保持基础能力的同时显著提升了情感驱动行为的预测准确性，为更全面的世界建模提供了新方向。
...
### [101] [Training Report of TeleChat3-MoE](https://arxiv.org/abs/2512.24157)
...
TL;DR: TeleChat3-MoE是一个基于混合专家架构的大语言模型系列，参数规模从1050亿到超过1万亿，在昇腾NPU集群上进行端到端训练。技术报告主要介绍了支持前沿模型规模可靠高效扩展的基础设施，包括数值精度验证、性能优化、并行化框架和集群级优化。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模不断增大（从1050亿到超过1万亿参数），需要建立可靠高效的基础设施来支持前沿模型规模的训练。现有基础设施在数千设备集群上实现线性扩展、确保数值精度一致性、优化多维度并行配置等方面面临挑战。

Method: 1. 系统化的数值精度验证方法：算子级和端到端验证，确保跨硬件平台和分布式并行策略的一致性；2. 性能优化套件：交错流水线调度、长序列训练的注意力感知数据调度、专家并行的分层重叠通信、DVM算子融合；3. 系统化并行化框架：基于分析估计和整数线性规划优化多维度并行配置；4. 集群级优化方法：解决大规模训练任务中的主机和设备端瓶颈。

Result: 这些基础设施改进在包含数千设备的集群上实现了显著的吞吐量提升和接近线性的扩展，为硬件生态系统上的大规模语言模型开发提供了坚实基础。具体优化包括：交错流水线调度减少空闲时间，注意力感知数据调度提升长序列训练效率，分层重叠通信优化专家并行通信，DVM算子融合减少计算开销。

Conclusion: TeleChat3-MoE的基础设施创新为超大规模混合专家模型训练提供了系统化解决方案，通过数值精度验证、性能优化、并行化框架和集群级优化，在昇腾NPU集群上实现了可靠高效的前沿模型规模扩展，为大规模语言模型开发奠定了坚实基础。
...
### [102] [MedKGI: Iterative Differential Diagnosis with Medical Knowledge Graphs and Information-Guided Inquiring](https://arxiv.org/abs/2512.24181)
*Qipeng Wang,Rui Sheng,Yafei Li,Huamin Qu,Yushi Sun,Min Zhu*

Main category: cs.CL

TL;DR: MedKGI是一个基于临床实践的诊断框架，通过整合医学知识图谱来约束推理、基于信息增益选择问题、采用OSCE格式结构化状态，解决了LLM在临床诊断中的幻觉、低效提问和对话一致性等问题。
...
Method: 提出MedKGI诊断框架：1) 整合医学知识图谱，将推理约束在验证过的医学本体上；2) 基于信息增益选择问题，最大化诊断效率；3) 采用OSCE格式结构化状态，在多轮对话中保持一致的证据跟踪。

Result: 在临床基准测试中，MedKGI在诊断准确性和询问效率方面均优于强大的LLM基线，平均提高对话效率30%，同时保持最先进的准确性。

Conclusion: MedKGI通过将临床实践原则整合到LLM驱动的诊断中，有效解决了当前模型在医疗诊断中的关键限制，实现了更准确、高效的临床推理。
...
### [103] [LAILA: A Large Trait-Based Dataset for Arabic Automated Essay Scoring](https://arxiv.org/abs/2512.24235)
...
Method: 设计了LAILA数据集，包含7,859篇阿拉伯语作文，标注了整体分数和七个维度的特质分数（相关性、组织性、词汇、风格、发展、机制、语法），详细描述了数据集的设计、收集和标注过程。

Result: 提供了使用最先进的阿拉伯语和英语模型在特定提示和跨提示设置下的基准结果，展示了数据集的有效性和实用性。

Conclusion: LAILA填补了阿拉伯语自动作文评分研究的关键需求，支持开发鲁棒的评分系统，推动了阿拉伯语AES领域的发展。
...
### [104] [Tracing the Flow of Knowledge From Science to Technology Using Deep Learning](https://arxiv.org/abs/2512.24259)
...
<details>
  <summary>Details</summary>
Motivation: 需要开发一个能够同时处理专利和科学文献的语言相似性模型，以更好地理解和预测专利与学术论文之间的引用关系，并探索不同司法管辖区引用模式的差异。

Method: 基于SPECTER2模型进行专利数据微调得到Pat-SPECTER模型，通过"赛马式"评估比较8种语言相似性模型在预测可信专利-论文引用方面的性能，并在两个真实场景（分离专利-论文对和预测专利-论文对）中验证模型能力。

Result: Pat-SPECTER模型在预测可信专利-论文引用方面表现最佳；验证了美国专利因诚信义务（duty of candor）要求而引用语义相似度较低的论文这一假设。

Conclusion: 专利微调的Pat-SPECTER模型能有效处理专利和科学文献的相似性分析，为学术研究和实践应用提供了有价值的工具，并揭示了不同司法管辖区引用行为的系统性差异。
...
### [105] [Joint Selection for Large-Scale Pre-Training Data via Policy Gradient-based Mask Learning](https://arxiv.org/abs/2512.24265)
*Ziqing Fan,Yuqiao Xian,Yan Sun,Li Shen*

Main category: cs.CL

TL;DR: DATAMASK是一种用于大规模预训练数据选择的高效联合学习框架，可同时优化质量和多样性指标，在万亿级token数据集上实现高效选择，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法在万亿级预训练数据集（如FineWeb、DCLM）上存在局限性：基于质量指标的选择在长期预训练中表现出严重收益递减，而基于多样性指标的选择会移除过多高质量样本，两者都限制了预训练LLM的能力。由于计算成本高，这些指标很少在单一选择过程中联合考虑。

Method: DATAMASK将数据选择过程视为掩码学习问题，通过迭代采样数据掩码、基于预定义目标计算策略梯度、更新掩码采样logits来实现。采用基于策略梯度的优化和各种加速增强技术，相比贪婪算法减少98.9%的选择时间。

Result: 从15万亿token的FineWeb数据集中选择约10%的子集（FineWeb-Mask），在12个多样化任务上评估：1.5B密集模型性能提升3.2%，7B MoE模型提升1.9%。选择时间相比贪婪算法减少98.9%，使万亿级token的联合学习研究成为可能。

Conclusion: DATAMASK提供了一个高效的大规模预训练数据选择框架，能够同时优化质量和多样性指标，显著提升模型性能并大幅减少计算成本，为高质量预训练数据选择提供了实用解决方案。
...
### [106] [Automated Analysis of Sustainability Reports: Using Large Language Models for the Extraction and Prediction of EU Taxonomy-Compliant KPIs](https://arxiv.org/abs/2512.24289)
*Jonathan Schmoll,Adam Jatowt*

Main category: cs.CL

TL;DR: 该研究创建了首个公开的欧盟分类法合规基准数据集，系统评估了LLM在合规工作流中的表现，发现LLM在定性任务中表现中等，但在定量KPI预测任务中完全失败，且模型置信度校准不佳。


<details>
  <summary>Details</summary>
Motivation: 欧盟分类法合规流程需要大量人工资源，LLM提供了自动化路径，但缺乏公开基准数据集阻碍了相关研究。为解决这一问题，研究者需要创建结构化数据集并系统评估LLM在合规工作流中的表现。

Method: 从190份企业报告中构建了包含真实经济活动数据和定量KPI的结构化数据集，使用该数据集对LLM在核心合规工作流中进行首次系统评估，包括定性经济活动识别和定量KPI预测任务，并测试了多步骤智能体框架。

Result: LLM在定性经济活动识别任务中表现中等，多步骤智能体框架略微提升了精确度；但在零样本设置的定量KPI预测任务中完全失败。发现一个悖论：简洁的元数据通常比完整的非结构化报告表现更好，且模型置信度分数校准不佳。

Conclusion: LLM尚未准备好实现完全自动化，但可以作为人类专家的强大辅助工具。该研究提供的公开基准数据集为未来研究奠定了基础。
...
TL;DR: FIGR通过强化学习将主动视觉思维融入多轮推理，构建视觉表征来外化中间结构假设，在复杂数学推理任务上超越纯文本方法。


<details>
  <summary>Details</summary>
Motivation: 复杂推理问题常涉及隐式的空间、几何和结构关系，这些关系在文本中未明确编码。纯文本推理难以捕捉复杂场景中的全局结构约束，需要更有效的表示方法。

Method: FIGR通过端到端强化学习将主动视觉思维整合到多轮推理中，在问题解决过程中构建视觉表征来外化中间结构假设。系统自适应地调节何时以及如何调用视觉推理，实现对难以从文本中捕捉的全局结构属性的更稳定、连贯的推理。

Result: 在具有挑战性的数学推理基准测试中，FIGR优于强大的纯文本思维链基线。具体来说，FIGR在AIME 2025上使基础模型提升了13.12%，在BeyondAIME上提升了11.00%，证明了图引导多模态推理在增强复杂推理稳定性和可靠性方面的有效性。

Conclusion: FIGR通过整合主动视觉思维和自适应视觉推理调用机制，显著提升了复杂推理任务的性能，特别是在处理涉及空间、几何和结构关系的数学问题时，证明了多模态表征对于捕捉全局结构约束的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在词级和句级任务表现良好，但在法律、医学等领域的长文档表示上存在困难。稀疏注意力机制能处理长输入但资源消耗大且难以捕获完整文档上下文，分层Transformer模型效率更高但无法清晰解释文档不同部分间的关系。人类通过略读重点段落理解整体内容的策略提供了灵感。

Method: 提出自监督对比学习框架：1）随机掩码文档段落；2）使用基于自然语言推理(NLI)的对比目标，将掩码段落与相关部分对齐，同时与不相关部分保持距离；3）模拟人类信息综合过程，生成更丰富且计算高效的文档表示。

Result: 在法学和生物医学文本上的实验表明，该方法在准确性和效率方面均取得显著提升，验证了基于人类略读策略的自监督对比学习框架的有效性。

Conclusion: 借鉴人类略读策略的自监督对比学习框架能够有效增强长文档表示，在保持计算效率的同时提升模型性能，为法律、医学等领域的文档理解提供了新思路。
...
Method: 比较了多种方法：1) 零样本提示不同规模的LLM；2) 微调mT5模型（含三种数据增强方法）；3) 多语言迁移；4) LLM翻译管道方法（源语言→英语→摘要→翻译回源语言）。使用五种不同指标进行评估

Result: 发现：1) 相似参数规模的LLM性能存在差异；2) 多语言微调的mT5基线在多数指标上优于包括零样本LLM在内的大多数方法；3) LLM作为评估者在低资源语言上可能不太可靠

Conclusion: 对于低资源语言文本摘要，多语言微调方法比零样本LLM方法更有效，且LLM评估在低资源语言上存在局限性，需要更可靠的评估方法
...
TL;DR: 开发了一个用于IELTS写作备考的智能修订平台，采用基于设计的研究方法，从规则系统演进到Transformer模型，实现了自动化评分和个性化反馈功能。


<details>
  <summary>Details</summary>
Motivation: 传统IELTS备考方法缺乏针对写作评分标准的个性化反馈，需要开发能够提供针对性指导的智能平台来弥补这一不足。

Method: 采用基于设计的研究方法进行迭代开发：早期使用规则系统，后期采用DistilBERT Transformer模型加回归头的架构，平台设计分离对话指导与写作界面以降低认知负荷。

Result: 规则系统存在中段压缩、低准确率和负R²值等问题；DistilBERT模型显著改善评分性能（MAE=0.66，正R²值）；自适应反馈使平均分数提高0.060个等级（p=0.011，效应量d=0.504）。

Conclusion: 自动化反馈最适合作为人工教学的补充，保守的表面修正比激进的结构干预更可靠；评估高分段作文仍具挑战，未来需进行纵向研究并引入官方考官验证。
...
TL;DR: 该论文填补了语音处理中段落分割任务的空白，建立了首个基准数据集，提出了保留原始文本的约束解码方法，并开发了高效的分层分割模型。


<details>
  <summary>Details</summary>
Motivation: 自动语音转录通常生成无结构的词流，降低了可读性和再利用性。当前语音处理领域缺乏专门的段落分割任务，文本分割领域也缺乏稳健且自然的基准数据集。

Method: 1) 建立TEDPara（人工标注的TED演讲）和YTSegPara（带合成标签的YouTube视频）作为首个段落分割基准；2) 提出约束解码方法，让大语言模型插入段落分隔符同时保留原始转录文本；3) 开发紧凑模型MiniSeg，并通过分层扩展实现章节和段落的联合预测。

Result: 1) 创建了首个专注于语音领域的段落分割基准数据集；2) 约束解码方法实现了忠实、句子对齐的评估；3) MiniSeg模型在保持最先进准确率的同时计算成本极低，分层扩展能有效联合预测章节和段落。
...
TL;DR: HaluNet：一个轻量级可训练的神经框架，通过融合语义嵌入与概率置信度和分布不确定性，集成多粒度标记级不确定性，实现高效的单次幻觉检测。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在问答任务中表现出色，但经常产生幻觉（包括事实错误或捏造内容）。基于内部不确定性信号的幻觉检测因其可扩展性和不依赖外部资源而具有吸引力。现有方法通常只关注准确捕捉单一类型的不确定性，而忽视了不同来源之间的互补性，特别是标记级概率不确定性和内部语义表示所传达的不确定性之间的互补关系。

Method: 提出HaluNet框架，这是一个轻量级可训练的神经框架，通过结合语义嵌入与概率置信度和分布不确定性，集成多粒度标记级不确定性。采用多分支架构，自适应地融合模型已知信息与其输出中表达的不确定性，实现高效的单次幻觉检测。

Result: 在SQuAD、TriviaQA和Natural Questions数据集上的实验表明，HaluNet在有无上下文访问的情况下都能提供强大的检测性能和良好的计算效率。

Conclusion: HaluNet展示了在基于LLM的问答系统中进行实时幻觉检测的潜力，通过有效整合互补的不确定性信号，提高了幻觉检测的准确性和效率。
...
TL;DR: KCL是韩国规范法律基准，用于评估语言模型的法律推理能力，与领域知识解耦，包含选择题和开放式问答两个部分


<details>
  <summary>Details</summary>
Motivation: 设计一个能够独立评估语言模型法律推理能力（而非依赖参数化领域知识）的基准，通过提供问题级别的支持判例来实现更忠实的推理能力解耦

Method: 构建包含两个组件的基准：KCL-MCQA（283个选择题，1,103个对齐判例）和KCL-Essay（169个开放式问题，550个对齐判例，2,739个实例级评分标准用于自动评估）

Result: 对30多个模型的系统评估显示存在较大差距（尤其在KCL-Essay部分），且专门针对推理训练的模型始终优于通用模型

Conclusion: KCL基准成功实现了法律推理能力与领域知识的解耦评估，揭示了当前模型的局限性，并提供了完整的基准数据集和评估代码供研究使用
...
TL;DR: CREST是一种无需训练的方法，通过识别和干预特定注意力头来引导LLM推理过程，提高准确性同时减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型依赖长链思维推理解决复杂任务，但这种方法效率低下：产生高延迟（过多token生成）或不稳定推理（在浅层不一致步骤和重复冗长推理之间摇摆）。需要一种方法来引导模型远离低效推理模式。

Method: CREST包含两个组件：(1)离线校准步骤：识别与特定认知行为（如验证和回溯）相关的认知头，并推导出头特定的引导向量；(2)推理时程序：旋转隐藏表示以抑制沿这些向量的分量，自适应地抑制非生产性推理行为。

Result: 在多样化的推理基准测试和模型中，CREST将准确性提高了高达17.5%，同时减少了37.6%的token使用量，实现了更快、更可靠的LLM推理。

Conclusion: CREST通过识别和干预特定注意力头来引导LLM推理过程，提供了一种简单有效的途径来实现更快、更可靠的LLM推理，同时提高准确性并降低计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 当前小型模型通常依赖蒸馏技术，缺乏原生推理和规划能力。作者旨在开发一个轻量级但具备强大智能体能力的语言模型，能够在有限计算资源下处理长上下文和复杂任务。

Method: 1. 采用密集多潜在注意力（MLA）架构和STEM导向词汇表，支持128k上下文窗口；2. 设计"常识-STEM-智能体"课程学习策略，使用约11T token进行多阶段训练；3. 针对智能体任务采用可扩展的中期训练，合成数学、编程和工具使用领域的多样化轨迹数据。

Result: Youtu-LLM在2B以下模型中达到新的SOTA水平。在通用基准测试中与更大模型竞争，在智能体特定任务上显著超越现有基线，证明了轻量级模型可以具备强大的内在智能体能力。

Conclusion: Youtu-LLM展示了通过系统化从头预训练和精心设计的课程学习，轻量级模型能够获得强大的推理和规划能力，为资源受限环境下的智能体应用提供了有效解决方案。
...
### [116] [Do Large Language Models Know What They Are Capable Of?](https://arxiv.org/abs/2512.24661)
...
TL;DR: 大型语言模型在预测自身任务成功率方面存在过度自信问题，其预测能力并未随模型规模增大而显著提升，且在多步任务中过度自信会加剧。通过失败经验学习可改善部分模型的决策能力。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型能否准确预测自身在任务中的成功率，以及这种预测能力是否会随着任务进展而改善。同时探究LLMs能否从上下文失败经验中学习，从而在失败代价高昂的场景中做出更好的决策。

Method: 通过测试多个LLMs来评估它们预测任务成功率的能力，包括在多步任务中跟踪预测变化。研究模型从上下文失败经验中学习的能力，分析不同模型（包括推理型和非推理型）的表现差异。

Result: 所有测试的LLMs都表现出过度自信，但大多数具有优于随机的判别能力。较新较大的模型并未显示出更强的判别能力（Claude模型除外）。在多步任务中，多个前沿LLMs的过度自信随着任务进展而加剧。通过失败经验学习，部分LLMs减少了过度自信并改善了决策，但并非所有模型都如此。所有LLMs的决策在给定其估计成功率的情况下近似理性，但过度乐观的估计导致决策质量不佳。

Conclusion: 当前LLM智能体受限于对自身能力缺乏认知，这影响了它们的决策质量。研究结果对AI滥用和错位风险具有重要启示，表明需要改进LLMs对自身能力的认知能力。
...
TL;DR: R-Debater是一个基于论证记忆的智能辩论框架，通过检索先验论据和辩论策略来生成多轮辩论，在单轮和多轮辩论中均优于现有LLM基线。


<details>
  <summary>Details</summary>
Motivation: 现有辩论系统缺乏跨轮次的立场一致性和证据支持能力，需要结合修辞学和记忆研究，构建能够回忆并适应先验论据的辩论框架。

Method: 基于修辞学和记忆研究，构建包含辩论知识库（检索案例证据和先验辩论策略）和基于角色的智能体系统，通过检索增强和结构化规划生成连贯的多轮辩论话语。

Result: 在ORCHID标准辩论数据集上评估，R-Debater在单轮话语生成（InspireScore）和多轮对抗模拟（Debatrix）中均优于强LLM基线，人类评估进一步证实其立场一致性和证据使用能力。

Conclusion: 检索增强与结构化规划的结合能够产生更忠实、立场对齐且连贯的多轮辩论，为构建更可靠的辩论系统提供了有效框架。
...
TL;DR: 提出MUSIC数据增强策略，通过合成跨多轮对话的对比样本训练多轮奖励模型，在保持单轮性能的同时提升多轮对话评估效果


<details>
  <summary>Details</summary>
Motivation: 多轮对话质量评估对LLM发展至关重要但成本高昂，现有基于单轮对比的偏好数据集无法捕捉多轮交互的细微差别，需要更有效的多轮奖励模型训练方法

Method: 提出MUSIC（多步指令对比）无监督数据增强策略，合成跨多轮对话的对比对话对，在Skywork偏好数据集上基于Gemma-2-9B-Instruct模型训练多轮奖励模型

Result: MUSIC增强的奖励模型在多项评估中优于基线方法，与先进专有LLM评判在多轮对话评估上更一致，且不损害标准单轮奖励模型基准性能

Conclusion: 跨多轮对比对于构建鲁棒的多轮奖励模型至关重要，MUSIC策略能有效提升多轮对话评估质量，为LLM训练提供更可靠的多轮信号
...
<details>
  <summary>Details</summary>
Motivation: 当前多组学研究依赖通路富集分析，但该方法受限于通路资源的结构性缺陷（如更新滞后、功能冗余、对分子状态和干预敏感性有限）。虽然已有研究探索使用大语言模型改进通路富集分析，但缺乏端到端多组学通路机制阐明的标准化基准，导致评估局限于小型手动整理数据集或临时案例研究，阻碍了可重复性进展。

Method: 通过严格的四阶段工作流程构建BIOME-Bench基准，用于评估大语言模型在多组学分析中的两个核心能力：1) 生物分子相互作用推断；2) 端到端多组学通路机制阐明。开发了针对这两个任务的评估协议，并在多个当代强模型上进行了全面实验。

Result: 实验结果表明，现有模型在多组学分析中仍存在显著缺陷：难以可靠地区分细粒度的生物分子关系类型，也无法生成忠实、稳健的通路水平机制解释。

Conclusion: BIOME-Bench为评估大语言模型在多组学分析中的能力提供了标准化基准，揭示了现有模型在生物分子相互作用推断和通路机制阐明方面的局限性，为未来模型改进指明了方向。
...
Method: 对当代和早期开源LLM进行测试时计算感知评估，在数学和推理密集型基准上绘制帕累托前沿，追踪帕累托效率随时间变化轨迹，并分析推理计算饱和点。

Result: 混合专家架构在评估设置中展现出性能与效率的良好平衡；帕累托效率随时间推移呈现准确性增益与计算单位之间的涌现趋势；推理计算存在饱和点，超过阈值后准确性增益递减。

Conclusion: 虽然扩展推理能力有益，但无法克服模型在特定复杂性方面的内在限制；混合专家架构是平衡性能与效率的有力候选方案；实际部署应考虑推理计算饱和点以优化资源分配。
...
TL;DR: 介绍一门NLP伦理课程及其教学方法，通过主动学习、互动活动和"教学相长"方式，培养学生在快速发展的NLP领域中的伦理意识和批判性思维。
...
Method: 采用基于主动学习的教学方法，包括互动课程、实践活动和"教学相长"方法。课程经过四年时间，在不同机构、教育层次和跨学科背景下不断优化和调整。

Result: 课程产生了许多可重复使用的产品，包括教学材料和面向不同受众的实际教育产品（由学生自己制作）。课程已在多个教育环境中成功实施和适应。

Conclusion: 通过分享教学方法和经验，希望为寻求将社会影响考量融入课程的教育工作者提供启发，促进NLP教育中伦理维度的整合。
...
TL;DR: 论文提出了一种因果机制解释标准——三角验证，用于评估多语言模型中电路发现的可靠性，要求电路在保持语义的跨语言变体中具有必要性、充分性和不变性。


<details>
  <summary>Details</summary>
Motivation: 多语言模型在整体性能上表现良好，但在不同语言、文字和文化中的行为往往不可预测。现有机制解释方法缺乏因果标准，无法区分真正的因果机制与表面相关的伪电路。

Method: 提出"三角验证"接受标准：1)必要性(消融电路会降低目标行为)；2)充分性(修补激活能转移行为)；3)不变性(这些效应在保持语义的跨语言变体家族中保持方向稳定和足够幅度。将三角验证形式化为因果抽象框架中的近似变换分数，基于分布交换干预。

Result: 三角验证提供了一个可证伪的机制解释标准，能够过滤掉通过单环境测试但在跨语言不变性测试中失败的伪电路。建立了比较实验协议，涵盖多个模型家族、语言对和任务。

Conclusion: 三角验证为多语言模型的机制解释提供了严格的因果标准，通过跨语言不变性测试确保发现的电路具有真正的因果机制，而非表面相关性，推动了实用可解释性议程。
...
<details>
  <summary>Details</summary>
Motivation: 当前世界面临生态、意义和语言三重危机，这些危机正在汇聚成元危机。大型AI正在加速所有这些危机，语言工程师在其中扮演关键角色，他们坚持的可扩展性叙事正在失败，为富豪和盗贼统治者提供关键人才，并创造看似价值中立的技术。迫切需要探索替代方案。

Method: 论文采用批判性分析和规范性论证的方法，通过分析当前NLP领域的问题，提出需要集体智慧来设计替代性发展路径。虽然没有具体的技术方法，但提出了理念框架的转变方向。

Result: 论文识别了当前NLP领域发展的根本问题：加剧三重危机、服务于不当利益、价值中立假象。提出了需要重新定位NLP发展方向的核心洞见。

Conclusion: 迫切需要探索替代方案，应用集体智慧设计一个以人类繁荣和生命星球为中心的、肯定生命的NLP未来，摆脱当前加剧危机的不可持续发展模式。
...
TL;DR: Encyclo-K是一个基于知识陈述的基准测试，通过从权威教科书中提取独立知识陈述并在测试时动态组合成评估问题，解决了现有基准测试的三个根本限制。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要基于问题级别构建，存在三个根本限制：1) 易受数据污染影响；2) 仅限于单知识点评估；3) 依赖昂贵的领域专家标注。需要重新思考基准测试的构建方式。

Method: 从权威教科书中提取独立的知识陈述作为基本单元，在测试时通过随机抽样动态组合成评估问题。每个问题聚合8-10个陈述进行多知识综合评估。标注者只需验证格式合规性，无需领域专业知识。

Result: 在50多个LLM上的实验表明，Encyclo-K具有强大的区分能力。即使是表现最好的OpenAI-GPT-5.1也仅达到62.07%的准确率。推理模型准确率范围为16.04%到62.07%，聊天模型为9.71%到50.40%。

Conclusion: Encyclo-K提供了一个可扩展的框架，用于动态评估LLM对多个细粒度学科知识陈述的综合理解能力，有效解决了现有基准测试的局限性。
...
<details>
  <summary>Details</summary>
Motivation: Hyper-Connections (HC)通过扩展残差流宽度和多样化连接模式提升了性能，但破坏了残差连接的恒等映射特性，导致训练不稳定、可扩展性受限和内存访问开销增加
...
<details>
  <summary>Details</summary>
Motivation: 现有战略对话研究虽然能准确估计信念，但缺乏使用这些信念进行生成的原则性机制。本文旨在填补这一空白，通过将信念估计转化为生成约束来实现可靠的战略对话。

Method: 首先形式化了对抗和对齐两种核心对话行为，通过概率约束将其操作化。提出了BEDA框架，包含世界集合、信念估计器和条件生成器三个组件，后者根据推断的信念选择行为并生成一致的语句。

Result: 在三种设置中均取得显著改进：在Conditional Keeper Burglar（对抗）中，成功率在不同骨干模型上至少提升5.0个百分点，使用GPT-4.1-nano时提升20.6个百分点；在Mutual Friends（合作）中平均提升9.3个百分点；在CaSiNo（谈判）中实现了相对于所有基线的最优交易。

Conclusion: 将信念估计转化为约束为可靠的战略对话提供了一个简单、通用的机制，BEDA框架在对抗、合作和谈判对话中均表现出色，证明了该方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 多步LLM流程的性能严重依赖于每个步骤使用的提示，但由于缺乏步骤级监督和步骤间依赖关系，联合优化这些提示非常困难。现有的端到端提示优化方法在这些条件下效果不佳，往往产生次优或不稳定的更新。

Method: ADOPT框架显式建模每个LLM步骤与最终任务结果之间的依赖关系，实现类似于计算解析导数的精确文本梯度估计。它将文本梯度估计与梯度更新解耦，将多提示优化简化为灵活的单提示优化步骤，并采用基于Shapley值的机制自适应分配优化资源。

Result: 在真实世界数据集和多样化流程结构上的实验表明，ADOPT是有效且鲁棒的，在性能上持续优于最先进的提示优化基线方法。

Conclusion: ADOPT通过显式建模步骤间依赖关系和实现精确的文本梯度估计，成功解决了多步LLM流程中的提示优化问题，为复杂任务的多步LLM流程提供了有效的优化框架。
...
TL;DR: 基于DeBERTa V3和LSTM的法律文档分类器，使用48个随机短片段（最大128词元）作为输入，通过Temporal部署实现可靠处理流程，加权F分数0.898，CPU处理中位时间498秒/100文件


<details>
  <summary>Details</summary>
Motivation: 法律文档分类面临专业词汇和长度挑战，直接使用Transformer模型处理完整文档可能不可行、昂贵或缓慢，需要更高效的解决方案

Method: 结合DeBERTa V3和LSTM的模型架构，输入为48个随机选择的短片段（最大128词元），使用Temporal构建可靠的部署管道

Result: 最佳模型获得0.898的加权F分数，CPU上的处理管道中位时间为每100个文件498秒

Conclusion: 提出的片段化输入方法有效解决了长法律文档的处理问题，结合Temporal的部署管道确保了处理流程的可靠性和鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 表情包已从单纯的幽默表达演变为表达各种情绪（包括抑郁情绪）的媒介，需要开发有效方法来识别社交媒体表情包中表现的抑郁症状

Method: 引入RESTOREx数据集（包含LLM生成和人工标注的解释），提出MAMAMemeia框架——基于认知分析治疗（CAT）能力的协作多智能体多方面讨论框架

Result: MAMAMemeia相比现有最佳方法提升7.55%的macro-F1分数，在超过30种方法中建立为新的基准

Conclusion: RESTOREx数据集和MAMAMemeia框架为检测社交媒体表情包中的抑郁症状提供了有效资源和方法，显著提升了识别性能
...
TL;DR: Thought Gestalt (TG)模型是一种循环Transformer，通过同时建模token和句子级"思想"状态，在保持GPT-2架构和训练目标的同时，提高了数据效率和关系方向泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer语言模型主要依赖表层共现统计，缺乏全局一致的实体和事件潜在表示，导致关系方向脆弱性（如逆转诅咒）、上下文错误和数据效率低下。受认知科学启发，人类理解语言时会将其转换为紧凑的事件式表示，而字面形式短暂存在。

Method: TG模型是一种循环Transformer，在两个抽象层次上建模语言：token和句子级"思想"状态。模型逐句生成token，同时交叉关注先前句子表示的记忆。token和句子表示使用相同模型参数生成，通过单一目标（下一个token交叉熵）训练。通过保留写入记忆的句子表示计算图，未来token损失的梯度通过交叉注意力反向传播，优化生成早期句子向量的参数。

Result: 在扩展实验中，TG相比匹配的GPT-2运行持续提高效率，扩展拟合表明GPT-2需要约5-8%更多数据和33-42%更多参数才能匹配TG的损失。TG还在父子逆转诅咒探针上减少了关系方向泛化错误。

Conclusion: TG模型通过引入句子级表示和循环记忆机制，在保持标准Transformer架构和训练目标的同时，提高了语言建模的数据效率和关系推理能力，为构建更稳健的语言模型提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 标准top-k检索在检索增强生成中经常返回冗余或近似重复的文本块，这会浪费token预算并降低下游生成质量。需要一种能够有效控制冗余、优化token利用的上下文选择方法。

Method: 提出AdaGReS框架，采用贪婪选择算法在token预算约束下优化集合级目标函数，该函数结合查询-块相关性和集合内冗余惩罚。引入闭式、实例自适应的相关性-冗余权衡参数校准，无需手动调参，能够适应候选池统计特性和预算限制。

Result: 在开放域问答（Natural Questions）和高冗余生物医学（药物）语料库上的实验表明，AdaGReS在冗余控制和上下文质量方面持续改进，转化为更好的端到端答案质量和跨设置鲁棒性。理论分析显示所提目标函数在实际嵌入相似性条件下具有epsilon近似子模性，为贪婪选择提供近最优性保证。

Conclusion: AdaGReS通过冗余感知的上下文选择和自适应参数校准，有效解决了RAG中冗余文本块浪费token预算的问题，提高了上下文选择质量和下游生成性能，为token预算约束下的RAG系统提供了理论保证的实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本驱动的3D人体运动编辑方法虽然取得显著进展，但由于其全局建模特性，难以精确控制细节化的、特定部位的运动。需要开发能够进行细粒度、部件级控制的运动编辑方法。
...
Result: 在知名基准测试上进行的大量定量和定性评估表明，PartMotionEdit优于现有最先进方法，实现了更精确的部件级运动编辑控制。

Conclusion: PartMotionEdit通过部件级语义调制框架解决了现有文本驱动3D人体运动编辑方法在细粒度控制方面的局限性，实现了精确、可解释的局部运动编辑，在运动编辑精度和语义对齐方面表现出色。
...
TL;DR: 关于"恐怖谷"现象对医学虚拟现实模拟训练影响的综述性文献指南


<details>
  <summary>Details</summary>
Motivation: 理解恐怖谷现象对医学虚拟现实训练的影响至关重要，因为医学训练需要高度的真实感和沉浸感，而恐怖谷效应可能导致学习者不适，影响训练效果

Method: 采用文献综述和证据为基础的可视化指南方法，结合计算机图形学、虚拟现实和医学教育专家的多学科视角进行分析

Result: 未提供具体研究结果，但强调需要理解和解决恐怖谷现象在医学虚拟现实训练中的影响

Conclusion: 恐怖谷现象是医学虚拟现实模拟训练中需要理解和解决的关键问题，多学科合作对于推进VR在医学教育中的应用至关重要
...
TL;DR: 论文提出DDFT协议评估语言模型的认知稳健性，发现模型规模与稳健性无关，而错误检测能力是关键瓶颈


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估主要测量理想条件下的知识掌握程度，但无法评估模型在现实压力下的稳健性。静态基准测试如MMLU和TruthfulQA无法区分模型是缺乏知识还是验证机制在信息退化或对抗性攻击下崩溃

Method: 引入Drill-Down和Fabricate Test（DDFT）协议，测量认知稳健性：模型在渐进语义压缩和对抗性伪造下保持事实准确性的能力。提出两系统认知模型：语义系统生成流畅文本，认知验证器验证事实准确性。评估9个前沿模型在8个知识领域、5个压缩级别（共1800个轮次评估）
...
Conclusion: DDFT框架为关键应用部署前评估认知稳健性提供了理论基础和实用工具。模型稳健性主要来自训练方法和验证机制，而非当前主流设计方法。挑战了模型规模与可靠性关系的假设
...
TL;DR: McCoy框架结合大语言模型与答案集编程，通过LLM将医学文献转化为ASP代码，结合患者数据进行疾病诊断预测


<details>
  <summary>Details</summary>
Motivation: 准确的疾病预测对及时干预、有效治疗和减少医疗并发症至关重要。符号AI在医疗保健中的应用因构建高质量知识库所需的大量努力而受到限制。

Method: McCoy框架结合大型语言模型（LLMs）与答案集编程（ASP）。它协调LLM将医学文献翻译成ASP代码，将其与患者数据结合，并使用ASP求解器进行处理以得出最终诊断。

Result: 初步结果显示，McCoy在小规模疾病诊断任务上表现出色，提供了一个强大且可解释的预测框架。

Conclusion: McCoy通过结合LLMs和ASP的优势，克服了符号AI在医疗领域应用的知识库构建障碍，为疾病预测提供了可解释的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统个性化搜索系统受限于静态用户画像和单一检索流程，无法捕捉用户动态、多维的信息需求演化。需要一种能够建模人类信息寻求行为复杂性、流动性和情境敏感性的新一代搜索系统。

Method: SPARK框架定义了基于角色、专业知识、任务上下文和领域的角色空间，引入角色协调器动态解析查询并激活最相关的专业智能体。每个智能体执行独立的检索增强生成过程，配备专用长短期记忆存储和情境感知推理模块。智能体间通过结构化通信协议（共享记忆库、迭代辩论、接力式知识转移）进行协作。

Result: 框架基于认知架构、多智能体协调理论和信息检索原理，建模了分布式智能体行为如何通过最小协调规则产生涌现的个性化特性。框架产生了关于协调效率、个性化质量和认知负载分布的可测试预测，并包含持续角色细化的自适应学习机制。

Conclusion: SPARK通过整合细粒度智能体专业化和协作检索，为能够捕捉人类信息寻求行为复杂性、流动性和情境敏感性的下一代搜索系统提供了理论框架和实现思路。
...
TL;DR: ROAD框架通过自动化调试而非随机搜索优化LLM提示，无需标注数据集，利用多智能体架构将失败日志转化为决策树协议，在冷启动场景下实现高效优化。


<details>
  <summary>Details</summary>
Motivation: 现有自动提示优化方法依赖大量标注开发集计算适应度分数，但在实际软件工程中，冷启动阶段通常只有混乱的生产日志和不断变化的故障模式，缺乏精炼数据集。

Method: ROAD采用多智能体架构：分析器进行根因分析，优化器进行模式聚合，教练进行策略集成，将非结构化失败日志转化为结构化的决策树协议，实现动态调试而非随机搜索。

Result: 在标准化学术基准和实时生产知识管理引擎上评估，ROAD在仅三次自动化迭代中实现成功率提升5.6%（73.6%至79.2%），搜索准确率提升3.8%；在零售领域复杂推理任务中，相对基线提升约19%性能。

Conclusion: 模仿人类工程循环的失败分析和补丁方法为部署可靠LLM智能体提供了可行且数据高效的替代方案，避免了资源密集的强化学习训练。
...
Journal reference: AAAI 2026 Workshop on AI for Scientific Research

TL;DR: 提出一种免训练的基于图的方法，用于解决ARC-AGI-3基准测试中的交互式推理任务，通过视觉帧处理、状态空间探索和图结构表示，在稀疏反馈环境中显著优于前沿LLM


<details>
  <summary>Details</summary>
Motivation: ARC-AGI-3基准测试包含类似游戏的任务，要求智能体通过有限交互推断任务机制并适应递增复杂度。当前最先进的LLM无法可靠解决这些任务，需要能够形成假设、测试假设并跟踪发现机制的方法

Method: 结合基于视觉的帧处理与使用图结构表示的系统化状态空间探索。方法包括：将视觉帧分割为有意义的组件；基于视觉显著性优先选择动作；维护探索状态和转换的有向图；通过跟踪访问状态和测试动作，优先选择提供到未测试状态-动作对最短路径的动作

Result: 在ARC-AGI-3预览挑战中，该结构化探索策略在六个游戏中解决了52个关卡的中位数30个，在私有排行榜上排名第3，显著优于前沿基于LLM的智能体

Conclusion: 即使没有学习，显式的图结构探索也可以作为交互式推理的强大基线，强调了在稀疏反馈环境中系统化状态跟踪和动作优先级的重要性，这些环境中当前LLM无法捕捉任务动态
...
TL;DR: 提出Risk-aware Stepwise Alignment (RSA)方法，通过嵌套风险度量将风险意识融入语言模型对齐过程，解决现有风险中性方法在罕见但灾难性有害行为方面的不足。
...
Method: RSA将安全对齐表述为token级别的风险感知约束策略优化问题，通过逐步对齐程序解决，利用嵌套风险度量推导token级别的策略更新。

Result: 实验结果表明，该方法在保持高帮助性的同时确保了强安全性，显著抑制了尾部风险（低概率但高影响的不安全响应）。

Conclusion: RSA通过将风险意识显式纳入策略优化过程，有效缓解了模型偏离参考策略的风险，并明确抑制了低概率高影响的有害行为，在温和假设下提供了理论最优性分析。
...
TL;DR: 该研究系统分析了基于JEPA世界模型（JEPA-WMs）的规划方法，通过实验确定了模型架构、训练目标和规划算法的最佳组合，提出了优于现有基线的新模型。


<details>
  <summary>Details</summary>
Motivation: AI领域长期存在开发能够解决广泛物理任务并在新任务和环境中泛化的智能体的挑战。基于世界模型的规划方法中，在表示空间进行规划的方法（JEPA-WMs）承诺通过抽象无关细节来提高规划效率，但其技术选择尚未得到系统研究。

Method: 将这类方法统一表征为JEPA-WMs，系统研究模型架构、训练目标和规划算法等关键组件。使用模拟环境和真实机器人数据进行实验，分析各组件对规划成功率的影响。

Result: 通过研究发现的最佳组合提出的新模型，在导航和操作任务上均优于两个现有基线模型DINO-WM和V-JEPA-2-AC。

Conclusion: 该研究为JEPA-WMs类方法提供了全面的技术分析，确定了关键设计选择的最佳实践，提出的改进模型在多种任务上表现出更好的性能，推动了表示空间规划方法的发展。
...
### [141] [Evaluating the Reasoning Abilities of LLMs on Underrepresented Mathematics Competition Problems](https://arxiv.org/abs/2512.24505)
...
TL;DR: 该研究分析了三个领先大语言模型（GPT-4o-mini、Gemini-2.0-Flash、DeepSeek-V3）在代表性不足的数学竞赛问题（密苏里大学数学竞赛）上的表现，发现DeepSeek-V3在所有三个数学领域表现最佳，但所有模型在几何问题上表现均较弱。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多使用相同数据集评估大语言模型的数学推理能力，这限制了研究结果的普适性，可能无法充分捕捉数学任务中的多样化挑战。本研究旨在通过分析LLMs在代表性不足的数学竞赛问题上的表现，更全面地了解其数学推理局限性。

Method: 使用密苏里大学数学竞赛中的微积分、解析几何和离散数学问题，对GPT-4o-mini、Gemini-2.0-Flash和DeepSeek-V3三个领先LLMs进行测试。将LLMs的响应与已知正确答案进行比较，以确定每个问题领域的准确性。同时分析LLMs的推理过程，探索不同问题类型和模型之间的错误模式。

Result: DeepSeek-V3在微积分、解析几何和离散数学三个类别中表现最佳，包括推理过程和最终答案的正确性。所有三个LLMs在几何问题上表现明显较弱。DeepSeek-V3的错误主要归因于计算和逻辑错误；GPT-4o-mini经常出现逻辑和方法相关的错误；Gemini则倾向于推理不完整和匆忙得出结论。

Conclusion: 在代表性不足的数学竞赛数据集上评估LLMs可以提供对其独特错误模式的更深入洞察，并突显结构化推理（特别是在几何领域）中持续存在的挑战。这种评估方法有助于更全面地理解LLMs的数学推理能力局限性。
...
### [142] [From Building Blocks to Planning: Multi-Step Spatial Reasoning in LLMs with Reinforcement Learning](https://arxiv.org/abs/2512.24532)
*Amir Tahmasbi,Sadegh Majidi,Kazem Taram,Aniket Bera*

Main category: cs.AI

TL;DR: 论文提出两阶段方法：先通过监督微调学习基础空间变换（旋转、平移、缩放），再冻结模型并用LoRA适配器在GRPO框架下学习多步规划策略，在ASCII艺术环境中实现空间推理。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在通用语言能力上表现优异，但在结构化环境中的空间变换和多步规划方面仍存在困难，这限制了其在导航和规划等应用中的效果。

Method: 1. 监督微调阶段：在基础空间变换（旋转、平移、缩放）上进行训练，使模型掌握空间物理知识；2. 策略学习阶段：冻结物理感知模型，在GRPO框架下训练轻量级LoRA适配器，学习将基础构建块组合为多步规划策略；3. 构建ASCII艺术数据集和对应的强化学习环境支持整个流程。

Result: 该方法在动态环境（有显式状态更新）和静态环境（模型需依赖内部状态）中均优于基线模型（包括通用骨干模型、物理感知模型和端到端RL模型），收敛更快且训练更稳定。注意力模式分析表明微调确实带来了空间理解能力的提升。
...
### [143] [Recursive Language Models](https://arxiv.org/abs/2512.24601)
...
TL;DR: 提出递归语言模型（RLMs），一种允许LLM处理任意长提示的推理策略，通过将长提示视为外部环境并递归调用自身处理提示片段


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型处理超长上下文的能力限制，突破模型上下文窗口的约束，使LLM能够处理任意长度的提示

Method: 提出递归语言模型（RLMs）推理策略，将长提示视为外部环境，允许LLM以编程方式检查、分解并递归调用自身处理提示片段

Result: RLMs成功处理超出模型上下文窗口两个数量级的输入，在四个多样化的长上下文任务中，即使对于较短提示也显著优于基础LLM和常见长上下文框架，同时保持可比（或更低）的查询成本

Conclusion: RLMs提供了一种有效且经济的方法来扩展LLM处理长上下文的能力，通过递归推理策略显著提升长提示处理质量
...
### [144] [Reinforcement Learning-Augmented LLM Agents for Collaborative Decision Making and Performance Optimization](https://arxiv.org/abs/2512.24609)
...
TL;DR: 提出强化学习增强的LLM多智能体协作框架，采用Dec-POMDP建模和CTDE训练，引入GRPO优化策略，在协作写作和编码任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在单智能体任务中表现良好，但在多智能体协作场景中缺乏协作意识，难以优化全局性能，需要专门的协作框架来解决这一问题

Method: 1) 将协作建模为去中心化部分可观察马尔可夫决策过程(Dec-POMDP)；2) 采用集中训练分散执行(CTDE)范式；3) 引入组相对策略优化(GRPO)算法，利用训练时的全局信号联合优化智能体策略；4) 设计简化的联合奖励函数，平衡任务质量、速度和协调成本

Result: 1) 任务处理速度比单智能体基线提升3倍；2) 协作写作中实现98.7%的结构/风格一致性；3) 编码任务中达到74.6%的测试通过率；4) 在协作写作和编码基准测试中持续优于强大多智能体LLM基线

Conclusion: 该强化学习增强的LLM多智能体框架为复杂工作流程中的可靠协作提供了实用路径，显著提升了多智能体协作的性能和效率
...
### [145] [Group Deliberation Oriented Multi-Agent Conversational Model for Complex Reasoning](https://arxiv.org/abs/2512.24613)
...
TL;DR: 提出面向群体审议的多智能体对话模型，通过三层角色分工架构（生成、验证、整合）提升复杂推理能力，引入自博弈机制和检索增强模块，在多项多跳推理任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决单一大型语言模型在复杂推理任务中的局限性，特别是多跳推理中的事实一致性和逻辑连贯性问题。传统单模型方法难以处理复杂的多路径推理和外部知识验证需求。

Method: 采用三层角色分工架构：1) 观点生成智能体产生多样化推理视角；2) 证据验证智能体检索外部知识并量化事实支持度；3) 一致性仲裁智能体整合逻辑连贯的结论。引入自博弈机制扩展多路径推理轨迹，检索增强模块动态补充外部知识。设计结合事实一致性和逻辑连贯性的复合奖励函数，应用改进的近端策略优化策略进行协同训练。

Result: 在HotpotQA上多跳推理准确率提升16.8%，在2WikiMultihopQA上提升14.3%，在MeetingBank上提升19.2%，一致性提升21.5%。相比主流多智能体方法，该模型实现了更高的推理效率。

Conclusion: 该群体审议导向的多智能体对话模型为复杂推理任务提供了有效且稳定的解决方案，通过智能体间的分工协作显著提升了多跳推理的准确性和一致性，同时保持了较高的推理效率。
...
### [146] [Youtu-Agent: Scaling Agent Productivity with Automated Generation and Hybrid Policy Optimization](https://arxiv.org/abs/2512.24615)
...
TL;DR: Youtu-Agent：一个用于自动化生成和持续演进的LLM智能体框架，解决现有框架配置成本高和静态能力问题，支持工作流和元智能体两种生成模式，并包含混合策略优化系统。
...
Method: 提出模块化框架Youtu-Agent，包含：1）结构化配置系统，解耦执行环境、工具包和上下文管理；2）两种生成范式：工作流模式用于标准任务，元智能体模式用于复杂非标准需求；3）混合策略优化系统：Agent Practice模块通过上下文优化积累经验，Agent RL模块集成分布式训练框架进行大规模强化学习。

Result: 在WebWalkerQA（71.47%）和GAIA（72.8%）上达到SOTA性能；自动化生成管道工具合成成功率超过81%；Practice模块在AIME 2024/2025上分别提升+2.7%和+5.4%；Agent RL训练在7B LLMs上实现40%加速，数学和通用/多跳QA基准上分别提升35%和21%。

Conclusion: Youtu-Agent框架有效解决了LLM智能体配置成本高和静态能力问题，通过自动化生成和混合优化策略实现了智能体的持续演进，在多个基准测试中表现出优越性能。
...
Journal reference: Mechanical Systems and Signal Processing, Volume 258, 2026, 114693

DOI: [10.1016/j.ymssp.2026.114693](https://doi.org/10.1016/j.ymssp.2026.114693)

Author affiliations: Pengcheng Xia: Agency for Science, Technology and Research / Shanghai Jiao Tong University / Singapore Institute of Manufacturing Technology; Yixiang Huang: Shanghai Jiao Tong University; Chengjin Qin: Shanghai Jiao Tong University; Chengliang Liu: Shanghai Jiao Tong University

TL;DR: 提出多模态跨域混合融合模型，通过双重解耦框架分离模态不变/特定特征和域不变/特定表示，结合跨域混合融合策略和三模态融合机制，提升未见工况下的故障诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有故障诊断方法在未见工况下性能显著下降，域自适应方法依赖目标域样本，且多数研究依赖单模态信号，忽略了多模态信息的互补性对提升模型泛化能力的作用。

Method: 提出多模态跨域混合融合模型，包含：1) 双重解耦框架，分离模态不变特征和模态特定特征，以及域不变表示和域特定表示；2) 跨域混合融合策略，随机混合跨域的模态信息以增强模态和域多样性；3) 三模态融合机制，自适应集成多模态异构信息。

Result: 在感应电机故障诊断任务中，针对未见恒定工况和时变工况进行实验，结果表明该方法始终优于先进方法，综合消融研究进一步验证了各组件和多模态融合的有效性。

Conclusion: 所提出的多模态跨域混合融合模型通过双重解耦、跨域混合融合和三模态融合机制，有效提升了故障诊断模型在未见工况下的泛化能力，为多模态域泛化故障诊断提供了有效解决方案。
...
### [148] [Explaining Why Things Go Where They Go: Interpretable Constructs of Human Organizational Preferences](https://arxiv.org/abs/2512.24829)
...
TL;DR: 论文提出了一种显式的物体排列偏好模型，包含四个可解释的构造维度，并通过问卷验证了这些维度的心理区分度和解释力，最后将其集成到MCTS规划器中用于机器人任务。


<details>
  <summary>Details</summary>
Motivation: 现有基于人类演示的潜在偏好模型虽然能有效预测，但缺乏对人类决策背后可解释因素的洞察。需要开发能够明确表达物体排列偏好的可解释框架。

Method: 提出了四个可解释的构造维度：空间实用性、习惯便利性、语义连贯性和常识适当性。通过63名参与者的在线研究设计和验证了自报告问卷，并将这些构造维度集成到蒙特卡洛树搜索规划器中。

Result: 研究结果证实了这四个构造维度的心理区分度及其在厨房和客厅两种场景中的解释力。基于参与者偏好指导的MCTS规划器能够生成与参与者排列高度一致的合理布局。

Conclusion: 该工作贡献了一个紧凑、可解释的物体排列偏好公式化框架，并展示了如何将其操作化用于机器人规划，为理解人类物体排列决策提供了新的理论视角。
...
### [149] [GenZ: Foundational models as latent variable generators within traditional statistical models](https://arxiv.org/abs/2512.24834)
*Marko Jojic,Nebojsa Jojic*

Main category: cs.AI

TL;DR: GenZ是一种混合模型，通过可解释的语义特征桥接基础模型和统计建模。它通过迭代过程发现数据集特定的语义特征，而不是依赖基础模型的领域知识，在房价预测和电影推荐任务中显著优于GPT-5基线。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然拥有广泛的领域知识，但往往无法捕捉对预测任务至关重要的数据集特定模式。现有方法过度依赖基础模型的领域理解，而忽略了数据集中特有的统计模式。

Method: 提出一种广义EM算法，联合优化语义特征描述符和统计模型参数。通过迭代过程发现语义特征描述：基于统计建模错误识别项目组并进行对比，而不是依赖基础模型的领域知识。使用冻结的基础模型根据发现的特征对项目进行分类，将这些判断视为潜在二元特征的噪声观测，通过学习的统计关系预测实值目标。

Result: 在房价预测任务中，使用从多模态列表数据中发现的语义特征实现了12%的中位数相对误差，显著优于依赖LLM领域知识的GPT-5基线（38%误差）。在Netflix电影嵌入预测中，仅从语义描述就能预测协同过滤表示，达到0.59余弦相似度，这相当于传统协同过滤需要约4000个用户评分才能达到的性能。

Conclusion: GenZ成功桥接了基础模型和统计建模，通过发现数据集特定的语义特征显著提升了预测性能。发现的特征揭示了与模型领域知识不同的数据集特定模式（如预测本地房地产市场的建筑细节、预测用户偏好的系列电影成员关系），证明了该方法在捕捉数据特定模式方面的有效性。
...
### [150] [A study on constraint extraction and exception exclusion in care worker scheduling](https://arxiv.org/abs/2512.24853)
*Koki Suenaga,Tomohiro Furuta,Satoshi Ono*

Main category: cs.AI

Journal reference: Artificial Life Robotics (2025)

DOI: [10.1007/s10015-025-01082-6](https://doi.org/10.1007/s10015-025-01082-6)

Author affiliations: Koki Suenaga: Kagoshima University; Tomohiro Furuta: Kagoshima University; Satoshi Ono: Kagoshima University
...
<details>
  <summary>Details</summary>
Motivation: 养老机构的排班条件因机构而异，需要从制定排班的管理者访谈中提取设施特定的约束条件，但现有约束提取技术难以处理异常约束。

Method: 使用约束模板提取连续工作日排班模式、员工组合等各种组件的组合，通过改变关注的天数和员工数量、调整提取焦点（模式或频率）来提取多样约束，并包含排除异常约束的机制。

Result: 实验表明，该方法成功创建了满足所有硬约束的排班表，并通过避免提取异常约束，减少了软约束的违反次数。

Conclusion: 提出的约束模板方法能有效从管理者访谈中提取养老机构特定的排班约束，排除异常情况，为约束规划求解器提供可用的约束条件，实现定制化排班生成。
...
### [151] [Let It Flow: Agentic Crafting on Rock and Roll, Building the ROME Model within an Open Agentic Learning Ecosystem](https://arxiv.org/abs/2512.24873)
...
<details>
  <summary>Details</summary>
Motivation: 开源社区缺乏系统化的智能体开发基础设施，需要建立端到端的生态系统来优化智能体LLM的生产流程，支持多轮次、交互式的智能体学习。

Method: 提出了ALE生态系统，包含三个组件：1) ROLL用于权重优化的后训练框架；2) ROCK用于轨迹生成的沙盒环境管理器；3) iFlow CLI用于高效上下文工程的智能体框架。开发了ROME开源智能体，基于超过100万条轨迹训练，采用数据组合协议合成复杂行为，并提出基于交互的策略对齐算法IPA，通过语义交互块而非单个token来分配信用。

Result: 在结构化设置中评估ROME，并引入Terminal Bench Pro基准测试。ROME在SWE-bench Verified和Terminal Bench等基准测试中表现出色，证明了ALE基础设施的有效性。

Conclusion: ALE提供了一个系统化的智能体学习生态系统，通过其组件和训练方法能够有效支持智能体在多轮交互环境中的学习和优化，ROME的成功验证了该框架的实用价值。
...
### [152] [Semi-Automated Data Annotation in Multisensor Datasets for Autonomous Vehicle Testing](https://arxiv.org/abs/2512.24896)
...
<details>
  <summary>Details</summary>
Motivation: 手动标注异构驾驶数据成本高、耗时长，需要高效解决方案来支持波兰自动驾驶研究的大规模数据集创建。

Method: 采用人在回路方法，结合AI与人工专业知识，包括自动生成初始标注、迭代模型重训练、数据匿名化和域适应技术，核心基于3D目标检测算法。

Result: 开发出的工具和方法实现了显著的时间节省，确保了跨不同传感器模态的一致高质量标注，直接支持DARTS项目加速标准化格式标注数据集的准备。

Conclusion: 该半自动标注流水线有效解决了大规模驾驶数据标注的挑战，为波兰自动驾驶研究提供了技术基础，平衡了效率与标注质量。
...
### [153] [Iterative Deployment Improves Planning Skills in LLMs](https://arxiv.org/abs/2512.24940)
*Augusto B. Corrêa,Yoav Gelberg,Luckeciano C. Melo,Ilia Shumailov,André G. Pereira,Yarin Gal*

Main category: cs.AI

TL;DR: 迭代部署LLM并通过用户数据筛选进行微调，可显著改变模型特性，在规划任务中实现能力提升和泛化，该过程在理论上等价于隐式奖励的强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 探索迭代部署大型语言模型时，通过用户从先前模型部署中精心筛选数据进行微调，如何影响后续模型特性变化，特别是这种机制与强化学习的理论联系及其对AI安全的影响。

Method: 采用迭代部署机制：1）部署LLM；2）用户从部署中筛选数据；3）用筛选数据微调新模型；4）重复此过程。在多个规划领域测试该机制，并进行理论分析证明其与强化学习的等价性。

Result: 在规划任务中观察到显著改进：后续模型展现出规划技能提升，并出现泛化能力，能够发现比初始模型长得多的规划方案。理论分析表明迭代部署实现了外层循环的强化学习训练，具有隐式奖励函数。

Conclusion: 迭代部署机制可视为显式强化学习的替代训练方案，依赖数据筛选而非显式奖励。这对AI安全有重要启示：重复部署隐含的奖励函数未明确定义，可能对未来模型部署特性产生意外影响。
...
### [154] [AMAP Agentic Planning Technical Report](https://arxiv.org/abs/2512.24957)
...
TL;DR: STAgent是一个专门用于时空理解的智能体大语言模型，能够处理受限兴趣点发现和行程规划等复杂任务，通过工具交互和分层训练方法在保持通用能力的同时提升专业性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个专门针对时空理解任务的智能体大语言模型，解决传统模型在处理复杂时空任务（如受限兴趣点发现和行程规划）时的局限性，同时保持模型的通用能力。

Method: 采用三个关键技术：1）包含10+领域特定工具的稳定工具环境，支持异步训练；2）分层数据筛选框架，以1:10,000的比例从海量数据中筛选高质量查询；3）级联训练方法，包括种子SFT阶段（评估查询难度）、第二SFT阶段（高确定性查询微调）和最终RL阶段（利用低确定性数据）。模型基于Qwen3-30B-A3B初始化。

Result: STAgent在TravelBench基准测试中表现出色，同时在广泛的通用基准测试中保持了其通用能力，证明了所提出的智能体模型的有效性。

Conclusion: STAgent通过专门的工具环境、高质量数据筛选和级联训练方法，成功创建了一个在时空理解任务上表现优异且不牺牲通用能力的智能体大语言模型，为复杂时空任务的解决提供了有效方案。
