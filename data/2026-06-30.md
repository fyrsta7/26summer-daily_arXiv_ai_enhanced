...
- [MOA: A Profiling-Guided LLM Framework for Memory-Optimization Automation at Codebase Scale](http://arxiv.org/abs/2606.31368v1) — 相关度 0.96：MOA用三个LLM agent自动检测并修复大规模C/C++内存低效，profiling指导加静态分析器生成与补丁生成，堆和二进制体积显著下降且专家接受率高。
- [JETO-Bench: A Reproducible Benchmark for Execution Time Improvement Patches in Java](http://arxiv.org/abs/2606.31767v1) — 相关度 0.95：构建可扩展的执行时间改进补丁基准并评测 OpenHands 编码 agent，直接属于自动程序性能优化与 agent 评测。
- [SWE-Router: Routing in Multi-turn Agentic Software Engineering Tasks](http://arxiv.org/abs/2607.00053v1) — 相关度 0.95：提出基于轨迹的价值路由SWE-Router，让便宜模型先探索再由部分轨迹决定是否升级，显著提升多轮SWE agent成本效率并给出最优性保证。
- [A Multi-Dimensional, Per-Pass Empirical Study of the LLVM Optimization Pipeline](http://arxiv.org/abs/2606.31238v1) — 相关度 0.94：对LLVM -O3管线做逐pass的8.4万次系统测量，揭示非单调性、帕累托支配与能量收益，为编译器自动调优与phase ordering提供关键实证。
- [AlgoBench: Benchmarking Algorithmic Adaptation in Code Generation](http://arxiv.org/abs/2607.00062v1) — 相关度 0.93：AlgoBench通过约束变换自动生成新算法问题并引入复杂度感知指标，评估代码LLM的算法改编能力，为代码生成benchmark提供新方法。
- [Benchmarking the Benchmarks: A Validity Audit of Tool-Calling Evaluation](http://arxiv.org/abs/2607.02577v1) — 相关度 0.90：系统审计工具调用基准的评估器有效性，提出归因分解指标与可配置基准，对 agent 评测机制有直接改进价值。
- [AdaTrans: Automated C to Rust Transformation via Error-Adaptive Repair](http://arxiv.org/abs/2606.31706v1) — 相关度 0.85：提出基于错误自适应修复的 C 到 Rust 自动转换框架，结合 RAG 与多阶段验证，直接面向代码变换与修复 agent。
- [FeatX: Editing Software by Editing Features for Repository-Level Code Evolution](http://arxiv.org/abs/2606.31206v1) — 相关度 0.85：提出面向feature的仓库级代码编辑工具FeatX，用层次化feature结构与三阶段Evolution Agent生成补丁，显著降低认知负荷并提升定位效果。
- [An Empirical Study of Security Calibration in Large Language Models for Code](http://arxiv.org/abs/2606.31159v1) — 相关度 0.82：首次大规模研究代码生成LLM的安全校准并检验校准引导修复，对代码agent的可信评估与安全修复有直接参考。
- [The Illusion of Safety: Multi-Tier Verification of AI vs. Human C++ Code](http://arxiv.org/abs/2607.00107v2) — 相关度 0.82：构建多层级验证基准比较 AI 与人类 C++ 代码安全性，对 AI 生成代码的正确性与安全验证有重要价值。
- [When Not to Write Memory: Governing False Promotion from Correlated Agent Traces](http://arxiv.org/abs/2607.02579v1) — 相关度 0.80：研究 agent 记忆写入路径中的错误提升治理，提出依赖感知的支持度评估策略，可迁移到代码 agent 的记忆与可靠性设计。
- [Falsification, Not Exposure: An Internally Preregistered Placebo-Controlled Decomposition of Self-Repair Feedback in Frozen Small Code Models](http://arxiv.org/abs/2606.31511v1) — 相关度 0.78：通过安慰剂对照分解自修复反馈，研究代码模型重试机制中的可执行反例价值，对代码 agent 反馈设计与评测有直接迁移价值。
- [CoCoMUT: A Tool for Code-Context Mining and Automated Dataset Generation](http://arxiv.org/abs/2606.31971v1) — 相关度 0.70：提供 Java 方法级代码上下文挖掘与数据集生成工具，可支持代码检索、测试、修复等 SE agent 的数据准备。
- [Do Machines Struggle Where Humans Do? LLM and Human Comprehension of Obfuscated Code](http://arxiv.org/abs/2606.31725v1) — 相关度 0.70：比较 LLM 与人类在混淆代码理解上的失败模式，对代码理解模型评估与可解释性有参考价值。
- [ScratchWorld: Evaluating If World Models Compute Executable Consequences](http://arxiv.org/abs/2606.31689v1) — 相关度 0.60：将 Scratch 程序作为可执行世界评测模型对状态变化和因果后果的预测，对程序执行理解与 agent 状态追踪评测有参考价值。
- [Governance Gaps in Agent Interoperability Protocols: What MCP, A2A, and ACP Cannot Express](http://arxiv.org/abs/2606.31498v1) — 相关度 0.58：系统分析MCP/A2A/ACP等agent互操作协议在治理需求上的能力缺口，为构建可治理的多agent代码协作系统提供设计依据。
