...
Selection reason: 系统研究MLLM生成网页的跨环境兼容性并构建数据集和检测工具，为前端代码生成agent提供可靠性评测方法。
...
Selection reason: 从静态调用图和多驱动覆盖角度分析fuzzing结构探索，其图抽象和分析方法可用于agent驱动的测试优化。
...
Selection reason: 提出基于测试用例的Rocq规格质量评估框架，避免与证明难度混淆，对LLM生成形式规格的评测有直接贡献。
...
Selection reason: 探索仅从测试和动态轨迹合成形式规格的黑盒方法，结合模型检查反馈迭代，对规格生成和agent验证有参考价值。
...
Selection reason: 系统刻画LLM修复补丁的冗长性问题并提出生成后精简适配器，直接改善APR输出可审查性，与代码agent修复高度相关。
...
Selection reason: 虽然面向自动驾驶垂直应用，但揭示了LLM辅助动态分析中构建集成是主要瓶颈，其失败分类对agent工具链设计有迁移启示。
