...
### [57] [A Taxonomy of Human-Robot Teamwork Requirements](https://arxiv.org/abs/2607.27302)
...
TL;DR: 提出一个人机团队协作（HRT）要求的层次分类法，包含6个高层类别和21个低层子类别，并通过专家评估和独立语料库验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的人机团队协作要求分散且缺乏统一框架，难以应对协作任务的复杂性，需要系统化的分类体系。

Method: 从14个跨领域来源提取361条要求，通过迭代分类和细化构建两级层次分类法，并用5位专家评估和独立448条要求语料库进行验证。

Result: 得到6个高层类别和21个低层子类别的分类法，专家评估确认其有效性，独立语料库展示其适用性。

Conclusion: 该分类法为理解和组织人机团队协作要求提供了统一框架，有助于跨领域应用。

中文摘要：自主系统越来越多地部署在安全和关键任务领域，在这些领域中，人类和机器人必须作为团队协作以完成复杂任务。现有的人机团队协作要求分散在不同的来源中，缺乏一个统一的框架来处理协作式人机任务的复杂性。我们通过提出一个从（学术和工业）文献、标准和监管指南分析中得出的人机团队协作（HRT）要求分类法来解决这一差距。我们从14个跨领域来源中提取了一个包含361条要求的构建语料库。通过迭代分类和细化，我们开发了一个包含6个高层类别和21个低层子类别的两级层次分类法，这些类别区分了信息提供、关系控制、决策支持、安全机制、性能监控和基础系统能力。我们通过5位领域专家的评估和在独立构建的448条要求语料库上的实用性演示来验证该分类法，该语料库来自19个来源，涵盖六个HRT领域。
...
### [58] [SIGIL: Compiling Agent Skills into Typed Harnesses](https://arxiv.org/abs/2607.27309)
...
### [59] [TrustChain-Review: A Risk-Adaptive Blockchain and Game-Theoretic Framework for Trustworthy AI-Assisted Code Review](https://arxiv.org/abs/2607.27310)
*Mohammad Naserameri*
...
TL;DR: 提出了TrustChain-Review框架，结合区块链证据、博弈论激励和风险自适应治理，在代码审查中平衡信任与成本。


<details>
  <summary>Details</summary>
Motivation: AI辅助开发加速了编码与审查，但导致问责困难，如验证不足、审查不严、声誉记录难审计。

Method: 使用区块链证据层、三方博弈模型和基于成本收益的风险规则，基于Diff质量估计数据集进行模拟，比较六种治理配置。

Result: 全证据配置信任和检测最强但成本高；风险自适应配置降低成本并提升效率，但信任和检测略低。

Conclusion: 高风险变更宜用强证据治理，常规低风险变更宜用选择性治理以平衡成本与可信度。

中文摘要：背景：AI辅助软件开发可以加速编码和审查，但也使得问责变得更加困难。开发人员可能提交未充分验证的代码，审查人员可能在有限检查的情况下批准更改，而集中式声誉记录可能难以审计。目标：本研究引入TrustChain-Review，一个结合可验证证据、战略激励和风险敏感治理的框架，以支持更值得信赖的代码审查。方法：该框架包括基于区块链的证据层、针对开发人员、审查人员和平台的三方博弈论模型，以及一条规则，即当预期收益证明其成本合理时应用更强的治理。评估使用受Diff质量估计数据集校准的受控模拟。比较了六种治理配置，在30次独立运行中评估声誉准确性、信任收敛、恶意审查检测、表面审查检测、平台净效用、治理成本和成本效率。结果：全证据配置产生最强的声誉、信任和检测结果，但其治理成本也最高。风险自适应配置通过选择性应用更强的控制来降低该成本并提高成本效率，尽管其信任和检测结果低于全证据设置。结论：强基于证据的治理最适合高风险或审计敏感的更改。对于常规或较低风险的贡献，选择性治理在可信度和运营成本之间提供了更实用的平衡。
...
### [60] [PROGRESS: Property-Guided Regression Search for Semantic Falsification](https://arxiv.org/abs/2607.27359)
...
### [61] [From Backlog Items to Security Guidance: Towards Continuous Security Compliance](https://arxiv.org/abs/2607.27374)
...
TL;DR: 提出了一种基于NLP的待办事项丰富系统，通过分类器和RAG流水线自动检测安全相关待办事项并链接到安全需求，在工业环境中验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 在受监管领域的持续软件工程中，待办事项通常缺乏明确的安全需求，工程师难以从简短描述中推断安全相关性，缺乏及时指导。

Method: 结合安全相关性分类器（面向召回率）与基于安全需求文档的检索增强生成（RAG）流水线，对待办事项进行安全检测和需求链接；分类器在五个基准上评估，RAG流水线由两名从业者评估。

Result: 发布288个待办事项数据集（Fleiss κ=0.787）；分类器F2=0.774（分布内），零样本G-measure≈0.65；RAG检索的24个条款中12个被评为相关性≥4/5。

Conclusion: NLP-based产品待办事项丰富能早期识别安全需求，有助于持续安全合规，但RAG流水线仍需改进。

中文摘要：在受监管领域的持续软件工程中，工程团队需要在整个开发生命周期中处理安全问题。然而，使安全需求在待办事项中明确表达仍然存在问题。工程师必须从简短、自由形式的描述中推断待办事项的安全相关性，并且往往缺乏关于适用需求的及时指导。我们提出了一种基于NLP的待办事项丰富系统，用于检测与安全相关的待办事项并将其链接到相关的安全需求。该方法将安全相关性分类器与检索增强生成（RAG）流水线相结合，后者基于安全需求文档。该方法在高度受监管领域的一家大型企业中进行了开发和评估。我们提出三项贡献。首先，我们发布了一个包含288个待办事项的数据集，由九名安全从业人员标注安全相关性，具有高度一致性（Fleiss' κ=0.787）。其次，一个面向召回率的分类器，在分布内达到F2=0.774，在五个既有基准上的平均零样本G-measure约等于0.65，匹配或超过了大多数已发表的经典机器学习和开源GPT基线。第三，我们初步评估了一个四阶段安全需求文档接地RAG流水线，由两名从业人员使用公司内部安全策略和CIS基准对工业待办事项进行测试。在检索到的24个条款中，有12个被评为相关性至少为4/5。我们的发现提供了初步指标，表明基于NLP的产品待办事项丰富可以支持工程师在开发过程早期识别安全需求。通过这项工作，我们旨在通过主动引入安全需求，促进持续软件工程中的持续安全合规。
...
### [62] [SWE-NFI: Studying and Benchmarking Coding Agents for Non-Functional Improvements](https://arxiv.org/abs/2607.27409)
...
### [63] [LimICE: Integrating LLM into ICE Framework for Efficient Loop Invariant Inference](https://arxiv.org/abs/2607.27606)
...
### [64] [Not as Sweet by Another Name: An Empirical Study of Format Robustness in LLM Document Workflows](https://arxiv.org/abs/2607.27648)
...
### [65] [Can Large Language Models Resolve Real Java Merge Conflicts? An Evaluation with a Calibrated LLM-as-Judge](https://arxiv.org/abs/2607.27674)
*Bowen Shen*
...
### [66] [A comparative analysis of automated techniques for security bug report identification](https://arxiv.org/abs/2607.27893)
*Muhammad Laiq*
...
TL;DR: 对识别安全相关错误报告的多种自动化技术进行了基准比较，发现SetFit整体最优，RoBERTa紧随其后，传统ML在某些场景下仍具竞争力，而GPT-5.2表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有研究零散，缺乏统一的比较框架，导致难以评估不同技术在识别安全相关错误报告任务上的实际效果，无法为研究者和实践者提供清晰的技术选择指导。

Method: 使用基准数据集，对7种技术（逻辑回归、SVM、随机森林、GPT-5.2、BERT-base、RoBERTa、SetFit）进行了系统的比较分析，并进行了跨项目迁移学习实验。

Result: SetFit取得最高F1分数0.80，在四个数据集中的三个上最优；RoBERTa表现接近；逻辑回归在特定情境下仍是强基线；GPT-5.2在零样本和少样本下表现差；跨项目迁移效果因项目特性而异。

Conclusion: SetFit是当前识别安全相关错误报告的最优技术，RoBERTa可作为替代；传统ML方法仍有价值；GPT-5.2不适合该任务；迁移学习需谨慎使用，需考虑项目数据规模与特异性。

中文摘要：及时识别与安全相关的错误报告对于最小化软件系统中的漏洞窗口至关重要。手动筛选传入的错误报告以识别安全相关问题对于大规模软件系统而言耗时、易错且不可扩展。因此，已经提出了各种自动技术，包括传统机器学习（ML）技术和大型语言模型，以促进这项任务。然而，文献仍然零散。大多数研究引入或优化一种特定技术，并针对有限的基线集进行评估，通常在不同的实验设置下进行。因此，很难比较它们的结果并得出关于现有方法有效性的可靠结论，使研究人员和从业者缺乏关于哪些技术最适合该任务的明确指导。为了解决这一差距，我们使用基准数据集对几种有前景的自动技术进行了比较分析，以识别与安全相关的错误报告。我们评估了逻辑回归、支持向量机、随机森林、OpenAI的GPT-5.2、BERT-base、RoBERTa和SetFit（一种最先进的少样本学习框架）。我们的结果表明，SetFit实现了最佳的整体性能，F1分数达到0.80，并在四个数据集中的三个上优于其他技术。RoBERTa表现具有竞争力，并在某些项目中接近SetFit，而传统ML技术，尤其是逻辑回归，在某些情况下仍然是一个强基线。相反，GPT-5.2在零样本和少样本设置中表现不佳。此外，跨项目实验表明，迁移学习可以改善数据有限项目的性能，但可能降低具有强烈项目特定特征项目的性能。
...
### [67] [The Case for Vibe Modeling: A Missing Step in AI-Based Trustworthy Software Development](https://arxiv.org/abs/2607.27923)
...
TL;DR: 本文提出“氛围建模”作为自然语言与代码生成之间的轻量级中间抽象，并通过学生调查评估其对LLM输出理解、验证、信任和有用性的影响。


<details>
  <summary>Details</summary>
Motivation: 当前AI辅助开发过于关注直接生成代码，缺乏保留人类意图和支持行为推理的中间表示，导致理解、验证、可追溯性和信任问题。

Method: 进行了一项学生调查研究，在多种AI辅助开发场景中测量对LLM输出理解、验证工作量、信任度以及对氛围建模的感知有用性。

Result: 研究结果提供了关于氛围建模潜力的初步证据，但具体数据未在摘要中给出，旨在为未来研究提供参考。

Conclusion: 氛围建模有望支持可信和可解释的AI软件工程，未来需要进一步研究验证其效果。

中文摘要：大型语言模型（LLM）越来越多地被用于根据自然语言提示生成软件工件。虽然这实现了快速原型设计并降低了软件创建的门槛，但也带来了与理解、验证、可追溯性和信任相关的挑战。在本文中，我们认为当前基于AI的开发实践过于侧重于直接生成代码，而对保留人类意图并支持系统行为推理的中间表示关注不足。我们主张将“氛围建模”（vibe modeling）作为自然语言交互与代码生成之间的轻量级中间抽象。为探索其潜力，我们进行了一项学生调查研究，考察了在几种AI辅助开发场景中，对LLM输出理解、验证工作、信任以及氛围建模感知有用性的看法。我们的结果旨在为未来通过氛围建模实现可信且可解释的基于AI的软件工程研究提供信息。
...
### [68] [Specification-Guided Synthesis of Deadlock-Free Communication Protocol Refinements with Large Language Models](https://arxiv.org/abs/2607.27964)
...
### [69] [Advancing Awkward Arrays for High-Performance CPU and GPU Processing](https://arxiv.org/abs/2607.28145)
...
TL;DR: 本文介绍了 Awkward Array 的 GPU 后端最新进展，利用 CUDA 和 CCCL 加速嵌套、可变长度数据的不规则工作负载。


<details>
  <summary>Details</summary>
Motivation: HL-LHC 分析日益依赖加速硬件，而嵌套和可变长度的数据结构因间接索引、分段操作和不规则内存访问模式难以在 GPU 上高效加速，因此需要专门的 GPU 后端。

Method: 提出了基于 NVIDIA CCCL 的 CUDA 实现，包括优化内存管理和针对 ragged 数组的分段归约算法，并描述了后端架构与自动化验证框架。

Result: 基准测试表明，与 CPU 和 CuPy 相比，CUDA 实现在不规则工作负载下显著提高了 GPU 吞吐量。

Conclusion: 这些开发在保持现有 Python 编程模型的同时，大幅提升了 GPU 性能，为高能物理等领域的 irregular 数据处理提供了高效加速方案。

中文摘要：Awkward Array 是一个用于表示和处理嵌套、可变长度数据的 Python 库，在高能物理领域被广泛使用。随着 HL-LHC 分析日益依赖加速硬件，不规则工作负载的高效执行变得至关重要。虽然密集数值数组能够自然地映射到 GPU，但嵌套和可变长度数据结构由于需要间接索引、分段操作和不规则内存访问模式，其加速仍然困难得多。我们介绍了 Awkward Array GPU 后端的最新进展，包括基于 NVIDIA CUDA Core Compute Libraries (CCCL) 构建的 CUDA 实现、优化的内存管理以及针对 ragged 数组的分段归约算法。这些开发在保留现有 Python 编程模型的同时，显著提升了不规则工作负载下的 GPU 吞吐量。我们描述了后端架构、自动化验证框架以及比较 CPU、CuPy 和 CUDA 实现的基准测试结果。
...
### [70] [Integrating AI into Requirements Quality Learning in Software Engineering Education: A TPACK-Guided Empirical Study](https://arxiv.org/abs/2607.28176)
...
TL;DR: 本研究探讨了TPACK引导的多代理AI工具在硕士级需求工程作业中的整合，发现学生有选择地使用AI作为分析支持，并在具体质量维度上有所提升，同时存在信任与可用性挑战。


<details>
  <summary>Details</summary>
Motivation: 由于生成式AI在软件工程实践中的快速采用，需要在软件工程教育中，特别是需求工程等概念密集型课程中，引入有教学法基础的AI整合方法。

Method: 使用混合方法设计（N=100；分析72份提交），在硕士级需求质量分析作业中整合TPACK引导的多代理AI工具，考察结构化作业设计对学生AI使用、理解及看法的影响。

Result: 学生有选择地使用AI工具，主要作为分析和评估的支持而非自动化；在价值表达和可测试性等具体质量维度上对齐改进最明显，可协商性效果混合；学生报告有条件的信任、主动改进和对质量标准的增强意识，同时存在适度的可用性挑战。

Conclusion: TPACK引导的支架式教学能够将AI赋能力量与教学目标和RE内容对齐，为RE教育中负责任的AI整合提供设计指导。

中文摘要：生成式人工智能（AI）在软件工程（SE）实践中的快速采用，产生了对SE教育中AI整合的基于教学法的需求，尤其是在需求工程（RE）等概念密集型学科中。本研究考察了TPACK引导的多代理AI工具在硕士级RE作业中的整合，该作业涉及需求质量分析。采用混合方法设计（N=100；分析了72份提交），我们考察了结构化作业设计如何塑造学生的AI使用，影响他们对用户故事质量标准理解，以及影响他们对AI益处和局限性的看法。结果表明，学生有选择地使用AI工具，主要作为分析和评估的支持，而非自动化。对于结构上具体的需求质量维度，如价值表达和可测试性，对齐改进最为明显，而可协商性显示出混合效果。学生报告了有条件的信任、主动改进和对质量标准的增强意识，同时存在适度的可用性挑战。研究结果表明，TPACK引导的支架式教学可以将AI的赋能力量与教学目标和RE内容相结合，为RE教育中负责任的AI整合提供设计指导。
...
### [71] [Agentic Metaverse Services: A New As-a-Service Paradigm](https://arxiv.org/abs/2607.28242)
...
TL;DR: 本文综述了生成式AI赋能的智能体服务在元宇宙中的新范式，提出了智能体元宇宙服务（AMServ）和元宇宙智能体即服务（Meta-AaaS）的概念、原则与应用。


<details>
  <summary>Details</summary>
Motivation: 随着GenAI将对话式聊天机器人升级为智能体AI，服务形式转向智能体服务与AaaS，而元宇宙作为虚拟生态系统需要新的服务模式，因此结合两者形成AMServ和Meta-AaaS，以推动服务计算新范式。

Method: 通过概述GenAI下智能体与服务的演进，分析智能体服务在元宇宙中的角色与原则，提出AMServ和Meta-AaaS的形式、特征与原则，并讨论典型应用例子。

Result: 给出了AMServ和Meta-AaaS的定义、原理和应用示例，并指出了未来发展趋势。

Conclusion: AMServ和Meta-AaaS将为AI时代人类生活和服务带来重大机遇，推动新兴服务业的快速发展。

中文摘要：生成式人工智能（GenAI）正在重构数字虚拟世界，通过增强智能体在自主学习、多模态交互、内容生成和协同决策方面的能力来升级智能体。特别是，从对话式聊天机器人向智能体人工智能的转变——这是GenAI最近最重大的技术突破——带来了一种新的服务形式，即智能体服务和智能体即服务（AaaS），其中智能体的能力被封装，如感知、决策、执行、协作和内容生成，以向用户提供定制化的智能体服务。元宇宙是一个面向人类生活、工作、创造和娱乐的虚拟生态系统，由新一代数字技术支持。通过将智能体服务与元宇宙相结合，产生了一种用于元宇宙业务处理的智能体元宇宙服务，记为AMServ，作为元宇宙服务的新形式。元宇宙环境中的AaaS，记为Meta-AaaS，作为实现AMServ的一种方法，已成为智能体服务和服务计算的新范式。本文概述了GenAI赋能的智能体和服务的发展历程与新特征，揭示了智能体服务在元宇宙环境中的角色和原则，提出了AMServ和Meta-AaaS的形式、特征和原则，讨论了AMServ和Meta-AaaS的典型应用示例，最后指出了AMServ和Meta-AaaS的新趋势和研究方向。AMServ和Meta-AaaS将为人工智能时代的人类社会和服务带来巨大机遇，并促进未来新兴服务业的快速发展。
...
### [72] [Agentic Method for Deterministic Validation of Legacy Code Migration](https://arxiv.org/abs/2607.28271)
...
### [73] [From Textual Requirements to Microservice Architectures - A Comprehensive Evaluation of LLM-Based Design Synthesis](https://arxiv.org/abs/2607.28307)
...
TL;DR: 本研究测试OpenAI o3从文本需求生成微服务架构的能力，发现少样本提示显著优于零样本，尤其在服务识别和通信恢复方面，专家评估也支持这一结果。


<details>
  <summary>Details</summary>
Motivation: 微服务识别主要依赖代码且手动，早期设计阶段仅有文本需求时缺乏自动化方法。尽管LLM有潜力，但缺乏从自然语言需求合成完整架构的经验证据。

Method: 使用OpenAI o3，在零样本和少样本提示下对Bookstore和PetClinic两个系统生成架构，通过参考架构比较（精确率、召回率、F1）和盲法专家评估（正确性、完整性、模块性、合理性）进行混合方法评价。

Result: 少样本提示下服务识别F1从0.79提升至0.97，通信恢复F1从0.61提升至0.82，且减少了不支持的依赖；专家评估认为少样本架构更模块化、连贯和合理。

Conclusion: OpenAI o3在少样本示例提示下能有效从文本需求合成微服务架构，但结果受模型和上下文限制，需进一步验证其通用性。

中文摘要：微服务架构已成为现代化单体系统的主流选择，然而识别合适的服务仍然具有挑战性且在很大程度上依赖人工。现有的分解方法主要基于代码，限制了在仅有文本需求可用的早期设计阶段的适用性。尽管大型语言模型（LLMs）取得了进展，但关于它们从自然语言需求中合成完整微服务架构（包括服务定义和服务间交互）的能力，现有的经验证据仍然有限。本研究探讨了LLM是否能够弥合需求工程与架构设计之间的鸿沟，仅从文本需求生成架构，并评估结果的结构一致性和感知质量。我们采用混合方法研究，使用OpenAI o3在零样本（ZS）和少样本（FS）提示条件下，针对两个系统（Bookstore、PetClinic）各执行一次。通过（i）与参考架构进行精确率、召回率和F1分数比较，用于服务识别和通信恢复；（ii）盲法专家评估正确性、完整性、模块性和合理性，并综合开放式反馈来评估架构。OpenAI o3在FS提示下识别服务的一致性更高（ZS的F1=0.79对比FS的F1=0.97）。通信恢复更具挑战性：ZS产生密集架构，召回率高但精确率低（F1=0.61），而FS提高了一致性，达到F1=0.82并减少了不支持的依赖。专家评估证实了这些结果，FS架构被认为比ZS输出更具模块性、连贯性和合理性。OpenAI o3在示例提示引导下显示出需求驱动合成的潜力。结果来自两个小型系统，具有模型和上下文特异性，并非与模型无关的证据。
...
### [74] [Structural Validation of LLM-Generated Microservice Decompositions Using Source-Code Dependencies](https://arxiv.org/abs/2607.28331)
...
TL;DR: 评估LLM生成的微服务分解时，必须控制映射覆盖率，否则提示策略间的差异可能反映方法偏差而非真实架构质量。


<details>
  <summary>Details</summary>
Motivation: 单体系统分解为微服务是软件现代化的关键，但LLM生成的分解是否保留源代码结构依赖尚不明确，需要评估其结构一致性。

Method: 提出基于静态依赖分析的自动化验证流水线，对OpenAI o3生成的PetClinic和Bookstore微服务分解，比较零样本和少样本提示，使用TPD和TVD指标，并进行鲁棒性分析控制类到服务映射覆盖率。

Result: 归一化后两种提示策略的结构一致性等价，TPD值分别为68.0%（PetClinic）和83.3%（Bookstore）。

Conclusion: 结构评估应明确控制映射覆盖率，否则提示策略间的表面差异可能源于方法偏差而非真实架构质量。

中文摘要：将单体系统分解为微服务是软件现代化中的关键活动。尽管大型语言模型（LLMs）能够从文本需求生成语义上合理的分解方案，但这些方案是否保留了源代码中实现的结构依赖仍不明确。本文评估了OpenAI o3为PetClinic和Bookstore系统生成的微服务分解的结构一致性。我们提出了一种基于静态依赖分析的自动化验证流水线，并使用依赖保留（TPD）和依赖违规（TVD）指标比较了零样本和少样本提示。进行了鲁棒性分析以控制类到服务映射覆盖率的差异。归一化后，两种提示策略产生了等效的结构一致性，TPD值分别为68.0%（PetClinic）和83.3%（Bookstore）。研究结果表明，对LLM生成的分解进行结构评估时应明确控制映射覆盖率，因为提示策略之间的明显差异可能反映方法偏差而非真实架构质量。
...
### [75] [Demystifying Solana Bots: From GitHub Blueprints to On-Chain Fingerprints](https://arxiv.org/abs/2607.28424)
...
TL;DR: 对Solana机器人进行了大规模实证研究，从GitHub仓库和链上交易两个视角，提出了15类机器人分类法和五阶段操作流水线，揭示了其链上行为差异。


<details>
  <summary>Details</summary>
Motivation: 缺乏对Solana机器人实际实现及其链上表现的系统性理解，现有研究多关注性能或安全，未覆盖机器人行为全貌。

Method: 收集586个GitHub机器人仓库和200个链上机器人地址（含4400万笔交易），采用混合方法分析实现与链上指纹，归纳分类法和流水线。

Result: 提出5个领域15类机器人分类法，识别共享的五阶段操作流水线，并发现跨平台和资产的交易行为系统性差异。

Conclusion: 为Solana机器人研究提供未来方向，并为构建和运营机器人提出实践建议。

中文摘要：Solana是一个新兴的区块链平台，旨在实现高吞吐量和低交易费用，使得大规模提交交易变得廉价，因此增加了机器人垃圾邮件及相关金融剥削的风险。Solana机器人通常是链下软件系统，通过构建和提交交易在竞争性的链上执行环境中运作，到2026年1月，去中心化交易所上与机器人相关的交易日均交易量超过2.5亿美元。先前关于Solana的研究考察了系统性能、智能合约安全以及特定的链上现象。然而，我们仍然缺乏对Solana机器人在实践中实现什么以及这些实现如何表现为可观察的链上执行指纹的系统性理解。为弥补这一空白，我们从两个互补视角对Solana机器人进行了大规模实证研究：(i)从GitHub收集的586个机器人仓库，以及(ii)Solana上的200个机器人地址，涵盖超过4400万笔链上交易。我们的研究得出了一个基于实现的Solana机器人分类法，包含五个领域（例如，交易操作、MEV和链上分析）下的15个类别，识别了在机器人实现中表现出的一个很大程度上共享的五阶段操作流水线，并揭示了Solana机器人在不同交易平台和资产上的链上交易行为的系统性变化。基于我们的发现，我们强调了未来的研究方向，并为在Solana区块链上构建和运营机器人提供了建议。
...
### [76] [Emerging Challenges in Threat Modeling for GenAI-Augmented Systems: A View from the Trenches](https://arxiv.org/abs/2607.28431)
...
TL;DR: 本文在中小型企业环境中探索性评估了三种GenAI感知威胁建模方法，发现它们对供应链和人为中心等特定风险支持有限，并报告了从业者的可用性看法。


<details>
  <summary>Details</summary>
Motivation: 随着GenAI系统普及，传统威胁建模方法（如STRIDE）无法覆盖GenAI特定风险，而现有方法在SME环境中的适用性尚未得到充分研究，因此需要探索可行的评估方法。

Method: 采用快速文献综述筛选相关技术，选出三种方法，并将其系统应用于一个包含GenAI增强系统的工业案例研究，同时收集从业者的反馈。

Result: 不同方法识别的威胁存在差异，且对软件供应链和人为中心安全等GenAI特定风险类别的支持有限；从业者报告了可用性和集成方面的挑战。

Conclusion: 现有GenAI感知威胁建模方法在SME工作流中具有一定潜力，但需改进对特定风险类别的覆盖，并降低采用难度以适应中小企业需求。

中文摘要：威胁建模仍然是安全软件工程中的一项核心任务，因为它能够从系统架构中识别安全问题。随着生成式人工智能（GenAI）在软件系统中日益普及，传统的威胁建模方法（如STRIDE）已不足以评估新兴的GenAI特定风险。在这项工作中，我们展示了在中小型企业（SME）环境中对GenAI感知威胁建模方法进行探索性评估的首批结果。为此，我们进行了快速文献综述以选择相关技术，并系统地将三种入围方法应用于涉及GenAI增强系统的工业案例研究。结果突出了每种技术所识别威胁的差异，并揭示了对某些GenAI特定风险类别的有限支持，特别是与软件供应链和以人为中心的安全问题相关的类别。我们进一步报告了从业者对这些方法在SME开发工作流中的可用性和集成性的看法，包括他们感知到的努力和采用挑战。
...
### [77] [CoGate: Confidence-Gated Co-Decoding for Secure Code Generation](https://arxiv.org/abs/2607.28529)
...
### [78] [PAIChecker: Uncovering and Checking PR-Issue Misalignment in SWE-Bench-Like Benchmarks](https://arxiv.org/abs/2607.28587)
...
### [79] [Change2Task: From Repository Changes to Executable Coding Agent Tasks and Environments](https://arxiv.org/abs/2607.28591)
