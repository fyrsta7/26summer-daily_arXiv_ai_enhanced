...
- [AgentMeter: Evaluating Model-CLI Matching for CLI-Based Local Task-Solving Agents](http://arxiv.org/abs/2606.21140v1) — 相关度 0.90：提出模型-CLI匹配评测基准与成本感知指标，强调本地任务agent应作为模型与CLI整体评测，直接服务agent评测。
- [The Alignment Problem in Constrained Code Generation](http://arxiv.org/abs/2606.21619v1) — 相关度 0.90：系统分析约束解码与LLM/目标语言不对齐导致的正确性灾难，对带形式保证的代码生成agent设计有直接指导意义。
- [KBSpec: LLM-driven Formal Specification Generation with Evolving Domain Knowledge Base](http://arxiv.org/abs/2606.21339v1) — 相关度 0.88：利用官方文档与验证器反馈构建自演化知识库，增强LLM生成JML规范，形式化验证agent的有效方法。
- [Is Agent Code Less Maintainable Than Human Code?](http://arxiv.org/abs/2606.21804v1) — 相关度 0.82：评估coding agent生成代码的可维护性，引入CodeThread控制实验框架，直接关系agent代码质量与评测。
- [Assessing Language Models for Salient Class Identification](http://arxiv.org/abs/2606.21629v1) — 相关度 0.76：构建ApacheJavaCM数据集并评估LM直接识别提交中显著类，轻量SLM即可胜任，可用于代码审查agent。
- [Context-Aware Generative AI for Automated Telecom Test Script Generation](http://arxiv.org/abs/2606.21151v1) — 相关度 0.75：基于知识图谱与MCP的上下文感知测试脚本生成agent，通过增量检测和RAG持续更新测试，方法可迁移到通用测试agent。
- [ATLAS: Agentic Taxonomy of Large-Scale Software Ecosystems](http://arxiv.org/abs/2606.21597v1) — 相关度 0.72：用Designer/Classifier双agent与自纠错循环自动构建软件仓库分层分类法，方法可迁移到代码检索与仓库理解agent。
- [Change Impact Recommendation for JavaScript: Lessons from History and Runtime Analysis](http://arxiv.org/abs/2606.21187v1) — 相关度 0.70：结合历史共变与动态依赖的JavaScript变更影响推荐，分析两类信号互补性，可用于代码变更理解和回归测试agent。
- [ConcernBERT: Learning Responsibilities Using Class Membership](http://arxiv.org/abs/2606.21647v1) — 相关度 0.65：用三元组损失和类成员上下文训练代码实体嵌入，支持职责识别与重构下游任务，可迁移到代码检索与架构恢复。
- [An Exploratory Case Study of LLM-Assisted Refactoring and Gameplay Feature Generation in an Endless Runner Game](http://arxiv.org/abs/2606.21171v1) — 相关度 0.60：对GPT-4o在重构与游戏功能生成中的实证案例，揭示LLM在局部变换与跨系统集成的能力差异，对agent任务边界有参考价值。
