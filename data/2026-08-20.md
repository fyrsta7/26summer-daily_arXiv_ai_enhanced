...
Selection reason: 提出 harnessed agentic RL 框架并显著提升编码 agent 的 SWE-bench 表现。
...
Selection reason: 基于证据引导的 agent skill 自动生成与安全验证框架，直接提升 agent 能力与可靠性。
...
Selection reason: 面向 commit 引入缺陷的差分切片模糊测试，方法可嵌入 agent 的变更验证流程。
...
Selection reason: 建模编码 agent 工作集与一致性债务，揭示上下文可用性对仓库级任务成败的影响。
...
Selection reason: 提供 agent 轨迹可视化与分析工具，支持大规模执行模式的比较与洞察。
...
### [39] [Benchmarking Automated Security Patch Backporting: How Far Are We?](https://arxiv.org/abs/2608.17671)
...
Selection reason: 面向安全补丁自动移植的基准与评测，直接涉及LLM智能体的代码生成与修复能力评估。
...
### [40] [GADR: Gathering Architecture Decision Records from Meeting Transcriptions](https://arxiv.org/abs/2608.17694)
...
Selection reason: 多智能体自纠正工作流从会议文本提取架构决策，其流程模式可迁移到代码智能体的结构化协作。
...
### [41] [What Aggregate Scores Miss: Measuring Item-Level Regressions in Commercial LLM API Migrations](https://arxiv.org/abs/2608.17719)
...
Selection reason: 针对LLM API迁移的细粒度回归评测方法，可迁移到代码智能体的基准评估设计。
