...
- [Agent Lightning v1.0: Towards Harnessed Agentic RL](https://arxiv.org/abs/2608.17528) — 相关度 0.95：提出 harnessed agentic RL 框架并显著提升编码 agent 的 SWE-bench 表现。
- [TRUSS: Towards Task-Reliable and User-Safe Automated Agent Skill Generation](https://arxiv.org/abs/2608.17588) — 相关度 0.90：基于证据引导的 agent skill 自动生成与安全验证框架，直接提升 agent 能力与可靠性。
- [The Working Set of a Coding Agent: Coherence Debt in Repository-Scale Tasks](https://arxiv.org/abs/2608.16630) — 相关度 0.90：建模编码 agent 工作集与一致性债务，揭示上下文可用性对仓库级任务成败的影响。
- [Benchmarking Automated Security Patch Backporting: How Far Are We?](https://arxiv.org/abs/2608.17671) — 相关度 0.85：面向安全补丁自动移植的基准与评测，直接涉及LLM智能体的代码生成与修复能力评估。
- [Graphectory Viewer: A Tool for Process-Centric Analysis of Agentic Software Trajectories](https://arxiv.org/abs/2608.17195) — 相关度 0.85：提供 agent 轨迹可视化与分析工具，支持大规模执行模式的比较与洞察。
- [COMMITGUARD: Differential Slice Fuzzing for Commit-Induced Bug Detection](https://arxiv.org/abs/2608.17401) — 相关度 0.80：面向 commit 引入缺陷的差分切片模糊测试，方法可嵌入 agent 的变更验证流程。
- [GADR: Gathering Architecture Decision Records from Meeting Transcriptions](https://arxiv.org/abs/2608.17694) — 相关度 0.60：多智能体自纠正工作流从会议文本提取架构决策，其流程模式可迁移到代码智能体的结构化协作。
- [What Aggregate Scores Miss: Measuring Item-Level Regressions in Commercial LLM API Migrations](https://arxiv.org/abs/2608.17719) — 相关度 0.55：针对LLM API迁移的细粒度回归评测方法，可迁移到代码智能体的基准评估设计。
