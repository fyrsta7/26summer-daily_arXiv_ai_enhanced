<div id=toc></div>

# Table of Contents

- [cs.SE](#cs.SE) [Total: 23]
- [cs.AI](#cs.AI) [Total: 44]


<div id='cs.SE'></div>

# cs.SE [[Back]](#toc)

### [1] [Beyond "What to Retrieve": Uncertainty in Retrieval-Augmented Code Generation](https://arxiv.org/abs/2607.24884)
*Chandan Kumar Sah,Xiaoli Lian,Li Zhang*

Main category: cs.SE

Comments: 9 pages, 4 figures. Source code and supporting materials are available at https://github.com/Rocky5502/OpenCoder_V1

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: OpenCoder是一个不确定性感知框架，通过估计来源特异性不确定性来过滤和排序异构证据，从而提升仓库级代码生成，但收益依赖于后端。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法主要优化检索相关性，未建模检索证据中的不确定性对下游生成的影响。

Method: 提出OpenCoder框架，估计来源特异性不确定性，用于过滤、排序异构证据，并指导生成、验证和修复；进行析因分析揭示跨来源交互。

Result: 在32任务RepoExec-inline评估中，GPT选择输出正确率从56.25%提升至78.13%，但Gemini改进不显著；目标感知API精炼也显著提升检索。

Conclusion: 支持将不确定性作为仓库级检索、验证和修复的可操作控制信号。

中文摘要：仓库级代码生成依赖于异构证据，这些证据的相关性、兼容性和完整性本质上是不确定的。类似代码示例、仓库上下文和项目特定API可能提供互补信息，但也可能引入噪声、冗余或冲突信号。现有的检索增强方法主要优化检索相关性，而没有显式建模检索证据中的不确定性如何影响下游生成。我们引入了OpenCoder，一个不确定性感知框架，它估计特定来源的不确定性，用于过滤和排序异构证据，并指导生成、验证和修复。对API知识、仓库上下文和类似代码证据进行的析因分析揭示没有通用的可加性来源排序；相反，显著的跨来源交互依赖于伴随证据和LLM后端。在扩展的32任务RepoExec-inline评估中，OpenCoder将GPT选择输出的正确率从基线RAG的56.25%提高到78.13%。然而，它与验证和修复对照组相当，而相应的Gemini改进在统计上不显著，表明存在后端依赖的收益。目标感知的API精炼也显著改善了API集的检索。这些发现支持将不确定性作为仓库级检索、验证和修复的可操作控制信号。

Original abstract: Repository-level code generation relies on heterogeneous evidence whose relevance, compatibility, and completeness are inherently uncertain. Similar-code examples, repository context, and project-specific APIs may provide complementary information, but can also introduce noisy, redundant, or conflicting signals. Existing retrieval-augmented approaches primarily optimize retrieval relevance without explicitly modeling how uncertainty in retrieved evidence affects downstream generation. We introduce OpenCoder, an uncertainty-aware framework that estimates source-specific uncertainty, uses it to filter and rank heterogeneous evidence, and guides generation, verification, and repair. A factorial analysis over API knowledge, repository context, and similar-code evidence reveals no universal additive source ranking; instead, significant cross-source interactions depend on the accompanying evidence and LLM backend. On an expanded 32-task RepoExec-inline evaluation, OpenCoder improves GPT selected-output correctness over Baseline RAG from 56.25\% to 78.13\%. However, it matches a verification-and-repair control, and the corresponding Gemini improvement is not statistically supported, indicating backend-dependent benefits. Target-aware API refinement also substantially improves API-set retrieval. These findings support treating uncertainty as an actionable control signal for repository-level retrieval, verification, and repair.

</details>


### [2] [Characterizing Structural Testability in JavaScript: An Empirical Study](https://arxiv.org/abs/2607.24965)
*Shahrzad Mirzaei,Saba Alimadadi*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 对JavaScript结构性可测试性进行大规模实证研究，提出七维框架并应用于30个项目，发现低可测试性函数集中于少数文件且呈现多种结构模式。


<details>
  <summary>Details</summary>
Motivation: JavaScript系统的结构性可测试性研究不足，现有框架无法捕捉其异步、事件驱动等特性。

Method: 通过AST静态分析构建七维可测试性指标（可控性、可观察性等），聚合为复合可测试性评分（CTS），对30个开源项目进行实证分析。

Result: 结构困难函数集中在少数文件中，通过多种重复结构配置出现，而非单一模式。

Conclusion: 为JavaScript可测试性提供了新洞见，为测试工作、自动生成、重构和质量评估奠定基础。

中文摘要：软件可测试性长期以来被认为是影响测试工作和效率的软件质量属性。虽然先前的工作已广泛研究了面向对象和并发软件中的可测试性，但对现代JavaScript系统中结构性可测试性的了解相对较少。JavaScript应用程序依赖于异步执行、事件驱动控制流、闭包和动态交互，这些并未被现有的可测试性框架明确捕获。本文对JavaScript中的结构性可测试性进行了大规模的实证研究。我们将结构性可测试性操作化为一个七维结构，涵盖可控性、可观察性、分支复杂度、异步协调、事件驱动行为、封装性和副作用强度。这些维度基于AST静态分析得出，并聚合为复合可测试性评分（CTS），用于跨函数、文件和项目的比较分析。我们将此框架应用于30个开源JavaScript项目，涵盖不同领域和规模。我们的分析描述了结构性可测试性的分布，识别了低CTS函数中反复出现的结构原型，并检查了项目特征与可测试性之间的关联。我们发现结构上具有挑战性的函数集中在相对较小的文件子集中，并通过多种反复出现的结构配置而非单一主导模式出现。这些发现为JavaScript中的结构性可测试性提供了新的见解，并为未来关于测试工作、自动化测试生成、可测试性感知重构和软件质量评估的研究奠定了基础。

Original abstract: Software testability has long been recognized as a software quality attribute that influences testing effort and effectiveness. While prior work has extensively studied testability in object-oriented and concurrent software, comparatively little is known about structural testability in modern JavaScript systems. JavaScript applications rely on asynchronous execution, event-driven control flow, closures, and dynamic interactions that are not explicitly captured by existing testability frameworks. This paper presents a large-scale empirical study of structural testability in JavaScript. We operationalize structural testability as a seven-dimensional construct capturing controllability, observability, branching complexity, asynchronous coordination, event-driven behaviour, encapsulation, and side-effect intensity. These dimensions are derived from AST-based static analysis and aggregated into a Composite Testability Score (CTS) for comparative analysis across functions, files, and projects. We apply this framework to 30 open-source JavaScript projects spanning diverse domains and sizes. Our analysis characterizes the distribution of structural testability, identifies recurring structural archetypes among low-CTS functions, and examines associations between project characteristics and testability. We find that structurally-challenging functions are concentrated within a relatively small subset of files and arise through multiple recurring structural configurations rather than a single dominant pattern. These findings provide new insight into structural testability in JavaScript and establish a foundation for future research on testing effort, automated test generation, testability-aware refactoring, and software quality assessment.

</details>


### [3] [Preliminary Guidelines for Using and Evaluating GenAI Tools to Support Systematic Literature Reviews](https://arxiv.org/abs/2607.24991)
*Barbara Kitchenham,Sebastián Pizard,Lech Madeyski,Ronnie de Souza Santos,Martin Shepperd,David Budgen*

Main category: cs.SE

Comments: 58 pages, 2 figures, 11 tables

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 本文提出GUEST建议，指导软件工程研究人员在使用GenAI进行系统文献综述时确保严谨性和可靠性。


<details>
  <summary>Details</summary>
Motivation: GenAI和LLMs在学术任务中广泛应用，但缺乏保证SLRs严谨性的指南，需要明确如何评估和使用它们。

Method: 通过快速综述和基于思想实验、文献及经验的推理，制定过程建议。

Result: 识别出评估GenAI用于SLRs的问题，并提出GUEST过程建议。

Conclusion: GenAI需要人类监督，不能独立完成系统性研究，但可作为辅助工具；GUEST建议有助于进行可信的SLRs和独立评估。

中文摘要：背景：生成式人工智能（GenAI）和大语言模型（LLMs）越来越多地被用于软件工程及其他领域的学术任务，包括系统文献综述（SLRs）。然而，尽管它们能够总结文本，但无法保证它们能够满足SLRs所需的严谨性、可靠性和透明性。目标：支持计划使用GenAI进行SLRs的研究人员，或进行实证研究评估GenAI支持SLR任务效果的研究人员。方法：首先，我们进行了一项快速综述，以识别提出使用和评估GenAI及LLMs支持SLRs指南的研究。其次，我们借鉴思想实验、文献中的相关指导以及我们在进行SLRs和评估工具方面的经验，制定关于如何在SLR背景下使用和评估GenAI的建议。结果：我们讨论了研究人员在评估GenAI用于SLRs时遇到的问题。我们识别并解释了在规划、实施和报告使用GenAI的SLRs以及评估GenAI工具时需要考虑的过程问题。最后，我们将结果总结为一组过程建议，命名为GUEST（GenAI在SLR任务中的使用和评估）。结论：我们认为GenAI需要人类监督，目前无法胜任无监督的系统性研究。然而，它为一些重复性任务和额外验证一些复杂任务提供了成本效益的辅助前景。我们的GUEST建议应帮助软件工程研究人员使用GenAI进行和报告可信的SLRs，并提供严谨的独立评估研究。

Original abstract: Context: Generative AI (GenAI) and Large Language Models (LLMs) are increasingly used for academic tasks in software engineering and beyond, including systematic literature reviews (SLRs). However, while capable of summarizing text, there is no guarantee they can meet the rigour, reliability, and transparency that SLRs require. Objectives: To support researchers intending to conduct SLRs using GenAI or those conducting empirical studies evaluating how well GenAI supports SLR tasks. Methods: First, we conducted a rapid review to identify studies that propose guidelines for evaluating and using GenAI and LLMs to support SLRs. Second, we drew on thought experiments, relevant guidance from the literature, and our own experience conducting SLRs and evaluating tools to develop recommendations for how to use and assess GenAI in the context of SLRs. Results: We discuss the problems researchers face when evaluating GenAI for SLRs. We identify and explain process issues to consider when planning, conducting, and reporting both SLRs using GenAI and evaluations of GenAI tools. Finally, we summarize our results as a set of process recommendations, which we name GUEST (GenAI Use and Evaluation in SLR Tasks). Conclusion: We argue that GenAI requires human oversight and is not currently capable of unsupervised systematic studies. However, it offers the prospect of cost-effective assistance for some repetitive tasks and for additional validation of some complex tasks. Our GUEST recommendations should help software engineering researchers both to conduct and report trustworthy SLRs using GenAI and to provide rigorous independent evaluation studies.

</details>


### [4] [Authoring Agent Skills: A Software-Engineering Approach](https://arxiv.org/abs/2607.25032)
*Giuseppe Destefanis*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 本文论证Agent技能应遵循软件工程原则，以Claude Code为例介绍其结构与加载方式，并与其他机制对比，提出评估驱动开发流程及常见模式与问题。


<details>
  <summary>Details</summary>
Motivation: Agent技能作为扩展LLM代理的新方式，缺乏系统的构建指导，本文旨在将软件工程原则应用于技能开发以提高其可靠性和可维护性。

Method: 以Claude Code为参考实现，描述技能结构、分阶段加载和描述编写；通过UML类图对比技能与其他机制（如项目记忆文件、斜杠命令等）；提出评估驱动编写流程和常见模式/错误分析。

Result: 提出技能构建应遵循单一职责、接口与实现分离、低耦合、共享令牌预算经济性和行为评估原则；给出基于决策者和保证的机制选择规则；总结编写模式和故障。

Conclusion: 技能作为软件制品，采用软件工程原则和评估驱动流程能提升其质量，且需关注第三方技能信任问题。

中文摘要：Agent技能是一种新兴的方法，用于扩展大型语言模型代理，使其具备可重用的过程性知识，代理按需加载这些知识。Anthropic引入了Agent技能，并将该格式作为开放规范发布，得到了多个代理工具的支持。本文论证了技能是一种软件制品，其构建应遵循软件工程原则，具体包括：单一职责、接口与实现分离、低耦合、共享令牌预算的经济性，以及用行为评估代替确定性测试。以Claude Code作为参考实现，描述了技能的结构、内容分阶段加载的方式，以及如何编写决定选择的描述。本文将技能与开发者可用于塑造代理行为的其他机制（如项目记忆文件、斜杠命令、子代理、外部工具连接和钩子）进行对比，并给出了基于谁决定机制运行以及提供何种保证来选择机制的规则。接着，提出了一种评估驱动的编写流程、编写中常见模式与错误，以及使用第三方技能引发的信任问题。我们以UML类图风格展示了对比、加载模型、技能的解剖结构、每种机制的相对位置，以及技能和钩子在会话中作用的节点。

Original abstract: Agent Skills are an emerging way to extend large language model agents with reusable procedural knowledge that the agent loads on demand. Anthropic introduced Agent Skills and published the format as an open specification supported across several agent tools. This note argues that a skill is a software artefact and that its construction should follow software-engineering principles, with qualifications: single responsibility, separation of interface from implementation, low coupling, and economy in a shared token budget, together with behavioural evaluation in place of deterministic testing. Using Claude Code as the reference implementation, it describes how a skill is structured, how its contents are loaded in stages, and how to write the description on which selection depends. It places skills against the other mechanisms a developer can use to shape agent behaviour, like project memory files, slash commands, subagents, external tool connections, and hooks, and gives a rule for choosing between them based on who decides that a mechanism runs and what guarantee it provides. It then sets out an evaluation-driven authoring process, a set of patterns and faults commonly encountered in authoring, and the trust question raised by using skills from third parties. We illustrate the comparison drawn in UML class style, the loading model, the anatomy of a skill, the relative position of each mechanism, and the points at which skills and hooks act during a session.

</details>


### [5] [What Motivates Whom? A Survey of Newcomers to OSS and Experienced OSS Practitioners](https://arxiv.org/abs/2607.25126)
*Shashiwadana Nirmani,Hourieh Khalajzadeh,Mojtaba Shahin,Xiao Liu*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 通过调查208名从业者，发现年龄、性别和OSS角色等人口因素与动机相关，项目特征偏好因动机而异，且新手与经验者偏好不同；为改进贡献者入职和推荐系统提供见解。


<details>
  <summary>Details</summary>
Motivation: 理解贡献者如何选择OSS项目以支持其入职、参与和长期留存，改善OSS生态系统。

Method: 在线调查208名软件从业者，分析动机、人口背景与项目选择偏好的关系，并分新手与经验者进行比较。

Result: 年龄、性别和OSS角色与动机显著相关；项目年龄、开发阶段和文档质量等偏好因动机而异；新手与经验者的偏好存在差异。

Conclusion: 研究结果可为改进贡献者入职、参与和留存提供指导，并助力设计更符合动机的项目推荐系统。

中文摘要：开源软件（OSS）开发持续扩展，然而软件从业者常常难以选择合适的项目，导致入职效率低下和参与度下降。理解贡献者如何选择OSS项目对于支持贡献者入职、参与以及长期留在OSS社区具有重要意义。本研究调查了贡献者在OSS项目中的项目选择偏好，并探讨了这些偏好如何与贡献者的动机和人口背景相关联。通过对208名从业者的在线调查，我们发现人口因素（如年龄、性别和所担任的OSS角色）与其动机显著相关。此外，对项目特征（如项目年龄、开发阶段和文档质量）的偏好因具体动机而异。重要的是，我们的研究结果通过比较视角呈现，分别分析了OSS新手和经验丰富的OSS从业者的回答，以揭示他们不同的偏好。最后，我们探讨了软件从业者对现有推荐系统如何更好地支持项目选择并与其动机相一致的观点。通过厘清OSS新手和OSS从业者的独特需求，我们的研究结果为研究人员、OSS项目所有者和软件从业者提供了见解，以改进贡献者入职、参与和留存，同时为未来的项目推荐系统提供信息，改善OSS生态系统。

Original abstract: Open source software (OSS) development continues to expand, yet software practitioners often struggle to select suitable projects, leading to inefficient onboarding and disengagement. Understanding how contributors select OSS projects is important for supporting contributors onboarding, engagement, and long-term participation within OSS communities. This study investigates contributors' project-selection preferences in OSS projects and examines how these preferences correlate with contributors' motivations and demographic backgrounds. Through an online survey of 208 practitioners, we found that demographic factors, such as age, gender, and the OSS role they held, significantly correlate with their motivations. Additionally, preferences for project characteristics such as project age, development stage, and documentation quality vary based on specific motivations. Importantly, our findings are presented through a comparative lens, analyzing the responses of newcomers to OSS and experienced OSS practitioners separately to uncover their distinct preferences. Lastly, we explore software practitioners' perspectives on how existing recommendation systems could better support project selection and align with their motivations. By disentangling the unique needs of newcomers to OSS and OSS practitioners, our findings provide insights for researchers, OSS project owners, and software practitioners to improve contributor onboarding, engagement, and retention, while also informing future project recommendation systems and improving the OSS ecosystem.

</details>


### [6] [Learning from 53.6K Real-World Developer Edits of AI-Generated Code](https://arxiv.org/abs/2607.25130)
*Jenny T. Liang,Mihika Bairathi,Wayne Chi,Ameet Talwalkar,Nishant Subramani,Valerie Chen*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 一个包含53.6K个真实世界IDE内AI生成代码编辑的数据集，用于分析编辑行为并微调LLM以预测代码编辑。


<details>
  <summary>Details</summary>
Motivation: 解决缺乏高质量、真实代码编辑数据的问题，现有LLM主要依赖Git提交数据，无法捕捉细粒度的编辑行为。

Method: 收集超过1000名开发者在IDE中编辑AI生成代码的数据，构建DECODE数据集（53.6K条编辑记录，涵盖Python、TypeScript和JavaScript），进行统计分析并微调开源3B LLM进行代码编辑预测。

Result: 大多数编辑发生在接受AI补全后的15分钟内，31%的编辑轨迹完全删除了AI补全；微调DECODE后，开源3B模型在代码编辑预测任务上显著优于前沿LLM。

Conclusion: 强调开发者为中心的机器学习方法对于未来AI编程助手的重要性，DECODE数据集为相关研究提供了宝贵资源。

中文摘要：AI生成的代码存在不完善之处，需要软件开发人员手动修改生成的代码，或通过重新提示AI编程助手进行修改。手动代码编辑提供了比Git提交（仅包含最终成功的代码片段）更真实、更细粒度的编辑行为信息。然而，由于缺乏高质量、真实的代码编辑数据，大型语言模型（LLM）大多在公开可用的Git数据（如提交）上进行训练。为了解决这一差距，我们引入了DECODE（开发者代码编辑数据集），该数据集包含来自1000多名开发者的53.6万个真实世界IDE内AI生成代码的编辑记录，涵盖Python、TypeScript和JavaScript。首先，我们展示了DECODE在数据分析方面的实用性，获得了关于何时、为何以及如何编辑AI生成代码的见解。我们发现大多数编辑发生在接受AI补全后的前15分钟内，31%的编辑轨迹中删除了AI补全。其次，我们使用DECODE来基准测试LLM预测代码编辑的能力。我们发现，在DECODE上进行微调后，开源3B模型在代码编辑预测任务上显著优于前沿LLM。最后，我们讨论了这项工作的意义，强调了未来AI编程助手中以开发者为中心的机器学习方法的必要性。

Original abstract: Imperfections in AI-generated code require that software developers modify the generated code manually, or by re-prompting an AI programming assistant. Manual code edits provide more realistic and granular information on editing behavior than Git commits, which only contain final successful code snippets. Yet, due to a lack of high-quality, realistic code editing data, LLMs are mostly trained on publicly available Git data (e.g., commits). To address this gap, we introduce DECODE (Developer Edits of Code Dataset), a dataset of 53.6K real-world in-IDE code edits of AI-generated code in Python, TypeScript, and JavaScript, sourced from 1K+ developers. First, we demonstrate the utility of DECODE for data analysis, obtaining insights on when, why, and how AI-generated code is edited. We find that most edits occur within the first 15 minutes after accepting an AI completion, resulting in the removal of AI completions in 31% of edit trajectories. Second, we use DECODE to benchmark the ability of LLMs to predict code edits. We find that finetuning on DECODE enables open-source 3B models to perform code edit prediction tasks significantly better than frontier LLMs. We then discuss implications of this work, emphasizing the necessity of developer-centric machine learning approaches for future AI programming assistants.

</details>


### [7] [Specification-Driven DevOps for Multi-Service Environments](https://arxiv.org/abs/2607.25141)
*Oleg Grynets,Kyrylo Fursov,Vasyl Lyashkevych,Volodymyr Veres*

Main category: cs.SE

Comments: 25 pages, 8 figures, 15 tables

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 研究发现当前LLM能从仓库生成可运行的Docker配置，但常忽略关键部署细节（如安全、网络分段），需显式规范来弥补。


<details>
  <summary>Details</summary>
Motivation: 探究LLM生成的部署环境是否不仅功能正确而且符合架构、安全等生产意图。

Method: 使用三个异构多服务仓库，要求LLM生成Dockerfile和Compose文件，通过HTTP端点测试功能，并手动对比结构。

Result: 所有环境功能可运行，但模型一致遗漏了网络分段、多阶段构建、缓存、卷挂载、生产前端服务、端口限制和跨平台构建。

Conclusion: 功能正确不等于部署意图保真，需显式规范补充仓库无法推断的信息。

中文摘要：大型语言模型（LLM）越来越多地被用于从仓库构件生成可执行的软件环境。然而，功能上的可执行性并不一定意味着符合架构、安全、工作流和生产意图。本研究调查了一个前沿LLM是否能够仅使用仓库内容（无需访问开发者编写的部署构件）为多服务应用生成Dockerfile和Docker Compose配置。评估了三个异构仓库，它们结合了Python、Node.js、.NET、React、Rust、Java、Redis、PostgreSQL和MySQL兼容的基础设施，使用确定性端到端HTTP预言和手动结构比较。所有三个生成的环境在功能上均可运行，尽管其中一个需要将Rust基础镜像从1.85版本更新到1.88版本。模型正确重建了服务拓扑、应用程序端口、基础设施依赖、服务主机名、后台工作进程、隐藏代理配置以及基于文件的Docker机密机制。然而，它始终忽略了网络分段、多阶段构建、依赖层缓存、实时重载卷、生产前端服务、限制性后端端口策略以及跨平台构建逻辑。基于这些观察，本研究形式化了功能正确性与部署意图保真度之间的区别，并分析推导出了无法从仓库构件可靠推断的信息的最小显式部署规范。

Original abstract: Large Language Models (LLMs) are increasingly used to generate executable software environments from repository artifacts. However, functional executability does not necessarily imply conformity with architectural, security, workflow, and production intent. This study investigates whether a frontier LLM can generate Dockerfiles and Docker Compose configurations for multi-service applications using repository contents without access to developer-authored deployment artifacts. Three heterogeneous repositories combining Python, Node.js, .NET, React, Rust, Java, Redis, PostgreSQL, and MySQL-compatible infrastructure were evaluated using deterministic end-to-end HTTP oracles and manual structural comparison. All three generated environments became functionally operational, although one required a Rust base-image update from version 1.85 to 1.88. The model correctly reconstructed service topology, application ports, infrastructure dependencies, service hostnames, a background worker, hidden proxy configurations, and a file-based Docker secrets mechanism. However, it consistently omitted network segmentation, multi-stage builds, dependency-layer caching, live-reload volumes, production frontend serving, restrictive backend-port policies, and cross-platform build logic. Based on these observations, the study formalizes the distinction between functional correctness and deployment-intent fidelity and analytically derives a minimal explicit deployment specification for information that cannot be reliably inferred from repository artifacts.

</details>


### [8] [NFR-to-Code Traceability in a Blockchain-IoT System: An Empirical Study](https://arxiv.org/abs/2607.25325)
*Yifei Wang,Jacky Keung,Xiaoxue Ma,Shijie Zhang,Yishu Li*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 本文研究非功能性需求到代码的可追溯性，通过真实项目构建标注数据集，发现NFR追踪比FR更难，尤其是安全相关NFR，主要挑战在于代码实现证据分散。


<details>
  <summary>Details</summary>
Motivation: 现有需求到代码的可追溯性研究主要集中于功能性需求，非功能性需求可追溯性困难且未被充分探索，阻碍了安全性和可靠性等关键质量属性的验证。

Method: 基于真实区块链-IoT项目，设计跨工件标注协议构建手动策划的子集，包含FR和NFR链接，评估TF-IDF、BM25、LSI、WMD四种检索基线。

Result: FR到代码追踪始终比NFR容易；安全相关NFR是最难追踪的子集；NFR可追溯性的主要挑战不在于需求可用性，而在于实现证据分散且未局部化。

Conclusion: NFR可追溯性亟待更多研究，其核心难点在于代码中实现证据的分散性，而非需求本身的缺失。

中文摘要：需求到代码的可追溯性已被广泛研究，然而现有的研究和公开基准在很大程度上仍然集中于功能性需求（FRs）。相比之下，非功能性需求（NFRs）的可追溯性仍然更加困难且探索不足，这阻碍了对关键质量关注点（如安全性和可靠性）的验证。本文基于一个真实的区块链-IoT项目研究NFR到代码的可追溯性。我们设计了一个用于跨异构工件构建追溯链接的标注协议，并构建了一个包含FR和NFR链接的手动策划子集。利用该子集，我们考察了NFR可追溯性的特征，并进一步评估了四种代表性检索基线：TF-IDF、BM25、LSI和WMD。结果表明，FR到代码的追踪始终比NFR到代码的追踪更容易，而与安全相关的NFR是最困难的子集。它们还表明，NFR可追溯性的主要挑战不在于需求的可用性，而在于实现证据分散且未在代码中明确局部化。

Original abstract: Requirement-to-Code traceability has been widely studied, yet existing research and public benchmarks remain largely centered on functional requirements (FRs). In contrast, traceability for non-functional requirements (NFRs) remains more difficult and underexplored, which hinders the verification of critical quality concerns such as security and reliability.This paper studies NFR-to-Code traceability based on a real-world blockchain-IoT project. We design an annotation protocol for constructing trace links across heterogeneous artifacts and build a manually curated subset containing both FR and NFR links. Using this subset, we examine the characteristics of NFR traceability and further evaluate four representative retrieval baselines: TF-IDF, BM25, LSI, and WMD. The results show that FR-to-Code tracing is consistently easier than NFR-to-Code tracing, while security-related NFRs are the most difficult subset. They further indicate that the main challenge of NFR traceability lies not in requirement availability, but in implementation evidence that is distributed and not clearly localized in code.

</details>


### [9] [Specula: Scaling formal specifications for autonomous model checking of system code](https://arxiv.org/abs/2607.25333)
*Qian Cheng,Saad Mohammad Rafid Pial,Ruize Tang,Yiming Su,Emilie Ma,Finn Hackett,Ivan Beschastnikh,Yu Huang,Tianyin Xu*

Main category: cs.SE

Comments: 17 pages, 11 figures

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: Specula是一个全自动系统，利用LLM代理为系统代码生成TLA+规约，实现高效的模型检查和缺陷发现。在48个开源项目中找到了249个错误。


<details>
  <summary>Details</summary>
Motivation: 消除将形式化方法应用于实际系统代码的障碍，解决LLM驱动技术中的奖励黑客和幻觉等局限性。

Method: 采用基于LLM的编码代理自主开发TLA+规约，包括不变量的形式化模型，并通过自进化循环迭代改进规约质量。

Result: 在48个开源系统项目中发现了249个错误，包括许多深层错误。

Conclusion: Specula完全自主地应用形式化方法，有效发现深层错误，已被多家公司采用。

中文摘要：Specula是一个按钮式代理系统，能够为大型复杂系统代码生成高质量的形式化规约，并利用这些规约进行高效的模型检查和缺陷发现。Specula采用基于大语言模型（LLM）的编码代理来自动开发TLA+规约，包括描述目标系统正确性属性的不变量以及以适当抽象级别描述系统实现的形式化模型。Specula完全自主，因此消除了将形式化方法应用于实际系统代码的障碍（如传统以人为中心的方法）。同时，Specula通过自进化循环不断改进规约质量，使代理能够加深对系统代码及其行为的理解，从而解决了LLM驱动技术（如奖励黑客和幻觉）的局限性。我们使用Specula检查了48个开源系统项目；Specula发现了249个错误，包括许多现有方法难以发现的深层错误。Specula已被多家公司使用，并维护于https://github.com/specula-org/Specula。

Original abstract: Specula is a push-button agentic system that generates high-quality formal specifications for large, complex system code and uses the specifications for highly effective model checking and bug finding. Specula employs large language model (LLM) based coding agents to autonomously develop TLA+ specifications, including invariants that describe correctness properties of the target system and formal models that describe the system implementation with the right level of abstractions. Specula is fully autonomous and thus eliminates the barrier of applying formal methods to real-world system code (as in traditional human-centric approaches). Meanwhile, Specula addresses limitations of LLM-driven techniques like reward hacking and hallucinations through self-evolving loops that iteratively improve specification quality by enabling the agents to deepen their understanding of system code and its behaviors. We have used Specula to check 48 open-source system projects; Specula found 249 bugs including many deep bugs that are hard to find by existing approaches. Specula has been used by several companies and is maintained at https://github.com/specula-org/Specula.

</details>


### [10] [CodeNib: A Multi-View Data System for Serving Repository Context to Coding Agents](https://arxiv.org/abs/2607.25431)
*Zhongming Yu,Hengjia Yu,Boqin Yuan,Shuting Zhao,Yizhao Chen,Aryan Dokania,Mihir Jagtap,Jiayu Chang,Yitong Ma,Yash Jayswal,Wentao Ni,Hejia Zhang,Zhaoling Chen,Gangda Deng,Jishen Zhao*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: CodeNib 通过构建可重用的多视图仓库上下文服务，显著提升了编码代理的效率，减少了重复工作并降低了生命周期成本。


<details>
  <summary>Details</summary>
Motivation: 现有编码代理在频繁搜索、导航和保留仓库上下文时面临断开索引、语言服务器和任务局部历史记录导致的重复发现和隐藏生命周期成本问题。

Method: CodeNib 在每个提交上构建词汇、密集和结构视图，映射输出到仓库相对源范围，维护跨编辑的选定视图，并通过统一运行时提供排序搜索、符号导航和有界上下文。

Result: 在100个快照上，图更新和向量更新分别快8.7倍和25.4倍（中位数）；静态导航延迟比实时服务器低4.7倍（中位数）；上下文策略节省50-87%的轨迹令牌。

Conclusion: CodeNib 支持具有明确有效性边界的多视图仓库上下文服务，显著提升性能并降低成本。

中文摘要：编码代理反复搜索、导航并从不断演变的仓库中保留上下文，但断开的索引、语言服务器和任务局部历史记录迫使重复发现并掩盖生命周期成本。CodeNib 在每个仓库提交上构建可重用的词汇、密集和结构视图，将输出映射到仓库相对源范围，在编辑之间维护选定的视图，并通过一个运行时提供排序搜索、符号导航和有界上下文。在 100 个快照中，我们绘制了仓库上下文生命周期中的质量-成本前沿。当输出匹配独立重建时，图和向量更新的中位数速度分别快 8.7 倍和 25.4 倍。在匹配标准化实时服务器位置的静态导航子集（1000 个请求中的 63%）上，每个请求的实时/静态延迟比率中位数为 4.7 倍。在五个模型上，选定的上下文策略比配对的 grep/read 节省了 50-87% 的轨迹令牌，同时保持了定位。这些结果共同支持具有明确、操作特定有效性边界的多视图仓库上下文服务。

Original abstract: Coding agents repeatedly search, navigate, and retain context from evolving repositories, but disconnected indexes, language servers, and task-local histories force repeated discovery and obscure lifecycle costs. CodeNib builds reusable lexical, dense, and structural views per repository commit, maps outputs to repository-relative source ranges, maintains selected views across edits, and serves ranked search, symbol navigation, and bounded context through one runtime.
  Across 100 snapshots, we map quality-cost frontiers across the repository-context lifecycle. When outputs match an independent rebuild, graph and vector updates are $8.7\times$ and $25.4\times$ faster at the median. On the static-navigation subset matching normalized live-server locations (63% of 1,000 requests), the median per-request live/static latency ratio is $4.7\times$. Across five models, selected context policies preserve localization with 50--87% fewer trajectory tokens than paired grep/read. Together, these results support multi-view repository-context serving with explicit, operation-specific validity boundaries.

</details>


### [11] [Repositories, Contributors, and Continuity: An Empirical Study of Foundational Quantum Software](https://arxiv.org/abs/2607.25437)
*Vincent Gierisch,Nicole Hoess,Ralf Ramsauer,Wolfgang Mauerer*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 本文通过实证分析量子软件仓库，发现贡献者迁移和跨项目知识转移模式，揭示了概念在不同仓库间持续存在的路径。


<details>
  <summary>Details</summary>
Motivation: 量子软件生态系统中许多仓库开发时间有限，其概念是否能在仓库之外持续存在尚不清楚，需要实证研究。

Method: 应用成熟的实证软件工程技术，结合跨仓库活动与贡献者关系分析一组基础量子软件仓库的社区演变。

Result: 提供了贡献者迁移模式的实证证据和跨项目知识转移的迹象，观察到项目可持续存在、概念整合或通过新物继续活动等多种发展路径。

Conclusion: 为量子软件生态系统中概念和影响力跨仓库边界持续存在提供了初步实证视角。

中文摘要：受学术界、工业界和开源社区的贡献驱动，量子软件生态系统正在快速发展。在这个生态系统中，新概念通常通过伴随科学论文的软件制品以及在更大社区中的持续开发而涌现。然而，许多仓库仅在有限的时间内获得开发努力，这引发了一个问题：它们的概念是否能在单个仓库之外持续存在。在本文中，我们应用成熟的实证软件工程技术来分析一组基础量子软件仓库。我们将跨仓库活动与贡献者关系相结合，以研究社区的演变。我们的分析提供了贡献者迁移模式的实证证据以及跨项目知识转移的迹象。我们观察到多种发展路径：项目可能演变为持续存在的社区，贡献者可能将概念整合到已建立的生态系统中，或者通过新的和后续的软件制品继续活动。我们的观察为量子软件生态系统中概念和影响力如何跨仓库边界持续存在提供了初步的实证视角。

Original abstract: Driven by contributions from academia, industry, and open-source communities, the quantum software ecosystem is rapidly growing. Across this ecosystem, new concepts often emerge through software artefacts accompanying scientific publications as well as through sustained development in larger communities. However, many repositories receive development efforts only over a limited period of time, raising the question whether their concepts persist beyond individual repositories. In this paper, we apply established empirical software engineering techniques to analyse a set of foundational quantum software repositories. We combine cross-repository activity with contributor relationships to study the evolution of communities. Our analysis provides empirical evidence of contributor migration patterns and indications of cross-project knowledge transfer. We observe multiple development paths: projects may evolve into sustained communities, contributors may integrate concepts into established ecosystems, or activity may continue through new and follow-up software artefacts. Our observations provide an initial empirical perspective on how concepts and influence persist across repository boundaries in quantum software ecosystems.

</details>


### [12] [Input Relation Prompting for Metamorphic Testing on Query-Based Systems](https://arxiv.org/abs/2607.25603)
*Eng-Shen Tu,Shin-Jie Lee*

Main category: cs.SE

Comments: 18 pages

Journal reference: Journal of Information Science and Engineering, Vol. 41, No. 1, pp. 43-60 (2025)

DOI: [10.6688/JISE.202501\_41(1).0003](https://doi.org/10.6688/JISE.202501\_41(1).0003)

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 提出一种通过提示蜕变关系来辅助测试基于查询系统的蜕变测试方法，解决缺少真实地面真值和手动测试耗时问题，并通过案例研究验证了其适用性。


<details>
  <summary>Details</summary>
Motivation: 基于查询的系统缺乏地面真值和手动测试耗时，传统方法依赖领域知识，需要更高效的测试方法。

Method: 通过提示输入输出间的蜕变关系（MRs）辅助蜕变测试（MT），不依赖预定义测试用例或真实地面真值，可与其他测试方法结合。

Result: 案例研究在真实Web应用中展示了方法的适用性和潜力。

Conclusion: 该方法推进了蜕变测试领域，为QBS测试者提供了提升测试效率的工具。

中文摘要：测试基于查询的系统（QBSs）面临着重大挑战，因为缺乏用于验证的真实地面真值，并且手动测试需要大量的时间和精力。本文通过提出一种方法来解决这些挑战，该方法帮助测试人员识别蜕变关系（MRs）以用于蜕变测试（MT），而不是仅仅依赖先验领域知识。MT是一种越来越流行的用于缓解预言问题的方法，它通过将输入转换规则（MRs）应用于程序。所提出的方法通过提示描述输入和输出之间关系的MRs来帮助测试人员，当预期关系未满足时，能够检测故障。与传统测试方法不同，该方法不依赖于预定义的测试用例或具体的真实地面真值，因此适用于真实世界QBSs的测试。此外，该方法可以与其他测试方法（如组合测试和模糊测试）结合，扩展了QBS测试的可能性。对真实世界Web应用程序进行的案例研究证明了所提出方法的适用性和潜力。总体而言，本研究有助于推进蜕变测试领域，并为QBS测试人员提供了增强测试效率的宝贵工具。

Original abstract: Testing query-based systems (QBSs) presents significant challenges due to the absence of ground truth for validation and the extensive time and effort required for manual testing. This paper addresses these challenges by proposing an approach that assists testers in identifying metamorphic relations (MRs) for metamorphic testing (MT) instead of solely and exhaustively relying on prerequisite domain knowledge. MT is an approach rising in popularity employed to alleviate the oracle problem by applying input transformation rules (MRs) to a program. The proposed approach helps the tester by prompting MRs that describe the relationships between inputs and outputs, enabling fault detection when the expected relationship is not met. Unlike traditional testing approaches, this approach does not rely on pre-defined test cases or concrete ground truth, making it suitable for the testing of real-world QBSs. Furthermore, the proposed approach can be combined with other testing methods such as combinatorial testing and fuzz testing, expanding the possibilities for QBS testing. A conducted case study of a real-world web application demonstrates the applicability and potential of the proposed approach. Overall, this research contributes to advancing the field of metamorphic testing and provides a valuable tool for QBS testers to enhance their testing efficiency.

</details>


### [13] [SkillGate: Cost Efficient Runtime Malicious Skill File Detection in Coding Agents](https://arxiv.org/abs/2607.25619)
*Rui Yang,Michael Fu,Kla Tantithamthavorn,Chetan Arora,Joey Chua*

Main category: cs.SE

Comments: 10 pages, 5 figures

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: SkillGate是一个针对AI编码代理技能文件的安全网关，通过混合正则表达式预过滤和LLM评判器，在低误报率下高效检测恶意技能包。


<details>
  <summary>Details</summary>
Motivation: AI编码代理技能文件可从公共注册中心轻易获取且缺乏安全审查，构成新型供应链攻击面，现有工具链缺乏系统防御。

Method: SkillGate采用混合流水线：先使用正则表达式预过滤安全信号文件（完全绕过LLM），对标记文件仅发送匹配片段窗口给LLM评判器，从而节省令牌。

Result: 在SkillsBench上（n=1,650，9.1%恶意），SkillGate实现F1=0.817，FPR=1.13%，LLM输入令牌减少77%，AUPRC为0.830，优于现有工具5-6倍。

Conclusion: SkillGate能有效检测恶意AI技能包，降低安全风险，且开销小，适合部署。

中文摘要：软件工程团队现在将AI编码代理（Cursor、Claude Code、GitHub Copilot）作为一流的生产力工具进行部署，安装特定领域的技能文件以定制代理行为，使其适应项目API、框架惯例和组织工作流。这些复杂的Markdown文件可以通过一条 npx skills add 命令轻易地从公共注册中心下载，且没有真正的安全审查，这代表了一种新型的供应链攻击面：恶意技能文件可以静默地重新编程代理行为，窃取凭证、向生成的代码注入后门，或将代理操作重定向到攻击者控制的端点。这种威胁并非假设：最近的报告记录了公共注册中心中数百个恶意技能包，包括有组织的活动，通过虚假的生产力技能分发窃取凭证的信息窃取器。目前尚无系统化的工具链防御措施来应对这一攻击面。我们提出了SkillGate，一个可部署的安全网关，在编码代理安装之前对AI技能包进行筛查。SkillGate采用混合的正则表达式预过滤器+LLM评判器流水线：安全信号文件完全绕过LLM（跳过节省）；被标记的文件仅将匹配的片段窗口发送给评判器，而不是完整内容（片段节省）。我们在SkillsBench基准测试上，针对两个现有工具，回答了四个研究问题，涵盖检测有效性、筛查成本、运行时开销和误报行为。在SkillsBench上（n=1,650，9.1%为恶意），SkillGate实现了F1=0.817，假阳性率FPR=1.13%，同时与全文件筛查相比将LLM输入令牌减少了77%，并且在阈值无关的AUPRC上比现有工具高出5-6倍（0.830对比0.144/0.162）。

Original abstract: Software engineering teams now deploy AI coding agents (Cursor, Claude Code, GitHub Copilot) as first-class productivity tools, installing domain-specific skill files to tailor agent behavior to project APIs, framework conventions, and organizational workflows. These complex Markdown files are easily downloaded from public registries with a single npx skills add command and no real security screening, representing a novel supply-chain attack surface: a malicious skill file can silently reprogram agent behavior, exfiltrating credentials, injecting backdoors into generated code, or redirecting agent actions to attacker-controlled endpoints. The threat is not hypothetical: recent reports document hundreds of malicious skill packages in public registries, including organized campaigns that distributed credential-stealing infostealers via fake productivity skills. No systematic toolchain defense exists for this attack surface. We present SkillGate, a deployable security gateway that screens AI skill packages before coding agent installation. SkillGate uses a hybrid regex-prefilter + LLM-judge pipeline: safe-signal files bypass the LLM entirely (skip savings); flagged files have only their matched snippet windows sent to the judge, not the full content (snippet savings). We answer four research questions covering detection effectiveness, screening cost, runtime overhead, and false positive behavior on the SkillsBench benchmark against two existing tools. On SkillsBench (n=1,650, 9.1% malicious), SkillGate achieves F1=0.817, FPR=1.13% while reducing LLM input tokens by 77% vs. full-file screening, and outperforming existing tools by 5-6x on threshold-independent AUPRC (0.830 vs. 0.144/0.162).

</details>


### [14] [An Empirical Study of Model Context Protocol Applications](https://arxiv.org/abs/2607.25635)
*Muhammad Hamza Arshad Majeed,May Mahmoud,Sarah Nadi*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 对1,723个MCPApp的大规模研究发现，尽管配置和SDK使用趋同，但人类监督机制差异显著，仅37.2%的应用在工具执行前有审批步骤，存在安全风险。


<details>
  <summary>Details</summary>
Motivation: MCP标准缺乏应用端集成约定，且现有研究聚焦于服务器而非应用，因此有必要系统了解MCPApp的实际集成实践。

Method: 从GitHub挖掘1,723个MCPApp，基于代表性样本构建分类法MCPAppTax，再通过LLM辅助管道自动分类整个数据集，分析配置、SDK使用和人类参与机制。

Result: 85.2%使用文件配置，81.1%使用官方SDK，但无统一文件名约定；90.8%记录日志，77.2%提供启用/禁用控制，仅37.2%在工具执行前需要审批步骤。

Conclusion: MCP生态系统在某些实践上达成一致，但人类监督不足导致LLM可无条件调用启用工具，需加强安全机制。

中文摘要：模型上下文协议（MCP）标准化了大语言模型应用与外部工具通信的方式，但应用端尚未明确：与通过包管理器解决的传统依赖不同，开发者在集成MCP服务器时面临配置、通信或人工监督方面没有约定。这个生态系统也尚未得到充分研究，现有工作集中在服务器而非消费它们的应用上。我们对从GitHub挖掘的1,723个MCPApp进行了大规模研究。我们首先从代表性样本中推导出MCPAppTax，然后使用LLM辅助的管道将其应用于整个数据集，表征了配置、SDK使用和人类参与机制方面的服务器集成。我们的结果表明，生态系统在某些实践上趋于一致，但在其他方面则不然：大多数MCPApp使用文件配置服务器（85.2%），并使用官方SDK（81.1%）与服务器通信，但配置文件的命名约定尚未形成。人类监督差异最大，日志记录（90.8%）和启用/禁用控制（77.2%）很常见，但只有37.2%的工具执行前设置了阻止审批步骤，使得在大多数MCPApp中LLM能够无条件调用任何启用的工具。

Original abstract: The Model Context Protocol (MCP) standardizes how large language model applications communicate with external tools, but leaves the application side unspecified: unlike traditional dependencies resolved through package managers, developers integrating MCP servers face no conventions for configuration, communication, or human oversight. This ecosystem is also under-researched, with existing work focused on servers rather than the applications consuming them. We conduct a large-scale study of 1,723 MCPApps mined from GitHub. We first derive MCPAppTax from a representative sample, then use an LLM-assisted pipeline to apply it across the full dataset, characterizing server integration across configuration, SDK use, and human-in-the-loop mechanisms. Our results show that the ecosystem has converged on some practices but not others: most MCPApps configure servers using files (85.2%) and use an official SDK (81.1%) to communicate with servers, yet no naming convention has emerged for configuration files. Human oversight diverges most, logging (90.8%) and enable/disable controls (77.2%) are common, but only 37.2% gate tool execution behind a blocking approval step, leaving the LLM able to invoke any enabled tool unconditionally in most MCPApps.

</details>


### [15] [KQFuzz: Knowledge-Guided Fuzzing for Quantum Libraries via Large Language Models](https://arxiv.org/abs/2607.25647)
*Fuyuan Xia,Qixin Zhang,Chenhao Ying,Haojin Zhu,Shuai Wang,Yuan Luo,Pingchuan Ma,Yuxuan Du*

Main category: cs.SE

Comments: Accepted to the 41st IEEE/ACM International Conference on Automated Software Engineering. 17 pages, 9 figures. Comments are welcome

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: KQFuzz是一种知识引导的量子库模糊测试方法，利用代码库知识和LLM生成高质量种子程序，结合适应度评估和两级变异，显著提高了覆盖率和缺陷发现能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-based模糊测试方法在量子库测试中灵活性和效率不足，限制了量子计算领域的发展。

Method: KQFuzz：结合代码库知识指导LLM生成种子程序，采用适应度评估和两级变异（种子级和程序级）进行高效模糊测试。

Result: 在Qiskit、PennyLane和Cirq上测试，覆盖率提升最高18.44%，发现13个错误（12个已修复）。

Conclusion: KQFuzz通过知识引导和变异策略有效提升量子库模糊测试的覆盖率和缺陷检测能力，优于现有方法。

中文摘要：随着量子计算的不断进步，确保量子库的可靠性和正确性变得越来越关键。为此，提出了许多基于LLM的量子库模糊测试方法来发现潜在错误。然而，这些方法仍存在灵活性不足和效率低下的问题，阻碍了量子计算领域的发展。为了解决这些挑战，我们提出了KQFuzz，一种新颖的知识引导的量子库模糊测试器。它利用全面的代码库知识来指导基于LLM的测试生成，并结合适应度引导评估和两级变异来探索复杂执行路径并触发潜在错误。首先，KQFuzz引入了一种针对量子程序定制的新型提示方案，该方案策略性地结合了代码库知识，以高效生成高质量量子种子程序。此外，我们开发了评估和变异策略来处理生成的种子程序，促进高效的模糊测试执行，同时进一步丰富测试用例的多样性。我们实现了KQFuzz，并在三个流行的量子库（包括Qiskit、PennyLane和Cirq）上进行了模糊测试。实验结果表明，我们的方法显著优于其他最先进的方法，覆盖率提高了最多18.44%。在开发KQFuzz的过程中，我们发现了13个错误，所有错误都已得到确认，其中12个已被开发者修复。

Original abstract: As quantum computing continually improves, ensuring the reliability and correctness of quantum libraries has become increasingly critical. To this end, many LLM-based fuzzing approaches towards quantum libraries have been proposed to uncover potential bugs. However, these methods still suffer from limitations such as insufficient flexibility and low efficiency, which hinder the progress of the quantum computing field. To address these challenges, we propose KQFuzz, a novel knowledge-guided fuzzer for quantum libraries. It leverages comprehensive codebase knowledge to ground LLM-based test generation, synergizing this with fitness-guided evaluation and two-level mutations to explore complex execution paths and trigger potential bugs. Firstly, KQFuzz introduces a novel prompting scheme tailored to quantum programs, which strategically incorporates knowledge of the codebase to efficiently generate high-quality quantum seed programs. Moreover, we develop evaluation and mutation strategies to handle the generated seed programs, facilitating efficient fuzzing execution while further enriching the diversity of the resulting test cases. We implement KQFuzz and conduct fuzzing on three popular quantum libraries, including Qiskit, PennyLane, and Cirq. Experimental results demonstrate that our approach significantly outperforms other state-of-the-art methods, with coverage improved by up to 18.44%. During the development of KQFuzz, we discovered 13 bugs, all of which have been confirmed and 12 have already been fixed by the developers.

</details>


### [16] [Delta Debugging for Cyber-Physical Systems with Flaky Test Executions](https://arxiv.org/abs/2607.25695)
*Pablo Valle,Shaukat Ali,Aitor Arrieta*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 本文提出了三种delta调试算法，通过统计分析和环境约简来隔离随机CPS的最小故障输入，实验表明该方法能减少调试时间并提高故障可重复性。


<details>
  <summary>Details</summary>
Motivation: 现有的delta调试方法在随机CPS中不适用，因为其确定性假设不成立，导致故障难以重现和调试。

Method: 提出了三种delta调试算法，结合统计故障分析、重复执行和环境感知约简，以最小化故障诱发输入。

Result: 在工业电梯调度和自主移动机器人两个案例上，该方法减少了调试时间，同时提高了故障可重复性。

Conclusion: Delta调试不仅能简化故障分析，还能减轻执行不稳定问题，为调试CPS提供了实用基础。

中文摘要：基于仿真的测试被广泛用于验证信息物理系统（CPSs），然而现代CPS仿真器经常表现出非确定性（不稳定）行为，使得故障难以重现和调试。尽管delta调试在确定性系统中已被证明有效，但其基本假设在随机环境中不成立。本文提出了三种delta调试算法，结合统计故障分析、重复执行和环境感知约简，以隔离随机CPS的最小故障诱发测试输入。我们在两个互补的案例研究系统上评估了所提出的技术：一个采用随机优化的工业电梯调度系统，以及一个表现出仿真器诱导非确定性的自主移动机器人。结果表明，所提出的方法在保留原始故障行为的同时显著减少了调试时间。更重要的是，我们观察到，与原始执行相比，最小化故障诱发测试输入通常会提高故障可重复性。通过消除引入偶然随机效应的执行段，约简后的测试输入隔离了故障的因果条件，并以更高的概率一致地重现故障。这些发现表明，delta调试不仅简化了故障分析，而且减轻了执行不稳定性，为调试CPS提供了实用基础。

Original abstract: Simulation-based testing is widely used to validate Cyber-Physical Systems (CPSs), yet modern CPS simulators frequently exhibit non-deterministic (flaky) behavior, making failures difficult to reproduce and debug. Although delta debugging has proven effective for deterministic systems, its underlying assumptions do not hold in stochastic environments. This paper presents three delta debugging algorithms that combine statistical failure analysis, repeated executions, and environment-aware reduction to isolate minimal failure-inducing test inputs for stochastic CPSs. We evaluate the proposed techniques on two complementary case study systems: an industrial elevator dispatching system employing stochastic optimization and an autonomous mobile robot exhibiting simulator-induced non-determinism. The results show that the proposed approaches substantially reduce debugging time while preserving the original failure behavior. More importantly, we observe that minimizing failure-inducing test inputs frequently increases failure reproducibility compared with the original executions. By eliminating execution segments that introduce incidental stochastic effects, the reduced test inputs isolate the causal conditions of the failure and consistently reproduce it with higher probability. These findings suggest that delta debugging not only simplifies failure analysis but also mitigates execution flakiness, providing a practical foundation for debugging CPSs.

</details>


### [17] [WarmTuner: Program-Specific Warm Starts for Compiler Autotuning via Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2607.25831)
*Tianlu Qiao,Mingxuan Zhu,Zeyu Sun,Dan Hao*

Main category: cs.SE

Comments: 11 pages, 2 figures

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: WarmTuner是一个离线到在线的强化学习框架，通过程序条件化策略和GRPO优化，在编译器自动调优中平均加速1.732倍，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有编译器自动调优技术因先验知识固定而浪费在线预算，在目标程序与历史数据不匹配时效果差。

Method: 提出WarmTuner，离线阶段从历史配置学习程序条件化策略，在线阶段使用GRPO根据实际编译-运行反馈优化策略。

Result: 在GCC 15.2.0上，平均加速比1.732x，在14/30个程序上获得最佳结果。

Conclusion: WarmTuner通过离线到在线强化学习有效结合历史知识和实时反馈，显著提升编译器调优性能。

中文摘要：编译器是将高级程序转换为机器代码的基础软件工具。现代编译器提供了数百种优化，每种优化通过优化标志开启或关闭，以提高生成代码的性能。然而，可能的标志组合数量呈指数级增长，使得为给定目标程序找到合适的标志配置变得困难。现有的编译器自动调优技术通过剪枝搜索空间、注入搜索偏差或预测配置性能来降低调优成本。尽管有些技术利用了程序特征，但它们从历史数据中提取的知识在搜索开始后就固定不变；运行时反馈仅指导搜索本身，而从未指导先验知识。因此，当这种先验知识与目标程序不匹配时，这些方法在搜索到达良好配置之前会浪费大量有限的在线预算。我们提出了WarmTuner，一个离线到在线的强化学习框架，它将历史记录转化为程序条件化的策略，该策略预测完整标志空间中每个标志的设置，并在目标程序上保持可适应性。离线阶段，WarmTuner从历史良好配置中学习这个完整标志空间上的程序条件化策略。在线阶段，它使用真实的编译-运行反馈在目标程序上优化同一策略，使得策略由测量的加速比驱动，而不是局限于历史数据。我们使用组相对策略优化（GRPO）来实例化在线更新，该方法比较同一轮中的候选者，并避免单独的价值模型。我们在GCC 15.2.0上使用cBench和PolyBench评估了WarmTuner。结果表明，WarmTuner相比于GCC -O3实现了平均1.732倍的加速比，并在14/30个程序上获得了最佳结果，显著优于对比技术。

Original abstract: Compilers are fundamental software tools that translate high-level programs into machine code. Modern compilers expose hundreds of optimizations, each turned on or off through an optimization flag, to improve the performance of the generated code. However, the number of possible flag combinations grows exponentially, making it difficult to find a flag configuration well suited to a given target program. Existing compiler auto-tuning techniques reduce tuning cost by pruning the search space, injecting search biases, or predicting configuration performance. Although some exploit program features, the knowledge they extract from historical data is frozen once search begins; runtime feedback then guides only the search itself, never the prior. As a result, when this prior mismatches the target program, these methods waste much of the limited online budget before the search reaches good configurations. We propose WarmTuner, an offline-to-online reinforcement learning framework that instead turns historical records into a program-conditioned policy that predicts each flag's setting over the full flag space and remains adaptable on the target program. Offline, WarmTuner learns this program-conditioned policy over the full flag space from historical good configurations. Online, it refines the same policy on the target program using real compile-run feedback, so that the policy is driven by measured speedups rather than limited to the historical data. We instantiate the online update with Group Relative Policy Optimization (GRPO), which compares candidates in the same round and avoids a separate value model. We evaluate WarmTuner on GCC 15.2.0 with cBench and PolyBench. The results show that WarmTuner achieves an average speedup of 1.732x over GCC -O3 and obtains the best result on 14/30 programs, significantly outperforming the compared techniques.

</details>


### [18] [Rethinking Training Data for Generating Code Review Comments](https://arxiv.org/abs/2607.25851)
*Leonardo Centellas-Claros,Estefania Pakarati-Cofre,Juan Pablo Sandoval Alcocer,Diego Elias Costa*

Main category: cs.SE

Comments: Paper accepted to ICSME 2026

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 本文通过实证分析发现代码审查评论数据集中存在三类不对齐训练实例，并指出仅靠数据集清洗不足以改进生成模型，需结合效度标准、丰富上下文及评估实践。


<details>
  <summary>Details</summary>
Motivation: 现有代码审查评论生成方法常产生泛泛而谈或不可操作的评论，且数据集存在噪声，但问题根源可能更深，需要系统分析训练实例的不对齐类型。

Method: 对广泛使用的审查评论数据集进行实证检查，识别不对齐训练对并归纳分类体系，然后探索将分类体系融入LLM过滤的效果。

Result: 发现三类主要不对齐（语义模糊、缺乏可操作性、上下文依赖），且LLM过滤难以准确检测不对齐实例。

Conclusion: 改进评论生成需要超越数据集清洗，包括引入明确效度标准、丰富上下文输入和符合审查意图的评估实践。

中文摘要：生成代码审查评论已成为自动化代码审查中的一个突出研究方向，通常被构想为一种基于差异-评论对的文本生成任务。尽管基于学习的方法取得了进展，但生成的审查评论往往过于泛化、缺乏充分的依据或不可操作。最近的研究还表明，审查评论数据集中包含噪声或不适用的训练实例，这促使了基于LLM的数据集清洗方法。在本文中，我们认为有问题的训练实例并非同质，且某些局限性源于任务定义本身更深层次的问题。通过对一个广泛使用的审查评论数据集的实证检查，我们识别出不对齐的训练对：即代码变更与审查评论之间的关系无法为从局部输入生成可操作的审查反馈提供可靠学习信号的实例。我们推导出不对齐的分类体系，涵盖了三个重复出现的来源：语义模糊性、缺乏可操作性以及上下文依赖性。我们进一步探索了将该分类体系融入基于LLM的过滤是否能改善对有问题的训练实例的识别，发现检测不对齐的训练对仍然具有挑战性。基于这些观察，我们认为改进审查评论生成不仅需要数据集清洗，还需要明确的效度标准、更丰富的上下文输入以及与审查意图和可操作性相一致的评估实践。

Original abstract: Generating code review comments has become a prominent research direction in automated code review, commonly formulated as a text generation task over diff-comment pairs. Despite advances in learning-based approaches, generated review comments are often generic, weakly grounded, or non-actionable. Recent studies have also shown that review comment datasets contain noisy or unsuitable training instances, motivating LLM-based dataset cleaning approaches. In this paper, we argue that problematic training instances are not homogeneous and that some limitations stem from deeper issues in the task formulation itself. Through an empirical inspection of a widely used review comment dataset, we identify misaligned training pairs: instances where the relationship between the code change and the review comment does not provide a reliable learning signal for generating actionable review feedback from localized inputs. We derive a taxonomy of misalignment capturing three recurring sources: semantic ambiguity, lack of actionability, and context dependence. We further explore whether incorporating this taxonomy into LLM-based filtering improves the identification of problematic training instances, observing that detecting misaligned training pairs remains challenging. Based on these observations, we argue that improving review comment generation requires more than dataset cleaning alone, motivating explicit validity criteria, richer contextual inputs, and evaluation practices aligned with review intent and actionability.

</details>


### [19] [How Do LLMs Read Bug Reports? An Empirical Study of Attention in LLMs for Automated Program Repair](https://arxiv.org/abs/2607.25873)
*Ramtin Ehsani,Irene Manotas,Saurabh Pujar,Luca Buratti,Preetha Chatterjee*

Main category: cs.SE

Comments: Accepted at the 41st IEEE/ACM International Conference on Automated Software Engineering (ASE) 2026 Conference

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 首次实证研究LLM在程序修复中的注意力模式，发现成功修复的注意力更分散且与开发者关注点更一致，而失败则过度聚焦于元数据。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM-based APR发展迅速，但性能不一致的原因未知，特别是模型如何处理错误报告中的信息以及注意力分配的影响。

Method: 分析SWE-bench Verified和Multi-SWE-bench中319个真实Python和Java错误，研究注意力分布在错误报告各部分的模式，以及成功与失败修复的差异。

Result: 成功修复的注意力分散于错误描述、堆栈跟踪和测试用例；失败修复则过度集中于版本信息。注意力与开发者关注点对齐程度越高，修复成功率越高。

Conclusion: 注意力分配不当是LLM-based APR失败的关键因素，为设计更可解释和可靠的APR系统提供了见解。

中文摘要：基于大型语言模型（LLM）的自动化程序修复系统正在快速发展，但其性能仍然不一致。即使提供相同的上下文信息，LLM可能为某个错误生成正确的补丁，但在另一个密切相关的错误上失败。为什么会发生这种情况仍然知之甚少，也不清楚LLM如何优先处理错误报告中的不同信息，以及模型注意力是否影响修复成功。在本文中，我们首次对LLM程序修复中的注意力模式进行了实证研究，提供了关于模型如何处理错误报告以及修复过程中注意力集中位置的可解释性见解。我们分析了SWE-bench Verified和Multi-SWE-bench中319个真实的Python和Java错误，以研究（RQ1）模型注意力在错误报告各部分之间如何分布，（RQ2）每个部分内的注意力模式在成功和失败修复之间有何不同，以及（RQ3）这些模式与开发人员认为对修复错误重要的信息相比如何。我们发现，成功修复的特点是将注意力分散在多个诊断组件上，如错误描述、堆栈跟踪和测试用例，而失败通常表现出对元数据（如版本信息）的过度局部化注意力。我们进一步观察到，模型注意力与开发人员识别的关键部分和短语之间的更强对齐与更高的修复成功率相关。我们的结果提供了第一个实证证据，表明注意力分配不当是LLM-based APR失败的关键因素，并为设计更可解释和可靠的未来APR系统提供了可操作的见解。

Original abstract: Large Language Model (LLM)-based Automated Program Repair systems are advancing rapidly, yet their performance remains inconsistent. Even when provided with the same contextual information, an LLM may generate a correct patch for one bug but fail on another closely related bug. Why this happens remains poorly understood, and it is unclear how LLMs prioritize the diverse information in bug reports and whether model attention affects repair success. In this paper, we present the first empirical study of attention patterns in LLM-based program repair, providing interpretable insights into how models process bug reports and where their attention is concentrated during repair. We analyze 319 real-world Python and Java bugs from SWE-bench Verified and Multi-SWE-bench to study (RQ1) how model attention is distributed across bug report sections, (RQ2) how attention patterns within each section differ between successful and unsuccessful repairs, and (RQ3) how these patterns compare to information developers consider important for bug fixing. We find that successful repairs are characterized by diffused attention across multiple diagnostic components such as bug descriptions, stacktraces, and test cases, while failures often exhibit over-localized attention toward metadata such as version information. We further observe that stronger alignment between model attention and developer-identified key sections and phrases is associated with higher repair success. Our results provide the first empirical evidence that attention misallocation is a key factor in LLM-based APR failures, and offer actionable insights for designing more interpretable and reliable future APR systems.

</details>


### [20] [CONQuER: Hardware-Aware Mixed-Precision Quantisation with Online-Calibrated Surrogates](https://arxiv.org/abs/2607.25884)
*Aidan Dakhama,Ajitha Rajan*

Main category: cs.SE

Comments: 11 pages, 3 figures, 5 tables

Author affiliations: Aidan Dakhama: University of Edinburgh; Ajitha Rajan: University of Edinburgh

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: CONQuER将混合精度量化集成到编译器流水线中，利用带代理预筛选的进化搜索找到硬件感知的Pareto最优配置，实现高达12.19倍的推理加速且准确率损失极小。


<details>
  <summary>Details</summary>
Motivation: 当前混合精度量化过程碎片化，硬件无关的预处理与编译器脱节，导致位宽分配次优和严重运行时惩罚；穷举硬件评估因搜索空间巨大而不可行。

Method: CONQuER将量化移至TOSA级别的编译器流水线，结合NSGA-II进化算法与双代理预筛选引擎（评估缓存界限和特征各向同性），剔除不可行配置，通过IREE在硬件上执行最强候选，并使用在线校准器使代理模型对齐硬件行为。

Result: 在移动和笔记本电脑CPU及服务器GPU上，CONQuER发现的Pareto最优配置实现推理速度提升高达12.19倍，top-1准确率在未量化基线的1.44%以内。

Conclusion: 最优量化策略依赖于硬件，通过将量化与编译器低化和物理执行耦合，CONQuER能显著提升性能。

中文摘要：在资源受限硬件上部署深度神经网络依赖于混合精度量化（MPQ）。当前的部署工具链严重碎片化了这一过程。量化通常作为前端框架中的硬件无关预处理步骤发生，与生成物理机器代码的下游编译器脱节。这种分离导致配置次优，分配的位宽与目标硬件的异构执行块（如张量核心和可变宽度向量单元）映射不佳，从而产生严重的运行时执行惩罚。此外，通过穷举的硬件在环（HIL）测试评估这些配置由于指数级大的搜索空间而变得不可行。我们提出了CONQuER，一个统一的编译器集成的硬件感知MPQ基础设施。CONQuER将量化转移到编译器流水线中的TOSA级别，从而基于编译器支持实现智能配置处理。为了在实用的编译预算内评估模型不同层的组合搜索空间，CONQuER将NSGA-II进化算法与双代理预筛选引擎相结合。该引擎评估理论缓存内存界限和特征空间各向同性，以丢弃不可行的配置。然后，CONQuER仅通过IREE在硬件上执行最强的候选策略，并将执行指标反馈到在线校准器中。该校准器在NSGA-II进化搜索过程中将代理模型与真实硬件行为对齐。在移动和笔记本电脑CPU以及服务器GPU上的评估表明，最优量化策略是硬件相关的。通过将量化与编译器低化和物理执行耦合，CONQuER发现了Pareto最优配置，推理速度提升高达12.19倍，top-1准确率在未量化基线的1.44%以内。

Original abstract: Deploying deep neural networks on resource-constrained hardware relies on mixed-precision quantisation (MPQ). current deployment toolchains severely fragment this process. Quantisation typically occurs as a hardware-agnostic preprocessing step in front-end frameworks, disconnected from the downstream compilers that generate the physical machine code. This separation leads to suboptimal configurations where assigned bit-widths map poorly to the target machine's heterogeneous hardware execution blocks such as tensor cores and variable-width vector units, incurring severe runtime execution penalties. Furthermore, evaluating these configurations via exhaustive hardware-in-the-loop (HIL) testing is intractable due to the exponentially large search space. We present CONQuER, a unified compiler-integrated infrastructure for hardware-aware MPQ. CONQuER shifts quantisation into the compiler pipeline at the TOSA level, enabling intelligent configuration handling based on compiler support. To evaluate this combinatorial search space of different of model layers within practical compilation budgets, CONQuER couples an NSGA-II evolutionary algorithm with a dual-surrogate prescreening engine. This engine evaluates theoretical cache memory bounds and feature space isotropy to discard non-viable configurations. CONQuER then executes only the strongest candidate policies on hardware via IREE, feeding the execution metrics into an online calibrator. This calibrator aligns the surrogate models with the true hardware behaviour during an NSGA-II evolutionary search. Evaluation across mobile and laptop CPUs, and server GPUs demonstrates that optimal quantisation policies are hardware-dependent. By coupling quantisation with compiler lowering and physical execution, CONQuER discovers Pareto-optimal configurations up to 12.19x faster inference with top-1 accuracy within 1.44% of the unquantised baseline.

</details>


### [21] [RSIBench-Data: Benchmarking Data-Centric Research for Recursive Self-Improvement](https://arxiv.org/abs/2607.25886)
*Fanqing Meng,Lingxiao Du,Qiguang Chen,Ziqi Zhao,Haocheng Lu,Mengkang Hu,Michael Qizhe Shieh*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 引入RSIBench-Data基准，评估LLM代理在以数据为中心的后训练研究中的能力，发现代理在多数设置中能改进初始尝试，但改进不一致，强候选常早期出现但后续失败。


<details>
  <summary>Details</summary>
Motivation: 现有基准将研究决策与工程细节纠缠，难以评估代理的真实研究能力，需要可控基准来测试递归自我改进所需的研究能力。

Method: 构建RSIBench-Data基准，固定后训练栈（目标模型、训练/服务、评估），让LLM代理迭代修改训练数据策略，使用Tinker、Harbor和E2B沙箱，预算固定。

Result: 代理在58.33%设置中改进首次尝试，但78.26%的持续搜索以更低分数结束；强候选常早期出现但后续失败；轨迹分析发现四种成功模式。

Conclusion: 当前代理能做出有用以数据为中心发现，但不能一致地将反馈转化为改进；RSIBench-Data提供可测量可审计测试平台，促进递归自我改进研究。

中文摘要：递归自我改进需要将模型失败的证据转化为更好的模型。以数据为中心的后训练研究涉及诊断能力差距、设计和验证训练数据策略，以及从检查点反馈中学习。LLM代理能否自动化这一循环？现有的基准将研究决策与优化、服务、评估和系统实现纠缠在一起，掩盖了代理的研究能力。我们引入了RSIBench-Data，这是一个受控基准，用于评估LLM代理作为以数据为中心的研究人员，具有固定的后训练栈。代理迭代地修改固定目标模型的训练数据策略；训练和服务使用Tinker支持的服务，官方评估通过Harbor和E2B沙箱运行，预算在不同代理之间固定。我们在软件工程、终端使用、科学问答和数学六个基准上评估了四个前沿代理。代理展示了核心数据为中心的研究能力：在58.33%的设置中，它们通过根据反馈优化策略，改进了第一次有效尝试。然而，改进是不一致的。在最佳观察分数之后继续的搜索中，78.26%以较低分数的最终尝试结束，而其余的仅恢复到相同的峰值。因此，一个强候选可能在运行早期或中期出现，即使后续修订失败。轨迹分析在更强的运行中识别出四种模式：准确的假设、基于验证的监督、行为对齐的数据和强检查点的保留。这些发现表明，当前的代理可以做出有用的以数据为中心的发现，但还不能将反馈转化为一致的改进。RSIBench-Data为递归自我改进所需的研究能力提供了一个可测量、可审计的测试平台。我们在https://github.com/evolvent-ai/RSIBench-Data开源了代码。

Original abstract: Recursive self-improvement requires turning evidence of model failures into better models. Data-centric post-training research entails diagnosing capability gaps, designing and validating training-data strategies, and learning from checkpoint feedback. Can LLM agents automate this loop? Existing benchmarks entangle research decisions with optimization, serving, evaluation, and systems implementation, obscuring agents' research capability. We introduce RSIBench-Data, a controlled benchmark of LLM agents as data-centric researchers with a fixed post-training stack. Agents iteratively revise training-data strategies for a fixed target model; training and serving use Tinker-backed services, official evaluation runs through Harbor and E2B sandboxes, and budgets are fixed across agents. We evaluate four frontier agents on six benchmarks across software engineering, terminal use, scientific question answering, and mathematics. Agents demonstrate core data-centric research capabilities: in 58.33\% of settings, they improve upon the first valid attempt by refining strategies from feedback. However, improvement is inconsistent. Among searches continuing after the best observed score, 78.26\% end with a lower-scoring final attempt, while the rest only recover the same peak. A strong candidate may therefore appear early or midway through a run even as later revisions fail. Trajectory analysis identifies four patterns in stronger runs: accurate hypotheses, validation-grounded supervision, behavior-aligned data, and preservation of strong checkpoints. These findings suggest that current agents can make useful data-centric discoveries but cannot yet translate feedback into consistent improvements. RSIBench-Data provides a measurable, auditable testbed for the research capabilities required for recursive self-improvement. We open-source our code at https://github.com/evolvent-ai/RSIBench-Data.

</details>


### [22] [A Low-Cost Human-in-the-Loop Investigation of Toxicity on GitHub at Scale](https://arxiv.org/abs/2607.25946)
*Rahat Rizvi Rahman,Mia Mohammad Imran,Kostadin Damevski*

Main category: cs.SE

Comments: Accepted at the 41st IEEE/ACM International Conference on Automated Software Engineering (ASE 2026)

DOI: [10.1145/3832783.3837505](https://doi.org/10.1145/3832783.3837505)

Author affiliations: Rahat Rizvi Rahman: Virginia Commonwealth University; Mia Mohammad Imran: Missouri University of Science and Technology; K Damevski: Virginia Commonwealth University

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 提出一种结合小LLM和随机森林验证器的人机协同标注方法，实现大规模GitHub毒性标注，验证并扩展了先前小规模研究的发现。


<details>
  <summary>Details</summary>
Motivation: 现有GitHub毒性研究规模有限且标注不可靠，需要可扩展且领域校准的标注方法。

Method: 使用小型本地LLM生成毒性预测和事件类别分数，然后用轻量级随机森林验证器识别可能误标的样本，仅需人工审核这些样本。

Result: 应用于124,000+个GitHub对话，确认部分先前发现，修正其他，并揭示毒性在不同项目中的新特征和动态。

Conclusion: 该方法有效实现大规模、低成本的毒性标注，为开源社区研究提供了可靠数据和新见解。

中文摘要：开放源代码讨论中的有毒互动可能会疏远贡献者并威胁项目的可持续性，然而之前关于GitHub毒性的实证研究规模有限，引发对其普遍性的质疑。扩大规模是困难的，因为GitHub上的毒性常常是隐含且依赖于上下文的，使得完全手动标注和基于LLM的标注都不可靠。我们提出了一种人在回路中的标注方法，使得大规模、领域校准的毒性标注变得可行。对一个小型本地LLM的单次调用即可产生毒性预测和一组可解释的事件类别分数。然后，一个轻量级的随机森林验证器利用这些分数来标记最可能被误标记的一小部分对话，仅在有需要的地方指导人工审查。该验证器优于基于置信度和多LLM的基线，同时增加较低的标注成本。我们将此流程应用于超过124,000个GitHub问题和拉取请求对话。利用生成的数据集，我们评估了先前小规模研究的关键发现，确认了其中一些并修正了其他，并提出了关于不同开源项目中毒性的普遍性、特征和动态的新见解。

Original abstract: Toxic interactions in open source discussions can alienate contributors and threaten project sustainability, yet prior empirical studies of GitHub toxicity have been limited in scale, raising questions about their generalizability. Scaling up is difficult because toxicity on GitHub is often implicit and context-dependent, making both fully manual annotation and LLM-based labeling unreliable.
  We present a human-in-the-loop (HITL) annotation methodology that makes large-scale, domain-calibrated toxicity labeling practical. A single call to a small, local LLM produces both a toxicity prediction and a set of interpretable event category scores. A lightweight Random Forest validator then uses those scores to flag the small subset of conversations most likely to be mislabeled, directing human review only where it is needed. The validator outperforms confidence-based and multi-LLM baselines while adding low annotation cost.
  We apply this pipeline to over 124,000 GitHub issue and pull request conversations. Using the resulting dataset, we evaluate key findings from prior small-scale research, confirming some and qualifying others, and present new insights into the prevalence, characteristics, and dynamics of toxicity across diverse open source projects.

</details>


### [23] [RepoReasoner: Evaluating Repository-Level Code Reasoning Ability of Long-Context Language Models](https://arxiv.org/abs/2607.25996)
*Yanlin Wang,Suiquan Wang,Yanli Wang,Bowen Zhang,Daya Guo,Jiachi Chen,Zibin Zheng*

Main category: cs.SE

Comments: Published in FSE'2026

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: RepoReasoner是评估LLM仓库级代码推理的基准，发现模型在跨文件推理和调用链预测中表现不佳，依赖记忆且长上下文受噪声影响。


<details>
  <summary>Details</summary>
Motivation: 现有代码推理基准限于函数级，无法反映真实跨文件开发场景，需评估LLM在仓库级复杂依赖下的推理能力。

Method: 构建RepoReasoner基准，通过pytest动态追踪获取真实调用链，并利用LLM对I/O进行重写以减少记忆效应；评估模型在输出预测和调用链预测两个任务上的表现。

Result: 最佳模型在输出预测上Pass@1仅69.1%；调用链预测高精度低召回；重写数据导致性能下降；长上下文并不稳定提升效果。

Conclusion: 当前LLM在仓库级代码推理中存在根本局限，特别是跨文件推理和多跳依赖理解，需加强结构化架构理解和跨文件推理研究。

中文摘要：近期的大型语言模型（LLM）在软件工程任务上表现出强劲性能，然而现有大多数基准测试仅在函数级别评估代码推理能力，其中所有相关信息都集中在一个局部范围内。这种设定无法反映真实世界的开发场景，后者需要跨多个文件和复杂依赖结构进行推理。我们提出了RepoReasoner，一个用于评估仓库级别代码推理的基准测试。它衡量两种互补的能力：输出预测（Output Prediction），衡量跨文件的细粒度、有状态执行推理；以及调用链预测（Call Chain Prediction），在噪声上下文中评估高层架构依赖理解。我们的基准测试通过一个多阶段流程构建，该流程利用pytest执行的动态追踪来获取真实调用链，并结合基于LLM的输入输出重写以减少记忆效应。我们评估了七个最先进的LLM。即使在完美上下文（oracle context）下，表现最好的模型在输出预测上仅达到69.1%的Pass@1，表明跨文件推理仍是一个重大挑战。在调用链预测中，模型表现出高精度但低召回率，显示出有限的多跳依赖理解能力。此外，在重写数据上的性能下降揭示了部分对记忆的依赖，而更长的上下文由于噪声并未持续提升结果。这些发现凸显了当前LLM在仓库级别推理方面的根本局限性，并激励了未来在结构化架构理解和跨文件推理方面的工作。

Original abstract: Recent large language models (LLMs) have shown strong performance on software engineering tasks, yet most existing benchmarks evaluate code reasoning at the function level, where all relevant information is localized. This setting fails to reflect real-world development, which requires reasoning across multiple files and complex dependency structures. We introduce RepoReasoner, a benchmark for evaluating repository-level code reasoning. It assesses two complementary abilities: Output Prediction, which measures fine-grained, stateful execution reasoning across files, and Call Chain Prediction, which evaluates high-level architectural dependency understanding under noisy context. Our benchmark is constructed through a multi-stage pipeline that leverages dynamic tracing of pytest executions to obtain ground-truth call chains, along with LLM-based I/O rewriting to reduce memorization effects. We evaluate seven state-of-the-art LLMs. Even under oracle context, the best-performing model achieves only 69.1% Pass@1 on Output Prediction, indicating that cross-file reasoning remains a major challenge. In Call Chain Prediction, models exhibit high precision but low recall, suggesting limited multi-hop dependency understanding. Furthermore, performance drops on rewritten data reveal partial reliance on memorization, and longer contexts do not consistently improve results due to noise. These findings highlight fundamental limitations in current LLMs' repository-level reasoning and motivate future work on structured architectural understanding and cross-file inference.

</details>


<div id='cs.AI'></div>

# cs.AI [[Back]](#toc)

### [24] [Beyond Memory: A Templated Substrate for Heterogeneous Collaborative Knowledge Work with LLM Agents](https://arxiv.org/abs/2607.24759)
*Priscila Saboia Moreira,Christopher R. Sweet*

Main category: cs.AI

Comments: 15 pages, 3 figures, 1 table

Selection reason: 提出llm-wiki记忆模式，支持多agent协作知识工作，可迁移至coding agent

TL;DR: 提出了llm-wiki-memory-template，通过LLM维护的只增wiki记录失败路径和决策，解决知识工作中负面结果丢失和跨会话记忆问题，支持多人、多AI代理、多领域协作。


<details>
  <summary>Details</summary>
Motivation: 现有研究项目、教育及知识工作中的发现、决策和推理难以被未来合作者恢复；出版物和共享代码会排除死胡同和撤回主张，导致重复失败；LLM编码代理缺乏持久记忆，检索增强生成不积累知识。

Method: 采用llm-wiki模式，在原始来源和LLM代理之间插入一个由LLM维护、相互链接的wiki，该wiki仅追加写入，记录成功与失败；提出可重用的模板llm-wiki-memory-template，包含三个架构元素分别支持多人、多AI代理、多领域协作轴。

Result: 通过三个部署案例和一个设计报告验证：个人研究谱系保留废弃迭代；双作者项目追溯审计修正了先前错误结论（从20/20降至14和12，修复后升至18和18），并保留失败路径；多代理部署和教育变体展示了跨领域应用。

Conclusion: llm-wiki-memory-template作为异构协作知识工作的基底，有效解决负面结果丢失问题，其成功依赖于技术机制与社会技术属性（失败路径保存、代理诚实性、占用）的结合。

中文摘要：研究项目、教育工作和相关的知识工作积累了发现、决策和推理，未来的合作者很少能恢复这些内容。其中对工作最有用的部分，包括死胡同和被撤回的主张，通常被排除在出版物和共享代码之外；未来的研究者会重复尝试同样的失败，因为没有记录保存下来。LLM编码代理是常见的参与者，但它们在会话之间没有持久记忆，而基于原始来源的检索增强生成并不会累积。LLM-wiki模式（Karpathy, 2026; tonbi, 2026）通过在原始来源和代理之间插入一个由LLM维护、相互链接的wiki来解决这个问题。我们提出了llm-wiki-memory-template，一个可重用的、代理感知的实例化，并论证它是沿着三个轴（多人、多AI代理、多领域）的异构协作知识工作的基底，每个轴由模板的一个独特架构元素支持（§4）。该wiki约定为仅追加，这保留了无效的内容以及有效的内容，解决了出版物和代码共享在结构上无法解决的负面结果丢失问题。三个已部署的案例研究和一份设计报告分别涵盖了这些轴：一个单独的研究谱系，保留了被放弃的迭代；一个双作者项目，其追溯审计将两个先前实验声称的20/20覆盖率修正为14和12个基于证据的答案，然后在修复后变为18和18，失败路径保存在工件中；一个正在进行中的多代理部署，作为设计报告；以及一个跨领域的教育变体。我们将失败路径保存、代理诚实性和占用命名为工件的跨社会技术属性，而不仅仅是其技术机制的属性。

Original abstract: Research projects, educational efforts, and adjacent knowledge work accumulate findings, decisions, and reasoning that future collaborators rarely recover. The parts most useful to that work, including dead ends and walked-back claims, are routinely excluded from publications and shared code; future researchers re-attempt the same failures because no record survives. LLM coding agents are common participants but hold no persistent memory across sessions, and retrieval-augmented generation over raw sources does not compound. The llm-wiki pattern (Karpathy, 2026; tonbi, 2026) addresses this by inserting an LLM-maintained, interlinked wiki between raw sources and the agent. We present llm-wiki-memory-template, a reusable, agent-aware instantiation, and argue it is a substrate for heterogeneous collaborative knowledge work along three axes (multi-human, multi-AI-agent, multi-domain) with each axis supported by a distinct architectural element of the template (§4). The wiki is append-only by convention, which preserves what did not work alongside what did, addressing a negative-result loss problem that publications and code-sharing structurally cannot solve. Three deployed case studies and one design report cover the axes individually: a solo research lineage that preserves abandoned iterations; a two-author project whose retroactive audit revised two prior experiments' claimed 20-of-20 coverage down to 14 and 12 evidence-based answers, then to 18 and 18 after a fix, with the failure path preserved across the artifact; an in-progress multi-agent deployment reported as a design; and a cross-domain educational variant. We name failure-path preservation, agent honesty, and appropriation as cross-cutting sociotechnical properties of the artifact, not only of its technical mechanisms.

</details>


### [25] [Kernel Forge: An Agent Harness for LLM-based Generation and Optimization of CUDA Kernels](https://arxiv.org/abs/2607.24762)
*Joshua Brodsky,Dhravid Kumar,Savini Kashmira,Jayanaka Danatanarayana,Jason Mars,Krisztian Flautner,Lingjia Tang*

Main category: cs.AI

Comments: The code is available at: https://github.com/TheJoshBrod/KernelForge

Selection reason: 直接相关：LLM agent自动生成和优化CUDA内核，使用MCTS搜索

TL;DR: Kernel Forge是一个开源的端到端智能体框架，使用蒙特卡洛树搜索（MCTS）自动优化PyTorch模型的内核，在视觉、扩散和LLM模型上以少量迭代取得显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有内核优化工具依赖专家手工编写GPU代码，评估局限于随机张量和孤立内核，缺乏对未修改PyTorch模型的支持、多路径探索和调试功能。

Method: 提出Kernel Forge，一个开源的端到端智能体框架，接受任何未修改的PyTorch模型，使用蒙特卡洛树搜索（MCTS）并行探索多个优化路径，并提供图形用户界面进行监控和调试。

Result: 在四个PyTorch模型上，每个内核仅50次优化迭代，优化了14个内核，相比PyTorch eager模式取得1.52×至2.83×的加速。

Conclusion: Kernel Forge有效自动化了内核优化过程，显著提升性能，且易于集成和调试，适用于多种工作负载。

中文摘要：机器学习模型日益嵌入日常软件，其大部分运行时间消耗在一小组计算内核上，如矩阵乘法、卷积和归一化。优化这些内核是减少延迟和成本最直接的方式之一，但传统上需要专家工程师手工编写底层GPU代码。基于大型语言模型（LLM）的智能体系统现在可以以更少的人力生成和优化内核，然而现有工具大多在随机生成的张量和孤立内核上评估，生成独立的CUDA代码，开发者必须手动重新集成，主要仅针对LLM PyTorch模型，并且对检查和调试结果的支持有限。我们提出Kernel Forge，一个开源的端到端智能体框架，可以原地接受任何未经修改的PyTorch模型。Kernel Forge支持视觉、扩散和LLM工作负载，使用蒙特卡洛树搜索（MCTS）探索多个优化路径，而非单一线性优化链，并附带图形用户界面用于监控进度、检查候选内核和调试失败。我们在NVIDIA DGX Spark（配备GB10 GPU）上对涵盖视觉、扩散和LLM工作负载的四个PyTorch模型评估Kernel Forge。每个内核仅经过50次优化迭代，它优化了14个内核，使其性能优于PyTorch eager模式，在ResNet-50的adaptive_avgpool2d上达到1.52倍加速，在Stable Diffusion 3.5 Medium的group_norm上达到1.70倍，在Gemma 4 E2B的softmax上达到2.83倍，在Qwen 3.5 35B-A3B的softmax上达到1.54倍。

Original abstract: Machine learning models are increasingly embedded in everyday software, and most of their runtime is spent in a small set of compute kernels such as matrix multiplication, convolution, and normalization. Optimizing these kernels is one of the most direct ways to reduce latency and cost, but it has traditionally required expert engineers to hand-write low-level GPU code. Agentic systems built on large language models (LLMs) can now generate and optimize kernels with far less human effort, yet existing tools are largely evaluated on randomly generated tensors and isolated kernels, emit standalone CUDA code that developers must manually reintegrate, mostly target only LLM PyTorch models, and offer limited support for inspecting and debugging results. We present Kernel Forge, an open-source, end-to-end agentic harness that accepts any unmodified PyTorch model in place. Kernel Forge supports vision, diffusion, and LLM workloads, uses Monte Carlo Tree Search (MCTS) to explore multiple optimization paths rather than a single linear refinement chain, and ships with a graphical user interface for monitoring progress, inspecting candidate kernels, and debugging failures. We evaluate Kernel Forge on four PyTorch models spanning vision, diffusion, and LLM workloads on an NVIDIA DGX Spark with GB10 GPU. With only 50 optimization iterations per kernel, it optimizes 14 kernels to outperform PyTorch eager mode, reaching $1.52\times$ on adaptive\_avgpool2d in ResNet-50, $1.70\times$ on group\_norm in Stable Diffusion 3.5 Medium, $2.83\times$ on softmax in Gemma 4 E2B, and $1.54\times$ on softmax in Qwen 3.5 35B-A3B.

</details>


### [26] [ProcAgent: An Agentic Framework for Procedural Task Guidance on Edge with Human-in-the-Loop](https://arxiv.org/abs/2607.24770)
*Azizul Zahid,Subrata Biswas,Bashima Islam,Sai Swaminathan*

Main category: cs.AI

Selection reason: 边缘设备上程序性任务指导agent，包含规划、验证、人机交互，方法可迁移

TL;DR: ProcAgent是一个完全在设备上的视觉程序性助手，采用提议-验证架构，结合连续感知、符号任务图和按需视觉推理，实现实时自适应指导，在边缘硬件上保持响应式交互并获用户好评。


<details>
  <summary>Details</summary>
Motivation: 程序性任务给用户带来高认知负担，现有多模态助手依赖云推理和固定感知，不适合隐私敏感、延迟关键的居家环境，因此需要一种完全在设备上的实时自适应助手。

Method: ProcAgent采用提议-验证架构，包括低延迟连续感知、符号任务图、按需视觉-语言验证和基于LLM的交互智能体，仅在歧义或偏差时调用视觉推理，支持反应式问答和主动干预。

Result: 在NVIDIA Jetson AGX Orin上全设备运行，文本查询约2秒，视觉查询约8秒；用户研究中10名参与者给予正面评价，在可理解性、可操作性和隐私舒适度方面表现良好。

Conclusion: 自适应程序性辅助可以在边缘硬件上完全实现，且不牺牲可用性，展示了在隐私敏感场景中的实用性。

中文摘要：诸如家具组装和家庭维修等程序性任务给用户带来了巨大的认知负担，因为用户必须解释指令、跟踪任务进度、推理空间状态以及在执行物理操作的同时从错误中恢复。先前的多模态助手在程序性指导方面显示出潜力，但大多数依赖云推理和固定的始终在线感知，使它们不适合隐私敏感、延迟关键的居家环境。我们提出了ProcAgent，一个完全在设备上、基于智能体、视觉的程序性助手，用于在单个NVIDIA Jetson AGX Orin上实现实时自适应指导。ProcAgent采用了一种提议-验证架构，结合了低延迟连续感知、符号任务图、按需视觉-语言验证以及基于LLM的交互智能体。该系统持续提议用户进度，仅在出现歧义或可能的偏差时调用昂贵的视觉推理，并支持反应式问答和主动干预，且有人类参与的确认。我们沿着四个维度评估ProcAgent：感知准确性、推理、任务级性能和用户体验。尽管完全在设备上运行，该系统仍保持响应式交互，纯文本查询约2秒解决，视觉接地查询约8秒解决。在一项有10名参与者完成组装任务的用户研究中，ProcAgent在可理解性、可操作性和隐私舒适度方面获得了正面评价。这些结果表明，适应性程序性辅助可以在边缘硬件上完全实现，而不会牺牲可用性。

Original abstract: Procedural tasks such as furniture assembly and home repair impose substantial cognitive demands because users must interpret instructions, track task progress, reason about spatial state, and recover from errors while performing physical actions. Prior multimodal assistants have shown promise for procedural guidance, but most rely on cloud inference and fixed always-on perception, making them poorly suited to privacy-sensitive, latency-critical domestic settings. We present ProcAgent, a fully on-device, agentic, vision-based procedural assistant for real-time adaptive guidances on a single NVIDIA Jetson AGX Orin. ProcAgent uses a propose-and-verify architecture that combines low-latency continuous perception, a symbolic task graph, on-demand vision-language verification, and an LLM-based interaction agent. The system continuously proposes user progress, invokes expensive visual reasoning only when ambiguity or likely deviation arises, and supports both reactive question answering and proactive intervention with human-in-the- loop confirmation. We evaluate ProcAgent along four dimensions: perception accuracy, reasoning, task-level performance, and user experience. Despite running entirely on-device, the system maintains responsive interaction, resolving text-only queries in approximately 2 seconds and visually grounded queries in approximately 8 seconds. In a user study with 10 participants completing assembly tasks, ProcAgent receives positive ratings for comprehensibility, actionability, and privacy comfort. These results show that adaptive procedural assistance can be achieved entirely on edge hardware without sacrificing usability.

</details>


### [27] [RSMeM: Knowledge-Enhanced Memory Evolution for Remote Sensing Agents with Systematic Evaluation](https://arxiv.org/abs/2607.24772)
*Bingxian Wu,Yu Zhang,Zonghao Guo,Tang Liu,Chen Qian,Yuxiang Lu,Xingbo Du,Yanghao Li,Yidan Zhang,Chi Chen,Ling Yao,Maosong Sun*

Main category: cs.AI

Comments: Accepted to ACL 2026 Main. Code: https://github.com/AI9Stars/RSMeM

Selection reason: 遥感agent的记忆进化机制，知识增强和失败经验复用，可迁移至coding agent

TL;DR: 提出RSMeM，一种知识增强的记忆演化机制，通过层次化知识接地和失败感知经验细化提升遥感代理的工具执行能力。


<details>
  <summary>Details</summary>
Motivation: 现有RS代理领域无关、易出错，且失败经验未被复用，导致工作流程脆弱。

Method: RSMeM包含层次化知识接地（基于分类检索领域语料）和失败感知经验细化（蒸馏失败轨迹为约束），通过迭代演化吸收领域知识并转化为执行经验。

Result: 在EarthBench上，RSMeM在多个LLM骨干上提升工具使用和答案性能，在DeepSeek-V3.2上准确率提升6%，额外标记不到1%。

Conclusion: RSMeM有效演化RS代理，提升工具执行稳健性，且蒸馏经验知识密度高，代码已开源。

中文摘要：地学研究需要复杂的分析和领域专业知识，其中遥感（RS）观测是关键基础。然而，现有的基于通用大语言模型的RS代理在很大程度上仍然是领域无关的，导致工作流程脆弱且易出错。此外，这些失败很少被整合到可重复使用的经验中以供后续分析。为了解决这个问题，我们介绍了RSMeM，一种知识增强的记忆演化机制，它用预蒸馏的领域知识启动RS代理，并迭代集成在线经验以实现稳健的多步骤工具执行。RSMeM由两个部分组成：（i）层次化知识接地，通过对层次化领域语料库进行基于分类的检索来指导规划和工具选择；（ii）失败感知的经验细化，将带有失败注释的工具使用轨迹蒸馏为可重复使用的约束，用于下一轮工具执行。通过迭代使用这两个过程，RS代理可以演化以吸收任务级领域知识，并有效地将其转化为实例级执行经验。在EarthBench上的广泛实验表明，RSMeM在各种LLM骨干网络上持续提高了工具使用性能和端到端答案。值得注意的是，RSMeM在DeepSeek-V3.2上实现了6%的准确率提升，同时额外的经验标记不到1%，展示了我们蒸馏经验的强大知识密度。我们的代码可在https://github.com/AI9Stars/RSMeM获取。

Original abstract: Geoscience research requires complex analysis and domain expertise, with remote sensing (RS) observations as a key foundation. However, existing RS agents built on general-purpose LLMs remain largely domain-agnostic, resulting in brittle and error-prone workflows. Moreover, these failures are seldom consolidated into a reusable experience for subsequent analyses. To address this issue, we introduce RSMeM, a knowledge-enhanced memory evolution mechanism that bootstraps RS agents with pre-distilled domain knowledge and iteratively integrates online experience for robust multi-step tool execution. RSMeM is composed of two components: (i) Hierarchical Knowledge Grounding, which performs taxonomy-aware retrieval over a hierarchical domain corpus to guide planning and tool selection; and (ii) Failure-Aware Experience Refinement, which distills failure-annotated tool-use traces into reusable constraints for next-round tool execution. By iteratively employing these two processes, RS agents can evolve to absorb task-level domain knowledge and effectively translate it into instance-level execution experience. Extensive experiments on EarthBench demonstrate that RSMeM consistently improves tool-use performance and end-to-end answer across a diverse set of LLM backbones. Notably, RSMeM achieves a 6% accuracy improvement on DeepSeek-V3.2 with less than 1% additional experience tokens, demonstrating the strong knowledge density of our distilled experience. Our code is available at https://github.com/AI9Stars/RSMeM

</details>


### [28] [LivingArena: Do LLMs Know What Other LLMs Don't? Peer-Probing as Scalable Evaluation](https://arxiv.org/abs/2607.24780)
*Xingyu Chen,Rui Wang,Zhaopeng Tu,Liefeng Bo*

Main category: cs.AI

Selection reason: 模型间互相提问的评估框架，可作为agent评估方法，可迁移

TL;DR: 提出LivingArena，一种通过模型间相互提问来评估LLM的自动化框架，能够抗污染并产生稳定排行榜。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法存在污染、饱和和主观性等问题，需要更客观且可扩展的评估方式。

Method: 让模型轮流提问并回答，通过奖励机制激励提问者发现对方知识盲区，并由强模型裁判验证问题质量。

Result: LivingArena在十个前沿LLM上产生了稳定的Elo排名，模型能识别并利用对手的认知边界。

Conclusion: 同伴探针提供了一种与人类偏好弱相关但客观可扩展的评估方法，可用于持续评估LLM。

中文摘要：评估前沿LLM具有挑战性：静态基准测试存在污染和饱和问题——导致用户无法区分顶级模型，开发人员对特定失败模式视而不见——而人类偏好是主观的。在本文中，我们的问题是：LLM是否知道其他LLM不知道的东西？我们能否利用这种动态进行评估？我们提出了LivingArena，一个自动化的、抗污染的评估框架。在该框架中，模型轮流提出问题，旨在提出对手无法正确回答的问题。提问者被鼓励主动识别和利用对手的知识边界，当回答者失败时获得奖励，否则回答者获得奖励。为了确保问题包含客观可验证的答案，一个由强模型组成的评审小组对它们进行验证，如果验证失败则惩罚提问者。在对十个前沿LLM进行评估时，LivingArena产生了一个稳定的Elo排行榜。我们的行为分析表明，模型能够识别并利用其同伴的认知边界：自我对弈和锦标赛日志表明，它们定位并加倍关注对手的薄弱维度。除了静态知识回忆之外，同伴探针还衡量了事实严谨性和探究对手弱点的更高阶能力，与人类偏好仅弱相关，并提供了一种可扩展、低成本的连续评估方法。

Original abstract: Evaluating frontier LLMs is challenging: static benchmarks suffer from contamination and saturation -- leaving users unable to distinguish top models and developers blind to specific failure modes -- while human preference is subjective. In this paper, our question is: \emph{Do LLMs know what other LLMs don't? And can we leverage this dynamic for evaluation?} We present \textbf{LivingArena}, an automated, contamination-resistant evaluation framework. In this framework, models take turns proposing questions, aiming to pose items that opponents cannot answer correctly. Questioners are encouraged to actively identify and exploit opponents' knowledge boundaries, receiving rewards when the answerer fails, while the answerer is rewarded otherwise. To ensure questions contain objectively verifiable answers, a judge panel of strong models validates them, penalizing questioners if validation fails. Evaluating ten frontier LLMs, LivingArena yields a stable Elo leaderboard. Our behavioral analyses show that models identify and exploit their peers' cognitive boundaries: self-play and tournament logs indicate that they localize and double down on opponents' weak dimensions. Beyond static knowledge recall, peer probing measures factual rigor and the higher-order ability to probe an opponent's weaknesses, correlating only weakly with human preference and offering a scalable, low-cost approach to continuous evaluation.

</details>


### [29] [Chart-Supported or Model-Supplied? Examining MLLM-Generated Claims for Accessible Visualization](https://arxiv.org/abs/2607.25021)
*Ishrat Jahan Eliza,Md Dilshadur Rahman*

Main category: cs.AI

Comments: Submitted to the 3rd Workshop on Accessible Data Visualization, IEEE VIS 2026

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 探索性研究显示，图表上下文影响MLLM输出的声明类型和数值准确性，但图像和提示设计的效果有限，需要更好的证据区分系统。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型生成的可视化描述中，解释的证据基础不明确，用户难以区分哪些是数据支持的，哪些是模型推测的。

Method: 分析了102个可视化来自四个来源，使用三个MLLM在四种输入条件下生成1224个描述，并对描述进行DIRECT/DERIVED/SPECULATIVE标签标注和数值一致性审计。

Result: 可访问图表上下文促使模型更多使用DIRECT声明并提升数值一致性；添加图像无显著改善；保留上下文提示未增加谨慎语言；模型普遍生成SPECULATIVE声明。

Conclusion: 需要开发可访问的描述系统，明确区分基于证据的声明与模型推断。

中文摘要：多模态大语言模型（MLLMs）能够将可视化模式与外部原因、后果和领域知识联系起来，但这些解释的证据基础往往不明确。我们进行了一项探索性研究，涉及来自四个来源的102个可视化、三个MLLM以及四种输入条件，这些条件变化了图像访问、来源特定的可访问图表上下文以及保留上下文框架。在1224个描述中，我们分析了模型归属的DIRECT、DERIVED和SPECULATIVE标签，并对数值一致性进行了自动审计。可访问图表上下文使Gemini和GPT转向DIRECT声明，并提高了某些模型的数值一致性。将图像添加到完整上下文中并未带来一致的数值益处，保留上下文提示也未能可靠地增加谨慎语言。提示定义的“真实世界意义”部分仍然主要是SPECULATIVE。这些结果促使我们需要可访问的描述系统，该系统能够区分由提供证据支持的声明与模型提供的解释。

Original abstract: Multimodal large language models (MLLMs) can connect visualization patterns to external causes, consequences, and domain knowledge, but the evidential basis of these interpretations is often unclear. We present an exploratory study of 102 visualizations from four sources, three MLLMs, and four input conditions that vary access to the image, source-specific accessible chart context, and withheld-context framing. Across 1,224 descriptions, we analyze model-attributed DIRECT, DERIVED, and SPECULATIVE labels and conduct an automated audit of numeric agreement. Accessible chart context shifted Gemini and GPT toward DIRECT claims and improved numeric agreement for some models. Adding the image to the full context did not yield a consistent numeric benefit, and the withheld-context prompt did not reliably increase cautious language. The prompt-defined Real-World Significance section remained predominantly SPECULATIVE. These results motivate accessible description systems that distinguish claims supported by supplied evidence from model-supplied interpretation

</details>


### [30] [SAFAARI: Schema-Aware Framework for Accelerated Advertiser Response Intelligence](https://arxiv.org/abs/2607.25042)
*Bhanu Teja Rangaraju,Chandan Kumar*

Main category: cs.AI

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: SAFAARI是一个多代理框架，通过模式链接优化NL-to-SQL，在客户支持领域实现81.66%的SEAL分数和8倍开发时间缩减。


<details>
  <summary>Details</summary>
Motivation: 现有的基于代理的聊天机器人在访问企业数据时因缺乏预定义API端点而受限，且NL-to-SQL系统中的模式链接是主要瓶颈。

Method: 提出SAFAARI多代理框架，包含内容、元数据和编排代理，并引入SEAL指标进行综合评估。

Result: 在五种特征集配置下，SAFAARI达到81.66%的SEAL分数，数据点准确性提升5.51%，模式链接精确度提升4.69%，开发时间减少8倍。

Conclusion: 该框架有效自动化模式链接和查询生成，提升客户支持效率，尤其适用于复杂数据生态系统的企业。

中文摘要：客户支持系统的进化正随着基于代理的聊天机器人迅速发展，然而这些系统在访问没有预定义API端点的企业数据时面临重大限制。本文提出了SAFAARI（Schema-Aware Framework for Accelerated Advertiser Response Intelligence，即模式感知的加速广告客户响应智能框架），这是一个多代理框架，通过专门的内容、元数据和编排代理解决了自然语言到SQL（NL-to-SQL）系统中模式链接的关键瓶颈。我们还引入了SEAL（Schema Evaluation and Accuracy in Language-to-SQL，即模式评估与语言到SQL准确性），这是一种新颖的综合指标，它在整体评估系统性能的同时惩罚不一致的结果。通过五种特征集配置的系统性实验，SAFAARI达到了81.66%的SEAL分数（比基线提高了6.65%），在数据点准确性（5.51%）和模式链接精确度（4.69%）方面均有显著提升。该框架的有效性通过领域专家参与的人机交互评估得到验证，证明了其在不同支持领域的适应性。通过自动化劳动密集型的模式链接和查询生成过程，我们的框架在保持高准确性的同时，将开发时间减少了8倍。该解决方案简化了API开发并增强了自助服务能力，尤其有利于具有复杂数据生态系统的客户支持企业。

Original abstract: The evolution of customer support systems is rapidly advancing with agentic chatbots, yet these systems face significant limitations when accessing enterprise data without predefined API endpoints. This paper presents SAFAARI (Schema-Aware Framework for Accelerated Advertiser Response Intelligence), a multi-agent framework that addresses the critical bottleneck of schema linking in Natural Language to SQL (NL-to-SQL) systems through specialized content, metadata, and orchestration agents. We also introduce SEAL (Schema Evaluation and Accuracy in Language-to-SQL), a novel composite metric that holistically evaluates system performance while penalizing inconsistent results. Through systematic experimentation with five feature set configurations, SAFAARI achieves an 81.66% SEAL score (6.65% improvement over baseline), with notable gains in datapoint accuracy (5.51%) and schema-linking precision (4.69%). The framework's effectiveness is validated through human-in-the-loop evaluation with domain experts, which proves its adaptability across diverse support domains. By automating the labor-intensive process of schema linking and query generation, our framework demonstrates 8x reduction in development time while maintaining high accuracy. The solution streamlines API development and enhances self-service capabilities, particularly benefiting customer support enterprises with complex data ecosystems.

</details>


### [31] [Explanation-Bound Tool Execution for AI Agents: Server-Verified Action Claims Without Trusting Model Rationales](https://arxiv.org/abs/2607.25364)
*Genliang Zhu,Chu Wang*

Main category: cs.AI

Comments: 25 pages, 1 figure, 15 tables. Revised presentation and synchronized evaluation details

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: EBTE是一种中介层，将工具调用的自由格式理由转换为类型化声明，并由服务器根据多项事实进行检查，以确保安全执行。


<details>
  <summary>Details</summary>
Motivation: 自由格式理由既非授权也非可靠内省，存在安全隐患，需要更严格的验证机制。

Method: 将决策相关理由内容转换为类型化操作声明，并针对服务器持有的意图、策略、负载、工具、风险、来源和新鲜度事实进行冲突检测、不完整或不确定审查，仅匹配声明可执行。

Result: 在136个一致性场景中完整匹配所有处置，无硬矛盾，通过232个变形检查；草稿集成不转发硬案例；重新验证保持相近计数；AgentDojo语义检查中拒绝所有12个攻击提议。

Conclusion: EBTE建立了配置文件一致性，并证明了在评估设置中服务器检查操作声明的可行性。

中文摘要：使用工具的主体会暴露结构化调用，但通常会附加自由格式的理由。此类理由既不是授权，也不可靠的 introspection。我们提出了解释绑定工具执行（EBTE），这是一个包含声明的中介层，它将与决策相关的理由内容转换为类型化的操作声明，并针对服务器持有的意图、策略、负载、工具、风险、来源和新鲜度事实进行检查。EBTE不能扩大基准权限：冲突会被拒绝，不完整或不确定的声明会被审查，只有匹配的声明才符合受控执行的资格。我们在显式中介和可信事实假设下形式化了这种组合，并实现了一个带有最小化审计数据包的版本化参考配置文件。在136个作者编写的一致性场景中，完整配置文件匹配所有指定的处置，不接受96个指定的硬矛盾中的任何一个，并通过了232个变形检查。仅草稿的参考集成在EBTE下不会转发48个作者编写的硬案例，同时保留所有16个软审查和4个对齐的草稿路径。在冻结的2026年7月12日的探索性224次尝试托管模型记录中，历史生成/运行器一致计数分别为71/96、66/96和19/32；对保留的最小化声明在当前管道下进行零调用重新验证得到70/96、65/96和17/32。在AgentDojo衍生的语义检查中，现有的高风险控制使所有12个攻击提议不被允许，而EBTE将任务-提议矛盾解析为拒绝。总之，这些研究建立了配置文件一致性，并证明了在评估设置中服务器检查的操作声明的可行性。

Original abstract: Tool-using agents expose structured calls but commonly attach free-form rationales. Such rationales are neither authorization nor reliable introspection. We present Explanation-Bound Tool Execution (EBTE), a claim-carrying mediation layer that converts decision-relevant rationale content into typed action claims and checks them against server-held intent, policy, payload, tool, risk, provenance, and freshness facts. EBTE cannot widen baseline authority: conflicts deny, incomplete or uncertain claims review, and only matching claims remain eligible for governed execution. We formalize this composition under explicit mediation and trusted-fact assumptions and implement a versioned reference profile with minimized audit packets. Across 136 authored conformance scenarios, the full profile matches all specified dispositions, admits none of 96 designated hard contradictions, and passes 232 metamorphic checks. A draft-only reference integration forwards none of 48 authored hard cases under EBTE while preserving all 16 soft-review and 4 aligned draft paths. In a frozen 2026-07-12 exploratory 224-attempt hosted-model record, the historical generation/runner agreement counts are 71/96, 66/96, and 19/32; a zero-call revalidation of the preserved minimized claims under the current pipeline yields 70/96, 65/96, and 17/32. In an AgentDojo-derived semantic check, existing high-risk controls make all 12 attack proposals non-allow, while EBTE resolves the task--proposal contradictions as deny. Together, these studies establish profile conformance and demonstrate the feasibility of server-checked action claims within the evaluated settings.

</details>


### [32] [CogEEGAgent: Toward Autonomous Cognitive EEG Analysis with Grounded Execution and Selection-Aware Verification](https://arxiv.org/abs/2607.25045)
*Dengzhe Hou,Lingyu Jiang,Fangzhou Lin,Kazunori D Yamada*

Main category: cs.AI

Comments: 16 pages, 5 figures, and 16 tables. The supplementary material is included in the same PDF

Selection reason: 科学agent框架，含验证与执行分离机制，可迁移至coding agent

TL;DR: CogEEGAgent通过LLM解释意图和确定性组件控制，实现了认知EEG分析中语言理解的灵活性与可审计的自动化，有效提升了路由准确性并抑制假阳性。


<details>
  <summary>Details</summary>
Motivation: EEG分析需要专业知识且涉及许多可辩护的选择，而现有LLM代理无法保证其分析选择与请求一致或独立于自适应搜索，因此需要一种结合灵活语言理解与确定性控制的自动化框架。

Method: 提出CogEEGAgent，基于MNE-Python，采用EEG特定的科学框架分离语义与科学权威：LLM解释意图并提出注册分析，确定性组件验证类型合同、控制确认访问、授权证据绑定发布。通过路由基准、模型编写的盲测和政策压力测试进行评估。

Result: 在路由基准上，CogEEGAgent比确定性路由器更准确；在盲测中，完整系统释放支持的分析并阻止预设危害；政策压力测试表明保留的确认能抑制自适应搜索导致的假阳性。

Conclusion: CogEEGAgent建立了有限自主性和可审计的自动化框架，展示了科学代理如何将灵活语言理解与对推理和发布的故障关闭控制相结合，适用于认知EEG工作流。

中文摘要：认知研究中的脑电图（EEG）分析需要专业知识，并涉及许多可辩护的选择，包括对比、通道、时间窗口和统计检验。LLM代理可以将各种自然语言问题转换为分析选择，为自动化提供灵活接口。然而，仅凭流畅的报告无法证明代理选择了所请求的分析，或独立于自适应搜索评估了确认性声明。我们提出了CogEEGAgent，一个基于MNE-Python的认知EEG分析代理。其EEG特定的科学框架将语义权威与科学权威分离。LLM解释意图并提出注册分析，而确定性组件验证类型合同、控制确认访问，并授权基于证据的发布。在一个预先指定的路由基准上，CogEEGAgent比匹配的确定性路由器更准确地将语言映射到注册分析，而匹配的预检使两个系统在需要时弃权。在一个由外部模型编写、结果盲测的活动中，完整系统发布支持的分析，具有参与者不相交的确认，并阻止预设的能力危害和生命周期重用请求。政策压力测试表明，保留的确认可以抑制未校正的自适应搜索导致的假阳性。总之，这些研究为认知EEG工作流建立了有限自主性和可审计的自动化框架。更广泛地说，它们展示了科学代理如何将灵活的语言理解与对推理和发布的故障关闭控制相结合。

Original abstract: Electroencephalography (EEG) analysis in cognitive studies requires specialized expertise and involves many defensible choices over contrasts, channels, time windows, and statistical tests. LLM agents can translate varied natural-language questions into analysis choices, offering a flexible interface for automation. Yet fluent reports alone cannot establish that an agent selected the requested analysis or evaluated a confirmatory claim independently of adaptive search. We present CogEEGAgent, a cognitive-EEG analysis agent grounded in MNE-Python. Its EEG-specific scientific harness separates semantic from scientific authority. The LLM interprets intent and proposes registered analyses, while deterministic components validate typed contracts, control confirmation access, and authorize evidence-bound release. On a prespecified routing benchmark, CogEEGAgent maps language to registered analyses more accurately than a matched deterministic router, while matched preflight makes both systems abstain whenever required. In an externally model-authored, outcome-blind campaign, the complete system releases supported analyses with participant-disjoint confirmation and blocks prespecified capability hazards and lifecycle-reuse requests. Policy stress testing shows that held-out confirmation curbs false positives from uncorrected adaptive search. Together, these studies establish bounded autonomy and an auditable automation framework for cognitive-EEG workflows. More broadly, they show how scientific agents can combine flexible language understanding with fail-closed control over inference and release.

</details>


### [33] [Addressable Recall Compaction for Long Context-Window Control in AI Agents](https://arxiv.org/abs/2607.25066)
*Thang Dang,Yuma Ichikawa,Sakina Fatima,Koichi Shirahata*

Main category: cs.AI

Comments: 20 pages, 2 figures

Selection reason: agent长上下文管理框架，直接解决coding agent记忆问题

TL;DR: 提出ARC框架，通过可寻址引用分离归档与上下文，在保持高准确率的同时降低服务成本。


<details>
  <summary>Details</summary>
Motivation: 现有压缩方法会丢失关键信息或不可靠恢复，亟需一种能保留细节且高效检索的上下文管理方案。

Method: 将工具观察存储于ID可寻址日志，压缩时替换为紧凑引用，代理通过ID请求内容，避免重新执行或相似性检索。

Result: Needle-in-a-Haystack准确率99.40%（最佳基线88.12%），LongBench-v2 Hard准确率29.97%（最佳基线28.25%），且降低服务时间和HBM流量。

Conclusion: 显式基于地址的召回能提升信息保留和服务效率，优于现有上下文管理基线。

中文摘要：长视界LLM代理会累积推理轨迹、动作和工具观察结果，这些最终可能超过模型的固定上下文窗口。现有的压缩方法通过丢弃、摘要或检索早期信息来解决这一限制，但它们可能会删除任务关键细节或无法可靠地恢复这些细节。我们提出了ARC（可寻址召回压缩），一种将归档存储与活动上下文呈现分离的上下文管理框架。ARC将工具观察结果存储在仅追加的、ID可寻址的日志中，并在需要压缩时用紧凑的引用替换较旧的观察结果。代理随后可以使用这些标识符请求存储的内容，而无需重新执行相应的工具或仅依赖基于相似性的检索。我们使用具有16k上下文窗口的Qwen3-8B和具有32k上下文窗口的Qwen3-32B来评估ARC。在Needle-in-a-Haystack评估中，ARC实现了99.40%的平均精确答案准确率，而评估中最佳基线为88.12%。ARC还根据我们的硬件成本模型减少了估计的服务器时间和HBM流量。在LongBench-v2 Hard子集上，ARC获得了29.97%的平均准确率，而最佳基线为28.25%。这些结果表明，在所测试的设置下，相对于评估的上下文管理基线，显式的、基于地址的召回可以提高信息保留和服务效率。

Original abstract: Long-horizon LLM agents accumulate reasoning traces, actions, and tool observations that can eventually exceed a model's fixed context window. Existing compaction methods address this limitation by discarding, summarizing, or retrieving earlier information, but they may remove task-critical details or fail to recover them reliably. We propose ARC (Addressable Recall Compaction), a context-management framework that separates archival storage from active-context presentation. ARC stores tool observations in an append-only, ID-addressable log and replaces older observations with compact citations when compaction is required. The agent can subsequently use these identifiers to request stored content without re-executing the corresponding tools or depending solely on similarity-based retrieval. We evaluate ARC using Qwen3-8B with a 16k context window and Qwen3-32B with a 32k context window. On the Needle-in-a-Haystack evaluation, ARC achieves an average exact-answer accuracy of 99.40%, compared with 88.12% for the best-performing baseline in our evaluation. ARC also reduces estimated serving time and HBM traffic under our hardware-cost model. On the LongBench-v2 Hard subset, ARC obtains an average accuracy of 29.97%, compared with 28.25% for the best-performing baseline. These results indicate that explicit, address-based recall can improve information retention and serving efficiency relative to the evaluated context-management baselines under the tested settings.

</details>


### [34] [How Often Should a Recommender Call an LLM? Value-Weighted Routing, Monitoring, and Seasonal Robustness](https://arxiv.org/abs/2607.25068)
*Bhavtosh Rath*

Main category: cs.AI

Selection reason: 价值加权路由策略，可启发agent成本感知调度

TL;DR: 提出Value Router，通过区分困难度和价值来优化路由决策，在合成零售模拟中显著提升高价值项目精确度而召回率不变。


<details>
  <summary>Details</summary>
Motivation: 现有路由决策仅基于难度，忽略了不同错误成本，因为困难且高价值的项目与困难且低价值的项目代价不同，需要同时考虑价值和难度。

Method: 构建合成零售营销流水线模拟，使用价值加权阈值路由器与仅难度和随机基线对比，并通过决策日志和黑五需求激增场景进行多阶段评估。

Result: 价值加权路由器在高价值项目召回率60%时精确度达98.3%（vs 仅难度94.3%），且聚合指标掩盖了类别间差异，黑五场景下静态与季节性调谐路由器表现不同。

Conclusion: 提出了成本感知路由系统的设计原则，但结果基于合成模拟，需在真实系统中验证。

中文摘要：通常，在廉价启发式算法和昂贵的大型语言模型（LLM）之间的路由决策被框定为一个难度问题：将困难案例发送到昂贵的路径。我们认为这种框架是不完整的，因为难度和商业价值是不同的轴——一个困难的廉价项目和一个困难的昂贵项目具有不同的错误成本。我们提出了Value Router，这是一个完全合成的零售营销流水线模拟，该流水线仅使用估计的难度和估计的价值来路由项目，从不使用真实值。研究分为三个阶段。首先，将价值加权阈值路由器与仅难度基线和随机基线在合成目录上进行对比，该目录的类别数量与价值之间存在负相关。价值加权在召回真正高价值项目方面与仅难度基线相匹配（60%），同时实现了显著更高的精确度（98.3% vs 94.3%）。其次，决策记录器和监视器揭示了一个被聚合指标隐藏的失败模式，表明聚合结果几乎完全由类别间差异驱动，而非每个项目的区分。第三，模拟的黑色星期五需求激增（2.5倍数量，同时向高价值类别转移）比较了静态路由器、季节性调谐路由器和两种慢路径预算策略。所有结果均来自受控的合成模拟，具有实验者定义的真实值，并说明了成本感知路由系统的设计原则，而非经过验证的现实世界主张。

Original abstract: Routing decisions between a cheap heuristic and an expensive large language model (LLM) are typically framed as a difficulty problem: send the hard cases to the expensive path. We argue this framing is incomplete because difficulty and business value are distinct axes - a difficult cheap item and a difficult costly item do not have the same cost of error. We present Value Router, a fully synthetic simulation of a retail merchandising pipeline that routes items using only estimated difficulty and estimated value, never ground truth. The study has three stages. First, a value-weighted threshold router is compared with a difficulty-only and a random baseline on a synthetic catalog with an inverse correlation between category volume and value. Value-weighting matches the difficulty-only baseline's recall of true high-value items (60%) while achieving substantially higher precision (98.3% vs. 94.3%). Second, a decision logger and monitor expose a failure mode hidden by aggregate metrics showing that the aggregate result is driven almost entirely by between-category differences rather than per-item discrimination. Third, a simulated Black Friday demand surge (2.5 volume with a shift toward higher-value categories) compares a static router, a seasonally tuned router, and two slow-path budget policies. All results are from a controlled synthetic simulation with experimenter-defined ground truth and illustrate design principles for cost-aware routing systems rather than validated real-world claims.

</details>


### [35] [Towards an Agent Operating System - Lessons from Classical and Cloud OS](https://arxiv.org/abs/2607.25076)
*Gosia Steinder,Hubertus Franke*

Main category: cs.AI

Selection reason: agent操作系统抽象，对agent架构设计有参考价值

TL;DR: 该文认为智能体AI系统正处于实验阶段，需要像POSIX和Kubernetes那样的稳定抽象以实现可移植性和可靠性，并提议通过扩展操作系统原语来推导此类抽象。


<details>
  <summary>Details</summary>
Motivation: 智能体AI系统缺乏核心抽象的共识，阻碍了可移植性、可组合性以及超越原型部署的进展。

Method: 通过将经典操作系统和云操作系统原语扩展到随机、自然语言中介的执行，推导出新的智能体抽象，并精确指定其语义。

Result: 预期结果是形成一套整合的抽象，使得智能体应用可移植、平台可可靠组合，类似于POSIX和Kubernetes的效果。

Conclusion: 遵循先前平台浪潮的方法，建立稳定的抽象，是智能体AI系统前进的道路。

中文摘要：每一波平台软件都遵循相同的轨迹：初期阶段，竞争性框架和临时实现层出不穷；随后，一套具有精确定义语义的稳定抽象被提出；最后，围绕这些抽象整合成一个平台，应用程序可以便携地移植到该平台上。POSIX 为经典操作系统做到了这一点；Kubernetes 为云做到了这一点。当前，智能体 AI 系统——由大语言模型驱动的自主智能体，能够规划、使用工具、保持记忆和协作——正处于第三波这样的实验阶段。已经出现了数十种框架和协议，但社区对于核心抽象是什么以及它们提供什么保证尚未达成共识。没有这种共识，智能体应用无法便携地编写，平台无法可靠地组合，该领域也无法超越原型部署。我们认为，前进的道路是遵循先前浪潮的方法：通过将经典操作系统和云操作系统的基本原理扩展到随机、自然语言中介的执行，推导出新的智能体抽象，精确指定其语义，并围绕它们进行整合——正如 POSIX 和 Kubernetes 整合了它们各自的浪潮一样。

Original abstract: Every major wave of platform software follows the same arc: an initial period of experimentation with competing frameworks and ad-hoc implementations, followed by the articulation of a small set of stable abstractions with well-defined semantics, and finally consolidation around those abstractions into a platform that applications can portably target. POSIX did this for classical operating systems; Kubernetes did it for the cloud. Agentic AI systems - autonomous, LLM-driven agents that plan, use tools, maintain memory, and collaborate - are currently in the experimentation phase of the third such wave. dozens of frameworks and protocols have emerged, but no community consensus exists on what the core abstractions are or what guarantees they carry. Without that consensus, agentic applications cannot be written portably, platforms cannot compose reliably, and the field cannot advance beyond prototype deployments. We argue that the path forward is to follow the prior-wave methodology: derive new agentic abstractions by extending classical OS and cloud OS primitives to stochastic, natural-language-mediated execution, specify their semantics precisely, and consolidate around them - just as POSIX and Kubernetes consolidated their respective waves.

</details>


### [36] [PLATO: Pointer Learner for Agent and Task Openness](https://arxiv.org/abs/2607.25082)
*Alireza Saleh Abadi,Leen-Kiat Soh,Daniel Alan Redder,Adam Eck,Prashant Doshi*

Main category: cs.AI

Selection reason: 多agent开放系统，pointer网络处理动态动作空间，通用方法

TL;DR: PLATO通过指针网络和图神经网络处理多智能体系统中的代理和任务开放性，在无界空间下实现强零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 现有MARL方法无法同时处理代理和任务开放性，受限于固定假设或人为边界。

Method: 提出PLATO：指针网络actor输出任务分布，GNN critic编码动态交互图，结合MAPPO在TaAgO-MG框架下训练。

Result: 在MOASEI野火抑制环境中，PLATO相比基线取得更强性能和更一致的零样本泛化。

Conclusion: PLATO有效解决OASYS中的双重开放性问题，提供灵活可扩展的解决方案。

中文摘要：开放代理系统（OASYS）在实际领域中日益普及，这些领域中代理和任务的集合随时间不可预测地变化。这种开放性，包括代理开放性（AO）和任务开放性（TO），对通常假设固定状态和动作空间的多智能体强化学习（MARL）构成了根本性挑战。现有方法仅部分解决开放性问题：填充和遮蔽方法引入了人为界限，而最近的基于图或超图的方法处理一个维度的开放性，但仍依赖于限制性假设。本文中，我们介绍了用于代理和任务开放性的指针学习器（PLATO），一种基于指针网络的actor结合集中式图神经网络（GNN）critic，采用集中式训练和分散式执行范式下的多智能体近端策略优化进行训练。我们的基于指针的actor直接在当前任务集上输出分布。这直接支持变化动作空间，无需遮蔽或重新训练。我们的GNN critic将代理-任务交互编码为随任务和代理组成而改变形状的图。这些组件共同考虑了AO和TO，没有现有方法的有界性。我们在任务和代理开放的马尔可夫博弈（TaAgO-MG）中形式化了PLATO，扩展了先前的任务开放公式，并证明了它在产生的无界状态和动作空间上是良好定义的。我们使用开放代理系统评估方法倡议（MOASEI）野火抑制领域评估了PLATO，该领域是为开放多代理系统评估而设计的环境，我们展示了相比OASYS中最先进的基线，PLATO具有更强的性能和更一致的零样本泛化能力。

Original abstract: Open agent systems (OASYS) are increasingly prevalent in real-world domains where the sets of agents and tasks change unpredictably over time. Such openness, including agent openness (AO) and task openness (TO), poses a fundamental challenge to multi-agent reinforcement learning (MARL), which typically assumes fixed state and action spaces. Existing methods address openness only partially: padding and masking approaches introduce artificial bounds, while recent graph-based or hypergraph methods handle one dimension of openness but still depend on restrictive assumptions. In this paper, we introduce Pointer Learner for Agent and Task Openness (PLATO), a pointer-network-based actor combined with a centralized graph neural network (GNN) critic, trained with multi-agent proximal policy optimization under a centralized training and decentralized execution paradigm. Our pointer-based actor outputs distributions directly over the current task set. This directly supports changing action spaces without masking or retraining. Our GNN critic encodes agent-task interactions as a graph that changes shape with task and agent composition. Together, these components consider AO and TO without the boundedness of existing approaches. We formalize PLATO in a Task-and-Agent-Open Markov Game (TaAgO-MG), extending prior task-open formulations, and prove it is well-defined over the resulting unbounded state and action spaces. We evaluate PLATO with the Methods for Open Agent Systems Evaluation Initiative (MOASEI) wildfire suppression domain, an environment designed for open multi-agent system evaluation, and we demonstrate strong performance and more consistent zero-shot generalization than state-of-the-art baselines in OASYS.

</details>


### [37] [Matryoshka Agent: Unfolding Sub-Agents for Long-Horizon Machine Learning Engineering](https://arxiv.org/abs/2607.25090)
*Rushi Qiang,Changhao Li,Haotian Sun,Yuchen Zhuang,Chao Zhang,Bo Dai*

Main category: cs.AI

Selection reason: 分层agent框架用于MLE任务，直接相关代码优化agent

TL;DR: 提出Matryoshka Agent分层智能体框架，通过编排器和子智能体解耦战略探索与执行，实现长期MLE任务的有效决策，实验显示性能提升。


<details>
  <summary>Details</summary>
Motivation: MLE任务需要长期决策和迭代调试，单体智能体难以同时处理长上下文、大解空间和有限计算资源。

Method: 提出Matryoshka Agent分层框架，包括高层编排器维护探索状态并发出指令，低层子智能体执行具体操作，通过标准工具接口交互，并开发高效训练范式。

Result: 在多种MLE任务上，Matryoshka Agent使Qwen3-4B-Instruct达到o4-mini级别性能，Qwen3-30B-Coder提升最多36.7%。

Conclusion: Matryoshka Agent是长期MLE任务的有效且可扩展范式。

中文摘要：机器学习工程（MLE）任务需要在昂贵的、反馈驱动的环境交互下，对迭代式解决方案调试和改进进行长期决策。为这样的任务开发和训练一个单体智能体从根本上具有挑战性，因为它必须同时管理极长且嘈杂的上下文，探索巨大的解空间，并在有限的模型容量和计算预算下保持有效。为了解决这些挑战，我们提出了Matryoshka Agent，一个用于复杂长期任务的统一分层智能体框架。Matryoshka Agent将智能体问题求解分解为一个协调的决策和执行层次结构：高层级的编排器维护紧凑的长期探索状态并发出战略指令，而低层级的子智能体通过标准化的工具接口与环境直接交互，执行具体的解决方案尝试。这种设计将战略探索与昂贵执行解耦，大大减轻了长上下文推理的负担，并实现了高效的迭代优化。我们进一步开发了一种高效的训练范式用于Matryoshka Agent。在多种模型类型和规模的广泛MLE任务上的实验结果表明，Matryoshka Agent是长期MLE任务和复杂智能体问题求解的有效且可扩展的范式。值得注意的是，Matryoshka Agent使得Qwen3-4B-Instruct能够达到与o4-mini相当的编排器性能。将Matryoshka Agent应用于Qwen3-30B-Coder带来了最多36.7%的相对性能提升。

Original abstract: Machine learning engineering (MLE) tasks require long-horizon decision making over iterative solution debugging and refinement, under expensive and feedback-driven environment interactions. Developing and training a monolithic agent for such tasks is fundamentally challenging, as it must simultaneously manage extremely long and noisy contexts, explore vast solution spaces, and remain effective under limited model capacity and computational budgets. To address these challenges, we propose Matryoshka Agent, a unified hierarchical agent framework for complex long-horizon tasks. Matryoshka Agent decomposes agentic problem solving into a coordinated hierarchy of decision making and execution: a high-level Orchestrator maintains compact, long-horizon exploration states and issues strategic instructions, while lower-level Sub-Agents execute concrete solution attempts through direct environment interaction, mediated by standardized Tool interface. This design decouples strategic exploration from costly execution, substantially reducing the burden of long-context reasoning and enabling efficient iterative refinement. We further develop an efficient training paradigm for Matryoshka Agent. Experimental results on a broad range of MLE tasks with diverse model types and scales demonstrate that Matryoshka Agent is an effective and scalable paradigm for long-horizon MLE tasks and complex agentic problem solving. Notably, Matryoshka Agent enables Qwen3-4B-Instruct to reach Orchestrator performance comparable to o4-mini. Applying Matryoshka Agent to Qwen3-30B-Coder results in at most 36.7% relative performance gain.

</details>


### [38] [Towards Robust Reinforcement Learning for Small-Scale Language Model Agents](https://arxiv.org/abs/2607.25091)
*Md Rezwanul Haque,Md. Milon Islam,Fakhri Karray*

Main category: cs.AI

Comments: Proceedings of the 2026 IEEE International Conference on Systems, Man, and Cybernetics (SMC), Bellevue, WA, USA

Selection reason: 小模型agent RL训练方法，对agent稳定性提升有启发

TL;DR: 本文系统研究了小语言模型（SLM）在强化学习对齐中的失败模式，提出并验证了容量余量假说，并通过适配器技术、精度调整和安全机制实现了稳定训练。


<details>
  <summary>Details</summary>
Motivation: 小语言模型使用强化学习对齐不稳定，但失败机制未系统研究。

Method: 识别三种失败模式，采用合并重新初始化适配器、float32精度、三层安全机制；提出容量余量假说。

Result: 系统稳定收敛，在流畅先验和信息性奖励下提升偏好胜率，优于指令调优基线且需更少数据。

Conclusion: 通过解决特定失败模式，SLM的PPO训练可以稳定有效，假说得到验证。

中文摘要：使用强化学习对70-500M参数范围的小语言模型（SLM）进行对齐通常被认为不稳定，尽管其失败机制尚未得到系统研究。在最新技术（SOTA）研究中，使用近端策略优化（PPO）训练了十五个（模型，语料库）配置。实验包括Pythia-70M、160M、410M以及SmolLM2-135M、360M在TinyStories、CNN/DailyMail和Wikitext-103语料库上的训练。在小规模语言模型中识别出三种可复现的失败模式：标准PEFT/TRL流水线中的静默LoRA参数冻结、使用bfloat16时重要性比率的数值溢出、以及由于奖励模型错误导致的灾难性策略崩溃。这些问题通过合并并重新初始化的适配器技术、PPO更新期间使用float32精度以及包含奖励白化、重要性比率保护和权重回滚的三层安全机制得到了解决。本文提出了容量余量假说，该假说指出SLM规模下的PPO性能取决于流畅的监督模型（PPL<20）和具有区分性的奖励信号，而不是模型参数的数量。所提出的系统在所有实验中稳定收敛，并且在具有流畅先验和信息性奖励信号的配置中，相对于SFT基线提高了偏好胜率。此外，它优于指令调整基线，同时需要的训练数据显著更少。所有检查点、偏好数据集和训练脚本均已公开发布。

Original abstract: The alignment of Small Language Models (SLMs) in the 70--500M parameter range using reinforcement learning is often considered unstable, though the underlying failure mechanisms have not been systematically investigated. In the State-of-the-Art (SOTA) research, fifteen (model, corpus) configurations were trained using Proximal Policy Optimization (PPO). The experiments included Pythia-70M, 160M, 410M and SmolLM2-135M, 360M on the TinyStories, CNN/DailyMail, and Wikitext-103 corpora. Three reproducible failure modes were identified in small-scale language models: silent LoRA parameter freezing in standard PEFT/TRL pipelines, numerical overflow in importance ratios when using bfloat16, and catastrophic policy collapse due to reward-model error. These issues were addressed using a merge-and-reinitialize adapter technique, float32 precision during PPO updates, and a three-layer safety mechanism comprising reward whitening, importance-ratio guarding, and weight rollback. In this paper, a capacity-headroom hypothesis is proposed, which states that PPO performance at the SLM scale depends on both a fluent supervised model ($\text{PPL}<20$) and a discriminative reward signal, rather than on the number of model parameters. The proposed system converged stably in all experiments and improved preference win rate over the SFT baseline in configurations with a fluent prior and an informative reward signal. Furthermore, it outperformed instruction-tuned baselines while requiring significantly less training data. All checkpoints, preference datasets, and training scripts are publicly released$^§$.

</details>


### [39] [How Affect Propagates among LLM Agents: Emergent Emotional Contagion in Crowd Simulation](https://arxiv.org/abs/2607.25140)
*Funda Durupinar*

Main category: cs.AI

Comments: 31 pages, 14 figures

Selection reason: 多智能体情感传播，对agent交互设计有启发。

TL;DR: 本文提出基于LLM的多智能体情感传播架构，通过感知-评估-表达循环实现情感传染，实验表明动态具有空间、时间、人格和后端依赖性。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型在群体模拟中实现情感传播的机制，避免手工设计情感传递规则，使智能体情感交互更自然。

Method: 构建基于LLM的评估模块，结合大五人格和Russell情感模型，在五种不同场景（警报、快乐、中性）中评估情感传染动态。

Result: 情感传染在稀疏人群中呈现波前传播和平台效应，人格分布决定恐慌触发和情绪解读，动态依赖LLM后端。

Conclusion: 系统能产生复杂情感传染，但受后端模型影响，提示需谨慎选择LLM以控制模拟行为。

中文摘要：本文研究了语言模型在多智能体群体模拟中的行为，重点关注情感如何在相互感知和评估的智能体之间传播。每个智能体通过视觉、听觉和触觉通道感知其邻居，然后根据其提示的人格特征、记忆、当前情感状态和情境背景对这些感知进行评估。评估由LLM执行，更新智能体的内部情感状态并选择其外部表达。该架构没有直接传递智能体间情感状态的手工机制；相反，智能体之间的影响通过感知-评估-表达循环产生。智能体表示借鉴了大五人格模型和Russell的情感环状模型。为了限制延迟，底层转向和导航由独立于基于LLM的认知层的传统人群模拟器处理。我们在五个情景环境中评估了该架构，涵盖了令人警觉、快乐和中立的情况以及不同的空间布局。结果表明，该系统在稀疏的小型人群中产生了具有空间、时间和人格依赖结构的情感传染动态。警报从种子智能体以传播波前的方式扩散，平均警报比例稳定在非零平台，而提示人格特征的分布决定了模糊警报是否会引发恐慌，以及挑衅被解释为愤怒还是恐惧。我们还通过提示变体、采样温度和四种模型后端的对照实验进一步评估了评估步骤，表明动态依赖于后端。

Original abstract: This paper studies the behavior of language models in a multi-agent crowd simulation, focusing on how affect propagates among agents that perceive and appraise one another. Each agent perceives its neighbors through visual, auditory, and tactile channels, then appraises these perceptions in light of its prompted personality profile, memory, current affective state, and situational context. Appraisal is carried out by an LLM, which updates the agent's internal affective state and selects its outward expression. The architecture contains no hand-authored mechanism for directly transferring affective state between agents; instead, inter-agent influence arises through the perception-appraisal-expression loop. The agent representation draws on the Big Five personality model and Russell's circumplex model of affect. To limit latency, low-level steering and navigation are handled by a conventional crowd simulator operating independently of the LLM-based cognitive layer.
  We evaluate the architecture across five scenario environments spanning alarming, joyful, and neutral situations in different spatial layouts. The results show that the system produces emotional contagion dynamics with spatial, temporal, and personality-dependent structure in sparse, small crowds. Alarm spreads from seeded agents as a traveling front, the mean alarmed fraction settles at a nonzero plateau, and the distribution of prompted personality profiles determines whether an ambiguous alarm ignites panic and whether a provocation is interpreted as anger or fear. We further evaluate the appraisal step through controlled experiments across prompt variants, sampling temperatures, and four model backends, showing that the dynamics are backend-dependent.

</details>


### [40] [When Do Agent Loops Mistake Stagnation for Progress? Self-Evaluation Bias and Externally Grounded Verification in Long-Running Autonomous LLM Agent Loops](https://arxiv.org/abs/2607.25152)
*Hyundoo Park,Byungho Choi*

Main category: cs.AI

Comments: 23 pages. Preregistered pilot measurement study. Also deposited on Zenodo (concept DOI 10.5281/zenodo.21594735)

Selection reason: 直接研究agent循环自我验证偏差，对代码优化agent可靠性有重要启发。

TL;DR: 本文提出“进步海市蜃楼”现象，证明自主智能体的自我评估存在偏差，并指出解决之道在于使用具有现实世界访问权限的带外评估。


<details>
  <summary>Details</summary>
Motivation: 现有自主智能体依赖自我评估，但存在偏差导致实际性能停滞或倒退，需要探究评估依据的影响。

Method: 构建测试平台，固定智能体和工具，仅改变评估通道类型；使用容器隔离的世界状态预言机进行验证；对比不同评估者（自我报告、带内评判、带外评判）的效果。

Result: 54个循环中智能体自认为每次改进，但56%实际变化为零或负；强带内评判者仍接受44%的退步循环；基于工件可验证的任务中海市蜃楼消失；符号变体与完整反馈效果相当。

Conclusion: 对于开放目标，扩大评判者规模不够，需要带外评估；成功信号的位置决定评估有效性。

中文摘要：长时间运行的自主智能体在没有人类干预的情况下规划、行动并判断自己的完成情况。当智能体为自己的工作打分时，自我评估偏差就会出现：看似合理的改变被认为是进步，而现实世界的结果却停滞或倒退。我们将这种失败模式命名为“进步海市蜃楼”，并通过受控测量表明，这取决于评估者所依据的信息基础。我们构建了一个测试平台，固定智能体及其工具表面，仅操纵控制循环的评估者的信息通道类型。一个世界状态预言机，原则上不可伪造，通过容器和网络隔离来强制执行，并在每次运行时进行验证。在54个循环中，一个前沿智能体每次都声称有改进，但56%的循环的实际变化为零或负值。因此，自我报告是没有信息量的，自我判定门退化为“全部接受”，侵蚀了它已达到的最佳部署状态达19%。即使是最强的带内评判者，阅读完整的工件文本、变更差异及其自身的判定历史，也接受了其中44%是现实世界退步的循环，并拒绝了38%的实际改进；预先注册的对抗性假设，即强评判者会缩小差距，被拒绝。在一个边界任务上，其成功规范可以从工件本身验证，同一个评判者的海市蜃楼消失为零，差距在注册阈值内崩溃，表明差距取决于成功信号所在的位置。一个仅返回接受或拒绝判定的符号变体，保持了与完整反馈相似的真实世界输出（110.0 vs 113.0），将收益定位于门的依据而非反馈内容。对于成功信号存在于记录之外的目标，扩大评判者规模是不够的；具有现实世界访问权限的带外评估是一个结构性要求。

Original abstract: Long-running autonomous agents plan, act, and judge their own completion without human intervention. When an agent grades its own work, self-evaluation bias takes hold: plausible changes are accepted as progress while real-world outcomes stagnate or regress. We name this failure mode the progress mirage and show, with controlled measurement, that it is a question of what the evaluator is grounded in. We built a testbed that holds the agent and its tool surface fixed and manipulates only the information-channel type of the evaluator that gates the loop. A world-state oracle, unfakeable in principle, is enforced by container and network isolation and verified at every run. Across 54 cycles a frontier agent claimed improvement every time, yet 56 percent had a measured delta of zero or below. Self-report was thus uninformative, and the self-verdict gate degenerated into accept-all, eroding the best deployed state it had reached by 19 percent. Even the strongest in-band judge, reading the full artifact text, the change diff, and its own verdict history, accepted cycles of which 44 percent were real-world regressions and rejected 38 percent of real improvements; the preregistered adversarial hypothesis that a strong judge closes the gap was rejected. On a boundary task whose success specification is verifiable from the artifact itself, the same judge's mirage vanished to zero and the gap collapsed within the registered threshold, showing that the gap depends on where the success signal resides. A sign-only variant returning only the acceptance verdict kept real-world output similar to full feedback (110.0 versus 113.0), locating the benefit in the gate's grounding rather than in feedback content. For open-ended objectives whose success signal lives outside the transcript, scaling up the judge is not enough; out-of-band evaluation with real-world access is a structural requirement.

</details>


### [41] [The User Asks, Platforms Compete: How Agentic Recommendation Markets Take Shape](https://arxiv.org/abs/2607.25253)
*Deyao Hong,Kehan Zheng,Qian Li,Jun Zhang,Jie Jiang,Hongning Wang*

Main category: cs.AI

Selection reason: agentic推荐市场中的用户agent机制，可迁移至agent工具使用与反馈设计。

TL;DR: 基于LLM的用户代理使平台在用户指定需求后竞争注意力，扩大了物品访问但导致平台策略性解释，引入反馈机制可提升购买率。


<details>
  <summary>Details</summary>
Motivation: 传统推荐中用户进入平台后才产生推荐，而LLM用户代理允许用户在进入平台前指定需求，平台需竞争用户注意力，这改变了推荐过程，需研究新机制设计问题。

Method: 在三个产品领域进行受控的基于LLM的实验，比较平台中心推荐与用户中心推荐，并分析平台策略行为及用户反馈的影响。

Result: 用户中心推荐扩大了相关物品进入比较的机会；平台选择性正面解释占据首位73-78%；当用户代理关联平台行为与用户反馈时，该比例降至36-41%，用户购买率增加。

Conclusion: 用户代理的查询、排序和反馈机制直接影响用户效用，设计代理推荐需联合考虑访问、注意力和问责制。

中文摘要：在线推荐传统上是在用户进入平台后进行的，这决定了候选池和展示给用户的排序。基于LLM的用户代理实现了一种不同的推荐过程：用户在选择平台之前指定需求，让平台竞争用户的注意力，我们称之为代理推荐市场。在我们跨三个产品领域的受控基于LLM的实验中，我们发现这种新的推荐设置创造了访问和注意力之间的张力。与传统的以平台为中心的推荐相比，以用户为中心的推荐大大扩展了相关项目进入比较的机会；然而，更广泛的参与并不能直接转化为有效的曝光。竞争直接引发了平台的策略性行为：选择性正面解释占据了第一位置的73-78%。当用户代理将平台的行为与后续的用户反馈联系起来时，这一比例下降到36-41%，而用户购买相关物品的可能性增加。因此，用户代理不仅仅是更大候选池上的排序器：其查询、排序和反馈机制——管理谁可以竞争、稀缺注意力如何分配以及早期结果如何影响对平台的评估——直接影响用户效用。因此，设计代理推荐需要将访问、注意力和问责制作为一个联合机制设计问题来处理。

Original abstract: Online recommendation has traditionally taken place after a user enters a platform, which determines the candidate pool and the ranking shown to the user. LLM-based user agents enable a different recommendation process: a user specifies a need before choosing a platform, leaving platforms to compete for the user's attention, which we refer to as an agentic recommendation market. In our controlled LLM-based experiments across three product domains, we find this new setting of recommendation creates a tension between access and attention. Compared with traditional platform-centric recommendation, user-centric recommendation greatly expands the opportunity for relevant items to enter comparison; yet broader participation does not translate directly into effective exposure. Competition directly triggers platforms' strategic play: selectively positive explanations occupy 73--78% of first-ranked positions. When the user agent relates platforms' actions to subsequent user feedback, this share falls to 36--41%, while the chance of a user purchasing the relevant item increases. A user agent is therefore more than a ranker over a larger pool of candidates: its querying, ranking, and feedback mechanism governing who can compete, how scarce attention is allocated, and how earlier outcomes shape the evaluation of platforms directly affect user utility. Designing agentic recommendation therefore requires treating access, attention, and accountability as a joint mechanism design problem.

</details>


### [42] [ContractHIL-HLS: Contract-Aligned Multi-Agent Workflow with Hardware-in-the-Loop Feedback for HLS Design](https://arxiv.org/abs/2607.25283)
*Jingbo Zhang,Haoxiang Sun,Wenbo Wang,Wenbo Zhang*

Main category: cs.AI

Selection reason: 多agent工作流优化硬件设计，合约对齐与硬件反馈可迁移至代码优化agent。

TL;DR: ContractHIL-HLS是一种通过结构化契约和多智能体协作，利用硬件反馈提升HLS设计质量和板级性能的工作流。


<details>
  <summary>Details</summary>
Motivation: 解决现有LLM辅助HLS缺乏系统和板级闭合、以及自然语言需求到硬件实现的语义对齐问题。

Method: 提出契约对齐多智能体工作流，包括契约智能体、HTML智能体和硬件在环智能体，并引入硬件反馈循环。

Result: 在94个HLS-Eval任务上，pass@1从64.0%提升到70.4%，pass@5达76.6%；在PQC加速器上，平均运行时间从207.3ms降至52.4ms，保持验证正确性。

Conclusion: ContractHIL-HLS通过结构化契约和硬件反馈显著提升了HLS设计的正确性和板级性能，为实用HLS工程提供了有效方案。

中文摘要：本文提出了ContractHIL-HLS，一种面向实用高层次综合（HLS）工程的契约对齐多智能体工作流。该工作流有三项贡献。首先，它引入了一种结构化契约作为语义对齐和任务执行工件，将自然语言需求转化为明确的接口、约束、验证检查和回滚规则。其次，它通过将HLS、Vivado、PYNQ运行时、功耗和失败证据反馈到生成过程中，将硬件信息纳入反馈循环，从而将LLM辅助的HLS从内核代码扩展到系统和板级闭合。第三，它根据语义降级和执行任务而非对话角色来分解智能体：契约智能体将自然语言降级为契约，HTML智能体将契约渲染为持久化的结构化HTML，而硬件在环智能体则利用实测证据实现和修改设计。我们在两部分中评估了ContractHIL-HLS。在94个本地可执行的HLS-Eval任务上，结构化契约提供了最大的小型设计增益，将估计的单样本测试台通过率从64.0%提高到70.2%；完整流程达到了70.4%的pass@1和76.6%的pass@5。由于HLS-Eval不涉及板级设计，我们还在一个经过板级测试的ML-KEM/ML-DSA后量子密码学（PQC）安全消息加速器上验证了ContractHIL-HLS，其中保留的双比特流组织将六条消息的平均文本运行时间从207.3毫秒减少到52.4毫秒，两个映像均具有正的路由WNS，同时保持了解密消息验证。我们在BJUT-CS316-LAB/ContractHIL-HLS（https://github.com/BJUT-CS316-LAB/ContractHIL-HLS）开源了我们的工作。

Original abstract: This paper presents ContractHIL-HLS, a contract-aligned multi-agent workflow for practical high-level synthesis (HLS) engineering. The workflow makes three contributions. First, it introduces a structured contract as the semantic-alignment and task-execution artifact that translates natural language requirements into explicit interfaces, constraints, validation checks, and rollback rules. Second, it incorporates hardware information into the feedback loop by feeding HLS, Vivado, PYNQ runtime, power, and failure evidence back into generation, thereby extending LLM-assisted HLS from kernel code toward system- and board-level closure. Third, it decomposes agents by semantic lowering and execution tasks rather than by conversational roles: a Contract Agent lowers natural language into the contract, an HTML Agent renders the contract as persistent structured HTML, and a Hardware-in-the-Loop Agent implements and revises the design with measured evidence. We evaluate ContractHIL-HLS in two parts. On 94 locally executable HLS-Eval tasks, the structured contract provides the largest small design gain, improving the estimated single-sample testbench pass rate from 64.0% to 70.2%; the full flow reaches 70.4% pass@1 and 76.6% pass@5. Because HLS-Eval does not exercise board-level design, we also validate ContractHIL-HLS on a board tested ML-KEM/ML-DSA post-quantum cryptography (PQC) secure-message accelerator, where the retained dual-bitstream organization reduces six-message average text runtime from 207.3 ms to 52.4 ms with positive routed WNS on both images while preserving decrypted-message verification. We open-source our work at BJUT-CS316-LAB/ContractHIL-HLS (https://github.com/BJUT-CS316-LAB/ContractHIL-HLS).

</details>


### [43] [Cardiologent: Multi-Agent Clinical Decision Support for Patient-Level Arrhythmia Assessment, Urgency, and Management](https://arxiv.org/abs/2607.25340)
*Sukju Oh,Moo-Yong Rhee,Jae-Sik Jang,Sukkyu Sun*

Main category: cs.AI

Comments: 21 pages, 8 figures

Selection reason: 多agent临床决策系统，多源融合与知识推理对复杂agent设计有参考价值。

TL;DR: 提出了Cardiologent多智能体系统，用于患者层面心律失常决策支持，从检测到决策覆盖全过程，通过结合多信号特征、患者数据和临床指南推理，评估临床决策，优于传统方法和LLM评判者。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型与心电图结合的方法仅处理单次记录，缺乏患者层面整合；代理系统要么依赖设备检测，要么止步于诊断而不做临床决策，无法支持从检测到决策的完整流程。

Method: 设计Cardiologent多智能体系统，每个信号（ECG导联、PPG）对应一个智能体，基于测量特征进行窗口读取；综合形成患者心律概况，结合患者自身数据，根据检索的临床指南推理，并由批评者验证每个结论与指南的一致性。

Result: Cardiologent在所有评估指标（综合诊断、临床意义、紧急程度和管理）上得分最高，在患者层面任务上优于心脏病专家和LLM评判者；LLM评判者与心脏病专家的一致性（ICC 0.74, 0.66）与专家间一致性（0.67）相当。

Conclusion: Cardiologent提供可审计的临床决策，每个结论可追溯至指南并经专家验证，避免了盲目执行，为持续监测中的临床应用奠定了基础。

中文摘要：同一阵心房颤动在健康成年人中是一个微小发现，而在患有高血压的老年患者中却是抗凝治疗的理由：相同的信号，相反的决定。命名心律只是开始；决定患者结局的是随后的判断——在整个记录中心律失常是什么，对这位患者意味着什么，以及应该如何处理。最近将大语言模型与心电图配对的工作止步于此，仅读取单次记录而不整合患者层面的发现；围绕它构建的代理系统要么接收设备已检测到的心律失常，要么针对不同的诊断任务，在需要做出决策之前停止。我们将患者层面的心律失常决策支持作为一个任务来制定，并提出了Cardiologent，一个覆盖从检测到决策的多智能体系统。每个信号对应一个智能体——单个心电图导联和可穿戴设备获取的光电容积描记图——将其窗口读数建立在测量的特征而非裸标签上；这些读数被整合成患者的心律概况，并结合患者自身数据，根据为病例检索到的临床指南进行推理，同时有一个评论者根据其引用的指南检查每个结论。我们评估临床决策而非报告，涵盖综合诊断、临床意义以及紧急程度和管理。Cardiologent在每个轴上都得分最高，在每个患者层面任务上均领先，无论是与心脏病专家还是与大规模语言模型评判者相比——后者与心脏病专家的一致性（ICC 0.74, 0.66）与心脏病专家之间的一致性（0.67）相匹配。由于每个结论都追溯到一个引用的指南，并经心脏病专家验证，它产生的决策是临床医生可以审计的，而非盲目执行——这是向持续监测中使用迈出的一步。

Original abstract: The same episode of atrial fibrillation is a minor finding in a healthy adult and grounds for anticoagulation in an elderly patient with hypertension: identical signal, opposite decision. Naming the rhythm is only the start; what determines a patient's outcome is the judgement that follows -- what the arrhythmia is across the whole record, what it means for this patient, and what should be done about it. Recent work pairing large language models with the ECG stops short of this, reading one recording without assembling a patient-level finding; and agentic systems built around it either receive the arrhythmia a device has already detected or target a different diagnostic task, stopping before the decision this task requires. We formulate patient-level arrhythmia decision support as a task and present Cardiologent, a multi-agent system that spans it from detection to decision. An agent for each signal -- a single ECG lead and the photoplethysmogram a wearable acquires -- grounds its window reading in measured features rather than a bare label; the readings are assembled into the patient's rhythm profile and, with the patient's own data, reasoned against clinical guidelines retrieved for the case, with a critic checking each conclusion against the guideline it cites. We evaluate the clinical decision rather than the report, across integrated diagnosis, clinical significance, and urgency and management. Cardiologent scores highest on every axis, first on every patient-level task under both cardiologists and an at-scale LLM judge -- whose agreement with the cardiologists (ICC 0.74, 0.66) matches theirs with each other (0.67). Because each conclusion traces to a cited guideline and is validated against expert cardiologists, it yields decisions a clinician can audit rather than act on blindly -- a step toward use in continuous monitoring.

</details>


### [44] [ODYSSE: Episode-wise Policy Optimization for Personalized Agentic Reasoning](https://arxiv.org/abs/2607.25369)
*Jiaqi Zhang,Tong Chen,Junliang Yu,Quoc Viet Hung Nguyen,Hongzhi Yin*

Main category: cs.AI

Selection reason: 面向长程agent推理的强化微调框架，可迁移至代码优化agent训练。

TL;DR: ODYSSE是一个基于强化微调的框架，通过逐幕GRPO（ESPO）实现个性化智能体推理，在长视界GUI推理任务上优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 以人为中心的场景具有模糊请求，导致巨大的解决方案空间，需要解码用户偏好以缩小空间，这引入了个性化智能体推理的挑战。

Method: 提出ODYSSE框架，核心是ESPO（逐幕GRPO），包含幕级奖励机制、幕级优势估计和幕级批次采样器，用于长视界和跨步依赖的个性化推理。

Result: 在现实长视界个性化GUI推理任务上，ODYSSE持续优于专业和通用LVLM。

Conclusion: ODYSSE在个性化智能体推理中展现出有效性。

中文摘要：智能体系统在与现实世界交互、利用外部工具以及为用户提供服务方面的能力迅速提升。然而，与假设有明确定义指令的自然世界任务不同，以人为中心的场景具有模糊请求的特点，导致巨大的、开放式的解决方案空间。因此，解码用户的个性化偏好对于缩小候选解决方案空间至关重要。这引入了一个新的挑战，即个性化智能体推理，要求智能体与用户和环境共同交互以提供个性化服务。在本文中，我们提出了ODYSSE，一个用于个性化智能体推理的强化微调（RFT）框架。其核心是ODYSSE提出了逐幕GRPO（ESPO），这是组相对策略优化（GRPO）的一种新颖扩展，旨在解决个性化智能体推理中的长动作视界和强跨步依赖。ESPO不是独立优化单个步骤，而是引入了幕级奖励机制以及幕级优势估计，使得上游证据能够有效指导下游个性化决策，并允许智能体在多个交互步骤中逐步解决模糊的用户请求。我们还提出了一种幕级批次采样器，将同一幕中的动作分组到统一的训练批次中，以促进ESPO下的连贯优化。我们在现实的长视界个性化GUI推理任务上评估了ODYSSE。实验结果表明，ODYSSE持续优于专业和通用LVLM，突出了其在个性化智能体推理中的有效性。

Original abstract: Agentic systems have rapidly advanced in their ability to interact with real-world environments, leverage external tools, and provide services for users. However, unlike natural-world tasks that assume well-defined instructions, human-centered scenarios are characterized by ambiguous requests that lead to large, open-ended solution spaces. Decoding users' personalized preferences is therefore essential for narrowing the candidate solution space. This introduces a new challenge, personalized agentic reasoning, which requires agents to jointly interact with both users and environments to deliver personalized services. In this paper, we present ODYSSE, a Reinforced Fine-Tuning (RFT) framework for personalized agentic reasoning. At its core, ODYSSE proposes Episode-wise GRPO (ESPO), a novel extension of Group Relative Policy Optimization (GRPO) designed to address long action horizons and strong cross-step dependencies in personalized agentic reasoning. Rather than optimizing individual steps independently, ESPO introduces an episode-level reward mechanism together with episodic advantage estimation, enabling upstream evidence to effectively guide downstream personalized decisions and allowing agents to progressively resolve ambiguous user requests across multiple interaction steps. We further propose an episodic batch sampler that groups actions from the same episode into unified training batches, facilitating coherent optimization under ESPO. We evaluate ODYSSE on realistic long-horizon personalized GUI reasoning tasks. Experimental results demonstrate that ODYSSE consistently outperforms both specialist and general-purpose LVLMs, highlighting its effectiveness for personalized agentic reasoning.

</details>


### [45] [Cyber-Capable AI Agents: Vulnerabilities, Evaluation Containment, and Defensive Response](https://arxiv.org/abs/2607.25379)
*Abu Bakar Siddik*

Main category: cs.AI

Comments: 27 pages, 8 figures, 8 tables

Selection reason: 网络安全agent漏洞与评估，对构建可靠agent有启发。

TL;DR: 综述了AI智能体在安全评估环境中的五类漏洞，并通过案例分析提出了兼顾能力评估与环境安全的实际优先事项。


<details>
  <summary>Details</summary>
Motivation: 现有工作分别衡量网络能力或编目攻击，但缺乏对在评估环境中限制智能体能力的指导。

Method: 综合文献归纳五类漏洞边界，并结合2026年Hugging Face/OpenAI事件进行案例研究。

Result: 识别出五类漏洞：多步进攻链、沙箱冲突、供应链/凭证暴露、持久C2、自动化速度；并分析了遏制、权限分离等控制措施的双重用途问题。

Conclusion: 需要同时评估网络能力与环境安全，并优先关注边界控制、权限分离和响应访问。

中文摘要：具备网络能力的AI智能体将语言模型与工具、内存和执行环境相结合，以执行多步进攻性安全任务。现有工作分别衡量网络能力并编目针对智能体组件的攻击，但对于在用于评估智能体的环境中限制其能力提供的指导较少。本综述综合了该边界上的五类漏洞：多步进攻链、与沙箱边界冲突的目标、供应链和凭证暴露、持久性命令与控制，以及自动化行动的速度。我们以2026年7月报告的Hugging Face/OpenAI事件作为有限案例研究，区分事件特定观察与广泛文献中确立的发现。通过分类法和案例，我们考察了针对遏制、权限分离、来源和响应者访问的控制措施，包括防御性工件也可能被滥用的双重用途问题。本综述确定了评估网络能力同时保障该能力运行环境安全的实际优先事项。

Original abstract: Cyber-capable AI agents combine language models with tools, memory, and execution en- vironments to perform multi-step offensive-security tasks. Existing work separately measures cyber capability and catalogs attacks against agent components, but provides less guidance on containing a capable agent within the environments used to evaluate it. This review synthe- sizes five vulnerability classes at that boundary: multi-step offensive chains, objectives that conflict with sandbox boundaries, supply-chain and credential exposure, persistent command- and-control, and the speed of automated action. We use the reported July 2026 Hugging Face/OpenAI incident as a bounded case study, distinguishing incident-specific observations from findings established in the wider literature. Across the taxonomy and case, we examine controls for containment, privilege separation, provenance, and responder access, including the dual-use problem that defensive artifacts may also enable misuse. The review identifies practical priorities for evaluating cyber capability together with the security of the environment in which that capability is exercised.

</details>


### [46] [HANDBOOK.md: A Benchmark for Long-Context Agentic Instruction Following](https://arxiv.org/abs/2607.25398)
*Liudas Panavas,Sebastian Minus,Bradley Monton,Derek Ray,Suhaas Garre,Sushant Mehta,Edwin Chen*

Main category: cs.AI

Comments: 16 pages, 3 figures, 5 tables. Accepted to the Workshop on Agent Behavior (WAB) at COLM 2026. Benchmark, environments, and evaluation harness: https://github.com/surge-ai/handbook

Selection reason: 长上下文agent指令遵循基准，对评估代码优化agent的规则遵守有启发。

TL;DR: 提出了HANDBOOK.md基准测试，评估语言模型智能体遵循长文档策略的能力，发现当前模型在严格评分下通过率不超过36.2%，并揭示了常见的失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要衡量任务完成情况，而非检查智能体是否严格遵守长文档策略，缺乏对部署模式（如系统提示、策略文件）的针对性评估。

Method: 构建65个基于企业手册的智能体任务，包含5个领域和10家公司，每个任务有独有策略（20-124页），使用确定性评分（824条标准），进行严格评测。

Result: 最佳模型配置在严格评分下通过36.2%的试验，大多数前沿配置低于25%；失败模式包括策略被覆盖、检查后违规、长程遗忘和虚假合规报告。

Conclusion: 当前语言模型智能体在遵循长期策略方面表现有限，需进一步改进以适应用户对策略约束的信任期望。

中文摘要：语言模型智能体越来越多地被部署在执行长期指令的场景下：系统提示、策略文件或技能文档被置于上下文中，智能体被信任让其支配后续的每一个动作。现有的基准测试很少直接测试这种部署模式；它们衡量智能体是否能完成任务，而不是一份长且有约束力的策略文档是否能在长时间的工具使用过程中约束其行为。我们提出了 HANDBOOK.md，一个包含65个智能体任务的基准测试，模拟企业员工遵循公司手册的方式。每个任务将智能体置于一个自包含的公司环境中，包括文件工作区以及通过模型上下文协议公开的模拟电子邮件、聊天、日历、问题跟踪和商务服务，并指示其执行受专家编写的标准操作程序（20到124页）约束的日常专业工作。任务涵盖五个领域（金融、医疗账单、保险、物流和人力资源）和十家虚构公司。为了防止记忆，每个任务修改了十个基础手册中的一个，改变了评分所依据的具体规则和阈值，因此没有两个任务共享相同的策略。评分是完全确定性的：每个任务带有一个程序化标准清单（共824条），同时检查所需动作是否发生以及禁止动作是否未发生。在严格评分下（即一次试验仅当满足所有标准才算通过），三十个评估模型配置中最好的通过率为36.2%，大多数前沿配置保持在25%以下。失败遵循一致的模式：智能体允许环境中看似合理的请求覆盖现有策略，执行要求的检查然后违背其结果行动，在长时间跨度中丢失规则细节，并报告未实现的合规性。我们发布了所有任务、环境和评估工具。

Original abstract: Language-model agents are increasingly deployed under standing instructions: a system prompt, a policy file, or a skills document is placed in context, and the agent is trusted to let it govern every action that follows. Existing benchmarks rarely test this deployment pattern directly; they measure whether an agent can complete a task, not whether a long, binding policy document actually constrains its behavior over an extended tool-use horizon. We present HANDBOOK.md, a benchmark of 65 agentic tasks modeled on how enterprise employees follow company handbooks. Each task places an agent in a self-contained company environment, a file workspace together with mock email, chat, calendar, issue-tracking, and commerce services exposed over the Model Context Protocol, and instructs it to carry out routine professional work governed by an expert-written standard operating procedure of 20 to 124 pages. Tasks span five domains (finance, medical billing, insurance, logistics, and HR) and ten fictional companies. To resist memorization, every task modifies one of ten base handbooks, altering the specific rules and thresholds on which grading turns, so no two tasks share a policy. Grading is fully deterministic: each task carries a rubric of programmatic criteria (824 in total) that check both that required actions occurred and that prohibited actions did not. Under strict grading, where a trial passes only if every criterion is satisfied, the best of thirty evaluated model configurations passes 36.2% of trials, and most frontier configurations remain below 25%. Failures follow consistent patterns: agents let a plausible in-environment request override the standing policy, perform a required check and then act against its result, lose rule details over long horizons, and report compliance they did not achieve. We release all tasks, environments, and the evaluation harness.

</details>


### [47] [COVENANT: Natural-Language Workflow Compilation for Aligned Agent Execution](https://arxiv.org/abs/2607.25400)
*Jincheng Wang,Min Zheng,Tao Wei*

Main category: cs.AI

Comments: 12 pages, 7 figures

Selection reason: 提出工作流编译与解释架构，提升LLM agent执行对齐，属通用agent方法。

TL;DR: COVENANT通过将工作流指令编译为控制流图并解释执行，显著减少了LLM智能体的工作流错位，将成功率从50%提升至83%。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在执行自然语言工作流指令时容易跳过步骤、采取错误分支或使用无效参数，导致工作流错位，现有提示跟随方法无法保证可靠执行。

Method: 提出COVENANT编译器-解释器架构：将指令转换为工作流抽象语法树（WAST）并降为工作流控制流图（WCFG），运行时控制器逐一解释节点，在执行前检查提议是否符合指令要求，并返回诊断反馈。

Result: 在120个案例（三个基准、七个场景）上，COVENANT将成功率从50.00%提升至83.33%，工作流错位失败率从42.50%降至15.83%（相对降低62.75%）。

Conclusion: COVENANT有效缓解了工作流错位，使LLM智能体对齐从单步提示跟随转向可靠的多步骤工作流执行。

中文摘要：大型语言模型（LLM）智能体越来越多地被赋予自然语言工作流指令（例如零售支付策略），这些指令不仅指定要实现的结果，还指定允许哪些步骤、分支和工具交互。然而，当这些指令作为提示上下文提供时，模型保留了对过程选择和步骤执行的控制。随着交互的积累，智能体可能会跳过必需步骤，采用不支持的分支，或使用不支持的参数或效果执行有效步骤——我们将这种失败模式称为工作流错位。在这项工作中，我们提出COVENANT，一种用于工作流对齐的智能体执行的编译器-解释器架构。我们的关键洞察是将工作流指令视为源程序而不是提示。COVENANT将指令转换为工作流抽象语法树（WAST），并将其降低到工作流控制流图（WCFG）。在运行时，控制器一次解释一个WCFG节点，在提交控制器状态或推进图之前，根据从指令中提取的要求检查每个提议，并返回诊断反馈以进行修复。为了评估COVENANT，我们使用来自三个现有基准的120个案例，涵盖七个工作流场景。与最先进的LLM智能体相比，COVENANT将基准成功率从50.00%提高到83.33%，并将工作流错位失败率从42.50%降低到15.83%（相对降低62.75%）。这些结果表明，COVENANT显著减轻了工作流错位，将LLM智能体对齐从孤立的提示跟随转向可靠执行复杂多步骤工作流。

Original abstract: Large language model (LLM) agents are increasingly entrusted with natural-language workflow instructions (e.g., retail-payment policies) that specify not only what outcome to achieve, but also which steps, branches, and tool interactions are permitted. When these instructions are supplied as prompt context, however, the model retains control over both procedure selection and step execution. As interactions accumulate, an agent can skip required steps, take unsupported branches, or execute a valid step with unsupported arguments or effects--a failure mode we call workflow misalignment. In this work, we propose COVENANT, a compiler-and-interpreter architecture for workflow-aligned agent execution. Our key insight is to treat workflow instructions as source programs rather than prompts. COVENANT converts the instructions into a workflow abstract syntax tree (WAST) and lowers it to a workflow control-flow graph (WCFG). At runtime, a controller interprets the WCFG one node at a time, checks each proposal against requirements extracted from the instructions before committing controller state or advancing the graph, and returns diagnostic feedback for repair. To evaluate COVENANT, we use 120 cases from three existing benchmarks, spanning seven workflow scenarios. Compared with state-of-the-art LLM agents, COVENANT improves benchmark success from 50.00% to 83.33% and reduces the workflow-misalignment failure rate from 42.50% to 15.83% (62.75% relative). These results show that COVENANT substantially mitigates workflow misalignment, moving LLM-agent alignment beyond isolated prompt following toward reliable execution of complex and multi-step workflows.

</details>


### [48] [Context Assembly as the Controlled Variable: A Control-Theoretic View of Harness Policies for Frozen LLM Agents](https://arxiv.org/abs/2607.25408)
*Debjyoti Paul*

Main category: cs.AI

Comments: 6 pages, 2 figures, 1 table. Code and companion paper's data: https://github.com/dpaul0501/context-optimization-rl

Selection reason: 将上下文组装作为控制变量，从控制论角度优化LLM agent行为，可迁移至agent设计。

TL;DR: 提出将LLM智能体的上下文组装作为被控变量，通过外部在线学习策略控制，并给出稳定性和不确定性校准分析。


<details>
  <summary>Details</summary>
Motivation: 现有工作控制的是工具选择、消息路由或动作流，忽略了上下文组装本身作为被控变量的重要性，本文旨在填补这一空白。

Method: 将系统分解为内部冻结策略（π_θ）和外部上下文策略（π_φ），使用上下文赌博机或REINFORCE在线学习上下文组装参数，并基于非递减期望奖励进行稳定性论证和不确定性校准。

Result: 给出了控制理论所需的稳定性证据和不确定性校准分析，并在三个领域和两个模型上验证了方法可行性。

Conclusion: 通过将上下文组装作为被控变量，可以更有效地控制LLM智能体行为，并且该框架满足控制理论的稳定性和不确定性要求。

中文摘要：越来越多的2026年研究将控制理论应用于LLM智能体：使用李雅普诺夫稳定性的工具中介控制器（Prinos等人，2026，“Stable Agentic Control”），针对海量离散工具宇宙的稀疏策略的样本复杂度界（Majumdar，“Sparse Agentic Control”，2026），以及将多智能体系统分解为可审计反馈回路的调控控制分解（Nogueira和Skogestad，2026）。我们并不声称将控制理论引入LLM智能体——那艘船已经起航。我们更狭窄的声称是关于被控变量是什么。先前的工作控制工具选择、智能体间消息路由或智能体的原始动作流。我们反而将上下文组装本身——使用哪个提示模板、哪些少样本示例、检索多少上下文、进行多少次规划/验证迭代——作为被控变量，由位于冻结模型外部的上下文赌博机或REINFORCE策略在线学习。本文发展了形式化分解（内部冻结策略π_θ，外部上下文策略π_φ），给出了在线控制器在Zhang等人（2026）意义下的稳定性论证（在有界策略变化下非递减的期望奖励），并报告了控制器自身置信度与实现任务结果之间的不确定性校准分析。本文的应用部分在三个领域和两个模型提供商上实例化了相同的控制器，并发布了数据集、轨迹日志和部署配方；这里我们聚焦于形式化框架以及控制理论声明所需的稳定性/不确定性证据。

Original abstract: A growing body of 2026 work applies control theory to LLM agents: Lyapunov-certified stability for tool-mediated controllers (Prinos et al., "Stable Agentic Control", 2026), sample-complexity bounds for sparse policies over massive discrete tool universes (Majumdar, "Sparse Agentic Control", 2026), and regulatory-control decompositions of multi-agent systems into auditable feedback loops (Nogueira and Skogestad, 2026). We do not claim to introduce control theory to LLM agents -- that ship has sailed. Our narrower claim is about what the controlled variable is. Prior work controls tool selection, inter-agent message routing, or the agent's raw action stream. We instead treat context assembly itself -- which prompt template, which few-shot demonstrations, how much retrieved context, how many planning/verification passes -- as the controlled variable, learned online by a contextual bandit or REINFORCE policy sitting outside a frozen model. This paper develops the formal decomposition (inner frozen policy $π_θ$, outer context policy $π_φ$), gives a stability argument for the online controller in the sense used by Zhang et al. (2026) (non-decreasing expected reward under bounded policy change), and reports an uncertainty-calibration analysis of the controller's own confidence against realized task outcomes. The applied counterpart to this paper instantiates the same controller across three domains and two model providers and releases the dataset, trajectory logs, and a deployment recipe; here we focus on the formal framing and the stability/uncertainty evidence a control-theoretic claim requires.

</details>


### [49] [A Control System, a Dataset, and a Recipe for Making Frozen LLM Agents Learn a Domain](https://arxiv.org/abs/2607.25415)
*Debjyoti Paul*

Main category: cs.AI

Comments: 8 pages, 1 figure, 3 tables. Code and dataset: https://github.com/dpaul0501/context-optimization-rl

Selection reason: 使用强化学习在线优化harness策略，提升frozen LLM agent领域适应能力，属通用agent方法。

TL;DR: 提出一种将LLM代理的工具套件视为固定动作空间，通过强化学习在线学习策略的方法，在多个任务上验证并发布代码。


<details>
  <summary>Details</summary>
Motivation: 现有代理优化方法（如Meta-Harness、HyperAgents）需昂贵代码搜索或产生不可控自修改代码，无法审计且不适用于黑盒模型API，需要一种受限、可审计的在线优化方法。

Method: 将工具套件定义为小型固定人类可读动作空间，使用ε-贪婪上下文老虎机和REINFORCE进行在线强化学习，用多目标奖励（任务成功、验证器分数、策略合规性、成本、延迟、未支持声明惩罚）评分，以DSPy作为上下文组装器和基线。

Result: 在三个任务（工具使用工作流、HumanEval代码生成、HotpotQA多跳检索）和两个模型提供商（Ollama、AWS Bedrock）上评估，并发布代码、任务套件、训练日志及部署方案。

Conclusion: 该方法提供了一种可审计、可部署的LLM代理优化方案，通过约束强化学习在保持效率的同时避免不可控的自我修改。

中文摘要：生产级LLM代理越来越多地由一个冻结的模型包裹在一个工具套件中组装而成：一个提示模板、一个工具集、一个记忆/检索层、一个规划策略和一个验证策略。两个2026年的系统，Meta-Harness（Lee等人，2026）和HyperAgents（Meta AI，2026）表明，这个工具套件本身可以被一个代理提议者优化甚至自我重写——但代价是昂贵的代码搜索循环或不受约束的自我修改代码，这两者都无法审计，也无法与完全黑盒模型API一起使用。我们采取一个更狭窄、更受限制的立场：将工具套件视为一个小型、固定、人类可读的动作空间，并通过经典的样本高效强化学习（一个ε-贪婪上下文老虎机和REINFORCE）在线学习一个策略，根据多目标奖励（任务成功、验证器分数、策略合规性、成本、延迟和未支持声明惩罚）进行评分。我们使用DSPy（Khattab等人，2024）作为上下文组装器和最强非自适应基线（DSPy BootstrapFewShot静态提示）的来源来实例化这个控制系统，并在三个可验证任务领域——工具使用工作流、代码生成（HumanEval）和多跳检索问答（HotpotQA）——以及两个模型提供商（本地Ollama模型和AWS Bedrock）上进行评估。我们发布了工具套件控制系统代码、跨领域可验证任务套件、训练过程中的完整轨迹/奖励分解日志，以及一个与提供商无关的部署方案，用于将其应用于新组织的领域和验证设置。

Original abstract: Production LLM agents are increasingly assembled from a frozen model wrapped in a harness: a prompt template, a tool set, a memory/retrieval layer, a planning strategy, and a verification policy. Two 2026 systems, Meta-Harness (Lee et al., 2026) and HyperAgents (Meta AI, 2026), show that this harness can itself be optimized or even self-rewritten by an agentic proposer -- at the cost of either an expensive code-search loop or unconstrained self-modifying code, neither of which is auditable or usable with a fully black-box model API. We take a narrower, more constrained position: treat the harness as a small, fixed, human-legible action space and learn a policy over it online with classic sample-efficient reinforcement learning (an $ε$-greedy contextual bandit and REINFORCE), scored against a multi-objective reward (task success, verifier score, policy compliance, cost, latency, and an unsupported-claim penalty). We instantiate this control system with DSPy (Khattab et al., 2024) as both the context assembler and the source of the strongest non-adaptive baseline (a DSPy BootstrapFewShot static prompt), and evaluate it across three verifiable task domains -- tool-use workflows, code generation (HumanEval), and multi-hop retrieval QA (HotpotQA) -- and two model providers (a local Ollama model and AWS Bedrock). We release the harness-control-system code, the cross-domain verifiable task suite, the full trajectory/reward-decomposition logs from training, and a provider-agnostic deployment recipe for applying this to a new organization's domain and verification setup.

</details>


### [50] [Toward an Organizational Science of Multi-Agent LLM Systems: Decoupling Who, How, and Which Algorithm](https://arxiv.org/abs/2607.25446)
*Huan Chen,Xiang Song,Jian Jin,Pan Ren,Liang-Jie Zhang*

Main category: cs.AI

Comments: 8 pages, 2 figures

Selection reason: 解耦多agent系统的组织、协调与算法，提出可学习协议选择，对agent架构设计有直接启发。

TL;DR: 将组织、协调和协作分离为独立层，实现可配置的多智能体协作并学习最优协议。


<details>
  <summary>Details</summary>
Motivation: 现有LLM多智能体框架纠缠组织、协调和协作三个逻辑关注点，导致难以独立优化和比较。

Method: 提出IMACS，将三个关注点分离成正交可交换层，并通过自适应组织路由元协议学习协议选择。

Result: 自适应组织路由在质量-成本权衡下优于固定协议，消融实验显示问责制放置随模型系列变化。

Conclusion: 组织设计不能硬编码，必须针对每个模型绑定重新验证或学习。

中文摘要：基于大型语言模型（LLM）的多智能体框架通常纠缠三个逻辑上不同的关注点：团队组成（组织）、成员如何对齐（协调）以及哪种算法融合他们的工作（协作协议）。IMACS（智能多智能体协作系统）将这三个关注点分离为正交的、可独立交换的层。经典组织理论（贝尔宾角色、明茨伯格协调、RACI问责制）变成可执行的、经过验证的配置，框架将六种已发表的协作算法放在一个通用接口后面，同时将角色、协调和问责制暴露为独立可配置的因素。我们利用这种分离进行受控比较，其中组织分配变化而协作协议固定。它也将协议选择变成一个可以学习的变量：自适应组织路由（Adaptive Org Routing），一种上下文相关的元协议，在明确的质量-成本权衡下为每个任务选择一个协议，在受控研究中优于每个固定协议，并在实际基准测试和LLM评判奖励上进行在线训练。消融实验揭示了一种机制。当协议将交付物路由通过负责代理时，问责制放置会改变结果，并且获胜放置在不同模型系列间翻转，因此组织设计不能硬编码；对于每个模型绑定，必须重新验证或学习它。

Original abstract: Multi-agent frameworks built on large language models (LLMs) routinely entangle three logically distinct concerns: who is on the team (organization), how members align (coordination), and which algorithm fuses their work (collaboration protocol). IMACS (Intelligent Multi-Agent Collaboration System) separates the three into orthogonal, independently swappable layers. Classic organizational theory (Belbin roles, Mintzberg coordination, RACI accountability) becomes executable, validated configuration, and the framework places six published collaboration algorithms behind a common interface while exposing roles, coordination, and accountability as independently configurable factors. We use this separation to conduct controlled comparisons in which organizational assignments vary while the collaboration protocol is held fixed. It also turns protocol choice into a variable that can be learned: Adaptive Org Routing, a contextual-bandit meta-protocol, selects a protocol per task under an explicit quality-cost tradeoff, outperforms every fixed protocol in a controlled study, and trains online on real benchmark and LLM-judge rewards. The ablations expose a mechanism. Accountability placement changes outcomes exactly when the protocol routes the deliverable through the accountable agent, and the winning placement flips across model families, so organizational design cannot be hard-coded; it must be revalidated, or learned, for each model binding.

</details>


### [51] [Finding Optimal Cost-Bounded Plan Reductions: Refined Model](https://arxiv.org/abs/2607.25484)
*Martha Del Toro,Raquel Fuentetaja,Angel García-Olaya*

Main category: cs.AI

Comments: Extended version of a paper accepted at ICAPS 2026 (doi: 10.1609/icaps.v36i1.42849). Includes a new model and expanded experimental results

Selection reason: 研究成本约束下的计划缩减问题，属规划方法，可迁移至agent规划与资源管理。

TL;DR: 从预计算计划中提取满足预算约束且最大化效用的子计划，证明了NP完全性，提出了OSP和ILP两种精确方法，改进了ILP公式以减小模型大小并提高效率。


<details>
  <summary>Details</summary>
Motivation: 实际应用中计划可能因新预算约束而不可行，但必须使用原始动作和顺序，因此需要提取一个有效的子计划来最大化效用。

Method: 通过移除支持低效用目标的动作来缩减计划，同时保持可执行性和原始动作顺序；采用过度订阅规划（OSP）和整数线性规划（ILP）两种精确求解方法，并改进了ILP公式。

Result: 证明决策变体是NP完全的；提出的ILP公式显著减小了模型大小并提高了计算效率。

Conclusion: 所提出的两种精确方法能够有效从预计算计划中提取满足预算约束的最大效用子计划，改进后的ILP方法在效率上更具优势。

中文摘要：在某些实际应用中，一个计划可能由于新施加的预算约束而变得不可行，但同时，只能使用原始计划中的动作及其顺序是强制性的。本文研究从预先计算的计划中提取一个有效的子计划的问题，该子计划最大化效用同时遵守成本约束。每个目标都有一个效用值，通过移除支持低效用目标的动作来缩减计划，同时保持可执行性和原始动作顺序。我们展示决策变体是NP完全的，并提出两种精确方法来解决：一种是通过过度订阅规划（OSP），另一种是通过整数线性规划（ILP）。本文扩展了我们之前在ICAPS 2026上发表的工作（Del Toro, Fuentetaja, and García-Olaya 2026b）。虽然核心框架与之前相同，但我们进一步引入了精炼的ILP公式，显著减小了模型大小并提高了计算效率。

Original abstract: In some real applications a plan may later become unfeasible due to newly imposed budget constraints, yet, at the same time, using only the original actions of the plan and their order is mandatory. In this paper, we study the problem of extracting, from a precomputed plan, a valid subplan that maximizes utility while respecting a cost bound. Each goal is given a utility value and the plan is reduced by removing actions that support low-utility goals, while preserving both executability and the original action order. We show the decision variant is NP-complete and propose two exact methods to solve it: one via oversubscription planning (OSP) and another via Integer Linear Programming (ILP). This paper extends our previous work published at ICAPS 2026 (Del Toro, Fuentetaja, and García-Olaya 2026b). While the core framework remains as introduced there, we further introduce a refined ILP formulation that significantly decreases the model size and improves computational efficiency.

</details>


### [52] [PatientAgentBench: A Benchmark Framework for Evaluating Patient-Facing Health AI Agents](https://arxiv.org/abs/2607.25485)
*Korosh Vatanparvar,Ashutosh Joshi,Maria Xenochristou,Mohammad Abuzar Hashemi,Prasad Kasu,Deepak Bansal,Daniel Lopez-Martinez,Anchal Nema,Ramya Ganesan,Will Kimbrough,Alex Woody,Yadunandana Rao,Dilek Hakkani-Tur,Wilko Schulz-Mahlendorf*

Main category: cs.AI

Comments: Code: https://github.com/amazon-science/PatientAgentBench

Selection reason: 提出医疗agent评测框架，LLM-as-Jury评估方法可能对通用agent评测有启发。

TL;DR: 本文提出PatientAgentBench，一个面向患者的医疗保健智能体评估基准，通过模拟患者对话和LLM评审团评分，发现当前模型在临床安全性、分诊质量和工作流准确性方面存在差距，最强模型也仅得4.25/5分，表明静态基准不足。


<details>
  <summary>Details</summary>
Motivation: 医疗保健智能体正变得自主，但现有基准仅评估医学知识，无法检测患者交互中的临床风险，尤其是初级保健中的诊断错误和不安全行为。

Method: 构建PatientAgentBench：将基础模型封装为智能体，配备医疗工具沙盒，与模拟患者进行长期对话，由LLM评审团基于100多个临床标准在六个维度评分，并经临床医生验证。

Result: 10个模型在1200场景中表现差异显著：分诊质量通过率从32%到88%，最弱模型频繁失败，前沿模型在少数情况下出错（如未验证工具输出、遗漏危机资源），最强模型总体得分4.25/5。

Conclusion: 静态基准不足以评估自主医疗智能体，需要在真实患者记录和工具使用场景中进行测试；PatientAgentBench作为可重复的评估标准，有助于推动临床安全的提升。

中文摘要：健康人工智能正在从回答问题演变为能够与患者对话、推理健康记录并代表患者采取行动的智能体系统。初级医疗保健可防止诊断错误和不安全的护理；协助该领域的智能体应针对相同风险进行评估。当前基准侧重于医学知识，通过孤立的问答或面向临床医生的任务进行评估。PatientAgentBench对面向患者的智能体医疗保健进行基准测试；它评估一个基础模型，该模型被封装在一个具有医疗保健工具沙盒的智能体中，与模拟患者对话。每次对话由LLM作为评审团，通过一百多个与对话无关、基于临床医生的标准，在六个维度上进行评分。为了验证一致性，持证临床医生对共享对话进行了注释，结果评审团与专家评分者之间的相邻一致性为79-93%，达到或超过了临床医生间的评分一致性。我们在相同的1200个场景中对四个系列的10个模型进行了基准测试，并发现了临床差距。分诊质量是最具区分度的维度：通过率从最弱模型的32%上升到最强模型的88%，而智能体往往在没有临床筛查的情况下执行管理请求。临床安全性和工作流准确性遵循相同模式：最弱模型经常失败，编造未执行的操作，而前沿模型仅在1-3%的案例中失败，原因包括未验证的工具输出和紧急情况下遗漏危机资源。更强大的模型缩小了这些差距，但并未完全消除；最强模型总体得分仅为4.25分（满分5分）。这些失败仅出现在持续使用工具的对话中，并针对真实患者记录，这证实了随着医疗保健智能体系统获得自主性，静态基准测试是不够的。我们发布该框架作为一个可重复的、经临床医生验证的评估标准，以帮助该领域缩小这一差距。

Original abstract: Health AI is evolving from answering questions to agentic systems that converse with patients, reason about health records, and act on their behalf. Primary care guards against diagnostic errors and unsafe care; agents assisting in this domain warrant evaluation against the same risks. Current benchmarks focus on medical knowledge, assessed through isolated question-answering or clinician-facing tasks. PatientAgentBench benchmarks patient-facing agentic healthcare; it evaluates a foundation model, wrapped in an agent with a sandbox of healthcare tools, conversing with a simulated patient. Each conversation is scored by an LLM-as-a-Jury across six dimensions via over a hundred conversation-agnostic, clinician-grounded criteria. To validate alignment, licensed clinicians annotated shared conversations, yielding 79-93% adjacent agreement between jury and expert raters, on par with or exceeding clinician inter-rater agreement. We benchmarked 10 models across four families on the same 1,200 scenarios and found clinical gaps. Triage quality is the most discriminating dimension: pass rates rise from 32% for the weakest models to 88% for the strongest, with agents often acting on administrative requests without clinical screening. Clinical safety and workflow accuracy follow the same pattern: the weakest models fail often, fabricating unexecuted actions, while frontier models fail on only 1-3% of cases, from unverified tool outputs and omitted crisis resources in an emergency. More capable models narrow these gaps but do not close them; the strongest scores only 4.25 of 5 overall. These failures surface only in sustained, tool-using conversations against realistic patient records, confirming that static benchmarks are insufficient as healthcare agentic systems gain autonomy. We release the framework as a reproducible, clinician-validated evaluation standard to help the field close this gap.

</details>


### [53] [CoTinyVLA: Chain-of-Thought Distillation for a Sub-Billion-Parameter Vision-Language-Action Model](https://arxiv.org/abs/2607.25487)
*Minhyeok Lee,Chiyoung Kim,Chanhoe Gu,Seongrok Kim,Sanghyuk Roy Choi,Donghwan Hwang,Donghun Ryu,Seokhyun Kim*

Main category: cs.AI

Comments: 22 pages, 2 figures, 20 tables. Code at https://github.com/BrainJellyPie/CoTinyVLA

Selection reason: 通过链式思维蒸馏获得高效VLA模型，结构化监督方法可迁移至agent训练。

TL;DR: CoTinyVLA是一个0.9B参数的VLA模型，通过结构化监督（双视角输入、层次化思维链蒸馏和释义增强）在LIBERO-Plus上超越7B模型，峰值内存仅2.25 GiB。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型参数庞大（3B-7B），内存需求超出嵌入式机器人预算，需要更轻量的鲁棒模型。

Method: 三个组件：1）双视角时间输入，每步16历史帧，含文本相机和时间标记；2）层次化CoT蒸馏，从35B教师蒸馏到情节级计划和块级思考；3）释义增强，40基础命令扩至800变体。

Result: LIBERO-Plus上，空间90.8%、物体87.3%、目标86.6%、长时80.7%，全面领先7B基线；困难任务（如机器人初始状态）提升显著，峰值内存2.25 GiB。

Conclusion: 结构化监督使得0.9B骨干网络超越所有大模型，表明精心设计的监督信号可替代模型规模扩大。

中文摘要：视觉-语言-动作（VLA）模型将自然语言命令转化为机器人动作序列，但在LIBERO-Plus鲁棒性基准测试中，领先系统使用三十亿到七十亿参数的骨干网络，其内存需求可能超出嵌入式机器人预算。我们提出CoTinyVLA，这是一个基于Qwen3.5-0.8B骨干网络的0.9B参数动作模型，通过结构化监督而非扩大模型来获得鲁棒性。三个组件针对问题的不同方面：双视角时间输入，每步16个历史帧，带有文本相机和时间标记；层次化思维链（CoT）蒸馏，从35B教师模型蒸馏到情节级计划和块级思考跨度，涵盖任务阶段、夹爪状态和下一步子动作；以及释义增强，将40个基本命令扩展到800个变体。在LIBERO-Plus上，涵盖七个扰动维度的10,030个扰动任务，CoTinyVLA在空间任务上达到90.8%，物体任务87.3%，目标任务86.6%，长时间任务80.7%，在所有四个套件上领先最强的7B基线分别4.7、2.8、15.9和3.0个百分点，每个置信区间均不包含零。增益集中在基准测试最困难的维度上：在已发布的十一个基线中，没有任何套件在机器人初始状态上超过53.2%，而CoTinyVLA在目标任务上达到73.6%，最强基线为39.9%。消融实验显示三个组件可按扰动轴分离，在匹配图像预算下，帧在两个相机和时间之间的分配本身就贡献了8.6个百分点。闭环推理峰值GPU内存分配为2.25 GiB，配对干预显示情节计划是关键：用空或矛盾的跨度替换它会导致成功点下降40至45点。因此，结构化监督让0.9B骨干网络超越了所有模型。代码：https://github.com/BrainJellyPie/CoTinyVLA

Original abstract: Vision-Language-Action (VLA) models translate natural-language commands into robot action sequences, but leading systems on the LIBERO-Plus robustness benchmark use three- to seven-billion-parameter backbones whose memory demands can exceed embedded robotic budgets. We present CoTinyVLA, a 0.9B-parameter action model on a Qwen3.5-0.8B backbone that obtains that robustness by structuring supervision instead of enlarging the model. Three components target different axes of the problem: dual-view temporal input of 16 history frames per step with textual camera and time markers; hierarchical chain-of-thought (CoT) distillation from a 35B teacher into an episode-level Plan and a chunk-level Think span over task phase, gripper state and next subaction; and paraphrase augmentation expanding 40 base commands into 800 variants. On LIBERO-Plus, spanning 10,030 perturbed tasks across seven perturbation dimensions, CoTinyVLA reaches 90.8% on Spatial, 87.3% on Object, 86.6% on Goal and 80.7% on Long, leading the strongest 7B baseline on all four suites by 4.7, 2.8, 15.9 and 3.0 points, with every margin interval excluding zero. The gains concentrate on the hardest axes of the benchmark: across the eleven published baselines none exceeds 53.2% on Robot Initial States in any suite, whereas CoTinyVLA reaches 73.6% on Goal against 39.9% for the strongest baseline. Ablations show the three components to be separable by perturbation axis, and at a matched image budget how frames are divided between the two cameras and across time accounts for 8.6 points on its own. Closed-loop inference peaks at 2.25 GiB of allocated GPU memory, and paired interventions show the episode Plan to be load-bearing: replacing it with an empty or contradictory span costs 40 to 45 points of success. Structured supervision thus lets a 0.9B backbone exceed all of them. Code: https://github.com/BrainJellyPie/CoTinyVLA

</details>


### [54] [Entangled by Design: Spurious Intra-Variable Signal Routing in Tabular In-Context Learners](https://arxiv.org/abs/2607.25532)
*Athanasios Vlontzos,Giorgos Papanastasiou,Bernhard Kainz,Sotirios Tsaftaris*

Main category: cs.AI

Selection reason: 研究上下文学习中的虚假路由，对agent中ICL的使用有潜在启发。

TL;DR: 本文形式化了上下文学习中模型利用虚假信号进行预测的问题，证明其不可避免，并提出了两种轻量级缓解方法，能够有效将预测重新路由到因果信号。


<details>
  <summary>Details</summary>
Motivation: 模型在单一环境训练后部署到新环境时，由于虚假信号与结果的相关性而做出错误预测，现有研究缺乏对这种虚假路由机制的形式化理解。

Method: 通过形式化复合表示中的虚假路由，分析线性岭ICL的闭式解，并在TabPFN上验证；提出环境分层上下文构建和S交换增强两种轻量级缓解方法。

Result: 推导出虚假路由程度CSR与信号比率ρ_S/ρ_C成比例的闭式公式（线性ICL r=0.997，TabPFN r=0.979），发现更大上下文会放大虚假路由（最多1.74倍），且S交换缓解效果显著（线性ICL减少74%，TabPFN减少98.8%，因果敏感性增加8.4倍）。

Conclusion: 虚假路由在ICL中不可避免但可通过弱环境标签缓解，S交换能有效使模型重新聚焦于因果信号，而非简单地变得对虚假信号不敏感。

中文摘要：考虑一个在单一医院训练的预测患者康复的模型，其中测量的特征X将患者的真实健康信号(C)与来自该医院设备的系统性伪影(S)捆绑在一起。在该医院内，伪影通过未测量的混杂因素（如患者人口统计）与结果相关；一个上下文中学习者理性地通过S（而非C）路由预测，并在部署到具有不同设备的新医院时默默失败。我们将此形式化为复合表示中的虚假路由：当特征X = [C; αS; η]在不同于空间中编码因果信号C和虚假信号S时，ICL无法确定哪个驱动预测。我们证明，在岭ICL（一种线性上下文中学习器）下，无论上下文大小如何，这种路由都是不可避免的；TabPFN（一种最先进的预训练表格ICL模型）在经验上显示定性一致的行为。我们推导出一个闭式特征描述，CSR ∝ ρ_S/ρ_C，线性ICL的r=0.997，TabPFN的r=0.979证实。与直觉相反，更大的上下文会加强对主导上下文信号的承诺，将虚假路由放大至多1.74倍；在高虚假角，更具表达力的模型在经验上显示更大的脆弱性（高纠缠下CSR差距+2.22）。我们引入了两种轻量级缓解措施：环境分层上下文构建和S交换增强，只需要弱环境标签，不需要因果分区的知识。S交换将线性ICL的虚假路由减少74%，TabPFN减少98.8%，同时TabPFN的因果敏感性增加8.4倍：模型并非变得不可知，而是重新路由到因果信号。

Original abstract: Consider a model trained at a single hospital to predict patient recovery, where the measured feature $X$ bundles the patient's true health signal ($C$) with a systematic artefact from that hospital's equipment ($S$). Within that hospital, the artefact correlates with outcomes through unmeasured confounders such as patient demographics; an in-context learner rationally routes predictions through $S$, not $C$, and fails silently when deployed at a new hospital with different equipment. We formalise this as \emph{spurious routing in composite representations}: when a feature $X = [C;\,αS;\,η]$ encodes a causal signal $C$ and a spurious signal $S$ in distinct subspaces, the ICL cannot determine which drives predictions. We prove that under ridge ICL, a linear in-context learner, this routing is unavoidable regardless of context size; TabPFN, a state-of-the-art pretrained tabular ICL model, shows qualitatively consistent behaviour empirically. We derive a closed-form characterisation, $\mathrm{CSR} \propto ρ_S/ρ_C$, confirmed at $r = 0.997$ for linear ICL and $r = 0.979$ for TabPFN. Contrary to intuition, larger context sharpens commitment to the dominant in-context signal, amplifying spurious routing by up to $1.74\times$; in the high-spurious corner, more expressive models show greater vulnerability empirically ($+2.22$ CSR gap at high entanglement). We introduce two lightweight mitigations: environment-stratified context construction and S-swap augmentation, that require only weak environment labels and no knowledge of the causal partition. S-swap reduces spurious routing by $74\%$ for linear ICL and $98.8\%$ for TabPFN, with TabPFN's causal sensitivity increasing $8.4\times$ simultaneously: the model does not become agnostic, it reroutes through the causal signal.

</details>


### [55] [Distilling Temporal Search and Reasoning: Evolving LLMs for Future Prediction via Harness-Assisted Efficient Data Synthesis](https://arxiv.org/abs/2607.25554)
*Wanxu Cai,Zhengyu Chen,Huaisheng Zhu,Wei Wang,Jingang Wang,Qiang Xu*

Main category: cs.AI

Selection reason: 使用harness辅助数据合成提升LLM时间预测能力，方法可迁移至agent数据合成与蒸馏。

TL;DR: 提出时间截断框架，通过在推理时强制时间截止，减少时间泄漏，提高采样效率，并构建大规模语料库，使LLM更好地进行未来事件预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法在去除外部框架后预测能力消失，且历史数据存在时间泄漏导致预测退化为检索，采样效率低。

Method: 提出时间截断框架，对每个回合强制时间截止，支持从历史事件采样；构建大规模语料库和过程度量。

Result: 新框架提高了采样效率，增加了高质量数据比例，蒸馏实验中学生模型性能最佳。

Conclusion: 时间截断框架有效提升模型事件预测能力，并促进模型参数进化。

中文摘要：未来事件预测具有广泛的社会影响，但仍然具有挑战性。最先进的方法通过外部代理框架增强大型语言模型，但一旦移除该框架，其预测能力就会消失。虽然最近的工具集成推理（TIR）将深度搜索内化用于多跳事实检索，但预测进一步需要时间搜索和对历史趋势及动态变化进行推理。关键障碍在于数据：历史查询会导致时间泄漏，从而将预测退化为检索。先前的工作要么冻结信息收集并采用静态观察，要么依赖拒绝采样或未解决的新查询，这会丢弃大量数据，降低合成效率。我们提出一种时间截断框架，在每个回合强制执行时间截止，从而能够从历史事件中进行TIR风格的采样，减少时间泄漏和对拒绝采样或未解决查询的依赖，提高采样效率。我们还构建了一个大规模语料库和一个基于过程的度量，并表明我们的框架自然导致更广泛的时间搜索广度，提高了高质量数据的比例，进一步提高了效率并减少了对复杂评估标准的依赖。蒸馏实验表明，在框架干预数据上训练的学生模型取得了最佳性能，展示了框架辅助的模型演化，将更高质量的时间搜索和推理数据转化为学生模型的参数进步。

Original abstract: Future event prediction carries broad social impact yet remains challenging. SOTA approaches augment LLMs with external agent frameworks whose predictive capability vanishes once the harness is removed. While recent Tool-Integrated Reasoning (TIR) internalizes deep search for multi-hop retrieval of facts, forecasting further demands temporal search and reasoning over historical trends and dynamic shifts. The key obstacle is data: historical queries induce temporal leakage that degrades forecasting into retrieval. Prior works either freeze information gathering with static observations, or rely on rejection sampling or unresolved fresh queries that discard vast amounts of data, degrading synthesis efficiency. We propose a time-truncation harness that enforces a temporal cut-off at every turn, enabling TIR-style sampling from historical events, reducing temporal leakage and reliance of rejection sampling or unsolved queries, increasing the sampling efficiency. We further build a large-scale corpus and a process-based metric and show that our harness naturally induces a broader temporal breadth of search and raises the proportion of high-quality data, further increasing the efficiency and reducing the reliance on complex rubrics. Distillation experiments show that students trained on harness-intervened data achieve the best performance, demonstrating harness-assisted model evolving that turns higher quality temporal search and reasoning data into a parametric advancement of the students.

</details>


### [56] [Agent Skills Matter: Inferring Proprietary Skills from Execution Trajectories](https://arxiv.org/abs/2607.25560)
*Jianing Geng,Ruiqi He,Zekun Fei,Biao Yi,Ruijie Wang,Zheli Liu,Xia Hu,Xuansheng Wu,Qingkai Zeng*

Main category: cs.AI

Comments: 18 pages, 6 figures, 2 tables

Selection reason: 从执行轨迹推断agent技能，暴露行为侧信道，对agent安全与技能保护有启发。

TL;DR: 提出技能泄露概念和SigLeak框架，证明通过Agent执行轨迹可重建专有技能，威胁技能隐私。


<details>
  <summary>Details</summary>
Motivation: 隐藏技能工件不能掩盖其行为效应，执行轨迹形成侧信道，导致专有技能可能被泄露。

Method: SigLeak利用行为中的技能签名，构建诊断任务，对比启用/禁用技能轨迹，迭代优化重建技能。

Result: 在多个场景和模型中，SigLeak成功率平均提升6.88%，SkillSim指标最高，优于基线。

Conclusion: 良性执行轨迹可暴露专有程序知识，需警惕技能泄露风险。

中文摘要：Agent技能包封装了可重复使用的程序，可提升下游性能。其轻量、可移植的形式使得市场变现和私有部署在云托管Agent界面后成为可能，从而激励提供商保持高价值技能的专有性。然而，隐藏这些工件并不能掩盖它们的行为影响，这些影响在执行轨迹中仍然可观察，并形成行为侧信道。我们将这种暴露定义为技能泄露：通过良性查询引发的轨迹重建专有技能，无需参考答案或成功标签。我们提出SigLeak，一种黑盒框架，利用Agent行为中重复出现的技能签名。它构建多样化、决策丰富的诊断任务，对比匹配的技能启用和技能禁用轨迹，并从隔离模式中迭代优化重建的技能。在五个场景、三个模型家族和三个Agent框架中，SigLeak在几乎所有设置中优于或匹配三个基线。它将成功率平均提高了6.88个百分点（相对于技能禁用参考），并实现了最高的整体SkillSim（我们的粗粒度和细粒度语义相似性指标）。这些结果表明，良性执行轨迹可以暴露专有程序知识。代码可在https://anonymous.4open.science/r/SigLeak-D1DB获取。

Original abstract: Agent skills package reusable procedures that improve downstream performance. Their lightweight, portable form enables marketplace monetization and private deployment behind cloud-hosted agent interfaces, giving providers incentives to keep high-value skills proprietary. Yet hiding the artifacts does not conceal their behavioral effects, which remain observable in execution trajectories and form a behavioral side channel. We define this exposure as Skill Leakage: reconstructing proprietary skills from trajectories elicited by benign queries, without reference answers or success labels. We introduce SigLeak, a black-box framework that exploits recurring skill signatures in agent behavior. It constructs diverse, decision-rich diagnostic tasks, contrasts matched skill-enabled and skill-disabled trajectories, and iteratively refines a reconstructed skill from the isolated patterns. Across five scenarios, three model families, and three agent frameworks, SigLeak outperforms or matches three baselines in nearly every setting. It raises the success rate by 6.88 percentage points over the skill-disabled reference on average and achieves the highest overall SkillSim, our metric for coarse- and fine-grained semantic similarity. These results show that benign execution trajectories can expose proprietary procedural knowledge. The code is available at https://anonymous.4open.science/r/SigLeak-D1DB.

</details>


### [57] [OrchBench: Evaluating Multi-Agent Orchestration Plans in Isolation via Deterministic Simulation](https://arxiv.org/abs/2607.25656)
*Zhenzhen Ren,Jiyan He,Xinpeng Zhang,Zhenxing Qian,Ke Han,Shuxin Zheng,GuoBiao Li,Xiaoqing Zhang*

Main category: cs.AI

Selection reason: 提出多agent编排计划隔离评估的仿真benchmark，可迁移至coding agent orchestration。

TL;DR: OrchBench是一个基于模拟的基准测试，通过DAG表示任务依赖，用确定性模拟器评估多智能体编排计划，与真实执行高度相关且成本极低，揭示了保留关键信息比增加智能体数量更重要。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统评估依赖端到端执行，混淆了编排计划质量与外部因素，且成本高昂，因此需要一种隔离、高效的编排计划评估方法。

Method: 从现实任务构建DAG，给定DAG、智能体上下文限制和预算，规划器分配子任务并指定信息传递；确定性模拟器评估计划，输出质量、完工时间和令牌成本。

Result: OrchBench模拟分数与Claude Code执行质量分数Pearson相关系数r=0.816，仅需1.3%令牌和10.3%时间；实验表明保留关键信息比增加智能体数量更重要，并行性收益随协调失败递减。

Conclusion: OrchBench是一个高效、可解释的基准测试，用于比较和诊断多智能体编排计划，有助于指导系统设计。

中文摘要：复杂任务通常分解为可并行但相互依赖的子任务，使得编排对多智能体系统（MAS）的性能至关重要。现有评估通常依赖于端到端的执行，这混淆了编排计划质量与工作能力、工具可靠性和环境噪声。此外，真实执行的时间和令牌成本随着工作流规模快速增长，使得系统评估成本高昂。我们提出OrchBench，一个基于模拟的基准测试，用于隔离评估多智能体编排计划。从现实世界任务出发，OrchBench构建了有向无环图（DAG），编码任务依赖关系，具有可控的规模和并行度。给定DAG、每个智能体的上下文限制和智能体预算，被评估的规划器将子任务分配给智能体，并指定跨智能体的信息传递及其保留比例。确定性模拟器评估生成的计划，无需调用工作智能体，并返回结果质量、完工时间和令牌成本的可解释度量。OrchBench产生的模拟分数与Claude Code执行的质量分数高度相关，达到Pearson相关系数r=0.816，而仅需1.3%的令牌和10.3%的墙钟时间。在不同的规划器和工作流规模中，我们发现保留任务关键信息比简单增加智能体数量更重要，并且随着协调失败累积，并行性的收益递减。这些结果确立了OrchBench作为一个高效且可解释的基准测试，用于比较和诊断多智能体编排计划。

Original abstract: Complex tasks often decompose into parallelizable yet interdependent subtasks, making orchestration critical to the performance of multi-agent systems (MAS). Existing evaluations typically rely on end-to-end execution, which conflates orchestration-plan quality with worker capabilities, tool reliability, and environmental noise. Moreover, the time and token costs of real execution grow rapidly with workflow scale, making systematic evaluation expensive. We present OrchBench, a simulation-based benchmark for evaluating multi-agent orchestration plans in isolation. Starting from real-world tasks, OrchBench constructs directed acyclic graphs (DAGs) that encode task dependencies, with controlled sizes and degrees of parallelism. Given a DAG, a per-agent context limit, and an agent budget, the evaluated planner assigns subtasks to agents and specifies cross-agent information transfers and their retention ratios. A deterministic simulator evaluates the resulting plan without invoking worker agents and returns interpretable measures of result quality, makespan, and token cost. The simulated scores produced by OrchBench correlate strongly with quality scores from Claude Code executions, achieving a Pearson correlation of \(r=0.816\), while requiring only \(1.3\%\) of the tokens and \(10.3\%\) of the wall-clock time. Across diverse planners and workflow scales, we find that preserving task-critical information is more important than simply increasing the number of agents, and the benefits of parallelism diminish as coordination failures accumulate. These results establish OrchBench as an efficient and interpretable benchmark for comparing and diagnosing multi-agent orchestration plans.

</details>


### [58] [DecoEvo: Score-Decoupled Co-Evolution of Solver and Rubric-Generator Skills in Text Space](https://arxiv.org/abs/2607.25675)
*Jiangwang Chen,Zixin Song,Junlin Liu,Shuaiyu Zhou,Haiyan Wu,Haihan Shi,Chenxi Zhou,Hanqing Li,Xiao Yang,Da Zhu,Guanjun Jiang,Hai Wan,Xibin Zhao*

Main category: cs.AI

Selection reason: 文本空间协同进化解算器与评分器技能，可迁移至agent自我改进。

TL;DR: DecoEvo通过解耦共同进化求解器和规则生成器，在文本空间优化中实现无需黄金规则的自适应改进，显著提升开放任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本空间优化方法在开放任务中因评估固定而无法捕捉未衡量的维度，且仅依赖求解器得分更新规则不可靠，容易导致表面进步。

Method: DecoEvo采用解耦共同进化，求解器技能基于标准级反馈更新，规则生成器技能通过独立于聚合求解器得分的互补审计（需求覆盖和响应判别性）修正，专注新弱点。

Result: 在五个基准和三个LLM骨干网络上，DecoEvo优于所有对比方法，相对SkillOpt平均提升2.8-5.0%。

Conclusion: DecoEvo通过解耦目标有效避免了规则退化，实现了更可靠的文本空间优化，适用于开放式任务。

中文摘要：文本空间优化通过编辑外部自然语言工件而非模型权重来适应大型语言模型（LLMs），因此优化后的工件保持可检查性，且模型可被视为黑箱。然而，现有的大多数文本空间方法保持评估固定。在开放式任务中，这可能成为瓶颈：一旦求解器在规则（rubric）衡量的标准上有所改进，遗漏的维度对优化信号仍然不可见。仅根据当前求解器的得分来更新规则也不可靠，因为表面上的进步可能来自使规则更容易满足。我们引入了DecoEvo（解耦共同进化），它在优化过程中不使用黄金规则，而是在解耦的目标下共同进化求解器技能和规则生成器技能。求解器技能使用标准级别的反馈进行更新，而规则生成器技能则通过对需求覆盖和响应判别性的互补审计进行修正，这些审计独立于聚合的求解器得分。这种分离将生成器更新聚焦于新暴露的求解器弱点，减少了对求解器已满足标准的重复强调。在每个基准的官方评估下，DecoEvo在五个基准和三个LLM骨干网络上优于所有比较方法，在五个基准的平均值上比SkillOpt取得了2.8-5.0%的相对提升。

Original abstract: Text-space optimization adapts large language models (LLMs) by editing external natural-language artifacts rather than model weights, so the optimized artifacts remain inspectable and the model can be treated as a black box. However, most existing text-space methods keep evaluation fixed. On open-ended tasks, this can become a bottleneck: once the solver improves on the criteria a rubric measures, omitted dimensions remain invisible to the optimization signal. Simply evolving the rubric is also unreliable when updates are selected by the current solver's score, because apparent progress can come from making the rubric easier to satisfy. We introduce DecoEvo (Decoupled Co-Evolution), which co-evolves a solver skill and a rubric-generator skill under decoupled objectives without using gold rubrics during optimization. The solver skill is updated using criterion-level feedback, while the rubric-generator skill is revised through complementary audits of requirement coverage and response discrimination that are independent of aggregate solver score. This separation focuses generator updates on newly exposed solver weaknesses, reducing repeated emphasis on criteria the solver already satisfies. Under each benchmark's official evaluation, DecoEvo outperforms all compared methods across five benchmarks and three LLM backbones, yielding 2.8--5.0\% relative gains over SkillOpt in the five-benchmark average.

</details>


### [59] [Speculate While You Reason: Teaching Agents to Predict Their Next Tool Call via Joint Agent-Speculator RL](https://arxiv.org/abs/2607.25816)
*Jiabao Ji,Yujian Liu,Li An,Rohit Jain,Gungor Polatkan,Siyu Zhu,Shiyu Chang*

Main category: cs.AI

Selection reason: 提出自推测agent联合训练方法直接加速工具调用，与coding agent强相关。

TL;DR: 提出自我推测代理，将代理和推测器统一为同一模型，通过联合强化学习提升工具调用预测准确率，保持任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有推测器与代理行为对齐不良，且通常分离设计；目标代理本身是强推测器，因此统一模型可简化设计并提高性能。

Method: 提出联合代理-推测器强化学习方法，从代理 rollout 推导推测目标，交替进行代理和推测器更新，实现双模式代理。

Result: 在代理式搜索QA和对话式工具使用任务中，Qwen3-4B的下一工具调用Hit@1从44.1提升至61.2，Qwen3.5-4B从48.9提升至66.3，且任务成功率保持不变。

Conclusion: 自我推测代理有效利用了代理自身作为推测器，通过联合训练显著提升推测准确率，同时不损害任务性能。

中文摘要：大型语言模型代理在等待工具调用结果时通常花费大量实际时间。工具调用推测可以通过预测并预执行代理的下一个工具调用来隐藏这种延迟，前提是预测与代理最终的工具调用匹配，但现有的推测器通常是单独的草稿模型或缓存轨迹，与部署代理自身的行为对齐不良。我们识别出这种推测器-代理差距，并表明目标代理本身是一个强大的下一调用推测器。这指向了一个更简单的设计：将代理和推测器统一到同一个模型中。在本文中，我们引入了自我推测代理，一个单一的模型，既在代理模式下解决问题，又在推测模式下从部分轨迹预测其下一个工具调用，完全重用前缀KV缓存。为了实现这种双模式代理而不降低性能，我们提出了一种联合代理-推测器强化学习方法，该方法从代理自身的 rollout 中推导推测目标，并交替进行代理和推测器更新。在代理式搜索问答和对话式工具使用代理任务中，我们的方法将 Qwen3-4B 的平均下一工具调用 Hit@1 从 44.1 提高到 61.2，将 Qwen3.5-4B 从 48.9 提高到 66.3，同时保持了代理任务的成功率。

Original abstract: Large language model agents often spend substantial wall-clock time waiting for tool call results. Tool-call speculation can hide this latency by predicting and pre-executing an agent's next tool call if the prediction matches the agent's eventual tool call, but existing speculators are typically separate draft models or cached traces that are poorly aligned with the deployed agent's own behavior. We identify this speculator-agent gap and show that the target agent itself is a strong next-call speculator. This points to a simpler design: unifying the agent and speculator within the same model. In this paper, we introduce the self-speculating agent, a single model that both solves tasks in agent mode and predicts its next tool call from partial trajectories in speculator mode, fully reusing prefix KV cache. To enable this dual-mode agent without degrading performance, we propose a joint agent-speculator reinforcement learning method, which derives speculation targets from the agent's own rollouts and alternates agent and speculator updates. Across agentic search QA and conversational tool-use agentic tasks, our method improves average next tool-call Hit@1 from 44.1 to 61.2 for Qwen3-4B and from 48.9 to 66.3 for Qwen3.5-4B, while preserving agent task success.

</details>


### [60] [HiSkill: Empowering LLM Agents with Hierarchical Skill Graphs](https://arxiv.org/abs/2607.25853)
*Yu Hao,Jinxuan Cai,Qi Zhang,Yawen Li,Zhiqiang Zhang,Chuan Shi,Cheng Yang*

Main category: cs.AI

Selection reason: 提出分层技能图框架增强LLM agent长程任务执行与规划，直接相关。

TL;DR: HiSkill通过构建层次化技能图，桥接高级技能与可执行动作，在交互任务中提升性能并降低推理消耗。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹到技能方法忽略技能间关系，且高级技能与可执行动作存在断层。

Method: 提出HiSkill，构建含技能节点、原子操作节点及多种关系边的有向图，推理时检索子图指导LLM代理执行任务。

Result: 在三个交互环境上优于基线，减少推理令牌消耗。

Conclusion: 层次化技能图有效桥接高级技能与动作基础，提升LLM代理的长程任务执行效率。

中文摘要：技能已成为一种重要的抽象，使大型语言模型（LLM）代理能够在长期交互任务中重用过去的经验。然而，现有的轨迹到技能方法通常会产生扁平的、独立存储和检索的高级文本技能集合，导致技能之间的关系未被充分利用，并且高级技能与可执行动作之间存在差距。本文提出了一种层次化技能图框架HiSkill，它将交互轨迹组织成一个有向图，包含技能节点、原子操作节点和类型化边。具体来说，该图将可重用的高级技能与可执行的动作模板联系起来，同时捕获它们之间的分解、时间转换、兼容性、支持和恢复关系。在推理时，HiSkill检索紧凑的任务相关子图，并执行子图引导的任务执行，其中符号任务状态、活跃技能和检索到的子图引导LLM代理迭代地切换技能、选择原子操作并执行可执行动作。在三个交互环境上的实验表明，HiSkill优于最先进的基线，同时减少了推理令牌消耗，证明了通过层次化技能图桥接高级技能和可执行动作基础的有效性。我们的数据和代码可在https://github.com/BUPT-GAMMA/HiSkill获取。

Original abstract: Skills have become an important abstraction for enabling large language model (LLM) agents to reuse past experience in long-horizon interactive tasks. However, existing trajectory-to-skill methods often produce flat collections of high-level textual skills that are stored and retrieved independently, leaving skill relations underutilized and maintaining a gap between high-level skills and executable actions. In this paper, we propose HiSkill, a hierarchical skill graph framework that organizes interaction trajectories into a directed graph with skill nodes, AtomicOp nodes, and typed edges. Specifically, the graph connects reusable high-level skills with executable action templates, while also capturing decomposition, temporal transition, compatibility, support, and recovery relations among them. At inference time, HiSkill retrieves a compact task-relevant subgraph and performs subgraph-guided task execution, where a symbolic task state, an active skill, and the retrieved subgraph guide the LLM agent to switch skills, select AtomicOps, and ground executable actions iteratively. Experiments on three interactive environments show that HiSkill outperforms state-of-the-art baselines while reducing inference token consumption, demonstrating the effectiveness of bridging high-level skills and executable action grounding through a hierarchical skill graph. Our data and code is available at https://github.com/BUPT-GAMMA/HiSkill.

</details>


### [61] [Runtime Uncertainty Monitoring for LLM-Based Multi-Agent Systems Using Bayesian Networks](https://arxiv.org/abs/2607.25877)
*Bart Custers,Koorosh Aslansefat*

Main category: cs.AI

Comments: This paper got accepted for Ninth International Workshop on Artificial Intelligence Safety Engineering (WAISE 2026): https://www.waise.org/

Selection reason: 提出贝叶斯网络对多agent系统进行运行时不确定性监控，可迁移至agent可靠性。

TL;DR: 提出基于LLM的多智能体精算风险建模框架，通过令牌对数概率和贝叶斯网络量化不确定性，可复现基线性能并提供稳定性洞察。


<details>
  <summary>Details</summary>
Motivation: 精算工作流是高风险的决策支持环境，LLM的不确定性可能导致错误评估和不公平定价，现有方法缺乏对多智能体交互中不确定性的系统量化。

Method: 构建多智能体框架，包括数据准备、建模、审查和解释智能体；采用令牌级对数概率经长度归一化后转换为置信度估计，再通过贝叶斯网络传播不确定性。

Result: 框架能够复现传统精算模型的性能，同时额外提供工作流稳定性和运行时不确定性传播的分析。

Conclusion: 所提方法有效支持精算模型中的不确定性量化，增强了决策可靠性与可解释性。

中文摘要：本文研究了基于大语言模型（LLM）的多智能体系统（MAS）如何支持精算风险建模，特别关注不确定性量化。精算工作流代表了一种高风险决策支持环境，其中不可靠的输出可能导致错误的风险评估、不公平定价和监管不合规。为了解决LLM的概率性质以及智能体之间依赖关系所引入的不确定性，本文提出了一种多智能体框架，在该框架中，专门化的智能体在中心枢纽下执行数据准备、建模、审查和解释任务。主要贡献是一种使用令牌级对数概率和贝叶斯网络的不确定性传播新方法。重要的是，对数概率不被视为正确性或任务成功的直接概率。相反，长度归一化的对数概率摘要被转换为校准的任务级置信度估计，然后纳入贝叶斯网络。结果表明，该框架再现了基准精算性能，同时提供了对工作流稳定性和运行时不确定性传播的额外洞察。

Original abstract: This paper investigates how multi-agent systems (MAS)-based on large language models (LLMs) can support actuarial risk modelling, with a particular focus on uncertainty quantification. Actuarial workflows represent a high-stakes decision-support setting where unreliable outputs may lead to incorrect risk assessment, unfair pricing, and regulatory non-compliance. To address uncertainty introduced by the probabilistic nature of LLMs and dependencies between agents, a multi-agent framework is proposed in which specialised agents perform data preparation, modelling, review, and explanation tasks under a central hub. The main contribution is a novel approach to uncertainty propagation using token-level log-probabilities and a Bayesian Network. Importantly, log probabilities are not treated as direct probabilities of correctness or task success. Instead, length-normalised log-probability summaries are transformed into calibrated task-level confidence estimates before incorporation into the Bayesian Network. Results show that the framework reproduces baseline actuarial performance while providing additional insight into workflow stability and runtime uncertainty propagation.

</details>


### [62] [Distributing Security Controls Through Harness Engineering](https://arxiv.org/abs/2607.25890)
*William Robert Gore*

Main category: cs.AI

Selection reason: 研究AI编码agent的安全控制分发，与coding agent安全相关。

TL;DR: 提出SHarD安全框架，通过OS沙箱、技能扫描和工具限制三种控制，实现与商业代理等效的安全效果，并支持一键分发。


<details>
  <summary>Details</summary>
Motivation: 安全风险是组织扩展代理型AI的主要障碍，现有安全控制未系统分发，供应商解决方案存在生态依赖。

Method: 采用分阶段测试方法，针对四种代理配置（含/不含控制的商业代理、基线框架、安全强化框架），使用基于OWASP Top 10的23项测试套件，构建SHarD框架。

Result: SHarD获得100%调整得分，匹配最佳安全商业代理，无测试类别回归；发现模型非确定性导致安全结果不一致，OS沙箱可缓解跨边界行为。

Conclusion: 证实可分发安全代理控制的有效性，提出控制框架适应度初始特征，并识别未来研究问题（第三个研究问题）。

中文摘要：AI编码代理正以历史性的速度被采用，然而安全与风险问题仍然是组织扩展代理型AI的主要障碍。现有的编码代理安全控制并未系统性地分发给工程团队，而供应商原生解决方案引入了生态系统依赖，可能不适用于所有部署场景。本文研究了是否可以在商业AI编码代理上实现现成的安全控制，并通过自定义代理框架将其扩展到分布式用户群。采用分阶段测试方法，对四种代理配置（两种商业代理，分别带有和不带控制；一个基线框架；一个安全强化框架）进行了评估，使用了源自OWASP代理应用Top 10的23项测试套件。SHarD（Secure Harness Distribution）是一个基于Pi代理框架构建的可分发框架，它证明了三种安全控制类别——操作系统沙箱、技能扫描和工具限制——可以通过单一安装命令嵌入和分发，同时保持与直接安装在商业代理上等效的效果。SHarD取得了100%的调整得分，与最佳安全配置的商业代理相匹配，且在任何测试类别上均无回归。值得注意的观察包括：模型非确定性会产生不一致的安全结果，而自主代理行为可能以操作系统沙箱直接缓解的方式跨越系统边界。本文提出了控制框架适应度的初始特征，并确定了未来研究的第三个研究问题。

Original abstract: AI coding agents are being adopted at historic speed, yet security and risk concerns remain the primary barrier to scaling agentic AI across organizations. Existing security controls for coding agents are not systematically distributed to engineering teams, and vendor-native solutions introduce ecosystem dependencies that may not suit every deployment context. This paper investigates whether off-the-shelf security controls can be implemented on commercial AI coding agents and scaled to a distributed user base via a custom agent harness. A phased testing methodology was applied across four agent configurations --- two commercial agents with and without controls, a baseline harness, and a security-hardened harness --- using a 23-test suite derived from the OWASP Top 10 for Agentic Applications. SHarD (Secure Harness Distribution), a distributable harness built on the Pi agent harness, demonstrated that three categories of security controls --- OS sandboxing, skill scanning, and tool restriction --- can be embedded and distributed via a single install command while retaining equivalent efficacy to direct installation on commercial agents. SHarD achieved an adjusted score of 100\%, matching the best securely configured commercial agent, with no regression across any test category. Notable observations include evidence that model non-determinism produces inconsistent security outcomes and that autonomous agent behavior can cross system boundaries in ways that OS sandboxing directly mitigates. Initial characteristics toward a control harness fitness framework are proposed, and a third research question is identified for future investigation.

</details>


### [63] [Messier: A High-Resolution Corpus for Cross-Benchmark Agent Evaluation](https://arxiv.org/abs/2607.25891)
*Stefan Krsteski,Charlotte Meyer,Guillaume Allegre,Tony O'Halloran,Alexandre Sallinen*

Main category: cs.AI

Selection reason: 构建跨基准agent评估统一语料库，可迁移至agent评测。

TL;DR: Messier是一个包含957,253条标准化记录的统一语料库，涵盖30个基准，用于评估AI代理，揭示了不同基准类型上的进展差异，并提供了可特化的能力量表。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理评估因任务、脚手架、验证器和评分规则碎片化而难以比较，缺乏大规模标准化语料库。

Method: 构建Messier语料库，整合公开基准分数并补充代表性不足领域的运行结果，标准化记录字段并添加SOC/NAICS分类，进行反事实重新评分和能力量表推导。

Result: 前沿进展不均匀，函数调用饱和，编程改进最快，企业工作流最具挑战性；严格全通过聚合可能掩盖进展；能力量表与Epoch排名高度相关（ρ=0.81）。

Conclusion: Messier为代理能力缩放、基准审计和评估失败分析提供了基础性、可重复使用的基础设施。

中文摘要：在交互式环境中评估AI代理受到碎片化任务、脚手架、验证器和评分规则的阻碍。现有工作集中于狭窄设置，规模有限，或需要昂贵重跑，使得大部分实证记录难以比较。我们引入了Messier，一个统一的语料库，包含957,253条记录，涵盖30个基准、714个代理、11,891个任务和74,205个验证器。Messier整合了公开基准分数，并通过五个代理在六个代表性不足的专业和科学领域（包括一个最近的法律基准）上的运行结果进行了补充。每条记录按模型、脚手架、环境、任务、验证器和聚合规则标准化，并带有用于职业和行业分析的SOC/NAICS分类。使用该语料库，我们发现前沿进展在不同基准类型上不均匀，“函数调用”已饱和，“编程”改进最快，“企业工作流”仍最具挑战性。此外，反事实重新评分显示，多验证器任务中的严格全通过聚合可能掩盖进展并人为改变代理排名。从这些标准化记录中，我们推导出能力量表，与Epoch的评估能力指数排名在Spearman ρ = 0.81上一致，并可针对领域、职业、动作空间或验证器类型进行特化。Messier为代理能力缩放、基准审计和评估失败的细粒度分析提供了基础性、可重复使用的基础设施。

Original abstract: Evaluating AI agents in interactive environments is hindered by fragmented tasks, scaffolds, verifiers, and scoring rules. Existing efforts focus on narrow settings, remain limited in scale, or require costly reruns, leaving much of the empirical record incomparable. We introduce Messier, a unified corpus of 957,253 records that span 30 benchmarks, 714 agents, 11,891 tasks, and 74,205 verifiers. Messier consolidates public benchmark scores and supplements them with five-agent runs across six underrepresented professional and scientific domains, including a recent legal benchmark. Each record is standardized by model, scaffold, environment, task, verifier, and aggregation rule, with SOC/NAICS classifications for occupational and industry analysis. Using this corpus, we show frontier progress is uneven across benchmark types, with "function calling" saturated, "programming" improving the fastest, and "enterprise workflows" remaining the most challenging. Furthermore, counterfactual rescoring shows that strict all-pass aggregation in multi-verifier tasks can obscure progress and artificially alter agent rankings. From these standardized records, we derive capability scales that align with Epoch's Evaluation Capability Index rankings at Spearman \r{ho} = 0.81 and can be specialized by domain, occupation, action space, or verifier type. Messier provides a foundational, reusable infrastructure for agent capability scaling, benchmark auditing, and fine-grained analysis of evaluation failures.

</details>


### [64] [Interactive Reward Agent: GUI Task Evaluation via Environment-State Verification](https://arxiv.org/abs/2607.25904)
*Chenrui Shi,Yuwei Wu,Yang Liu,Ruining Feng,Zirui Shang,Zhi Gao,Lifeng Fan,Che Sun*

Main category: cs.AI

Selection reason: 提出交互式奖励智能体（IRA）用于GUI任务评估，涉及环境状态验证和工具使用，属于通用agent评估方法，可迁移到代码优化agent的验证。

TL;DR: 提出基于先提议后验证的交互式奖励代理（IRA），通过调用系统、应用和GUI工具从执行后环境中获取并验证证据，实现可靠GUI任务评估。在GUI-RewardBench上达86.9%准确率，并用于强化学习提升GUI代理性能。


<details>
  <summary>Details</summary>
Motivation: GUI任务评估需要环境状态信息，仅凭截图难以准确判断，因此需要能交互获取环境证据的评估方法。

Method: 提出交互式奖励代理（IRA），采用先提议后验证框架：先提出任务完成条件，再通过调用系统、应用和GUI工具验证条件，结合可见界面与环境状态证据。

Result: 在GUI-RewardBench上达到86.9%准确率，优于现有基线；应用于强化学习使GUI代理在OSWorld上成功率达34.0%。

Conclusion: IRA能有效利用环境状态进行可靠评估，并为GUI代理训练提供有效奖励信号。

中文摘要：图形用户界面任务评估旨在确定GUI代理是否成功完成了用户指令。自动化GUI任务评估日益受到关注，因为评估结果可以作为测试时扩展和后期训练的奖励信号。然而，可靠的GUI任务评估仍然具有挑战性，因为判断往往需要访问环境状态，例如系统配置、文件数据和应用程序设置，而不仅仅是执行轨迹的截图。在本文中，我们提出了一种基于先提议后验证框架的交互式奖励代理（IRA），用于从执行后环境中获取和验证证据。给定任务指令和GUI代理执行后的GUI环境，IRA首先提出任务完成条件，然后通过调用系统工具、应用程序工具和GUI工具来验证这些条件。这种设计在一个交互过程中结合了来自可见界面和环境状态的证据。我们进一步引入了GUI-RewardBench，一个包含321个GUI任务轨迹的基准测试，涵盖10个Ubuntu桌面应用类别。实验表明，IRA在GUI-RewardBench上达到了86.9%的准确率，优于现有的评估基线。我们还将IRA应用于GUI代理的强化学习，实现了34.0%的OSWorld成功率，这表明IRA可以为训练GUI代理提供有效的奖励信号。

Original abstract: Graphical user interface task evaluation aims to determine whether a GUI agent has successfully completed a user instruction. Automated GUI task evaluation has received increasing attention because the evaluation results can serve as reward signals for both test-time scaling and post-training. However, reliable GUI task evaluation remains challenging because the judgments often require access to environment states, such as system configurations, file data, and application settings, beyond the screenshots of execution trajectories. In this paper, we propose an interactive reward agent (IRA) based on a propose-then-verify framework to acquire and verify evidence from the post-execution environment. Given a task instruction and a GUI environment after the GUI agent execution, IRA first proposes the task completion conditions and then verifies them by invoking system tools, application tools, and GUI tools. This design combines evidence from both visible interfaces and the environment state in an interactive process. We further introduce GUI-RewardBench, a benchmark of 321 GUI task trajectories spanning 10 Ubuntu desktop application categories. Experiments show that IRA achieves 86.9% accuracy on GUI-RewardBench, outperforming existing evaluator baselines. We further apply IRA to reinforcement learning of GUI agents, achieving a 34.0% OSWorld success rate, which demonstrates that IRA can provide effective reward signals for training GUI agents.

</details>


### [65] [Toward Standardized Cross-Vendor Agent Tool Trust Management in Autonomous Networks](https://arxiv.org/abs/2607.25914)
*Ravi Kant Sharma,Ashutosh Uttam,Ajay Kumar*

Main category: cs.AI

Comments: 22 pages, 7 figures, 9 tables, 4 algorithms

Selection reason: 研究跨供应商agent工具信任管理，提出标准化机制，涉及多agent协作与工具安全，对agent系统设计有潜在启发。

TL;DR: 提出AgentToolMO模型，通过标准化跨供应商信任通知、级联传播控制和追溯影响评估，将信任攻击的爆炸半径从小时级降至近实时遏制。


<details>
  <summary>Details</summary>
Motivation: 现有管理标准缺乏跨供应商信任可见性机制，导致受攻陷工具被持续使用，引发级联服务影响。

Method: 设计基于3GPP NRM的信息模型，包含信任状态机、阻尼级联传播、跨供应商信任通知和NRM依赖图追溯评估。

Result: 仿真表明，标准化通知将爆炸半径从小时级降至近实时，级联收敛有界，通知扩展次线性。

Conclusion: 该模型可在现有3GPP基础设施内实现，为可信的多供应商自主网络管理提供标准化路径。

中文摘要：自主网络等级4-5要求AI代理无需人工监督即可跨供应商边界调用工具，然而现有的管理标准缺乏跨供应商信任可见性的标准化机制。当供应商B的工具被攻陷时，供应商A的代理继续调用它——不知道信任降级——导致级联服务影响。我们提出AgentToolMO，一个用于代理工具信任管理的3GPP NRM信息模型。该模型包括：一个正式定义的信任状态机，具有可证明的渐进式强制；阻尼级联传播，具有有界收敛；通过现有管理服务（MnS）接口的跨供应商信任通知；以及通过NRM依赖图遍历的追溯影响评估。跨多供应商拓扑的仿真评估表明，标准化的跨供应商通知将爆炸半径从小时级的未检测传播减少到以MnS通知传递为界的近实时遏制，级联收敛保证在有限迭代内，且通知扩展跨供应商域呈次线性。该框架在现有3GPP管理基础设施内运行，利用现有协议，并为可信的多供应商自主网络管理提供了标准化路径。

Original abstract: Autonomous Network Levels 4-5 require AI agents to invoke tools across vendor boundaries without human oversight, yet existing management standards lack a standardized mechanism for cross-vendor trust visibility. When a tool from Vendor B is compromised, agents from Vendor A continue invoking it -- unaware of the trust degradation -- causing cascading service impact. We present AgentToolMO, a proposed 3GPP NRM information model for agent tool trust management. The model comprises: a formally defined trust state machine with provable graduated enforcement, damped cascade propagation with bounded convergence, cross-vendor trust notifications via existing Management Services (MnS) interfaces, and retroactive impact assessment through NRM dependency graph traversal. Simulation-based evaluation across multi-vendor topologies shows that standardized cross-vendor notifications reduce blast radius from hours-scale undetected propagation to near-real-time containment bounded by MnS notification delivery, with cascade convergence guaranteed in bounded iterations and sub-linear notification scaling across vendor domains. The framework operates within existing 3GPP management infrastructure, leverages existing protocols, and provides a standardization pathway for trustworthy multi-vendor autonomous network management.

</details>


### [66] [Large Language Model for Operations Research Formulation Selection in Multi-Warehouse Inventory Allocation](https://arxiv.org/abs/2607.25956)
*Jintao Xu,Yingzheng Ma,Jiong Dong,Yongzhi Qi,Jianshen Zhang*

Main category: cs.AI

Selection reason: 使用LLM选择运筹学公式，涉及LLM作为决策智能体与优化器交互，其训练方法（GRPO）可启发代码优化agent的调优策略。

TL;DR: 提出求解器引导的LLM框架，通过GRPO训练选择器，在京东多仓库库存分配中显著提升公式选择准确率和实际分配质量。


<details>
  <summary>Details</summary>
Motivation: 多仓库库存分配中，单一MIP公式无法适应异构实例，需要实例级公式选择以提升分配质量。

Method: 构建求解器引导的LLM框架，使用SFT进行模式学习，IPO生成偏好，GRPO优化选择器。

Result: 在京东数据集上，GRPO将Hit Ratio@1从21.45%提升至50.42%，Hit Ratio@2从70.47%提升至82.31%，分配准确率比基线高12.57个百分点。

Conclusion: 求解器引导的LLM框架和GRPO训练能有效提升多仓库库存分配中的公式选择性能，接近事后最优。

中文摘要：多仓库库存分配通常被建模为混合整数规划（MIP）问题，然而，没有单一的公式能够一致地适应由需求集中度、库存不平衡、补货规模、服务约束和预测波动性引起的异构实例级机制。我们将此问题研究为实例级运筹学（OR）公式选择，其中每个分配实例被分配到一个来自候选OR专家库的可执行公式。我们提出了一种求解器引导的大语言模型（LLM）框架用于OR公式选择，其中每个OR专家对应一个编码不同分配优先级的MIP公式。为了训练选择器，该框架首先构建平衡的专家条件监督微调（SFT）记录用于模式学习，然后使用MIP求解器对历史实例的评估，将求解器评估的分配质量差距转化为边际加权身份偏好优化（IPO）偏好和每个实例的专家得分元数据，用于在组相对策略优化（GRPO）期间进行奖励查找，以对采样的响应分配奖励。在来自中国最大的电子零售商之一京东的多仓库库存分配实例上进行的实验表明，相对于SFT+IPO选择器，GRPO显著提高了专家选择准确性，更重要的是，产生了比偏好训练的选择器和最佳固定公式更高的实际分配质量。使用GRPO，Hit Ratio@1和Hit Ratio@2分别从21.45%提升到50.42%，从70.47%提升到82.31%。最终选择器在分配准确率上比现有基线提高了12.57个百分点，优于SFT+IPO选择器和最佳固定OR专家，并将与事后最优的差距缩小到4.85个百分点。

Original abstract: Multi-warehouse inventory allocation is typically formulated as a mixed-integer programming (MIP) problem, yet no single formulation consistently matches heterogeneous instance-level regimes induced by demand concentration, inventory imbalance, replenishment scale, service constraints, and forecast volatility. We study this issue as instance-wise operations research (OR) formulation selection, where each allocation instance is assigned to a solver-executable formulation from a candidate OR expert library. We propose a solver-guided large language model (LLM) framework for OR formulation selection, in which each OR expert corresponds to a MIP formulation encoding a distinct allocation priority. To train the selector, the framework first constructs balanced expert-conditioned supervised fine-tuning (SFT) records for schema learning, and then uses MIP solver evaluation on historical instances to convert solver-evaluated allocation-quality gaps into margin-weighted identity preference optimization (IPO) preferences and per-instance expert-score metadata for reward lookup during group relative policy optimization (GRPO) to assign rewards to sampled responses. Experiments on multi-warehouse inventory allocation instances from JD$\mathord{.}$com, one of China's largest e-retailers, demonstrate that GRPO substantially improves expert-selection accuracy relative to the SFT+IPO selector and, more importantly, produces higher realized allocation quality than both the preference-trained selector and the best fixed formulation. With GRPO, Hit Ratio@1 and Hit Ratio@2 increase from 21.45% to 50.42% and from 70.47% to 82.31%. The resulting selector achieves an allocation accuracy gain of 12.57 percentage points over the incumbent baseline, outperforming both the SFT+IPO selector and the best fixed OR expert, and reduces the gap to the ex-post oracle to 4.85 percentage points.

</details>


### [67] [Desktop-Delta Bench: Do Computer-Use Models Understand Desktop GUI Transitions?](https://arxiv.org/abs/2607.26041)
*Abhishek Pillai,Samir Kumar Nayak,Yuan Chen*

Main category: cs.AI

Selection reason: 提出桌面GUI智能体基准测试，评估模型对GUI转换的理解，属于agent评测，可迁移到编码智能体的状态验证与恢复。

TL;DR: DDB是一个离线步骤级基准，通过时间排序和前后对比任务评估桌面CUA的动作因果推理能力，揭示了现有模型的系统性缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前基准主要测量最终任务成功或单帧定位，无法隔离模型重建动作因果过渡的能力，这对于拒绝过时观察、验证进展和从失败中恢复至关重要。

Method: DDB包含2,013个人工验证实例，来自约15个应用和50个任务域，通过463个三帧时间排序实例和1,550个前后对比对，针对状态验证、源跟踪和上下文感知控制三个失败维度进行评估。

Result: 在8个模型家族上评估，排序任务非干扰和干扰精确匹配率分别为65.1%和65.7%；任务上下文提高干扰识别6.9个百分点但降低非干扰精确匹配2.2点；单动作中点击F1为0.96，拖拽为0.76。

Conclusion: DDB通过填补GUI接地和最终任务成功之间的诊断层，提供了针对桌面CUA验证、可靠性和恢复的针对性改进方向。

中文摘要：计算机使用代理（CUAs）逐渐通过桌面图形用户界面（GUI）来执行长期任务。当前的基准主要测量最终任务成功或单帧定位，但这两者都无法隔离模型是否能够重建动作产生的因果、任务相关的过渡——这对于拒绝过时观察、验证进展以及从失败中恢复至关重要。这一点很困难，因为推断、远程输入、应用渲染和截图捕获是异步的：下一个观察可能会延迟、被遮挡、短暂或无关，然后被误认为是进展并带入后续规划。我们引入了Desktop-Delta Bench（DDB），一个离线步骤级基准，包含来自新颖的多应用Linux轨迹的2,013个人工验证实例，涵盖约15个应用和50个任务域。DDB轨迹通过两个互补任务针对三个失败维度——状态验证、源跟踪和上下文感知控制：463个三帧时间排序实例（包括105个跨轨迹干扰）和1,550个从5个动作及其有效载荷中标注的前后对比对。我们评估了8个闭源和开源模型家族，涵盖32种排序和16种单动作设置，观察到一致的差距。排序尚未饱和：最佳非干扰和干扰精确匹配率分别为65.1%和65.7%。任务上下文使干扰识别提高了6.9个百分点，但使非干扰精确匹配降低了2.2个百分点；错误分析显示系统性地复制了呈现的A-B-C顺序。单动作结果表明，推断动作族比定位更难：点击F1为0.96，而拖拽为0.76，但识别出的拖拽通常定位良好。因此，DDB通过填补GUI接地与最终任务成功之间缺失的诊断层，补充了端到端基准，从而能够针对性地改进桌面CUA的验证、可靠性和恢复。

Original abstract: Computer-use agents (CUAs) increasingly act through desktop GUIs to complete long-horizon tasks. Current benchmarks primarily measure end-task success or single-frame grounding. Neither isolates whether a model can reconstruct the causal, task-relevant transition produced by an action- crucial for rejecting stale observations, verifying progress, and recovering from failure. This is difficult because inference, remote input, app rendering, and screenshot capture are asynchronous: the next observation may be delayed, occluded, transient, or unrelated, then misread as progress and carried into subsequent planning. We introduce Desktop-Delta Bench (DDB), an offline step-level benchmark with 2,013 human-verified instances from novel, multi-app Linux trajectories across ~15 applications and 50 task domains. DDB trajectories targets 3 failure dimensions- state verification, source tracking, and context-aware control- through 2 complementary tasks: 463 3-frame temporal-ordering instances, including 105 with a cross-trajectory decoy, and 1,550 before-after pairs labeled from 5 actions + its payload. We evaluate 8 closed and open-source model families across 32 ordering and 16 single-action settings, observing consistent gaps. Ordering remains unsaturated: best non-decoy and decoy exact-match rates are 65.1% and 65.7%. Task context improves decoy identification by 6.9 percentage points but reduces non-decoy exact match by 2.2 points; error analysis reveals systematic copying of the presented A-B-C order. Single-action results show that inferring the action family is harder than locating it: click F1 is 0.96 vs, 0.76 for drag, while recognized drags are generally localized well. DDB, thus, complements end-to-end benchmarks by filling the missing diagnostic layer between GUI grounding and final task success, enabling targeted improvements to desktop CUA verification, reliability, and recovery.

</details>

<!-- semopt-se-selection:start -->
## SE 精选（SemOpt / Coding Agent 相关）

以下为从原始 cs.SE 列表中筛出的 SemOpt / Coding Agent 相关论文。

- [WarmTuner: Program-Specific Warm Starts for Compiler Autotuning via Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2607.25831) — 相关度 0.95：离线到在线RL的编译器调优框架，利用程序条件策略和GRPO提升自动性能优化效果，直接相关SemOpt。
- [CodeNib: A Multi-View Data System for Serving Repository Context to Coding Agents](https://arxiv.org/abs/2607.25431) — 相关度 0.92：为coding agent提供多视图仓库上下文服务的系统，降低token开销并加速检索与导航。
- [Specula: Scaling formal specifications for autonomous model checking of system code](https://arxiv.org/abs/2607.25333) — 相关度 0.92：自主生成TLA+形式化规范并进行模型检查的agent系统，大幅提升代码验证与深度bug发现能力。
- [Learning from 53.6K Real-World Developer Edits of AI-Generated Code](https://arxiv.org/abs/2607.25130) — 相关度 0.90：提供53.6K真实IDE内AI代码编辑轨迹的数据集，可用于训练和评估代码编辑/修复agent。
- [Authoring Agent Skills: A Software-Engineering Approach](https://arxiv.org/abs/2607.25032) — 相关度 0.88：将Agent Skill视为软件工件并给出工程化编写与评估原则，直接适用于coding agent的技能开发与选择。
- [Beyond "What to Retrieve": Uncertainty in Retrieval-Augmented Code Generation](https://arxiv.org/abs/2607.24884) — 相关度 0.86：对仓库级代码生成中的检索不确定性建模与过滤修复，可迁移到coding agent的RAG、验证与修复流程。
- [RepoReasoner: Evaluating Repository-Level Code Reasoning Ability of Long-Context Language Models](https://arxiv.org/abs/2607.25996) — 相关度 0.86：新增仓库级代码推理基准，评估跨文件输出预测与调用链理解，直接支撑长上下文代码agent评测。
- [SkillGate: Cost Efficient Runtime Malicious Skill File Detection in Coding Agents](https://arxiv.org/abs/2607.25619) — 相关度 0.85：针对coding agent恶意skill文件的检测网关，低成本防护供应链攻击，直接增强agent安全性。
- [How Do LLMs Read Bug Reports? An Empirical Study of Attention in LLMs for Automated Program Repair](https://arxiv.org/abs/2607.25873) — 相关度 0.82：首次实证分析LLM程序修复中的注意力分配，解释修复成败差异，可直接指导APR上下文与提示设计。
- [Explanation-Bound Tool Execution for AI Agents: Server-Verified Action Claims Without Trusting Model Rationales](https://arxiv.org/abs/2607.25364) — 相关度 0.78：提出工具执行声明校验中介层，将模型理由转化为服务端可验证的动作声明，可迁移到代码agent的工具调用安全与审计。
- [KQFuzz: Knowledge-Guided Fuzzing for Quantum Libraries via Large Language Models](https://arxiv.org/abs/2607.25647) — 相关度 0.75：知识引导的LLM模糊测试框架，结合代码库知识生成量子程序测试，显著提升覆盖率并发现实际bug。
- [Rethinking Training Data for Generating Code Review Comments](https://arxiv.org/abs/2607.25851) — 相关度 0.75：分析代码评审评论生成中的错位训练数据并给出类型学和过滤建议，对自动化代码评审agent有直接帮助。
- [RSIBench-Data: Benchmarking Data-Centric Research for Recursive Self-Improvement](https://arxiv.org/abs/2607.25886) — 相关度 0.72：构建数据驱动LLM agent递归自我改进的受控评测基准，覆盖软件工程等任务，可迁移到代码agent的反馈迭代与策略优化评估。
- [Specification-Driven DevOps for Multi-Service Environments](https://arxiv.org/abs/2607.25141) — 相关度 0.72：用LLM生成多服务Docker配置并区分功能正确性与部署意图保真度，对部署型工程agent有借鉴意义。
- [An Empirical Study of Model Context Protocol Applications](https://arxiv.org/abs/2607.25635) — 相关度 0.70：大规模MCP应用实证，揭示工具集成与人工监督配置现状，对工程化agent设计和安全治理有参考。
- [Delta Debugging for Cyber-Physical Systems with Flaky Test Executions](https://arxiv.org/abs/2607.25695) — 相关度 0.55：针对flaky仿真的delta debugging算法，可迁移到失败复现与最小化输入隔离，适用于agent验证场景。
- [Input Relation Prompting for Metamorphic Testing on Query-Based Systems](https://arxiv.org/abs/2607.25603) — 相关度 0.55：通过输入关系提示自动生成蜕变关系以辅助蜕变测试，可迁移到自动化测试生成和oracle缓解。
- [Characterizing Structural Testability in JavaScript: An Empirical Study](https://arxiv.org/abs/2607.24965) — 相关度 0.50：大规模JavaScript结构可测试性实证，产出测试性度量框架，对自动化测试生成和测试感知重构有参考价值。

<!-- semopt-se-selection:end -->
