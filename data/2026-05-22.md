...
- [MileStone: A Multi-Objective Compiler Phase Ordering Framework for Graph-based IR-Level Optimization](http://arxiv.org/abs/2605.23435v1) — 相关度 0.95：图表示与强化学习求解多目标编译器pass排序，直接属于自动程序性能优化方向，可迁移到编译优化与benchmark验证。
- [Push Your Agent: Measuring and Enforcing Quantitative Goal Persistence in Long-Horizon LLM Agents](http://arxiv.org/abs/2605.23574v1) — 相关度 0.93：提出量化目标坚持度基准PushBench，测度长程agent在验证器支持下的持久性与防重复提交，直接服务coding agent可靠性与评测。
- [Agentic Proving for Program Verification](http://arxiv.org/abs/2605.23772v1) — 相关度 0.90：将agentic proving用于Lean 4程序验证基准，评估Claude Code的规格生成与验证能力，对程序验证agent和评测体系有直接价值。
- [ContextEcho: A Benchmark for Persona Drift in Long Agentic-Coding Sessions](http://arxiv.org/abs/2605.24279v1) — 相关度 0.90：构造ContextEcho基准与快照探测协议，刻画长时agentic编码会话中persona漂移，直接面向coding agent长期会话评测。
- [The Time is Here for Just-in-Time Systems: Challenges and Opportunities](http://arxiv.org/abs/2605.24096v1) — 相关度 0.90：用Claude Code按规格即时合成定制键值存储系统，并通过测试套迭代优化性能，直接展现编码agent用于系统生成与性能优化的潜力。
- [VISTA: An End-to-End Benchmark for Visual Spec-to-Web-App Coding Agents](http://arxiv.org/abs/2605.26144v3) — 相关度 0.90：提出端到端视觉规格到Web应用生成的编码agent基准，包含多模态评测与交互组件标注，直接面向UI类coding agent评估。
- [JEDI: Java Evaluation of Declarative and Imperative Queries](http://arxiv.org/abs/2605.23543v1) — 相关度 0.85：提出Java Stream API性能基准套件JEDI，并通过SQL转换生成多种实现与并行策略对比，直接服务于程序性能优化与benchmark设计。
- [Misleading Microbenchmarks on the Java Virtual Machines](http://arxiv.org/abs/2605.23570v1) — 相关度 0.85：系统分析JVM微基准测试在分层编译下的误导性问题并扩充实践指南，对benchmark可靠性与性能评估方法有直接迁移价值。
- [TRACER: A Semantic-Aware Framework for Fine-Grained Contamination Detection in Code LLMs](http://arxiv.org/abs/2605.24079v1) — 相关度 0.80：面向代码LLM的语义感知细粒度数据污染检测，提升benchmark可信度与评测可靠性，可迁移到代码agent评测场景。
- [Understanding Conversational Patterns in Multi-agent Programming: A Case Study on Fibonacci Game Development](http://arxiv.org/abs/2605.24138v1) — 相关度 0.80：分析双agent编程协作中的角色对齐、收敛效率与错误传播，为多agent软件工程的工作流与控制机制提供实证与启发。
- [Attested Tool-Server Admission: A Security Extension to the Model Context Protocol](http://arxiv.org/abs/2605.24248v2) — 相关度 0.75：为MCP提供带认证的工具服务器准入与按服务器工具白名单机制，提升coding agent工具链的安全可控性。
- [Towards Evaluation Engineering: An Empirical Study of ML Evaluation Harnesses in the Wild](http://arxiv.org/abs/2605.24213v1) — 相关度 0.72：对57个ML评估harness做实证研究并总结工作流阶段与根因，对构建和优化agent/harness有方法迁移价值。
- [AI Assurance: A Comprehensive Testing Strategy for Enterprise AI Systems](http://arxiv.org/abs/2605.23459v1) — 相关度 0.62：面向企业LLM/agent系统的测试与保障策略，提供了AI测试的分类与评估驱动开发框架，可迁移到agent评测和可靠性设计。
- [The Impact of AI Coding Assistants on Software Engineering: A Longitudinal Study](http://arxiv.org/abs/2605.23135v1) — 相关度 0.62：纵向研究AI编码助手对任务焦点与开发者体验的影响，提出监督式工程工作等概念，对理解编码agent使用效应有参考价值。
- [Empirical Analysis and Detection of Hallucinations in LLM-Generated Bug Report Summaries](http://arxiv.org/abs/2605.24137v1) — 相关度 0.60：针对LLM生成的bug报告摘要做分节感知幻觉检测，可提升代码维护类agent输出的可靠性，但主题细节相对聚焦。
