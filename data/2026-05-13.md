...
- [AgentLens: Revealing The Lucky Pass Problem in SWE-Agent Evaluation](http://arxiv.org/abs/2605.12925v3) — 相关度 0.95：揭示SWE-Agent评测中的Lucky Pass问题,提出过程级评测框架与数据集,直接推进Agent评测方法。
- [Effective Harness Engineering for Algorithm Discovery with Coding Agents](http://arxiv.org/abs/2605.15221v1) — 相关度 0.95：系统研究算法发现场景中编码Agent的harness设计,包括token预算、hack检测与安全执行,可直接迁移到SemOpt。
- [PerfCodeBench: Benchmarking LLMs for System-Level High-Performance Code Optimization](http://arxiv.org/abs/2605.15222v2) — 相关度 0.95：直接构建LLM系统级性能优化基准，评测正确性与运行时效率，完全符合SemOpt主题。
- [SWE-Cycle: Benchmarking Code Agents across the Complete Issue Resolution Cycle](http://arxiv.org/abs/2605.13139v1) — 相关度 0.95：构建跨环境重建、实现、验证测试的完整issue解决周期benchmark,并引入执行能力评测Agent,直接服务代码Agent评估。
- [CRANE: Constrained Reasoning Injection for Code Agents via Nullspace Editing](http://arxiv.org/abs/2605.14084v2) — 相关度 0.90：通过零训练的检查点参数编辑提升代码智能体的推理与工具遵循能力，直接增强智能体性能。
- [PBT-Bench: Benchmarking AI Agents on Property-Based Testing](http://arxiv.org/abs/2605.15229v3) — 相关度 0.90：专门针对性质测试智能体的基准PBT-Bench，评测代理从文档提取不变量与生成输入策略的能力，高度相关。
- [Protocol-Driven Development: Governing Generated Software Through Invariants and Continuous Evidence](http://arxiv.org/abs/2605.12981v3) — 相关度 0.90：提出协议驱动开发,以不变量与证据链治理生成软件,为代码Agent的可信生成提供通用治理模型。
- [SieveFL: Hierarchical Runtime-Aware Pruning for Scalable LLM-Based Fault Localization](http://arxiv.org/abs/2605.13491v1) — 相关度 0.90：提出分层剪枝的LLM故障定位框架，在低成本硬件上超越现有基线，直接面向代码智能体的关键任务。
- [AI Harness Engineering: A Runtime Substrate for Foundation-Model Software Agents](http://arxiv.org/abs/2605.13357v1) — 相关度 0.85：提出AI Harness Engineering框架，形式化面向基础模型软件代理的运行支撑层，直接关乎代码智能体的可靠性。
- [Is Agentic AI Ready for Real-World Hardware Engineering? A Deep Dive with Phoenix-bench](http://arxiv.org/abs/2605.15226v1) — 相关度 0.85：构建硬件工程领域的智能体基准Phoenix-bench，并系统比较软件智能体的迁移性，对代码智能体评估有重要价值。
- [SkillOps: Managing LLM Agent Skill Libraries as Self-Maintaining Software Ecosystems](http://arxiv.org/abs/2605.13716v1) — 相关度 0.80：提出LLM代理技能库的维护框架SkillOps，针对技能技术债，可迁移到智能体长期运行与记忆管理。
- [Improving Code Translation with Syntax-Guided and Semantic-aware Preference Optimization](http://arxiv.org/abs/2605.13229v1) — 相关度 0.75：提出语法引导与语义感知的偏好优化方法改进代码翻译，可迁移到代码生成与修复的对齐训练。
- [The Readability Spectrum: Patterns, Issues, and Prompt Effects in LLM-Generated Code](http://arxiv.org/abs/2605.13280v1) — 相关度 0.75：系统评估LLM生成代码的可读性并分析提示影响，为代码生成质量评测提供方法。
- [Automatic Detection of Reference Counting Bugs in Linux Kernel Drivers](http://arxiv.org/abs/2605.13246v2) — 相关度 0.70：面向Linux驱动引用计数缺陷的自动化程序分析工具，属于通用缺陷检测方法。
- [CA2: Code-Aware Agent for Automated Game Testing](http://arxiv.org/abs/2605.13918v1) — 相关度 0.70：利用调用栈信息的代码感知代理进行自动化游戏测试，可迁移到一般代码测试智能体设计。
- [Code-Centric Detection of Vulnerability-Fixing Commits: A Unified Benchmark and Empirical Study](http://arxiv.org/abs/2605.13138v1) — 相关度 0.70：提供漏洞修复提交检测的统一基准与实证研究,对代码安全分析Agent的评测有参考价值。
- [LLM-Based Robustness Testing of Microservice Applications: An Empirical Study](http://arxiv.org/abs/2605.14202v1) — 相关度 0.70：LLM生成微服务API鲁棒性测试的经验研究，对测试生成及agent测试策略有迁移价值。
- [Neurosymbolic Auditing of Natural-Language Software Requirements](http://arxiv.org/abs/2605.13817v1) — 相关度 0.70：结合LLM与SMT求解器审计自然语言需求，是一种可迁移到需求分析智能体的神经符号方法。
- [Scalable Deductive Verification of Data-Level Parallel Programs](http://arxiv.org/abs/2605.13616v1) — 相关度 0.70：提出提升数据级并行程序演绎验证可扩展性的技术，属于程序验证加速方法。
- [TruncProof: A Guardrail for LLM-based JSON Generation under Token-Length Constraints](http://arxiv.org/abs/2605.13076v3) — 相关度 0.70：基于LL(1)解析器的token约束JSON生成方法,可作为代码Agent结构化输出鲁棒性的通用技术。
- [Method-level Change-proneness: A Better Metric for Black-box Test Suite Minimization](http://arxiv.org/abs/2605.15232v1) — 相关度 0.65：提出方法级变更倾向的测试套件最小化方法，为测试优化提供新的度量指标。
- [Robust Mutation Analysis of Quantum Programs Under Noise](http://arxiv.org/abs/2605.13279v1) — 相关度 0.65：研究噪声下量子程序的变异分析，为特定领域的软件测试方法提供可借鉴的评估策略。
- [(How) Do Large Language Models Understand High-Level Message Sequence Charts?](http://arxiv.org/abs/2605.13773v2) — 相关度 0.60：评估LLM对消息序列图形式语义的理解能力，对软件设计制品处理的智能体能力测评有参考价值。
- [Case Studies and Reflections on Agentic Software Engineering for Rapid Development of Digital Music Instruments](http://arxiv.org/abs/2605.14016v1) — 相关度 0.60：对代理式软件工程在音频软件开发中的案例与反思，总结了ASE实践中的有效做法，有启发意义。
- [ReproScore: Separating Readiness from Outcome in Research Software Reproducibility Assessment](http://arxiv.org/abs/2605.13275v1) — 相关度 0.60：提出软件可复现性评估的分离式框架与合成指标，对工程化评测有参考价值。
- [UIBenchKit: A unified toolkit for design-to-code model evaluation](http://arxiv.org/abs/2605.13141v1) — 相关度 0.60：设计到代码生成任务的统一评估工具包,虽垂直UI领域,但评测框架对代码生成Agent的benchmark构建有一定启发。
