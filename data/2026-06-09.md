...
- [An Ocean Model Ported by a Large Language Model: Experience and Lessons from FESOM2 (Fortran to C to C++/Kokkos)](http://arxiv.org/abs/2606.11356v1) — 相关度 0.90：LLM 智能体在专家指导下完成 Fortran 到 C++/Kokkos 的完整移植，兼顾正确性验证与 GPU 性能提升，是 coding agent 长程任务的典型范例。
- [DeNovoSWE: Scaling Long-Horizon Environments for Generating Entire Repositories from Scratch](http://arxiv.org/abs/2606.10728v2) — 相关度 0.90：构建大规模全仓库生成数据集并验证训练效果，直接面向长程SWE agent能力提升，高度相关。
- [Skill Coverage: A Test Adequacy Metric for Agent Skills](http://arxiv.org/abs/2606.20659v2) — 相关度 0.88：提出技能覆盖度指标，基于轨迹评估agent技能执行并给出失败信号，可直接用于代码agent技能测试与改进。
- [MASTOR: A Multi-Agent Approach to Semantic Test Oracle Generation for RESTful APIs](http://arxiv.org/abs/2606.10465v1) — 相关度 0.85：多智能体生成RESTful API语义测试oracle，含挑战者评审机制，直接涉及agent测试生成与自我改进，相关性强。
- [Beyond Coverage and Kill Scores: Empirically Measuring Test Suite Behavioural Gaps](http://arxiv.org/abs/2606.10417v1) — 相关度 0.72：提出行为差距度量补充覆盖率和变异分数，可用于评估测试套件质量，对agent测试生成与验证有参考价值。
- [Context-Based Adversarial Attacks on AI Code Generators: Vulnerability Analysis and Implications](http://arxiv.org/abs/2606.10945v1) — 相关度 0.70：针对 AI 代码生成器的上下文对抗攻击与防御，可直接迁移到 coding agent 的安全测试与加固。
- [RECON: An LLM-Enhanced Backward Constraint Analysis Framework](http://arxiv.org/abs/2606.10264v1) — 相关度 0.70：将LLM语义理解与静态程序分析结合进行约束提取，比符号执行更快且可解释，适用于程序分析类agent能力增强。
- [Operationalizing Property-Based Testing for Data-Intensive Scalable Computing Systems](http://arxiv.org/abs/2606.11132v1) — 相关度 0.65：为 Spark 等系统提供属性化测试引擎与可复用元属性，其语义测试方法可用于 agent 生成代码的正确性验证。
- [Learning Splitting Heuristics for Parallel String Solvers](http://arxiv.org/abs/2606.20656v1) — 相关度 0.62：用数据驱动学习字符串求解器的分裂启发式，属于求解器性能优化，可迁移到程序分析与约束求解的自动调优。
- [Making Software Meaningful](http://arxiv.org/abs/2606.11051v1) — 相关度 0.60：提出用显式行为语义支撑 LLM 模块化代码生成与 agent 问责，对 coding agent 设计有概念性迁移价值。
- [Watts and Debts of Agentic Frameworks: An Empirical Study (Registered Report)](http://arxiv.org/abs/2606.10702v1) — 相关度 0.60：注册报告研究agent框架技术债与能耗关联，为agent框架质量与可持续性评估提供实证视角，有参考价值。
- [Securing Code Understanding: Detecting Natural Backdoor Vulnerability in Code Language Models](http://arxiv.org/abs/2606.10846v1) — 相关度 0.58：研究CodeLM自然后门漏洞并提出检测方法，对代码agent使用的模型安全性评估有参考价值，但非核心方向。
- [Exploring and Complementing End Users' Requirements in IoT enabled System](http://arxiv.org/abs/2606.10598v1) — 相关度 0.55：面向IoT规则完成的意图驱动多agent框架，虽领域垂直，但其结合LLM推理与可追溯性模型可部分迁移到需求分析agent。
- [GapFuzz: Cross-Plane Divergence Fuzzing for Distributed SDN Controllers](http://arxiv.org/abs/2606.11035v2) — 相关度 0.55：面向分布式 SDN 控制器的并发模糊测试方法，其交叉平面状态探测与定时搜索思路可用于分布式系统测试。
- [Writing Better Software Explanations: A Guideline-Based Approach](http://arxiv.org/abs/2606.10880v1) — 相关度 0.55：基于指南的LLM辅助解释生成与迭代修订流程，可迁移到agent的受控生成与质量检查工作流设计。
- [Early Comparative Evaluation of Transformer Models for Multilingual Software Vulnerability Detection](http://arxiv.org/abs/2606.10925v1) — 相关度 0.52：多语言漏洞检测的Transformer模型比较与评估，提供数据点可参考，但方法贡献有限。
- [From Quality Properties to Practice: A Guideline and Workflow for Explainability Requirements](http://arxiv.org/abs/2606.10882v1) — 相关度 0.50：提出指南驱动的可解释性需求工作流，LLM辅助迭代生成与评估，对需求工程agent有一般性参考。
- [Understanding and mitigating the risks of OpenClaw for non-technical users: A practical guide with Skill](http://arxiv.org/abs/2606.11007v1) — 相关度 0.50：对 agent 框架的风险分类与自动化防护 Skill 可迁移到 coding agent 的安全配置与使用。
