...
- [Code Is More Than Text: Uncertainty Estimation for Code Generation](http://arxiv.org/abs/2606.09577v1) — 相关度 0.90：提出代码专属的三轴不确定性估计，显著提升代码生成可靠性与选择性预测，直接适用于代码agent。
- [FASE: Fast Adaptive Semantic Entropy for Code Quality](http://arxiv.org/abs/2606.09800v1) — 相关度 0.90：快速自适应语义熵用于多智能体代码生成的不确定性量化，低成本高效，可直接集成到agent工作流。
- [ATTAIN: Automated Exploit Failure Analysis through Trace-Driven Diff Analysis](http://arxiv.org/abs/2606.09060v1) — 相关度 0.85：用LLM工具循环进行差分分析定位漏洞影响版本，展示了agent化程序分析的通用模式。
- [Empirical Study for Structured Output Control in LLMs for Software Engineering](http://arxiv.org/abs/2606.09395v1) — 相关度 0.85：系统评估LLM在SE任务中的结构化输出控制，强调格式保真与语义正确性，对agent输出层有关键意义。
- [Multi-task LLMs for Bug Classification: Efficient Inference with Auxiliary Decoding Heads](http://arxiv.org/abs/2606.09956v1) — 相关度 0.85：通过多任务LLM与辅助解码头实现高效的行级缺陷定位，推理开销低，适用bug修复agent。
- [TestMap: Evidence Infrastructure for Foundation-Model-Assisted Test Generation](http://arxiv.org/abs/2606.10211v1) — 相关度 0.82：提供自动化、证据驱动的生成测试验证基础设施，记录测试候选全生命周期，对LLM辅助测试生成与评估有直接借鉴价值。
- [Autonomous Incident Resolution at Hyperscale: An Agentic AI Architecture for Network Operations](http://arxiv.org/abs/2606.09122v1) — 相关度 0.80：生产级多agent自主事件处理架构，包含分层分解、工具调用与闭环验证，可迁移到软件agent。
- [Context Rot in AI-Assisted Software Development: Repurposing Documentation Consistency for AI Configuration Artifacts](http://arxiv.org/abs/2606.09090v1) — 相关度 0.80：研究AI配置文件（CLAUDE.md等）的上下文腐化检测，直接关系到持久化agent上下文的维护。
- [Less Context, Better Agents: Efficient Context Engineering for Long-Horizon Tool-Using LLM Agents](http://arxiv.org/abs/2606.10209v1) — 相关度 0.78：针对工具使用型LLM Agent的上下文剪枝与摘要策略，显著提升长程任务可靠性与效率，可直接迁移到coding agent上下文管理。
- [What makes a harness a harness: necessary and sufficient conditions for an agent harness](http://arxiv.org/abs/2606.10106v1) — 相关度 0.72：对agent harness给出可操作定义与判定测试，有助于代码agent框架与评测的边界澄清，可迁移到agent工程实践。
- [Data-aware Static Analysis: Improving Detection of Semantic Faults in Machine Learning Code Using Data Characteristics](http://arxiv.org/abs/2606.09957v1) — 相关度 0.70：面向机器学习代码的数据感知静态分析，通过数据特征检测语义错误，可增强代码分析工具。
- [Agentic Persona Generation with Critique-Refinement: An Industrial Evaluation](http://arxiv.org/abs/2606.09637v1) — 相关度 0.65：基于批判-改进循环的agentic人物生成方法，展示了多agent协作与迭代精化的工作流模式。
- [Anomaly Detection and Root Cause Analysis for Microservice Systems](http://arxiv.org/abs/2606.09942v1) — 相关度 0.60：面向微服务系统的异常检测与RCA方法与基准，可用于运维分析agent。
- [Understanding How Enterprises Adopt the Model Context Protocol for LLM-Driven Software Engineering](http://arxiv.org/abs/2606.09182v1) — 相关度 0.60：对企业MCP采用情况的实证研究，揭示工具集成挑战，对agent工具生态有参考价值。
- [Harness Engineering for Physical AI: Robot Middleware Is the Harness Layer](http://arxiv.org/abs/2606.09416v1) — 相关度 0.50：将语言agent的harness概念扩展到物理AI，提出了投影、隔离、转移三类强制机制，可启发代码agent的边界控制。
