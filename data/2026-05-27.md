...
- [Beyond pass@k: Redundancy-Aware RLVR for Multi-Sample Code Generation](http://arxiv.org/abs/2605.28022v1) — 相关度 0.92：研究多采样代码生成中 Pass@k 与 RLVR 的冗余问题，并引入反冗余奖励，与代码生成评测和优化直接相关。
- [Converted, Not Equivalent: Benchmarking Codebase Conversion via Observational Equivalence](http://arxiv.org/abs/2605.29054v2) — 相关度 0.92：基准化代码库转换的观测等价验证，揭示 agent 自验证偏差，对代码库级 agent 评测与可靠性直接重要。
- [Tool Forge: A Validation-Carrying Toolchain for Governed Agentic Execution](http://arxiv.org/abs/2605.28000v1) — 相关度 0.88：面向 agent 工具层的验证携带 toolchain，覆盖工具生成、沙箱验证、路由与治理，对 tool-use coding agent 工程化有直接价值。
- [Multi-Agent LLM-based Metamorphic Testing for REST APIs](http://arxiv.org/abs/2605.28321v1) — 相关度 0.83：多 agent LLM 工作流做 REST API 蜕变测试，直接属于软件测试 agent 的通用方法。
- [SCDBench: A Benchmark for LLM-Based Smart Contract Decompilers](http://arxiv.org/abs/2605.29059v1) — 相关度 0.82：LLM 智能合约反编译器 benchmark，含编译性、ABI 与语义差分验证，可迁移到代码生成语义一致性评测。
- [Towards Demystifying and Repairing LLM-in-the-Loop Vulnerabilities](http://arxiv.org/abs/2605.28893v2) — 相关度 0.82：系统研究 LLM-in-the-Loop 漏洞并评测 LLM 修复能力，对代码 agent 安全修复和漏洞评测有迁移价值。
- [GUI Agents for Continual Game Generation](http://arxiv.org/abs/2605.28258v1) — 相关度 0.78：用 GUI agent 作为游戏生成的可玩性评估者和 playtester，形成 agentic 生成-评测闭环，对交互式代码生成有借鉴意义。
- [DeltaMCP: Incremental Regeneration via Spec-Aware Transformation for MCP servers](http://arxiv.org/abs/2605.28148v1) — 相关度 0.72：基于 OpenAPI 规范增量再生成 MCP 服务器，面向 agent 工具生态的维护与同步，工程方法可迁移。
- [Do LLMs Favor Their Providers? Measuring Vertical Integration Bias in Code Generation](http://arxiv.org/abs/2605.28515v1) — 相关度 0.72：提出垂直整合偏见 benchmark，度量直接与 agentic 代码生成中的生态偏向，评测设计可迁移。
- [From paper to benchmark: agentic, framework-based reproduction of under-specified methods in machine health intelligence](http://arxiv.org/abs/2605.28371v1) — 相关度 0.72：agent 将论文复现为共享 benchmark 实现，slot-binding 与假设显式化机制可迁移到工程化代码生成与论文到代码任务。
- [Efficient and Scalable Provenance Tracking for LLM-Generated Code Snippets](http://arxiv.org/abs/2605.28510v2) — 相关度 0.65：面向 LLM 生成代码的溯源追踪结合向量检索与指纹，对代码检索/归属/license 合规有参考价值。
- [Confident Learning-based Network for Detecting Bug-Inducing Commits on SZZ with Noisy Labels](http://arxiv.org/abs/2605.27880v1) — 相关度 0.55：软件缺陷引入提交检测结合噪声标签清洗与图语义建模，属于程序分析与缺陷预测的通用方法。
- [Rethinking Software Empirical Studies with Structural Causal Models](http://arxiv.org/abs/2605.28482v1) — 相关度 0.55：用结构因果模型强化软件实证研究，案例为 prompt 策略对代码生成的影响，可作为 agent 评测实验的方法参考。
