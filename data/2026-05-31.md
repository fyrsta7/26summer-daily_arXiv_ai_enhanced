...
- [SABER: Benchmarking Operational Safety of LLM Coding Agents in Stateful Project Workspaces](http://arxiv.org/abs/2606.01317v1) — 相关度 0.95：评测coding agent在有状态项目工作区中的操作安全基准，直接覆盖代码agent安全评估需求。
- [BenchEvolver: Frontier Task Synthesis via Solution-Centric Evolution](http://arxiv.org/abs/2606.01286v1) — 相关度 0.90：通过解演化自动合成更难且可验证的编程任务，破解benchmark饱和，同时提供训练信号，对代码agent评测和训练有直接价值。
- [FVSpec: Real-World Property-Based Tests as Lean Challenges](http://arxiv.org/abs/2606.01008v1) — 相关度 0.90：构建真实属性测试转Lean规范的多agent流水线及基准，直接服务AI辅助形式化验证研究。
- [Reducing Token Usage of State-in-Context Agents using Minification](http://arxiv.org/abs/2606.01326v1) — 相关度 0.90：针对state-in-context agent的源码minification降token方法，直接优化代码agent的输入效率，并公开实现。
- [LLM Consortium for Software Design Refinement: A Controlled Experiment on Multi-Agent Collaboration Topologies](http://arxiv.org/abs/2606.01490v1) — 相关度 0.85：系统比较12种多智能体协作拓扑的受控实验，为代码agent的多agent工作流设计提供实证依据。
- [Bridging Requirements and Architecture: Multi-Agent Orchestration with External Knowledge and Hierarchical Memory](http://arxiv.org/abs/2606.01385v1) — 相关度 0.80：多智能体架构设计框架引入RAG和分层记忆机制，其工作流可迁移到软件架构agent的迭代式设计。
- [ClawHub Security Signals: When VirusTotal, Static Analysis, and SkillSpector Disagree](http://arxiv.org/abs/2606.01494v1) — 相关度 0.75：发布agent技能安全数据集并分析多扫描器分歧，支撑代码agent技能生态的安全评估与治理。
- [From Rocq to Metal: A Pipeline for Formally Verified Microcontroller Firmware](http://arxiv.org/abs/2606.02651v1) — 相关度 0.75：建立形式化验证固件管线并融合LLM辅助tactic合成，对AI生成代码的正确性验证和形式化方法有迁移价值。
- [Needles at Scale: LLM-Assisted Target Selection for Windows Vulnerability Research](http://arxiv.org/abs/2606.01364v1) — 相关度 0.70：面向大规模二进制漏洞研究的LLM目标选择流水线，符号化-富化-采样方法可迁移至代码分析agent的优先级排序。
