...
- [Overeager Coding Agents: Measuring Out-of-Scope Actions on Benign Tasks](http://arxiv.org/abs/2605.18583v1) — 相关度 0.92：构建overeager行为基准并验证测量有效性，是coding agent安全与可靠性评估的直接benchmark。
- [ProcCtrlBench: Evaluating Process-Level Defects and Control Preservation in LLM Coding Agents](http://arxiv.org/abs/2605.20251v4) — 相关度 0.90：新benchmark聚焦LLM编码Agent执行过程的缺陷类型与控制保持，补充结果导向评估的盲区，对agent评测有重要价值。
- [Same Signal, Different Semantics: A Cross-Framework Behavioral Analysis of Software Engineering Agents](http://arxiv.org/abs/2605.18332v1) — 相关度 0.88：基于64k SWE-bench运行分析框架vs LLM对agent行为信号的影响，对跨框架agent测评有直接指导价值。
- [BLAgent: Agentic RAG for File-Level Bug Localization](http://arxiv.org/abs/2605.17965v2) — 相关度 0.87：Agentic RAG用于文件级缺陷定位，结合结构感知检索与两阶段重排，显著提升定位准确率并降低APR成本，直接增强修复agent。
- [Improving BM25 Code Retrieval Under Fixed Generic Tokenization: Adaptive q-Log Odds as a Drop-In BM25 Fix](http://arxiv.org/abs/2605.18561v1) — 相关度 0.85：提出即插即用的q-log IDF改造显著提升代码检索，可直接用于RAG代码agent的上下文召回。
- [PROTEA: Offline Evaluation and Iterative Refinement for Multi-Agent LLM Workflows](http://arxiv.org/abs/2605.18032v1) — 相关度 0.85：提供多Agent LLM工作流的离线评估与迭代优化接口，含反向节点评估与prompt修订，直接用于调试和优化agent流程。
- [Prompt Optimization for LLM Code Generation via Reinforcement Learning](http://arxiv.org/abs/2605.19102v1) — 相关度 0.85：用强化学习结合单测反馈优化代码生成prompt，直接改进LLM代码生成的功能正确性。
- [Lean Refactor: Multi-Objective Controllable Proof Optimization via Agentic Strategy Search](http://arxiv.org/abs/2605.20244v1) — 相关度 0.83：agentic策略搜索实现Lean证明的多目标可控重构，兼顾长度、编译成本与版本兼容，可迁移到代码压缩与版本鲁棒优化。
- [CommitDistill: A Lightweight Knowledge-Centric Memory Layer for Software Repositories](http://arxiv.org/abs/2605.18284v1) — 相关度 0.82：为代码仓库蒸馏结构化知识记忆并通过TF-IDF检索，可直接迁移到coding agent的长期记忆设计。
- [A-ProS: Towards Reliable Autonomous Programming Through Multi-Model Feedback](http://arxiv.org/abs/2605.18073v1) — 相关度 0.81：多模型反馈框架将解题生成与专门调试分离，在有状态迭代下显著提升竞赛编程正确率，体现了多模型协作对agent的价值。
- [Three Heads Are Better Than One: A Multi-perspective Reasoning Framework for Enhanced Vulnerability Detection](http://arxiv.org/abs/2605.18153v1) — 相关度 0.80：多视角推理框架通过多个LLM agent的辩论与纠错提升漏洞检测性能，其协作验证机制可迁移至代码分析与安全审查。
- [Reversa: A Reverse Documentation Engineering Framework for Converting Legacy Software into Operational Specifications for AI Agents](http://arxiv.org/abs/2605.18684v1) — 相关度 0.78：多智能体流水线将遗留代码转为可追踪的规格说明，为agent提供可靠上下文与行为契约。
- [LLM-Based Static Verification of Code Against Natural-Language Requirements: An Industrial Experience Report](http://arxiv.org/abs/2605.17926v1) — 相关度 0.76：两阶段LLM静态验证代码是否满足自然语言需求，通过规则抽取与代码审计降低幻觉，对agent的验证与测试预言机问题有贡献。
- [Supporting System Testing with a Multi-Agent LLM-based Framework for Knowledge Graph Extraction: A Case Study with Ethernet Switch Systems](http://arxiv.org/abs/2605.19180v1) — 相关度 0.75：多agent从技术文档提取知识图谱以支持系统测试，EEI循环可迁移到测试自动化agent。
- [Contextualized Code Pretraining for Code Generation](http://arxiv.org/abs/2605.17957v1) — 相关度 0.72：提出调用上下文感知的代码预训练框架与CallerEval基准，提升真实仓库中代码生成与环境集成能力，对coding agent有参考价值。
- [Verify-Gated Completion as Admission Control in a Governed Multi-Agent Runtime: A Bounded Architecture Case Study](http://arxiv.org/abs/2605.17998v2) — 相关度 0.71：研究多Agent运行时中验证门控的完成准入控制，强调可审计与fail-closed，对agent可靠性设计有方法论迁移价值。
- [LogRouter: Adaptive Two-Level LLM Routing for Log Question Answering in Big Data Systems](http://arxiv.org/abs/2605.18015v1) — 相关度 0.56：面向日志问答的两级LLM路由系统，通过成本感知调度降低延迟并保持质量，对agent推理资源优化有一定参考。
