...
- [A Unified Issue Resolution Benchmark for Requirement Clarification, Planning, and Code Generation for Coding Agents](https://arxiv.org/abs/2608.09072) — 相关度 0.95：提出包含需求澄清、规划与代码生成中间真值标注的Issue解决基准，直接评估代码Agent的关键能力瓶颈。
- [Improving Constraint Models with LLM Agents](https://arxiv.org/abs/2608.08127) — 相关度 0.95：利用LLM Agent自动重构约束模型以大幅提升求解性能，直接属于自动程序优化方向。
- [Ouroboros: A Self-Developing Frontier Coding Agent with Reviewed Core Evolution](https://arxiv.org/abs/2608.08311) — 相关度 0.95：Ouroboros 自进化 coding agent 通过评审提交改进自身工具与上下文，多个基准刷新 SOTA，直接相关。
- [Refining LLM-based Directed Test Input Generation via Runtime Value Feedback](https://arxiv.org/abs/2608.07926) — 相关度 0.88：ReDig 用运行时值反馈循环精化 LLM 的定向测试输入生成，提升目标行可达率，可直接用于测试 agent。
- [AI Evaluation Should Measure Verification Cost, Not Correctness Alone](https://arxiv.org/abs/2608.08709) — 相关度 0.85：提出评估AI生成代码时应考虑验证成本而非仅正确性，对代码Agent的评测与可靠性度量有直接参考价值。
- [DevIntent: How Much Does LLM-Generated Code Violate Developer Intent?](https://arxiv.org/abs/2608.07614) — 相关度 0.85：提出 Intent Violation Rate 与隐藏约束测试基准，揭示 LLM 生成代码违背开发意图的问题，对代码评测有迁移价值。
- [Entropy-based Code Adversarial Translation for Real-world Repository Migration](https://arxiv.org/abs/2608.09273) — 相关度 0.85：提出基于生成器-判别器的多Agent框架进行仓库级代码迁移，涉及长程代码任务与自演化记忆，可迁移至代码Agent。
- [GALA: Graph-Augmented LLM Agents for Root Cause Analysis and Incident Response in Microservices](https://arxiv.org/abs/2608.08968) — 相关度 0.82：GALA+ 用服务依赖图约束 LLM agent 探索，提升微服务根因分析与事件响应，STRIX 评分可迁移。
- [On the Robustness of LLMs' Internal Representation of Code Correctness](https://arxiv.org/abs/2608.08266) — 相关度 0.80：系统分析 LLM 内部表示对代码正确性的信号鲁棒性，为无执行代码评测提供重要发现。
- [Software Engineering for and with GUI Agent](https://arxiv.org/abs/2608.09278) — 相关度 0.72：GUI agent 软件工程综述，梳理架构与评测缺口，其生命周期与可靠性观点可迁移到 coding agent 工程化。
- [MetaSpace: Metamorphic Testing for Spatial Cognition in Embodied Agents](https://arxiv.org/abs/2608.07533) — 相关度 0.60：提出基于蜕变测试的智能体空间认知评估框架，方法可迁移至代码Agent的验证与测试。
- [Verication-driven closed-loop multi-agent large language modelframework for code-compliant structural design](https://arxiv.org/abs/2608.07978) — 相关度 0.60：闭环多 agent 加外部物理验证器修复代码合规问题，验证器反馈机制可迁移到代码 agent 的验证循环。
