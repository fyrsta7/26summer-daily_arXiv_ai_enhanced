...
- [Empowering Autonomous Debugging Agents with Efficient Dynamic Analysis](http://arxiv.org/abs/2604.24212v1) — 相关度 0.96：为调试agent设计基于函数级执行轨迹的高效动态分析接口，在SWE-bench上显著提升修复率并降低成本，直接相关。
- [Constraint-Guided Multi-Agent Decompilation for Executable Binary Recovery](http://arxiv.org/abs/2604.23940v2) — 相关度 0.94：多agent反编译，使用约束引导验证，对代码生成和验证有直接价值。
- [Learning from Execution: Self-Evolving Memory for Private-Library Code Generation](http://arxiv.org/abs/2604.24222v3) — 相关度 0.93：针对私有库代码生成的自演化记忆框架，将执行反馈组织为多层级使用指南，直接增强RAG代码生成。
- [BenchGuard: Who Guards the Benchmarks? Automated Auditing of LLM Agent Benchmarks](http://arxiv.org/abs/2604.24955v1) — 相关度 0.90：提出BenchGuard自动化审计LLM agent benchmark，直接针对agent评估基础设施，对构建可靠benchmark有重要价值。
- [Context-Augmented Code Generation: How Product Context Improves AI Coding Agent Decision Compliance by 49%](http://arxiv.org/abs/2605.08112v1) — 相关度 0.90：研究产品上下文检索对AI coding agent决策合规性的提升，直接涉及代码agent的上下文增强与测评。
- [AgentPulse: A Continuous Multi-Signal Framework for Evaluating AI Agents in Deployment](http://arxiv.org/abs/2604.24038v1) — 相关度 0.88：多信号持续评估agent部署表现，方法论对agent评测有迁移价值。
- [Mono2Sls: Automated Monolith-to-Serverless Migration via Multi-Stage Pipeline with Static Analysis](http://arxiv.org/abs/2604.24550v1) — 相关度 0.86：多阶段静态分析加多LLM agent的自动迁移管线，agent协作、中间产物与验证机制可直接借鉴。
- [FGDM: Reasoning Aware Multi-Agentic Framework for Software Bug Detection using Chain of Thought and Tree of Thought Prompting](http://arxiv.org/abs/2604.24831v1) — 相关度 0.85：提出基于CoT/ToT的多智能体框架用于软件bug检测与修复，直接涉及代码agent的推理与修复机制。
- [RefEvo: Agentic Design with Co-Evolutionary Verification for Agile Reference Model Generation](http://arxiv.org/abs/2604.24218v1) — 相关度 0.85：多agent动态规划与协同进化验证的LLM代码生成框架，验证与上下文压缩机制可迁移到硬件或软件代码生成agent。
- [SWE-QA: A Dataset and Benchmark for Complex Code Understanding](http://arxiv.org/abs/2604.24814v1) — 相关度 0.85：构建多跳代码理解数据集与评测基准，直接面向复杂代码理解与模型评测，可用于代码agent推理与检索能力评估。
- [Measuring the Unmeasurable: Markov Chain Reliability for LLM Agents](http://arxiv.org/abs/2604.24579v1) — 相关度 0.84：用马尔可夫链统一agent执行轨迹可靠性指标并给出拟合与不确定性检验，可迁移到代码agent评测。
- [MAS-SZZ: Multi-Agentic SZZ Algorithm for Vulnerability-Inducing Commit Identification](http://arxiv.org/abs/2604.24398v1) — 相关度 0.82：多agent协作的漏洞引入提交定位算法，将agent用于代码历史与变更分析，对安全分析agent有迁移价值。
- [Defective Task Descriptions in LLM-Based Code Generation: Detection and Analysis](http://arxiv.org/abs/2604.24703v1) — 相关度 0.80：检测任务描述缺陷的轻量分类器并分析缺陷对LLM代码生成的影响，可直接提升代码生成agent的输入鲁棒性。
- [Leveraging LLMs for Multi-File DSL Code Generation: An Industrial Case Study](http://arxiv.org/abs/2604.24678v1) — 相关度 0.80：工业DSL多文件代码生成的LLM适配与评测案例，涉及结构化表示、微调与执行验证，对代码生成agent有迁移价值。
- [Vulnerability Identification by Harnessing Inter-connected Multi-Source Information](http://arxiv.org/abs/2604.24028v2) — 相关度 0.80：利用多源信息识别漏洞，对程序分析和漏洞检测有参考价值。
- [When Prompt Under-Specification Improves Code Correctness: An Exploratory Study of Prompt Wording and Structure Effects on LLM-Based Code Generation](http://arxiv.org/abs/2604.24712v1) — 相关度 0.80：研究提示词欠规格对LLM代码生成正确性的影响，提供写鲁棒prompt的实践经验，对代码生成agent有参考价值。
- [Evaluation of LLM-Based Software Engineering Tools: Practices, Challenges, and Future Directions](http://arxiv.org/abs/2604.24621v1) — 相关度 0.78：系统讨论LLM软件工程工具评测的挑战与方向，对code agent benchmark设计与评估有指导价值。
- [Evaluating Cryptographic API Misuse Detectors for Go](http://arxiv.org/abs/2604.24085v1) — 相关度 0.75：Go加密API误用检测器评估，提供分类法和实证，对测试工具开发有参考价值。
- [Large Language Models for Multilingual Code Intelligence: A Survey](http://arxiv.org/abs/2604.25960v1) — 相关度 0.75：综述多语言代码生成与翻译，涉及跨语言泛化，对多语言代码agent有直接参考价值。
- [Understanding the Limits of Automated Evaluation for Code Review Bots in Practice](http://arxiv.org/abs/2604.24525v1) — 相关度 0.75：工业环境下自动化代码评审bot的评估局限研究，对LLM代码评审agent的评测方法与标签设计有直接参考价值。
- [Dont Stop Early: Scalable Enterprise Deep Research with Controlled Information Flow and Evidence-Aware Termination](http://arxiv.org/abs/2604.24978v1) — 相关度 0.70：提出企业深度研究agent架构，包含信息流控制和证据感知终止，这些工作流机制可迁移到长程代码agent。
- [Toward a Science of Intent: Closure Gaps and Delegation Envelopes for Open-World AI Agents](http://arxiv.org/abs/2604.25000v2) — 相关度 0.65：提出闭包差距与委托包络概念，为开放世界agent的验证与部署提供理论框架，可启发代码agent的规划。
- [On the Footprints of Reviewer Bots Feedback on Agentic Pull Requests in OSS GitHub Repositories](http://arxiv.org/abs/2604.24450v1) — 相关度 0.62：实证分析reviewer bot对agentic PR结果的影响，为coding agent工作流中的评审反馈策略提供参考依据。
- [From Heuristics to Transformers: A Comprehensive Survey of Type Inference from Stripped Binaries](http://arxiv.org/abs/2606.23692v1) — 相关度 0.55：二进制类型推断综述，虽偏逆向工程，但其中神经符号方法可启发代码理解与agent的语义分析。
