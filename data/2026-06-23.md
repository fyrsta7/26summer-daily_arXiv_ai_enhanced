...
- [Agentic Generation of AST Transformation Rules for Fixing Breaking Updates](http://arxiv.org/abs/2606.24446v1) — 相关度 0.90：用 agent 生成可跨项目迁移的 AST 变换规则修复编译失败，直接属于代码修复 agent 的高价值方法。
- [EvoClawBench: Can Agents Learn Reusable Skills from Their Own Runs?](http://arxiv.org/abs/2607.09711v1) — 相关度 0.85：提出衡量 agent 从自身运行学习可复用技能的 benchmark，覆盖编码任务并揭示技能 authoring 的选择性收益，直接适用于编码 agent。
- [AutoSpec: Safety Rule Evolution for LLM Agents via Inductive Logic Programming](http://arxiv.org/abs/2606.24245v3) — 相关度 0.80：通过反例引导归纳逻辑编程自动演化 LLM agent 安全规则，显著降低误报并保持可解释性，适用于代码执行 agent 的安全约束。
- [Test-Input Generation for Tensor Programs: What Actually Finds Kernel Bugs](http://arxiv.org/abs/2606.27396v1) — 相关度 0.80：在 26 个 tensor kernel 上对比七种测试输入生成策略的 bug 召回与误报，为测试 agent 的策略选择提供实证依据。
- [The Verifier is the Curriculum: Execution-Gated Self-Distillation for Cross-Family Game Generation](http://arxiv.org/abs/2607.09709v1) — 相关度 0.80：以严格启动作为不可欺骗的验证门控进行自蒸馏，验证了验证器精度决定生成模型学习收益，可迁移到代码生成 agent 训练。
- [Detecting AI Coding Agents in Open Source: A Validated Multi-Method Census of 180 Million Repositories](http://arxiv.org/abs/2606.24429v1) — 相关度 0.70：提出多方法检测开源 AI 编码 agent 的框架并给出验证统计，为编码 agent 生态感知与评测提供基础数据。
- [LLM4MTLs: Automated Generation and Empirical Evaluation of Model Transformation Languages](http://arxiv.org/abs/2606.25193v1) — 相关度 0.70：系统评估 LLM 生成模型转换语言的提示策略与语法/语义质量，对代码生成 agent 的提示工程有参考价值。
- [MANGO: Automated Multi-Agent Test Oracle Generation for Vision-Language-Action Models](http://arxiv.org/abs/2606.24815v1) — 相关度 0.65：多 agent 自动生成细粒度测试预言用于机器人模型测试，其 oracle 生成与多角色精化机制可迁移到代码 agent 测试。
- [Agon: An Autonomous Large-Scale Omnidisciplinary Research System Built on Prompt Economy](http://arxiv.org/abs/2606.24177v1) — 相关度 0.60：提出基于 Prompt Economy 的自主研究编排系统与失败分类法，其循环验证与人工接管机制对 agent 工作流有参考价值。
- [Bit-Precise Conformance Testing of Simulink Model Checkers](http://arxiv.org/abs/2606.24719v2) — 相关度 0.60：对 Simulink 模型检查器做位精确一致性测试，其一致性测试方法对验证工具可靠性和测试生成有参考价值。
