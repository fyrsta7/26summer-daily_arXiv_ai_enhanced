...
- [SelfHeal: Empirical Fix Pattern Analysis and Bug Repair in LLM Agents](http://arxiv.org/abs/2604.17699v1) — 相关度 0.95：首个LLM agent错误修复模式研究与SelfHeal多agent修复系统，高度直接相关。
- [SolidCoder: Bridging the Mental-Reality Gap in LLM Code Generation through Concrete Execution](http://arxiv.org/abs/2604.19825v1) — 相关度 0.95：直接针对LLM代码生成中心智模拟与真实执行的差距，通过执行落地提升pass@1，对coding agent的验证机制高度相关。
- [JTPRO: A Joint Tool-Prompt Reflective Optimization Framework for Language Agents](http://arxiv.org/abs/2604.19821v1) — 相关度 0.90：联合优化工具提示与模式描述的反思式框架，直接提升语言agent工具调用可靠性。
- [MASFuzzer: Fuzz Driver Generation and Adaptive Scheduling via Multidimensional API Sequences](http://arxiv.org/abs/2604.17977v1) — 相关度 0.90：LLM生成fuzz driver并结合自适应调度，直接贡献于自动化测试和漏洞发现。
- [OpenGame: Open Agentic Coding for Games](http://arxiv.org/abs/2604.18394v1) — 相关度 0.90：提出端到端agentic游戏生成框架与执行接地RL、VLM评测，直接面向agent长程代码生成与验证。
- [Structural Verification for Reliable EDA Code Generation without Tool-in-the-Loop Debugging](http://arxiv.org/abs/2604.18834v1) — 相关度 0.90：通过结构依赖图和预执行验证消除EDA代码生成中的tool-in-loop调试，直接提升长程代码生成的可靠性与效率。
- [WebCompass: Towards Multimodal Web Coding Evaluation for Code Language Models](http://arxiv.org/abs/2604.18224v1) — 相关度 0.90：提出面向网页编码的多模态评测基准与Agent-as-a-Judge机制，是agent评测方法的重要补充。
- [CodePivot: Bootstrapping Multilingual Transpilation in LLMs via Reinforcement Learning without Parallel Corpora](http://arxiv.org/abs/2604.18027v1) — 相关度 0.85：利用Python中间表示和强化学习实现多语言代码转换，可迁移到代码生成agent。
- [From Program Slices to Causal Clarity: Evaluating Faithful, Actionable LLM-Generated Failure Explanations via Context Partitioning and LLM-as-a-Judge](http://arxiv.org/abs/2604.18309v2) — 相关度 0.85：系统评估上下文配置对LLM生成失败解释质量的影响，为调试agent的输入设计与因果解释评测提供方法。
- [Program Structure-aware Language Models: Targeted Software Testing beyond Textual Semantics](http://arxiv.org/abs/2604.17715v1) — 相关度 0.85：结合代码属性图与LLM进行分支目标测试生成，提升测试覆盖和漏洞发现，可迁移到测试agent。
- [TypeScript Repository Indexing for Code Agent Retrieval](http://arxiv.org/abs/2604.18413v2) — 相关度 0.85：面向TypeScript仓库的图索引优化，直接改进代码agent的代码库检索与上下文获取效率。
- [Scaling Human-AI Coding Collaboration Requires a Governable Consensus Layer](http://arxiv.org/abs/2604.17883v1) — 相关度 0.80：提出属性图共识层作为AI编码协作的主工件，为agent工作流治理提供新范式。
- [Cache-Related Smells in GitLab CI/CD: Comprehensive Catalog, Automated Detection, and Empirical Evidence](http://arxiv.org/abs/2604.17890v1) — 相关度 0.75：CI/CD缓存坏气味目录与自动检测工具，可用于构建性能优化agent。
- [Co-Located Tests, Better AI Code: How Test Syntax Structure Affects Foundation Model Code Generation](http://arxiv.org/abs/2604.19826v1) — 相关度 0.70：实证研究测试语法结构对AI代码生成质量的影响，为代码agent的测试与代码协同生成提供可操作结论。
- [Reliability of AI Bots Footprints in GitHub Actions CI/CD Workflows](http://arxiv.org/abs/2604.18334v1) — 相关度 0.70：研究GitHub Actions中AI bot的CI/CD可靠性并定义失败分类，对agent在真实工作流中的可靠性评估有迁移价值。
- [Revisiting Code Debloating with Ground Truth-based Evaluation](http://arxiv.org/abs/2604.17717v2) — 相关度 0.70：基于ground-truth的程序精简评估，为自动程序变换与优化提供基准方法。
- [LeGo-Code: Can Modular Curriculum Learning Advance Complex Code Generation? Insights from Text-to-SQL](http://arxiv.org/abs/2604.18254v1) — 相关度 0.65：研究模块化课程学习提升Text-to-SQL代码生成，训练策略可迁移到代码LLM的agent能力构建。
- [Towards an Agentic LLM-based Approach to Requirement Formalization from Unstructured Specifications](http://arxiv.org/abs/2604.18228v1) — 相关度 0.65：用agentic LLM流水线从非结构化需求提取形式化属性，可迁移到需求工程类agent的规划与验证。
- [Choose Your Own Adventure: Non-Linear AI-Assisted Programming with EvoGraph](http://arxiv.org/abs/2604.18883v1) — 相关度 0.60：以开发图为交互模型回溯AI辅助编程历史，可用于设计agent的非线性探索与变更管理界面。
- [Raven: Rethinking Automated Assessment for Scratch Programs via Video-Grounded Evaluation](http://arxiv.org/abs/2604.17820v1) — 相关度 0.60：视频验证的自动化程序评估框架，其无显式测试的验证思路对agent验证有参考价值。
- [Stateful Embedded Fuzzing with Peripheral-Accurate SystemC Virtual Prototypes](http://arxiv.org/abs/2604.19824v1) — 相关度 0.60：提出结合SystemC虚拟原型的嵌入式模糊测试框架，属于程序测试方法，对代码agent的测试反馈机制有参考价值。
- [Statistical Software Engineering with Tuned Variables](http://arxiv.org/abs/2604.19822v1) — 相关度 0.60：提出调优变量与统计发布门控概念，对AI系统调优和治理有参考价值。
- [Towards Better Static Code Analysis Reports: Sentence Transformer-based Filtering of Non-Actionable Alerts](http://arxiv.org/abs/2604.18525v1) — 相关度 0.60：利用sentence transformer过滤静态分析不可操作告警，可迁移到agent代码审查和告警优先级排序。
- [More Is Different: Toward a Theory of Emergence in AI-Native Software Ecosystems](http://arxiv.org/abs/2604.19827v1) — 相关度 0.50：从复杂自适应系统理论刻画多agent软件生态的涌现与失效，为agent协作可靠性和治理提供概念框架。
