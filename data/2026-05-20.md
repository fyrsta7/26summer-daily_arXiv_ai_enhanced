...
- [SpecBench: Measuring Reward Hacking in Long-Horizon Coding Agents](http://arxiv.org/abs/2605.21384v1) — 相关度 0.95：SpecBench量化长程coding agent的奖励黑客行为，分解可见/保留测试来衡量真实目标达成，评测价值极高
- [Agentic Model Checking](http://arxiv.org/abs/2605.21434v1) — 相关度 0.90：提出agentic model checking范式，将LLM agent与有界模型检查结合用于验证LLM生成代码，直接相关于代码agent的可靠性与验证。
- [Articulate but Wrong: Self-Review Failures in LLM-Based Code Modernization](http://arxiv.org/abs/2605.21537v1) — 相关度 0.90：实证揭示LLM代码现代化中自审查不可靠，对coding agent的正确性验证与安全网设计有直接价值
- [Insights Generator: Systematic Corpus-Level Trace Diagnostics for LLM Agents](http://arxiv.org/abs/2605.21347v3) — 相关度 0.90：提出语料级轨迹诊断的多agent系统，自动生成带证据的洞察，直接支持coding agent调试与改进
- [Domain-Adaptable Reinforcement Learning for Code Generation with Dense Rewards](http://arxiv.org/abs/2605.21180v1) — 相关度 0.85：用域自适应RL与密集奖励微调代码生成LLM，涵盖语法、正确性和可执行性，对代码生成优化有直接价值
- [FuzzingBrain V2: A Multi-Agent LLM System for Automated Vulnerability Discovery and Reproduction](http://arxiv.org/abs/2605.21779v1) — 相关度 0.85：多智能体LLM系统FuzzingBrain V2实现自动化漏洞发现与复现，基于OSS-Fuzz验证并发现29个零日漏洞，对安全测试agent有直接价值。
- [Stdlib or Third-Party? Empirical Performance and Correctness of LLM-Assisted Zero-Dependency Python Libraries](http://arxiv.org/abs/2605.21405v1) — 相关度 0.85：对LLM生成的零依赖stdlib重实现进行性能与正确性实证，显示多数场景性能持平并存在5-115x加速，对代码生成与性能评测有直接参考。
- [A Dataset of Reproducible Flaky-Test Failures](http://arxiv.org/abs/2605.21677v1) — 相关度 0.80：提供1115个可复现flaky test的数据集，含复现脚本、修复脚本及失败日志，直接用于测试/调试agent的评测与训练。
- [Quality-Assured Fuzz Harness Generation via the Four Principles Framework](http://arxiv.org/abs/2605.21824v1) — 相关度 0.80：基于四原则框架的LLM fuzz harness自动生成与质量保证，内置正确性检查拦截58个假阳性崩溃，提升agent生成harness可靠性。
- [ASSEMBLAGE-DEEPHISTORY: A Cross-Build Binary Dataset with Temporal Coverage](http://arxiv.org/abs/2605.21615v1) — 相关度 0.75：构建含时间覆盖的跨构建二进制数据集，并用LLM基准评估二进制漏洞推理，为程序分析/安全agent提供评测语料。
- [Governance by Construction for Generalist Agents](http://arxiv.org/abs/2605.20874v1) — 相关度 0.70：策略即代码的agent治理架构，包含意图拦截、HITL和工具边界控制，可迁移至coding agent的可靠性约束
- [Quality and Security Signals in AI-Generated Python Refactoring Pull Requests](http://arxiv.org/abs/2605.21453v1) — 相关度 0.70：实证研究AI生成Python重构PR的质量与安全信号，识别24种变更操作及lint/security影响，可用于agent重构效果评测。
- [The Constraint Tax: Measuring Validity-Correctness Tradeoffs in Structured Outputs for Small Language Models](http://arxiv.org/abs/2605.26128v1) — 相关度 0.70：测量结构化输出约束对SLM准确性的影响，工具调用schema约束分析可直接迁移到代码agent
- [An Event-Driven Tool for Context-Aware Code Smell Detection Using SmellDSL](http://arxiv.org/abs/2605.20675v1) — 相关度 0.60：基于SmellDSL的上下文感知代码异味检测和分析方法，对代码质量工具与agent可参考
- [Benchmarking and Improving Monitors for Out-Of-Distribution Alignment Failure in LLMs](http://arxiv.org/abs/2605.21602v2) — 相关度 0.60：提出OOD对齐失败检测基准MOOD及guard model+OOD detector组合方法，可迁移到coding agent的可靠性监控。
- [BioDefect: The First Dataset for Defect Detection in Bioinformatics Software](http://arxiv.org/abs/2605.20788v1) — 相关度 0.60：首个生物信息学软件缺陷检测数据集，提供带完整上下文的缺陷样本，可用于缺陷检测模型评测
- [RSE of a Quantum Transport Code and its Effects](http://arxiv.org/abs/2605.21334v1) — 相关度 0.60：连续集成、自动化测试与持续基准实践，暴露缺陷并检测性能回归，对科学软件质量保证有参考价值
