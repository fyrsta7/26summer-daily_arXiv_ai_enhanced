...
Comments: Final accepted manuscript. 12 pages, 13 figures

Journal reference: Proceedings of the 40th IEEE/ACM International Conference on Automated Software Engineering (ASE), Seoul, Korea, Republic of, 2025, pp. 2338-2349

DOI: [10.1109/ASE63991.2025.00193](https://doi.org/10.1109/ASE63991.2025.00193)

Author affiliations: Akira Mori: National Institute of Advanced Industrial Science and Technology; Masatomo Hashimoto: Chiba Institute of Technology
...
- [PERFOPT-Bench: Evaluating Coding Agents on Software Performance Optimization](https://arxiv.org/abs/2607.07744) — 相关度 0.97：专门评测编码智能体在性能优化任务上的基准，涵盖profile、诊断、修改、验证完整回路，直接对应SemOpt研究。
- [TTHE: Test-Time Harness Evolution](https://arxiv.org/abs/2607.08124) — 相关度 0.95：将 harness 视为测试时可进化程序，用执行轨迹无监督优化 agent 流程，直接面向代码 agent 的自适应改进。
- [DeepSWE: Measuring Frontier Coding Agents on Original, Long-Horizon Engineering Tasks](https://arxiv.org/abs/2607.07946) — 相关度 0.91：提出原创长周期软件工程任务基准并用手写验证器评测，避免预训练污染，对编码智能体评测有重要价值。
- [Bug Report Specification Refinement with Trajectory Guidance for Automated Program Repair](https://arxiv.org/abs/2607.07882) — 相关度 0.87：通过轨迹引导细化bug报告规格，显著提升多个APR智能体的修复率，对智能体输入优化有直接价值。
- [From Prompts to Contracts: Harness Engineering for Auditable Enterprise LLM Agents](https://arxiv.org/abs/2607.08028) — 相关度 0.85：用 harness、契约、校验构件将 LLM agent 审计化，对工程化 agent 的可靠落地与模型替换有强参考价值。
- [ProjAgent: Procedural Similarity Retrieval for Repository-Level Code Generation](https://arxiv.org/abs/2607.08691) — 相关度 0.85：提出过程相似性检索与静态分析反馈修复，用于仓库级代码生成，直接提升 agent 代码生成能力。
- [Functional and Secure Code Generation with Task Vectors](https://arxiv.org/abs/2607.07881) — 相关度 0.70：利用任务向量提升代码生成的功能性与安全性，无需解码修改，可迁移到编码LLM的安全增强。
- [REFORGE: A Method for Benchmarking LLMs' Reverse Engineering Capabilities in Decompiled Binary Function Naming](https://arxiv.org/abs/2607.07738) — 相关度 0.60：反编译函数命名基准强调对齐不确定性，对代码理解agent评测有参考价值。
- [3100 Opinions on Code Review in an AI World: Building Causal Theory from Practitioner Discourse](https://arxiv.org/abs/2607.07980) — 相关度 0.55：对 AI 生成 PR 的代码审查机制构建因果理论，可为代码 agent 的评审反馈与审查流程设计提供参考。
- [Aleena: Alignment Agent for Research Software Engineering Collaborations](https://arxiv.org/abs/2607.08043) — 相关度 0.55：面向研究软件工程协作的生命周期对齐 agent，其项目状态追踪与决策连续性机制可迁移到多 agent 协作场景。
- [Workflow as Knowledge: Semantic Persistence for LLM-Mediated Workflows](https://arxiv.org/abs/2607.08740) — 相关度 0.50：将工作流建模为可检查、可恢复的知识对象，为代码 agent 工作流持久化与审计提供概念框架。
