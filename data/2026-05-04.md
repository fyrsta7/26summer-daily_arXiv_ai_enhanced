...
- [AOCI: Symbolic-Semantic Indexing for Practical Repository-Scale Code Understanding with LLMs](http://arxiv.org/abs/2605.02421v1) — 相关度 0.90：提出仓库级符号-语义索引，为LLM提供结构化蓝图，直接提升代码理解与agent任务效果。
- [ARIADNE: Agentic Reward-Informed Adaptive Decision Exploration via Blackboard-Driven MCTS for Competitive Program Generation](http://arxiv.org/abs/2605.02431v1) — 相关度 0.90：基于黑板MCTS的竞争程序生成框架，融合执行反馈与代码修复，直接可迁移至coding agent。
- [ARISE: A Repository-level Graph Representation and Toolset for Agentic Program Repair and Fault Localization](http://arxiv.org/abs/2605.03117v2) — 相关度 0.90：仓库级数据流图与切片工具用于agent化程序修复，提升定位精度，直接适用于SWE任务。
- [Kerncap: Automated Kernel Extraction and Isolation for AMD GPUs](http://arxiv.org/abs/2605.03208v2) — 相关度 0.90：自动提取并隔离GPU内核，加速调优循环，为autotuning agent提供基础工具。
- [TSCG: Deterministic Tool-Schema Compilation for Agentic LLM Deployments](http://arxiv.org/abs/2605.04107v1) — 相关度 0.87：确定性工具模式编译显著提升小型模型工具调用成功率，对agent的工具反馈与部署有直接价值。
- [HEJ-Robust: A Robustness Benchmark for LLM-Based Automated Program Repair](http://arxiv.org/abs/2605.02215v3) — 相关度 0.85：构建程序修复鲁棒性基准，揭示LLM修复模型对语法变体的脆弱性，直接指导评测。
- [Learning Correct Behavior from Examples: Validating Sequential Execution in Autonomous Agents](http://arxiv.org/abs/2605.03159v1) — 相关度 0.85：从少量轨迹学习正确行为的agent验证算法，适用于UI、代码生成等序列执行验证。
- [LLM-Assisted Repository-Level Generation with Structured Spec-Driven Engineering](http://arxiv.org/abs/2605.02455v1) — 相关度 0.82：结构化规格驱动工程范式，通过可验证的规格引导仓库级代码生成，对agent可靠性有迁移价值。
- [Terminus-4B: Can a Smaller Model Replace Frontier LLMs at Agentic Execution Tasks?](http://arxiv.org/abs/2605.03195v2) — 相关度 0.82：微调小模型作为终端执行子agent，减少主agent token消耗且性能不降，直接相关agent架构。
- [ARIS: Autonomous Research via Adversarial Multi-Agent Collaboration](http://arxiv.org/abs/2605.03042v1) — 相关度 0.80：自主研究的多agent对抗协作与证据审计框架，对代码agent的验证与多agent协作有借鉴。
- [AutoResearch: An Execution-Grounded Multi-Agent Framework for Reliable Research Workflow Automation](http://arxiv.org/abs/2607.02520v1) — 相关度 0.80：执行接地多Agent研究自动化框架，集成代码执行、修复、引用验证和决策控制，可迁移到代码Agent。
- [DocSync: Agentic Documentation Maintenance via Critic-Guided Reflexion](http://arxiv.org/abs/2605.02163v1) — 相关度 0.80：结合AST与RAG的Agentic文档维护，并采用Critic-guided反思循环，方法可直接迁移到代码Agent。
- [EvoPoC: Automated Exploit Synthesis for DeFi Smart Contracts via Hierarchical Knowledge Graphs](http://arxiv.org/abs/2605.02868v1) — 相关度 0.80：利用层次知识图谱与SMT验证的智能合约漏洞利用合成agent，结构化推理机制可迁移。
- [SCRIBE: Practical Static Binary Patching via Binary-Aware Recompilation of Decompiled Code](http://arxiv.org/abs/2605.02121v1) — 相关度 0.80：通过二进制感知重编译实现静态补丁，LLM可全自动补丁成功率100%，对自动修复有直接价值。
- [AI-Generated Smells: An Analysis of Code and Architecture in LLM and Agent-Driven Development](http://arxiv.org/abs/2605.02741v1) — 相关度 0.75：系统审计AI生成代码的技术债与架构衰减，揭示体积-质量逆律，可指导代码agent的生成质量控制。
- [Beyond Translation Accuracy: Addressing False Failures in LLM-Based Code Translation](http://arxiv.org/abs/2605.02195v3) — 相关度 0.70：系统分析LLM代码翻译中的评估假失败，为翻译任务评测标准提供重要参考。
- [CommitSuite: A Comprehensive Benchmark for Commit Classification and Message Generation](http://arxiv.org/abs/2605.02256v1) — 相关度 0.70：提供大规模Commit分类与消息生成基准，附带语义标注和免参考评估，可直接用于SE任务评测。
- [Standing on the Shoulders of Giants: Stabilized Knowledge Distillation for Cross--Language Code Clone Detection](http://arxiv.org/abs/2605.02860v1) — 相关度 0.70：用于跨语言克隆检测的知识蒸馏与响应稳定化方法，可迁移至代码agent的输出可靠性。
- [A Validated Prompt Bank for Malicious Code Generation: Separating Executable Weapons from Security Knowledge in 1,554 Consensus-Labeled Prompts](http://arxiv.org/abs/2605.03179v1) — 相关度 0.65：恶意代码提示词库与武器-知识分类轴，为代码生成安全评测提供可靠性基准。
- [Causal Software Engineering: A Vision and Roadmap](http://arxiv.org/abs/2605.02454v1) — 相关度 0.60：提出因果软件工程愿景，因果推理可启发代码agent的规划与反事实调试，但为概念性贡献。
- [The Conversations Beneath the Code: Triadic Data for Long-Horizon Software Engineering Agents](http://arxiv.org/abs/2605.02244v1) — 相关度 0.60：提出长程软件工程Agent训练数据的三方数据框架，对Agent数据构建有启发。
- [These Aren't the Reviews You're Looking For How Humans Review AI-Generated Pull Requests](http://arxiv.org/abs/2605.02273v1) — 相关度 0.60：分析AI生成PR的人类评审行为，对理解Agent工作流中人工监督机制有参考价值。
