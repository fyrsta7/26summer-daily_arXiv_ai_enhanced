...
- [From Patches to Trajectories: Privileged Process Supervision for Software-Engineering Agents](http://arxiv.org/abs/2605.21996v1) — 相关度 0.95：利用开发者参考补丁监督SWE agent轨迹构建，在SWE-bench Verified上Pass@1提升10.8点并降低约15%推理成本，直接核心相关。
- [SWE-Mutation: Can LLMs Generate Reliable Test Suites in Software Engineering?](http://arxiv.org/abs/2605.22175v1) — 相关度 0.90：SWE-Mutation基准通过系统性变异评估LLM生成测试套件的判别力，并设计agentic变异框架，直接服务于SWE agent的测试与评估。
- [VeriScale: Adversarial Test-Suite Scaling for Verifiable Code Generation](http://arxiv.org/abs/2605.22368v1) — 相关度 0.90：提出对抗式测试集扩展与精简框架，直接提升代码生成benchmark的判别力与可验证性。
- ["Refactoring Runaway": Understanding and Mitigating Tangled Refactorings in Coding Agents for Issue Resolution](http://arxiv.org/abs/2605.22526v1) — 相关度 0.85：实证分析并缓解coding agent的tangled refactoring，提升可编译性与问题解决率。
- [A measurement substrate for agentic Kubernetes operations: Methodology and a case study in retrieval-compounding falsification](http://arxiv.org/abs/2605.23058v1) — 相关度 0.80：为Kubernetes运维agent构建可证伪的测量框架，包含对照控制与预注册决策矩阵，方法论可迁移。
- [Automated Repair of TEE Partitioning Issues via DSL-Guided and LLM-Assisted Patching](http://arxiv.org/abs/2605.22087v1) — 相关度 0.80：DSL引导+LLM辅助的TEE分区问题自动修复框架，生成测试客户端验证修复，修复成功率达87.6%，对自动修复agent有参考。
- [Finding Missing Input Validation in TEEs via LLM-Assisted Symbolic Execution](http://arxiv.org/abs/2605.22058v1) — 相关度 0.80：LLM辅助符号执行框架SymTEE，自动将TEE代码切片转换为KLEE可用的mock环境，以低成本检测输入验证缺失，方法可迁移到其他验证任务。
- [More Skills, Worse Agents? Skill Shadowing Degrades Performance When Expanding Skill Libraries](http://arxiv.org/abs/2605.24050v2) — 相关度 0.80：揭示技能库扩大导致agent性能下降的skill shadowing效应，提出分解与上界分析方法。
- [Why Are Agentic Pull Requests Merged or Rejected? An Empirical Study](http://arxiv.org/abs/2605.22534v1) — 相关度 0.80：揭示agentic PR合并/拒绝结果不能直接反映agent能力，提出交互感知评估方法。
- [Benchmarking Autonomous Agents against Temporal, Spatial, and Semantic Evasions](http://arxiv.org/abs/2605.22321v1) — 相关度 0.75：提出时间/空间/语义三类规避攻击并构建A3S-Bench基准，系统评估自主agent的安全脆弱性，可迁移到coding agent的防御设计。
- [PITMuS: A Tool for Automated Bug Dataset Generation via Source-Level Mutant Reconstruction](http://arxiv.org/abs/2605.21930v1) — 相关度 0.75：PITMuS通过变异重建生成源码级bug数据集，输出可执行的buggy/fix代码对，为修复agent训练与评测提供新数据源。
- [Security of LLM-generated Code: A Comparative Analysis](http://arxiv.org/abs/2605.23091v1) — 相关度 0.75：系统比较多个LLM生成代码的安全性，为agent输出验证与测评提供实证依据。
- [SynAE: A Framework for Measuring the Quality of Synthetic Data for Tool-Calling Agent Evaluations](http://arxiv.org/abs/2605.22564v1) — 相关度 0.75：为tool-calling agent合成数据质量提供多轴评估框架，可用于评测基准构建。
- [Contractual Skills: A GovernSpec Design Framework for Enterprise AI Agents](http://arxiv.org/abs/2605.22634v2) — 相关度 0.70：为企业AI agent设计契约式技能框架，显式化任务边界与验收标准，提升可靠性与治理。
- [LLM Code Smells: A Taxonomy and Detection Approach](http://arxiv.org/abs/2605.22976v1) — 相关度 0.70：提出LLM code smells分类与静态检测工具，帮助提升LLM集成代码质量。
- [Philosophical Dispositions as Behavioral Constraints for AI-Assisted Code Review: An Empirical Study](http://arxiv.org/abs/2605.23108v1) — 相关度 0.70：用哲学倾向约束AI代码审查行为，实证显示能产生独特且有效的审查发现，可用于评审agent设计。
- [Sibyl-AutoResearch: Autonomous Research Needs Self-Evolving Trial-and-Error Harnesses, Not Paper Generators](http://arxiv.org/abs/2605.22343v1) — 相关度 0.70：自进化AutoResearch框架通过试错harness和行为转换实现自我改进，其记忆、失败登记、系统修复等机制可迁移到长程coding agent。
- [Finding Performance Issues in Database Systems by Exploiting Dormant Code Paths](http://arxiv.org/abs/2605.22992v1) — 相关度 0.65：通过分支翻转分析系统性发现数据库性能问题，方法可迁移到其他系统的性能调优。
- [HarnessAPI: A Skill-First Framework for Unified Streaming APIs and MCP Tools](http://arxiv.org/abs/2605.22733v1) — 相关度 0.65：统一HTTP与MCP工具封装，减少agent工具开发样板代码，有工程迁移价值。
- [Astragalus: Automatic Configuration Repair for Production Networks](http://arxiv.org/abs/2605.22092v2) — 相关度 0.60：提出语法驱动的网络配置自动修复方法Astragalus，借鉴APR的localize-fix-validate流程，配置修复方法对工程化agent有迁移价值。
- [Deterministic vs. Probabilistic Summarisation: An Empirical Trade-off Study in Design Pattern Centric Java Code](http://arxiv.org/abs/2605.21943v1) — 相关度 0.50：对比确定性vs概率性代码摘要生成，揭示语义深度与可复现性的权衡，对agent中代码文档生成技术选型有参考价值。
