...
- [Evaluating LLMs on Real-World Software Performance Optimization](http://arxiv.org/abs/2606.25530v1) — 相关度 0.95：SWE-Pro 是面向真实仓库性能优化的 benchmark，覆盖时间/内存权衡与噪声感知测量，与 SemOpt 直接相关。
- [Unlocking Model Potentials Through Adaptive Multi-Agent Scaffolding for Efficient Issue Resolution](http://arxiv.org/abs/2606.25514v1) — 相关度 0.95：提出去中心化多 Agent 架构 icat-agent，在 SWE-bench 上显著提升 issue 解决率，是直接可迁移的 Coding Agent 方法。
- [Beyond Function Calling: Benchmarking Tool-Using Agents under Tool-Environment Unreliability](http://arxiv.org/abs/2606.25819v2) — 相关度 0.90：ToolBench-X 在工具不可靠场景下评测 Tool-Using Agent，对 Agent 鲁棒性与故障恢复有直接迁移价值。
- [CodeChat-Eval: Evaluating Large Language Models in Multi-Turn Code Refinement Dialogues](http://arxiv.org/abs/2606.25747v2) — 相关度 0.85：CodeChat-Eval 评估多轮代码精修对话中的功能保持，弥补代码生成评测的多轮场景空缺。
- [How Do Developers Maintain and Evolve Their Agents' Instructions? An Empirical Study](http://arxiv.org/abs/2606.25257v1) — 相关度 0.85：对编码 Agent 指令文件（ACF）的演化与维护进行实证研究，直接关系 Agent 指令的工程化设计与治理。
- [IntentTester: Intent-Driven Multi-agent Framework for Cross-Library Test Migration](http://arxiv.org/abs/2606.25588v1) — 相关度 0.85：IntentTester 用多 Agent 与测试描述语言实现跨库/跨语言测试迁移，是测试生成 Agent 的可迁移方法。
- [Representation Matters: An Empirical Study of Program Representations for LLM Vulnerability Reasoning](http://arxiv.org/abs/2606.25356v1) — 相关度 0.85：系统比较 AST/CFG/PDG 等程序表示对 LLM 漏洞推理的影响，可迁移到代码 Agent 的上下文表示设计。
- [LibEvoBench: Probing Temporal Knowledge Stratification in Code Generation Models](http://arxiv.org/abs/2606.25402v1) — 相关度 0.80：新基准 LibEvoBench 评估代码生成模型对多版本 API 的时序知识，为代码 Agent 评测增加新维度。
- [Helpful or Harmful? Evaluating LLM-Assisted Vulnerability Patching via a Human Study](http://arxiv.org/abs/2606.25973v1) — 相关度 0.75：通过人类研究评估LLM辅助漏洞修复，Ghost Tests验证补丁完整性，对代码修复agent评估有直接参考。
- [Recall Before Rerank: Benchmarking Deep Learning Models for Large-Scale Code-to-Code Retrieval](http://arxiv.org/abs/2606.27401v1) — 相关度 0.75：大规模 code-to-code 检索召回基准与查询改写方案，对 Agent 的代码检索和上下文获取有参考价值。
- [Semantic Code Clone Detection: Are We There Yet?](http://arxiv.org/abs/2606.25272v1) — 相关度 0.75：提出克隆算子框架与分布偏移评测，检验语义克隆检测的真实泛化性，对代码分析与评测有参考价值。
- [Reinforcement Learning for Software Vulnerability Analysis: A Systematic Review with Emphasis on C/C++ Source Code and Static Analysis](http://arxiv.org/abs/2606.28403v1) — 相关度 0.72：系统综述强化学习在 C/C++ 漏洞分析与静态分析中的应用，为漏洞检测 Agent 的设计提供视角。
- [Automated Detection of Configuration-Specific Security Vulnerabilities via Patch Analysis](http://arxiv.org/abs/2606.25863v1) — 相关度 0.70：PatchLens 通过补丁分析推导漏洞影响条件，可用于配置感知的漏洞评估与测试选择。
- [On the Viability of Requirements Generation From Code: An Experience Report](http://arxiv.org/abs/2606.25550v1) — 相关度 0.60：经验报告 LLM/RAG Agent 从代码生成需求时的失败模式与人工监督必要性，对 Agent 流程验证有参考价值。
- [Project-wise Comparison of Software Birthmarks Using Weighted Partial Similarity](http://arxiv.org/abs/2606.25418v2) — 相关度 0.55：项目级软件胎记比较与部分相似度方法，对代码复用/抄袭检测有参考价值，但与 Agent 关联较弱。
