...
- [FeedbackLLM: Metadata driven Multi-Agentic Language Agnostic Test Case Generator with Evolving prompt and Coverage Feedback](http://arxiv.org/abs/2605.01264v1) — 相关度 0.85：多agent测试用例生成，利用行/分支覆盖反馈迭代优化，与测试agent直接相关。
- [LiveFMBench: Unveiling the Power and Limits of Agentic Workflows in Specification Generation](http://arxiv.org/abs/2605.01394v1) — 相关度 0.85：提出防泄漏的形式规约生成基准并揭示评估高估现象，对agent生成与验证评测有重要价值。
- [Feedback-Normalized Developer Memory for Reinforcement-Learning Coding Agents: A Safety-Gated MCP Architecture](http://arxiv.org/abs/2605.01567v1) — 相关度 0.80：为RL coding agent设计安全门控的记忆架构并含OPE评估，直接增强长程执行的记忆与验证能力。
- [Practical Limits of Autonomous Test Repair: A Multi-Agent Case Study with LLM-Driven Discovery and Self-Correction](http://arxiv.org/abs/2605.01471v1) — 相关度 0.80：多agent自主测试修复的工业案例，量化修复收敛与失败模式，为agent自主性与验证边界提供实证。
- [ClarifySTL: An Interactive LLM Agent Framework for STL Transformation through Requirements Clarification](http://arxiv.org/abs/2605.01209v1) — 相关度 0.70：通过交互式澄清将自然语言需求转为STL规约，其澄清-更新循环可迁移到agent的规格获取。
- [Neuro-Symbolic Agents for Hallucination-Free Requirements Reuse](http://arxiv.org/abs/2605.01562v2) — 相关度 0.55：神经符号多agent需求复用框架，用确定性符号验证器约束LLM生成，可迁移到代码生成的结构约束。
