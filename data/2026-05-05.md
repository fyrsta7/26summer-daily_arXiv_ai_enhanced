...
- [Root-Cause-Driven Automated Vulnerability Repair](http://arxiv.org/abs/2605.04251v1) — 相关度 0.95：根因驱动漏洞修复+两阶段质量评估，与agent修复和评测直接相关。
- [Reproduction Test Generation for Java SWE Issues](http://arxiv.org/abs/2605.04320v2) — 相关度 0.93：Java复现测试生成benchmark和工具，为agent提供执行反馈。
- [ProgramBench: Can Language Models Rebuild Programs From Scratch?](http://arxiv.org/abs/2605.03546v1) — 相关度 0.90：提出程序重建基准ProgramBench，全面评测软件工程agent的架构设计与端到端实现能力，高度相关。
- [When Context Hurts: The Crossover Effect of Knowledge Transfer on Multi-Agent Design Exploration](http://arxiv.org/abs/2605.04361v1) — 相关度 0.88：上下文注入对多agent设计探索的影响，指导条件化上下文设计。
- [KVerus: Scalable and Resilient Formal Verification Proof Generation for Rust Code](http://arxiv.org/abs/2605.03822v1) — 相关度 0.85：KVerus利用RAG和自适应性验证自动生成与修复Rust形式化证明，跨文件依赖处理对coding agent的长程代码验证极具价值。
- [LATS-RCA: Language Agent Tree Search for Root Cause Analysis in Microservices](http://arxiv.org/abs/2605.03505v2) — 相关度 0.85：将语言agent树搜索用于微服务根因分析，反射引导的多agent诊断机制对代码agent的调试与长程推理有直接迁移价值。
- [MOSAIC-Bench: Measuring Compositional Vulnerability Induction in Coding Agents](http://arxiv.org/abs/2605.03952v1) — 相关度 0.85：提出MOSAIC-Bench评估coding agent在分解任务中的漏洞诱导风险，为agent安全评测与防御提供重要基准。
- [Mitigating False Positives in Static Memory Safety Analysis of Rust Programs via Reinforcement Learning](http://arxiv.org/abs/2605.04000v2) — 相关度 0.85：RL抑制静态分析误报，可迁移到代码agent的反馈验证机制。
- [Identifier-Free Code Embedding Models for Scalable Search](http://arxiv.org/abs/2605.05251v1) — 相关度 0.82：代码嵌入模型双向关联源码头与反编译代码，直接增强代码检索。
- [Generating Proof-of-Vulnerability Tests to Help Enhance the Security of Complex Software](http://arxiv.org/abs/2605.03956v1) — 相关度 0.80：结合调用路径分析、执行反馈与LLM生成可利用性PoV测试，方法可迁移至面向安全测试的coding agent。
- [POSTCONDBENCH: Benchmarking Correctness and Completeness in Formal Postcondition Inference](http://arxiv.org/abs/2605.03356v1) — 相关度 0.80：提出多语言后置条件生成基准POSTCONDBENCH，用于评测LLM的代码分析与生成能力，直接相关。
- [Lightweight Vulnerability Detection from Code Metrics and Token Features](http://arxiv.org/abs/2605.04260v1) — 相关度 0.78：轻量级漏洞检测基线，可作agent快速triage或反馈信号。
- [Beyond Rules: LLM-Powered Linting for Quantum Programs](http://arxiv.org/abs/2605.03943v1) — 相关度 0.75：将LLM与CoT/RAG结合用于量子程序静态分析linting，展示了LLM作为linter的通用范式，可迁移至代码分析agent。
- [Deep Graph-Language Fusion for Structure-Aware Code Generation](http://arxiv.org/abs/2605.03689v1) — 相关度 0.75：提出图-语言深度融合框架CGFuse用于代码生成，图特征注入PLM内部的方法对结构感知代码生成有直接帮助。
- [A Multi-Agent Consensus Protocol for Stable Software Remodularization](http://arxiv.org/abs/2605.04188v1) — 相关度 0.72：多agent协商协议用于软件重构，可迁移到分布式agent协作。
- [ARGUS: Defending LLM Agents Against Context-Aware Prompt Injection](http://arxiv.org/abs/2605.03378v2) — 相关度 0.70：针对LLM agent的上下文感知提示注入防御，因果溯源审计机制可迁移至coding agent的可靠性与安全防护。
- [From TinyGo to gc Compiler: Extending Zorya's Concolic Framework to Real-World Go Binaries](http://arxiv.org/abs/2605.03492v2) — 相关度 0.65：扩展concolic执行框架至真实Go二进制漏洞检测，属于程序分析通用方法，对agent辅助漏洞挖掘有参考价值。
- [Randomized and Diverse Input State Generation for Quantum Program Testing](http://arxiv.org/abs/2605.03957v1) — 相关度 0.65：提出量子程序测试输入生成器与多样性评分，覆盖量子态空间的测试方法对测试agent设计有参考价值。
