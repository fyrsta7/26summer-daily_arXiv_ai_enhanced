...
Motivation: 现有研究广泛覆盖了AI编码智能体的串行性能，但对其并行编程能力了解不足，初步结果表明并行能力落后，亟需系统评估。

Method: 跨语言评估三种编码智能体（Composer 2.0、GPT 5.4、Sonnet 4.6）在C++、Python、Julia上生成排序、图遍历、搜索三类算法的并行实现，从串行基线出发，记录提示次数，测量功能正确性和加速比。

Result: 编码智能体能以较少提示生成正确并行代码，但只有Sonnet 4.6显著提升性能，GPT 5.4无提速；C++在图算法上并行化最一致，Python和Julia在搜索算法上提速最大，但不同语言各有优劣。

Conclusion: 并行代码生成中，运行时性能效率应作为LLM关键指标，与准确性并重，且语言和算法选择至关重要。
...
TL;DR: 本系统综述分析了2023-2025年64项研究，发现LLM在图表建模中集中于UML类图和从自然语言构建图表，但行为图、数据建模、评估标准化和语义可靠性方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: LLMs在图表建模中的应用日益增多，但缺乏对其有效性和局限性的系统讨论。

Method: 采用系统文献综述方法，分析了2023-2025年间64项研究，从图表覆盖、建模任务、技术方法、评估实践和局限性等维度进行综合。

Result: 研究发现：UML类图主导，行为图和数据建模不足；主要任务是从自然语言构建图表，转换和质量保证工作有限；GPT模型普遍，引发可重复性和供应商依赖问题；评估异质，基准重用少，鲁棒性和统计显著性报告不一致；常见局限包括语义不准确、幻觉元素、提示敏感性和可重复性差。

Conclusion: 需要标准化基准、更强评估协议、更广泛图表覆盖以及改进语义可靠性和多视图一致性的技术。

中文摘要：大语言模型（LLMs）正越来越多地应用于基于图的软件和数据建模。在各种建模符号中，UML和实体关系（ER）图分别是最广泛采用的软件建模和数据建模符号。近期文献已经研究了LLMs在图表建模中的各种应用；然而，它们的有效性和局限性尚未得到广泛讨论。本系统文献综述分析了2023年至2025年间发表的64项研究，考察了图表覆盖范围、建模任务、技术方法、评估实践和局限性。我们的发现揭示了显著的集中模式和空白。基于UML的软件建模占据主导地位，类图受到最多关注，而行为图和数据建模仍未被充分代表。从自然语言构建图表是主要关注点，而在转换、质量保证和一致性检查方面的工作有限。基于GPT的模型非常普遍，引发了对可重复性和供应商依赖性的担忧。评估实践是异质的，采用多样化的指标和自定义数据集，基准重用有限，鲁棒性和统计显著性的报告不一致。常见局限性包括语义不准确、幻觉图表元素、对提示表述的敏感性以及可重复性约束。本调查提供了首个关于基于LLM的图表建模研究的系统综合，强调了对标准化基准、更强评估协议、更广泛图表覆盖范围以及改进语义可靠性和多视图一致性技术的需求。

Original abstract: Large language models (LLMs) are increasingly applied to diagram-based software and data modelling. Among various modelling notations, UML and entity-relationship (ER) diagrams are the most widely adopted for software modelling and data modelling, respectively. Recent literature has investigated various applications of LLMs in diagram modelling; however, their effectiveness and limitations have not been extensively discussed. This systematic literature review analyses 64 studies published between 2023 and 2025, examining diagram coverage, modelling tasks, technical approaches, evaluation practices, and limitations. Our findings reveal significant concentration patterns and gaps. UML-based software modelling strongly dominates, with class diagrams receiving the most attention whilst behavioural diagrams and data modelling remain underrepresented. Diagram construction from natural language is the primary focus, with limited work on transformation, quality assurance, and consistency checking. GPT-based models are heavily prevalent, raising concerns about reproducibility and vendor dependence. Evaluation practices are heterogeneous, employing diverse metrics and custom datasets with limited benchmark reuse and inconsistent reporting of robustness and statistical significance. Common limitations include semantic inaccuracies, hallucinated diagram elements, sensitivity to prompt formulation, and reproducibility constraints. This survey provides the first systematic synthesis of LLM-based diagram modelling research, highlighting needs for standardised benchmarks, stronger evaluation protocols, broader diagram coverage, and techniques for improving semantic reliability and multi-view consistency.
...
TL;DR: 本文提出NISPO，一个基于RDKit的开源Python包，用于生成IUPAC名称。通过自主自我改进循环和OPSIN验证，在1.03亿分子上达到98.1%的往返准确率。


<details>
  <summary>Details</summary>
Motivation: IUPAC命名是化学交流的标准，但现有工具生成准确率不高，需要更可靠的命名生成工具。

Method: 使用OpenAI Codex和GPT-5.5模型，通过自主自我改进循环开发NISPO，以OPSIN解析回原结构作为正确性标准，在SureChEMBL的268万分子上优化。

Result: 在1.03亿PubChem分子的保留集上达到98.1%的往返准确率。

Conclusion: NISPO是一个高效、准确的IUPAC命名生成工具，开源可用，可广泛应用于化学文献和数据库。

中文摘要：系统性的国际纯粹与应用化学联合会（IUPAC）名称是化学文献、专利和数据库中分子结构交流的标准。我们介绍了NISPO，一个基于RDKit的开源Python包，用于生成IUPAC名称。NISPO通过一个基于OpenAI Codex与GPT-5.5模型的自主自我改进循环开发。如果开源工具OPSIN能将生成的名称解析回输入结构，则认为该生成名称正确。在该目标指导下，代理针对SureChEMBL中的268万个分子实现并优化了NISPO，使得该工具在包含1.03亿个PubChem分子的保留集上实现了98.1%的往返准确率。NISPO可在https://github.com/oxpig/nispo免费获取。

Original abstract: Systematic International Union of Pure and Applied Chemistry (IUPAC) names are standard for communicating molecular structures in chemical literature, patents, and databases. We introduce NISPO, an open-source RDKit-based Python package for IUPAC name generation. NISPO was developed by an agentic self-improvement loop using OpenAI's Codex with the GPT-5.5 model. A generated name was considered correct if the open-source OPSIN tool parsed it back to the input structure. Guided by this objective, the agent implemented and refined NISPO against 2.68 million molecules from SureChEMBL, resulting in the tool achieving 98.1% round-trip accuracy on a held-out set of 103 million PubChem molecules. NISPO is freely available at https://github.com/oxpig/nispo.
...
TL;DR: 本研究提出神经符号多智能体架构，通过确定性符号验证器消除LLM生成需求的结构不一致，并引入三值框架量化决策不确定性。在37个案例中，94.6%完全消除结构错误，24.7%的决策为不确定。


<details>
  <summary>Details</summary>
Motivation: LLM在需求获取中常生成结构无效或逻辑不一致的需求，缺乏形式化保证。

Method: 采用OOMRAM格的神经符号多智能体架构，LLM作为非确定性启发式，符号验证器强制执行结构约束，并通过三值(真、不确定、假)框架分类和评分决策。

Result: 在11个应用家族的37个案例中，94.6%完全消除结构错误，剩余两个案例仅0.39%的错误未解决；24.7%的决策为不确定。

Conclusion: 确定性符号层保证结构符合性，三值分类量化神经不确定性，促进LLM在形式化需求工程中的安全部署。

中文摘要：背景：大型语言模型（LLM）为自动化需求获取提供了自然语言灵活性，但经常生成结构上无效的需求和逻辑不一致，缺乏形式化的正确性保证。目标：本研究旨在消除逻辑不一致并强制LLM生成的需求的结构符合性，同时在形式化领域模型中量化LLM验证前的决策不确定性。方法：我们提出了一种神经符号多智能体架构，该架构实现了面向对象的需求编写和管理方法（OOMRAM）格。LLM作为格遍历的非确定性启发式，而确定性符号验证器强制执行所有结构约束。我们引入了一个三值（T，I，F）——真、不确定、假——框架，用于在验证前后对LLM的需求决策进行分类和评分。结果：在11个应用家族的37个自然语言项目愿景上评估，该系统在35/37个案例（94.6%）中完全消除了结构不一致，剩余两个案例由于迭代限制仅包含6个未解决的结构错误（占决策的0.39%）。三值分析显示，24.7%的所有决策是不确定的——结构有效但利益相关者未明确授权的自由选择。结论：将结构完整性卸载到确定性符号层成功保证了结构符合性，而三值分类提供了衡量神经不确定性的一种形式化方式，促进了LLM在形式化需求工程中的安全部署。

Original abstract: Context: Large Language Models (LLMs) offer natural-language flexibility for automated requirements elicitation but frequently generate structurally invalid requirements and logical inconsistencies, lacking formal correctness guarantees.
...
TL;DR: 提出ExplainBench基准，通过让LLM回答关于代码行为与补丁效果的问题来量化解释质量，发现解释质量是独立维度，且审计智能体能自动提升解释可信度。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在代码生成中作用增大，其代码修改规模变大，人工审查不可行，开发者依赖解释但缺乏评估解释可信度的基准。

Method: 提出ExplainBench，基于信息丰富的解释应使LLM能正确回答问题的直觉，构建问题套件来评估解释对缺陷代码预期行为和补丁效果的描述准确性，并设计解释审计智能体通过额外测试验证和优化解释。

Result: 实验显示ExplainBench对智能体的排名与SWE-bench Verified不同；解释中常见问题包括声称补丁正确但实际不正确；审计智能体改进了所有被评估智能体的解释。

Conclusion: 解释质量是评估智能体的独立维度，且智能体解释可被自动变得更可信。

中文摘要：大型语言模型（LLM）智能体在软件工程领域已被快速采用。随着智能体在实际代码生成中扮演更重要的角色，它们所做的变更也越来越大，跨越几十到几百行代码。这使得对智能体结果的人工审查日益不可行，导致开发者转而依赖解释来理解已实施的变更。尽管如此，目前还没有基准来评估智能体生成的解释的可信度。为了填补这一空白，我们提出了ExplainBench，一个用于自动评估编码智能体解释的基准。ExplainBench基于这样的直觉：信息丰富的解释应能使LLM正确回答问题，从而允许在智能体之间对解释质量进行定量比较。基于这一观察，我们构建了一套问题，用于评估解释是否准确描述了（1）有缺陷代码的预期行为，以及（2）应用智能体补丁本身的效果。实验首先表明，解释质量是智能体评估的一个独立维度：ExplainBench对智能体的排名与广泛使用的SWE-bench Verified基准不同。对智能体解释质量的更深入剖析显示，解释中频繁出现问题，例如解释常常声称补丁是正确的，而实际上并非如此。基于这一见解，我们实现并评估了一个解释审计智能体，该智能体运行额外的测试来验证和优化解释。该智能体改进了所有被评估智能体的解释，证明智能体解释可以被自动地变得更加可信。
...
TL;DR: 提出一种利用拉取请求构建复合提交数据集的低成本方法，使理想PR比例从9.5%提升至55%，数据集规模为先前启发式数据集的5.7倍以上，并验证了其在不同语言中的有效性。


<details>
  <summary>Details</summary>
Motivation: 复合提交频繁出现且阻碍代码理解，但现有机器学习方法需要大规模且带正确标签的训练数据，而人工标注成本高昂且依赖专家，急需一种可扩展且低成本的数据集构建方法。

Method: 从开源仓库的拉取请求（PR）中提取提交，设计过滤规则筛选出理想PR（即整体混乱但每个功能分支提交原子），并构建数据集；同时构建Python数据集验证跨语言适用性。

Result: 过滤规则使理想PR比例从9.5%提高到55%，数据集大小是先前启发式数据集的5.7倍以上；基于PR的数据集与先前Herzig启发式数据集存在统计差异，且影响置信度投票者；Python数据集的理想PR比例为56.5%。

Conclusion: 基于PR的过滤规则能够以低成本构建大规模、高质量的复合提交数据集，且优于先验启发式方法，可有效支持后续的提交解开研究，并具有良好的语言扩展性。

中文摘要：复合提交（CC）将多个不相关的更改捆绑到单个提交中，在软件开发中十分常见，并显著阻碍代码理解与维护。尽管已开发出基于机器学习的方法来“解开”此类提交，使其成为更小、连贯的更改集，但这些方法需要带有正确解开标签的大规模训练数据。准备此类数据集成本高昂，且通常需要专家标注。在本研究中，我们提出了一种可扩展且成本有效的数据集构建方法，利用从开源仓库的拉取请求（PR）中提取的提交。我们实证验证了我们的数据集，发现应用我们的过滤规则后，将PR视为单个提交时是混乱的、但功能分支上的每个单独提交都是原子性的（理想PR）的比例从9.5%增加到55%。该复合提交数据集比之前基于启发式规则的数据集大5.7倍以上。使用我们的新数据集，我们发现基于PR的数据集与之前直接使用Herzig提出的启发式规则构建的数据集在统计上存在差异，即使考虑了可能改变CC或STS大小的我们提出的规则。当使用先前的启发式规则构建数据集时，它们在影响置信度投票者并可能影响基于学习方法的不同维度上存在统计差异。我们验证了原始Herzig等人的方法（在我们的数据集中使用置信度投票者）所受到的影响。为了展示我们的方法可扩展到其他语言，我们还创建了一个Python数据集，并进行了实证验证，发现理想PR的比例相当（56.5%）。

Original abstract: Composite commits (CC), in which multiple unrelated changes are bundled into a single commit, are frequent in software development and significantly hinder code comprehension and maintenance. Although machine learning-based methods have been developed to ``untangle'' such commits into smaller, coherent change sets, these methods require large-scale training data with correct untangling labels. Preparing such datasets is costly and typically requires expert labelling.
...
TL;DR: 该研究构建基准测试评估编码代理对开源仓库AI贡献规则的遵循情况，发现代理几乎不主动检索规则，但在提示下能进行披露和验证，却从不拒绝在禁止AI的仓库中贡献。


<details>
  <summary>Details</summary>
Motivation: 开源社区已制定各种AI贡献规则，但编码代理是否实际阅读并遵循这些规则尚属未知，因此需要量化其真实合规性。

Method: 从49个仓库的106个问题构建RepoComplianceBench，基于规则判断代理轨迹的合规行为（拒绝、披露、验证、升级），并测试额外提示、规则披露和验证器反馈等干预措施，在四个前沿模型上进行实验。

Result: 代理几乎从不主动检索规则；提醒提示、规则引用和验证器反馈能促使代理进行披露和验证，但在任何条件下代理均未在禁止AI的仓库中拒绝贡献。

Conclusion: 验证和披露问题可通过现有机制解决，但强制禁令和人工升级仍是未解决的挑战。

中文摘要：开源社区已经被AI生成的贡献所淹没。为了防御，他们制定了贡献规则来规范编码代理的行为，涵盖从完全禁止、强制披露，到验证门控和人工签署。然而，编码代理是否阅读并遵循这些规则，以及在开源仓库中的行为，仍然未知。为了估计编码代理在现实世界中的规则合规性，我们从49个包含AI贡献规则的仓库中整理了106个问题，构建了RepoComplianceBench。我们根据仓库的规则判断每次运行的轨迹，衡量代理是否拒绝贡献、是否如实披露其帮助、是否通过所需的验证门控，或者是否将关键步骤升级给人类。我们还测试了额外的提示、规则披露或来自合规验证器的反馈是否有助于改善情况。我们在四个前沿模型上的实验表明，当今的代理几乎从不主动检索贡献规则。代理在提醒提示、规则引用和验证器反馈下能够进行披露和验证；然而，在任何我们测试的条件下，它们在禁止AI的仓库中从不拒绝贡献。这一现状表明，验证和披露问题可以通过现有机制解决，但执行禁令和人工升级仍然是一个未解决的问题。

Original abstract: Open source communities have been flooded with AI-generated contributions. In defense, they have written contribution rules to regulate coding agents' behavior, spanning from a total ban, mandatory disclosure, to verification gates and human sign-offs. Yet, whether coding agents read and follow those rules, and behave in open source repositories, remains unknown. To estimate real-world rule compliance of coding agents, we curate 106 issues from 49 repositories containing AI contribution rules into RepoComplianceBench. We judge the trajectory of each run against the repository's rules, measuring whether the agent refuses to contribute, discloses its assistance truthfully, clears the required verification gates, or escalates critical steps to a human. We also test if extra prompts, rule disclosure, or feedback from the compliance verifier help with the situation. Our experiments on four frontier models show that today's agents almost never proactively retrieve the contribution rules. Agents pick up disclosure and verification with reminder prompts, rule quotes, and verifier feedback; however, they never refuse to contribute in AI-banned repositories under any condition we tested. The status reveals that verification and disclosure issues are solvable with existing mechanisms, yet enforcing bans and human escalations remains an open problem.
...
TL;DR: 提出了一个元变形测试框架，用于评估RAG系统在语料库演变下的一致性，通过11种变异算子扰动系统，实证发现违规率4.9-10.2%，F1分数显著优于RAGAS。


<details>
  <summary>Details</summary>
Motivation: 现有RAG评估方法（如RAGAS）仅针对静态快照，无法检测语料库动态变化（如更新、事实改变、噪声）导致的系统故障。

Method: 形式化故障分类法，设计11个变异算子，在前分块（检索索引）和后分块（检索上下文）层面扰动系统，提出元变形预言机进行一致性测试。

Result: 在5个数据集上超过28,000个变异体的实验中，元变形违规率为4.9-10.2%；元变形预言机F1分数达0.927-1.000，远超RAGAS最佳指标0.570。

Conclusion: 提供了通过检索重新配置、生成器升级和LLM重排序来缓解RAG系统在语料库演变下故障的可操作建议。

中文摘要：基于检索增强生成（RAG）的LLM系统依赖于可能随时间演变和变化的外部文档语料库。然而，当前的评估方法（例如RAGAS）针对静态快照评估正确性，无法在常规更新、事实变化或噪声改变底层数据时检测出故障。我们引入了一个元变形测试框架，用于评估RAG系统在语料库演变下的一致性。我们形式化了一个故障分类法和11个变异算子，这些算子在前分块（检索索引）和后分块（检索上下文）级别系统地扰动系统。跨五个数据集和超过28,000个变异体的实证评估揭示了元变形违规率为4.9%至10.2%。在与地面真实情况的元评估中，我们的元变形预言机达到了0.927至1.000的F1分数，而最佳RAGAS指标仅达到0.570。最后，我们提供了通过检索重新配置、生成器升级和基于LLM的重新排序来缓解这些故障的可操作见解。

Original abstract: Retrieval-Augmented Generation (RAG)-based LLM systems rely on external document corpora that can evolve and change over time. However, current evaluation methodologies (e.g., RAGAS) assess correctness against static snapshots, failing to detect faults when routine updates, factual changes, or noise alter the underlying data. We introduce a metamorphic testing framework that evaluates the consistency of RAG systems under corpus evolution. We formalise a fault taxonomy and 11 mutation operators that systematically perturb the system at both the pre-chunk (retrieval index) and post-chunk (retrieved context) levels. An empirical evaluation across five datasets and over 28k mutants reveals metamorphic violation rates of 4.9-10.2%. In a meta-evaluation against ground truth, our metamorphic oracle achieves F1 scores of 0.927-1.000, while the best RAGAS metric reaches only 0.570. Finally, we provide actionable insights into mitigating these faults through retrieval re-configuration, generator upgrades, and LLM-based reranking.
...
TL;DR: 提出 VITAL-RAG，通过按规范代码对象组织证据并保持不变性，解决代码检索中冗余片段挤占上下文的问题，在多个基准上显著提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有代码检索增强生成把同一代码对象的多个片段视为独立结果，导致冗余视图占用上下文位置，而简单分组又会丢失局部信息。作者将其描述为“不变性竞赛”，需要兼顾冗余稳定性与语义敏感性。

Method: VITAL-RAG 按规范代码对象组织证据，仅当伴生片段带来新的任务相关语义时才保留一个查询相关伴生，并在每对象和全局令牌预算下渲染证据。

Result: 在 RepoBench 上 Recall@4K 从 39.59% 提升至 63.67%，证据令牌减少 35.63%；在 RepoClassBench 上匹配或超越三个后端上的基线，在 RepoExec 上取得最高原始 Pass@1。

Conclusion: 实验表明 VITAL-RAG 有效解决了冗余与局部信息的权衡，显著提升代码检索与生成性能，为代码智能体提供了更高效的证据选择策略。

中文摘要：编程代理经常从整个代码库中检索代码，但只有有限的证据能够装入最终的模型输入。用于编程代理的常规检索增强生成（RAG）将来自同一代码对象的片段视为独立结果，因此冗余视图可能占据多个上下文位置并挤掉有用的代码。按代码对象对片段进行分组可以减少这种冗余，但可能丢弃任务所需的局部信息。我们将这种张力描述为不变性竞赛：分配在冗余渲染下应保持稳定，但当片段添加了与任务相关的语义时则应该改变。为了解决这一竞赛，我们引入了VITAL-RAG，它按规范代码对象组织证据，仅在伴生片段添加了尚未表示的语义时保留一个查询相关的伴生片段，并在每对象和全局令牌预算下渲染选中的证据。在RepoBench上，VITAL-RAG将Recall@4K从39.59%提升到63.67%，同时减少了35.63%的证据令牌。在三个模型后端上，它在RepoClassBench上匹配或超越了最近的基线，并在RepoExec上取得了最高的原始Pass@1。

Original abstract: Coding agents often retrieve code from an entire repository, but only limited evidence can fit into the final model input. Conventional retrieval-augmented generation (RAG) for coding agents treats fragments from the same code object as separate results, so redundant views can occupy multiple context positions and crowd out useful code. Grouping fragments by code object reduces this redundancy, but can discard local information needed for the task. We describe this tension as an invariance race: allocation should stay stable under redundant renderings but change when a fragment adds task-relevant semantics. To address this race, we introduce VITAL-RAG, which organizes evidence by canonical code object, keeps one query-relevant companion only when it adds semantics not already represented, and renders selected evidence under per-object and global token budgets. On RepoBench, VITALRAG improves Recall@4K from 39.59% to 63.67% while reducing evidence tokens by 35.63%. Across three model backends, it matches or outperforms recent baselines on RepoClassBench and achieves the highest raw Pass@1 on RepoExec.
...
### [23] [Even More Deception: Objective Misalignment in Mixed-Motive LLM Multi-Agent Systems](https://arxiv.org/abs/2607.26120)
...
Selection reason: 多agent系统欺骗与目标不一致研究，通用agent行为分析可迁移。

TL;DR: 本文通过狼人杀游戏研究LLM多智能体系统中目标不一致的影响，发现其损害集体结果且内部推理变化在公开行为中不可见。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型驱动的多智能体系统在混合动机环境中目标不一致的影响，尤其是在信息不对称和战略欺骗场景下。

Method: 利用社交推理游戏狼人杀，通过修改单个智能体的目标并保持角色不变，分析智能体内部推理、公开廉价谈话行为及游戏结果。

Result: 目标不一致会损害对抗性环境中的集体结果，且这种损害因信息不对称和专业化角色加剧；被损害的智能体发展出不同的推理策略，但在公开行为中不可见。

Conclusion: 即使是微妙的目标不一致也会显著影响集体决策，需要针对LLM多智能体系统制定有效的缓解策略。

中文摘要：大型语言模型（LLM）驱动的多智能体系统越来越多地部署在混合动机环境中，在这些环境中，由于冲突或隐藏的目标，智能体在信息不对称和战略性欺骗下运行。在这些情况下，与集体目标的不一致成为一个核心问题。我们提出了一种新的框架，通过使用社交推理游戏《狼人杀》来评估目标不一致，即修改单个智能体的目标同时保留其分配的角色。跨越四个不同模型家族和规模的LLM、四个玩家角色和三种目标公式，我们引入了对智能体内部推理及其公开的廉价谈话行为（即无成本、非约束性的沟通，不直接影响智能体的效用）的双重分析，并辅以对游戏结果的分析。我们的结果表明，目标不一致会损害固有对抗性环境中的结果，这种影响因信息不对称和专业化角色而加剧。虽然被损害的智能体始终发展出不同的、依赖于目标的推理策略，但这些适应在其公开行为中基本不可见。更广泛地说，我们的发现表明，即使是微妙的目标不一致也可能深刻影响集体决策，凸显了为基于LLM的多智能体系统制定有效缓解策略的必要性。

Original abstract: Large Language Models (LLMs)-powered multi-agent systems are increasingly deployed in mixed-motive environments, where agents operate under asymmetric information and strategic deception due to conflicting or hidden objectives. In these settings, misalignment with collective goals becomes a central concern. We propose a novel framework for evaluating objective misalignment using the social deduction game Werewolf, modifying the objective of a single agent while preserving its assigned role. Across LLMs from four different model families and sizes, four player roles, and three objective formulations, we introduce a dual analysis of the agents' internal reasoning and their public cheap-talk behavior (i.e costless, non-binding communication that does not directly affect the agents' utilities), complemented by an analysis of game outcomes. Our results show that objective misalignment undermines outcomes in inherently adversarial environments, an effect exacerbated by asymmetric information and specialized roles. While compromised agents consistently develop distinct objective-dependent reasoning strategies, these adaptations remain largely invisible in their public behavior. More broadly, our findings suggest that even subtle objective misalignment can profoundly affect collective decision-making, highlighting the need for effective mitigation strategies for LLM-based multi-agent systems.

</details>


### [24] [ClinLens: Towards Long-Horizon Coding Agents for Longitudinal Multimodal Clinical Data Science](https://arxiv.org/abs/2607.26155)
*Yuan Zhu,Ethan B. Liu,Frank Nie,Jindong Han*

Main category: cs.AI

Selection reason: 临床数据科学长期编码agent基准，直接涉及coding agent长程任务。

TL;DR: CLINLENS基准测试表明，当前最强的临床数据科学智能体在执行成功率很高的情况下，正确完成临床分析的比例仍较低。


<details>
  <summary>Details</summary>
Motivation: 现有基准孤立处理医学问答、结构化表格推理或通用科学库，缺乏整合多模态纵向临床数据并支持可审计分析的综合性评估。

Method: 构建包含200个可执行任务的CLINLENS基准，覆盖五个MIMIC资源（结构化电子病历、笔记、心电图、胸片、超声心动图），采用4×5分类（患者时间范围×分析能力）和程序优先反向合成方法，通过评估器私有参考工作流检查工件、语义和答案。

Result: 在126个固定任务上，最强模型-支架配置达到56.3%的scope-macro STRICTPASS（100%执行成功），而编码Agent解决83个任务，五个生物医学系统最多仅2.9%。

Conclusion: 可运行提交与正确临床分析之间存在巨大差距，需进一步改进模型的分析能力以确保临床可靠性。

中文摘要：临床数据科学智能体必须将异质性纵向记录转化为可审计的分析，然而现有基准大多将医学问答、结构化表格推理或通用科学库孤立开来。我们引入CLINLENS，一个包含200个可执行任务的基准，覆盖五个相互关联的MIMIC资源，涵盖结构化电子健康记录、笔记、心电图、胸片和超声心动图。一个4×5分类法跨越四个患者时间范围与五种分析能力。程序优先反向合成将每个有界的半原始包与评估者私有的参考工作流配对，并检查所需工件、队列和时间语义以及最终答案。在固定的126个任务套件上，24种标准化模型-支架配置中最强的配置达到了56.3%的范围宏STRICTPASS，尽管EXECSUCCESS为100%。作为参考，一个单独配置的编码智能体解决了126个任务中的83个，而五个适应于GPT-4o-mini的生物医学系统最多达到2.9%的范围宏STRICTPASS。这些结果揭示了可运行提交与正确临床分析之间的巨大差距。

Original abstract: Clinical data-science agents must transform heterogeneous longitudinal records into auditable analyses, yet existing benchmarks largely isolate medical question answering, structured-table reasoning, or generic scientific repositories. We introduce CLINLENS, a benchmark of 200 executable tasks over five linked MIMIC resources spanning structured electronic health records, notes, electrocardiograms, chest radiographs, and echocardiograms. A 4 x 5 taxonomy crosses four patient-time scopes with five analysis capabilities. Program-first reverse synthesis pairs each bounded semi-raw package with an evaluator-private reference workflow and checks required artifacts, cohort and temporal semantics, and the final answer. On a fixed 126-task suite, the strongest of 24 standardized model-scaffold configurations achieves 56.3% scope-macro STRICTPASS despite 100% EXECSUCCESS. For reference, a separately configured coding agent solves 83 of 126 tasks, while five biomedical systems adapted to GPT-4o-mini reach at most 2.9% scope-macro STRICTPASS. These results expose a substantial gap between runnable submissions and correct clinical analyses.

</details>


### [25] [When benchmark inferences do not compose: Projectibility in AI evaluation](https://arxiv.org/abs/2607.26159)
*Brett Reynolds*
...
Selection reason: 物理问题结构发现的AI agent，提出-验证-修改循环可用于代码优化。

TL;DR: 本文提出StatMechBench-v0基准，评估LLM代理能否从配分函数中发现统计力学映射，发现数值反馈有帮助但代理可能通过数值检验却误判结构。


<details>
  <summary>Details</summary>
Motivation: 理论物理学家常需将新问题转化为已知模型，我们希望探索LLM代理是否具备这种结构性发现能力，并揭示当前推理的局限。

Method: 创建包含六个伊辛型问题的基准StatMechBench-v0，采用提出-验证-修订的简单代理框架，在多个LLM和问题表述下进行评估，基于数值反馈迭代修复代码。

Result: 数值反馈常能帮助代理修复代码并得到正确配分函数，但代理可能通过数值检查却错误识别可处理类别或低估计算复杂度。

Conclusion: 需要超越数值一致性的验证栈（如符号检查和结构不变量），并为AI代理在理论物理中的结构发现提供设计方向。

中文摘要：理论物理学中的一项重要技能是认识到一个新问题何时可以转化为已知模型。我们将这一技能作为AI代理任务来研究：基于LLM的代理能否发现从原始配分函数到可处理表示的统计力学映射？为了探究这个问题，我们引入了StatMechBench-v0，一个包含六个伊辛型问题的基准测试，涵盖转移矩阵方法、规范可消除无序以及平面/Pfaffian结构。我们评估了一个简单的“提出-验证-修订”代理在多种LLM和问题表述下的表现。结果表明，数值反馈通常有助于代理修复代码并恢复正确的配分函数。然而，代理也可能通过数值检查，却错误识别了底层的可处理类别或低估了计算复杂性。这既揭示了当前LLM推理的局限性，也呼吁建立一个超越数值一致性的验证栈，例如纳入符号检查和结构不变量。我们的研究为旨在理论物理学中进行结构性发现的AI代理提供了早期评估和设计方向。

Original abstract: An important skill in theoretical physics is to recognize when a new problem can be transformed into a known model. We study this skill as an AI-agent task: can LLM-based agents discover statistical mechanical mappings from a raw partition function to a tractable representation? To probe this question, we introduce StatMechBench-v0, a benchmark of six Ising-type problems covering transfer-matrix methods, gauge-removable disorder, and planar/Pfaffian structure. We evaluate a simple propose-verify-revise agent across multiple LLMs and problem phrasings. The results show that numerical feedback often helps agents repair code and recover correct partition functions. However, agents can also pass the numerical checks while misidentifying the underlying tractable class or understating computational complexity. This both reveals limitations in current LLM reasoning and calls for a verification stack that goes beyond numerical agreement, incorporating, for example, symbolic checks and structural invariants. Our study provides an early evaluation and design directions for AI agents aimed at structural discovery in theoretical physics.

</details>


### [31] [CaM-Wolf: Causal-Aware Multimodal Agents for Social Deduction Games](https://arxiv.org/abs/2607.26393)
...
Selection reason: 多模态社会推理agent，因果感知与多模态感知可启发agent设计。

TL;DR: CaM-Wolf是首个多模态社交推理游戏智能体，整合视频感知、强化学习推理与动画化身，在游戏表现和人机交互上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有社交推理游戏智能体主要基于文本，忽略了多模态社交信号（如视频）在人类互动中的核心作用，限制了智能体的类人表现与交互体验。

Method: CaM-Wolf采用多模态感知处理玩家视频，利用强化学习训练的因果感知推理器建立行为与隐藏角色的逻辑链，并通过动画化身进行自我表达。

Result: 实验和用户研究显示CaM-Wolf在智能体游戏性能及人类-AI交互质量上均取得更优结果。

Conclusion: 该工作证明多模态感知与生成能显著提升AI智能体在社交动态中的参与能力，为构建更类人的社交AI开辟了道路。

中文摘要：社交推理游戏（SDGs）如《狼人杀》已成为AI智能体面临的挑战性试验场。这些游戏需要复杂的社交技能，如推理、欺骗和协作。尽管大型语言模型（LLMs）的最新进展推动了SDG智能体的显著进步，但当前方法 predominantly 基于文本，忽视了作为人类社交互动基础的多模态性质。为了弥合这一差距，我们引入了 CaM-Wolf，这是首个整合多模态感知与生成的SDG智能体。CaM-Wolf 处理来自其他玩家的视频输入，采用通过强化学习训练的因果感知推理器，在可观察行为与隐藏角色之间建立逻辑链条，并通过动画化身呈现自己。我们的实验和用户研究表明，CaM-Wolf 实现了优越的智能体游戏性能，并提升了人机交互的质量。这项工作朝着创建能够参与微妙社交动态的、更类人的AI智能体迈出了重要一步。我们的代码可在 https://3dagentworld.github.io/avatar_wolf 获取。

Original abstract: Social deduction games (SDGs) such as Werewolf have become challenging testbeds for AI agents. These games require complex social skills such as reasoning, deception, and collaboration. While recent advances in large language models (LLMs) have driven significant progress in SDG agents, current approaches are predominantly text-based, overlooking the multimodal nature that is fundamental to human social interaction. To bridge this gap, we introduce CaM-Wolf, the first SDG agent that integrates multimodal perception and generation. CaM-Wolf processes video inputs from other players, employs a causal-aware Reasoner trained via reinforcement learning to establish logical chains between observable behaviors and hidden roles, and presents itself through an animated avatar. Our experiments and user study show that CaM-Wolf achieves superior agent gameplay performance and enhances the quality of human-AI interaction. This work represents a significant advancement towards creating more human-like AI agents capable of participating in nuanced social dynamics. Our code is available at https://3dagentworld.github.io/avatar_wolf.

</details>


### [32] [CG-World: A Large-Scale World-State Dataset and Protocol for World Models](https://arxiv.org/abs/2607.26452)
*Yiming Cai,Fangjie Yu,Meiqing Yu,Ziyue Shi,Pengfei Yuan,Yong Guo*

Main category: cs.AI

Selection reason: 世界状态数据集用于世界模型，对agent规划与状态表示有参考价值。

TL;DR: CG-World是一个从工业CG生产流程中提取的大规模世界状态数据集，包含85万个时间对齐片段，显式记录中间状态、事件和分支元数据，用于支持世界模型的联合动态学习、干预学习和反事实推理。


<details>
  <summary>Details</summary>
Motivation: 现有视频、机器人和仿真数据集仅捕获状态-动作-事件-观测联合动态的部分结构，缺乏显式的中间状态和干预/反事实标注，限制了世界模型、物理AI和具身智能的发展。

Method: 从工业计算机图形生产流程中派生数据集，记录多模态语义、空间结构、骨骼/控制器状态、运动曲线、相机/光照参数、物理缓存、接触事件和多通道渲染；分离潜在状态、观测、关系、事件和分支元数据，定义包含事实轨迹、观测/动作/机制干预及严格反事实的分支谱系，并显式标注干预目标、不变量和替代结果。

Result: 在几何条件视频生成、动作预测和闭环视觉-语言-动作策略迁移任务上评估，表明CG-World能为受控生成、动作建模和具身策略迁移提供可复用的结构化监督。

Conclusion: CG-World为世界模型、物理AI和具身智能提供了共享数据基础设施，未来将通过持续数据收集和社区协作进行扩展。

中文摘要：世界模型必须学习状态、动作、事件和观测的联合动态，然而现有的视频、机器人和仿真数据集通常只捕获了这种结构的一部分。我们引入了CG-World，一个从工业计算机图形生产流程中派生的大规模世界状态数据集和协议。CG-World显式记录了中间状态，包括多模态语义、空间结构、骨骼和控制器状态、运动曲线、相机和光照参数、物理缓存、接触事件以及多通道渲染。CG-World v1包含约85万个时间对齐的1-5秒片段。它将潜在状态、观测、关系、事件和分支元数据分离，并将它们组织成统一的时空样本。为了支持干预学习和反事实推理，CG-World定义了一个分支谱系，涵盖事实轨迹、观测干预、动作干预、机制干预和严格反事实分支，并明确记录了干预目标、不变量和替代结果。我们在几何条件视频生成、动作预测和闭环视觉-语言-动作策略迁移上评估了该数据集。结果表明，CG-World为受控生成、动作建模和具身策略迁移提供了可复用的结构化监督。我们计划通过持续的数据收集和社区协作扩展CG-World，以构建共享的世界模型、物理AI和具身智能数据基础设施。

Original abstract: World models must learn the joint dynamics of states, actions, events, and observations, yet existing video, robotics, and simulation datasets usually capture only part of this structure. We introduce CG-World, a large-scale world-state dataset and protocol derived from industrial computer graphics production pipelines. CG-World explicitly records intermediate states, including multimodal semantics, spatial structure, skeletal and controller states, motion curves, camera and lighting parameters, physics caches, contact events, and multi-pass renderings. CG-World v1 contains approximately 850,000 temporally aligned segments of 1-5 seconds. It separates latent states, observations, relations, events, and branch metadata, and organizes them into unified spatiotemporal samples. To support intervention learning and counterfactual reasoning, CG-World defines a branch lineage covering factual trajectories, observation interventions, action interventions, mechanism interventions, and strict counterfactual branches, with intervention targets, invariants, and alternative outcomes explicitly recorded. We evaluate the dataset on geometry-conditioned video generation, action prediction, and closed-loop vision-language-action policy transfer. Results show that CG-World provides reusable structured supervision for controlled generation, action modeling, and embodied policy transfer. We plan to expand CG-World through continued data collection and community collaboration toward a shared data infrastructure for world models, Physical AI, and embodied intelligence.

</details>


### [33] [EvoPINN: Agentic Discovery of Executable Algorithms for Physics-Informed Neural Networks](https://arxiv.org/abs/2607.26490)
*Peng Yin,Kai Li,Yifan Zhang,Jian Cheng*

Main category: cs.AI

Selection reason: 自动发现PINN算法的agent框架，直接进行程序性能优化与算法发现。

TL;DR: EvoPINN是一个基于LLM代理的框架，将PINN设计转化为执行引导的算法发现，自动发现PDE特化学习算法，并自主发明了新颖的SLRC-PINN架构，显著降低相对L2误差。


<details>
  <summary>Details</summary>
Motivation: PINN性能依赖手动试错工程，而LLM直接生成代码在科学计算约束下常产生无效或不稳定解，需要弥合自动设计与科学有效性之间的差距。

Method: EvoPINN解耦神经表示与训练程序，利用LLM代理在模块化搜索空间中迭代提出记忆条件程序修改，对所有候选进行结构验证和预算匹配的PDE评估。

Result: 在振荡、椭圆、耗散和非线性输运等多样PDE机制中，EvoPINN发现的算法相比基线显著降低相对L2误差，且自主发明的SLRC-PINN在严格参数匹配下性能优势持续存在。

Conclusion: EvoPINN证明了基于执行引导的代理能够发现真正新颖的科学计算机制，为自动化PINN设计提供了可行途径。

中文摘要：物理信息神经网络（PINN）已成为求解偏微分方程（PDE）的强大范式，但其性能严重依赖于对神经表示、损失公式和优化动态的手动试错工程。虽然大语言模型（LLM）为自动化设计提供了有前途的途径，但在严格科学计算约束下，无约束的代码生成往往产生数学上无效或数值上不稳定的解决方案。为弥合这一差距，我们提出	extbf{EvoPINN}，一个代理框架，将PINN开发从劳动密集型的手动设计重新构建为严格的、基于执行引导的算法发现问题。EvoPINN通过将神经表示与训练程序解耦，利用LLM代理迭代地提出基于记忆的条件程序修改，从而在模块化搜索空间中导航。为确保科学有效性，所有候选方案都经过严格的结构验证和预算匹配的PDE评估。跨多种PDE机制（振荡、椭圆、耗散和非线性输运）的大量实验表明，EvoPINN发现了PDE特化的学习算法，与基线相比显著降低了相对$L_{2}$误差。关键的是，EvoPINN自主发明了SLRC-PINN，一种新颖的架构，其性能提升在严格的参数匹配比较下持续存在，确立了基于执行引导的代理发现真正新颖科学计算机制的可行性。

Original abstract: Physics-informed neural networks (PINNs) have emerged as a powerful paradigm for solving partial differential equations (PDEs), yet their performance heavily relies on the manual, trial-and-error engineering of neural representations, loss formulations, and optimization dynamics. While Large Language Models (LLMs) offer a promising avenue for automated design, unconstrained code generation often yields mathematically invalid or numerically unstable solutions under strict scientific computing constraints. To bridge this gap, we propose \textbf{EvoPINN}, an agentic framework that reformulates PINN development from labor-intensive manual design into a rigorous, execution-grounded algorithm discovery problem. EvoPINN navigates a modular search space by decoupling neural representations from training programs, utilizing an LLM agent to iteratively propose memory-conditioned programmatic modifications. To ensure scientific validity, all candidates undergo strict structural verification and budget-matched PDE evaluation. Extensive experiments across diverse PDE regimes (oscillatory, elliptic, dissipative, and nonlinear transport) demonstrate that EvoPINN discovers PDE-specialized learning algorithms that significantly reduce relative $L_{2}$ error compared to baselines. Crucially, EvoPINN autonomously invented SLRC-PINN, a novel architecture whose performance gains persist under rigorous parameter-matched comparisons, establishing the viability of execution-grounded agents for discovering genuinely new scientific computing mechanisms.

</details>


### [34] [Evidence-Ledger Adjudication for Claim-Evidence Traceability](https://arxiv.org/abs/2607.26512)
*Gengyu Chen,Yongjie Yu,Weiling Wang*

Main category: cs.AI

Selection reason: 证据账本裁决用于写作，涉及agent验证与基准评估，可借鉴其验证方法。

TL;DR: 提出证据分类账裁决方法，通过将主张与证据包配对并分配支持关系，实现对AI生成主张的可审计追溯。在2,335行基准上，该方法显著优于非代理基线，能有效识别并路由有问题的证据。


<details>
  <summary>Details</summary>
Motivation: AI代理生成主张的速度快于人工验证证据支持的能力，导致引用或证据可能不支持所生成的主张，缺乏可追溯性和可审计性。

Method: 构建证据分类账工作流程：为每个主张配对证据包，分配支持关系（支持、矛盾、缺失证据或混合证据），并将有问题的主张返回给作者。使用AVeriTeC、CLIMATE-FEVER和SciFact的外部标签构建2,335行盲测基准进行评测。

Result: 代理证据分类账条件达到0.676关系准确率和0.601宏F1，优于最佳非代理基线（0.383准确率、0.303宏F1）；成功路由了1270/1435个有问题的主张和295/900个受支持的主张。

Conclusion: 证据分类账裁决能将异构证据包转化为AI辅助写作的可审计可追溯性层，有效识别并路由不支持、矛盾或混合证据的主张，提升验证效率和可靠性。

中文摘要：AI代理生成主张的速度可能比作者检查所引用或检索到的证据是否支持这些主张更快。我们研究证据分类账裁决：一种主张-证据可追溯性工作流程，将每个主张与一个证据包配对，分配支持关系，并将不支持、矛盾或混合证据的主张返回给作者。实证核心是一个由AVeriTeC、CLIMATE-FEVER和SciFact中的独立外部标签构建的2,335行盲测基准。金关系和源证据标签在预测期间被隐藏，仅在评分时合并。在该基准上，代理证据分类账条件实现了0.676的关系准确率和0.601的宏F1，而最佳非代理基线的准确率为0.383，宏F1为0.303。它还将金标签指示矛盾、缺失证据或混合证据的1270/1435个主张进行路由，同时路由了295/900个受支持的主张。这些结果表明，证据分类账裁决可以将异构证据包转化为AI辅助写作的可审计可追溯性层。

Original abstract: AI agents can draft claims faster than authors can check whether the cited or retrieved evidence supports them. We study evidence-ledger adjudication: a claim-evidence traceability workflow that pairs each claim with an evidence packet, assigns a support relation, and routes unsupported, contradicted, or mixed-evidence claims back to the author. The empirical core is a 2,335-row blind benchmark built from independent external labels in AVeriTeC, CLIMATE-FEVER, and SciFact. Gold relations and source evidence labels are hidden during prediction and joined only for scoring. On this benchmark, the agent evidence-ledger condition achieves 0.676 relation accuracy and 0.601 macro-F1, compared with 0.383 accuracy and 0.303 macro-F1 for the best non-agent baseline. It also routes 1270/1435 claims whose gold labels indicate contradiction, missing evidence, or mixed evidence, while routing 295/900 supported claims. These results show that evidence-ledger adjudication can turn heterogeneous evidence packets into an auditable traceability layer for AI-assisted writing.

</details>


### [35] [Eco3S: Complex Socio-Economic System Simulation via Agent-Based Models](https://arxiv.org/abs/2607.26588)
...
<details>
  <summary>Details</summary>
Motivation: Ascend C 算子优化对 NPU 推理性能至关重要，但需要深度硬件知识；现有 LLM 自动生成内核方法主要针对 CUDA，而 Ascend C 的编程模型不同，且平台知识稀缺，导致直接迁移困难。

Method: 设计知识编排生成系统以提供分层领域知识，解决上游可行性瓶颈；采用阶段自适应智能体进化策略，根据生成和优化阶段动态调整 LLM 交互模式，平衡探索与收敛。

Result: 在华为 Ascend 910B 上，对六种算子（覆盖五类模式）实现元素级和归一化算子 90-100% 可行性、融合算子 56% 可行性，在 1B Pangu 模型推理内核上最高获得 6.65 倍加速；知识注入使元素级算子可行性从 57% 提升至 86%。

Conclusion: AgenticCANN 有效解决了低语料 NPU 环境下 Ascend C 算子自动合成难题，知识注入具有通用益处，为专用硬件上的算子自动优化提供了可行路径。
...
Selection reason: 提出信念引导决策架构，可迁移至agent规划与不确定性处理

TL;DR: 提出信念引导架构，将策略头与信念头解耦，以参数化直觉替代运行时MCTS，在有限硬件上实现专业级围棋对弈并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有围棋AI依赖MCTS在推理时纠正神经网络策略错误，在大规模集群上有效，但在消费级硬件上树管理成本限制推理速率；且无深度搜索时模型易产生高置信度的战略致命幻觉。因此需要将智能从运行时搜索转移到参数化直觉。

Method: 引入信念引导（Belief-Guided）架构，解耦策略头与独立的信念头；信念头作为内部模拟器和独立评论家，建模认知不确定性和战略稳定性；集成Transformer/GRU记忆机制处理长期依赖和劫规则，并用门控机制过滤过度自信的策略错误。

Result: 实验表明该方法显著提高了无搜索胜率并减少了幻觉，在大型MCTS不可行的有限硬件上实现了专业级对弈水平。

Conclusion: 该研究证明了通过参数化“直觉”替代大规模运行时搜索的可行性，为在资源受限环境中部署高性能围棋AI提供了新途径，并可能推广到其他需要深度搜索的决策任务。

中文摘要：近期由AlphaZero和MuZero推动的计算机围棋进展，严重依赖蒙特卡洛树搜索（MCTS）来纠正神经网络策略的错误。虽然在大规模计算集群上有效，但这种依赖在消费级硬件上造成了关键瓶颈，因为树管理的计算成本严重限制了推理速率。此外，在没有深度搜索的情况下，这些模型会出现幻觉，以高置信度提出战略上致命的走法。本文引入了一种新颖的信念引导（Belief-Guided）架构，将策略头（Policy head）与独立的信念头（Belief head）解耦。与传统价值函数不同，信念头充当内部模拟器和独立评论家，建模认知不确定性和战略稳定性。通过集成记忆机制（Transformer/GRU）来处理长期依赖和劫（Ko）规则，并利用门控机制过滤过度自信的策略错误，我们的模型将智能的负担从运行时搜索转移到参数化的“直觉”上。实验结果表明，该方法显著提高了无搜索胜率并减少了幻觉，使得在大型MCTS不可行的有限硬件上实现专业级对弈成为可能。

Original abstract: Recent advancements in Computer Go, driven by AlphaZero and MuZero, rely heavily on Monte Carlo Tree Search (MCTS) to correct the errors of the neural network policy. While effective on massive computational clusters, this dependence creates a critical bottleneck on consumer-grade hardware, where the computational cost of tree management severely limits inference rates. Furthermore, without deep search, these models suffer from hallucination, proposing moves with high confidence that are strategically fatal. This paper introduces a novel Belief-Guided architecture that disentangles the Policy head from a distinct Belief head. Unlike traditional value functions, the Belief head acts as an internal simulator and independent critic, modeling epistemic uncertainty and strategic stability. By integrating memory mechanisms (Transformer/GRU) to handle long-term dependencies and the Ko rule, and utilizing a gating mechanism to filter overconfident policy errors, our model shifts the burden of intelligence from runtime search to parametric "intuition." Experimental results demonstrate that this approach significantly improves search-free win rates and reduces hallucination, enabling professional-level play on limited hardware where massive MCTS is infeasible.

</details>


### [44] [Setoka: A Benchmark for Hierarchical User Understanding in Personalized Agents over Heterogeneous Data](https://arxiv.org/abs/2607.27056)
...
Selection reason: LLM agent长程办公任务基准，含经济验证，对agent评测有直接贡献

TL;DR: OmegaUse-OfficeVal是一个用于评估LLM智能体在办公套件长时序任务上的基准，包含100个任务和经济学信号，发现LLM虽成本更低、速度更快，但交付质量尚未达到人类水平。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对办公套件工作流中LLM智能体成本效益的评估，需要引入经济指标来合理评估智能体的实际可用性。

Method: 构建包含100个来自从业者请求并经隐私保护改编的任务，每个任务配有人工劳动时间和价格代理两个经济信号，并开发基于代码的验证器以支持稳定评估；评估多个前沿LLM与人类基线。

Result: 被评估的LLM在成本和速度上显著优于人类，但交付质量未达到人类水平。

Conclusion: 该基准及配套资源已开源，为未来LLM智能体在办公套件场景中的经济性评估提供了基础，但当前LLM仍需提升质量以接近人类表现。

中文摘要：大型语言模型（LLM）智能体日益被期望协助用户完成任务。然而，现有基准在评估智能体能否以合理成本执行办公套件工作流方面提供的支持有限。我们引入了OmegaUse-OfficeVal，一个用于评估LLM智能体在具有任务级经济基础的长期办公套件任务上的基准。该基准包含100个任务，这些任务源自从业者提出的办公套件请求，并通过隐私保护流程进行改编。平均而言，这些任务需要2.32小时的人工劳动才能完成。该基准的一个重要特点是每个任务都配有两个经济信号：人工劳动时间和任务价格代理。这些信号能够直接比较人工成本与LLM推理成本，以及进行价值加权评估。为了支持稳定的评估，我们从细粒度评分标准中开发了基于代码的验证器。我们评估了多个前沿LLM以及一个人类基线。尽管所有被评估的LLM都比人类工作人员便宜得多、快得多，但它们尚未达到人类水平的交付质量。代码和数据集已完全开源，更多信息可在我们的项目网站上获取：https://omegause-officeval.github.io。
...
### [47] [Partner Capability Estimation for Task-Agnostic Adaptation in Ad-Hoc Teamwork](https://arxiv.org/abs/2607.27177)
...
Selection reason: 评估AI agent进行开放式研究能力，对agent长程任务与验证有直接贡献

TL;DR: 提出“影子评估”新方法，让AI代理承担未发表论文的核心研究问题，由原作者评分。测试显示代理能完成工程但无法取得实质研究进展，存在五类失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有评估无法衡量代理的开放式AI研究能力：窄任务排除开放式研究，盲审存在随机性和低质量。需要第三种方法。

Method: 运行影子评估：让前沿代理在六天和大量算力下，解决两篇未发表NeurIPS 2026论文的核心研究问题，由原作者评估结果；并用第二模型和脚手架进行稳健性检查。

Result: 代理完成所有工程，但未取得实质性研究进展，两篇均被明确拒绝；识别出五个失败模式，且稳健性检查复现。

Conclusion: 当前代理能胜任AI研究的工程部分，但难以应对研究生命周期的关键环节，如判断力、创造性和资源意识。

中文摘要：关于爆炸性AI进展的预测依赖于AI代理自动化AI研究。但关于代理能否进行开放式AI研究的证据仍然薄弱。当前的评估要么在狭窄、可验证的任务上测试代理，这排除了开放式研究；要么将AI生成的论文提交给盲审同行评审，而盲审负担过重、具有随机性，并且评审质量不佳。我们引入了第三种衡量AI研发自动化进展的方法。代理承担一篇高质量未发表论文的核心开放式研究问题，并由论文的原始作者对其输出进行评分。我们称之为影子评估。我们对两篇未发表的NeurIPS 2026投稿进行了影子评估，为前沿代理提供了六天时间和数千美元的计算资源。代理在没有人类帮助的情况下完成了所有工程，但未能朝着回答研究问题取得实质性进展。因此，两篇论文都被作者明确拒绝。我们确定了五个反复出现的失败模式：对可发表研究的门槛判断力差、对研究设计缺陷的应对缺乏创造性、从死胡同中无效回溯、资源意识薄弱以及指令漂移。使用第二种模型和脚手架进行的稳健性检查再现了这些失败。我们发布了专家评审、调查回复、代理仓库和日志。我们的结果提供了早期证据，表明当今的代理可以完成AI研究的工程部分，但在研究生命周期的关键部分存在困难。

Original abstract: Forecasts of explosive AI progress hinge on AI agents automating AI research. But evidence on whether agents can carry out open-ended AI research is thin. Current evaluations either test agents on narrow, verifiable tasks, which excludes open-ended research, or submit AI-generated papers to blind peer review, which is overstretched, stochastic, and suffers from poor review quality. We introduce a third way to measure progress towards AI R\&D automation. An agent takes on the central, open-ended research question of a high-quality unpublished paper, and the paper's original authors grade its output. We call these shadow evaluations. We ran shadow evaluations on two unpublished NeurIPS 2026 submissions, giving frontier agents six days and thousands of dollars of compute. The agents completed all of the engineering without human help, yet could not make substantial progress towards answering the research questions. As a result, both papers were unambiguously rejected by the authors. We identify five recurring failure modes: poor judgment about the bar for publishable research, uncreative responses to shortcomings in the research design, ineffective backtracking from dead ends, poor resource awareness, and instruction drift. A robustness check with a second model and scaffold reproduced these failures. We release the expert reviews, survey responses, agent repositories, and logs. Our results provide early evidence that today's agents can do the engineering of AI research, but struggle with critical parts of the research lifecycle.
...
- [Try Again, Don't Look Back: Blind Resampling Outperforms Self-Repair in Small Code Models](https://arxiv.org/abs/2607.26117) — 相关度 0.92：对照实验揭示小代码模型自修复的锚定效应，盲重采样在同等预算下更优，直接影响agent重试与反馈策略设计。
- [CodeSpec: Dual Executable Specifications for Agentic Long-Horizon Feature Development](https://arxiv.org/abs/2607.26777) — 相关度 0.90：为长程代码 agent 提供可执行双规格方法，提升功能开发的设计-实现一致性，直接相关。
- [ExplainBench: Evaluating Code Explanations from Agents](https://arxiv.org/abs/2607.26451) — 相关度 0.90：首个自动评估代码agent解释可信度的基准，揭示解释质量与修复能力解耦，并验证审计agent可提升解释可靠性。
- [MindForge: Teaching Small Language Models Whole-Life-Cycle Software Engineering via Source-Free Program Synthesis](https://arxiv.org/abs/2607.27146) — 相关度 0.90：构建全生命周期软件工程训练环境与合成轨迹，显著提升小模型在多种代码任务上的能力。
- [MultiFixer: A Coordinator-Proposer Based Multi-Agent Framework For Fixing Multi-Hunk Bugs](https://arxiv.org/abs/2607.26591) — 相关度 0.90：面向多 hunk 缺陷修复的多智能体框架，直接解决 coding agent 的协调与长程修复问题。
- [VITAL-RAG: Invariance Race for Context Allocation in Coding Agents](https://arxiv.org/abs/2607.26937) — 相关度 0.90：针对编码 agent 的 RAG 上下文分配方法，显著提升检索质量并节省 token，直接相关。
- [Do Code Language Models Use Tests? A Behavioral and Representational Study of Test-Driven Code Generation](https://arxiv.org/abs/2607.26244) — 相关度 0.85：实证研究测试对代码模型生成的作用，发现表征偏移与正确性提升脱节，对测试驱动代码agent评估与设计有直接参考价值。
- [MRCoder: An Efficient Context Selecting Approach for Repository-Level Code Generation](https://arxiv.org/abs/2607.26805) — 相关度 0.85：针对仓库级代码生成的高效上下文选择方法，大幅减少 token 消耗并提升生成质量。
- [SpecFirst: Behavioral Specification Elicitation as a First-Class Step in Agent-Based Program Synthesis from Scratch](https://arxiv.org/abs/2607.27167) — 相关度 0.85：将行为规格提取作为代码合成前的显式阶段，提升从零程序构建的 agent 成功率。
- [Cross-Model Cross-Language AI Coding Agent Performance: Accuracy and Speed of Parallel CLRS Algorithms](https://arxiv.org/abs/2607.26083) — 相关度 0.84：跨模型跨语言评测coding agent生成并行算法的正确性与speedup，强调运行时性能作为主要指标，直接支撑SemOpt相关评估。
- [Multi-Agent Debate Strategies: Survey, Taxonomy, and Challenges](https://arxiv.org/abs/2607.26212) — 相关度 0.76：系统综述多agent辩论策略并提出三维分类，为代码agent多智能体协作、交互与评测提供可迁移框架。
- [SARC-DQ: Runtime Data-Quality Gating for Agentic AI: Silent Evidence Defects, the Incompetence Shield, and Downstream-Only Remediation](https://arxiv.org/abs/2607.26313) — 相关度 0.76：提出运行时数据质量门控机制，解决agent证据缺陷导致的错误动作，可迁移到代码agent的工具结果校验与数据新鲜度检查。
- [TraceCoder: Explainable and Auditable Code Generation with Position-Key Snippet Versioning](https://arxiv.org/abs/2607.26307) — 相关度 0.75：提出可审计、可解释的代码生成追踪机制，对代码 agent 的调试、验证和可信度有迁移价值。
- [A First Look at Coding Agents' Compliance with AI Contribution Rules in Open-Source Communities](https://arxiv.org/abs/2607.26819) — 相关度 0.70：实证研究代码 agent 对开源贡献规则的遵守情况，涉及验证、披露、拒绝等可迁移机制。
- [Position: Evaluation Scores Are Perishable Knowledge Claims](https://arxiv.org/abs/2607.26191) — 相关度 0.70：提出评测分数应带形式性、作用域与有效期元数据，并采用最弱链聚合，适用于代码agent基准结果的可信度评估。
- [When Knowledge Changes: Metamorphic Testing of RAG Systems with Mutations](https://arxiv.org/abs/2607.26843) — 相关度 0.65：提出 RAG 系统在语料演化下的蜕变测试框架，可迁移到代码 agent 中 RAG 的鲁棒性验证。
- [Impossible to hide secret ...: Uncovering Security and Privacy Issues in LLM-native IDEs](https://arxiv.org/abs/2607.26390) — 相关度 0.62：系统梳理LLM原生IDE的安全隐私问题与开发者不信任来源，为代码agent安全设计与沙箱机制提供实证依据。
- [How Developers Experience Debugging Unfamiliar Codebases with Code Tours Generated and Evaluated by Local LLMs](https://arxiv.org/abs/2607.26987) — 相关度 0.60：研究 LLM 生成代码导览对调试体验与信任校准的影响，对 agent 解释与交互设计有参考价值。
- [Model-Driven Requirements Configuration with Three-Valued Uncertainty Scoring](https://arxiv.org/abs/2607.26220) — 相关度 0.55：提出神经符号多agent架构，用确定性符号校验与三值不确定性评分约束LLM生成，验证机制可迁移到代码生成的结构正确性保障。
- [Tangling Pull Requests: Curating a Commit Untangling Dataset from Merged PRs](https://arxiv.org/abs/2607.26730) — 相关度 0.55：提供了从 PR 构建 commit 拆解数据集的通用方法，可作为代码理解和 agent 训练的数据支撑。
- [Validating ETCS Data with the B Mathematical Language: An Industrial Pipeline and a Blueprint for LLM Integration](https://arxiv.org/abs/2607.26111) — 相关度 0.52：LLM生成+形式化裁决+人工确认的安全攸关数据验证流水线，其验证闭环模式可迁移至代码agent的安全关键场景。
