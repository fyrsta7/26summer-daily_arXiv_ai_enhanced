...
- [Agentic Fuzzing: Opportunities and Challenges](http://arxiv.org/abs/2605.10074v1) — 相关度 0.95：提出 agentic fuzzing，用 LLM agent 直接推理 root cause 找逻辑 bug，成果显著且方法论可迁移。
- [CppPerf: An Automated Pipeline and Dataset for Performance-Improving C++ Commits](http://arxiv.org/abs/2605.10890v1) — 相关度 0.95：提供性能改进C++提交的自动化挖掘管道与数据集，直接面向程序性能优化与修复评测。
- [CrackMeBench: Binary Reverse Engineering for Agents](http://arxiv.org/abs/2605.10597v1) — 相关度 0.90：引入CrackMeBench基准，评估语言模型智能体的二进制逆向工程能力，提供可复现的测试床，直接相关于编码智能体评测。
- [Primal Generation, Dual Judgment: Self-Training from Test-Time Scaling](http://arxiv.org/abs/2605.11299v2) — 相关度 0.90：提出从测试时缩放中自我训练以提升代码生成与判断能力，强化学习方法可直接迁移。
- [Shepherd: Enabling Programmable Meta-Agents via Reversible Agentic Execution Traces](http://arxiv.org/abs/2605.10913v3) — 相关度 0.90：提出可逆执行迹的元智能体平台，支持监督、反事实优化与训练，对智能体工作流有直接贡献。
- [Beyond Autonomy: A Dynamic Tiered AgentRunner Framework for Governable and Resilient Enterprise AI Execution](http://arxiv.org/abs/2605.10223v1) — 相关度 0.85：提出风险分层、分权制衡与验证恢复的 agent 执行框架，可借鉴到代码 agent 的治理与可靠性设计。
- [Combining Mechanical and Agentic Specification Inference for Move](http://arxiv.org/abs/2605.10005v2) — 相关度 0.85：将 WP 分析与 Claude Code 等 agentic CLI 结合做形式化规格推断，直接展示 agent 与验证工具协同。
- [Comment and Control: Hijacking Agentic Workflows via Context-Grounded Evolution](http://arxiv.org/abs/2605.11229v1) — 相关度 0.85：系统分析并利用agentic工作流中的提示注入风险，提出的上下文演化框架对安全agent有参考价值。
- [ComplexMCP: Evaluation of LLM Agents in Dynamic, Interdependent, and Large-Scale Tool Sandbox](http://arxiv.org/abs/2605.10787v2) — 相关度 0.85：提供动态、相互依赖的大规模工具沙箱基准，评估LLM智能体工具调用与恢复能力，含详细失败分析。
- [Deterministic vs. LLM-Controlled Orchestration for COBOL-to-Python Modernization](http://arxiv.org/abs/2605.09894v1) — 相关度 0.85：控制实验表明确定性编排在同等精度下更稳更省，为 coding agent 工作流设计提供了关键证据。
- [Instruction Adherence in Coding Agent Configuration Files: A Factorial Study of Four File-Structure Variables](http://arxiv.org/abs/2605.10039v1) — 相关度 0.85：大规模因子实验揭示配置结构对 coding agent 指令遵循无显著影响，可纠正 agent 设计假设。
- [Step Rejection Fine-Tuning: A Practical Distillation Recipe](http://arxiv.org/abs/2605.10674v1) — 相关度 0.85：提出步骤级拒绝微调SRFT，在SWE-bench上提升智能体解决率，对代码智能体训练有直接迁移价值。
- [Usability as a Weapon: Attacking the Safety of LLM-Based Code Generation via Usability Requirements](http://arxiv.org/abs/2605.10133v1) — 相关度 0.85：揭示可用性需求可诱使 coding LLM 丢失安全约束，对 agent 代码生成安全性评测有直接价值。
- [AutoSOUP: Safety-Oriented Unit Proof Generation for Component-level Memory-Safety Verification](http://arxiv.org/abs/2605.10712v1) — 相关度 0.80：自动化组件级内存安全验证，结合LLM与确定性程序合成，对程序分析与验证有方法论贡献。
- [LLM-Based Code Documentation Generation and Multi-Judge Evaluation](http://arxiv.org/abs/2606.09852v1) — 相关度 0.80：用多 LLM 作为裁判评估文档生成质量，可迁移到代码生成质量评估与模型选择。
- [Natural Language based Specification and Verification](http://arxiv.org/abs/2605.11315v1) — 相关度 0.80：探索用自然语言进行规格说明与组合验证，对LLM代码验证有方法论贡献。
- [Quantitative Symbolic Patch Impact Analysis](http://arxiv.org/abs/2605.13885v1) — 相关度 0.80：提出定量偏等价分析，量化补丁行为差异，对程序分析与验证有明确方法贡献。
- [BenchCAD: A Comprehensive, Industry-Standard Benchmark for Programmatic CAD](http://arxiv.org/abs/2605.10865v2) — 相关度 0.70：面向工业CAD的多模态代码生成基准，评测范式可迁移至其他领域代码生成任务。
- [Correct-by-Construction G-Code Generation: A Neuro-Symbolic Approach via Separation Logic](http://arxiv.org/abs/2605.10568v3) — 相关度 0.70：神经符号 G-code 生成结合 LLM 与分离逻辑证明器，并利用证明失败做自纠正，对正确性约束代码生成有借鉴。
- [On Problems of Implicit Context Compression for Software Engineering Agents](http://arxiv.org/abs/2605.11051v1) — 相关度 0.70：分析隐式上下文压缩在软件工程智能体上的失败现象，对智能体长程上下文处理有参考价值。
- [Continuous Discovery of Vulnerabilities in LLM Serving Systems with Fuzzing](http://arxiv.org/abs/2605.11202v1) — 相关度 0.65：面向LLM推理引擎的模糊测试方法，可迁移至并发系统的安全与可靠性测试。
- [Formal Verification of Imperative First-Class Functions in Move](http://arxiv.org/abs/2605.10007v2) — 相关度 0.65：Move 函数值的形式化验证与规格推断方法，可为代码 agent 提供验证后端与正确性保证。
- [PaperFit: Vision-in-the-Loop Typesetting Optimization for Scientific Documents](http://arxiv.org/abs/2605.10341v1) — 相关度 0.65：视觉闭环迭代修复的 agent 用于排版优化，其 render-diagnose-repair 模式可迁移到代码生成/验证场景。
- [LLARS: Enabling Domain Expert & Developer Collaboration for LLM Prompting, Generation and Evaluation](http://arxiv.org/abs/2605.10593v1) — 相关度 0.60：提供 prompt 协作、批量生成与混合评估平台，其评估机制可迁移到代码 agent 的选择与评测。
- [TeleResilienceBench: Quantifying Resilience for LLM Reasoning in Telecommunications](http://arxiv.org/abs/2605.09929v1) — 相关度 0.60：提出 reasoning resilience 基准与 CFR 指标，可迁移到代码 agent 对错误推理的恢复能力评估。
- [VISOR: A Vision-Language Model-based Test Oracle for Testing Robots](http://arxiv.org/abs/2605.10408v2) — 相关度 0.60：基于 VLM 的测试预言机与不确定性量化，可迁移到代码 agent 的测试自动化与结果验证。
- [MARGIN: Margin-Aware Regularized Geometry for Imbalanced Vulnerability Detection](http://arxiv.org/abs/2605.10240v3) — 相关度 0.55：针对不平衡漏洞检测的几何度量学习方法，可迁移到代码缺陷分析与表示学习，但与 agent 关系较远。
