...
- [A Single Patch Is Not Enough: Deterministic Fusion of Repair Candidates](http://arxiv.org/abs/2607.01597v1) — 相关度 0.95：确定性融合LLM修复候选补丁，提升coding agent的pass@1，直接解决评测与选择问题。
- [Can Coding Agents Implement Missed Compiler Optimizations? Evaluating LLM Agents on LLVM Peephole Optimizations](http://arxiv.org/abs/2607.02684v1) — 相关度 0.95：提出 PeepholeBench，评测 LLM agent 补全 LLVM 缺失 peephole 优化，与程序优化和 coding agent 直接相关。
- [Understanding Agent-Based Patching of Compiler Missed Optimizations](http://arxiv.org/abs/2607.02370v2) — 相关度 0.95：系统研究 agent 修复编译器漏优化并引入历史知识增强，与 SemOpt 核心直接相关。
- [Beyond Textual Repository Exploration: Dual-Modal Structural Reasoning for Agentic Issue Resolution](http://arxiv.org/abs/2607.01929v1) — 相关度 0.92：提出双模态结构图谱增强 issue-resolution agent 的仓库探索，并在 SWE-bench 上验证，直接相关。
- [Coding Agents Are Guessing: Measuring Action-Boundary Violations in Underspecified DevOps Instructions](http://arxiv.org/abs/2607.02294v1) — 相关度 0.92：提出 UnderSpecBench 测量 coding agent 在欠指定 DevOps 指令下的越界行为，直接相关安全性评估。
- [Hawk: Harnessing Hardware-Aware Knowledge for High-Performance NPU Kernel Generation](http://arxiv.org/abs/2607.01590v2) — 相关度 0.90：面向NPU的硬件感知LLM内核生成，结合运行时反馈与知识蒸馏，直接用于性能优化。
- [Refploit: Facilitating Exploit Construction via Code-Agent Trajectory Repair](http://arxiv.org/abs/2607.01760v1) — 相关度 0.90：基于代码agent轨迹修复的漏洞利用生成，直接研究agent修复与验证机制。
- [TestEvo-Bench: An Executable and Live Benchmark for Test and Code Co-Evolution](http://arxiv.org/abs/2607.02469v1) — 相关度 0.90：提出可执行、实时的测试与代码协同演化基准，直接评测测试自动化 agent 的生成与更新能力。
- [Archer: Towards Agentic Review for Compiler Optimizations](http://arxiv.org/abs/2607.01808v1) — 相关度 0.85：面向编译器优化的agentic评审工具，结合义务引导和确定性验证，直接相关。
- [Characterizing and Bridging the Diagnostic Gap in eBPF Verifier Rejections](http://arxiv.org/abs/2607.02748v1) — 相关度 0.85：重构 eBPF 验证器拒绝定位并评测 LLM 修复，对程序分析和 agent 修复任务有直接价值。
- [Regression Accumulation in Multi-Turn LLM Programming Conversations](http://arxiv.org/abs/2607.01855v1) — 相关度 0.85：多轮编程对话回归累积研究，提出Verification Gate策略提升agent可靠性。
- [SkillFuzz: Fuzzing Skill Composition for Implicit Intents Discovery in Open Skill Marketplaces](http://arxiv.org/abs/2607.02345v1) — 相关度 0.85：将技能组合模糊测试用于发现 LLM agent 隐式意图，提供执行前安全测试方法。
- [Cloak and Detonate: Scanner Evasion and Dynamic Detection of Agent Skill Malware](http://arxiv.org/abs/2607.02357v2) — 相关度 0.82：针对 agent skill 恶意载荷的规避与动态检测，直接提升 coding agent 供应链安全。
- [Guiding Human Validation of LLM-Generated Code via Verifiable Literate Programming](http://arxiv.org/abs/2607.02333v1) — 相关度 0.82：可验证文生编程框架以 NL 文档为中间层，支持 LLM 代码的细粒度人工验证与修复。
- [Steerability via constraints: a substrate for scalable oversight of coding agents](http://arxiv.org/abs/2607.02389v1) — 相关度 0.82：用约束与工具实现 coding agent 的可扩展监督，显著提高后门检测率。
- [A Systematic Methodology for Evaluating Failure Independence in LLM-Generated Code](http://arxiv.org/abs/2607.02808v1) — 相关度 0.80：系统评估 LLM 生成代码的失效独立性，对多版本/多 agent 可靠性设计与验证有参考。
- [Reasoning effort, not tool access, buys first-try reliability in agentic code generation: an observational study](http://arxiv.org/abs/2607.02436v1) — 相关度 0.80：观测研究表明推理努力而非工具访问决定 agentic 代码生成首次可靠性，对 agent 配置有直接指导。
- [Prompt Coverage Adequacy](http://arxiv.org/abs/2607.02057v1) — 相关度 0.78：提出 prompt 覆盖充分度作为 LLM 生成代码的测试准则，直接关联代码生成验证。
- [Underspecification does not imply Incoherence: The Risks of Semantic Collapse in Coding Models](http://arxiv.org/abs/2607.01953v1) — 相关度 0.78：发现 LLM 在提示欠指定时发生语义坍缩，影响代码生成评测与正确性估计，可迁移到 agent 可靠性。
- [Knowledge Over Parameters: Evolving Smart Contract Vulnerability Detection](http://arxiv.org/abs/2607.01742v1) — 相关度 0.75：知识演化驱动的智能合约漏洞检测，结合运行时与LLM推理，可迁移至代码分析agent。
- [Mitigating Package Hallucinations in Large Language Models via Model Editing](http://arxiv.org/abs/2607.02052v1) — 相关度 0.75：通过模型编辑抑制包幻觉，直接提升 LLM 代码生成与依赖配置的可靠性。
- [KRCA: An Efficient Root Cause Analysis System in Hyper-Scale Microservice Systems via Agentic AI](http://arxiv.org/abs/2607.01788v1) — 相关度 0.70：微服务RCA多agent系统，采用记忆增强与因果约束，可迁移到agent协作与可靠性。
- [AgentFlow: Building Agent Dependency Graphs for Static Analysis of Agent Programs](http://arxiv.org/abs/2607.01640v1) — 相关度 0.65：构建agent程序依赖图的静态分析方法，可迁移到代码agent的治理、安全与理解。
- [A Preliminary Study on Explaining Risk of Code Changes using LLM-Based Prediction Models](http://arxiv.org/abs/2607.02782v1) — 相关度 0.60：利用 LLM 注意力突出 diff 中风险行，可为代码审查 agent 提供轻量风险引导。
- [AI Writes Faster Than Humans Can Review: A Longitudinal Study of an Enterprise 2x Mandate](http://arxiv.org/abs/2607.01904v1) — 相关度 0.60：纵向实证研究 AI 编码工具采纳与代码审查自动化，对评估 coding agent 的工程化影响有参考价值。
- [LLMoxie: Exploring Agentic AI for Scientific Software Development](http://arxiv.org/abs/2607.02703v1) — 相关度 0.60：描述面向科学软件的 agentic AI 平台与插件技能体系，其工作流、可审计性和领域适配经验可迁移到代码 agent。
- [When Agents Do Not Stop: Uncovering Infinite Agentic Loops in LLM Agents](http://arxiv.org/abs/2607.01641v1) — 相关度 0.60：静态分析检测LLM agent无限循环，提升agent可靠性的通用方法。
- [Rethinking Complexity Metrics for LLM-Integrated Applications: Beyond Source Code](http://arxiv.org/abs/2607.01903v1) — 相关度 0.55：提出面向LLM集成应用的双层复杂度度量，可用于agent软件工程评估。
- [Epic-Organized vs. Requirement-Aligned Gherkin: An Empirical Evaluation of LLM-Based Acceptance Criteria Generation](http://arxiv.org/abs/2607.01980v1) — 相关度 0.52：比较 epic-organized 与 requirement-aligned 的 LLM 验收标准生成策略，对 agent 生成测试类工件有参考价值。
