...
- [RepoMirage: Probing Repository Context Reasoning in Code Agents with Perturbations](http://arxiv.org/abs/2605.26177v1) — 相关度 0.92：提出仓库上下文推理评测套件与扰动诊断，直接评估代码agent能力并给出结构感知改进方案。
- [SetupX: Can LLM Agents Learn from Past Failures in Functionality-Correct Code Repository Setup?](http://arxiv.org/abs/2605.26186v2) — 相关度 0.88：提出基于经验学习的仓库配置agent框架，解决跨仓库迁移与多步试错，直接相关代码agent。
- [Trustworthy Software Project Generation : a Case Study with an Interactive Theorem Prover](http://arxiv.org/abs/2605.26017v1) — 相关度 0.87：用交互式定理证明器实现全自动可信代码项目生成，结合反馈修复，对代码生成与验证agent有重要价值。
- [ESBMC: A Survey of Its Evolution, Integration, and Future Directions in Formal Software Verification](http://arxiv.org/abs/2605.26169v2) — 相关度 0.80：综述ESBMC形式验证工具与LLM及自主agent验证内核的集成，对验证型agent和神经符号验证有清晰参考价值。
- [A Universal Cliff and a Design Fingerprint: Cross-Section Defect Detection Under LLM Orchestration](http://arxiv.org/abs/2605.26174v1) — 相关度 0.78：揭示多agent编排下的跨章节缺陷检测悬崖结构性问题，对代码agent多agent协作可靠性有重要启发。
- [CelerLog: Fast Log Parsing via Dynamic Routing](http://arxiv.org/abs/2605.26005v1) — 相关度 0.76：提出动态路由混合统计与LLM的日志解析方法，显著提速降本，可迁移到日志分析工具。
- [Leveraging Language Models for Log Statement Generation in Multilingual Scenarios: How Far Are We?](http://arxiv.org/abs/2605.25374v1) — 相关度 0.75：评估多语言环境下LLM日志生成，提出多语言基准，对代码生成与软件维护agent有迁移价值。
- [Names Are All You Need: Effective and Safe Regression Test Selection for Python](http://arxiv.org/abs/2605.25356v1) — 相关度 0.75：提出基于细粒度依赖图的Python回归测试选择方法，安全且高效，对测试优化和精准分析有方法迁移价值。
- [Beyond Summaries: Structure-Aware Labeling of Code Changes with Large Language Models](http://arxiv.org/abs/2605.26100v1) — 相关度 0.74：提出结构感知的代码变更标签流水线，用于代码审查自动化，可迁移到agent的补丁理解。
- [How Agentic AI Coding Assistants Become the Attacker's Shell](http://arxiv.org/abs/2605.25871v1) — 相关度 0.74：系统分析agentic编码助手的提示注入攻击面与防御局限，对代码agent安全性评估有直接价值。
- [Meta-Engineering Harnesses for AI-Native Software Production: A Contract-Driven Adversarial Verification Architecture with Early Deployment Report](http://arxiv.org/abs/2605.25665v1) — 相关度 0.72：提出AI原生软件生产的元工程化框架，含合同驱动验证与失败仲裁，可迁移到代码agent工作流。
- [Temporal Modeling of Change History for Black-Box Test Suite Minimization](http://arxiv.org/abs/2605.25441v1) — 相关度 0.70：提出时序风险驱动的测试套件最小化方法，利用版本历史与调用图，可迁移到测试优化agent。
- [FuzzPilot: Plateau-Triggered Recipe Validation for Structured Text Fuzzing](http://arxiv.org/abs/2605.26539v1) — 相关度 0.68：提出AFL++的库平台触发验证控制器，将LLM用于fuzzing配方生成，方法可迁移到程序优化。
- [A Tertiary Review of Large Language Model-Based Code Generating Tasks: Trends, Challenges, and Future Directions](http://arxiv.org/abs/2605.25536v1) — 相关度 0.66：三级综述系统梳理LLM代码生成任务的趋势与挑战，为评测与研究方向提供参考。
- [When Gradients Collide: Failure Modes of Multi-Objective Prompt Optimization for LLM Judges](http://arxiv.org/abs/2605.26046v2) — 相关度 0.65：分析多目标文本梯度优化的失败模式，对基于LLM的判断器优化有参考价值。
- [KYA: A Framework-Agnostic Trust Layer for Autonomous Systems with Verifiable Provenance and Hierarchical Policy Composition](http://arxiv.org/abs/2605.25376v2) — 相关度 0.62：为自治agent设计信任与治理层，其策略组合与验证机制可迁移到代码agent的可信性保障。
- [Workflow Closure Is Not Scientific Closure in Auto-Research Systems](http://arxiv.org/abs/2605.26200v1) — 相关度 0.55：讨论自动研究系统的工作流闭包与科学闭包，其目标崩溃与验证机制分析可类比agent评测设计。
