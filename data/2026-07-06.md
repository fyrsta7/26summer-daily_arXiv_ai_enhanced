...
- [Beyond the Need for Speed: Energy-Aware Code Generation via Simulation-Guided Reinforcement Learning](http://arxiv.org/abs/2607.04577v1) — 相关度 0.95：用模拟器反馈训练能效感知代码生成模型，直接结合代码生成与性能优化。
- [KAT-Coder-V2.5 Technical Report](http://arxiv.org/abs/2607.05471v1) — 相关度 0.95：训练自主代码agent的技术报告，涵盖环境重建、RL与多教师蒸馏，直接代表coding agent前沿。
- [ToolFailBench: Diagnosing Tool-Use Failures in LLM Agents](http://arxiv.org/abs/2607.04686v1) — 相关度 0.95：工具调用失败诊断基准，细粒度刻画agent工具使用缺陷，直接适用于代码agent评测。
- [What Do AI Agents Actually Change? An Empirical Taxonomy of Mutation Patterns in Performance-Improving Pull Requests](http://arxiv.org/abs/2607.05666v1) — 相关度 0.95：对AI agent性能改进PR的突变模式进行实证分类，直接揭示agent代码优化行为，为SBSE变异算子提供先验
- [From Failing to Passing: Evolving Natural Language Prompt Optimization Rules for LLM Code Generation](http://arxiv.org/abs/2607.05121v1) — 相关度 0.90：演化自然语言提示词优化规则并组合执行反馈修复，显著提升代码生成能力，直接相关
- [Is Three the Magic Number? An Empirical Evaluation of LLM-Based Repair Loops](http://arxiv.org/abs/2607.05197v1) — 相关度 0.90：系统评估LLM修复循环的迭代次数影响，发现收益递减规律，直接指导agent工作流预算设计
- [ShadowProbe: Language-Extensible Detection of Hidden Algorithmic Complexity Vulnerabilities](http://arxiv.org/abs/2607.05474v1) — 相关度 0.90：检测隐藏算法复杂度漏洞，结合静态分析、LLM测试生成与统计验证，与性能分析和agent工具直接相关。
- [Can LLMs Really Recover Microservice Failures? A Recovery-Aware Evaluation of Diagnosis-to-Action Reasoning](http://arxiv.org/abs/2607.04623v1) — 相关度 0.85：提出诊断到恢复动作的评测框架与基准，揭示LLM在行动有效性上的不足，对agent验证有直接价值。
- [LLM-Based Test Oracles: Source-of-Authority Taxonomy -- A Systematic Literature Review](http://arxiv.org/abs/2607.05031v1) — 相关度 0.85：对LLM测试预言机的权威来源进行系统综述，为agent测试与验证提供完整分类学
- [Latent Programming Horizons in Coding Agents](http://arxiv.org/abs/2607.05188v1) — 相关度 0.85：证明coding agent隐状态线性编码程序属性并预测未来编辑结果，为agent可解释性与监控开辟新方向
- [On the risk of coding before testing: An empirical study on LLM-based test generation workflow](http://arxiv.org/abs/2607.05139v1) — 相关度 0.85：揭示LLM生成代码对后续测试生成的误差传播问题，挑战agentic流程的测试独立性，具重要实证价值
- [RustMizan: A Compilable, Contamination-Aware Benchmarking Framework for Rust Vulnerabilities](http://arxiv.org/abs/2607.04729v1) — 相关度 0.85：可编译防污染的Rust漏洞benchmark，含多粒度定位与变异测试，服务于安全代码agent评测。
- [A Comprehensive Study of Implementation Bugs in Multi-modal Agents](http://arxiv.org/abs/2607.04974v1) — 相关度 0.80：系统研究多模态agent实现缺陷并构建自动bug识别工具，可直接迁移到agent测试与调试
- [Agents with Feelings? Personality and Emotion in Multi-Agent Software Teams](http://arxiv.org/abs/2607.05659v1) — 相关度 0.80：实证研究人格与情绪对多智能体软件团队的影响，识别性能与协作成本的关键设计维度，直接相关
- [An Exploration of Agentic Information Fusion for Test Maintenance Prediction](http://arxiv.org/abs/2607.04786v1) — 相关度 0.80：多agent信息融合预测测试维护，展示静态与语义分析协同，对测试agent有参考价值。
- [Finetuning Lightweight LLMs for Control Flow Graph Generation](http://arxiv.org/abs/2607.04582v1) — 相关度 0.80：微调轻量LLM生成控制流图，为代码分析agent提供统一跨语言程序表示方法。
- [AI Agent Pull Requests on GitHub: Frequency, Structure, and Merge Conflict Rates](http://arxiv.org/abs/2607.04697v2) — 相关度 0.75：实证分析AI agent并发PR与合并冲突，为多agent协作与代码合并风险提供量化依据。
- [EvalLoop: A Methodology for Evaluation-Driven Iterative Improvement of Business AI Systems](http://arxiv.org/abs/2607.05638v1) — 相关度 0.75：提出面向业务LLM系统的评估驱动迭代改进方法论，其维度诊断与失败模式分类可迁移到agent评测与调优
- [Intent-Based Mutation Testing: From Naturally Written Programming Intents to Mutants](http://arxiv.org/abs/2607.05149v1) — 相关度 0.75：提出意图驱动变异测试并用LLM实现，产生与传统变异互补的测试用例，可迁移到agent测试生成
- [QuTuner: Feature- and Learning-Guided Optimization Pass Tuning for Quantum Compilers](http://arxiv.org/abs/2607.04586v1) — 相关度 0.75：特征与学习引导的编译器pass调优框架，其检索排序+轻量精调范式可迁移到经典优化。
- [Teaching LLMs a Low-Resource Language: Enhancing Code Completion in Pharo](http://arxiv.org/abs/2607.04939v1) — 相关度 0.75：为低资源语言训练代码补全LLM的端到端流程，其数据与评测方法可迁移到代码生成agent。
- [A Mechanistic Lens on Semantic Conflicts: Using Activation Patching to Understand LLM Behavior](http://arxiv.org/abs/2607.05587v1) — 相关度 0.70：用激活修补分析LLM在语义冲突下的行为，揭示代码与语义线索的因果机制，对程序理解与测试生成有参考价值
- [Dashboard2Code: Evaluating Multimodal Models on Reconstructing Interactive Dashboards](http://arxiv.org/abs/2607.04727v1) — 相关度 0.70：多模态模型重建交互仪表盘的benchmark，提出动态交互测试评估框架，可迁移到UI代码agent评测。
- [From Conversation to Contribution: Characterizing Coding Agent in Open-Source Software](http://arxiv.org/abs/2607.05677v1) — 相关度 0.70：实证刻画开源软件开发中编码助手的对话式使用及其影响，为编码agent的设计与评估提供参考。
- [Three-Phase Evaluation of AI-Assisted Software Development Life Cycle](http://arxiv.org/abs/2607.05125v1) — 相关度 0.70：实证评估不同AI自主水平对软件开发生产率、需求遵循与认知负荷的影响，对agent工作流设计有参考价值
- [Using Process Mining to Generate AI Agents from Software Engineering Process Records](http://arxiv.org/abs/2607.04948v1) — 相关度 0.70：用过程挖掘从仓库事件日志自动生成项目专属AI agent角色，为agent工作流设计提供新思路。
- [A Temporal Reasoning Benchmarking Framework for LRMs via Difficulty-controlled and Dynamic Test Generation](http://arxiv.org/abs/2607.04784v1) — 相关度 0.60：时间推理benchmark框架，其难度控制与推理忠实度验证可借鉴到agent评测设计。
- [Can Code Specify a System Precisely Enough to Formally Verify It?](http://arxiv.org/abs/2607.05076v1) — 相关度 0.60：研究LLM生成规约进行形式验证时的契约结构影响，为agent驱动的验证任务提供可迁移启示
- [Correctness, confidence, and context: Framing software assurance in the AI age](http://arxiv.org/abs/2607.04667v2) — 相关度 0.60：关于AI时代软件保证的概念性讨论，为agent可靠性与评测思维框架提供参考。
- [LLM-Driven CI-CD Workflow Intelligence for Cyber Systems Engineering](http://arxiv.org/abs/2607.04579v1) — 相关度 0.60：LLM分析CI/CD工作流并生成修复建议，工作流智能方法可迁移到软件工程agent。
- [TypeGo: An OS Runtime for Embodied Agents](http://arxiv.org/abs/2607.05482v1) — 相关度 0.60：提出操作系统式agent运行时，以异步规划与调度隐藏LLM延迟，其架构思想可迁移到coding agent系统
