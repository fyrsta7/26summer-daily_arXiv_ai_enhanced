...
- [CodegenBench: Can LLMs Write Efficient Code Across Architectures?](http://arxiv.org/abs/2606.04023v1) — 相关度 0.95：构建跨架构高效代码生成基准（BLAS与专用kernel），直接评估LLM生成高效代码能力，与SemOpt高度相关。
- [Monitoring Agentic Systems Before They're Reliable](http://arxiv.org/abs/2606.02494v1) — 相关度 0.90：提出agent系统三维监控范围与方差表征方法，验证结构缺陷掩盖任务级错误，对agent可靠性监控有直接价值。
- [Improving LLM-Based Go Code Review through Issue-List Generation and Context Augmentation](http://arxiv.org/abs/2606.01859v1) — 相关度 0.85：提出issue-list评审与上下文增强策略，并系统评估LLM代码评审，方法可迁移到代码agent的反馈生成与评估。
- [Trust-Calibrated Code Review: A Participatory Design Study of Review Workflows for LLM-Generated Multi-File Changes](http://arxiv.org/abs/2606.01969v1) — 相关度 0.85：针对LLM生成的多文件变更提出信任校准的三级评审工作流与设计框架，直接指导agent代码审查工具开发。
- [Algorithmic algorithm development with LLMs: A Case Study on LLM-Usage for Contraction Order Optimization in Tensor Networks](http://arxiv.org/abs/2606.01975v1) — 相关度 0.80：使用验证器引导的进化编码agent优化张量网络收缩顺序，展示LLM算法开发与验证闭环，可迁移到程序优化。
- [Neither Layer Alone: Epistemic Integrity Requires Hierarchical Joint Design for Long-Running AI Agents](http://arxiv.org/abs/2606.04017v1) — 相关度 0.80：提出长程agent模型-框架接口契约和分层完整性设计，对代码agent可靠性架构和跨会话一致性有重要参考。
- [Acceptance-Test-Driven Evaluation Protocols for Business-Centric LLM Systems](http://arxiv.org/abs/2606.02755v1) — 相关度 0.75：提出基于验收测试驱动的LLM系统评估协议，补充发布门禁与行为契约，可迁移到agent的验证与评测流程。
- [An Agentic Approach Towards Replication Package Quality Evaluation](http://arxiv.org/abs/2606.02006v1) — 相关度 0.70：提出多agent自动评估复现包质量，将开放科学标准转化为可验证准则，对agent评测机制有参考价值。
- [Many a Little Makes a Mickle: A Code-Centric Empirical Study of Data Minimization Principle in Android App Development](http://arxiv.org/abs/2606.02960v1) — 相关度 0.60：发现LLM在Android代码生成中重现数据最小化风险，并提出编码准则消除问题，对代码生成agent的合规性评估有参考。
