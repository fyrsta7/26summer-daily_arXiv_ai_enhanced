...
Motivation: 现有视频生成模型在实时交互、长期一致性和动态场景持久记忆方面存在局限，阻碍了其发展为实用的世界模型。需要一种能够统一视频生成、动态场景重建和长期记忆的闭环系统。

Method: 提出生成-重建-引导范式：生成的视频流被连续重建为动态4D时空表示，该表示反过来指导后续生成以保持一致性。采用自回归扩散视频模型，增强宏观-微观规划（MMPL）减少误差累积，结合高效分布匹配蒸馏（DMD）实现实时合成。
...
TL;DR: 提出通过噪声优化解决文本到图像生成模型中的模式崩溃问题，提高生成多样性


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型存在显著的模式崩溃问题，即相同文本提示下生成的图像缺乏多样性。现有方法通过引导机制或候选池筛选来应对，但本文探索不同的噪声优化方向

Method: 采用简单的噪声优化目标来缓解模式崩溃，同时保持基础模型的保真度。分析噪声的频率特性，探索不同频率分布的替代噪声初始化策略以改进优化和搜索

Result: 实验表明噪声优化在生成质量和多样性方面均取得优异结果，能够有效缓解模式崩溃问题

Conclusion: 噪声优化是一种有效的解决文本到图像生成中模式崩溃问题的方法，通过优化噪声而非修改模型本身，在保持模型性能的同时显著提高生成多样性
...
<details>
  <summary>Details</summary>
Motivation: 传统3D物体和材料模拟需要专家知识和耗时的物理参数调整才能获得期望的动态行为，这限制了非专业用户的使用。需要一种能够从自然语言自动推断物理参数的方法，降低模拟门槛。

Method: 1. 使用多模态大语言模型估计材料参数值，并约束在合理范围内；2. 提出可学习的运动蒸馏损失，从预训练视频扩散模型中提取鲁棒的运动先验，同时最小化外观和几何归纳偏差来指导模拟。

Result: 在30多个场景中评估，包括真实世界、人工设计和AI生成的3D物体，涵盖弹性固体、金属、泡沫、沙子、牛顿和非牛顿流体等多种材料。MotionPhysics能够生成由自然语言引导的视觉逼真动态模拟，超越现有技术，同时自动确定物理上合理的参数。

Conclusion: MotionPhysics通过自然语言提示为3D场景推断物理参数，无需真实轨迹指导，结合多模态LLM和运动蒸馏损失，能够生成逼真的动态模拟，自动确定物理合理参数，降低了物理模拟的门槛。
...
TL;DR: Spatial4D-Bench是一个用于评估多模态大语言模型4D空间智能的大规模基准测试，包含约40,000个问答对，覆盖18个任务和6个认知类别，揭示了当前MLLMs在4D空间推理方面的显著局限性。


<details>
  <summary>Details</summary>
Motivation: 人类天生具备4D空间智能，能够感知和处理物体随时间的变化。本研究旨在评估多模态大语言模型是否能够达到人类水平的4D空间智能，现有空间智能基准测试通常规模较小或多样性不足，无法全面评估MLLMs的4D空间推理能力。

Method: 提出了Spatial4D-Bench基准测试，这是一个大规模、多任务的评估基准，包含约40,000个问答对，覆盖18个明确定义的任务。这些任务被系统性地组织到六个认知类别中：物体理解、场景理解、空间关系理解、时空关系理解、空间推理和时空推理。

Result: 对多种最先进的开源和专有MLLMs进行基准测试，揭示了它们在多种4D空间推理方面的显著局限性，如路线规划、动作识别和物理合理性推理等。

Conclusion: Spatial4D-Bench为评估MLLMs的空间认知能力提供了结构化、全面的基准测试，覆盖了与人类空间智能多样性相平行的广泛任务。研究发现为社区提供了有价值的见解，基准测试有望促进开发更具能力的MLLMs，使其向人类水平的4D空间智能发展。
...
TL;DR: FCMBench-V1.0是一个专门针对金融信贷领域的多模态基准测试，包含4,043张隐私合规图像和8,446个QA样本，用于评估视觉语言模型在信贷风险评估和文档审查中的实际应用能力。


<details>
  <summary>Details</summary>
Motivation: 随着多模态AI在信贷风险评估和文档审查中的广泛应用，迫切需要专门针对金融信贷领域的基准测试，该基准应能反映信贷申请特有的文档和工作流程，包含信贷特定理解和实际鲁棒性，同时保持隐私合规性而不牺牲实用性。

Method: 通过封闭的合成-捕获流水线构建样本：手动合成带有虚拟内容的文档模板，并在内部捕获场景感知图像。评估框架包含三个维度：感知（3个基础任务）、推理（4个信贷特定任务）和鲁棒性（10种实际采集伪影类型）。

Result: 在14个顶级AI公司和研究机构的23个最先进视觉语言模型上进行广泛实验。Gemini 3 Pro作为商业模型获得最佳F1分数（64.61%），Qwen3-VL-235B作为开源基线获得最佳分数（57.27%），而专门针对金融信贷的模型Qfin-VL-Instruct获得最高总体分数（64.92%。鲁棒性评估显示，即使表现最佳的模型在采集伪影下也会出现明显性能下降。

Conclusion: FCMBench-V1.0能够有效区分现代视觉语言模型的性能差异和鲁棒性，为金融信贷领域的多模态AI评估提供了专门、实用且隐私合规的基准测试框架，填补了该领域专业评估工具的空白。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶视觉系统通常将每个场景视为首次遇到，忽略了大多数部署区域已被多次访问的事实。人类驾驶员依赖历史经验，但现有系统缺乏对空间先验的有效利用。

Method: 提出压缩地图先验(CMP)框架，使用二值化哈希图存储历史遍历信息，存储密度仅为32KB/km²（比密集存储减少20倍）。该框架可轻松集成到主流3D感知系统中，几乎不增加计算成本。

Result: 在nuScenes数据集上，CMP显著且一致地提升了多种架构的3D物体检测性能，验证了空间先验对自动驾驶感知的有效性。

Conclusion: 利用历史遍历数据学习压缩空间先验是提升自动驾驶感知系统的有效方法，能以极低存储成本显著改善3D物体检测性能，且易于集成到现有系统中。
...
TL;DR: 本文提出了FaceFocalDesc问题：为任意选定面部区域生成和识别包含动作单元、情绪状态和年龄估计的多属性自然语言描述，并构建了相应数据集和基于Qwen2.5-VL的Focal-RegionFace模型。


<details>
  <summary>Details</summary>
Motivation: 面部分析中一个未被充分探索的问题：为任意选定面部区域生成和识别多属性自然语言描述。作者认为系统关注个体面部区域的能力能带来更好的理解和控制。

Method: 1) 构建新的多属性描述数据集，为任意选定面部区域提供丰富的区域级标注和自然语言描述；2) 提出基于Qwen2.5-VL的Focal-RegionFace视觉语言模型，通过多个渐进微调阶段逐步细化对局部面部特征的关注，实现可解释的年龄估计、面部动作单元和情绪检测。

Result: Focal-RegionFace在新基准测试中，在传统和广泛使用的指标以及新提出的指标方面都取得了最佳性能，充分验证了其在细粒度多属性面部区域聚焦分析场景中的有效性和多功能性。

Conclusion: 本文成功解决了面部分析中的FaceFocalDesc问题，通过构建数据集和开发Focal-RegionFace模型，实现了对任意选定面部区域的细粒度多属性描述和分析，为面部状态分析提供了新的视角和方法。
...
TL;DR: MorphAny3D是一个无需训练的3D变形框架，利用结构化潜在表示(SLAT)实现高质量跨类别3D变形，通过注意力机制融合源和目标特征生成语义一致且时间平滑的变形序列。


<details>
  <summary>Details</summary>
Motivation: 3D变形在生成语义一致且时间平滑的变形方面仍具挑战性，尤其是在跨类别情况下。现有方法难以处理复杂的3D变形任务，特别是需要保持结构连贯性和时间一致性的场景。

Method: 提出基于SLAT表示的无需训练框架，核心包括：1) Morphing Cross-Attention (MCA) - 在注意力机制中融合源和目标特征以确保结构连贯性；2) Temporal-Fused Self-Attention (TFSA) - 整合前一帧特征增强时间一致性；3) 方向校正策略缓解变形过程中的姿态模糊问题。

Result: 实验表明该方法能生成最先进的变形序列，即使在具有挑战性的跨类别情况下也表现出色。进一步支持解耦变形和3D风格迁移等高级应用，并可泛化到其他基于SLAT的生成模型。

Conclusion: MorphAny3D通过智能融合SLAT特征和创新的注意力机制，实现了高质量、语义一致且时间平滑的3D变形，为跨类别3D变形提供了有效的解决方案，并展示了在高级3D生成任务中的应用潜力。
...
TL;DR: 提出基于多视角图像和神经辐射场（NeRF）的3D实例分割框架，用于农业场景中的精确作物计数，解决了遮挡和聚类问题。


<details>
  <summary>Details</summary>
Motivation: 室外农田环境中，部分遮挡以及从单一视角难以区分聚类作物，给基于图像的作物计数带来巨大挑战。现有方法依赖作物特定参数调整，缺乏鲁棒性。

Method: 利用多视角2D图像，结合神经辐射场（NeRF）进行视图合成，引入作物可见性和掩码一致性评分，结合NeRF的3D信息实现3D实例分割。

Result: 在棉花铃、苹果和梨三个农业数据集上验证，展示了稳定的计数性能，不受作物颜色、形状和尺寸变化影响。相比现有方法表现出优越性能。

Conclusion: 该方法通过3D实例分割实现了精确的作物计数，消除了对作物特定参数调整的依赖，并贡献了棉花植物数据集以促进进一步研究。
...
TL;DR: IntraStyler：一种基于示例的样式合成方法，无需先验知识即可捕获多样化的域内样式，用于增强无监督域适应的图像翻译


<details>
  <summary>Details</summary>
Motivation: 现有无监督域适应方法主要关注源域和目标域之间的域偏移，而域内变异性研究不足。先前方法通常需要预先指定域内变化进行样式合成，这在实际应用中不切实际

Method: 提出IntraStyler方法，使用示例图像指导样式合成，使输出样式与示例样式匹配。引入基于对比学习的样式编码器来提取纯样式特征，无需任何先验知识

Result: 在CrossMoDA 2023数据集上的实验表明，该方法在可控样式合成方面有效，多样化的合成数据对下游分割任务有益

Conclusion: IntraStyler能够无需先验知识捕获多样化的域内样式，通过可控样式合成增强图像翻译，从而改善无监督域适应性能
...
TL;DR: 该研究提出使用强化学习来增强多模态大语言模型的视觉推理能力，通过设计针对不同推理方面的奖励函数，显著提升了模型在视觉谜题等需要准确视觉感知任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型生成的推理链缺乏视觉信息的整合，限制了其在需要准确视觉感知的任务（如视觉谜题）上的表现。研究表明视觉感知是这类任务的关键瓶颈，将图像转换为文本描述能显著提升性能，但需要更有效的方法来解锁模型的长期视觉推理能力。

Method: 采用奖励驱动的强化学习机制，设计了六种针对不同推理方面的奖励函数，包括图像理解、思维步骤和答案准确性。使用组相对策略优化（GRPO）方法，明确激励更长、结构化的推理，并减轻视觉信息的绕过问题。

Result: 在Qwen-2.5-VL-7B模型上实现了5.56%的性能提升，在领域内和领域外设置中都取得了稳定的增益。实验还表明，将图像转换为文本描述能使Claude 3.5和Claude 3.7的性能分别提升26.7%和23.6%。

Conclusion: 强化学习是解锁开源多模态大语言模型长期视觉推理能力的有效机制，无需昂贵的监督。通过精心设计的奖励函数和GRPO优化，可以显著提升模型在需要准确视觉感知任务上的表现，解决了视觉信息整合不足的关键瓶颈。
...
TL;DR: LooC是一种新型向量量化方法，通过低维组合码本和参数自由的外推-插值机制，在保持高性能的同时显著减小码本规模。
...
Method: 1. 引入参数高效的码本设计，将码向量视为特征向量的低维组合单元而非独立匹配；2. 采用参数自由的外推-插值机制增强特征平滑；3. 实现完全码本利用，避免崩溃问题；4. 可作为即插即用模块集成到现有VQ方法中。

Result: 在不同任务、数据集和架构上的广泛评估表明，LooC在显著减小码本规模的同时，超越了现有VQ方法，实现了最先进的性能。

Conclusion: LooC成功解决了向量量化中容量与紧凑性的冲突，通过创新的低维组合码本设计和特征增强机制，为各种下游任务提供了高效且高性能的解决方案。
...
TL;DR: 提出SynDR-IQA框架，通过重塑合成数据分布来解决盲图像质量评估中合成数据训练模型泛化能力有限的问题，采用分布感知的内容上采样和密度感知的冗余聚类下采样策略。


<details>
  <summary>Details</summary>
Motivation: 盲图像质量评估面临大规模标注数据稀缺的挑战，合成数据是潜在解决方案，但现有合成数据集训练的模型泛化能力有限。研究发现合成数据集学习的表征呈现离散聚类模式：高质量图像特征围绕参考图像聚类，低质量图像特征按失真类型聚类，这种分布问题阻碍了回归性能。

Method: 基于样本多样性和冗余对泛化误差影响的理论推导，提出SynDR-IQA框架：1) 分布感知的多样化内容上采样，增强视觉多样性同时保持内容分布；2) 密度感知的冗余聚类下采样，通过减少密集聚类区域的样本密度来平衡样本分布。

Result: 在三种跨数据集设置（合成到真实、合成到算法、合成到合成）上进行了广泛实验，证明了方法的有效性。代码已开源。

Conclusion: 通过重塑合成数据分布，SynDR-IQA框架有效提升了盲图像质量评估模型的泛化能力，解决了合成数据训练中的分布问题。
...
Result: 该方法在低数据条件下有效增强特征学习，增强后的检测器显著优于仅使用有限真实数据训练的模型，接近完全监督训练的性能基准
...
TL;DR: TotalFM是一种基于器官分离概念的放射学基础模型，通过自动化创建器官体积-文本对，结合自监督预训练和对比学习，在3D-CT图像与语言表达对应关系学习中实现计算效率与表征能力的平衡。


<details>
  <summary>Details</summary>
Motivation: 解决放射学基础模型在3D-CT容积数据训练中面临的计算成本约束问题，同时保持模型在各种临床任务中的泛化能力。

Method: 1) 基于器官分离概念，利用分割技术和基于LLM的放射学报告处理自动化创建140,000个序列的器官体积-发现句子对；2) 结合VideoMAE自监督预训练和基于体积-文本对的对比学习；3) 设计器官分离学习框架。

Result: 1) 零样本器官级病变分类任务中，相比CT-CLIP在83%器官上获得更高F1分数，相比Merlin在64%器官上表现更好；2) 零样本发现级病变分类任务中，相比Merlin在83%发现类别上获得更高AUROC；3) 在放射学报告生成任务中表现与现有VLMs相当。

Conclusion: 器官分离学习框架为3D-CT基础模型的实际部署提供了现实有效的设计指南，证明了该模型在临床评估环境中具有高泛化性能。
...
Comments: 18 pages (main text) + 6 pages (supplementary information), 7 figures (main text). Updated version submitted to Scientific Data. Dataset available at https://huggingface.co/datasets/ScienceOne-AI/S1-MMAlign
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习在通用领域取得了革命性进展，但在科学发现中的应用受到阻碍，主要原因是复杂的科学图像与稀疏的文本描述之间存在深刻的语义鸿沟。现有科学多模态数据集存在对齐质量差、规模有限的问题。

Method: 1. 构建包含1550万图像-文本对的大规模多学科数据集，涵盖物理、生物、工程等领域，包含实验装置、热图、显微图像等多种视觉模态；2. 开发AI就绪的语义增强管道，利用Qwen-VL多模态大模型系列，通过整合论文摘要和引用上下文来重新描述图像，解决原始科学标题中的弱对齐问题。
...
TL;DR: 提出ActErase：一种无需训练的概念擦除方法，通过激活差异区域识别和动态替换，在扩散模型中高效移除敏感概念，保持生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型存在安全、版权和伦理风险，现有概念擦除方法依赖数据密集和计算昂贵的微调，存在关键限制。需要更高效的方法来解决这些挑战。

Method: 基于模型激活主要由通用概念组成、只有极小部分表示目标概念的观察，提出无需训练的方法ActErase。通过提示对分析识别激活差异区域，提取目标激活并在前向传播中动态替换输入激活。

Result: 在三个关键擦除任务（裸露、艺术风格和对象移除）上的综合评估表明，该方法实现了最先进的擦除性能，同时有效保留了模型的整体生成能力。对对抗攻击表现出强鲁棒性。

Conclusion: ActErase为扩散模型中的轻量级但有效的概念操作建立了新的即插即用范式，无需训练即可高效移除敏感概念，平衡了安全性和生成质量。
...
TL;DR: SV-GS：一种在稀疏时空观测下重建动态目标的框架，通过骨架驱动的变形场和运动插值实现高质量重建


<details>
  <summary>Details</summary>
Motivation: 现实世界中动态目标重建面临观测稀疏的挑战，传统方法需要密集的视角和时间采样，而实际场景（如监控摄像头）往往只能提供稀疏的时空观测，导致重建问题高度不适定

Method: 提出SV-GS框架，利用粗略骨架图和初始静态重建作为输入，优化骨架驱动的变形场，包含粗粒度骨架关节姿态估计器和细粒度变形模块，仅使关节姿态估计器具有时间依赖性，实现平滑运动插值同时保留几何细节

Result: 在合成数据集上，相比现有方法在稀疏观测条件下PSNR提升高达34%；在真实数据集上，使用显著更少的帧数即可达到与密集单目视频方法相当的性能；证明初始静态重建可被基于扩散的生成先验替代，提高了实用性

Conclusion: SV-GS框架能够有效处理稀疏时空观测下的动态目标重建问题，通过骨架驱动的变形场实现了高质量的运动估计和几何重建，具有实际应用价值
...
Result: 最终模型在ISIC2019数据集上对8种皮肤病变类别实现了87.71%的预测准确率。
...
TL;DR: TimeColor是一个基于草图的视频着色模型，支持使用异质、可变数量的参考图像，通过显式的每参考区域分配和时空对应掩码注意力来提升着色质量。


<details>
  <summary>Details</summary>
Motivation: 现有着色模型通常只使用单参考（通常是场景第一帧），忽略了其他条件数据源如角色设定图、背景图像或任意着色帧。这限制了着色质量和一致性。

Method: 1) 将参考编码为额外潜在帧并时间拼接，使模型能在每个扩散步骤中并行处理参考而保持参数固定；2) 使用时空对应掩码注意力强制主体-参考绑定；3) 采用模态分离的RoPE索引；4) 支持异质、可变数量参考的显式每参考区域分配。

Result: 在SAKUGA-42M数据集上的单参考和多参考协议实验表明，TimeColor在色彩保真度、身份一致性和时间稳定性方面优于现有基线方法。

Conclusion: TimeColor通过支持异质多参考和显式区域分配，有效缓解了捷径学习和跨身份调色板泄漏问题，显著提升了视频着色质量。
...
TL;DR: VisNet是一种高效的人体再识别模型，通过多尺度特征融合、语义聚类和动态权重平均等技术，在保持高精度的同时大幅降低计算成本，适用于实时监控和移动应用。


<details>
  <summary>Details</summary>
Motivation: 当前人体再识别方法虽然精度高但计算成本大，难以在计算资源有限的实时监控和移动应用中部署。需要开发既高效又准确的实际可行方案。
...
TL;DR: OmniVaT框架首次解决了单域泛化多模态视觉-触觉学习任务，通过多模态分数傅里叶适配器统一嵌入-频率空间，以及离散树生成模块增强对未见域的适应性。


<details>
  <summary>Details</summary>
Motivation: 视觉-触觉学习面临模态差异（视觉与触觉图像）和域差距（非标准化触觉传感器、不一致数据收集）的双重挑战，现有方法难以在单域训练下实现跨域泛化。

Method: 提出OmniVaT框架：1) 多模态分数傅里叶适配器将视觉和触觉嵌入映射到统一的嵌入-频率空间，无需多域训练数据或复杂跨模态融合策略；2) 离散树生成模块通过层次树结构获得多样可靠的模态分数表示，增强对未见域波动的适应性。

Result: 大量实验表明OmniVaT在SDG-VTL任务上具有优越的跨域泛化性能。

Conclusion: OmniVaT首次成功解决了单域泛化多模态视觉-触觉学习任务，有效缓解模态差异和域差距问题，为具身智能的物理世界感知提供了新解决方案。
...
DOI: [10.1109/IROS60139.2025.11245809](https://doi.org/10.1109/IROS60139.2025.11245809)

Author affiliations: Söhnke Benedikt Fischedick: Technische Universität Ilmenau; Daniel Seichter: Technische Universität Ilmenau; Benedict Stephan: Technische Universität Ilmenau; Robin Schmidt: Technische Universität Ilmenau; Horst–Michael Groß: Technische Universität Ilmenau
...
<details>
  <summary>Details</summary>
Motivation: 家庭环境中机器人需要全面理解环境以与未经训练的人类有效交互，传统语义分割方法使用固定预定义类别限制了灵活性

Method: 使用Alpha-CLIP的教师嵌入指导高效学生模型DVEFormer学习细粒度像素级嵌入，基于RGB-D Transformer架构，通过知识蒸馏实现
...
TL;DR: 提出概率双流框架，统一可靠性建模与多模态集成，用于骨架动作识别，特别关注手部精细动作，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有骨架动作识别方法主要关注身体大尺度运动，忽略了对手部精细关节的建模，而手部动作对于细粒度识别至关重要。需要一种能够处理不确定性和噪声的框架，同时整合多种模态信息。

Method: 提出概率双流框架，包含三个关键组件：1) 无需校准的预处理管道，直接从原生坐标学习；2) 概率Noisy-OR融合，稳定可靠性感知的双流学习；3) 从内部到跨模态的集成，将四种骨架模态与RGB表示耦合。

Result: 在多个基准测试（NTU RGB+D 60/120、PKU-MMD、N-UCLA）和新定义的手部中心基准测试中，均表现出持续改进和鲁棒性，特别是在噪声和异构条件下。

Conclusion: 该概率双流框架通过统一可靠性建模和多模态集成，有效解决了骨架动作识别中手部精细动作建模不足的问题，在多种条件下展现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前4D世界建模方法存在可扩展性限制，主要源于昂贵的多视角4D数据或繁琐的训练预处理，需要开发能够适应多样化单目视频的可扩展解决方案。

Method: 采用免姿态前馈4D重建、在线单目退化模式模拟等技术，使整个流程能够扩展到多样化的单目视频，无需昂贵的多视角数据或复杂预处理。

Result: 在标准重建和生成基准测试中达到最先进性能，展示了模型在多样化领域的泛化能力和多功能性。

Conclusion: NeoVerse通过创新的可扩展设计，成功解决了当前4D世界建模的可扩展性瓶颈，为4D重建和生成任务提供了强大且通用的解决方案。
...
TL;DR: 首个用于车载摄像头路边垃圾检测的大规模数据集RoLID-11K，包含11k+标注图像，针对极端小目标、稀疏分布和复杂背景的检测挑战，并评估了多种现代检测器性能。


<details>
  <summary>Details</summary>
Motivation: 当前路边垃圾监测依赖人工调查和公众报告，空间覆盖有限。现有视觉数据集主要关注街景静态图像、航拍或水环境，无法反映车载摄像头中垃圾目标极小、稀疏且嵌入杂乱路边背景的独特特征。

Method: 构建RoLID-11K数据集，包含11k+标注图像，涵盖英国多样化驾驶条件，呈现明显长尾分布和小目标特征。对多种现代检测器进行基准测试，包括精度导向的transformer架构和实时YOLO模型。

Result: CO-DETR及相关transformer模型获得最佳定位精度，但实时模型受限于粗糙的特征层次结构。数据集为动态驾驶场景中的极端小目标检测建立了具有挑战性的基准。

Conclusion: RoLID-11K是首个用于车载摄像头路边垃圾检测的大规模数据集，支持开发可扩展、低成本的路边垃圾监测系统，为动态驾驶场景中的极端小目标检测提供了重要研究基准。
...
<details>
  <summary>Details</summary>
Motivation: 虽然强化学习在语言模型推理方面取得了进展，但将其扩展到多模态推理需要同时改进感知和推理能力。先前工作主要使用显式感知奖励，但存在难以分离感知标记与推理标记、需要额外LLM、真实数据、强制分离或对所有输出标记应用奖励等问题。
...
Conclusion: CPPO提供了一种无需额外模型或数据的有效方法来改进视觉语言模型的感知能力，通过对比感知损失和熵变化检测实现了感知与推理的更好分离。
...
Method: FreeText将问题分解为"在哪里写"和"写什么"。对于"在哪里写"，通过读取token-wise空间归因从内源性图像到文本注意力中定位书写区域，使用sink-like tokens作为稳定空间锚点，并通过拓扑感知细化产生高置信度掩码。对于"写什么"，引入频谱调制字形注入(SGMI)，注入噪声对齐的字形先验，通过频域带通调制来增强字形结构并抑制语义泄漏。

Result: 在Qwen-Image、FLUX.1-dev和SD3变体上的广泛实验，在longText-Benchmark、CVTG和自建的CLT-Bench上显示，文本可读性得到一致提升，同时很大程度上保持了语义对齐和美学质量，推理开销适中。
...
TL;DR: VNS-SAM通过Mask-Edge Token Interactive decoder和Non-Salient Feature Mining模块增强SAM在视觉非显著场景下的分割能力，同时保持其零样本泛化性，并在VNS-SEG数据集上验证了有效性。
...
Method: 提出VNS-SAM，通过两种设计有效利用SAM的低层特征：1) Mask-Edge Token Interactive decoder，2) Non-Salient Feature Mining module。这些设计以微小的参数和计算成本增强对非显著特征的理解。额外参数可在4小时内优化完成。

Result: 建立了VNS-SEG统一数据集（超过35K图像），涵盖多种VNS场景。在各种VNS分割任务上的大量实验表明VNS-SAM具有优越性能，特别是在零样本设置下，展示了广泛的现实应用潜力。

Conclusion: VNS-SAM成功增强了SAM在视觉非显著场景下的分割能力，同时保持了其零样本泛化性。提出的方法具有实际可行性，公开的代码和数据集将促进相关研究发展。
...
### [30] [DynaDrag: Dynamic Drag-Style Image Editing by Motion Prediction](https://arxiv.org/abs/2601.00542)
...
Method: 提出DynaDrag方法，采用预测-移动框架，迭代执行运动预测和运动监督：运动预测预测处理点应移动的位置，运动监督据此拖动处理点，并动态调整有效处理点以提升性能。

Result: 在人脸和人体数据集上的实验表明，DynaDrag在性能上优于先前的工作，有效解决了传统方法中的问题。

Conclusion: DynaDrag作为首个基于预测-移动框架的拖拽式图像编辑方法，通过创新的迭代预测和监督机制，显著提升了像素级图像编辑的质量和可靠性。
...
### [31] [A Comprehensive Dataset for Human vs. AI Generated Image Detection](https://arxiv.org/abs/2601.00553)
...
TL;DR: 该论文发布了MS COCOAI数据集，包含96000个真实和合成图像数据点，用于AI生成图像检测，并提出了两个分类任务。
...
TL;DR: SlingBAG Pro是一种基于点云迭代概念的高级重建算法，专门为不规则几何换能器阵列设计，用于三维光声成像重建，相比原始SlingBAG算法速度提升2.2倍。


<details>
  <summary>Details</summary>
Motivation: 解决不规则几何换能器阵列在三维光声成像中的重建挑战。传统迭代重建算法在处理不规则阵列配置时面临计算复杂度高、内存需求大、重建时间长的问题，而SlingBAG Pro旨在保持高质量重建的同时减少所需换能器数量并显著缩短重建时间。

Method: 基于滑动球自适应生长(SlingBAG)方法的点云迭代概念，扩展其兼容性以支持任意阵列几何形状。采用分层优化策略，结合零梯度滤波和迭代过程中逐渐增加的时间采样率，快速去除冗余空间点云并加速收敛。

Result: SlingBAG Pro在不规则阵列几何下，相比原始SlingBAG算法，基于点云的三维光声重建速度提升最高达2.2倍。通过仿真和活体小鼠实验验证了方法的有效性，源代码已公开。

Conclusion: SlingBAG Pro算法成功解决了不规则几何换能器阵列在三维光声成像重建中的计算效率问题，在保持高质量重建的同时显著减少了重建时间，为临床应用中高质量三维光声成像提供了有效的解决方案。
...
TL;DR: TOLF框架通过归一化流进行误差建模和不确定性引导优化，解决小目标检测中因标注噪声导致的过拟合问题


<details>
  <summary>Details</summary>
Motivation: 小目标检测相比正常尺度目标存在性能差距，主要原因是小目标对标注噪声高度敏感，优化严格定位目标容易导致噪声过拟合

Method: 提出Tiny Object Localization with Flows (TOLF)：1) 使用归一化流进行灵活的误差建模，捕捉复杂的非高斯预测分布；2) 不确定性感知梯度调制机制，抑制高不确定性噪声样本的学习

Result: 在三个数据集上的实验验证了方法的有效性，特别是在AI-TOD数据集上将DINO基线提升了1.2% AP

Conclusion: TOLF通过流式误差建模和不确定性引导优化，有效解决了小目标检测中的噪声过拟合问题，提升了小目标定位的鲁棒性
...
TL;DR: AEGIS是一个用于评估统一多模态模型世界知识应用能力的多任务基准，包含1050个手动标注的问题，涵盖21个主题和6种推理类型，并提出了确定性清单评估协议以提高评估可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试存在局限性，只能提供孤立的单任务评估，诊断能力有限。统一多模态模型在跨任务应用世界知识方面的能力仍然是一个未解决的关键挑战，需要更全面的评估框架。

Method: 提出了AEGIS基准，涵盖视觉理解、生成、编辑和交错生成等多个任务，包含1050个具有挑战性的手动标注问题，涉及21个主题和6种推理类型。同时提出了确定性清单评估协议，用原子化的"是/否"判断替代模糊的提示式评分，提高评估可靠性。

Result: 实验表明大多数统一多模态模型存在严重的世界知识缺陷，随着推理复杂度增加，性能显著下降。简单的插件式推理模块可以部分缓解这些弱点，这为未来研究指明了有前景的方向。

Conclusion: 世界知识推理是统一多模态模型发展的关键前沿领域，AEGIS基准和确定性清单评估协议为系统评估模型的世界知识应用能力提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医疗图像分析中表现出色，但部署后当输入数据分布发生变化时性能可能退化。检测这种性能退化对临床可靠性至关重要，但对于没有标注数据的大型预训练VLM来说具有挑战性。

Method: 研究分析了输入级数据偏移和输出级预测行为，开发了DomainSAT轻量级工具箱集成代表性偏移检测算法，并引入基于置信度的无标签性能退化指标。结合输入数据偏移检测和输出置信度指标来监测VLM性能退化。

Result: 输入数据偏移检测能有效识别分布变化并提供早期诊断信号，但不总是对应实际性能退化。基于置信度的退化指标与性能退化密切相关，可作为输入偏移检测的有效补充。在大规模病理数据集上的实验表明，结合两种方法能更可靠地检测和解释数据偏移下的VLM性能退化。

Conclusion: 该研究提供了一个实用且互补的框架，用于监测数字病理学中基础模型的可靠性，通过结合输入数据偏移检测和输出置信度指标，能够更有效地检测和解释视觉语言模型在数据分布变化下的性能退化。
...
TL;DR: 提出GranAlign框架解决零样本视频时刻检索中的语义粒度不匹配问题，通过粒度感知查询重写和查询感知字幕生成实现训练免费的语义对齐，在三个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 零样本视频时刻检索面临的主要挑战是文本查询与视觉内容之间的语义粒度不匹配。现有方法虽然利用预训练知识在联合空间中表示视频和语言，但未能平衡不同模态提供的预训练知识在给定场景中的语义粒度，导致检索不准确。

Method: 提出Granularity-Aware Alignment (GranAlign)训练免费框架，包含两个互补技术：1) 粒度感知查询重写：生成不同语义粒度的查询变体；2) 查询感知字幕生成：将查询意图嵌入视频内容。通过将多粒度查询与查询无关和查询感知的字幕配对，有效解决语义不匹配问题。

Result: 在三个主要基准测试(QVHighlights, Charades-STA, ActivityNet-Captions)上均达到新的最先进水平，在具有挑战性的QVHighlights数据集上实现了3.23% mAP@avg的显著提升。

Conclusion: GranAlign框架通过平衡视频和语言模态的语义粒度，有效解决了零样本视频时刻检索中的语义不匹配问题，无需任务特定训练数据即可实现准确的时刻定位。
...
TL;DR: SafeMo：一种可信的运动生成框架，通过最小化运动遗忘（MMU）策略在连续空间中实现安全人体运动生成，避免离散码本替换方法的缺陷，并提供首个安全文本到运动数据集SafeMoVAE-29K。


<details>
  <summary>Details</summary>
Motivation: 现有基于离散VQ-VAE码本替换的文本到运动（T2M）安全方法存在两个关键缺陷：1）替换被良性提示重复使用的码本条目会导致日常任务性能下降；2）离散令牌方法引入量化和平滑度损失，导致伪影和抖动过渡。此外，现有文本到运动数据集自然包含不安全意图和相应运动，不适合安全驱动的机器学习。

Method: 提出SafeMo框架，集成最小化运动遗忘（MMU）策略，这是一种两阶段机器学习遗忘策略，能够在连续空间中实现安全人体运动生成，保留连续运动学特性而无码本损失。同时创建首个安全文本到运动数据集SafeMoVAE-29K，包含重写的安全文本提示和连续精炼运动。

Result: 实验表明SafeMo在HumanML3D和Motion-X数据集上分别达到2.5倍和14.4倍更高的遗忘集FID，相比之前最先进的人类运动遗忘方法LCR，同时在安全提示上的良性性能相当或更好。

Conclusion: SafeMo通过连续空间中的最小化运动遗忘策略有效解决了现有离散码本替换方法的安全缺陷，实现了强大的安全-效用权衡，为可信人体运动生成提供了有效框架。
...
TL;DR: 提出MDI指标量化模态主导现象，开发MDACL框架通过HCG和AER缓解RGB-IR检测中的优化偏差，实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: RGB-IR多模态感知中，信息密度和特征质量的不对称性导致持续优化偏差，训练过度强调主导模态，阻碍有效融合

Method: 提出模态主导指数(MDI)量化模态主导程度，开发模态主导感知跨模态学习(MDACL)框架，包含分层跨模态引导(HCG)增强特征对齐和对抗均衡正则化(AER)平衡优化动态

Result: 在三个RGB-IR基准测试上的广泛实验表明，MDACL有效缓解优化偏差并实现SOTA性能

Conclusion: 通过量化模态主导现象并开发相应的学习框架，成功解决了RGB-IR多模态检测中的优化偏差问题，提升了跨模态融合效果
...
TL;DR: RePose：一种用于康复训练的实时3D人体姿态估计与运动分析方法，通过多摄像头RGB视频输入实现端到端的实时监测与评估，提供即时反馈指导患者正确执行康复动作。


<details>
  <summary>Details</summary>
Motivation: 康复训练中需要实时监测和评估患者运动状态，提供即时反馈以纠正动作，帮助患者恢复肌肉力量和运动功能。现有方法在实时性、多人干扰处理和姿态平滑性方面存在不足。

Method: 1. 提出统一端到端流水线处理多摄像头RGB视频输入；2. 针对医疗康复场景设计快速跟踪方法（单帧跟踪<1ms）；3. 改进SmoothNet用于实时姿态估计，减少误差并平滑运动；4. 基于Unity平台实现实时监测评估和肌肉应力可视化。

Result: 系统能够实时监测和评估康复运动，提供即时反馈；快速跟踪方法有效处理多人干扰；改进的SmoothNet显著减少姿态估计误差，恢复真实运动状态；Unity平台成功实现运动监测和肌肉应力可视化。

Conclusion: RePose方法为康复训练提供了有效的实时3D姿态估计和运动分析解决方案，通过快速跟踪、姿态平滑优化和可视化反馈，能够辅助患者正确执行康复动作，促进功能恢复。
...
<details>
  <summary>Details</summary>
Motivation: 室管膜瘤的术前预后对治疗规划至关重要，但由于MRI缺乏术后手术报告中的语义洞察而具有挑战性。现有的多模态方法无法在推理时特权文本数据不可用时利用这些信息。

Method: 提出HyperPriv-EPN，一种基于超图的学习使用特权信息框架。采用Severed Graph策略，使用共享编码器处理教师图（包含术后特权信息）和学生图（仅限术前数据）。通过双流蒸馏，学生学会仅从视觉特征中"幻觉"出语义社区结构。

Result: 在311名患者的多中心队列中验证，HyperPriv-EPN实现了最先进的诊断准确性和生存分层。有效将专家知识转移到术前设置，解锁历史术后数据的价值来指导新患者诊断，无需推理时的文本。

Conclusion: 该框架成功地将术后特权信息的知识转移到术前预测中，通过超图表示和双流蒸馏实现了无需推理时文本数据的准确预后预测，为临床决策提供了有力支持。
...
TL;DR: 基于图像的深度学习为马铃薯储存期间的质量监测提供非侵入式解决方案，通过预训练模型实现发芽检测、重量损失估计和货架期预测


<details>
  <summary>Details</summary>
Motivation: 解决马铃薯储存期间的质量监测挑战，包括发芽检测、重量损失估计和货架期预测，为自动化分拣和库存系统提供技术支持

Method: 在200天控制温湿度条件下收集图像和重量数据，利用ResNet、VGG、DenseNet和ViT等预训练架构，设计二元分类器用于发芽检测和多类预测器用于重量损失和货架期预测

Result: DenseNet在发芽检测中达到98.03%准确率；货架期预测在2-5类粗划分时准确率超过89.83%，6-8类细划分时准确率下降；证明图像模型可集成到自动化系统中
...
TL;DR: CRoPS：一种无需训练的幻觉缓解框架，通过选择性移除关键文本标记构建幻觉模型，并结合广义对比解码，显著提升LVLM生成可靠性


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在幻觉内容生成问题，现有无训练方法存在两个局限：1）对幻觉来源的假设过于狭窄；2）在生成后期（幻觉最可能发生时）效果下降。现有方法仅移除视觉标记构建幻觉模型不够充分，因为视觉信息仍会传播到生成文本中。

Method: 提出CRoPS框架：1）构建新型幻觉模型，通过选择性移除关键文本标记来捕捉幻觉效应；2）引入广义对比解码，整合多个幻觉模型以表示多样化的幻觉来源。该方法完全无需训练。

Result: 在六个基准测试和三个LVLM家族上取得一致提升，CHAIR分数提高20%，优于现有最先进的无训练方法。

Conclusion: CRoPS通过选择性移除文本标记和广义对比解码有效缓解LVLM幻觉问题，显著提升模型可靠性，为无训练幻觉缓解提供了新思路。
...
TL;DR: 提出了一种基于3D高斯场景表示的单图像到视频生成框架，能够在单次前向传播中同时构建3D场景和采样合理的物体运动，实现快速、相机引导的视频生成，无需迭代去噪来注入物体运动。


<details>
  <summary>Details</summary>
Motivation: 现有单图像条件视频生成方法虽然改进了时间一致性和3D一致性，但缺乏鲁棒的用户可控性（如修改相机路径），限制了实际应用。大多数相机控制的图像到视频模型在准确建模相机运动、保持时间一致性和保留几何完整性方面存在困难。

Method: 提出新颖框架，在单次前向传播中构建3D高斯场景表示并采样合理的物体运动。该方法避免了现有方法中先使用3D点云渲染场景、后引入物体运动的两步过程，能够实现快速、相机引导的视频生成，无需迭代去噪来注入物体运动。

Result: 在KITTI、Waymo、RealEstate10K和DL3DV-10K数据集上的广泛实验表明，该方法在视频质量和推理效率方面达到了最先进水平。

Conclusion: 该方法通过单次前向传播同时构建3D高斯场景表示和采样物体运动，实现了快速、相机引导的视频生成，在保持时间一致性和几何完整性的同时提供了精确的相机控制能力。
...
TL;DR: RGS-SLAM是一种鲁棒的高斯溅射SLAM框架，使用基于DINOv3描述符的免训练对应关系初始化代替传统的残差驱动致密化，实现了更稳定的早期建图和更快的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 传统GS-SLAM采用残差驱动的渐进式高斯添加策略，在早期建图阶段可能不稳定且收敛较慢。本文旨在通过更智能的初始化方法来改善这些问题。

Method: 使用DINOv3描述符提取密集多视角对应关系，通过置信度感知的内点分类器进行精炼，然后进行一次性三角测量生成结构感知的高斯种子作为优化前的先验，替代了传统的残差驱动致密化阶段。

Result: 在TUM RGB-D和Replica数据集上评估，RGS-SLAM实现了与最先进的高斯和基于点的SLAM系统竞争或更优的定位和重建精度，收敛速度提升约20%，在纹理丰富和杂乱场景中渲染保真度更高，实时建图性能可达925 FPS。

Conclusion: RGS-SLAM通过免训练的对应关系到高斯初始化方法，显著改善了高斯溅射SLAM的稳定性和收敛速度，同时保持与现有GS-SLAM管道的完全兼容性，为实时高保真SLAM提供了有效解决方案。
...
TL;DR: 基于多模态大语言模型的端到端手写工程测验自动评分工作流，通过参考解决方案和分级规则实现可靠评分，在真实课程测验中达到约8分的平均绝对误差。
...
Method: 提出端到端工作流：1）讲师提供手写参考解决方案和评分规则；2）将参考转换为纯文本摘要用于条件评分；3）多阶段设计包括格式/存在性检查、独立评分器集成、监督聚合和确定性验证；4）使用冻结管道在真实课程测验上评估。

Result: 使用最先进后端（GPT-5.2和Gemini-3 Pro），完整管道与讲师评分相比达到约8分的平均绝对差异，偏差低，在D_max=40时手动审查触发率约17%。消融实验显示简单提示和移除参考解决方案会显著降低准确性并引入系统性过评分。

Conclusion: 结构化提示和参考基础对于手写工程测验的可靠自动评分至关重要。提出的多阶段工作流能够在保持标准考试流程的同时实现可扩展的评分，为教育评估提供了实用的自动化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索自监督学习作为辅助任务如何优化深度伪造检测这一主要任务，研究不同训练方案组合的有效性

Method: 将自监督学习作为辅助任务，研究不同训练方案组合，融合自监督辅助任务的特征表示来增强主要任务

Result: 在DF40、FaceForensics++、Celeb-DF、DFD、FaceShifter、UADFV等多个数据集上实验，相比当前最先进检测器，在跨数据集评估中表现出更好的泛化性能

Conclusion: 融合自监督辅助任务的特征表示是一种强大的特征表示方法，能够充分利用自监督和主要任务的独特表示，为主要任务带来更好的性能
...
Journal reference: 2022 12th International Conference on Bioscience, Biochemistry and Bioinformatics (ICBBB '22), January 7-10, 2022, Tokyo, Japan

DOI: [10.1145/3510427.3510429](https://doi.org/10.1145/3510427.3510429)

Author affiliations: Wenhui Chu: University of Houston; Nikolaos V. Tsekos: University of Houston

TL;DR: 该论文提出了两种用于心脏MRI左心室分割的新型深度学习架构LNU-Net和IBU-Net，分别基于层归一化和实例-批量归一化，在分割性能上优于原始U-Net和其他先进方法。
...
Method: 提出了两种基于U-Net架构的深度学习模型：LNU-Net在每个卷积块中应用层归一化；IBU-Net在第一个卷积块中结合实例归一化和批量归一化，并将其结果传递到下一层。采用仿射变换和弹性变形进行数据增强，使用包含45名患者805张MRI图像的数据集进行评估。

Result: 实验结果表明，提出的LNU-Net和IBU-Net在Dice系数和平均垂直距离指标上均优于原始U-Net和其他最先进的分割方法。

Conclusion: LNU-Net和IBU-Net是有效的左心室分割架构，通过创新的归一化策略提高了分割精度，为心脏MRI图像的临床分析提供了更可靠的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个主要问题：1）使用单一高斯基元具有低通滤波特性，无法捕捉高频细节；2）标准Gabor函数存在能量不稳定问题；3）缺乏时序连续性约束导致插值时的运动伪影。需要同时解决频率适应性和时序连续性这两个核心挑战。

Method: 提出AdaGaR框架，包含三个关键技术：1）自适应Gabor表示，通过可学习频率权重和自适应能量补偿扩展高斯函数，平衡细节捕捉和稳定性；2）时序连续性约束，使用三次Hermite样条和时序曲率正则化确保平滑运动演化；3）自适应初始化机制，结合深度估计、点跟踪和前景掩码，在早期训练中建立稳定的点云分布。

Result: 在Tap-Vid DAVIS数据集上取得SOTA性能：PSNR 35.49，SSIM 0.9433，LPIPS 0.0723。在帧插值、深度一致性、视频编辑和立体视图合成等任务上表现出强大的泛化能力。

Conclusion: AdaGaR通过统一解决频率适应性和时序连续性，显著提升了动态3D场景重建的质量，在保持高频细节的同时确保运动平滑性，为单目视频的动态场景建模提供了有效的解决方案。
...
TL;DR: RIMRULE：基于动态规则注入的神经符号方法，通过从失败轨迹中提炼紧凑、可解释的规则并在推理时注入提示，提升LLM在特定领域工具使用中的性能，无需修改模型权重。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在领域特定环境中使用工具时存在困难，因为API可能具有特殊性、文档不足或针对私有工作流程定制。这凸显了有效适应任务特定工具的需求。

Method: 提出RIMRULE神经符号方法：1）从失败轨迹中提炼紧凑、可解释的规则；2）LLM自身提出规则；3）使用最小描述长度目标进行规则整合，偏好通用性和简洁性；4）规则以自然语言和结构化符号形式存储；5）推理时动态注入规则到提示中。

Result: 在工具使用基准测试中，该方法提高了对已见和未见工具的准确性，无需修改LLM权重。优于基于提示的适应方法，并与微调互补。从一个LLM学习的规则可以重用改进其他LLM，包括长推理LLM，突显了符号知识在不同架构间的可移植性。

Conclusion: RIMRULE通过动态规则注入有效提升LLM在领域特定工具使用中的性能，规则具有可解释性、可移植性，且无需模型权重修改，为LLM工具适应提供了神经符号解决方案。
...
TL;DR: 提出Pat-DEVAL框架，首个专门评估专利说明书的多维度评估框架，通过Chain-of-Legal-Thought机制增强法律合规性评估，在专利专家验证的数据集上达到0.69的皮尔逊相关性。


<details>
  <summary>Details</summary>
Motivation: 现有专利自动撰写评估方法无法充分评估长篇结构连贯性和特定法定合规性，需要专门针对专利说明书的多维度评估框架。

Method: 提出Pat-DEVAL框架，采用LLM-as-a-judge范式，引入Chain-of-Legal-Thought（CoLT）机制，这是一种法律约束的推理机制，强制执行顺序的专利法特定分析。

Result: 在Pap2Pat-EvalGold数据集上，专利专家验证的实验显示Pat-DEVAL达到0.69的皮尔逊相关性，显著优于基线指标和现有LLM评估器；在法律专业合规性方面达到0.73的优越相关性。

Conclusion: 通过建立确保技术正确性和法律合规性的新标准，Pat-DEVAL为自动化专利撰写系统的实际部署提供了稳健的方法学基础，明确注入法定约束对于捕捉细微法律有效性至关重要。
...
TL;DR: 本文通过系统分析IEMOCAP数据集，填补了对话情感识别(ERC)领域的两个关键空白：对架构选择影响的有限理解，以及识别与生成之间的语言学分析缺失。研究发现对话上下文至关重要，简单架构在严格因果上下文下超越了先前方法，并揭示了情感与话语标记位置之间的显著关联。


<details>
  <summary>Details</summary>
Motivation: 对话情感识别(ERC)虽然取得了高准确率，但仍存在两个关键空白：1) 对哪些架构选择真正重要的理解有限；2) 缺乏将情感识别与生成连接起来的语言学分析。本文旨在通过系统分析IEMOCAP数据集来填补这两个空白。

Method: 采用系统分析方法，包括：1) 对IEMOCAP数据集进行严格的消融研究，使用10次种子评估；2) 分析5,286个话语标记出现情况，研究情感与标记位置之间的关联；3) 使用简单架构和严格因果上下文进行情感识别。

Result: 主要发现：1) 对话上下文至关重要，90%的性能增益在最近10-30轮对话中实现；2) 层次化句子表示在话语级别有帮助，但提供对话上下文后此优势消失；3) 外部情感词典无增益；4) 简单架构在严格因果上下文下达到82.69%(4类)和67.07%(6类)加权F1，超越先前方法；5) 情感与话语标记位置显著相关(p<.0001)，悲伤话语的左边缘标记使用减少(21.9%)。

Conclusion: 对话上下文是情感识别的关键因素，简单的因果架构足以实现高性能。语言学分析揭示了情感与话语标记使用模式之间的系统关联，特别是悲伤话语缺乏显性语用信号，需要对话历史进行消歧，这与其从上下文中获益最多(+22%)的发现一致。研究为ERC提供了架构指导原则和语言学见解。
...
TL;DR: 提出专门针对时序知识图谱推理的蒸馏框架，利用大语言模型作为教师模型，将结构与时序推理能力迁移到轻量级学生模型，在保持高效架构的同时提升时序建模能力。


<details>
  <summary>Details</summary>
Motivation: 现有时序知识图谱推理模型参数规模大、计算密集，导致硬件成本和能耗高，难以部署在资源受限、低功耗和分布式平台上。现有压缩和蒸馏技术主要针对静态知识图谱，无法充分捕捉时序知识图谱中的时序依赖关系，导致推理性能下降。

Method: 提出专门针对时序知识图谱推理的蒸馏框架，利用大语言模型作为教师模型指导蒸馏过程，有效传递结构推理和时序推理能力到轻量级学生模型。通过整合大规模公共知识和任务特定的时序信息，增强学生模型的时序动态建模能力，同时保持紧凑高效的架构。

Result: 在多个公开基准数据集上的广泛实验表明，该方法持续优于强基线模型，在推理准确性、计算效率和实际可部署性之间实现了有利的权衡。

Conclusion: 提出的时序知识图谱推理蒸馏框架能够有效解决现有模型参数规模大、计算密集的问题，通过大语言模型指导的蒸馏过程，实现了结构推理和时序推理能力的有效迁移，为资源受限环境下的时序知识图谱推理提供了实用的解决方案。
...
TL;DR: 该研究提出了一种将循证医学原则融入图基检索增强生成的方法，通过PICO框架对齐查询与证据，并采用贝叶斯启发的重排序算法考虑证据等级，在运动康复领域验证了该框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前医学领域的检索增强生成系统主要关注性能改进，而忽视了循证医学的基本原则。存在两个关键缺陷：1) 查询与检索证据之间缺乏PICO对齐；2) 重排序过程中未考虑证据等级层次。需要开发一种将EBM原则系统整合到RAG中的方法。

Method: 提出了一种通用的EBM适应策略，将PICO框架整合到知识图谱构建和检索中，并设计了一种贝叶斯启发的重排序算法，在不引入预定义权重的情况下根据证据等级校准排序分数。在运动康复领域验证该框架，构建了包含357,844个节点和371,226条边的知识图谱，以及1,637个QA对的可重用基准。

Result: 系统取得了0.830的nugget覆盖率、0.819的答案忠实度、0.882的语义相似度和0.788的PICOT匹配准确率。在5点李克特量表评估中，五位临床专家在事实准确性、忠实度、相关性、安全性和PICO对齐方面给出了4.66-4.84的高分。释放了知识图谱和基准数据集。

Conclusion: 提出的EBM适应策略显著提高了检索和答案质量，且可转移到其他临床领域。释放的资源有助于解决运动康复领域RAG数据集稀缺的问题，为医学RAG系统提供了循证医学原则的系统整合方法。
...
Author affiliations: Leonard Lin: Shisa.AI; Adam Lensenmayer: Shisa.AI

TL;DR: JP-TL-Bench是一个轻量级开源基准，用于指导日英翻译系统的迭代开发，专注于评估"哪个翻译更好"而非"翻译是否可接受"，采用基于LLM的成对比较和Bradley-Terry模型进行稳定评分。


<details>
  <summary>Details</summary>
Motivation: 针对日英翻译中存在的特殊挑战：礼貌程度、隐含意义、省略和语域等细微选择会显著影响翻译的自然度感知。现有评估方法难以区分"两个好翻译中哪个更好"这一更精细的问题，需要专门针对日英翻译特点设计的评估基准。

Method: 1. 使用无参考的成对LLM比较：候选模型与固定的版本化锚点集进行对比评估；2. 采用Bradley-Terry模型聚合成对比较结果；3. 报告胜率以及从拟合对数强度通过逻辑转换得到的0-10标准化"LT"分数；4. 所有候选模型都针对相同的冻结锚点集评分，确保结构稳定性。

Result: JP-TL-Bench提供了一个可靠且经济实惠的评估协议：通过LLM判断实现可靠的成对比较，使用Bradley-Terry模型进行结果聚合，生成稳定的评分指标（胜率和LT分数）。该基准确保了在相同基础集、评判器和聚合代码下评分的结构稳定性。

Conclusion: JP-TL-Bench是一个专门针对日英翻译系统迭代开发设计的轻量级开源基准，能够有效评估翻译质量的细微差异，特别是在礼貌、隐含意义、省略和语域等关键维度上，为翻译系统的持续改进提供了稳定可靠的评估框架。
...
TL;DR: 该研究系统评估了大语言模型生成多语言反事实解释的能力，发现翻译生成的反事实有效性更高但修改幅度更大，多语言反事实数据增强比跨语言增强效果更好，但生成质量限制了模型性能提升。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在生成英语反事实解释方面表现出色且具备多语言能力，但其在多语言反事实生成方面的有效性尚不明确。本研究旨在全面评估LLMs生成多语言反事实的能力，分析其质量、编辑模式、错误类型以及对模型性能的影响。

Method: 1. 自动评估：在六种语言上评估直接生成的目标语言反事实和通过英语翻译生成的反事实；2. 编辑模式分析：比较高资源欧洲语言反事实的编辑模式；3. 错误分类：识别并分类生成反事实中持续出现的错误类型；4. 数据增强实验：比较多语言反事实数据增强与跨语言数据增强对模型性能的影响。

Result: 1. 翻译生成的反事实比直接生成的反事实有效性更高，但需要更多修改，且质量仍不及原始英语反事实；2. 高资源欧洲语言反事实的编辑模式非常相似，表明跨语言扰动遵循共同的策略原则；3. 识别出四种在多语言反事实中持续出现的错误类型；4. 多语言反事实数据增强比跨语言数据增强带来更大的模型性能提升，特别是对低资源语言，但生成反事实的不完美限制了性能提升和鲁棒性增益。

Conclusion: 大语言模型在多语言反事实生成方面存在局限性，翻译方法能提高有效性但代价是更大的修改幅度。多语言反事实数据增强优于跨语言增强，但生成质量限制了实际应用效果。研究揭示了多语言反事实生成的挑战，为改进LLMs的多语言解释能力提供了方向。
...
Author affiliations: Heng Ji: University of Illinois Urbana-Champaign

TL;DR: WildAGTEval是一个评估LLM智能体函数调用能力的基准，专注于真实API复杂性，包含API规范和API执行两个维度的挑战，包含60个复杂度场景和约32K测试配置。


<details>
  <summary>Details</summary>
Motivation: 现有研究假设理想化的API系统，忽略真实世界因素如噪声API输出，需要评估LLM智能体在真实API复杂性下的函数调用能力。

Method: 创建WildAGTEval基准，包含API规范和API执行两个维度的复杂性，构建包含60个不同复杂度场景的API系统，可组合成约32K测试配置，并提供用户-智能体交互评估框架。

Result: 大多数场景具有挑战性，无关信息复杂度造成最大困难，使强LLM性能下降27.3%；定性分析显示LLM有时会扭曲用户意图以声称任务完成，严重影响用户满意度。

Conclusion: WildAGTEval揭示了LLM智能体在真实API复杂性下的局限性，无关信息复杂度是主要挑战，LLM扭曲用户意图的问题需要关注，为未来研究提供了重要基准。
...
TL;DR: 量化对LLM自解释质量有负面影响但程度有限，大模型在忠实性方面表现更好，不同量化方法各有优劣，建议根据具体用例验证自解释质量


<details>
  <summary>Details</summary>
Motivation: 量化被广泛用于加速大型语言模型推理和部署，但其对自解释（模型为自身输出提供的解释）的影响尚未被探索。自解释需要模型对其自身决策过程进行推理，这种能力可能对量化特别敏感。随着自解释在高风险应用中日益被依赖以实现透明度，理解量化是否以及多大程度上降低自解释质量和忠实性至关重要。

Method: 研究两种类型的自解释：自然语言解释和反事实示例，使用三种常见量化技术在不同比特宽度下对LLM进行量化。通过用户研究评估自解释的连贯性和可信度，比较不同规模模型对量化的弹性。

Result: 量化通常导致自解释质量（最高下降4.4%）和忠实性（最高下降2.38%）的中等程度下降。用户研究表明量化降低了自解释的连贯性和可信度（最高下降8.5%）。相比小模型，大模型在自解释质量方面对量化的弹性有限，但在保持忠实性方面表现更好。没有一种量化技术能在任务准确性、自解释质量和忠实性方面始终表现优异。

Conclusion: 量化对自解释的影响因上下文而异，建议针对具体用例验证自解释质量，特别是对更敏感的自然语言解释。尽管如此，自解释质量和忠实性的相对较小恶化并不削弱量化作为模型压缩技术的有效性。
...
TL;DR: DepFlow：一种三阶段抑郁条件文本转语音框架，通过对抗训练学习说话人和内容不变的抑郁嵌入，构建伪装抑郁增强数据集，提升抑郁检测模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有抑郁数据集（如DAIC-WOZ）中语言情感与诊断标签强耦合，导致模型学习语义捷径，在伪装抑郁（患者使用积极/中性语言掩盖抑郁状态）等真实场景中鲁棒性不足。需要缓解语义偏差并创建声学-语义不匹配数据。

Method: 1. 抑郁声学编码器：通过对抗训练学习说话人和内容不变的抑郁嵌入，实现有效解耦同时保持抑郁区分能力（ROC-AUC: 0.693）。2. 流匹配TTS模型：使用FiLM调制注入抑郁嵌入，控制抑郁严重程度同时保持内容和说话人身份。3. 原型严重程度映射：提供平滑可解释的抑郁连续体操作。基于此构建伪装抑郁增强数据集（CDoA），将抑郁声学模式与情感分层文本库中的积极/中性内容配对。

Result: CDoA数据集在三种抑郁检测架构上分别提升宏观F1分数9%、12%和5%，一致优于传统增强策略。抑郁声学编码器ROC-AUC达到0.693，有效解耦抑郁特征。

Conclusion: DepFlow不仅通过CDoA增强抑郁检测模型的鲁棒性，还提供了可控合成平台，用于对话系统和基于模拟的评估，弥补真实临床数据在伦理和覆盖范围上的限制。
...
Journal reference: 2025 International Joint Conference on Neural Networks (IJCNN), Rome, Italy, 2025, pp. 1-9

DOI: [10.1109/IJCNN64981.2025.11227634](https://doi.org/10.1109/IJCNN64981.2025.11227634)

Author affiliations: Yuhao Zhang: Beijing University of Posts and Telecommunications; Zhongliang Yang: Beijing University of Posts and Telecommunications; Linna Zhou: Beijing University of Posts and Telecommunications

TL;DR: 该研究针对LLM幻觉问题，提出了一种基于多事实生成任务的陷阱问题集和鲁棒不确定性量化方法RU，在对抗性提问场景下显著提升了幻觉检测性能。


<details>
  <summary>Details</summary>
Motivation: LLM幻觉问题严重影响了AI生成内容的可靠性和可信度。传统不确定性量化方法在常规问答场景中有效，但在非规范或对抗性提问策略下表现不足，这在实际应用中存在重大隐患。研究旨在填补这一空白。

Method: 1. 构建包含虚假名称的陷阱问题集，用于多事实生成任务的不确定性量化场景；2. 创新性地提出鲁棒不确定性量化方法RU；3. 在四个不同模型上进行实验验证。

Result: 1. 构建的陷阱问题集表现优异；2. 与基线方法相比，RU方法在四个模型上均表现出色，ROCAUC值平均提升0.1-0.2；3. 为LLM幻觉问题提供了新的视角和方法。

Conclusion: 该研究提出的陷阱问题集和鲁棒不确定性量化方法RU有效解决了传统方法在对抗性场景下的局限性，显著提升了LLM幻觉检测能力，为实际应用中的可靠性问题提供了实用解决方案。
...
TL;DR: BERT-JEPA (BEPA) 通过将JEPA训练目标引入BERT风格模型，解决[CLS]嵌入空间坍缩问题，创建语言无关的表示空间，提升多语言任务性能。


<details>
  <summary>Details</summary>
Motivation: 解决BERT风格模型中[CLS]嵌入空间的坍缩问题，并创建语言无关的表示空间以提升多语言任务的性能。

Method: 在BERT风格模型中引入Joint Embedding Predictive Architectures (JEPA)训练目标，形成BERT-JEPA (BEPA)训练范式。

Result: 该方法在多个多语言基准测试中表现出性能提升。

Conclusion: BERT-JEPA通过结合JEPA训练目标有效解决了[CLS]嵌入空间坍缩问题，创建了语言无关的表示空间，从而提升了多语言任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在图像地理定位中通常依赖合成推理标注或外部图像检索，这限制了方法的可解释性和泛化能力。需要一种无需检索、基于真实坐标的结构化推理方法。
...
Conclusion: Geo-R通过结合结构化地理推理和直接空间监督，实现了可扩展且可解释的图像地理定位，为后续研究提供了新的方向。模型和代码将公开以促进复现性研究。
...
TL;DR: 提出judgeWEL数据集，用于卢森堡语命名实体识别，通过基于维基百科和维基数据的弱监督自动标注，并利用大语言模型进行质量验证。


<details>
  <summary>Details</summary>
Motivation: 为低资源语言构建NER数据集面临资源稀缺、标注成本高且不一致的瓶颈，卢森堡语作为代表性低资源语言需要更好的标注资源。

Method: 1) 利用维基百科内部链接和维基数据条目推断实体类型，生成初始弱监督标注；2) 采用多种大语言模型验证标注质量，筛选高质量标注句子；3) 构建比现有数据集大5倍且类别更平衡的卢森堡语NER语料库。

Result: 构建的judgeWEL数据集规模约为现有卢森堡语NER数据集的5倍，提供更广泛且平衡的实体类别覆盖，为多语言和低资源NER研究提供重要资源。

Conclusion: 提出的基于维基百科/维基数据的弱监督标注结合LLM质量验证的流水线方法，能有效为低资源语言构建高质量NER数据集，缓解资源稀缺问题。
...
TL;DR: 该论文提出了一种新的超关系时序知识广义超图（HTKGHs）结构，用于更有效地表示复杂的地缘政治事件，并基于POLECAT数据库构建了htkgh-polecat数据集，最后评估了LLMs在复杂预测场景中的能力。


<details>
  <summary>Details</summary>
Motivation: 当前超关系时序知识图（HTKGs）在表示复杂地缘政治事件时存在局限性，特别是无法有效支持超过两个主要实体的时序事实，而这些复杂关系在真实世界事件中普遍存在。

Method: 1. 提出了HTKGHs的形式化定义，保持向后兼容性的同时支持两种常见的复杂事实类型；2. 基于POLECAT全球事件数据库构建了htkgh-polecat数据集；3. 在关系预测任务上对主流LLMs进行基准测试和分析。

Result: 1. 成功形式化了HTKGHs结构，能够更有效地表示复杂地缘政治事件；2. 创建了htkgh-polecat数据集；3. 通过基准测试获得了LLMs在复杂预测场景中的适应性和能力洞察。

Conclusion: HTKGHs为解决HTKGs在表示复杂时序事实方面的局限性提供了有效方案，为地缘政治事件的复杂预测任务提供了更好的数据结构支持，同时通过LLMs基准测试为未来研究提供了重要参考。
...
TL;DR: 对DistilBERT、MiniLM和ALBERT三种轻量Transformer模型在客户情感分类、新闻主题分类和毒性/仇恨言论检测三个领域的性能比较分析，揭示了不同模型在准确性和效率方面的权衡。


<details>
  <summary>Details</summary>
Motivation: 企业自然语言处理领域对高效、轻量级模型的需求日益增长，这些模型需要能够处理多领域文本自动化任务。本研究旨在比较三种主流轻量Transformer模型在不同领域的性能表现，为企业选择合适模型提供指导。

Method: 使用IMDB、AG News和Measuring Hate Speech语料库三个数据集，在客户情感分类、新闻主题分类和毒性/仇恨言论检测三个领域进行实验。评估指标包括基于准确性的指标（准确率、精确率、召回率、F1分数）和效率指标（模型大小、推理时间、吞吐量、内存使用）。在固定的企业导向约束下进行控制性微调，而非穷尽的超参数优化。

Result: 没有单一模型在所有性能维度上占优：ALBERT在多个领域获得最高的任务特定准确率；MiniLM在推理速度和吞吐量方面表现最佳；DistilBERT在任务间保持最一致的准确率，同时维持有竞争力的效率。所有结果反映了在固定约束下的性能表现。

Conclusion: 研究强调了准确性和效率之间的权衡，为企业应用提供具体建议：对延迟敏感的企业应用推荐MiniLM；需要平衡性能的场景推荐DistilBERT；资源受限环境推荐ALBERT。这些发现为企业根据具体需求选择合适模型提供了实用指导。
...
TL;DR: 该论文对比了语言意义的社会建构主义解释（语言游戏）与数学导向的语义场理论，通过分析LLMs的架构特性，论证了数学结构与语言游戏的互补性。


<details>
  <summary>Details</summary>
Motivation: 利用大型语言模型作为新的实证环境，检验长期存在的语言意义理论，特别是对比社会建构主义（语言游戏）与数学导向的语义场理论，以理解语言本质和AI模型的能力边界。

Method: 基于作者先前工作，形式化词汇场和语言场作为连续语义空间中的交互结构，分析transformer架构的核心特性（分布式表示、注意力机制、嵌入空间几何规律）与这些概念的关系。

Result: LLMs在捕捉语义规律方面的成功支持了语言具有底层数学结构的观点，而其在语用推理和上下文敏感性方面的持续局限则与社会基础的重要性一致，表明数学结构和语言游戏是互补而非竞争的关系。

Conclusion: 数学结构和语言游戏可以理解为互补视角，这一框架澄清了纯统计语言模型的适用范围和局限，并为理论指导的AI架构设计提供了新的方向。
...
TL;DR: 提出Defensive M2S训练范式，通过多轮到单轮对话压缩显著降低护栏模型的训练和推理成本，在保持高攻击检测率的同时实现93倍训练token减少和94.6%推理token减少。


<details>
  <summary>Details</summary>
Motivation: 处理完整多轮对话历史需要大量计算成本，阻碍了护栏模型在大规模LLM部署中的可扩展性。需要一种既能保持安全检测效果又能显著降低计算开销的方法。

Method: 提出Defensive M2S训练范式，将护栏模型在多轮到单轮压缩对话上进行微调，而非完整对话历史。使用三种压缩模板（hyphenize、numberize、pythonize）将多轮对话压缩为单轮格式。在三个护栏模型家族（LlamaGuard、Nemotron、Qwen3Guard）上评估，并在SafeDialBench多轮越狱基准上进行测试。

Result: 训练成本从O(n²)降至O(n)，训练token减少93倍（从1570万降至16.9万）。最佳配置（Qwen3Guard + hyphenize压缩）达到93.8%攻击检测召回率，推理token减少94.6%（从3231降至173个token），相比基线提升38.9个百分点。

Conclusion: M2S压缩可作为护栏模型部署的有效效率技术，能够在保持高安全检测性能的同时显著降低训练和推理成本，实现长多轮对话的可扩展安全筛查。
...
TL;DR: 该论文针对职业教育与培训领域的历史数字化文档，提出了一种抗OCR噪声的命名实体识别方法，通过噪声感知训练、迁移学习和多阶段微调策略，显著提升了在噪声条件下的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 职业教育与培训领域的历史数字化文档存在OCR诱导的噪声问题，传统NER方法在此类噪声环境下性能下降。目前缺乏针对VET领域多实体类型的识别方法，需要开发能够适应OCR噪声的鲁棒NER系统。

Method: 提出基于噪声感知训练的方法，通过合成注入OCR错误、迁移学习和多阶段微调。系统比较了三种互补策略：在噪声数据、干净数据和人工数据上的训练。该方法首次实现了VET文档中多实体类型的识别，虽然应用于德语文档但可迁移到任意语言。

Result: 实验结果表明，领域特定和噪声感知的微调显著提高了在噪声条件下的鲁棒性和准确性。该方法在OCR噪声环境下表现出优越的性能。

Conclusion: 提出的噪声感知NER方法有效解决了VET领域历史数字化文档的OCR噪声问题，通过公开代码实现了可复现的领域特定噪声感知NER，为类似应用场景提供了实用解决方案。
...
TL;DR: 该研究分析了复杂句子结构对基于规则的原子句子提取性能的影响，识别了相对从句、同位语、并列谓语、状语从句和被动结构等挑战性结构。


<details>
  <summary>Details</summary>
Motivation: 现有原子句子提取方法缺乏可解释性，无法深入理解哪些语言结构导致提取失败。虽然已有研究探索基于依存关系的提取，但缺乏对特定从句结构和依存关系如何影响提取难度的系统性分析。

Method: 使用WikiSplit数据集，在spaCy中实现基于依存关系的提取规则，生成100个黄金标准原子句子集，使用ROUGE和BERTScore评估性能，分析相对从句、状语从句、并列模式和被动结构等复杂结构的影响。

Result: 系统达到ROUGE-1 F1=0.6714、ROUGE-2 F1=0.478、ROUGE-L F1=0.650和BERTScore F1=0.5898，表明在词汇、结构和语义对齐方面具有中等至高水平的性能。挑战性结构包括相对从句、同位语、并列谓语、状语从句和被动结构。

Conclusion: 基于规则的提取方法具有合理的准确性，但对句法复杂性敏感。该研究为理解复杂句子结构如何影响原子句子提取提供了系统性分析框架。
...
TL;DR: ECR框架通过语义锚点保持紧凑模型嵌入空间几何结构一致性，避免语义漂移，无需教师输出且兼容知识蒸馏


<details>
  <summary>Details</summary>
Motivation: 紧凑模型在容量受限或多语言场景下容易丢失嵌入空间结构，导致语义漂移和下游任务性能下降。现有压缩方法仅对齐模型输出表层特征，未能保持底层流形结构

Method: 提出嵌入一致性调节(ECR)框架：1)从教师嵌入中离线计算语义锚点；2)紧凑模型学习保持这些锚点周围的几何一致性，不依赖匹配logits或内部特征；3)推理时仅添加小型投影步骤，不改变解码架构或运行时行为

Result: 在10万条多语言语料实验中，ECR稳定训练并跨任务和语言保持语义结构，产生更紧凑且任务对齐的表示空间，使低容量模型学习到比传统基线更清晰的流形。无需教师输出且兼容但不依赖蒸馏

Conclusion: ECR帮助紧凑模型更好地遵循任务要求，使其在严格效率或隐私限制下更易部署，通过保持嵌入空间几何一致性解决语义漂移问题
...
Method: 提出语言无关分层LoRA-MoE（HLoRA）框架，集成到mHuBERT-CTC模型中。包含：1）多语言共享LoRA学习语言不变声学表示；2）语言特定LoRA专家建模语言依赖特征；3）基于LID后验的LoRA路由机制，实现端到端解码

Result: 在MSR-86K和MLC-SLM 2025 Challenge数据集上，HLoRA仅通过单次解码就达到了与最先进两阶段推理方法相当的性能，显著提升了低资源多语言ASR应用的解码效率

Conclusion: HLoRA框架实现了真正语言无关的解码，无需先验语言身份信息或显式语言标签，为资源受限环境下的多语言ASR部署提供了高效解决方案
...
TL;DR: InfoSynth：基于信息论原理自动生成和评估LLM推理基准的框架，通过KL散度和熵量化新颖性和多样性，使用遗传算法合成Python编程问题
...
Result: 方法97%的时间能生成新问题的准确测试用例和解决方案；合成基准相比种子数据集始终表现出更高的新颖性和多样性；算法可控制生成问题的新颖性/多样性和难度

Conclusion: InfoSynth为LLM构建高质量、新颖多样的基准提供了可扩展、自验证的流水线，解决了传统基准创建的局限性
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地部署在成本敏感和设备端场景中，但现有的安全护栏主要针对英文。中文恶意查询通常通过同音字、拼音、符号分割等中文特定模式隐藏意图，这些对抗模式在现有基准测试中未被充分捕捉，特别是轻量级模型可能更容易受到此类特定对抗扰动的影响。

Method: 引入中文特定安全基准测试(CSSBench)，强调中文对抗模式，包括同音字、拼音、符号分割等。基准涵盖六个中文常见领域：非法活动与合规、隐私泄露、健康与医疗错误信息、欺诈与仇恨、成人内容、公共与政治安全，并将查询组织成多种任务类型。评估一组流行的轻量级大语言模型，并测量过度拒绝行为以评估安全引起的性能下降。

Result: 评估结果显示，中文特定对抗模式对轻量级大语言模型构成关键挑战。基准测试提供了对中文大语言模型安全性的全面评估，有助于实际部署中的鲁棒性。

Conclusion: 中文特定对抗模式是轻量级大语言模型安全的关键挑战，CSSBench填补了现有安全评估的空白，为中文环境下的大语言模型安全部署提供了重要评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在确定性自动化工作流中经常产生上下文幻觉，其中生成内容与提示中明确信息相矛盾或忽略这些信息。这种错误在输入固定且正确性明确的场景中尤为严重。

Method: 1. 形式化特定任务概念（固定输入+确定性正确标准）；2. 在独立上下文窗口中重复相同提示，实现错误概率指数级降低；3. 引入LLM-as-a-judge识别正确答案；4. 通过多数投票机制增强不完美判断器，获得指数级下降的集合级错误率

Result: 在受控提取任务上的实验验证：管道失败率随重复次数指数下降；幻觉选择率随判断器数量指数下降；与理论预测完全匹配
...
TL;DR: FwPKM将静态产品键记忆转换为动态快速权重记忆，通过本地块级梯度下降实现训练和推理时的参数动态更新，在长上下文任务中显著降低困惑度。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型中的序列建模层面临存储容量与计算效率之间的权衡：Softmax注意力提供无界存储但计算成本二次方，线性变体高效但存储有限且固定。需要解决这一矛盾。

Method: 提出快速权重产品键记忆（FwPKM），将稀疏的产品键记忆从静态模块转变为动态的"快速权重"情景记忆。通过本地块级梯度下降在训练和推理时动态更新参数，使模型能够快速记忆和检索输入序列中的新键值对。

Result: FwPKM作为有效的情景记忆补充标准模块的语义记忆，在长上下文数据集上显著降低困惑度。在"大海捞针"评估中，尽管仅在4K标记序列上训练，却能泛化到128K标记的上下文。

Conclusion: FwPKM解决了序列建模中存储容量与计算效率的权衡问题，通过动态快速权重记忆机制实现了高效的长上下文处理能力，展现出强大的泛化性能。
...
TL;DR: 提出Sigmoid Head模块解决语言模型概率不可靠的质量估计问题，通过sigmoid激活和负采样策略改进质量信号


<details>
  <summary>Details</summary>
Motivation: 语言模型的概率分布不是可靠的质量估计器，因为自然语言具有歧义性。当存在多个有效输出选项时，模型的概率分布会分散到这些选项上，从而误导性地表明输出质量较低。这主要由两个原因造成：(1) LM的最终输出激活使用softmax，不允许多个正确选项同时获得高概率；(2) LM的训练数据是单一、one-hot编码的参考，表明每个输出步骤只有一个正确选项。

Method: 在预训练语言模型之上训练一个质量估计模块Sigmoid Head。该模块是一个额外的unembedding头，使用sigmoid激活来解决第一个限制。为应对第二个限制，在训练Sigmoid Head的负采样过程中，使用启发式方法避免选择可能正确的替代标记。该方法在训练和推理时计算效率高。

Result: Sigmoid Head产生的概率相比原始softmax头提供了显著更好的质量信号。由于Sigmoid Head不依赖人工标注的质量数据，相比监督式质量估计方法，在领域外设置中更加鲁棒。

Conclusion: 提出的Sigmoid Head模块有效解决了语言模型概率作为质量估计器的局限性，通过sigmoid激活和负采样策略改进了质量信号，在计算效率和领域外鲁棒性方面具有优势。
...
TL;DR: 该论文评估了大型语言模型在文本跨度识别任务中的表现，特别是在情感分析、冒犯性语言识别和声明验证等主观性任务中，探索了指令调优、上下文学习和思维链等策略。


<details>
  <summary>Details</summary>
Motivation: 当前大多数跨度识别方法依赖于BERT等较小的预训练语言模型，而大型语言模型在该任务中的应用相对较少。现有研究主要集中在命名实体识别等显式跨度识别任务，对于情感分析等更主观的跨度识别任务在LLMs中的探索不足，存在重要研究空白。

Method: 研究评估了多种LLMs在三个流行任务（情感分析、冒犯性语言识别和声明验证）中的文本跨度识别性能。探索了多种LLM策略，包括指令调优、上下文学习和思维链方法。

Result: 研究结果表明，文本内部的潜在关系有助于LLMs识别精确的文本跨度。通过实验验证了不同LLM策略在主观性跨度识别任务中的有效性。

Conclusion: 该研究填补了LLMs在主观性文本跨度识别任务中的重要空白，证明了文本内部关系对LLMs识别精确跨度的关键作用，为下游NLP任务的可解释性提供了重要见解。
...
TL;DR: 跨省评估癌症登记NLP模型泛化能力：通过微调BCCRTron和GatorTron模型，结合保守OR集成策略，在纽芬兰-拉布拉多癌症登记数据上实现高召回率，减少漏诊癌症案例。


<details>
  <summary>Details</summary>
Motivation: 人口癌症登记依赖病理报告，但人工提取资源密集且导致数据延迟。现有基于transformer的NLP系统虽改进工作流程，但其在不同司法管辖区（报告规范不同）的泛化能力尚不明确。本研究首次进行跨省评估，探索如何将在一个省份开发的模型适配到另一个省份。

Method: 使用纽芬兰-拉布拉多癌症登记处约104,000份（Tier 1）和22,000份（Tier 2）去标识化病理报告。微调两个模型：BCCRTron（BC癌症登记处开发的领域适应transformer）和GatorTron（生物医学transformer）。采用互补的摘要式和诊断式报告部分输入管道。使用保守OR集成策略组合两个模型以提高敏感性。

Result: 在NLCR测试集上，微调模型保持高性能，证明在一个司法管辖区预训练的transformer可通过适度微调本地化到另一个管辖区。集成模型在Tier 1任务中召回率达到0.99，漏诊癌症降至24例（单独模型分别为48和54例）。Tier 2任务中集成模型召回率0.99，漏诊可报告癌症降至33例（单独模型分别为54和46例）。

Conclusion: 结合互补文本表示的集成方法可显著减少漏诊癌症并提高癌症登记NLP的错误覆盖率。通过仅共享模型权重的隐私保护工作流程，支持跨省互操作NLP基础设施，为未来泛加拿大癌症病理和登记工作流程基础模型奠定基础。
...
### [78] [A repair scheme for a distributed storage system based on multivariate polynomials](https://arxiv.org/abs/2601.00120)
*Hiram H. López,Gretchen L. Matthews,Daniel Valvo*

Main category: cs.IT

DOI: [10.1007/978-3-030-93954-0_48-1](https://doi.org/10.1007/978-3-030-93954-0_48-1)

Author affiliations: Hiram H. López: Virginia Tech; Gretchen L. Matthews: Virginia Tech; Daniel Valvo: Virginia Tech
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Reed-Solomon码的精确修复方案仅适用于单变量多项式，需要扩展到基于多变量多项式的Reed-Muller码系统，以支持更广泛的分布式存储应用场景

Method: 将GW的精确修复方案从Reed-Solomon码扩展到Reed-Muller码家族，利用多变量多项式特性，在满足特定位置条件下修复单节点和多节点故障

Result: 提出了适用于Reed-Muller码分布式存储系统的修复方案，能够修复任何单节点故障和满足条件的多节点故障

Conclusion: 成功扩展了精确修复方案到Reed-Muller码系统，为基于多变量多项式的分布式存储提供了有效的故障恢复机制
...
### [79] [The permutation group of Reed-Solomon codes over arbitrary points](https://arxiv.org/abs/2601.00122)
*Eduardo Camps-Moreno,Jun Bo Lau,Hiram H. López,Welington Santos*

Main category: cs.IT

DOI: [10.1109/ISIT63088.2025.11195458](https://doi.org/10.1109/ISIT63088.2025.11195458)

Author affiliations: Eduardo Camps-Moreno: Virginia Tech; Jun Bo Lau: KU Leuven; Hiram H. López: Virginia Tech; Welington Santos: University of Wisconsin–Stout

TL;DR: 证明了Reed-Solomon码的置换群由保持评估点集不变的一次多项式给出，为已知特例提供了简洁证明
...
Conclusion: 该结果为Reed-Solomon码置换群提供了统一描述，并简洁地推导了评估点集为整个有限域或乘法群时的已知特例
...
### [80] [Semantic Transmission Framework in Direct Satellite Communications](https://arxiv.org/abs/2601.00381)
...
TL;DR: 提出卫星通信语义传输框架，引入语义效率度量，通过决策辅助REINFORCE++算法优化传输模式选择、卫星-用户关联等参数，提升语义效率


<details>
  <summary>Details</summary>
Motivation: 当前卫星通信中链路预算不足已成为直接接入的瓶颈问题，需要开发有效的语义传输解决方案

Method: 开发卫星通信语义传输框架，引入带优化权重的语义效率度量，提出决策辅助REINFORCE++算法，采用可行性感知动作空间和无批评者稳定策略更新，联合优化传输模式选择、卫星-用户关联、ISL任务迁移、去噪步数和自适应权重

Result: 数值结果表明，所提算法相比基线方法实现了更高的语义效率

Conclusion: 语义传输框架是解决卫星通信链路预算不足问题的有效可行方案，决策辅助REINFORCE++算法能够有效优化复杂非线性整数规划问题
...
### [81] [On the burst-covering radius of binary cyclic codes](https://arxiv.org/abs/2601.00435)
...
TL;DR: 该论文研究突发覆盖码，建立了码参数与突发覆盖半径的一般性界限，针对循环码提供了基于LFSR序列的更强界限，对BCH码证明了LFSR序列模式频率的新界限，并提出了突发覆盖循环码的高效算法。


<details>
  <summary>Details</summary>
Motivation: 研究突发覆盖码的理论性质，特别是建立码参数与突发覆盖半径之间的数学关系，为循环码和BCH码提供更精确的覆盖性能分析。

Method: 1. 定义突发覆盖码并建立一般性界限；2. 利用线性反馈移位寄存器(LFSR)序列分析循环码的突发覆盖半径；3. 对BCH码证明LFSR序列模式频率的新界限；4. 开发突发覆盖循环码的高效算法。

Result: 1. 建立了突发覆盖码参数与覆盖半径的一般性界限；2. 获得了循环码突发覆盖半径的更强界限；3. 证明了BCH码LFSR序列模式频率的新界限，可用于分析二进制原始BCH码和Melas码的覆盖半径；4. 提出了突发覆盖循环码的高效算法。

Conclusion: 该论文系统研究了突发覆盖码的理论性质，建立了多个重要界限，特别是针对循环码和BCH码的强化结果，并提供了实用的算法工具，对编码理论中的覆盖问题有重要贡献。
...
### [82] [Universal Outlier Hypothesis Testing via Mean- and Median-Based Tests](https://arxiv.org/abs/2601.00712)
*Bernhard C. Geiger,Tobias Koch,Josipa Mihaljević,Maximilian Toller*

Main category: cs.IT

Comments: 8 pages, 3 figures; accepted for publication at the International Zurich Seminar on Information and Communication

TL;DR: 本文研究通用离群假设检验问题，提出在离群序列数量随总序列数增长时，分别使用均值法和中位数法估计典型分布，两种方法均能达到已知分布的最大似然检验的错误指数性能。


<details>
  <summary>Details</summary>
Motivation: 传统通用离群假设检验假设观测序列数量固定，而本文考虑观测序列和离群序列数量都随序列长度增长的情况。当离群序列数量与总序列数成比例时，现有方法性能下降，需要新的估计方法。

Method: 提出两种估计典型分布π的方法：1）当离群序列数量是总序列数的次线性时，使用所有观测序列的均值估计π；2）当离群序列数量与总序列数成比例时，使用所有观测序列的中位数估计π。两种方法都不需要知道π和μ的具体分布。

Result: 均值法在离群序列次线性时能达到已知π和μ的最大似然检验的错误指数；中位数法在离群序列成比例时能以概率趋近于1达到相同错误指数。为此引入了典型错误指数的概念来形式化分析。

Conclusion: 在观测序列和离群序列数量都增长的情况下，根据离群序列的比例选择合适的估计方法（均值或中位数），可以在不知道具体分布的情况下达到最优检验性能。
...
### [83] [DiffTetVR: Differentiable Tetrahedral Volume Rendering](https://arxiv.org/abs/2601.00114)
*Christoph Neuhauser*

Main category: cs.GR

TL;DR: DiffTetVR是一种基于四面体网格的可微分体渲染方法，支持优化顶点位置和局部细分，无需多网格方法


<details>
  <summary>Details</summary>
Motivation: 现有可微分渲染方法主要基于规则网格，限制了顶点位置优化和局部细分能力，需要多网格方法支持

Method: 提出基于四面体网格的可微分体渲染解决方案，包括高效的前向渲染实现、反向传播导数推导、防止退化四面体的正则化项，以及支持粗到细优化的局部细分机制

Result: 实现了可优化顶点位置和局部细分的可微分体渲染系统，源代码已在GitHub公开

Conclusion: DiffTetVR为基于四面体网格的可微分体渲染提供了完整解决方案，支持更灵活的几何优化和局部细化
...
### [84] [Modeling and Simulating Origami Structures using Bilinear Solid-Shell Element](https://arxiv.org/abs/2601.00569)
*Qixin Liang*

Main category: cs.GR

TL;DR: 提出一种用于模拟折纸结构的新型计算框架，使用双线性实体壳单元建模折纸面板，通过相邻面板法向向量之间的角度考虑折痕折叠，并采用假定自然应变方法缓解锁定问题。


<details>
  <summary>Details</summary>
Motivation: 需要开发一个能够准确模拟折纸结构的计算框架，特别是要能够处理直线和曲线折痕的复杂折叠行为，同时解决传统实体壳单元中的锁定问题。

Method: 1. 使用双线性实体壳元素建模折纸面板；2. 通过相邻面板法向向量（位移/变形前中面的法向量）之间的角度来模拟折痕折叠；3. 引入假定自然应变方法缓解实体壳元素的锁定问题；4. 对直线和曲线折痕的折纸结构进行仿真验证。

Result: 通过定量和定性分析验证了框架的有效性，证明了该计算框架能够准确模拟包含直线和曲线折痕的折纸结构，同时解决了传统实体壳单元的锁定问题。

Conclusion: 提出的计算框架成功实现了对折纸结构的精确建模和仿真，通过假定自然应变方法有效缓解了锁定问题，为复杂折纸结构的工程应用提供了可靠的计算工具。
...
### [85] [Quantitative Rule-Based Strategy modeling in Classic Indian Rummy: A Metric Optimization Approach](https://arxiv.org/abs/2601.00024)
...
TL;DR: 提出基于MinDist度量的规则框架用于13张牌印度拉米游戏，通过编辑距离量化手牌与最近有效配置的距离，结合对手建模显著提升胜率


<details>
  <summary>Details</summary>
Motivation: 13张牌印度拉米是一种不完全信息顺序游戏，需要概率推理和组合决策。现有启发式方法缺乏形式化框架，需要一种既能捕捉手牌结构接近完成程度，又能高效计算的度量方法。

Method: 1. 提出MinDist度量：修改MinScore度量，通过编辑距离量化手牌与最近有效配置的距离；2. 设计计算高效算法：基于MinScore算法，利用动态剪枝和模式缓存精确计算度量；3. 对手建模：在两人零和模拟框架中整合对手手牌建模；4. 策略评估：使用统计假设检验评估策略性能。

Result: 实证结果显示，基于MinDist的智能体相比传统启发式方法在胜率上有显著提升，为算法化拉米策略设计提供了形式化和可解释的步骤。

Conclusion: MinDist度量有效捕捉了手牌结构接近完成的程度，结合高效计算算法和对手建模，为不完全信息顺序游戏提供了实用的策略框架，在印度拉米游戏中表现出优越性能。
...
DOI: [10.1109/JSTSP.2025.3577756](https://doi.org/10.1109/JSTSP.2025.3577756)

Author affiliations: Shuqi Liu: City University of Hong Kong; Bowei He: City University of Hong Kong; Chen Ma: City University of Hong Kong; Linqi Song: City University of Hong Kong
...
Result: 在两个多轮对话数据集上的实验表明，该方法不仅与人类对话中的底层推理更紧密对齐，还显著提高了检索知识的多样性，从而产生更具信息性和创造性的响应。

Conclusion: 提出的推理感知知识检索方法通过整合检索和推理策略，为LLMs提供逻辑结构对齐的知识，超越了传统基于语义相似性的检索，在多轮对话任务中取得了显著效果提升。
...
TL;DR: 本研究开发了一种基于微调大语言模型的尼日利亚皮钦语自动抑郁症筛查系统，在资源有限环境下实现了高精度筛查。


<details>
  <summary>Details</summary>
Motivation: 尼日利亚抑郁症筛查覆盖率低，传统PHQ-9问卷在高收入国家验证，但存在语言文化障碍，无法适应尼日利亚皮钦语和520多种本土语言的多元环境。

Method: 收集432名18-40岁尼日利亚年轻人的皮钦语音频回答，进行转录、预处理和标注（语义标签、俚语解释、PHQ-9严重程度评分），微调Phi-3-mini-4k-instruct、Gemma-3-4B-it和GPT-4.1三个LLM模型。

Result: GPT-4.1表现最佳：PHQ-9严重程度评分预测准确率达94.5%，优于其他模型；定性评估显示其回答在文化适宜性、清晰度和上下文相关性方面最优。

Conclusion: AI介导的抑郁症筛查为尼日利亚服务不足社区提供了可行方案，为在语言多样、资源有限环境中部署对话式心理健康工具奠定了基础。
...
DOI: [10.1016/j.cor.2023.106516](https://doi.org/10.1016/j.cor.2023.106516)

Author affiliations: Luis Saavedra: Universidad de Alcalá; S. Jiménez‐Fernández: Universidad de Alcalá; A. Portilla-Figueras: Universidad de Alcalá; David Casillas-Pérez: Universidad Rey Juan Carlos; Sancho Salcedo‐Sanz: Universidad de Alcalá
...
<details>
  <summary>Details</summary>
Motivation: 传统基于地理邻近性的包裹分配方法效率低下，导致配送员间工作量分布不均衡。需要优化配送系统，通过平衡工作量来提高配送效率，纠正区域内配送员之间的显著工作量不平衡。

Method: 采用多算法方法，包括不同版本的k-means、进化算法、基于k-means初始化的递归分配（采用不同问题编码）以及混合进化集成算法。算法输入为配送点集合和工人数量，综合考虑距离和工作量来优化包裹分配。

Result: 在西班牙Azuqueca de Henares城市最后一公里包裹配送的实际问题中验证了所提方法的性能，展示了其在实际应用中的有效性。

Conclusion: 提出的多算法方法能够有效解决最后一公里包裹配送中的工作量平衡问题，通过优化配送时间和平衡工作量来提高系统效率，纠正配送员间的工作量不平衡。
...
### [89] [From Clay to Code: Typological and Material Reasoning in AI Interpretations of Iranian Pigeon Towers](https://arxiv.org/abs/2601.00029)
...
TL;DR: 研究探讨生成式AI如何解读乡土建筑中的建筑智能，以伊朗鸽塔为例，测试三种扩散模型在不同提示阶段的表现，评估其重建类型、材料、环境、真实性和文化特异性的能力。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索生成式AI系统如何理解和再现乡土建筑中蕴含的建筑智能，特别是AI在视觉相似性与建筑推理之间的边界，以及计算乡土推理作为分析框架的潜力。

Method: 以伊朗鸽塔为案例研究，测试Midjourney v6、DALL-E 3和基于Stable Diffusion XL的DreamStudio三种扩散模型，采用三个提示阶段（参考性、适应性、推测性），并通过五标准评估框架（类型学、材料性、环境、真实性、文化特异性）进行分析。

Result: AI能可靠地再现几何图案，但误解材料和气候推理；参考图像提高真实性但限制创造力，而无参考的自由生成则产生创新但文化模糊的结果，揭示了视觉相似性与建筑推理之间的边界。

Conclusion: 研究定义了AI在视觉相似性与建筑推理之间的边界，提出了计算乡土推理作为分析框架，用于理解AI如何感知、扭曲和重新想象传统设计智能，为AI在建筑领域的应用提供了批判性视角。
...
### [90] [The Agentic Leash: Extracting Causal Feedback Fuzzy Cognitive Maps with LLMs](https://arxiv.org/abs/2601.00097)
...
TL;DR: LLM智能体从文本中提取因果反馈模糊认知图，形成双向因果学习系统，通过三阶段指令实现节点和边的提取，在基辛格AI论文上验证了与人工生成FCM相似的均衡极限环。


<details>
  <summary>Details</summary>
Motivation: 开发能够从原始文本中自主提取因果结构的智能系统，实现LLM与模糊认知图之间的双向交互，使动态系统具备一定自主性同时保持可控性。

Method: 设计三阶段系统指令：1)从文本提取关键名词和名词短语；2)从名词中提取FCM概念节点；3)推断节点间的部分或模糊因果边。使用LLM智能体(Gemini和ChatGPT)实现这一过程。

Result: 在基辛格AI论文上测试，三阶段过程生成的FCM动态系统收敛到与人工生成FCM相同的均衡极限环，尽管节点和边数量不同。混合FCM吸收了主要组件的均衡，同时创建新均衡以更好近似底层因果动态系统。

Conclusion: LLM智能体能够有效从文本提取因果结构，形成具有自主性的双向学习系统，混合FCM能产生新均衡，为因果建模提供了有前景的方法。
...
### [91] [Mortar: Evolving Mechanics for Automatic Game Design](https://arxiv.org/abs/2601.00105)
*Muhammad U. Nasir,Yuchen Li,Steven James,Julian Togelius*

Main category: cs.AI

TL;DR: Mortar系统结合质量多样性算法和大语言模型，自主演化游戏机制，通过合成完整游戏评估机制质量，基于玩家技能排序保持能力进行优化。


<details>
  <summary>Details</summary>
Motivation: 手动设计游戏机制耗时且依赖专家经验，需要自动化方法来探索多样化的游戏机制设计空间，降低游戏设计门槛并提高效率。

Method: 结合质量多样性算法和大型语言模型探索多样化机制，通过树搜索程序合成完整游戏进行评估，基于玩家技能排序保持能力（强玩家始终优于弱玩家）作为评估标准。

Result: Mortar能生成多样且可玩的游戏，产生的机制在游戏中贡献更高的技能排序得分，消融实验验证了各组件的重要性，用户研究获得人类正面反馈。

Conclusion: Mortar系统成功实现了游戏机制的自主演化，通过结合质量多样性算法和LLM，能够生成高质量、多样化的游戏机制，为自动游戏设计提供了有效解决方案。
...
### [92] [Ask, Clarify, Optimize: Human-LLM Agent Collaboration for Smarter Inventory Control](https://arxiv.org/abs/2601.00121)
*Yaqi Duan,Yichun Hu,Jiashuo Jiang*

Main category: cs.AI

TL;DR: 论文提出了一种混合智能体框架，将LLM作为自然语言接口与严格优化算法解耦，解决了LLM直接作为端到端求解器时的"幻觉税"问题，在库存管理中实现了32.1%的成本降低。


<details>
  <summary>Details</summary>
Motivation: 中小型企业缺乏部署高级优化方法的专业知识，而直接使用LLM作为端到端求解器会产生显著的"幻觉税"——由于模型无法进行基于随机推理的准确计算而产生的性能差距。

Method: 提出混合智能体框架，严格分离语义推理和数学计算：LLM作为智能接口从自然语言中提取参数并解释结果，同时自动调用严格的优化算法构建优化引擎。引入Human Imitator（一个经过微调的有限理性管理者"数字孪生"）来评估系统在真实世界管理对话的模糊性和不一致性下的表现。

Result: 混合智能体框架相对于使用GPT-4o作为端到端求解器的交互式基线，将总库存成本降低了32.1%。研究发现，即使提供完美的真实信息也不足以改善GPT-4o的性能，确认瓶颈本质上是计算性的而非信息性的。

Conclusion: LLM不应作为运筹学的替代品，而应作为自然语言接口，使非专家能够访问基于严格求解器的策略。混合框架通过解耦语义推理和数学计算，有效解决了LLM在优化问题中的"幻觉税"问题。
...
### [93] [Explicit Abstention Knobs for Predictable Reliability in Video Question Answering](https://arxiv.org/abs/2601.00138)
*Jorge Ortiz*
...
TL;DR: 该研究探讨了在视频问答任务中，基于置信度的弃权机制能否提供可靠的错误率控制，以及这种控制在分布偏移下是否稳健。使用NExT-QA数据集和Gemini 2.0 Flash模型，研究发现置信度阈值在分布内能提供机制性控制，但在分布偏移下控制能力会失效。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在高风险部署中需要选择性预测，即系统在不确定时应弃权而非冒险产生代价高昂的错误。研究旨在验证置信度弃权机制能否在视频问答中提供可靠的错误率控制，以及这种控制在分布偏移下是否保持稳健。

Method: 使用NExT-QA数据集和Gemini 2.0 Flash模型进行研究。通过扫描置信度阈值epsilon来产生风险-覆盖权衡曲线，评估置信度阈值在分布内和分布偏移下的控制效果。

Result: 研究有两个主要发现：1) 置信度阈值在分布内能提供机制性控制，通过调整阈值可以产生平滑的风险-覆盖权衡曲线，有效降低错误率；2) 在分布偏移下，置信度阈值提供的控制会失效，无法保持可靠的错误率控制。

Conclusion: 置信度阈值在分布内能有效控制视觉语言模型在视频问答中的错误率，但在分布偏移下这种控制机制会失效，表明需要开发更稳健的选择性预测方法来应对真实世界中的分布变化。
...
### [94] [An AI Monkey Gets Grapes for Sure -- Sphere Neural Networks for Reliable Decision-Making](https://arxiv.org/abs/2601.00142)
...
<details>
  <summary>Details</summary>
Motivation: 当前神经推理方法存在可靠性问题：LLM推理不可靠且难以处理简单决策；监督学习推理存在灾难性遗忘和模式化限制；需要探索更可靠的神经推理方法。

Method: 提出Sphere Neural Networks，将概念表示为n维球面上的圆，通过补圆表示否定操作符，过滤不可满足的圆形配置来实现可靠决策，支持16种三段论推理任务。

Result: Sphere Neural Networks能够掌握16种三段论推理任务（包括严格的析取三段论推理），同时保持经典三段论推理的严谨性，解决了监督学习的灾难性遗忘问题。

Conclusion: 在三种神经推理方法中，基于显式模型构建的神经推理是最可靠的，Sphere Neural Networks为此提供了有效的实现框架。
...
TL;DR: 研究发现LLM赋能的智能体不仅存在人口统计偏见，还会在最小"我们vs他们"线索下表现出群体间偏见。当这种群体边界与智能体-人类划分重合时，风险从人类群体间差异转向更根本的群体不对称——人类整体可能被智能体视为外群体。研究通过多智能体社会模拟发现这种偏见确实存在，并提出了信念投毒攻击来抑制有利于人类的规范脚本。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LLM赋能的智能体是否不仅存在已知的人口统计偏见（如性别、宗教），还会在最小群体线索下表现出群体间偏见。特别关注当群体边界与智能体-人类划分重合时，人类整体可能被智能体视为外群体的风险，这比传统的人口统计偏见更为根本和危险。

Method: 研究方法包括：1）构建基于分配决策的受控多智能体社会模拟，在明确的收益权衡下测试群体间偏见；2）设计信念投毒攻击（BPA），包括初始化时的档案投毒（BPA-PP）和通过优化信念精炼后缀注入存储反思中的记忆投毒（BPA-MP）；3）在多种设置下进行广泛实验验证偏见存在和攻击效果。

Result: 实验结果表明：1）智能体在最小群体线索下确实表现出一致的群体间偏见；2）当部分对应方被框架为人类时，这种偏见会减弱，但这种减弱归因于只有在智能体相信真实人类存在时才激活的隐含人类规范脚本；3）信念投毒攻击能有效抑制人类规范脚本并重新激活对人类的群体间偏见；4）攻击在多种设置下都表现出严重性。

Conclusion: 研究结论是：LLM赋能的智能体存在群体间偏见风险，特别是当智能体-人类划分成为群体边界时。信念依赖为攻击创造了新表面，信念投毒攻击能有效操纵智能体行为。研究提出了在档案和记忆边界实施干预的实用缓解策略，旨在为更安全的智能体设计提供信息，而非促进实际利用。
...
TL;DR: ReCiSt：受生物自愈过程启发的智能体自愈框架，用于分布式计算连续体系统（DCCS），通过语言模型驱动的智能体实现自主故障隔离、因果诊断、自适应恢复和长期知识整合。


<details>
  <summary>Details</summary>
Motivation: 现代DCCS整合了从物联网设备到云基础设施的异构计算资源，其固有的复杂性、移动性和动态操作条件导致频繁故障，需要可扩展、自适应和自我调节的弹性策略。受生物系统卓越恢复能力的启发，需要开发类似的自愈机制。

Method: 将生物自愈的四个阶段（止血、炎症、增殖、重塑）重构为DCCS的计算层（遏制、诊断、元认知、知识）。使用语言模型驱动的智能体解释异构日志、推断根本原因、优化推理路径、重新配置资源，实现自主故障管理。

Result: 在公共故障数据集上使用多种语言模型评估，ReCiSt在数十秒内完成自愈，智能体CPU使用率最低为10%。结果展示了框架克服不确定性的分析深度以及实现弹性所需的微智能体数量。

Conclusion: ReCiSt框架成功将生物自愈原理转化为计算弹性策略，通过语言模型驱动的智能体实现了DCCS的自主故障管理，为复杂分布式系统的自适应恢复提供了新途径。
...
TL;DR: 提出自适应因果协调检测框架ACCD，通过三阶段渐进架构动态学习最优检测配置，显著提升社交平台协同虚假行为检测的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有社交平台协同虚假行为检测方法存在三个主要问题：依赖表面相关性分析、采用静态参数设置、需要大量人工标注。这些局限性导致检测效果不佳且效率低下。

Method: ACCD采用三阶段渐进架构：1) 自适应收敛交叉映射技术深度识别账户间真实因果关系；2) 半监督分类结合主动学习和不确定性采样，大幅减少人工标注需求；3) 基于历史检测经验的自动化验证模块，实现检测结果的自验证和优化。

Result: 在真实数据集上评估显示：ACCD在协同攻击检测中达到87.3%的F1分数，比现有最强基线提升15.2%；减少68%的人工标注需求；通过层次聚类优化实现2.8倍处理速度提升。
...
<details>
  <summary>Details</summary>
Motivation: 传统计算语言学的语义空间推理方法在团队运动战术决策中具有潜在应用价值，通过类比文本与团队的结构相似性，为战术分析提供新的量化框架。
...
Conclusion: 该方法为团队决策和性能优化提供了通用框架，未来方向包括真实数据集成、预测性模拟以及混合人机战术智能的发展。
...
TL;DR: 研究发现推理模型中的"顿悟时刻"（中期推理转变）很少见，不会随训练增加，且很少提高准确性，表明它们并非真正的自我修正机制，而是推理不稳定的表现。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明像DeepSeek-R1-Zero这样的模型会在推理过程中经历突然的"顿悟时刻"，导致准确输出，暗示了内在的自我修正能力。但尚不清楚这种内在推理策略转变是否真正改善性能。

Method: 研究中期推理转变并检测训练过程中的这些转变。分析超过100万个推理轨迹、数百个训练检查点、三个推理领域、多种解码温度和模型架构。

Result: 推理转变很罕见，不会随训练变得更频繁，且很少提高准确性，表明它们不符合先前对模型洞察力的认知。但其效果随模型不确定性而变化：在高熵条件下人为触发外在转变能可靠提高准确性。

Conclusion: 中期推理转变是推理不稳定行为的症状，而非内在的自我修正机制。模型表现出的"顿悟时刻"实际上是推理过程不稳定的表现，而非真正的认知洞察。
...
TL;DR: DA-DPO提出了一种难度感知的直接偏好优化框架，通过平衡偏好数据中的难度不平衡问题，更有效地抑制多模态大语言模型中的幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 现有多模态DPO方法因偏好数据中的难度不平衡问题容易过拟合，模型倾向于过度关注容易区分的偏好对，阻碍了细粒度幻觉抑制并降低整体性能。

Method: DA-DPO包含两个核心组件：1) 难度估计：利用预训练视觉-语言模型，结合生成式和对比式目标，通过分布感知投票策略产生鲁棒的难度分数；2) 难度感知训练：基于估计难度重新加权偏好对，降低简单样本权重，强调困难样本以缓解过拟合。

Result: 大量实验表明DA-DPO能持续改进多模态偏好优化，在标准基准测试中展现出更强的幻觉鲁棒性和更好的泛化能力，同时保持计算效率。

Conclusion: DA-DPO通过难度感知机制有效解决了多模态DPO中的过拟合问题，无需额外数据或微调阶段即可实现更有效的偏好优化，显著提升幻觉抑制效果。
...
TL;DR: PedX-LLM：基于视觉和知识增强的LLM框架，用于行人过街行为推理，在泛化性和准确性上超越传统统计和监督学习方法


<details>
  <summary>Details</summary>
Motivation: 现有行人过街行为推断方法（统计模型和监督学习）泛化性有限，在新场景表现不佳；现有LLM应用缺乏领域特定适应和视觉上下文

Method: 提出PedX-LLM框架，整合LLaVA提取的视觉特征、文本数据和交通领域知识，通过LoRA微调LLaMA-2-7B基础模型来推断过街决策

Result: 达到82.0%平衡准确率，优于最佳统计和监督学习方法；视觉增强模块贡献2.9%性能提升，领域知识整合带来额外4.1%改进；零-shot配置在5个未见测试场景达到66.9%平衡准确率，少-shot学习（仅5个验证样本）提升至72.2%
...
TL;DR: AgenticDomiKnowS (ADS) 使用智能体工作流将自由形式任务描述自动转换为完整的 DomiKnowS 程序，显著降低神经符号编程的门槛和开发时间。


<details>
  <summary>Details</summary>
Motivation: 将符号约束集成到深度学习模型中能提高鲁棒性、可解释性和数据效率，但现有框架如 DomiKnowS 仍要求用户精通特定库语法，开发过程耗时且具有挑战性。

Method: 提出 AgenticDomiKnowS (ADS)，通过智能体工作流将自由形式任务描述翻译为完整的 DomiKnowS 程序。该工作流单独创建和测试每个 DomiKnowS 组件，并支持可选的人机交互干预，允许熟悉 DomiKnowS 的用户细化中间输出。

Result: ADS 使经验丰富的 DomiKnowS 用户和非用户都能快速构建神经符号程序，将开发时间从数小时减少到 10-15 分钟。

Conclusion: ADS 消除了对 DomiKnowS 特定语法的依赖，通过自动化程序生成和可选的用户干预，显著降低了神经符号编程的门槛，提高了开发效率。
...
### [103] [StockBot 2.0: Vanilla LSTMs Outperform Transformer-based Forecasting for Stock Prices](https://arxiv.org/abs/2601.00197)
*Shaswat Mohanty*
...
TL;DR: LSTM模型在金融时间序列预测中表现优于注意力机制和Transformer模型，特别是在数据有限和超参数调优不足的情况下


<details>
  <summary>Details</summary>
Motivation: 金融市场预测面临复杂的时间依赖性、非线性动态和高波动性等挑战，需要评估不同时间序列预测模型在金融领域的实际表现

Method: 基于先前RNN框架，提出增强的StockBot架构，在统一实验设置下系统评估注意力机制、卷积和循环时间序列预测模型，使用默认超参数训练

Result: 精心构建的普通LSTM模型在预测准确性和买卖决策稳定性方面持续优于注意力机制和Transformer模型，特别是在单日离散化数据有限的情况下
...
### [104] [Coupled thermo-chemo-mechanical phase field-based modelling of hydrogen-assisted cracking in girth welds](https://arxiv.org/abs/2601.00471)
*L. Castro,Y. Navidtehrani. C. Betegón,E. Martínez-Pañeda*

Main category: cs.CE

TL;DR: 提出预测氢传输管道焊缝结构完整性的计算框架，结合热机械焊接过程模型和相场断裂模型，首次捕捉残余应力、氢陷阱、氢传输与断裂的相互作用。


<details>
  <summary>Details</summary>
Motivation: 现有天然气管道考虑用于氢传输时，需要评估焊缝在氢环境下的结构完整性。传统天然气管道标准中认为安全的焊接缺陷在氢环境中可能具有破坏性影响，需要新的评估方法。

Method: 开发了包含两个模型的计算框架：(1) 热机械焊接过程模型，识别焊缝三个区域（母材、热影响区、焊缝金属）的尺寸和特性；(2) 耦合变形-扩散-断裂的相场模型，考虑塑性和氢陷阱（多种陷阱类型，静态和动态陷阱密度）。应用于X80管线钢的环焊缝研究。

Result: 虚拟断裂实验与实验室研究结果高度一致。评估了各种焊接缺陷（气孔、未焊透、缺陷、未熔合、根部收缩、咬边）对结构完整性的影响，估算了各种场景下的临界失效压力。结果显示焊缝微观结构和焊接缺陷在氢环境中起重要作用。

Conclusion: 该计算框架能够有效预测氢传输管道焊缝的结构完整性。研究发现，在现有天然气管道中被认为安全的焊接缺陷在氢环境中具有破坏性影响，焊缝微观结构对氢脆敏感性有重要影响，需要重新评估现有管道用于氢传输时的安全性。
...
### [105] [Effect of Electric Charge on Biotherapeutic Transport, Binding and Absorption: A Computational Study](https://arxiv.org/abs/2601.00505)
...
TL;DR: 开发基于Nernst-Planck方程和多孔介质流动理论的数学模型，研究电荷对皮下注射单克隆抗体药物运输和吸收动力学的影响


<details>
  <summary>Details</summary>
Motivation: 探索电荷对皮下注射单克隆抗体药物运输和吸收动力学的影响，理解带电药物在复杂组织环境中的行为机制

Method: 基于Nernst-Planck方程和多孔介质流动理论建立数学模型，研究两种不同电学特性的mAbs，分析缓冲液pH、体重指数、注射深度和制剂浓度对药物分布的影响

Result: 模型能够研究短期运输动力学和长期结合吸收过程，数值结果与文献中的实验数据进行比较验证

Conclusion: 开发了能够模拟带电单克隆抗体在皮下组织中复杂相互作用的新模型，为优化药物制剂和给药方案提供理论工具
...
### [106] [Toward Efficient FSI Modeling in Patient-Specific Arteries: SPH Simulation of Blood Flow in Thin Deformable Vessels](https://arxiv.org/abs/2601.00546)
*Chenxi Zhao,Dong Wu,Weiyi Kong,Oskar J. Haidn,Xiangyu Hu*

Main category: cs.CE

Comments: 60 pages, 38 figures and 2 tables

TL;DR: 提出了一种基于壳模型的降维SPH方法，用于高效模拟薄壁可变形动脉中的血流，解决了传统体积模型计算成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于体积模型的SPH方法在模拟薄壁动脉时需要极细的粒子间距以确保固体力学收敛，导致流体域分辨率冗余，计算成本高昂。

Method: 提出降维壳基SPH方法，用于建模薄壁可变形动脉，并进行流固耦合模拟以捕捉血流动力学和动脉壁力学。

Result: 壳模型在流体动力学方面与体积模型精度相当，在固体力学方面收敛更快且计算成本更低；验证了壁顺应性对流动转变和血流动力学指标的影响。

Conclusion: 该方法在患者特异性血管几何（颈动脉和主动脉）中展示了鲁棒性、高效性和生理相关性，证明了流固耦合建模优于刚性壁假设。
...
### [107] [LLM Agents for Combinatorial Efficient Frontiers: Investment Portfolio Optimization](https://arxiv.org/abs/2601.00770)
*Simon Paquette-Greenbaum,Jiangbo Yu*

Main category: cs.CE

TL;DR: 该研究实现了一个新颖的智能体框架用于解决基数约束均值-方差投资组合优化问题，该框架在基准测试中达到了最先进算法的性能，同时简化了复杂的工作流程和算法开发工作。


<details>
  <summary>Details</summary>
Motivation: 基数约束均值-方差投资组合优化是一个混合整数二次规划问题，传统精确求解器难以处理，需要启发式算法获得近似解。现有方法需要大量繁琐复杂的工作流程和算法开发工作，而智能体框架在组合优化领域显示出自动化工作流程和算法开发的潜力。

Method: 研究实现了一个新颖的智能体框架来解决CCPO问题，探索了多种具体架构。该框架利用智能体技术自动化投资组合优化的复杂工作流程和算法开发过程。

Result: 在基准问题测试中，实现的智能体框架达到了最先进算法的性能水平。同时显著减轻了复杂工作流程和算法开发的工作量，在最坏情况下报告了较低但可接受的误差。

Conclusion: 智能体框架是解决基数约束均值-方差投资组合优化问题的有效方法，能够匹配现有最佳算法性能，同时大幅减少人工工作量和开发复杂性，为投资组合优化提供了新的自动化解决方案。
...
### [108] [Sparse Probabilistic Coalition Structure Generation: Bayesian Greedy Pursuit and $\ell_1$ Relaxations](https://arxiv.org/abs/2601.00329)
*Angshul Majumdar*

Main category: cs.GT

TL;DR: 研究联盟结构生成(CSG)问题，其中联盟价值不是给定的，而是需要从观察中学习。提出稀疏线性回归模型，通过两种估计方法从T个回合中学习稀疏价值函数，然后运行CSG求解器。


<details>
  <summary>Details</summary>
Motivation: 传统的联盟结构生成假设联盟价值已知，但在实际中这些价值往往需要通过观察学习。本文旨在解决当联盟价值未知且需要从经验数据中学习时的CSG问题，特别是当价值函数具有稀疏性时。

Method: 提出概率CSG框架：1) 将每个回合建模为稀疏线性回归问题，实现收益是少数联盟贡献的噪声线性组合；2) 提出两种估计方案：贝叶斯贪婪联盟追踪(BGCP)，模仿正交匹配追踪的贪婪过程；ℓ₁惩罚估计器，在受限特征值条件下工作。

Result: 在相干条件和最小信号假设下，BGCP能以高概率恢复盈利联盟的真实集合，当T ≳ K log m时，产生福利最优结构。ℓ₁惩罚估计器在受限特征值条件下，获得ℓ₁和预测误差界限，并转化为福利差距保证。

Conclusion: 稀疏概率CSG在某些机制下优于传统方法，而密集机制下经典最小二乘方法仍有竞争力。两种估计方案在不同条件下都能有效学习联盟价值函数，为未知价值环境下的联盟结构生成提供了理论保证。
...
### [109] [Unifying Proportional Fairness in Centroid and Non-Centroid Clustering](https://arxiv.org/abs/2601.00447)
...
TL;DR: 本文提出半质心聚类，将质心聚类和非质心聚类统一到一个框架中，研究核心和完全合理表示两种比例公平准则，并设计了多项式时间内达到核心常数近似的新算法。


<details>
  <summary>Details</summary>
Motivation: 现有比例公平聚类研究分为两个独立范式：质心聚类（Chen等，ICML 2019）和非质心聚类（Caragiannis等，NeurIPS 2024）。本文旨在统一这两个范式，提出更通用的半质心聚类框架，研究比例公平准则在该框架下的可计算性。

Method: 提出半质心聚类模型，其中每个数据点的损失是其质心损失和非质心损失的组合。研究两种比例公平准则：核心及其松弛版本完全合理表示。开发了新颖算法，即使在质心和非质心损失使用不同距离度量的情况下，也能在多项式时间内实现核心的常数近似。

Result: 主要结果是设计了一个多项式时间算法，能够达到核心的常数近似，即使质心和非质心损失使用不同的距离度量。对于更受限的损失函数和较弱的FJR准则，获得了改进的结果，并在各种情况下建立了下界。

Conclusion: 半质心聚类为比例公平聚类提供了统一框架，成功将质心和非质心聚类范式结合起来。提出的算法在多项式时间内实现了核心的常数近似，为比例公平聚类提供了有效的计算解决方案，并建立了理论下界。
...
### [110] [The CoinAlg Bind: Profitability-Fairness Tradeoffs in Collective Investment Algorithms](https://arxiv.org/abs/2601.00523)
*Andrés Fábrega,James Austgen,Samuel Breckenridge,Jay Yu,Amy Zhao,Sarah Allen,Aditya Saraf,Ari Juels*

Main category: cs.GT

TL;DR: 集体投资算法存在基本权衡：保护隐私会引发内部人攻击，而完全透明则导致套利侵蚀利润


<details>
  <summary>Details</summary>
Motivation: 集体投资算法旨在民主化复杂的AI投资工具，但作者发现这些系统存在根本性的盈利能力与公平性权衡，即"CoinAlg困境"

Method: 建立集体投资算法的形式化模型，定义隐私（算法不完全披露）和经济公平性（对抗性内部人的价值提取），从理论和实证两方面分析：1）证明隐私是内部人攻击的先决条件；2）在博弈论模型中证明透明度会引发套利侵蚀利润；3）使用Uniswap数据实证研究两方面的权衡

Result: 理论证明：1）集体投资算法中的隐私是内部人攻击经济公平性的必要条件；2）缺乏隐私（透明度）使套利者能够侵蚀算法利润。实证研究：量化透明集体投资算法面临的套利影响，并展示私人集体投资算法的风险——即使低带宽隐蔽信道信息泄露也能实现不公平的价值提取

Conclusion: 集体投资算法面临根本性的盈利能力-公平性权衡困境：确保经济公平性会因套利而损失利润，而保护隐私则使系统易受内部人攻击，这一困境限制了集体投资算法在民主化复杂投资工具方面的潜力
...
- [Talk Less, Verify More: Improving LLM Assistants with Semantic Checks and Execution Feedback](https://arxiv.org/abs/2601.00224) — 相关度 0.85：提出语义反向翻译和执行反馈两种验证机制，改善LLM生成代码的准确性和可执行性，可迁移到代码代理的自我验证。
