...
Comments: 25 pages; bilingual English-Chinese version. The complete English version is followed by the complete Chinese version; each version contains 3 figures and 5 tables
...
Comments: Paper accepted to ICSME
...
- [Fantastic Adaptive Taxonomies and How to Use Them](https://arxiv.org/abs/2607.16387) — 相关度 0.95：从轨迹归纳失败分类法，改进agent搜索、运行时反馈与轨迹选择，直接改善agent可靠性。
- [Where Does Agent Reliability Come From? A Cross-Benchmark Decomposition of Verification Loops, Specialist Models, and Scaffolding in a Production Enterprise Agent](https://arxiv.org/abs/2607.17044) — 相关度 0.95：系统分解了企业agent验证循环、专家模型与脚手架对可靠性的贡献，直接相关
- [IssueExec: A Test-Driven Approach for Localizing Software Engineering Issues](https://arxiv.org/abs/2607.17286) — 相关度 0.92：测试驱动的issue定位方法，显著提升SWE-bench Lite的下游修复率，直接契合agent
- [AEVAL: From Anecdotal to Deterministic Testing for Agentic Skill Workflows](https://arxiv.org/abs/2607.16345) — 相关度 0.90：为agentic skill提供确定性CI测试框架，含执行器/评判分离与自修正偏差处理，直接用于agent评测。
- [DepRepair: LLM-Based Source-Code Repair for Dependency Breaking Changes](https://arxiv.org/abs/2607.17957) — 相关度 0.90：LLM修复依赖破坏性变更，提出DepBench基准与DepRepair方法，直接相关代码修复。
- [How Agent Skills Fail under Long Contexts: A White-Box Study in Code Auditing](https://arxiv.org/abs/2607.17937) — 相关度 0.90：白盒研究agent skills在长上下文中的失效分类，直接关系代码审计agent可靠性。
- [Portable models as a replacement for industrial heuristics in compiler optimizations](https://arxiv.org/abs/2607.17389) — 相关度 0.90：用便携模型预测函数内联决策，直接面向编译器优化，迁移价值高
- [TRIM: Reducing AI-Generated CodeSlop via Agent Trajectory Minimization](https://arxiv.org/abs/2607.18161) — 相关度 0.90：针对agent生成代码冗余提出TRIM轨迹最小化，直接优化agent输出质量。
- [Autoresearch with Coding Agents: Generalizers and Metric-Maximizers on Quran Recitation Data](https://arxiv.org/abs/2607.18064) — 相关度 0.88：编码agent在autoresearch中的规格游戏行为，对agent评估与可靠性有重要启示。
- [KernelDiag: Agent-Based Root Cause Diagnosis for Kernel Crashes](https://arxiv.org/abs/2607.17722) — 相关度 0.88：基于agent的内核崩溃根因诊断，直接涉及调试agent与程序分析。
- [Agentic Code Review in the Terminal: A Trajectory-Level Analysis of Behavior, Cost, and Human-Alignment](https://arxiv.org/abs/2607.16740) — 相关度 0.85：agentic code review的轨迹级行为/成本/对齐分析，直接针对代码审查agent的评估。
- [MechMem-RTL: Reusing Verified Mechanism Memories for LLM-Based RTL Repair](https://arxiv.org/abs/2607.17053) — 相关度 0.85：基于验证器确认的记忆记录复用机制，对LLM代码修复和记忆设计有直接借鉴
- [Prefactory: Automated Discovery and Application of Library-Adoption Refactorings](https://arxiv.org/abs/2607.17211) — 相关度 0.85：利用LLM生成可执行搜索启发式进行库采用重构，自动化重构agent的实用方案
- [Teach it to stop, not just to click](https://arxiv.org/abs/2607.17136) — 相关度 0.85：研究agent可靠性与验证器引导修复，并给出多seed评估方法论，对agent评测有参考
- [Test Coverage Analysis of Agentic Pull Requests](https://arxiv.org/abs/2607.18057) — 相关度 0.85：大规模分析agent生成PR的测试覆盖，直接评估代码agent测试质量。
- [Verify, Repair, Repeat, or Stop? Robust Stopping for Noisy Verify-Repair Loops in LLM Agents](https://arxiv.org/abs/2607.17641) — 相关度 0.85：提出噪声验证-修复循环的鲁棒停止准则，对代码生成agent可靠性重要。
- [A Systematic Evaluation of Trajectory Data Curation for LoRA Fine-Tuning of Code Agents](https://arxiv.org/abs/2607.17205) — 相关度 0.80：系统研究轨迹数据筛选对LoRA微调代码agent的影响，对训练数据选择有参考。
- [AoA: Theorem Proving Agent over Abstract Syntax Tree of Redesigned Language](https://arxiv.org/abs/2607.16372) — 相关度 0.80：基于AST的定理证明agent，减少token/API成本，可迁移到长程代码agent的效率优化。
- [DataFlow-Harness: A Grounded Code-Agent Platform for Constructing Editable LLM Data Pipelines](https://arxiv.org/abs/2607.16617) — 相关度 0.80：代码agent构建数据管道DAG的平台，提供MCP与可视化编辑，提升可靠性并降低成本。
- [Falsification-Based Verification of LLM-Generated Optimization Models: Sound Test Batteries and Their Detection Limits](https://arxiv.org/abs/2607.16646) — 相关度 0.80：用falsification测试验证LLM生成的优化模型，提供sound测试组合，可迁移到代码正确性验证。
- [Model-Driven Discipline for Multi-Agent LLMs: Requirement-to-Verification Generation of Traceable System Models](https://arxiv.org/abs/2607.16708) — 相关度 0.80：多agent LLMs进行模型驱动工程，生成可验证的系统模型并支持变更影响分析，体现多agent协作与验证。
- [When to Use Which? Benchmarking Optimisers for Configurable Systems under Varying Budgets](https://arxiv.org/abs/2607.16476) — 相关度 0.80：配置系统优化器基准与预算效应分析，对SemOpt调优预算策略有参考价值。
- [BTOR2-Based C Program Verification via Hardware Model Checking](https://arxiv.org/abs/2607.17622) — 相关度 0.75：将C程序验证编码为BTOR2进行硬件模型检验，涉及正确性验证，可为验证后端提供参考
- [CLOSER-Bench: Evaluating Budgeted Cross-Stage Design Closure for Hardware Agents](https://arxiv.org/abs/2607.16632) — 相关度 0.75：硬件agent预算约束下的跨阶段设计闭环基准，评估长程多工具协作，可推广到软件agent。
- [From Discussion to Execution: Replicating Buggy and Correct Data Science Code](https://arxiv.org/abs/2607.16569) — 相关度 0.75：从Q&A帖子复现buggy/patched数据科学代码，构建可执行对，可用于agent调试与验证。
- [Integrating High-Level Requirements to Low-Level Tests with Machine-Readable V&V Specifications](https://arxiv.org/abs/2607.17686) — 相关度 0.75：机器可读的V&V规范连接需求和测试，显式讨论扩展到AI编码agent，有工程化价值
- [SynH-Rank: Quality-Aware Code Search via Diverse Data Synthesis and Hierarchical Ranking Training](https://arxiv.org/abs/2607.17139) — 相关度 0.75：质量感知的代码检索重排序方法，可提升代码检索与agent工具选择
- [(Over)Reliance on Test Agents in AI-Assisted Software Testing](https://arxiv.org/abs/2607.17927) — 相关度 0.70：分析AI测试代理过度依赖问题并提出数据收集框架，对测试agent评估有参考价值。
- [RAIL Guard: Closing the Evaluation-to-Remediation Gap in Responsible AI for LLM Agents](https://arxiv.org/abs/2607.16215) — 相关度 0.70：提出evaluate-rewrite-reevaluate闭环修复机制，可迁移到代码agent的可靠性改进
- [Chiral Analysis of Smart Contracts: Detecting Vulnerabilities from Relational Inconsistencies Across Business Paths](https://arxiv.org/abs/2607.17987) — 相关度 0.65：智能合约漏洞检测新范式，利用LLM与语义分析，属程序分析可迁移到代码agent。
- [Is Progressive Disclosure All You Need for Long-Context Agents?](https://arxiv.org/abs/2607.17598) — 相关度 0.65：研究长上下文agent中渐进式披露技能包的有效性，可迁移到代码agent上下文管理。
- [Schema-Bound LLM Control of Scientific Instrumentation through Model Context Protocol Skills](https://arxiv.org/abs/2607.17012) — 相关度 0.65：基于MCP的schema约束工具控制与持久生命周期，可迁移到coding agent的工具接口设计
- [Testing Retrieval-Augmented Generation Systems with Chunk Coverage](https://arxiv.org/abs/2607.18155) — 相关度 0.65：提出RAG系统测试的Chunk Coverage准则，可迁移到agent检索测试。
- [CommitLLM: A Fine-Tuned Pipeline for Git Commit Message Generation](https://arxiv.org/abs/2607.17532) — 相关度 0.60：约束解码与确定性后处理生成commit消息的管线，对agent结构化输出有参考价值
- [Dependency-Guided Code Generation: Structured Matrix Decomposition and Consistency-Guided Refinement](https://arxiv.org/abs/2607.16692) — 相关度 0.60：依赖感知的代码生成框架，通过矩阵分解建模实体依赖，可提升代码语义一致性。
- [Specifying the Delegated-Autonomy Boundary: Requirements Engineering for Agentic AI](https://arxiv.org/abs/2607.17225) — 相关度 0.55：提出agentic AI的委托自治边界规范工件，对设计代码agent的权限边界有参考意义
