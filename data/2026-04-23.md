...
- [Feedback Over Form: Why Execution Feedback Matters More Than Pipeline Topology in 1-3B Code Generation](http://arxiv.org/abs/2604.21950v1) — 相关度 0.90：在小模型代码生成中证明执行反馈比流水线拓扑更关键，并对自演化搜索、早停等有实证结论，直接指导 code agent 设计。
- [You Don't Need Public Tests to Generate Correct Code](http://arxiv.org/abs/2604.21598v2) — 相关度 0.90：提出无需公共测试用例的自生成输入与模拟执行框架，直接改进代码生成agent的自校正能力。
- [Call-Chain-Aware LLM-Based Test Generation for Java Projects](http://arxiv.org/abs/2604.22046v1) — 相关度 0.85：调用链感知的LLM测试生成方法，显式建模依赖上下文，对代码agent的测试生成有直接迁移价值。
- [How Consistent Are LLM Agents? Measuring Behavioral Reproducibility in Multi-Step Tool-Calling Pipelines](http://arxiv.org/abs/2605.28840v1) — 相关度 0.85：系统测量多步工具调用agent的行为一致性，提出可靠性评估维度，直接适用于代码agent的鲁棒性研究。
- [Strategic Heterogeneous Multi-Agent Architecture for Cost-Effective Code Vulnerability Detection](http://arxiv.org/abs/2604.21282v1) — 相关度 0.85：提出异构多agent架构用于漏洞检测，结合云LLM专家与本地验证器，可迁移到代码agent的协作与验证机制。
- [A Metamorphic Testing Approach to Diagnosing Memorization in LLM-Based Program Repair](http://arxiv.org/abs/2604.21579v1) — 相关度 0.80：结合蜕变测试与NLL诊断LLM程序修复中的记忆泄漏，提供了评测可靠性的方法。
- [Assessing the Impact of Requirement Ambiguity on LLM-based Function-Level Code Generation](http://arxiv.org/abs/2604.21505v1) — 相关度 0.80：首个面向需求歧义的代码生成benchmark，系统评估歧义对LLM代码生成的影响，具有评测方法价值。
- [FlyCatcher: Neural Inference of Runtime Checkers from Tests](http://arxiv.org/abs/2604.22028v1) — 相关度 0.80：结合LLM、静态分析与动态验证从测试推断运行时检查器，方法可迁移到代码agent的验证与错误检测。
- [Generalizing Test Cases for Comprehensive Test Scenario Coverage](http://arxiv.org/abs/2604.21771v2) — 相关度 0.80：提出从初始测试泛化测试场景的框架，提升场景覆盖，对自动化测试生成agent有直接贡献。
- [Less Is More: Measuring How LLM Involvement affects Chatbot Accuracy in Static Analysis](http://arxiv.org/abs/2604.21746v1) — 相关度 0.80：比较直接生成、结构化中间表示和agentic工具调用的静态分析查询生成，发现结构化约束效果更优，可迁移到代码分析agent设计。
- [SpecSyn: LLM-based Synthesis and Refinement of Formal Specifications for Real-world Program Verification](http://arxiv.org/abs/2604.21570v1) — 相关度 0.80：基于LLM的正式规格生成与精化，用于程序验证，包含变异判别机制，可迁移到代码验证agent。
- [VLAA-GUI: Knowing When to Stop, Recover, and Search, A Modular Framework for GUI Automation](http://arxiv.org/abs/2604.21375v2) — 相关度 0.80：模块化GUI自动化框架包含停止、恢复、搜索机制，可迁移到代码agent的长程执行与容错设计。
- [CrossCommitVuln-Bench: A Dataset of Multi-Commit Python Vulnerabilities Invisible to Per-Commit Static Analysis](http://arxiv.org/abs/2604.21917v1) — 相关度 0.75：提出多提交漏洞数据集，揭示逐个提交静态分析的盲区，可用于构建代码安全分析与agent验证基准。
- [PrismaDV: Automated Task-Aware Data Unit Test Generation](http://arxiv.org/abs/2604.21765v1) — 相关度 0.75：复合AI系统自动生成任务感知的数据单元测试，结合提示优化，对agent化测试生成有借鉴意义。
- [Agentic AI-assisted coding offers a unique opportunity to instill epistemic grounding during software development](http://arxiv.org/abs/2604.21744v1) — 相关度 0.70：提出领域约束的epistemic grounding文档来约束agentic coding，增强代码agent的可靠性与领域正确性。
- [From If-Statements to ML Pipelines: Revisiting Bias in Code-Generation](http://arxiv.org/abs/2604.21716v1) — 相关度 0.70：指出条件语句评测低估代码生成偏差，提出基于ML pipeline的偏差评测，对代码生成评估有参考价值。
- [Can Large Language Models Assist the Comprehension of ROS2 Software Architectures?](http://arxiv.org/abs/2604.21699v1) — 相关度 0.65：评估LLM理解ROS2软件架构的能力，为软件架构理解agent提供实证参考。
- [Conjecture and Inquiry: Quantifying Software Performance Requirements via Interactive Retrieval-Augmented Preference Elicitation](http://arxiv.org/abs/2604.21380v2) — 相关度 0.65：通过交互式检索增强偏好学习量化性能需求，可迁移至agent在需求理解与交互澄清中的应用。
- [Hidden Dependencies and Component Variants in SBOM-Based Software Composition Analysis](http://arxiv.org/abs/2604.21278v1) — 相关度 0.60：揭示 SBOM 中隐藏依赖与组件变体导致的漏洞报告不一致，为依赖安全分析/供应链 agent 提供实证依据。
- [MathDuels: Evaluating LLMs as Problem Posers and Solvers](http://arxiv.org/abs/2604.21916v2) — 相关度 0.60：提出自博弈双角色LLM评测机制，可迁移到代码agent的能力评估与动态benchmark设计。
- [mcdok at SemEval-2026 Task 13: Finetuning LLMs for Detection of Machine-Generated Code](http://arxiv.org/abs/2604.21365v1) — 相关度 0.60：针对机器生成代码检测的微调方法，涉及代码理解和归因，对代码agent生成内容识别有参考价值。
- [Ethics Testing: Proactive Identification of Generative AI System Harms](http://arxiv.org/abs/2604.22089v1) — 相关度 0.50：提出伦理测试概念以识别生成式AI系统危害，对GAI代码生成风险评估有参考价值。
