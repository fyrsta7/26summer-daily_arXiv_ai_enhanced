...
- [Tool-Schema Compression Enables Agentic RAG Under Constrained Context Budgets](http://arxiv.org/abs/2605.26165v1) — 相关度 0.85：研究工具schema压缩以解决agentic RAG上下文预算冲突，对受限上下文下代码agent的工具调用效率提升非常相关。
- [Multi-Agent Specification-based Metamorphic Testing of FMU-Based Simulations](http://arxiv.org/abs/2605.25101v1) — 相关度 0.80：LLM多agent工作流从规范提取蜕变关系并生成测试，对基于规范的自动化测试生成有清晰可迁移性。
- [PR-Aware Automated Unit Test Generation: Challenges and Opportunities](http://arxiv.org/abs/2605.25285v1) — 相关度 0.80：系统评估PR感知的测试生成（EvoSuite vs GPT-4o）并定义F2P指标，为agent测试生成提供实证基础。
- [Specification-Based Code-Text-Code Reengineering for LLM-Mediated Software Evolution](http://arxiv.org/abs/2605.25232v1) — 相关度 0.80：提出Code2Text2Code的规范式重工程框架，结合中间表示与验证，对LLM驱动的代码演化有直接参考价值。
- [Inverting the Shield: Systematically Generating Safety Tests from Policy Specifications](http://arxiv.org/abs/2605.24883v1) — 相关度 0.75：基于规范生成LLM安全测试，通过FOL形式化和策略图实现可覆盖测试，适用于agent的安全性评测与测试生成。
- [Specifying AI-SDLC Processes: A Protocol Language for Human-Agent Boundaries](http://arxiv.org/abs/2606.20615v2) — 相关度 0.75：为AI-SDLC流程设计协议语言以定义人机边界与审批门，可迁移至多agent软件工程流程的规范与校验。
