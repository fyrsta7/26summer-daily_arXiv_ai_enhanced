...
Selection reason: 结合编译诊断、CodeQL静态分析和KLEE符号执行与检索修复，显著提升LLM生成C代码的编译通过率和安全性，体现工具反馈价值
...
Selection reason: vibe coding多声文献综述，刻画生成-评估-修订工作流及开发者角色转变，可参考理解agent实践
...
Selection reason: 跨模型代码审查实验揭示结对不对称收益，对多agent协作和代码审查排布有直接指导
...
Selection reason: 度量代码文本/图像表示的token核算差异，为coding agent输入选择提供实证成本数据
...
Selection reason: 揭示输出格式与模型身份的交互效应并对coding agent成功率有显著影响，提出格式化工具设计原则
...
Selection reason: LLM引导KLEE定向符号执行，结合安全语义标记和循环退出优先，显著提升漏洞发现覆盖率
...
Selection reason: 利用多采样SLM与编译器反馈优化射电天文代码，展示低能耗agentic代码优化的可行路径
...
Selection reason: 实证分析agentic pull request的特性与演化，为评估coding agent真实开发贡献提供证据
...
Selection reason: 面向并行编码agent提出变更意图声明、动态作用域与确定性控制平面，直接解决集成冲突和范围蔓延问题。
...
Selection reason: 构建基于已解决事故历史的LLM运维助手，结构化检索与MAPE-K闭环可迁移至代码agent的故障诊断。
...
Selection reason: 多agent框架自动生成Rust验证规格，含生成-预检-验证循环和候选选择策略，对验证型agent有直接价值。
...
Selection reason: 大规模实证探究开发者对AI审查评论的响应规律，识别关键影响因素，为改进代码审查agent提供依据。
...
Selection reason: 提出Agent所用云技能包的测试覆盖度度量与改进推荐，可迁移至agent工具评估与测试优化。
...
Selection reason: 提出对话式AI审查助手范式，把审查从一次性评论转向人机交互，为审查agent设计提供新方向。
...
Selection reason: 利用LLM从测试套件自动生成Rust验证harness并迭代修正，可增强agent的验证与缺陷检测能力。
...
Selection reason: 实验对比人类与CLLM的多种协作模式及TDD流程，对代码agent的交互与工作流设计有参考价值。
...
Selection reason: 提出测试用例质量挖掘与二部图互验证的TDD闭环框架，有效提升代码生成可靠性与选择稳定性。
