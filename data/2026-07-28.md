...
### [30] [From Obligation to Specification: A Survey on Validating EU AI Act Requirements in RE](https://arxiv.org/abs/2607.21608)
...
TL;DR: 该研究通过专家访谈和在线调查发现，组织在应对欧盟《人工智能法案》的需求工程方面准备不足，缺乏结构化机制；基于LLM的验证工具被认为有前景，但需警惕完全自动化，并提出了闭环方法的最低要求。


<details>
  <summary>Details</summary>
Motivation: 探究组织对欧盟《人工智能法案》在需求工程方面的准备情况，以及基于LLM的智能体验证工具能否帮助将监管义务转化为可审计的需求和证据。

Method: 采用混合方法探索性研究：专家访谈（N=10）和在线调查（N=15），参与者涵盖需求工程、数据科学、开发和合规角色。

Result: 尽管欧盟《人工智能法案》被视为高度相关，但缺乏捕获监管义务、传播更新和维护全生命周期可追溯性的结构化机制；参与者认为LLM工具有前景但强烈反对全自动化，强调需要保障措施。

Conclusion: 基于发现，提出了面向欧盟《人工智能法案》就绪的闭环方法的最低要求，包括结构化机制和人工监督的必要性。

中文摘要：随着欧盟《人工智能法案》的生效，开发或运营人工智能系统的组织面临着透明度、风险管理和可追溯性方面的新义务。对于需求工程（RE）而言，这些义务必须转化为可测试、可审计的需求以及可验证的证据。然而，许多组织目前缺乏系统化的流程来实现这一目标。我们假设基于大语言模型（LLM）的智能体验证工具能够支持这种转化，从而帮助弥合这一差距。我们开展了一项混合方法探索性研究，包括专家访谈（N=10）和在线调查（N=15），以评估组织面向欧盟《人工智能法案》的需求工程准备情况，以及对基于LLM的智能体闭环验证工具的认知，参与者涵盖需求工程、数据科学、开发和合规等角色。我们的结果表明，尽管欧盟《人工智能法案》被认为高度相关，但捕获监管义务、将更新传播到项目中以及保持全生命周期可追溯性和证据的结构化机制往往缺失。参与者认为基于LLM的工具在将义务映射到需求、评估覆盖范围和组织证据方面有前景，但对完全自动化表示强烈担忧，并强调需要保障措施。基于这些发现，我们概述了面向欧盟《人工智能法案》就绪的闭环方法的最低要求。
...
### [31] [Tool-Guided Retrieval-Augmented Repair for Securing LLM-Generated C Code](https://arxiv.org/abs/2607.21641)
...
### [32] [Vibe Coding in Software Development: A Multivocal Literature Review](https://arxiv.org/abs/2607.21652)
...
### [33] [Cross-Model LLM Code Review: Should you use Claude to review Codex or vice versa?](https://arxiv.org/abs/2607.21656)
...
### [34] [Pixels for Programs? A Cross-Provider Case Study of Input-Token Accounting for Source Code as Text and Images](https://arxiv.org/abs/2607.21672)
*Ronak Bhalgami*
...
### [35] [Output Format x Model Identity: Interaction Effects in Single-Round Coding Agent Performance](https://arxiv.org/abs/2607.21674)
*Yang Yang*
...
### [36] [Directed Symbolic Execution for Vulnerability Discovery: An LLM-Guided Approach in KLEE](https://arxiv.org/abs/2607.21676)
...
### [37] [Enhancing SLMs for Sustainable Code Optimization in Radio-Astronomy](https://arxiv.org/abs/2607.21677)
...
### [38] [When Model Release Meets Model Reuse: Producer-Consumer Misalignment in Hugging Face](https://arxiv.org/abs/2607.21738)
...
TL;DR: 对PTLM供应链中生产者-消费者错位的首次双视角调查，发现他们在文档、溯源和治理偏好上存在显著不一致。


<details>
  <summary>Details</summary>
Motivation: 先前研究记录了PTLM供应链的结构性问题，但未探索生产者-消费者之间的错位及其背后的期望和交互。

Method: 对50名Hugging Face生产者和95名GitHub消费者进行双视角问卷调查，涵盖模型发现、文档实践、溯源追踪和模型治理采纳四个维度。

Result: 生产者和消费者依赖相同文档但对元数据记录位置有分歧；溯源动机不同（生产者重可复现性，消费者重质量可靠性）；治理偏好差异（生产者重简化发布，消费者重文档和依赖透明度）。

Conclusion: 需改进PTLM供应链中的文档规范、溯源可见性和治理支持，以缓解生产者与消费者之间的错位。

中文摘要：预训练语言模型（PTLM）在现代软件系统中越来越被重用为依赖项，尽管先前的工作已经记录了PTLM供应链中持续存在的结构性问题，例如不一致的发布实践、不完整的元数据以及Hugging Face和GitHub仓库之间的分歧。这些问题的一个先前未探索的角度是，PTLM生产者的发布实践与消费者的模型重用需求之间的错位可能解释了其中的几个挑战，然而这种错位背后的人类期望、过程和交互从未被研究过。因此，我们进行了首次双视角调查，涉及50名Hugging Face生产者和95名GitHub消费者，以检查在四个AI供应链维度上的生产者-消费者错位：模型发现、文档实践、溯源追踪和模型治理采纳。我们发现，尽管生产者和消费者依赖相同的文档工件，但他们在关键元数据应该记录在哪里存在分歧。即使27.9%的生产者和31.4%的消费者追溯超越直接父模型的谱系，生产者主要为了可证明性和可复现性而追踪模型谱系，而消费者则主要由质量和可靠性问题驱动。此外，生产者认为简化模型发布的治理机制将产生最大的积极影响，而消费者则优先考虑文档和依赖透明度。这些发现突出了在PTLM供应链中改进模型文档规范、谱系可见性和治理支持的机会。
...
### [39] [How Do AI Coding Agents Contribute to Software Development? an Empirical Study of Agentic Pull Requests](https://arxiv.org/abs/2607.21832)
...
### [40] [No Snake Oil: Verifying Python Package Builds](https://arxiv.org/abs/2607.21888)
...
TL;DR: Python包重建中字节级等价率低，但daleq4py通过规范化函数显著提高了可接受的等价重建比例。


<details>
  <summary>Details</summary>
Motivation: Python包供应链面临恶意软件注入风险，独立重建可检测受损构建，但现有工具（macaron和oss-rebuild）的字节级等价率低，需要更灵活的等价性判定方法以扩大可信重建范围。

Method: 提出daleq4py工具，采用基于保留来源的Datalog规则的规范化函数核，通过比较规范化后的轮子来建立等价性，而非严格的字节级比较。

Result: 在12,180个PyPI流行版本中，macaron和oss-rebuild的字节级等价率分别为15.4%和19.1%，而daleq4py将源等价重建的等价率提升至60.2%和78.9%。

Conclusion: daleq4py通过规范化方法有效扩展了可接受的等价重建集合，能保留用户对可重建版本的大部分保证，提升了供应链安全分析的实用性。

中文摘要：Python已成为与AI交互的默认语言，其包通过Python包索引（PyPI）等注册表分发。这需要分析包含这些包的供应链。其中一种分析方法是重建包，以识别注入恶意软件的受损构建。在加固环境中的独立重建具有额外优势，即它们可以生成并记录来源，以增加包的可信度。两个旨在自动化此类重建并大规模运行的工具是macaron和oss-rebuild。我们研究了来自PyPI的12,180个流行版本，发现字节级等价率普遍较低。我们分析了它们生成不同轮子的原因，并发现原始轮子与重建轮子之间通常仍可建立等价性，从而保留了用户对可重建版本所期望的大部分保证。我们提出并评估了daleq4py，这是一个通过基于保留来源的Datalog规则的规范化函数的核来建立Python轮子等价性的工具。实验结果表明，daleq4py大大扩展了可被接受为等价的重建集合。尽管只有15.4%的macaron重建和19.1%的oss-rebuild重建与已发布的PyPI轮子字节级相同，但daleq4py分别为60.2%和78.9%的源等价重建建立了轮子等价性。
...
### [41] [Claim Plane: Enforceable Change Intents and Dynamic Scope for Parallel Coding Agents](https://arxiv.org/abs/2607.21909)
*Maxim Nikolaev*
...
### [42] [Leveraging Resolved Incident History for LLM-Assisted Software Bug Diagnosis](https://arxiv.org/abs/2607.21911)
...
### [43] [KaPilot: LLM-Assisted Generation of Kani Specifications for Unsafe Rust Verification](https://arxiv.org/abs/2607.21957)
...
### [44] ["Go Home Copilot, You're Drunk": Understanding Developer Responses to Agent-Generated Code Review Comments](https://arxiv.org/abs/2607.21997)
...
### [45] [Are Production Cloud Skills Adequately Tested? Measuring and Governing Skill Test Coverage in Practice](https://arxiv.org/abs/2607.22015)
...
### [46] [Code Review is a Conversation: Toward Conversational AI Review Assistants](https://arxiv.org/abs/2607.22095)
*Rosalia Tufano*
...
### [47] [No Edges, No Verdict: A Large-Scale Empirical Study of Declared Dependency Graphs in 78K SBOMs in the Wild](https://arxiv.org/abs/2607.22140)
...
TL;DR: 首次大规模分析SBOM依赖图，发现52.9%没有边，并证明基于不完整图的“无路径=不可达”推断有害，提出用“未知”级别替换，将KEV召回率从0.600提升至0.950。


<details>
  <summary>Details</summary>
Motivation: SBOM作为依赖图用于漏洞分析，但对其实际结构缺乏大规模实证研究，导致消费者可能做出错误推理。

Method: 对Wild SBOMs数据集的78,612个SBOM进行依赖图拓扑分析，统计边存在性、孤儿组件比例，并设计退化检测器替换封闭世界推理，评估KEV召回率变化。

Result: 52.9% SBOM无边，8.8%退化（中位孤儿93%），38.3%连接良好；边存在性由生成器决定；CycloneDX compositions使用率0.10%；替换推理后KEV召回率从0.600升至0.950。

Conclusion: 当前SBOM依赖图普遍不完整，消费者不应默认“无路径即不可达”，需引入显式“未知”状态和退化检测以提高漏洞优先级排序的准确性。

中文摘要：软件物料清单（SBOM）不仅作为组件清单被消费，还作为依赖图：漏洞分类、可达性过滤和影响分析都遍历SBOM声明的边。我们首次对来自Wild SBOMs数据集的78,612个真实SBOM文件（77,092个可解析）中声明的依赖图进行了大规模表征。我们发现总体分为三种状态：52.9%的SBOM根本没有声明任何边（未满足NTIA对依赖关系的最低要素要求），8.8%声明了依赖块但使大多数组件孤立（退化状态；在此类具有至少50个组件的SBOM中，孤儿组件的比例中位数为93%，并且我们由Syft生成的11个容器镜像SBOM也属于此状态，孤儿比例为95-98%），另外38.3%形成了连接良好的图。边的生成由生成器决定，而非所描述的软件（不同工具的无边率从0%到100%），而用于声明图不完整性的规范级机制（CycloneDX compositions）仅被0.10%的总体使用。我们认为在前两种状态下，常见的消费者推断“无路径意味着不可达”是一个不完整的封闭世界结论，基于明显不完整的工件得出；在生产漏洞优先级排序系统中，将由此产生的否决替换为由退化检测器保护的显式“未知”级别，将KEV召回率从0.600恢复到0.950（控制重新评分；实时端到端运行为0.957），而不会导致警报泛滥。我们发布了我们的流式扫描器和完整的每个SBOM拓扑数据集。
...
### [48] [HarnessLLM: Rust Verification Harness Generation with Large Language Models](https://arxiv.org/abs/2607.22161)
...
### [49] [Integrating Energy Efficiency into Software Development: Developer Perspectives and Requirements](https://arxiv.org/abs/2607.22168)
...
TL;DR: 通过访谈发现，软件开发者很少明确考虑能效，节能主要通过性能优化间接实现；对AI辅助工具的需求包括可操作建议、无缝集成和透明度。


<details>
  <summary>Details</summary>
Motivation: 软件能耗影响被忽视，研究旨在了解开发者对能效的感知及对AI辅助工具的需求与障碍。

Method: 对10名专业软件开发者进行半结构化访谈，采用Mayring的定性内容分析，并部分结合技术接受模型。

Result: 能效在开发中未显性考虑，节能通过性能优化达成；障碍包括意识不足和交付时间压力；需求包括可操作优化建议、工具集成、数据透明度。

Conclusion: 研究提供了开发者视角的节能工具需求，为设计符合实践的AI辅助解决方案提供见解。

中文摘要：在信息和通信技术能源足迹日益增长的背景下，行业优化工作主要集中在硬件上，而软件对能源消耗的影响往往被忽视。尽管研究中已经开发了优化软件能耗的技术方法，但这些方法在日常开发实践中的采用仍然有限。本研究调查了软件开发人员在日常工作中如何感知能效，以及他们对支持节能开发的AI辅助工具提出了哪些需求和障碍。作为欧洲GreenCode项目的一部分，我们按照Mayring的方法进行了定性内容分析，对十名专业软件开发人员进行了半结构化访谈。随后部分通过技术接受模型的视角解读了所确定的需求。结果表明，能效在日常开发活动中很少发挥显性作用。相反，节能通常通过性能优化间接实现。识别出的明确考虑能效的障碍包括意识有限和强烈关注按时交付。访谈进一步揭示了对实用工具支持的需求，例如可操作的优化建议和与常见开发环境的无缝集成。此外，对AI辅助优化工具的接受度很大程度上取决于数据使用的透明度、与实际工具自身能耗相比的实际节能效果以及训练数据的披露。本研究为节能软件开发工具的需求提供了以开发者为中心的视角，并为设计符合实际开发实践的AI辅助解决方案提供了见解。
...
### [50] [Comparing and Conceptualizing Data Protection Requirements Worldwide for Privacy Regulatory Compliance](https://arxiv.org/abs/2607.22270)
...
TL;DR: 本文通过对70位法律专家的访谈和多个国家数据保护法规的分析，识别出跨境数据流动中常见和分歧的监管要求，并将其转化为DPO故事以辅助合规管理。


<details>
  <summary>Details</summary>
Motivation: 随着数据跨境流动增加，企业面临不同司法管辖区的数据保护要求差异，早期识别要求共性与分歧对降低合规成本和返工至关重要。

Method: 采用演绎定性分析对70位法律专家访谈进行编码，并结合系统内容分析G20等经济体的数据保护法规，提炼出通用与分歧要求。

Result: 识别出常见要求（如同意）和分歧要求（如被遗忘权），并生成了按SDLC阶段和企业架构层分类的DPO故事。

Conclusion: 本文为管理跨境数据流动合规提供了概念化要求框架和实用工具（DPO故事），有助于企业在软件开发生命周期早期应对监管差异。

中文摘要：社会的日益数字化加剧了个人数据的收集、处理与共享，这些数据越来越多地跨越国界和监管管辖区，促使全球数据保护框架激增。这些跨境个人数据流动（TPDF）对当今经济至关重要，但管理这些数据的组织必须协调不同司法管辖区（有时存在细微差异）的数据保护要求。对于需求工程而言，这是核心挑战：监管数据保护要求（RDPR）复杂且不能直接转化为软件需求，尤其是当框架施加相似、不完全相同或相互矛盾的义务时。因此，识别哪些要求是共享的、哪些存在分歧对于管理TPDF至关重要，而在软件开发生命周期（SDLC）后期处理这些要求会导致代价高昂的返工，因此早期识别对于合规和利益相关者沟通至关重要。本文从数据保护法律专家的角度识别并概念化了全球常见的RDPR，回答了：（SQ1）哪些要求在不同法规中常见，它们如何被概念化；以及（SQ2）哪些要求存在分歧，它们在概念上有何不同。我们结合了对来自G20经济体及其他国家70位法律专家的访谈的演绎定性分析，以及对这些经济体数据保护法规的系统内容分析。我们识别了常见要求（如同意）和分歧要求（如被遗忘权）。鉴于它们对SDLC和企业架构的影响，我们将这些发现转化为一组数据保护官（DPO）故事，采用用户故事符号，按SDLC阶段和企业架构层分类，以帮助组织管理TPDF合规。
...
### [51] [Vibe Coding: An Experiment with Test-Driven Development](https://arxiv.org/abs/2607.22406)
...
### [52] [MineValiCoder: Reliable Code Generation with Test Case Quality Mining and Bipartite Graph-Based Mutual Validation](https://arxiv.org/abs/2607.22471)
...
### [53] [A Preliminary Search for Evidence on Government Software Engineering Practices: Results from Three Rapid Reviews](https://arxiv.org/abs/2607.22485)
...
TL;DR: 本文通过三次快速综述发现，政府软件工程实践在主流学术文献中证据极少（984篇中仅4篇），呼吁专门渠道和政学合作。


<details>
  <summary>Details</summary>
Motivation: 政府机构是软件开发的重要主体，但其SE实践缺乏经验证据，且从业者认为学术可见度低，因此需要系统评估证据可用性。

Method: 进行了三次快速综述，检索2024年同行评审出版物，涵盖顶级国际SE会议、南美地区SE会议和公共部门数字化通讯，共筛选984篇论文。

Result: 仅发现4项研究涉及政府SE实践，多数为地区性经验报告或案例研究，表明主流学术场合中政府SE证据稀缺。

Conclusion: 需要专门的出版渠道、政学合作激励和灰色文献评估改进，以支持公共部门基于证据的软件工程。

中文摘要：政府机构是主要的软件开发者和数字化转型的推动者，然而关于其软件工程（SE）实践的经验证据在很大程度上仍未得到探索。本文报告了三次快速综述，涉及2024年同行评审的出版物，以评估政府SE实践证据的可用性。我们审查了一部分顶级国际SE会议、南美地区SE会议以及一份关于公共部门数字化的精选通讯。在筛选的984篇论文中，我们仅识别出四项研究报道了由政府机构、为政府机构或与政府机构合作进行的SE实践，其中大多数发表在地区性会议上，作为经验报告或案例研究。我们的发现与从业者的看法一致，即关于政府SE实践的学术证据在主流SE场合中的可见度较低。我们讨论了其影响，包括需要专门的出版渠道、激励政府与学术界的合作，以及改进灰色文献的评估方法，灰色文献目前是政府机构的关键信息来源。这项工作为理解证据格局提供了初步研究，并确定了未来研究的方向，以更好地支持公共部门的基于证据的软件工程。
...
- [Claim Plane: Enforceable Change Intents and Dynamic Scope for Parallel Coding Agents](https://arxiv.org/abs/2607.21909) — 相关度 0.85：面向并行编码agent提出变更意图声明、动态作用域与确定性控制平面，直接解决集成冲突和范围蔓延问题。
- [MineValiCoder: Reliable Code Generation with Test Case Quality Mining and Bipartite Graph-Based Mutual Validation](https://arxiv.org/abs/2607.22471) — 相关度 0.85：提出测试用例质量挖掘与二部图互验证的TDD闭环框架，有效提升代码生成可靠性与选择稳定性。
- [Output Format x Model Identity: Interaction Effects in Single-Round Coding Agent Performance](https://arxiv.org/abs/2607.21674) — 相关度 0.85：揭示输出格式与模型身份的交互效应并对coding agent成功率有显著影响，提出格式化工具设计原则
- [Tool-Guided Retrieval-Augmented Repair for Securing LLM-Generated C Code](https://arxiv.org/abs/2607.21641) — 相关度 0.85：结合编译诊断、CodeQL静态分析和KLEE符号执行与检索修复，显著提升LLM生成C代码的编译通过率和安全性，体现工具反馈价值
- [Cross-Model LLM Code Review: Should you use Claude to review Codex or vice versa?](https://arxiv.org/abs/2607.21656) — 相关度 0.80：跨模型代码审查实验揭示结对不对称收益，对多agent协作和代码审查排布有直接指导
- [Directed Symbolic Execution for Vulnerability Discovery: An LLM-Guided Approach in KLEE](https://arxiv.org/abs/2607.21676) — 相关度 0.80：LLM引导KLEE定向符号执行，结合安全语义标记和循环退出优先，显著提升漏洞发现覆盖率
- [Enhancing SLMs for Sustainable Code Optimization in Radio-Astronomy](https://arxiv.org/abs/2607.21677) — 相关度 0.80：利用多采样SLM与编译器反馈优化射电天文代码，展示低能耗agentic代码优化的可行路径
- [HarnessLLM: Rust Verification Harness Generation with Large Language Models](https://arxiv.org/abs/2607.22161) — 相关度 0.80：利用LLM从测试套件自动生成Rust验证harness并迭代修正，可增强agent的验证与缺陷检测能力。
- [KaPilot: LLM-Assisted Generation of Kani Specifications for Unsafe Rust Verification](https://arxiv.org/abs/2607.21957) — 相关度 0.80：多agent框架自动生成Rust验证规格，含生成-预检-验证循环和候选选择策略，对验证型agent有直接价值。
- [How Do AI Coding Agents Contribute to Software Development? an Empirical Study of Agentic Pull Requests](https://arxiv.org/abs/2607.21832) — 相关度 0.75：实证分析agentic pull request的特性与演化，为评估coding agent真实开发贡献提供证据
- [Leveraging Resolved Incident History for LLM-Assisted Software Bug Diagnosis](https://arxiv.org/abs/2607.21911) — 相关度 0.75：构建基于已解决事故历史的LLM运维助手，结构化检索与MAPE-K闭环可迁移至代码agent的故障诊断。
- ["Go Home Copilot, You're Drunk": Understanding Developer Responses to Agent-Generated Code Review Comments](https://arxiv.org/abs/2607.21997) — 相关度 0.72：大规模实证探究开发者对AI审查评论的响应规律，识别关键影响因素，为改进代码审查agent提供依据。
- [Code Review is a Conversation: Toward Conversational AI Review Assistants](https://arxiv.org/abs/2607.22095) — 相关度 0.70：提出对话式AI审查助手范式，把审查从一次性评论转向人机交互，为审查agent设计提供新方向。
- [Pixels for Programs? A Cross-Provider Case Study of Input-Token Accounting for Source Code as Text and Images](https://arxiv.org/abs/2607.21672) — 相关度 0.70：度量代码文本/图像表示的token核算差异，为coding agent输入选择提供实证成本数据
- [Vibe Coding: An Experiment with Test-Driven Development](https://arxiv.org/abs/2607.22406) — 相关度 0.70：实验对比人类与CLLM的多种协作模式及TDD流程，对代码agent的交互与工作流设计有参考价值。
- [Are Production Cloud Skills Adequately Tested? Measuring and Governing Skill Test Coverage in Practice](https://arxiv.org/abs/2607.22015) — 相关度 0.65：提出Agent所用云技能包的测试覆盖度度量与改进推荐，可迁移至agent工具评估与测试优化。
- [Vibe Coding in Software Development: A Multivocal Literature Review](https://arxiv.org/abs/2607.21652) — 相关度 0.60：vibe coding多声文献综述，刻画生成-评估-修订工作流及开发者角色转变，可参考理解agent实践
