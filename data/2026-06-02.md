...
- [FLARE: Fine-Grained Diagnostic Feedback for LLM Code Refinement](http://arxiv.org/abs/2606.03852v1) — 相关度 0.95：提供轻量诊断模型预测行级可疑位置，用于LLM代码精修的细粒度反馈，直接改进代码生成与修复智能体。
- [Lean4Agent: Formal Modeling and Verification for Agent Workflow and Trajectory](http://arxiv.org/abs/2606.06523v2) — 相关度 0.95：用Lean4形式化建模与验证智能体工作流和轨迹，验证通过的工作流显著提升SWE效果，直接增强智能体可靠性与形式化方法应用。
- [SPOQ: Specialist Orchestrated Queuing for Multi-Agent Software Engineering](http://arxiv.org/abs/2606.03115v1) — 相关度 0.90：提出SPOQ多agent编排方法，包含波次调度、双重验证门与人类参与，经多实验验证，对多agent软件工程有直接价值。
- [Token Budgets: An Empirical Catalog of 63 LLM-Agent Budget-Overrun Incidents, with an Affine-Typed Rust Mitigation as a Case Study](http://arxiv.org/abs/2606.04056v1) — 相关度 0.90：提供63例LLM智能体预算超支事故分类学，并用affine类型系统在编译期阻止预算违规，直接提升多智能体可靠性。
- [Proof-Carrying Agent Actions: Model-Agnostic Runtime Governance for Heterogeneous Agent Systems](http://arxiv.org/abs/2606.04104v1) — 相关度 0.85：提出基于动作证书的运行时无关治理模型与五检查点机制，针对异构智能体系统的高风险动作授权与事后证明，强化智能体可靠性。
- [The Invisible Lottery: How Subtle Cues Steer Algorithm Choice in LLM Code Generation](http://arxiv.org/abs/2606.04057v1) — 相关度 0.85：大规模实验揭示提示词细微线索会系统性偏置LLM代码生成的算法选择，对代码智能体评估与可控性有重要价值。
- [Toward Pre-Deployment Assurance for Enterprise AI Agents: Ontology-Grounded Simulation and Trust Certification](http://arxiv.org/abs/2606.04037v2) — 相关度 0.85：提出本体驱动的企业AI agent预部署验证与信任认证框架，包含自动化场景生成与评估，对agent认证与评测有参考。
- [DDOR: Delta Debugging for Explainable Overrefusal Testing and Repair](http://arxiv.org/abs/2606.03601v1) — 相关度 0.80：用delta debugging定位最小拒答片段并生成测试套件与提示修复，为LLM智能体的过度拒答测试与修复提供可迁移方法。
- [The Impact of Configuring Agentic AI Coding Tools on Build-vs-Buy Decisions: A Study Protocol](http://arxiv.org/abs/2606.03907v1) — 相关度 0.75：预注册实验方案研究配置机制对Agentic编码工具构建/购买决策的影响，并计划发布基准，紧扣编码智能体行为与评测。
- [Where Do Large Language Models Fail on Competitive Programming? A Taxonomy of Failures by Algorithm Type and Difficulty Rating](http://arxiv.org/abs/2606.05228v1) — 相关度 0.75：对Codeforces问题按算法类别与难度建立LLM失败分类学，并对比CoT消融，为代码生成智能体评测与提示策略提供实证依据。
- [Neural Change Prediction: Relating Software Changes to Their Effects and Vice Versa](http://arxiv.org/abs/2606.03378v1) — 相关度 0.72：通过大量突变与执行学习代码变更与行为间关系，可支撑自动修复、变更定位与效果预测，对代码分析与智能体有迁移价值。
- [SkillGuard: A Permission-Centric Framework for Agent Skill Security](http://arxiv.org/abs/2606.03024v2) — 相关度 0.70：提出基于权限的agent技能安全框架，将技能视为权限工件，可迁移到编码agent的工具与技能管理。
- [HighTide: An Agent-Curated Open-Source VLSI Benchmark Suite](http://arxiv.org/abs/2606.04126v1) — 相关度 0.65：发布智能体策展的开源VLSI基准，含12个智能体技能与决策日志长期记忆，对工程化智能体与基准构建有参考价值。
- [Agentic Generation and Evolution of Knowledge Models](http://arxiv.org/abs/2606.03662v2) — 相关度 0.60：提出智能体化知识模型生成与演化的TrustModel框架，可支撑基于模型的测试与持续演化，对工程化智能体有参考价值。
- [Making Embodied AI Reliable: A Community Agenda from Testing to Formal Verification](http://arxiv.org/abs/2606.03593v1) — 相关度 0.55：提出面向AI系统的测试、形式化验证与运行时保障的一体化可靠性工作流，其验证与反馈机制可迁移至代码智能体。
