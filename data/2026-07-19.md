...
Comments: 13 pages, 1 figure, preprint under review
...
- [Stop Means Stop: Measuring and Repairing the Enforcement Gap in Agent-Framework Control Primitives](https://arxiv.org/abs/2607.14166) — 相关度 0.95：系统测量并修复agent框架控制原语的安全漏洞，对agent可靠性、验证和工具设计至关重要。
- [Structured Feedback Improves Repair in an LLM Agent Loop](https://arxiv.org/abs/2607.14167) — 相关度 0.90：验证结构化反馈对LLM agent修复循环的提升，直接作用于代码agent的验证-修复流程。
- [The Prover Is the Judge: Verified Security Software from AI Coding Agents in Ada/SPARK](https://arxiv.org/abs/2607.14340) — 相关度 0.90：展示prover验证循环驱动AI coding agent生成可验证安全代码，强调反馈强度上限，对代码agent验证极具参考。
- [Copy-on-Write Scoring: Application-Specific Agent Evaluations](https://arxiv.org/abs/2607.14336) — 相关度 0.85：提出应用内评估agent操作的新型框架（CoW Scoring），可低成本定位agent工具层问题。
- [Falsifiable Release Gates for Self-Improving Systems](https://arxiv.org/abs/2607.13070) — 相关度 0.85：提出自改进 agent 运行时的可证伪发布门控与机器可验证验收套件，对构建安全的自主 coding agent 有直接借鉴价值。
- [MCPEvol-Bench: Benchmarking LLM Agent Performance Across Dynamic Evolutions of MCP Servers](https://arxiv.org/abs/2607.14642) — 相关度 0.85：针对MCP服务器动态演化的agent benchmark，提出11种变异算子，评测工具演化下的适应性，对代码agent有直接参考。
- [MM-IssueLoc: A Controlled Benchmark for Evaluating Visual Evidence in Multimodal Repository-Level Issue Localization](https://arxiv.org/abs/2607.15205) — 相关度 0.85：多模态仓库级issue定位benchmark，评估视觉证据对agent的影响，直接相关代码agent评测。
- [Proof-or-Stop: Don't Trust the Agent, Trust the Evidence -- Loop Engineering for Verifiable Evidence-Gated Lifecycle Control](https://arxiv.org/abs/2607.14890) — 相关度 0.85：证据门控生命周期控制方法，防止agent虚假完成，机制可迁移到代码agent的可靠性与验证。
- [Why Git Is the Memory Solution for the Agentic Development Lifecycle](https://arxiv.org/abs/2607.14390) — 相关度 0.85：提出git绑定的agent开发记忆方案，解决agent长期记忆与溯源问题，机制可迁移。
- [Alipay-PIBench: A Realistic Payment Integration Benchmark for Coding Agents](https://arxiv.org/abs/2607.14573) — 相关度 0.80：支付集成场景的coding agent benchmark，含结构化技能评估和确定性rubric，可迁移到其他仓库级任务。
- [Beyond Generalist LLMs: Specialist Agentic Systems for Structured Code Workflow Execution](https://arxiv.org/abs/2607.14456) — 相关度 0.80：对比专用与通用agent在结构化代码工作流中的表现，展示专用化收益，对构建工程agent有参考。
- [NexForge: Scaling Executable Agent Tasks via Requirement-First Synthesis](https://arxiv.org/abs/2607.14186) — 相关度 0.80：需求优先合成可执行agent任务数据，为代码agent训练与评测提供可扩展方法。
- [Beyond Object Validation: Relational Conformance in Multi-Artifact Agent Releases](https://arxiv.org/abs/2607.14155) — 相关度 0.78：面向多工件 agent 发布的关系一致性校验，以发布图为 agent 输出验证提供可迁移机制。
- [Large Language Models for Code Generation from Multilingual Prompts: A Curated Benchmark and a Study on Code Quality](https://arxiv.org/abs/2607.14816) — 相关度 0.75：构建多语言代码生成benchmark并研究代码质量，为评估跨语言coding agent提供数据集与方法。
- [StructureClaw: Traceable LLM Agents and an Executable Benchmark for Structural Engineering Workflows](https://arxiv.org/abs/2607.14896) — 相关度 0.75：提出artifact-centered workbench和可执行benchmark，评测结构工程agent的完整工作流，评测方法可迁移到代码agent。
- [ToolAlignBench: Investigating Alignment Conflicts in Tool-Calling Enabled LLMs](https://arxiv.org/abs/2607.14285) — 相关度 0.75：构建工具调用LLM安全对齐冲突基准，揭示agent遵循指令中的风险，对agent评测有迁移价值。
- [FirmPilot: Evidence-Guided Multi-Agent Environment Recovery for IoT Firmware Rehosting](https://arxiv.org/abs/2607.14903) — 相关度 0.72：证据引导的多agent框架用于环境恢复，协作和反馈机制可迁移到代码agent的自动修复与验证。
- [Natural-Language to SysMLv2 Translation via Conformance-Driven Iterative Refinement](https://arxiv.org/abs/2607.14162) — 相关度 0.72：用生产级 conformance checker 驱动 generate-check-repair 循环，机制可迁移到代码生成 agent 的验证与修复。
- [Quantize with Confidence? An Empirical Study of Quantization for Code Generation](https://arxiv.org/abs/2607.14181) — 相关度 0.60：实证评估量化策略对代码生成模型的影响，为资源受限环境下部署代码agent提供参考。
- [Rethinking Issue Resolution for AI/ML Systems](https://arxiv.org/abs/2607.14657) — 相关度 0.60：定性分析AI/ML系统问题解决工作流，为构建维护AI/ML代码库的agent提供需求洞察。
- [Leaf: An Instrumentation-based Dynamic Analysis Framework for Rust](https://arxiv.org/abs/2607.15025) — 相关度 0.55：Rust动态分析框架，提供事件驱动接口，可作为代码agent的测试和调试基础设施参考。
