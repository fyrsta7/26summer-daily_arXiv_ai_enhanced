...
- [QuoteBench: How Matched Scores Can Hide Command-Path Failures](https://arxiv.org/abs/2608.13547) — 相关度 0.90：针对LLM coding agent的Bash命令生成契约与执行路径边界提出评估基准，直接揭示matched score的误导性，对agent评测有方法贡献。
- [Specification-first convergence with an AI coding agent: a case study of dismantling a core architectural invariant across 189 files in a 717k-line codebase with no test oracle and no human code review](https://arxiv.org/abs/2608.12440) — 相关度 0.90：展示规范优先协议下AI coding agent完成大规模架构重构的完整案例，含审计循环和收敛判据，对agent长程执行有直接参考。
- [CAPRI: Contract-Aware Proof Repair for Isabelle](https://arxiv.org/abs/2608.13459) — 相关度 0.75：提出契约感知的LLM形式化证明修复工作流，用独立检查器强制执行编辑契约，可迁移到agent安全修改代码的场景。
- [SynWeaver: Website-Prior Task and Trajectory Co-Synthesis for Web Agents](https://arxiv.org/abs/2608.12429) — 相关度 0.75：提出网站先验的任务与轨迹协同合成框架，其数据合成和验证修复思路可迁移到coding agent的训练数据生成。
- [Memorization Diagnostics for Code LLMs Should be Scale-Aware](https://arxiv.org/abs/2608.12771) — 相关度 0.70：揭示代码LLM记忆探测方法在规模增大时的失效，提出分离表示负载与记忆的评估思路，对code agent评测设计有借鉴。
- [Integration-First Structural Coverage for Embedded Software:Trace-Based Evidence, Hybrid Runtime Analysis, and Cross-Variant Consolidation](https://arxiv.org/abs/2608.13322) — 相关度 0.60：提出面向嵌入式软件的结构覆盖率方法，结合trace与混合运行时分析，对Agent测试验证中的覆盖率评估有参考价值。
