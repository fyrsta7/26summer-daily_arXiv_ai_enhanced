...
- [Multi-level Code Optimization via Mixture of Prompts](https://arxiv.org/abs/2607.23665) — 相关度 0.95：直接面向LLM多级代码优化，提出Mixture-of-Prompts方法，与SemOpt核心高度相关。
- [CRAFT: Learn the Schema, Execute the Plan](https://arxiv.org/abs/2607.22642) — 相关度 0.90：企业级编码agent的两阶段后训练方法，涵盖规划、工具选择和失败恢复，直接提升多轮可靠性与token效率。
- [Execution-Grounded Security Testing for Coding Agents in Software Engineering Pipelines](https://arxiv.org/abs/2607.22569) — 相关度 0.90：提出执行层安全测试框架，针对coding agent在软件工程流程中的不安全行为，对agent安全验证重要。
- [TLA$^{+}$-Bench: An Execution-Grounded Benchmark and Dataset for Natural-Language to TLA+ Specification Generation](https://arxiv.org/abs/2607.23425) — 相关度 0.90：提供执行语义验证的TLA+规格生成基准，对评估agent的规范生成正确性极具价值。
- [Adversarial Test-Hardening for AI-Written Code: An Instrument Autopsy and a Pre-Registered Causal Estimate of the Critic Loop](https://arxiv.org/abs/2607.23002) — 相关度 0.85：提出对抗性测试硬化循环与变异测试，用于AI生成代码的验证，对coding agent的测试与评测有直接价值。
- [AssumptionMiner: Extracting, Tracing, and Revising Implicit Assumptions in LLM Code Generation](https://arxiv.org/abs/2607.22898) — 相关度 0.85：将隐式假设作为一等公民，结合AST依赖图进行假设提取与定向再生成，提升LLM代码生成的可控性。
- [Where Is the Cost of Third-Party API Routers in Agentic Software Development?](https://arxiv.org/abs/2607.23624) — 相关度 0.85：首次实证研究第三方API路由器对coding agent的注入攻击，并提出防御评估框架，对agent安全与可靠性关键。
- [The Best Programming Language for Tokenmaxxing: An Investigation of Coding Agent Behavior Across Programming Languages](https://arxiv.org/abs/2607.22807) — 相关度 0.80：实证分析不同编程语言下编码agent的token消耗与轨迹行为，为多语言agent成本与效率优化提供依据。
- [Towards LLM-assisted High-Quality Property Generation for Solidity Smart Contracts](https://arxiv.org/abs/2607.23308) — 相关度 0.80：利用LLM生成Solidity智能合约的高质量属性，并用变异测试评估，对自动化测试生成有直接借鉴。
- [Evaluating and Mitigating the Misguidance Effect of Buggy Code in LLM-Generated Unit Tests](https://arxiv.org/abs/2607.22883) — 相关度 0.78：量化buggy代码对LLM测试生成的误导效应，并提出基于规格的生成范式，减少误导并提高测试有效性。
- [Industrial Practice of LLM-Based Test Case Carving and Assertion Generation (Experience Paper)](https://arxiv.org/abs/2607.24000) — 相关度 0.75：工业级LLM测试用例雕刻与断言生成工具，对回归测试自动化有参考价值。
- [RESTOR: Automated Test Oracle Generation for RESTful APIs via Reinforcement Learning](https://arxiv.org/abs/2607.23963) — 相关度 0.75：使用强化学习与LLM生成REST API测试断言，对自动化测试oracle生成有直接借鉴。
- [StateAct: Program State, before Pixels, for Long-Horizon Computer-Use Agents](https://arxiv.org/abs/2607.22798) — 相关度 0.75：提出基于程序状态而非像素的代码优先多agent框架，验证与记忆均绑定状态，可迁移至长程编码agent。
- [Do Coverage and Mutation Scores of LLM-Generated Test Suites Correlate with Their Effectiveness? (Replicability Study)](https://arxiv.org/abs/2607.22880) — 相关度 0.70：大规模复制研究LLM生成测试的覆盖率/变异分数与真实缺陷检测的相关性，指导测试评估方法选择。
- [Bifrost: Empowering Pretrained Language Model with Fallibility Representation for Log-Based Fault Diagnosis](https://arxiv.org/abs/2607.23169) — 相关度 0.65：为日志故障诊断提出可迁移的表示学习方法，对代码调试和程序分析有参考价值。
- [Enhancing Code Understanding for Impact Analysis by Combining Transformers and Program Dependence Graphs](https://arxiv.org/abs/2607.23355) — 相关度 0.65：结合Transformer与程序依赖图进行影响分析，并提出新基准，可迁移到代码理解与agent上下文化。
- [LinkRank: A Learning-to-Rank Framework for One-to-Many Issue-Commit Traceability](https://arxiv.org/abs/2607.23610) — 相关度 0.60：提出一对多issue-commit追溯的学习排序框架与数据集，对agent的项目理解与追踪有参考价值。
- [Semantic-Enhanced Automatic Refinement of Architecture Recovery Results Using LLMs](https://arxiv.org/abs/2607.23774) — 相关度 0.60：用LLM结合依赖分析自动精化架构恢复结果，对agent的代码架构理解有一定迁移价值。
