...
- [Precise Debugging Benchmark: Is Your Model Debugging or Regenerating?](http://arxiv.org/abs/2604.17338v4) — 相关度 0.90：提出精确调试基准PDB与编辑级精度/缺陷级召回指标，直接评估agent是否真正调试而非重生成。
- [Project Prometheus: Bridging the Intent Gap in Agentic Program Repair via Reverse-Engineered Executable Specifications](http://arxiv.org/abs/2604.17464v1) — 相关度 0.90：反向工程可执行规格桥接意图鸿沟，多agent RQA循环显著提升程序修复准确率，直接关联coding agent。
- [Single-Language Evidence Is Insufficient for Automated Logging: A Multilingual Benchmark and Empirical Study with LLMs](http://arxiv.org/abs/2604.17529v1) — 相关度 0.85：多语言日志生成基准与LLM实证，直接服务软件工程评测与代码生成任务。
- [SynthFix: Adaptive Neuro-Symbolic Code Vulnerability Repair](http://arxiv.org/abs/2604.17184v2) — 相关度 0.85：结合编译器反馈的神经符号漏洞修复，训练与推理中的符号证据设计直接适用于代码修复agent。
- [AIRA: AI-Induced Risk Audit: A Structured Inspection Framework for AI-Generated Code](http://arxiv.org/abs/2604.17587v1) — 相关度 0.80：提出AI生成代码的风险审计框架，有助于评估agent产出的安全性与失败模式。
- [Augmenting unit test suites from integration tests](http://arxiv.org/abs/2604.17508v2) — 相关度 0.75：从集成测试自动生成单元测试的方法，对测试生成和agent验证有迁移价值。
- [From Language to Action: Enhancing LLM Task Efficiency with Task-Aware MCP Server Recommendation](http://arxiv.org/abs/2604.17234v1) — 相关度 0.70：任务感知的MCP服务器推荐，为代码agent的工具检索与集成提供结构化方法和数据集。
- [Layer-wise MoE Routing Locality under Shared-Prefix Code Generation: Token-Identity Decomposition and Compile-Equivalent Fork Redundancy](http://arxiv.org/abs/2604.17182v1) — 相关度 0.70：分析共享前缀代码生成中MoE路由的局部性，发现编译等价冗余，可提升多候选代码生成与搜索效率。
- [KnowPilot: Your Knowledge-Driven Copilot for Domain Tasks](http://arxiv.org/abs/2604.19820v1) — 相关度 0.65：领域知识增强的生成式agent框架，知识检索与经验记忆机制可迁移到代码agent的领域适应。
- [A Pilot Study on Detecting Software Design Patterns with Large Language Models: An Empirical Evaluation](http://arxiv.org/abs/2604.17329v1) — 相关度 0.60：比较LLM与集成方法检测设计模式，其多模态代码提示策略可为代码理解与分析agent提供参考。
- [React-ing to Grace Hopper 200: Five Open-Weights Coding Models, One React Native App, One GH200, One Weekend](http://arxiv.org/abs/2604.17187v1) — 相关度 0.60：在单一真实App生成任务上对比五个编码模型，揭示SWE-Bench排名不预示任务性能及部署实践发现。
