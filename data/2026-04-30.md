...
- [Claw-Eval-Live: A Live Agent Benchmark for Evolving Real-World Workflows](http://arxiv.org/abs/2604.28139v2) — 相关度 0.92：活体工作流Agent基准，含可刷新信号层、执行痕迹审计与确定性/LLM混合评测，直接适用于代码Agent评测。
- [ScaleBox: Enabling High-Fidelity and Scalable Code Verification for Large Language Models](http://arxiv.org/abs/2604.27467v1) — 相关度 0.90：提供高保真、可扩展的代码沙箱系统，支持LLM代码验证与RLVR训练，直接服务代码agent基础设施。
- [To Diff or Not to Diff? Structure-Aware and Adaptive Output Formats for Efficient LLM-based Code Editing](http://arxiv.org/abs/2604.27296v1) — 相关度 0.85：提出结构感知 diff 格式与自适应输出策略，在保持编辑准确率同时降低延迟成本，直接优化代码编辑 agent
- [From Mirage to Grounding: Towards Reliable Multimodal Circuit-to-Verilog Code Generation](http://arxiv.org/abs/2604.27969v2) — 相关度 0.80：发现多模态代码生成中的Mirage现象并提出VeriGround模型，解决视觉-代码生成可靠性问题，与代码agent可靠性直接相关。
- [LLM-as-a-Judge for Human-AI Co-Creation: A Reliability-Aware Evaluation Framework for Coding](http://arxiv.org/abs/2604.27727v1) — 相关度 0.80：提出可靠性感知的LLM-as-a-Judge评估框架，覆盖多指标与轨迹分析，为代码agent评估提供完整模板。
- [REBENCH: A Procedural, Fair-by-Construction Benchmark for LLMs on Stripped-Binary Types and Names (Extended Version)](http://arxiv.org/abs/2604.27319v1) — 相关度 0.80：构建LLM二进制逆向工程评估基准，涵盖类型与函数名恢复等任务，对代码分析agent评测有直接迁移价值。
- [How Code Representation Shapes False-Positive Dynamics in Cross-Language LLM Vulnerability Detection](http://arxiv.org/abs/2604.27714v1) — 相关度 0.70：揭示代码表示形式对跨语言LLM漏洞检测假阳性的影响，提出表示一致性门控，可用于安全agent设计。
- [Test Before You Deploy: Governing Updates in the LLM Supply Chain](http://arxiv.org/abs/2604.27789v1) — 相关度 0.70：针对LLM服务静默更新提出部署侧兼容性治理框架，含契约、风险测试与门控，对agent可靠运维有参考价值。
- [TokenScope: Token-Level Explainability and Interpretability for Code-Oriented Tasks in Large Language Models](http://arxiv.org/abs/2607.01235v1) — 相关度 0.70：提供token级可解释性工具，支持交互式替换与AST聚合，帮助理解与调试代码生成agent行为。
- [Hierarchical Fault Detection and Diagnosis for Transformer Architectures](http://arxiv.org/abs/2604.28118v2) — 相关度 0.60：提出Transformer分层故障诊断方法与benchmark，可辅助调试LLM基座驱动的代码Agent可靠性问题。
- [Tail-aware N-version Machine Learning Models for Reliable API Recommendation](http://arxiv.org/abs/2604.27647v1) — 相关度 0.60：通过N-version模型与多数投票提升API推荐的可靠性，可迁移到代码生成agent的可靠性增强。
- [Understanding Bugs in Template Engine-Based Applications: Symptoms, Root Causes, and Fix Patterns](http://arxiv.org/abs/2604.27692v1) — 相关度 0.60：系统分析模板引擎应用缺陷的症状、根因与修复模式，为调试agent提供可操作的Bug模式知识。
- [DEPTEX: Organization-First, Open Source Dependency Risk Monitoring](http://arxiv.org/abs/2605.00179v1) — 相关度 0.55：结合CPG切片与LLM语义验证评估漏洞影响，可迁移至代码Agent的供应链安全分析。
