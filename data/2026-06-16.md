...
- [Exploring Statistical Change Point Detection Techniques for Performance Anomaly Detection at Mozilla](http://arxiv.org/abs/2606.18377v1) — 相关度 0.90：系统评估 25 种变化点检测方法用于性能异常检测，并构建标注基准，直接服务性能回归检测与 benchmark。
- [PracRepair: LLM-Empowered Automated Program Repair Inspired by Human-Like Debugging Practices](http://arxiv.org/abs/2606.17612v1) — 相关度 0.90：提出LLM启发的人类式调试APR框架，在Defects4J等基准上显著提升修复效果，直接相关。
- [A Framework for Evaluating Agentic Skills at Scale](http://arxiv.org/abs/2606.17819v1) — 相关度 0.80：提出大规模评估agent skill的框架与数据集，直接适用于coding agent的技能评估。
- [All Smoke, No Alarm: Oracle Signals in Agent-Authored Test Code](http://arxiv.org/abs/2606.18168v1) — 相关度 0.80：研究 agent 生成测试代码中的 oracle 信号质量，对评估 coding agent 测试能力的评测方法有直接迁移价值。
- [Beyond the GUI Paradigm: Do Mobile Agents Need the Phone Screen?](http://arxiv.org/abs/2606.19388v1) — 相关度 0.80：评估编码agent在移动CLI任务上的表现并引入基准套件，直接涉及coding agent评测。
- [Position: Coding Benchmarks Are Misaligned with Agentic Software Engineering](http://arxiv.org/abs/2606.17799v1) — 相关度 0.80：立场论文指出编码基准与agentic SE错位，对agent评测设计有重要启示。
- [SoK: AI-Augmented Binary Reversing](http://arxiv.org/abs/2606.17398v1) — 相关度 0.80：系统化综述AI增强的二进制逆向，涵盖LLM和agentic AI，对程序分析类agent的评测与方法有参考价值。
- [Unlocking LLM Code Correction with Iterative Feedback Loops](http://arxiv.org/abs/2606.17514v1) — 相关度 0.80：系统研究LLM通过执行反馈迭代修正代码的能力，直接关注coding agent的反馈循环与评测。
- [CEO-Bench: Can Agents Play the Long Game?](http://arxiv.org/abs/2606.18543v1) — 相关度 0.75：CEO-Bench 模拟 500 天长程运营任务，评价 agent 的长期规划与适应能力，可迁移到长程 coding agent 评测。
- [Finding Compiler-Platform Interaction Bugs in Deep Learning Pipelines via Cross-Layer Constraints](http://arxiv.org/abs/2606.18421v1) — 相关度 0.75：面向深度学习编译器的跨层约束测试框架，可迁移到编译器/运行时缺陷检测与验证。
- [From Specification to Execution: AI Assisted Scientific Workflow Management](http://arxiv.org/abs/2606.18425v1) — 相关度 0.70：用 LLM 生成科学工作流并引入调试 agent 与 MCP 集成，对长程 agent 工作流设计有参考价值。
- [Interpretable and Verifiable Hardware Generation with LLM-Driven Stepwise Refinement](http://arxiv.org/abs/2606.19387v2) — 相关度 0.70：LLM驱动逐步精化生成可验证硬件，方法论可迁移到代码生成与形式化验证结合的场景。
- [Planning to Hammer: Difficulty-Aware Decomposition for Automating Rocq Proofs](http://arxiv.org/abs/2606.17981v1) — 相关度 0.70：将LLM规划与CoqHammer结合实现自动化定理证明，对代码正确性验证的agent化有参考价值。
- [When the Next Step Is Not One Step: Distribution-Aware Execution Modeling for Concurrent Go Programs](http://arxiv.org/abs/2606.17508v1) — 相关度 0.70：针对并发Go程序建模不确定性执行，以分布训练提升预测并发布数据集，对程序分析与调试有迁移价值。
- [Trustworthy Self-Composable Big-Data-as-a-Service: An LLM-Orchestrated Multi-Agent Framework for Automated Data Engineering, AutoML, MLOps Deployment, and Drift-Aware Lifecycle Optimization](http://arxiv.org/abs/2606.17915v1) — 相关度 0.60：LLM编排多agent的BDaaS生命周期框架，其编排、治理与人工介入机制可迁移到工程化agent。
