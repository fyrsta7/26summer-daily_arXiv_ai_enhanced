...
- [LoopsBench: From Harness Engineering to Loop Engineering in Benchmarking Coding Agent](https://arxiv.org/abs/2608.00267) — 相关度 0.95：提出面向长时程代码代理的LOOPSBENCH基准，强调依赖DAG与持续回归，直接服务代码代理评测。
- [Turning Interaction History into Execution State: A Runtime Layer for Long-Horizon Coding Agents](https://arxiv.org/abs/2608.00808) — 相关度 0.95：为长程编码agent设计确定性运行时层Ledger，显式跟踪执行状态，在SWE-bench上显著提升Pass@1并降低成本，直接相关。
- [Vul4Py: Benchmarking Automated Vulnerability Repair in Python with Paired Exploit and Functional Oracles](https://arxiv.org/abs/2608.00692) — 相关度 0.95：提出Python漏洞修复基准Vul4Py，用配对oracle验证自动修复，并对比专用工具、LLM和SE agent，与代码修复agent高度相关。
- [Coding Agents as Test-Suite Auditors: Finding What Official Suites Miss While Approaching What They Catch](https://arxiv.org/abs/2608.01715) — 相关度 0.90：编码智能体作为测试套件审计者，构建对抗性测试并验证漏洞，直接相关。
- [Less Is More: Tuning Configurable Systems with Imperfect Fidelity](https://arxiv.org/abs/2608.00759) — 相关度 0.90：研究不完美保真度下的配置调优，提出MFTune，直接面向系统性能优化与预算效率，属SemOpt核心主题。
- [SWE-Touch: Benchmarking Coding Agents When Users Touch the Code](https://arxiv.org/abs/2608.02499) — 相关度 0.90：SWE-Touch 基准模拟用户修改代码，测试 agent 状态感知，直接相关。
- [VulnGym: Benchmarking Coding Agents for Repository-Level Vulnerability Detection](https://arxiv.org/abs/2608.02001) — 相关度 0.90：提出 VulnGym 基准评估仓库级漏洞检测 coding agents，直接相关。
- [Effective and Efficient Context Retrieval via Partial Dependency Graph for Repository-Level Code Generation](https://arxiv.org/abs/2608.01927) — 相关度 0.88：基于部分依赖图的仓库级代码生成上下文检索，显著提升 Pass@1，直接相关。
- [Real-Time Detection and Repair of LLM Agent Failures](https://arxiv.org/abs/2608.02464) — 相关度 0.88：实时检测和修复 LLM agent 故障，采用遥测与确定性验证，直接相关。
- [Cross-Benchmark Generalization in Long-Horizon Agents](https://arxiv.org/abs/2608.00181) — 相关度 0.85：长时程MCP代理的后训练行为可跨benchmark迁移并提升SWE-bench Pro，与代码代理泛化密切相关。
- [Deep Agentic Search for Repository-Level Code Question Answering: An Empirical Study](https://arxiv.org/abs/2608.01507) — 相关度 0.85：实证对比语义检索与深度agentic检索在仓库级代码问答中的效果，分析上下文污染与子agent交接失败，直接关系代码agent设计。
- [RefactorAssist: Agentic Refinement for Reliable Code Refactoring](https://arxiv.org/abs/2608.00924) — 相关度 0.85：提出RefactorAssist agent，利用测试日志和上下文检索迭代修复LLM重构错误，是测试引导的agent修复典型工作。
- [Unreliable in Practice? A Comprehensive Study of Errors in LLM-Generated Code](https://arxiv.org/abs/2608.00661) — 相关度 0.85：系统研究LLM生成代码的编译/运行时错误模式，对代码agent的可靠性评估与修复策略有直接参考价值。
- [CASPER-Change-Aware Slice Prioritization for Efficient Regression Testing of LLM-based systems](https://arxiv.org/abs/2608.00378) — 相关度 0.80：面向LLM系统的变更感知切片回归测试方法，可直接应用于代码代理版本回归评测。
- [Claim Plane: Reliability Gains and the Limits of Selective Concurrency for Parallel Coding Agents: A 30-Pair, Three-Seed Confirmatory Study of Deterministic Pre-Write Admission](https://arxiv.org/abs/2608.00947) — 相关度 0.80：研究并行编码agent的确定性预写准入机制，提升集成可靠性，涉及多agent协调与可靠性方法。
- [Don't Offer What Can't Be Done: Deterministic Executability Gating for LLM Skill Selection at Scale](https://arxiv.org/abs/2608.01050) — 相关度 0.76：确定性可执行性门控用于 LLM 技能选择，可迁移到工具反馈过滤。
- [Distilling Reasoning Traces into Advisory Prompts for Software Engineering Tasks](https://arxiv.org/abs/2608.00437) — 相关度 0.75：将推理轨迹蒸馏为建议性提示以减少编程任务中的LLM错误，对代码代理提示优化有参考价值。
- [Doc2CI: A Multi-Service Study of CI Configuration Generation Using Large Language Models](https://arxiv.org/abs/2608.01451) — 相关度 0.75：大规模研究LLM生成CI配置，提出Doc2CI基准与schema修复方法，涉及配置生成与验证，对SE agent有参考价值。
- [MADE: Belief-Driven Dual-Agent Coordination for Autonomous Model Deployment](https://arxiv.org/abs/2608.01189) — 相关度 0.75：提出MADE双agent协调系统完成模型自动部署，含M2ABench基准，是软件工程agent在部署任务上的应用。
- [TELLER: Non-intrusive Cross-Layer Root-Cause Analysis for LLM Inference](https://arxiv.org/abs/2608.01975) — 相关度 0.75：LLM 推理的跨层根因分析框架，方法可迁移到代码 agent 的调试与 profiling。
- [What Could the Agent See at 19:05? Generating Temporal Enterprise Scenarios from Real Research and Replaying Them to Evaluate Agents](https://arxiv.org/abs/2608.01042) — 相关度 0.75：生成时间演进的企业场景用于评估agent，解决多快照评估成本问题，对agent评测环境构建有借鉴价值。
- [Trajectories That Segment Themselves: Agent-Declared Boundaries as a Training Unit](https://arxiv.org/abs/2608.02302) — 相关度 0.72：智能体自声明轨迹分段，为长程训练数据提供新单元，可迁移到代码 agent。
- [Documentation vs. Code Patterns: What Drives LLM-Based Exception Oracle Generation?](https://arxiv.org/abs/2608.00884) — 相关度 0.70：干预式研究LLM异常oracle生成依赖的线索，揭示捷径信号问题，对测试agent的鲁棒性评估具有方法论启发。
- [EduPluginBench: Executable Assurance for AI-Generated Educational Plugins](https://arxiv.org/abs/2608.00739) — 相关度 0.70：提出生成式插件可执行保障基准EduPluginBench，关注生成代码的权限、生命周期等非功能契约，对生成代码验证有方法论价值。
- [Post-Training on Office Work Improves Software Engineering: A Behavioral Account of Cross-Domain Transfer](https://arxiv.org/abs/2608.01604) — 相关度 0.70：办公任务后训练提升软件工程表现，证明长程执行行为跨域迁移。
- [From AI Technical Debt to Agentic Technical Debt: A Systematic Mapping of Root Causes and Manifestations in Agentic AI Systems](https://arxiv.org/abs/2608.01001) — 相关度 0.68：系统映射 agentic 技术债务，对 agent 系统可靠性和多 agent 协作有参考价值。
- [AiFlow: Token-Native Reactive Orchestration with Bounded Backpressure for Streaming LLM Applications](https://arxiv.org/abs/2608.00558) — 相关度 0.60：提出token原生响应式编排与有界背压机制，可迁移到代码代理的工具调用与多代理工作流可靠性设计。
- [Pretraining on Call Graphs: When Binary Analysis Tasks Profit From Context](https://arxiv.org/abs/2608.02084) — 相关度 0.60：调用图预训练改善二进制函数嵌入，对代码检索和分析有迁移价值。
- [RAG-TESTER: Automated End-to-End Testing of Retrieval-Augmented Large Language Models](https://arxiv.org/abs/2608.00054) — 相关度 0.60：面向RAG系统的自动化端到端测试方法，其覆盖导向的测试生成可迁移到检索增强的代码agent评测。
- [Structure-Aware Semantic Chunking with Title-Chain Prefixes: A 1600-Query Evaluation and the Measurement Trap in Text-Transform Ablations](https://arxiv.org/abs/2608.00824) — 相关度 0.60：提出结构感知语义分块与标题链前缀方法，并揭示文本变换消融中的测量陷阱，对RAG类代码检索有参考价值。
- [ACEM: A Cost Estimation Model for Agentic Software Engineering](https://arxiv.org/abs/2608.02582) — 相关度 0.55：Agentic 软件工程成本估算模型，可迁移到 agent 成本管理。
- [AgenTag: Attribution of AI Coding Agents from Behavioral Fingerprints](https://arxiv.org/abs/2608.00966) — 相关度 0.55：通过行为指纹对AI编码agent进行归属识别，可用于agent治理与实证研究，但并非直接优化方法。
- [ECLAIR: A Causally-Grounded AI Framework for Scientific Discovery in Empirical Software Engineering](https://arxiv.org/abs/2608.02323) — 相关度 0.55：因果 AI 框架用于实证软件工程，可迁移到 agent 的实验设计与反思机制。
- [How Well Do LLMs Generate Taxonomies in the SE Domain? A Multi-perspective Evaluation Framework](https://arxiv.org/abs/2608.01592) — 相关度 0.55：评估LLM在SE领域生成分类法的质量与成本，其多视角评测框架可用于agent失败模式分类研究。
- [Lossless Tensor Compression as Program Synthesis](https://arxiv.org/abs/2608.02162) — 相关度 0.50：将无损张量压缩形式化为程序合成，程序合成方法可迁移到代码生成。
