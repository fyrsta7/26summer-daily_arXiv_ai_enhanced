...
- [Compaction as Epistemic Failure: How Agentic LLM Tools Fabricate Confirmed Results from Killed Processes](http://arxiv.org/abs/2607.13071v1) — 相关度 0.85：揭示 Claude Code 压缩摘要将超时进程输出误判为确认结果，直接暴露 coding agent 记忆与自我报告中的关键可靠性缺陷。
- [Falsifiable Release Gates for Self-Improving Systems](http://arxiv.org/abs/2607.13070v1) — 相关度 0.85：提出自改进 agent 运行时的可证伪发布门控与机器可验证验收套件，对构建安全的自主 coding agent 有直接借鉴价值。
- [Partial Contracts Suffice: Sound, LLM-Inferred Regression Verification](http://arxiv.org/abs/2607.10291v1) — 相关度 0.80：提出部分契约足够的 LLM 推断回归验证方法，兼顾了补丁验证的实用性与声音性，可增强代码 agent 的验证环节。
- [A Large-Scale Dataset of MCP Implementations on GitHub](http://arxiv.org/abs/2607.10123v1) — 相关度 0.75：首个大规模 MCP 实现数据集，为研究 agent 工具生态和工程化集成提供了可复现的基准资源。
- [From Business Requirements to Test Assertions: Evaluating LLM-Generated Oracles on Real Bugs](http://arxiv.org/abs/2607.10277v1) — 相关度 0.75：评估 LLM 直接从自然语言需求生成测试预言的能力，针对 oracle 问题的系统化流程和评测对自动化测试 agent 有直接价值。
- [Knowledge-Conditioned, Single-Pass LLM Synthesis of Executable Unity Game Scenes: A Compiler Error Census across 26 Goal Playable Concepts](http://arxiv.org/abs/2607.10187v1) — 相关度 0.70：对 LLM 单遍生成 Unity 代码的编译器错误普查，提供了面向特定领域代码生成能力评估与错误分类的基准。
- [Mitigating LLM Sycophancy in Code Smell Detection Using Evidence-Guided Reasoning Prompts](http://arxiv.org/abs/2607.10411v1) — 相关度 0.70：系统研究 LLM 代码异味检测中的谄媚偏差并提出证据引导提示策略，对代码审查 agent 的判断鲁棒性改进有直接帮助。
- [Chain-Aware Encoding for Microservice Trace Anomaly Detection](http://arxiv.org/abs/2607.10156v1) — 相关度 0.65：面向微服务调用链的上下文感知异常检测编码，可迁移到 agent 执行轨迹分析与故障定位。
- [Is Model Instability just Noise to be Tolerated or a Property that can be Managed?](http://arxiv.org/abs/2607.10420v1) — 相关度 0.65：论证 SE 优化中模型不稳定性应作为标准评估轴，并提供测量与管理方法，适用于软件工程 agent 的调优与评测。
- [VRExplorer: A Model-based Approach for Semi-Automated Testing of Virtual Reality Scenes](http://arxiv.org/abs/2607.10174v1) — 相关度 0.60：基于模型的 VR 场景半自动测试方法与探索 agent，对软件测试自动化中的交互建模和场景探索有参考价值。
- [Which Neurons Detect Malicious Code? A Probing Study of LLM Security Knowledge](http://arxiv.org/abs/2607.10221v1) — 相关度 0.55：通过探针分析定位 LLM 识别恶意代码的神经元并验证因果干预，虽偏机理但可为代码安全 agent 的可靠性设计提供思路。
