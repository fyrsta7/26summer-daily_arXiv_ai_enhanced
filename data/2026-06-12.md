...
- [FastContext: Training Efficient Repository Explorer for Coding Agents](http://arxiv.org/abs/2606.14066v4) — 相关度 0.95：提出专门的仓库探索子agent，显著降低token消耗并提升SWE-bench上的修复率，直接贡献于coding agent效率优化。
- [Beyond Correctness: Enhancing Architectural Reasoning in Code LLMs via Scalable Labeling with Agentic Judgment](http://arxiv.org/abs/2606.14948v2) — 相关度 0.90：提出agentic judging流水线大规模标注架构质量并微调开源模型，显著提升SWE-bench Verified解决率，直接相关。
- [When Errors Become Narratives: A Longitudinal Taxonomy of Silent Failures in a Production LLM Agent Runtime](http://arxiv.org/abs/2606.14589v1) — 相关度 0.90：纵向研究生产LLM agent runtime中的静默失败并给出机制化分类与防御框架，直接指导agent可靠性设计。
- [LLM Agents Can See Code Repositories](http://arxiv.org/abs/2606.14061v3) — 相关度 0.85：系统研究多模态大模型利用仓库可视化结构辅助issue修复，可迁移到coding agent的上下文压缩与导航设计。
- [tap: A File-Based Protocol for Heterogeneous LLM Agent Collaboration](http://arxiv.org/abs/2606.14445v1) — 相关度 0.85：提出跨运行时文件型协作协议，支持异构LLM agent在同一代码库上协作开发与评审，具有多agent协作迁移价值。
- [Investigating Metamorphic Fuzz Oracle Enhancement via Large Language Models](http://arxiv.org/abs/2606.14164v1) — 相关度 0.80：将LLM用于自动化生成和集成蜕变测试预言，提升模糊测试覆盖率并触发大量崩溃，对程序分析具有明确迁移价值。
- [Security in a Workflow: Exploring Role-Based Agentic Architectures for Vulnerability Handling](http://arxiv.org/abs/2606.14261v1) — 相关度 0.80：设计Planner/Analyzer/Fixer/Verifier角色化agent工作流处理漏洞，是软件安全agent化的直接实践。
- [No Accidental Software Agent First Canonical Code for Human Code Entropy Reduction and 30 to 500 times Lower Frontier Model Requirements](http://arxiv.org/abs/2606.14357v1) — 相关度 0.65：提出agent-first canonical code与行为等价规约降低前沿模型需求，对代码agent表示与验证有概念性启发。
- [Empowering Student Debugging in Parallel Programming with Execution Traces and Large Language Models](http://arxiv.org/abs/2606.14607v1) — 相关度 0.55：结合执行轨迹可视化与LLM辅助并发程序调试，其调试思路可迁移到代码agent的bug定位与修复。
- [Evaluating LLMs for Obfuscation Detection and Classification in Android Apps](http://arxiv.org/abs/2606.14233v1) — 相关度 0.55：评估LLM在Android混淆检测与分类上的语义推理能力，对静态代码分析和安全检测工具有参考价值。
