...
- [Breaking, Stale, or Missing? Benchmarking Coding Agents on Project-Level Test Evolution](http://arxiv.org/abs/2605.06125v1) — 相关度 0.95：提出项目级测试演化基准TEBench，直接评测coding agent的测试定位与更新能力。
- [An Empirical Study of Proactive Coding Assistants in Real-World Software Development](http://arxiv.org/abs/2605.05700v1) — 相关度 0.92：基于真实IDE轨迹构建ProCodeBench，揭示模拟到现实的差距，为主动编码助手评估提供数据基准。
- [Teaching LLMs Program Semantics via Symbolic Execution Traces](http://arxiv.org/abs/2605.06184v1) — 相关度 0.88：用符号执行轨迹训练LLM并构造500个C验证任务基准，直接提升程序语义理解与违例检测，可迁移到代码agent的验证与推理。
- [Agentic Coding Needs Proactivity, Not Just Autonomy](http://arxiv.org/abs/2605.06717v1) — 相关度 0.86：定义coding agent的proactivity三层分类与评估协议，指导长程自主agent设计。
- [BUILD-AND-FIND: An Effort-Aware Protocol for Evaluating Agent-Managed Codebases](http://arxiv.org/abs/2605.06136v1) — 相关度 0.86：提出BUILD-AND-FIND协议，评估agent所建代码库对下游agent的可恢复性，新颖的评测维度。
- [MAS-Algorithm: A Workflow for Solving Algorithmic Programming Problems with a Multi-Agent System](http://arxiv.org/abs/2605.05949v2) — 相关度 0.86：多agent工作流分解算法编程问题，模块化协作模式可迁移到代码agent框架。
- [SmellBench: Evaluating LLM Agents on Architectural Code Smell Repair](http://arxiv.org/abs/2605.07001v2) — 相关度 0.86：首次系统评测LLM agent修复架构代码坏味，构建SmellBench框架并分析误报与代码质量影响，直接适用于代码agent重构评估。
- [SiblingRepair: Sibling-Based Multi-Hunk Repair with Large Language Models](http://arxiv.org/abs/2605.06209v1) — 相关度 0.85：基于LLM的兄弟位置多hunk自动程序修复方法，引入两种互补修复策略，可迁移到代码agent的缺陷修复与补丁生成。
- [Beyond Accuracy: Policy Invariance as a Reliability Test for LLM Safety Judges](http://arxiv.org/abs/2605.06161v1) — 相关度 0.82：提出策略不变性检验LLM安全裁判的可靠性，其审计协议可迁移到代码agent评估。
- [Constraint Decay: The Fragility of LLM Agents in Backend Code Generation](http://arxiv.org/abs/2605.06445v1) — 相关度 0.82：系统研究LLM agent在多文件后端代码生成中对结构约束的衰减现象，提供评测与错误分析，直接关联coding agent能力边界。
- [Correct Code, Vulnerable Dependencies: A Large Scale Measurement Study of LLM-Specified Library Versions](http://arxiv.org/abs/2605.06279v1) — 相关度 0.80：大规模测量LLM生成代码中第三方库版本的安全与兼容风险，提供PinTrace基准，对代码生成agent的依赖管理有参考价值。
- [Heimdallr: Characterizing and Detecting LLM-Induced Security Risks in GitHub CI Workflows](http://arxiv.org/abs/2605.05969v2) — 相关度 0.80：构建LLM-Workflow属性图检测CI中LLM安全风险，对agent工作流的安全分析有直接参考。
- [On Fixing Insecure AI-Generated Code through Model Fine-Tuning and Prompting Strategies](http://arxiv.org/abs/2605.05867v1) — 相关度 0.80：系统评估微调和提示对AI生成代码安全性的修复效果，为代码agent安全修复提供策略参考。
- [ScarfBench: A Benchmark for Cross-Framework Application Migration in Enterprise Java](http://arxiv.org/abs/2605.06754v2) — 相关度 0.80：发布跨框架迁移基准ScarfBench，包含可执行oracle与agent评测，直接适用于评估代码agent的重构与迁移能力。
- [A Self-Healing Framework for Reliable LLM-Based Autonomous Agents](http://arxiv.org/abs/2605.06737v1) — 相关度 0.78：提出LLM软件agent的自愈框架，包含失败检测、可靠性评估与自适应重规划，对代码agent的可靠性设计有直接迁移价值。
- [Computer Use at the Edge of the Statistical Precipice](http://arxiv.org/abs/2605.08261v1) — 相关度 0.76：揭示交互式agent基准的统计陷阱并提出PRISM环境设计原则与聚合框架，对代码agent评测方法学有强迁移价值。
- [Evaluating Non-English Developer Support in Machine Learning for Software Engineering](http://arxiv.org/abs/2605.05902v1) — 相关度 0.76：多语言代码注释生成与评估研究，揭示非英语评测盲区，并发布标注数据集。
- [From Agent Loops to Deterministic Graphs: Execution Lineage for Reproducible AI-Native Work](http://arxiv.org/abs/2605.06365v1) — 相关度 0.75：提出执行血缘DAG模型改进agentic工作流的可复现性与状态维护，可迁移到代码agent的工件管理与增量更新。
- [Stop Comparing LLM Agents Without Disclosing the Harness](http://arxiv.org/abs/2605.23950v1) — 相关度 0.75：论证agent harness对长程任务性能的支配作用并给出评估框架，对代码agent比较与评测方法论有直接启示。
- [Eta Given Delta: Defining LLM Tool Efficiency With Marginal Tool Utility](http://arxiv.org/abs/2607.14108v1) — 相关度 0.72：提出边际工具效用和工具效率指标来量化LLM agent轨迹中的工具调用价值，可迁移到coding agent工具集精简与评测。
- [Is this Build Failure Related to my Patch? An Empirical Study of Unrelated Build Failures in Continuous Integration](http://arxiv.org/abs/2605.05564v1) — 相关度 0.72：用PU学习预测CI中无关构建失败，可迁移到agent对构建反馈的判别与调试。
- [Schedule-and-Calibrate: Utility-Guided Multi-Task Reinforcement Learning for Code LLMs](http://arxiv.org/abs/2605.06111v1) — 相关度 0.72：面向多任务代码RL的效用调度与校准，提升代码LLM训练效率，可增强agent基座。
- [To What Extent Does Agent-generated Code Require Maintenance? An Empirical Study](http://arxiv.org/abs/2605.06464v2) — 相关度 0.68：实证研究agent生成代码的维护频率与修改类型，为代码agent维护性评测和长期演化提供数据与方法参考。
- [From Chat to Interview: Agentic Requirements Elicitation with an Experience Ontology](http://arxiv.org/abs/2605.05828v1) — 相关度 0.66：用本体引导agent进行需求访谈，可推广至代码agent的上下文规划与信息获取。
- [Exploring the Effectiveness of Abstract Syntax Tree Patterns for Algorithm Recognition](http://arxiv.org/abs/2605.06098v1) — 相关度 0.62：AST模式匹配算法识别，可辅助代码检索与优化机会发现，但技术较传统。
- [Operationalizing Ethics for AI Agents: How Developers Encode Values into Repository Context Files](http://arxiv.org/abs/2605.05584v1) — 相关度 0.62：研究AGENTS.md中的价值观编码，为agent行为约束的上下文设计提供参考。
