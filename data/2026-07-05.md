...
- [Auto: The AGI Compiler](http://arxiv.org/abs/2607.04542v1) — 相关度 0.95：提出将智能体行为编译为可验证程序与原子的 AGI 编译器，直接优化执行成本与确定性。
- [Teaching Code LLMs to Reason with Intermediate Formal Specifications](http://arxiv.org/abs/2607.04232v1) — 相关度 0.95：验证引导的代码 LLM 训练框架，生成中间形式化规范，直接提升代码推理与修复。
- [Correct but Slow: An Empirical Study of the GPU Kernel Evaluation Gap in Modern Domain-Specific Languages](http://arxiv.org/abs/2607.04454v3) — 相关度 0.90：实证研究 GPU DSL 内核的正确性性能鸿沟，为性能验证提供轻量筛选方法。
- [Obey, Diverge, Collapse: Blind Obedience to Incorrect Instructions Drives Code LLMs to Irrecoverable Code Semantic Collapse](http://arxiv.org/abs/2607.04537v1) — 相关度 0.90：揭示代码 LLM 对错误指令的盲目服从导致不可恢复崩溃，对智能体可靠性至关重要。
- [SEDCoT: Enhancing LLM-Based COBOL Code Translation via Symbolic Execution and Delta Debugging](http://arxiv.org/abs/2607.04092v1) — 相关度 0.90：结合符号执行与三角洲调试修复 LLM 代码翻译，提升语义正确性。
- [An Evaluation of Role-Based Multi-Agent Code Generation on Repository-Scale Problems](http://arxiv.org/abs/2607.04212v1) — 相关度 0.85：评测角色型多智能体在仓库级代码生成中的表现，对多智能体协作研究有参考。
- [Benchmarking API Drift in LLM-Generated Quantum Code Across Successive SDK Versions](http://arxiv.org/abs/2607.04072v1) — 相关度 0.80：基准测试 LLM 生成量子代码的 API 版本一致性，可迁移到通用代码生成评测。
- [Using OAI Overlay to Enhance REST API Fuzzing](http://arxiv.org/abs/2607.04325v1) — 相关度 0.75：利用 OAI Overlay 标准增强 REST API 模糊测试，具有自动化测试迁移价值。
