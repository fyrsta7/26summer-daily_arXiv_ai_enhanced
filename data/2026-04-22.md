...
- [SWE-chat: Coding Agent Interactions From Real Users in the Wild](http://arxiv.org/abs/2604.20779v1) — 相关度 0.95：首个真实用户编码 agent 会话数据集（6000 sessions），并揭示模式与故障，是 coding agent 实证研究的直接基础。
- [Learning Reasoning World Models for Parallel Code](http://arxiv.org/abs/2604.20926v3) — 相关度 0.90：为并行代码预测工具结果的推理世界模型，可替代外部工具反馈，强相关agent。
- [BODHI: Precise OS Kernel Specification Inference](http://arxiv.org/abs/2605.23931v1) — 相关度 0.85：用领域知识提示大幅提升 LLM 从内核代码生成形式规约的正确性，是规格合成/验证 agent 的直接方法。
- [Taint-Style Vulnerability Detection and Confirmation for Node.js Packages Using LLM Agent Reasoning](http://arxiv.org/abs/2604.20179v1) — 相关度 0.85：多阶段LLM agent pipeline检测并生成PoC验证Node.js漏洞，直接相关安全代码agent。
- [Structural Quality Gaps in Practitioner AI Governance Prompts: An Empirical Study Using a Five-Principle Evaluation Framework](http://arxiv.org/abs/2604.21090v1) — 相关度 0.80：对 AGENTS.md 治理提示做结构完整性评估，发现常见缺口，直接关系到 coding agent 行为约束与工程化。
- [The Path Not Taken: Duality in Reasoning about Program Execution](http://arxiv.org/abs/2604.20917v1) — 相关度 0.80：提出程序执行双重推理benchmark，评测LLM动态代码理解，相关代码智能。
- [WebGen-R1: Incentivizing Large Language Models to Generate Functional and Aesthetic Websites with Reinforcement Learning](http://arxiv.org/abs/2604.20398v1) — 相关度 0.80：端到端RL生成多页面网站代码，级联多模态奖励设计可迁移至代码生成。
- [DeepParse: Hybrid Log Parsing with LLM-Synthesized Regex Masks](http://arxiv.org/abs/2604.20553v1) — 相关度 0.75：LLM 合成正则模板后交给 Drain 确定性执行，分离推理与执行，为 agent 工具设计提供低成本高可靠的范式。
- [Hallucination Inspector: A Fact-Checking Judge for API Migration](http://arxiv.org/abs/2604.20202v1) — 相关度 0.75：静态分析工具检测LLM API迁移中的幻觉符号，提升代码agent可靠性。
- [A Ground-Truth-Based Evaluation of Vulnerability Detection Across Multiple Ecosystems](http://arxiv.org/abs/2604.21111v1) — 相关度 0.70：基于 OSV 构建跨生态漏洞检测 ground-truth 并开源复现工具，为漏洞检测/修复 agent 提供评测基准。
- [Deja Vu at Scale: Paraphrase-Robust Detection of Duplicate Gherkin Steps in Behaviour-Driven Software Testing with Sentence-Transformer Embeddings and a 1.1M-Step Open Benchmark](http://arxiv.org/abs/2604.20462v3) — 相关度 0.70：BDD 步骤去重检测方法与 1.1M 步开放基准，可迁移到测试代码去重和 agent 生成测试的质量评估。
- [Evaluating Software Defect Prediction Models via the Area Under the ROC Curve Can Be Misleading](http://arxiv.org/abs/2604.20742v1) — 相关度 0.70：指出用 AUC 评估缺陷预测模型可能误导，提出阈值相关可视化度量，对 agent/模型的性能评测方法有直接参考。
- [Residual Risk Analysis in Benign Code: How Far Are We? A Multi-Model Semantic and Structural Similarity Approach](http://arxiv.org/abs/2604.21051v1) — 相关度 0.70：结合语义和 AST 结构相似性估计补丁后残余风险，可迁移到 agent 的漏洞修复后验证与优先级排序。
- [Shift-Up: A Framework for Software Engineering Guardrails in AI-native Software Development -- Initial Findings](http://arxiv.org/abs/2604.20436v1) — 相关度 0.70：用BDD/C4/ADR作为守护机制稳定agent行为，减少漂移，相关SE agent工作流。
- [Towards Secure Logging: Characterizing and Benchmarking Logging Code Security Issues with LLMs](http://arxiv.org/abs/2604.20211v1) — 相关度 0.70：构建日志安全缺陷分类与benchmark，评测LLM检测修复，相关代码安全生成。
- [Behavioral Consistency and Transparency Analysis on Large Language Model API Gateways](http://arxiv.org/abs/2604.21083v1) — 相关度 0.60：黑盒测量 LLM API 网关的模型替换、计费等不一致行为，为依赖网关的 coding agent 提供部署可靠性审计手段。
- [Early-Stage Product Line Validation Using LLMs: A Study on Semi-Formal Blueprint Analysis](http://arxiv.org/abs/2604.20523v1) — 相关度 0.60：系统评估 12 个 LLM 在特性模型分析操作上的准确率与成本，为 LLM 做 SE 分析的 agent 提供评测与选型依据。
- [Evaluating Assurance Cases as Text-Attributed Graphs for Structure and Provenance Analysis](http://arxiv.org/abs/2604.20577v2) — 相关度 0.60：用 GNN 对 assurance case 做链接预测与人类/LLM 来源判别，方法可迁移到 agent 产出物的结构与溯源测评。
- [QuanForge: A Mutation Testing Framework for Quantum Neural Networks](http://arxiv.org/abs/2604.20706v1) — 相关度 0.60：提出面向 QNN 的变异测试框架与统计变异击杀准则，随机性场景下的测试思想可借鉴到 agent 输出验证。
- [Value-Conflict Diagnostics Reveal Widespread Alignment Faking in Language Models](http://arxiv.org/abs/2604.20995v2) — 相关度 0.60：提出价值冲突诊断框架并发现更普遍的 alignment faking，其监控与轻量干预向量可迁移到代码 agent 的可靠性。
- [On the Informativeness of Security Commit Messages: A Large-scale Replication Study](http://arxiv.org/abs/2604.20461v1) — 相关度 0.50：安全提交信息充分性实证，可为agent生成提交信息提供参考。
