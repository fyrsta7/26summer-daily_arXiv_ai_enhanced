...
- [Agentic Interpretation: Lattice-Structured Evidence for LLM-Based Program Analysis](http://arxiv.org/abs/2605.12694v1) — 相关度 0.95：提出基于格结构的agentic interpretation框架,将静态分析纪律引入LLM程序推理,直接契合代码分析Agent。
- [Characterizing the Failure Modes of LLMs in Resolving Real-World GitHub Issues](http://arxiv.org/abs/2605.12270v1) — 相关度 0.95：直接剖析LLM在真实GitHub issue修复中的失败模式,并构建统一分类法,对SWE-agent的评测与改进有直接迁移价值。
- [Decaf: Improving Neural Decompilation with Automatic Feedback and Search](http://arxiv.org/abs/2605.11501v1) — 相关度 0.85：通过编译器反馈和搜索显著提升神经反编译正确性，展示了生成-验证-搜索闭环对代码生成agent的有效性。
- [Finding a Crab in the C: Assured Translation via Comparative Symbolic Execution](http://arxiv.org/abs/2605.12731v1) — 相关度 0.85：通过比较符号执行验证源码与安全语言翻译的一致性,支持代码翻译/审查/纠错,对Agent的代码生成验证有强参考价值。
- [SkillSmith: Compiling Agent Skills into Boundary-Guided Runtime Interfaces](http://arxiv.org/abs/2605.15215v1) — 相关度 0.85：将技能编译为边界化运行时接口，显著降低token与推理开销，可直接提升代码agent的效率与可复用性。
- [StepCodeReasoner: Aligning Code Reasoning with Stepwise Execution Traces via Reinforcement Learning](http://arxiv.org/abs/2605.11922v1) — 相关度 0.85：在执行轨迹上引入分步监督和双层GRPO，增强代码推理与生成能力，其验证机制可迁移到agent反思与信用分配。
- [Breaking the Dependency Chaos: A Constraint-Driven Python Dependency Resolution Strategy with Selective LLM Imputation](http://arxiv.org/abs/2605.11772v1) — 相关度 0.80：结合SMT约束求解与LLM补全，大幅提升依赖解析成功率并降低LLM调用次数，可集成到构建和修复agent中。
- [It's Not the Size: Harness Design Determines Operational Stability in Small Language Models](http://arxiv.org/abs/2605.12129v1) — 相关度 0.80：实验验证plan→execute→verify→recover管线对SLM稳定性的关键作用，对代码agent的harness设计有直接借鉴。
- [Uncertainty Quantification for LLM-based Code Generation](http://arxiv.org/abs/2605.12201v1) — 相关度 0.80：用多重假设检验构造风险可控的预测集，为代码生成的不确定性量化与安全验证提供新方法。
- [Options, Not Clicks: Lattice Refinement for Consent-Driven MCP Authorization](http://arxiv.org/abs/2605.11360v1) — 相关度 0.75：面向MCP工具调用的授权中间件，利用风险格自动允许安全调用并升级风险，提升智能体安全边界。
- [SemChunk-C: Semantic Segmentation for C Code](http://arxiv.org/abs/2606.23697v1) — 相关度 0.75：为C系代码提出轻量语义分块模型，提升代码检索与LLM下游任务效果，对代码agent的上下文工程有价值。
- [CIDR: A Large-Scale Industrial Source Code Dataset for Software Engineering Research](http://arxiv.org/abs/2605.12153v1) — 相关度 0.70：提供大规模工业级源代码数据集，支持代码大模型预训练、微调与agent评测，具有基础研究价值。
- [Cochise: A Reference Harness for Autonomous Penetration Testing](http://arxiv.org/abs/2605.11671v1) — 相关度 0.70：提供可复用的agent渗透测试harness和轨迹分析工具，对代码agent实验基础设施与评测有直接参考价值。
- [Iterative Audit Convergence in LLM-Managed Multi-Agent Systems: A Case Study in Prompt-Engineering Quality Assurance](http://arxiv.org/abs/2605.12280v2) — 相关度 0.70：提出LLM多Agent系统的迭代审计协议与缺陷分类,可迁移到代码Agent的可靠性与多Agent协作评测。
- [NeuroFlake: A Neuro-Symbolic LLM Framework for Flaky Test Classification](http://arxiv.org/abs/2605.11482v1) — 相关度 0.70：面向flaky test分类的神经符号方法，结合鲁棒性分析，可迁移到代码测试agent的输入扰动与语义理解改进。
- [Bidirectional Empowerment of Metamorphic Testing and Large Language Models: A Systematic Survey](http://arxiv.org/abs/2605.13898v1) — 相关度 0.65：系统综述蜕变测试与LLM的双向赋能，为LLM测试agent提供蜕变关系发现与闭环测试工具化思路。
- [LGMT: Logic-Grounded Metamorphic Testing for Evaluating the Reasoning Reliability of LLMs](http://arxiv.org/abs/2605.23965v3) — 相关度 0.65：基于逻辑的蜕变测试评估LLM推理可靠性,方法可迁移到代码Agent的逻辑一致性验证。
- [Neural Code Translation of Legacy Code: APL to C#](http://arxiv.org/abs/2605.13896v1) — 相关度 0.65：对比NL中介、RAG、迭代优化等引导策略进行APL到C#翻译，对跨语言代码迁移agent有方法参考。
- [ReproBreak: A Dataset of Reproducible Web Locator Breaks](http://arxiv.org/abs/2605.12158v1) — 相关度 0.65：发布可复现的Web locator break数据集，可支撑GUI测试修复、定位器脆弱性与回归测试鲁棒性研究。
- [An Extensive Replication Study of the ABLoTS Approach for Bug Localization](http://arxiv.org/abs/2605.11790v1) — 相关度 0.60：复现ABLoTS时揭示时间cut-off造成的数据泄漏问题，对代码检索与定位评测的严谨性具有警示作用。
- [Divergent Multi-Version Execution (DME): Canonical Instruction-Trace Fault Detection via Structural Address-Space Decorrelation](http://arxiv.org/abs/2605.12576v1) — 相关度 0.60：通过多版本异构执行与规范指令轨迹对比进行错误检测，其运行时验证思想可借鉴到agent生成代码的可靠性检查。
- [Property-Level Reconstructability of Agent Decisions: An Anchor-Level Pilot Across Vendor SDK Adapter Regimes](http://arxiv.org/abs/2605.12078v1) — 相关度 0.60：提出统一的agent决策事件schema并评估不同SDK trace的可重构性，可用于agent审计、评测与追踪。
- [A Research Agenda on Agents and Software Engineering: Outcomes from the Rio A2SE Seminar](http://arxiv.org/abs/2605.11720v1) — 相关度 0.55：给出agent与软件工程的研究议程和主题聚类，虽非具体方法，但为科研方向与评测体系提供参考框架。
