...
- [Active-SWE: Benchmarking Coding Agents for Proactive Bug Fixing without Issue Reports](https://arxiv.org/abs/2608.04682) — 相关度 0.90：新benchmark评估agent主动发现并修复多bug，扩展SWE评测范畴，直接相关
- [AdaptAgent: A Multi-agent, Domain-Guided Reasoning Framework for Code Adaptation](https://arxiv.org/abs/2608.04459) — 相关度 0.90：多agent分解代码适配任务，通过策略与编译器验证迭代，直接符合coding agent研究方向
- [COMPAS: Difficulty-Aware Joint Search for Optimizing Code Generation](https://arxiv.org/abs/2608.04336) — 相关度 0.90：难度感知联合搜索模型/提示/解码配置并路由，优化代码生成质量与成本，直接相关
- [EA-Graph: Artifact-Anchored Verification Memory for Coding Agents under Upstream Drift](https://arxiv.org/abs/2608.04278) — 相关度 0.90：为编码agent提供artifact-anchored验证记忆，处理上游漂移时声明有效性，直接相关
- [ExeCRE: Execution-Consistency Guided Reliability Estimation for Self-Correcting Code Generation](https://arxiv.org/abs/2608.04439) — 相关度 0.90：基于执行一致性估计代码可靠性，减少自纠正中的误导反馈，直接提升agent正确性
- [Scrouting: Cost-Aware Routing of Coding Agents by Scouting the Repository First](https://arxiv.org/abs/2608.04804) — 相关度 0.90：通过先侦察仓库再路由低成本/高解决率的编码agent，直接属于agent成本优化
- [The Order Is the Guarantee: Verifier-Budgeted Code Deletion with Static-First Learned Proposals](https://arxiv.org/abs/2608.04611) — 相关度 0.90：在有限验证预算下用排序和静态优先策略删除代码，覆盖agent代码维护与验证
- [SciCode-Verified: How Benchmark Defects Underestimated the Scientific-Coding Ability of Language Models](https://arxiv.org/abs/2608.04975) — 相关度 0.88：审计并修正SciCode基准缺陷后模型能力评估大幅回升，是面向科学代码生成的评测质量方法贡献。
- [OctoLong: Mid-Training On Cross-Repository Code Contexts Enhances Long-Context Modeling](https://arxiv.org/abs/2608.05141) — 相关度 0.80：用AST和语言服务构造跨仓库长上下文训练数据，显著提升长程检索与仓库级agent任务能力。
- [RepoProbe: Benchmarking Architecture-Aware Repository Comprehension with Checklists](https://arxiv.org/abs/2608.04783) — 相关度 0.80：提出仓库理解benchmark和checklist验证协议，改善agent评测可靠性，直接相关
- [RepairFormer: Automated Repair of Structured Inputs Using Transformers](https://arxiv.org/abs/2608.05060) — 相关度 0.76：用Transformer将结构化输入修复建模为带格式标签和边界定位的序列生成，可迁移到代码与配置文件的自动修复。
- [A Unified Model for Cross-Domain Clone Detection via Model Merging](https://arxiv.org/abs/2608.04215) — 相关度 0.70：系统研究模型合并构建跨域克隆检测器，为SE模型优化和OOD鲁棒性提供实证与方法
- [CURATE: Leveraging LLM Agents to Compose, Catalog, and Deploy Reproducible Workflows](https://arxiv.org/abs/2608.04270) — 相关度 0.70：多agent系统管理可复现工作流生命周期，包含目录复用和人在环路，可迁移到agent工作流设计
- [MergeSE: Post-Hoc Model Merging for Software Engineering Tasks Without Retraining](https://arxiv.org/abs/2608.04181) — 相关度 0.70：提出训练无关的模型合并工具，支持SE任务，对代码模型优化与合并方法有直接参考价值
- [SONAR: Task-Aware Code Summary Evaluation for LLM Consumers Without References](https://arxiv.org/abs/2608.04195) — 相关度 0.70：提出无参考的代码摘要评估框架，以代码再生为信号，可迁移到agent对摘要质量的判断
- [An Exploratory Study of Agent Plans for Agentic AI Coding Tools in Open-Source Software](https://arxiv.org/abs/2608.04661) — 相关度 0.60：对开源仓库Agent Plans的实证分析，揭示agent任务规划与执行指导，有参考价值
- [Neuro-Symbolic Proof-of-Vulnerability Generation with Open-Weight Models](https://arxiv.org/abs/2608.04217) — 相关度 0.60：神经符号的PoV生成框架结合LLM和SMT，对漏洞验证与agent安全分析有参考价值
- [What We Observe as LLM Behavior Can Be a Side-effect of Inference Backend](https://arxiv.org/abs/2608.04714) — 相关度 0.60：揭示推理后端对LLM benchmark结果的影响，提示agent评测需披露后端配置，有方法性贡献
- [CoPlan: A Trustworthy Co-Intelligence Interface for Care Planning through Role-Based Contestable Argument Graphs](https://arxiv.org/abs/2608.05107) — 相关度 0.56：提出基于角色可争辩参数图的多agent协作与人类可质疑机制，对代码agent的规划/评审工作流有迁移价值。
- [An Approach for Embedding-Guided Function Reuse Detection in Embedded C Software](https://arxiv.org/abs/2608.04137) — 相关度 0.50：使用RAG与硬件兼容性验证器检测嵌入式C函数复用，可作为代码检索/复用方法参考，但领域较专
