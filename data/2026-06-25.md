...
- [Glite ARF: Verifier-Driven Research with Parallel LLM Coding Agents](http://arxiv.org/abs/2606.27416v1) — 相关度 0.90：并行 LLM coding agent 的验证驱动框架，强调可审计性和隔离，对 agent 工作流有直接借鉴。
- [How Much Static Structure Do Code Agents Need? A Study of Deterministic Anchoring](http://arxiv.org/abs/2606.26979v2) — 相关度 0.90：研究静态结构注释对代码agent定位与可复现性的影响，直接改进agent导航与稳定性。
- [Mostly Automatic Translation of Language Interpreters from C to Safe Rust](http://arxiv.org/abs/2606.27122v1) — 相关度 0.90：多 agent 编排 C 到 Rust 解释器翻译，结合 feature reduction 和自动验证，对长程代码 agent 有直接迁移价值。
- [To Run or Not to Run: Analyzing the Cost-Effectiveness of Code Execution in LLM-Based Program Repair](http://arxiv.org/abs/2606.26978v1) — 相关度 0.90：实证分析LLM程序修复中代码执行的成本效益，直接指导agent执行策略的取舍与设计。
- [A Deterministic Control Plane for LLM Coding Agents](http://arxiv.org/abs/2606.26924v1) — 相关度 0.88：为LLM coding agent提出确定性控制平面，管理配置供应链、权限与审计，直接提升agent可靠性与安全性。
- [ConcoLixir: Reactive LLM Discovery Oracles for Python Concolic Testing](http://arxiv.org/abs/2606.26545v1) — 相关度 0.85：LLM作为发现预言机增强Python concolic测试覆盖率，直接结合LLM与测试生成，迁移价值高。
- [The Spec Growth Engine: Spec-Anchored, Code-Coupled, Drift-Enforced Architecture for AI-Assisted Software Development](http://arxiv.org/abs/2606.27045v1) — 相关度 0.83：提出Spec Growth Engine管理规格图与上下文范围，解决agent上下文爆炸与spec-code漂移，对工作流有迁移价值。
- [Towards Evaluation of Implicit Software World Models in Coding LLMs](http://arxiv.org/abs/2606.27406v1) — 相关度 0.82：扩展coding LLM评测到内存、时间等执行资源维度，直接补充软件世界模型与agent能力评估。
- [An Empirical Study of LLM-Generated Specifications for VeriFast](http://arxiv.org/abs/2606.26490v1) — 相关度 0.80：系统评估LLM生成VeriFast分离逻辑规范，对代码agent的正确性验证与规范生成有直接参考。
- [Knowledge-Based Pull Requests: A Trusted Workflow for Agent-Mediated Knowledge Collaboration](http://arxiv.org/abs/2606.26721v1) — 相关度 0.80：提出基于知识包的PR工作流，区分知识与实现合并，对多agent协作与信任治理有明确借鉴。
- [Smaller Models, Unexpected Costs: Trade-offs in LLM Quantization for Automated Program Repair](http://arxiv.org/abs/2606.27205v2) — 相关度 0.80：系统评估 LLM 量化对自动程序修复的影响，涉及效率与效果权衡，对 coding agent 部署有用。
- [CLIR: Liveness-Driven and Structure-Aware Fuzzing for the Cranelift Compiler](http://arxiv.org/abs/2606.26977v1) — 相关度 0.75：面向Cranelift编译器的差分模糊测试框架，生成合法IR并有效检测缺陷，属于编译器测试通用方法。
- [NOVA: A Verification-Aware Agent Harness for Architecture Evolution in Industrial Recommender Systems](http://arxiv.org/abs/2606.27243v2) — 相关度 0.75：验证感知的 agent harness 用于架构演化，其验证级联和任务级控制可迁移到代码 agent。
- [Are LLMs Ready for Anti-Pattern Detection in Microservice Architectures?](http://arxiv.org/abs/2606.26927v1) — 相关度 0.70：评估LLM检测微服务架构反模式，可为代码分析型agent提供能力参考与补充静态分析。
- [Test Case Selection for Deep Neural Networks: A Replication Study on LLMs for Code](http://arxiv.org/abs/2606.27601v1) — 相关度 0.70：在 LLM 代码模型上复现测试用例选择技术，对代码模型的测试和评测有贡献。
- [On the Reproducibility of Quantum Software Defect Datasets: A Case Study of Bugs4Q](http://arxiv.org/abs/2606.27124v1) — 相关度 0.60：量子缺陷数据集可复现性研究，对 benchmark 维护和评测可靠性有参考价值。
- [Evaluation-Strategy Gap in Fault Diagnosis of Deep Learning Programs](http://arxiv.org/abs/2606.26492v1) — 相关度 0.55：揭示DL程序故障诊断评估策略的泛化差距，对调试技术评估方法的迁移有参考价值。
- [Speculative Refinement: A Hybrid Autoregressive Diffusion Decoding Strategy and Its Behavior Across Benchmarks](http://arxiv.org/abs/2606.27474v1) — 相关度 0.55：混合解码策略在代码基准上的评估发现，对代码生成评测方法有参考价值。
