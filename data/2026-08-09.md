...
### [4] [Predicting Agile Success: The Critical Few Factors](https://arxiv.org/abs/2608.06228)
...
TL;DR: 本研究通过PLS-SEM分析敏捷项目关键成功因素，提出一个评估敏捷项目成功的新模型。


<details>
  <summary>Details</summary>
Motivation: 敏捷项目成功率虽高于传统方法但仍偏低，因为缺乏对关键成功因素和当代成功标准的严格评估。

Method: 采用实证主义方法，使用SmartPLS进行PLS-SEM分析，检验假设以识别CSF与项目成功标准之间的关系。

Result: 发现少数关键因素显著影响敏捷项目成功，并开发了新模型。

Conclusion: 所提出的模型可用于评估和衡量敏捷项目成功，帮助提高项目成功率。

中文摘要：虽然敏捷项目比传统软件开发项目管理方法更成功，但其总体成功率仍然相对较低，有很大比例的项目仍被视为面临挑战或失败。这种低项目成功率归因于项目未根据敏捷项目的关键成功因素（CSF）和当代项目成功标准进行严格评估。为了识别敏捷从业者认为有助于敏捷软件开发项目成功的CSF，本研究采用实证主义方法调查敏捷软件开发项目的CSF。使用SmartPLS的PLS-SEM分析数据并检验假设，以识别结构之间及项目成功标准之间的显著关系。研究发现，少数关键因素对敏捷项目成功有显著贡献。该研究开发了一个新颖模型，可用于评估和衡量项目成功。
...
### [5] [Automatic Translation of Unstructured Requirements into Linear Temporal Logic through Large Language Models](https://arxiv.org/abs/2608.06287)
...
TL;DR: 本文评估了六个现代LLM在少样本提示下从非结构化需求生成LTL公式的能力，结果显示无需微调即可达到实际可用性能，表明LLM可作为形式化验证的前端助手。


<details>
  <summary>Details</summary>
Motivation: 安全关键系统的验证依赖精确形式化规范，但从非结构化自然语言需求手动生成LTL公式既困难又耗时，现有自动化方法有限，需探索LLM能否缓解这一瓶颈。

Method: 使用六个现成LLM，采用少样本提示策略，在15个异构需求上生成LTL公式；每个需求-模型对生成5次，共450个候选公式；通过人工语义评估、pass@k（k=1,3,5）和自一致性度量进行性能分析，并辅以自然语言解释和可视化讨论。

Result: 通用LLM在无需任务特定微调的情况下，在非结构化NL到LTL任务上取得了实际显著的性能；pass@k显示较高成功率，人工评估确认语义正确性，自一致性表明语法可复现性良好。

Conclusion: 现代LLM作为半自动形式化工作流的前端助手具有可行性，能够辅助将非结构化需求转换为形式化规范，提升安全关键系统开发的效率。

中文摘要：将非结构化的自然语言需求自动转换为形式化规范仍然是一个挑战，尤其是在需求工程和形式化方法领域，对于安全关键和任务关键系统而言，其验证依赖于数学上精确的规范。本文评估了当代现成的大型语言模型（LLM）是否能够通过直接从非结构化需求生成线性时序逻辑（LTL）公式来帮助弥合这一差距。该研究使用少样本提示策略，在一个包含15个结构各异需求的异构基准上考察了六个现代LLM。每个需求-模型对收集了五次独立生成，总共产生了450个候选LTL公式。性能通过人工语义评估、k∈{1,3,5}时的pass@k，以及一种捕获随机试验中语法可复现性的自一致性度量来评估。结果表明，当前通用的LLM能够在无需任务特定微调的情况下，在非结构化自然语言到LTL任务上取得实际显著的性能。该研究还通过将生成的公式与模型生成的自然语言解释配对，并讨论基于时间线的LTL可视化的补充使用，考虑了对非专家的可理解性。研究结果表明，现代LLM正在成为半自动化形式化工作流程中可行的前端助手。
...
### [7] [DoctorAgents: an agentic framework to iteratively refine AutoML pipeline for small clinical temporal data](https://arxiv.org/abs/2608.05375)
...
### [8] [Recursive Synthesis for Long-Horizon Terminal Tasks](https://arxiv.org/abs/2608.05466)
...
### [9] [Innovation-Residual Auditing of Autonomous Analysis Agents: Localization, Detection Limits, Error Control, and Identifiability](https://arxiv.org/abs/2608.05490)
...
### [11] [SkillTV-Bench: Benchmarking How Well Judges Perform on Skill-Augmented Agentic Execution](https://arxiv.org/abs/2608.05573)
...
### [12] [StepReflect: Structured UI Transition Reflection for Mobile GUI Agents](https://arxiv.org/abs/2608.05587)
...
### [13] [SkillHEX: Improving Agent Skills via Hypothesis-Driven Autonomous Exploration and Exploitation](https://arxiv.org/abs/2608.05628)
...
### [14] [Refining Over Resampling: Test-Time Self-Correction for LLM Reasoning](https://arxiv.org/abs/2608.05643)
...
### [15] [DreamGuard: Efficient Runtime Guardrail for LLM Agents via Risk-Aware World Model](https://arxiv.org/abs/2608.05695)
...
### [16] [RA-CAD: Learning Post-Execution Critique for State-Aware Text-to-CAD Generation](https://arxiv.org/abs/2608.05714)
...
### [17] [Unified Agent: Managing Interactions across Devices](https://arxiv.org/abs/2608.05729)
...
### [18] [When Do Prompt-Side Agent Playbooks Transfer? Accuracy, Cost, and Runtime Shift in Agent Deployment](https://arxiv.org/abs/2608.05778)
...
### [21] [When Agentic AI Meets Integrated Sensing and Communication](https://arxiv.org/abs/2608.05792)
...
### [22] [When Self-Evolution Backfires: Pre-Commit Gating against Skill Contamination in LLM Agents](https://arxiv.org/abs/2608.05810)
...
### [23] [Seeing Is Not Deciding: Can Multimodal LLMs Act as Effective CEOs?](https://arxiv.org/abs/2608.05864)
...
### [24] [Personalized Deep Research Query Refinement with Graph-Scaffolded Evidence Grounding](https://arxiv.org/abs/2608.05876)
...
### [26] [GAUGE: A Measurement-Grounded Benchmark for Physical Fidelity in Simulation Engines and Video World Models](https://arxiv.org/abs/2608.05948)
...
### [27] [VLMs for Videogame Data Annotation](https://arxiv.org/abs/2608.05949)
...
### [30] [OPERA: Operator-residual feedback for reliable autonomous optical experiments with language-model agents](https://arxiv.org/abs/2608.05990)
...
### [31] [HERALD: Counterfactual Audits and Minimal Repairs for Proof-of-Retrieval Rewards](https://arxiv.org/abs/2608.06012)
...
### [32] [From Economic Agents to Agentic Economies: A Systems Blueprint for Economic World Models](https://arxiv.org/abs/2608.06020)
...
### [33] [When History Lies: Evaluating and Improving Tool Use under Misleading Multi-Turn Histories](https://arxiv.org/abs/2608.06057)
...
### [34] [Evaluating Investment Logic in Large Language Models: A Real-World Benchmark Towards Personalzied Financial Agents](https://arxiv.org/abs/2608.06108)
...
### [35] [Contextual Information Policy Optimization for Search Agents](https://arxiv.org/abs/2608.06128)
...
### [36] [FinEvo-Bench: A Longitudinal Benchmark for Self-Evolving Agents in Professional Financial Workflows](https://arxiv.org/abs/2608.06144)
...
### [37] [iARCS: Iterative Agentic RL for Controllable 3D Scene Generation](https://arxiv.org/abs/2608.06161)
...
### [38] [MicroEvo: Knowledge-Guided LLM Sampling for Efficient Microarchitecture Design Space Exploration](https://arxiv.org/abs/2608.06183)
...
### [39] [Comparative Approaches to Agent Retrieval over Large Skill Libraries](https://arxiv.org/abs/2608.06196)
...
### [40] [EnvACE: Internalizing Environment Dynamics via World Rehearsal for Agentic Reinforcement Learning](https://arxiv.org/abs/2608.06197)
...
### [41] [The Illusion of Visual Tool-Use: A Causal Audit of Thinking with Images](https://arxiv.org/abs/2608.06270)
...
### [43] [Beyond Top-K: Replacing Black-Box Retrieval with Interpretable Agentic Operations](https://arxiv.org/abs/2608.06305)
...
### [44] [TRAJDEBUG: Tracing Error Lifecycle to Identify Critical Failures in Long-Horizon Agent Trajectories](https://arxiv.org/abs/2608.06346)
