...
- [Agent trajectories as programs: fingerprinting and programming coding-agent behavior](http://arxiv.org/abs/2606.16988v1) — 相关度 0.95：提出编码代理行为指纹识别与程序化表示，在SWE-Bench上分析代理结构，普适性强。
- [Looking Is Not Picking: An Attention-Segment Account of Tool-Selection Failures in LLM Agents](http://arxiv.org/abs/2606.16364v2) — 相关度 0.90：揭示LLM代理工具选择中注意力与决策分离机制，提出注意力引导的无训练选择器，可迁移至编码代理的工具选择。
- [Neuro-Symbolic Software Verification: Hyper-charging Local Language Models with Symbolic Reasoning at Scale](http://arxiv.org/abs/2606.16886v1) — 相关度 0.90：结合本地LLM与符号推理进行循环不变式合成，利用验证器反馈迭代，直接关乎代码验证与工具反馈。
- [Selection Without Signal, Recovery Through Expression: A Measurement Study of Post-Hoc Falsification Operators for Frozen Small Code Models](http://arxiv.org/abs/2606.16999v1) — 相关度 0.90：系统评测冻结代码模型的后处理操作符，提出无损提取恢复与自适应早停，直接指导代码代理采样策略。
- [The Right Call for Software Benchmarking: Consistent Decisions in Stateful Environments](http://arxiv.org/abs/2606.17261v1) — 相关度 0.90：针对有状态环境提出一致决策的基准实验设计，直接改进软件性能比较的方法论。
- [No Resource, No Benchmarks, No Problem? Evaluating and Improving LLMs for Code Generation in No-Resource Languages](http://arxiv.org/abs/2606.16827v1) — 相关度 0.85：构建无资源语言代码生成基准并比较预训练/微调策略，为代码代理在低资源语言上的部署提供实用路径。
- [Binary Decompilation LLM with Feedback-Driven Multi-Turn Refinement](http://arxiv.org/abs/2606.16162v1) — 相关度 0.80：将二进制反编译建模为多轮RL，利用编译/执行/I/O反馈修正生成，其反馈驱动范式可迁移到代码生成agent。
- [Towards LLM Accelerated Rapid Reviews for Software Tool Discovery -- Case for Log Anomaly Detection](http://arxiv.org/abs/2606.16839v1) — 相关度 0.80：提出LLM筛选和编码代理运行工具的流水线，自动化软件工具发现与执行，可迁移到代理评价。
- [Vibe Coding Ate My Homework: An evaluation of AI approaches to greenfield software engineering and programming](http://arxiv.org/abs/2606.18293v2) — 相关度 0.80：评估自然语言编程在绿地任务上的能力并分析现有基准，对代码代理能力边界有直接参考。
- [Trust-Aware Multi-Agent Traceability: Confidence-Calibrated Knowledge Graphs for Consistent Software Artifact Management](http://arxiv.org/abs/2606.17203v1) — 相关度 0.75：提出置信度校准的共享知识图谱协调多代理，减少错误传播，适用于软件工程代理的可靠协作。
- [Cluster-Aware Dual-Level Test Specification Generation for Large-Scale Automotive Software Requirements](http://arxiv.org/abs/2606.17197v1) — 相关度 0.70：提出聚类感知的双层测试规格生成管线，解决大规模需求下集成测试覆盖，可迁移到代理测试生成。
- [PromptMN: Pseudo Prompting Language](http://arxiv.org/abs/2606.17164v1) — 相关度 0.70：提出结构化伪提示语言减少指令歧义，可提升代码代理的上下文解析与可审查性。
- [Understanding Automated Web GUI Testing: An Empirical Study Across Exploration Strategies and State Abstractions](http://arxiv.org/abs/2606.16650v1) — 相关度 0.65：系统比较多种探索策略与状态抽象对Web GUI测试的影响，方法论对代码代理测试策略设计有参考价值。
- [Bistable by Construction: Wall-Clock-Calibrated State Monitors Have No Moment-Detection Regime at Agent Cadence](http://arxiv.org/abs/2606.19386v1) — 相关度 0.60：研究agent流上的运行时监控器校准机制，指出wall-clock vs sample-time的陷阱，对代码agent的可靠监控有迁移价值。
- [Toward Self-Evolution-Ready Workflow Harnesses: A Reversible Migration Path and Convertibility Taxonomy for Expert LLM Pipelines](http://arxiv.org/abs/2606.24598v1) — 相关度 0.60：提出专家LLM管道迁移为自适应工作流的路径与可转换性分类，对代理工作流设计有借鉴意义。
