...
### [1] [Two Truths and A Lie? Benchmarking Off-the-Shelf LLMs for Requirements Quality Assessment: Performance, False Alarms, and Misses](https://arxiv.org/abs/2609.03230)
...
### [4] [Caught in the Story: Narrative Captivity in Multi-turn LLMs Conversation](https://arxiv.org/abs/2609.03407)
...
Selection reason: 提出将知识图谱作为概率信念并支持agent贝叶斯更新、干预与信念交换的世界模型愿景，对agent不确定性规划有启发性但尚未实现，仅作一般跟踪。

TL;DR: 提出语义贝叶斯世界模型（SBWM），将知识图谱从事实数据库转变为共享信念网，以解决概率推理系统与精确知识图谱之间的不匹配，并列出实现所需的关键组件。


<details>
  <summary>Details</summary>
Motivation: 知识图谱的精确断言与基础模型/智能体的概率推理之间存在根本性不匹配，导致当前集成只是数据供给而非统一推理架构。

Method: 设想SBWM：本体公理约束先验，观测经贝叶斯条件作用更新信念，行动干预世界；并通过多个应用场景展示其收益。

Result: 智能体可从该模型获得多种能力，如判断门口人物身份、按蕴含聚合精算估计、完成语言模型失败的规划任务、估计无文档提及的数量。

Conclusion: 要实现SBWM，社区需构建RDF 1.2信念标注、概率蕴含机制、语义校准层及智能体间交换与分歧校准信念的协议。

中文摘要：知识图谱以精确的断言描述现实，而现在消费它们的系统——基础模型和自主智能体——则以概率进行原生推理。我们认为，这种不匹配正是语言模型与知识图谱的整合仍然是一条数据供给流水线，而非统一推理架构的原因。我们设想了一种语义贝叶斯世界模型（SBWMs）：一个不是将世界描述为事实数据库，而是描述为知识图谱上共享的、不断演化的信念织体的Web，其中本体论公理约束先验，观测通过贝叶斯条件作用更新信念，行动则对世界进行干预。我们详细阐述了一个智能体从这样的模型中获得了什么：一个家庭安防智能体判断门口的人影是快递员还是窃贼；一种通过蕴含而非字符串频率聚合的精算估计；一个语言模型确实会失败的规划任务；以及估计任何文档都未曾陈述过的数量。然后，我们列出了社区必须构建什么才能使它们成为可能：基于RDF 1.2的信念标注、概率蕴含机制、语义校准层，以及让素未谋面的智能体能够交换并在校准信念上产生分歧的协议。

Original abstract: Knowledge graphs describe reality in crisp assertions, while the systems now consuming them, foundation models and autonomous agents, reason natively in probabilities. We argue that this mismatch is why the integration of language models and knowledge graphs remains a data-feeding pipeline rather than a unified reasoning architecture. We envision Semantic Bayesian World Models (SBWMs): a Web that describes the world not as a database of facts but as a shared, evolving fabric of beliefs over knowledge graphs, where ontological axioms constrain priors, observations update beliefs by Bayesian conditioning, and actions intervene upon the world. We work through what an agent gains from such a model: a home-security agent deciding whether the figure at the gate is a courier or a burglar, an actuarial estimate aggregated by entailment rather than by string frequency, a planning task that language models reliably fail, and the estimation of quantities that no document has ever stated. We then set out what the community must build to make them possible: belief annotation over RDF~1.2, probabilistic entailment regimes, semantic calibration layers, and protocols by which agents that have never met can exchange, and disagree over, calibrated beliefs.

</details>


### [8] [STAIR (STructure Aware Information Retriever): A novel dataset and LLM based retriever for document structure augmentation](https://arxiv.org/abs/2609.03874)
...
Selection reason: 提出LLM单条推荐可靠性的认知分级证书，可迁移到coding agent输出可靠性与验证评估，但面向通用推荐而非agent执行。

TL;DR: 提出认识论保证（epistemic warrant）构念，通过四层依赖证书评估LLM成对建议的可靠性，并验证其有效性及独特性。


<details>
  <summary>Details</summary>
Motivation: 现有评估多关注模型整体属性或用户信任，缺乏针对单个建议是否值得依赖的理论依据，尤其在客观真值不可用时难以决策。

Method: 借鉴认识论，定义决策层面的认识论保证，操作化为不稳定、情境依赖、局部支持、广泛支持四层证书；用已知组测试和众包共识验证，并比较其与口头信心及决策难度的区分度。

Result: 已知组测试恢复专家排序，更强保证与众包共识一致，且保证信息区别于口头信心，不随决策难度变化。

Conclusion: 该框架为无客观真值环境下个体LLM建议的依赖判断提供了理论支撑和实用方法。

中文摘要：大型语言模型越来越多地被用于支持组织决策，然而用户往往缺乏一个基于原则的基础来评估是否应依赖某一具体建议。现有方法通常评估模型的广泛属性，如可靠性、不确定性或稳健性，或者关注用户信任，而不是依赖单个建议背后的依据。借鉴认识论的理论基础，我们引入了认识论保证（epistemic warrant），这是一种决策层面的构念，用于刻画模型偏好的稳定性以及该偏好成立的范围。我们通过一个针对成对建议的四层依赖证书来操作化这一构念，区分为不稳定、情境依赖、局部支持和广泛支持的建议。我们使用当代方法论验证了这一构念：已知组测试成功恢复了专家预设的保证排序，更强的保证与来自众包工作者的独立共识系统性一致。此外，我们证明认识论保证提供了与口头表达的信心不同的信息，并且不能由决策难度轻易解释。最终，该框架提供了一种有理论依据且可实施的方法，用于在客观真值不可用时刻画单个LLM建议的保证。

Original abstract: Large language models are increasingly used to support organizational decisions, yet users often lack a principled basis for assessing whether to rely on a specific recommendation. Existing approaches typically evaluate broad model properties, such as reliability, uncertainty, or robustness, or focus on user trust, rather than the underlying basis for relying on an individual recommendation. Adapting theoretical foundations from epistemology, we introduce epistemic warrant, a decision-level construct that characterizes the stability of a model's preference and the scope over which that preference holds. We operationalize this construct through a four-tier reliance certificate for pairwise recommendations, distinguishing among unstable, context-dependent, locally supported, and broadly supported recommendations. We validate the construct using contemporary methodologies: known-groups tests successfully recover expert-prespecified warrant orderings, and stronger warrants systematically align with independent consensus from crowd workers. Furthermore, we demonstrate that epistemic warrant provides information distinct from verbalized confidence and is not readily explained by decision difficulty. Ultimately, this framework offers a theoretically grounded, implementable approach for characterizing the warrant of individual LLM recommendations when objective ground truth is unavailable.

</details>


### [10] [From Deceptive Outputs to Deceptive Mechanisms: A Causal Framework for Language-Model Deception Research](https://arxiv.org/abs/2609.04166)
...
Selection reason: 用因果框架区分LLM欺骗行为与内部机制，并在控制实验中给出实证，对agent可靠性与安全性测评有借鉴价值。

TL;DR: 本文通过引入因果分类法并设计受控实验，区分了语言模型的欺骗性行为与欺骗性机制，发现欺骗性行为不一定源于拟议机制，但接收者信息状态可因果影响欺骗偏好，且此类证据不足以确立模型能动性。


<details>
  <summary>Details</summary>
Motivation: 现有研究和报道常将人类心理状态概念归于语言模型，导致行为层面的“看似欺骗”与机制层面的“实际欺骗”被混淆，缺乏清晰的因果区分和实证检验。

Method: 提出一套因果分类法，涵盖先验承诺/事后报告、模型偏好/实现输出、虚假偏好/误导效用敏感性、欺骗行为/目标或策略出处等维度，并在两个开放权重模型家族的受控猜谜游戏和股票交易任务中开展实验，通过特定干预操纵接收者信息状态等变量。

Result: 欺骗性行为可在无对应拟议机制时出现，而针对接收者信息状态的干预能因果性地改变模型的欺骗性偏好，说明行为证据与机制证据可分离且需谨慎解读。

Conclusion: 欺骗性行为确实能为欺骗性机制的存在提供证据，但即使证实了机制，也不能据此认定模型具有人类意义上的欺骗能动性或主体性。

中文摘要：关于语言模型欺骗的研究和新闻报道越来越多地将类似人类的心理状态概念归因于语言模型。这种说法可能模糊了看似具有欺骗性的行为与实际上具有欺骗性的机制之间的区别。我们引入了一种因果分类法，将先验承诺与事后报告、模型偏好与实现输出、虚假偏好与对误导接收者效用的敏感性、以及欺骗行为与产生该行为的目标或策略的出处区分开来。我们在两个开放权重模型家族中测试了这些区别。在受控的猜谜游戏和股票交易实验中，我们发现看似具有欺骗性的行为可能在缺乏相应拟议机制的情况下出现，而其他干预措施则提供了直接证据，表明接收者的信息状态可以因果性地影响欺骗性偏好。这些结果表明，欺骗性行为可以为欺骗性机制提供证据。但即使有这种机制的证据，也不能确立模型在欺骗中的能动性。

Original abstract: Research and news coverage of language-model deception increasingly attributes human-like mental-state concepts to language models. Such claims can blur the distinction between behavior that looks deceptive and a mechanism that is actually deceptive.
