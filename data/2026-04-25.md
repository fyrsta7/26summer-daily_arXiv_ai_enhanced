...
- [CUJBench: Benchmarking LLM-Agent on Cross-Modal Failure Diagnosis from Browser to Backend](http://arxiv.org/abs/2604.23455v2) — 相关度 0.95：首个跨模态故障诊断基准，直接评估LLM agent的故障定位能力，方法有迁移价值。
- [RAT: RunAnyThing via Fully Automated Environment Configuration](http://arxiv.org/abs/2604.23190v2) — 相关度 0.90：RAT实现跨语言仓库级自动化环境配置，直接解决coding agent的沙箱与环境依赖瓶颈，并配套基准RATBench。
- [Can LLMs be Effective Code Contributors? A Study on Open-source Projects](http://arxiv.org/abs/2604.23340v1) — 相关度 0.85：用验证与测试框架评估LLM在真实开源项目中的代码贡献能力，识别失败模式，为agent代码修复提供实证参考。
- [Code Broker: A Multi-Agent System for Automated Code Quality Assessment](http://arxiv.org/abs/2604.23088v2) — 相关度 0.80：多agent代码质量评估系统Code Broker，结合LLM与静态分析，可用于自动化代码审查与质量反馈。
- [No Test Cases, No Problem: Distillation-Driven Code Generation for Scientific Workflows](http://arxiv.org/abs/2604.23106v1) — 相关度 0.80：无测试用例场景下基于蒸馏的多agent科学代码生成框架，提出CCW上下文窗口缓解幻觉，可迁移到agent推理一致性。
- [An Empirical Evaluation of Locally Deployed LLMs for Bug Detection in Python Code](http://arxiv.org/abs/2604.23361v1) — 相关度 0.75：评估本地LLM在Python bug检测中的表现，对代码agent的bug定位能力有参考价值。
- [ArgRE: Formal Argumentation for Conflict Resolution in Multi-Agent Requirements Negotiation](http://arxiv.org/abs/2604.23124v1) — 相关度 0.75：将Dung形式论证嵌入多agent需求协商，提供可审计的冲突消解机制，可迁移到多agent协作与评审流程。
- [RANalyzer: Automated Continuous RAN Software Evaluation and Regression Analysis](http://arxiv.org/abs/2604.23153v1) — 相关度 0.70：结合LLM语义提取与残差分析的自动化测试回归框架，可用于软件持续评测与性能归因。
- [Towards Agentic Test-Driven Quality Assurance for 6G Networks](http://arxiv.org/abs/2604.23285v1) — 相关度 0.65：提出agentic测试驱动QA编排框架，从意图生成验证测试，虽面向6G网络但范式可迁移到软件开发agent。
- [Empirical Insights of Test Selection Metrics under Multiple Testing Objectives and Distribution Shifts](http://arxiv.org/abs/2604.23342v1) — 相关度 0.60：大规模实证评估15种测试选择指标在多目标OOD场景下的表现，可指导agent测试输入筛选与验证策略。
