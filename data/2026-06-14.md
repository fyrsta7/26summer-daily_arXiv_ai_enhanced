...
- [LLM-as-Code: Agentic Programming for Agent Harness](http://arxiv.org/abs/2606.15874v2) — 相关度 0.90：主张由程序控制流主导、LLM作为被调用组件，为agent harness设计提供颠覆性架构，明显可迁移。
- [Open-SWE-Traces: Advancing Dual-Mode Multilingual Distillation for Software Engineering Agents](http://arxiv.org/abs/2606.16038v1) — 相关度 0.90：发布20万条多语言SWE agent轨迹数据集并验证蒸馏效果，直接服务代码agent训练与评测。
- [Configuration Smells in AGENTS.md Files: Common Mistakes in Configuring Coding Agents](http://arxiv.org/abs/2606.15828v4) — 相关度 0.85：首次系统化定义AGENTS.md配置文件smells并给出检测启发式，直接指导coding agent配置修复。
- [Snyk VulnBench JS 1.0: Can LLMs Find the Same Bugs Twice?](http://arxiv.org/abs/2606.15762v1) — 相关度 0.80：构建漏洞扫描可重复性基准，说明agentic LLM与SAST互补，对代码安全agent评测有直接价值。
- [Software Delegation Contracts: Measuring Reviewability in AI Coding-Agent Work](http://arxiv.org/abs/2606.17099v1) — 相关度 0.80：通过受控实验测量软件委托契约对coding agent可审查性的影响，为agent工作包评审机制提供实证。
- [DynAMO:Dynamic Asset Management Orchestration via Topological Multi-Agent Scheduling](http://arxiv.org/abs/2606.19382v1) — 相关度 0.70：Plan-then-Execute多agent调度，支持并行拓扑执行和结构化上下文剪枝，可迁移到代码agent工作流编排。
- [Graphical-Probabilistic Modeling of Generative Flows in LLM-Native Software Systems](http://arxiv.org/abs/2606.15943v1) — 相关度 0.70：用图概率模型对LLM原生软件的生成流建模，可为agent系统设计分析与评测提供形式化基础。
- [Green SARC: Predictive Cost and Carbon Governance for Agentic AI Systems](http://arxiv.org/abs/2606.15954v1) — 相关度 0.70：在agent循环中设置成本/碳预算门控并用保形校准预测，其控制机制可迁移到代码agent的资源治理。
- [AuditWeave: A Tamper-Evident, Auditor-Navigable Evidence Layer for AI-Assisted and Data-Transformation Workflows](http://arxiv.org/abs/2607.09682v1) — 相关度 0.65：为AI与数据工作流提供防篡改、可审计的哈希链证据层，可增强代码agent的可追溯与审计能力。
