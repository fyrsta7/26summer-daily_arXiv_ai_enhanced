...
- [CUDABeaver: Benchmarking LLM-Based Automated CUDA Debugging](http://arxiv.org/abs/2605.08455v2) — 相关度 0.95：CUDA调试benchmark，强调性能保持的修复评价，直接面向LLM agent调试。
- [SWE Atlas: Benchmarking Coding Agents Beyond Issue Resolution](http://arxiv.org/abs/2605.08366v1) — 相关度 0.95：覆盖问答、测试编写、重构的多工作流coding agent基准，评估正确性与工程质量。
- [Kernel Foundry: A Diagnosis-driven Evolutionary Kernel Optimizer with Multi-Experts](http://arxiv.org/abs/2605.30359v1) — 相关度 0.92：提出诊断驱动的进化GPU kernel优化框架Kernel Foundry，结合多专家检索初始化与结构化工反馈，直接属于自动程序性能优化。
- [RepoZero: Can LLMs Generate a Code Repository from Scratch?](http://arxiv.org/abs/2605.07122v3) — 相关度 0.92：提出RepoZero基准，以执行等价性验证agent从零生成整个代码仓库，并设计ACE迭代测试生成框架，直接推进仓库级代码合成评测。
- [A Dataset of Agentic AI Coding Tool Configurations](http://arxiv.org/abs/2605.08435v1) — 相关度 0.90：大型AI编码工具配置数据集，支持context engineering与agent采纳研究。
- [Coding Agents Don't Know When to Act](http://arxiv.org/abs/2605.07769v1) — 相关度 0.90：提出FixedBench评测agent面对已修复问题是否会不当行动，揭示action bias，对coding agent决策与评测至关重要。
- [VeriContest: A Competitive-Programming Benchmark for Verifiable Code Generation](http://arxiv.org/abs/2605.08553v1) — 相关度 0.90：可验证代码生成benchmark，分离规范、代码、证明评估，揭示关键瓶颈。
- [MASPrism: Lightweight Failure Attribution for Multi-Agent Systems Using Prefill-Stage Signals](http://arxiv.org/abs/2605.07509v2) — 相关度 0.85：用prefill阶段信号做轻量级多agent失败归因，零输出token且速度快，直接适用于长程agent执行日志诊断。
- [Similar Pattern Annotation via Retrieval Knowledge for LLM-Based Test Code Fault Localization](http://arxiv.org/abs/2605.07957v1) — 相关度 0.85：基于检索的LLM测试代码故障定位，可提升CI环境中agent调试效率。
- [Mage: Multi-Axis Evaluation of LLM-Generated Executable Game Scenes Beyond Compile-Pass Rate](http://arxiv.org/abs/2605.07342v1) — 相关度 0.82：构建多轴代码生成评测协议，证明编译通过率与功能正确性可能反相关，直接适用于代码生成agent评测。
- [Tool Calling is Linearly Readable and Steerable in Language Models](http://arxiv.org/abs/2605.07990v2) — 相关度 0.80：揭示工具调用的线性可读/可操控方向，可用于agent工具选择的误差检测与干预。
- [Unsafe by Flow: Uncovering Bidirectional Data-Flow Risks in MCP Ecosystem](http://arxiv.org/abs/2605.07836v1) — 相关度 0.80：MCP双向数据流静态分析框架，直接增强LLM代理工具调用的安全性与可靠性。
- [Boosting Automatic Java-to-Cangjie Translation with Multi-Stage LLM Training and Error Repair](http://arxiv.org/abs/2605.07403v1) — 相关度 0.75：针对低资源语言Java到仓颉翻译的多阶段LLM训练与基于编译器反馈的错误修复，其迭代修复机制可迁移到代码agent。
- [Do not copy and paste! Rewriting strategies for code retrieval](http://arxiv.org/abs/2605.08299v1) — 相关度 0.75：系统研究代码检索的重写策略与代价收益，提出Delta H作为便宜预测指标，对RAG式代码检索agent有直接帮助。
- [Bridging the Programming Language Gap: Constructing a Multilingual Shared Semantic Space through AST Unification and Graph Matching](http://arxiv.org/abs/2605.07788v1) — 相关度 0.70：通过AST统一与图匹配构建跨语言共享语义空间，显著改进跨语言代码检索与克隆检测，可迁移到代码检索agent。
- [Can LLMs Solve Science or Just Write Code? Evaluating Quantum Solver Generation](http://arxiv.org/abs/2605.07525v2) — 相关度 0.70：评测LLM生成量子求解器的迭代细化与数值正确性，强调执行成功不等于科学正确，对代码生成验证有借鉴意义。
- [Characterizing and Mitigating False-Positive Bug Reports in the Linux Kernel](http://arxiv.org/abs/2605.07678v1) — 相关度 0.70：系统刻画Linux内核误报bug报告并用RAG缓解，其false-positive识别方法可迁移到代码修复agent的issue triage。
- [Collaborator or Assistant? How AI Coding Agents Partition Work Across Pull Request Lifecycles](http://arxiv.org/abs/2605.08017v2) — 相关度 0.70：提出AI编码代理在PR生命周期中的协作者-助手分类法，为agent治理设计提供参考。
- [From Assistance to Agency: Rethinking Autonomy and Control in CI/CD Pipelines](http://arxiv.org/abs/2605.07062v1) — 相关度 0.70：提出agentic CI/CD中数据面与控制面权威转移的愿景与研究议程，为软件工程agent的自主性设计提供概念框架。
- [SARC: A Governance-by-Architecture Framework for Agentic AI Systems](http://arxiv.org/abs/2605.07728v1) — 相关度 0.70：提出面向工具型agent的运行时治理架构，将约束编入执行回路并支持审计，对多agent可靠性与约束执行有迁移价值。
- [SkillSelect-Serve: QoS-Aware Budgeted Skill Service Recommendation for LLM Agents](http://arxiv.org/abs/2607.00011v2) — 相关度 0.70：将agent技能作为服务进行预算、风险和工具约束感知的推荐，对代码agent技能选择与工作流编排有直接参考价值。
- [Securing Computer-Use Agents: A Unified Architecture-Lifecycle Framework for Deployment-Grounded Reliability](http://arxiv.org/abs/2605.07110v1) — 相关度 0.68：提出计算机使用agent的架构-生命周期可靠性框架，涵盖感知、决策、执行与权限控制，安全与监控机制可迁移到代码agent。
- [CellScientist: Dual-Space Hierarchical Orchestration for Closed-Loop Refinement of Virtual Cell Models](http://arxiv.org/abs/2605.07335v1) — 相关度 0.65：提出闭环的假设-实现双层LLM建模精化框架，将执行失败路由到可修正环节，对代码agent的自我修正与失败归因有迁移价值。
- [LLMSYS-HPOBench: Hyperparameter Optimization Benchmark Suite for Real-World LLM Systems](http://arxiv.org/abs/2605.08305v1) — 相关度 0.65：发布真实LLM系统HPO基准套件，覆盖推理目标与成本指标，对LLM推理系统调优与benchmark有参考价值。
