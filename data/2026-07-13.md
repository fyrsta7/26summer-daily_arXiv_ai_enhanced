...
- [AutoTrace: From Patches to Triggers via Agentic Interprocedural Exploration](http://arxiv.org/abs/2607.12058v1) — 相关度 0.95：agentic跨过程漏洞触发定位，融合LLM决策与可验证证据，对代码agent安全分析有直接价值。
- [Self-Improving AI Coding Agents Through Accumulated Behavioral Rules: A Closed-Loop Framework](http://arxiv.org/abs/2607.13091v1) — 相关度 0.95：闭环规则积累使编码agent跨会话改进，0%复发率，对agent持久记忆与反思有直接价值。
- [Which Optimizer, At What Budget? A Tournament of Optimizers for Search-Based SE](http://arxiv.org/abs/2607.11705v1) — 相关度 0.95：系统比较20种优化器并给出预算相关的选择指南，直接支撑自动调优。
- [BackendForge: Benchmarking Agentic End-to-End Code Generation with Backend Services](http://arxiv.org/abs/2607.11042v1) — 相关度 0.90：面向后端服务端到端代码生成的agentic benchmark，强调可执行部署与契约测试，对代码agent评测有直接价值。
- [Beyond Test Presence: Assessing the Quality and Robustness of Agent-Generated Tests in Open-Source Projects](http://arxiv.org/abs/2607.12068v1) — 相关度 0.90：大规模评估agent生成测试的断言强度、边界覆盖与脆弱性，揭示stealth debt，对agent测试评估有直接借鉴。
- [Cross-Cutting Security Analysis of LLM-Generated Code via Metamorphic Testing and Association Rule Mining](http://arxiv.org/abs/2607.12089v1) — 相关度 0.90：用蜕变测试+关联规则挖掘LLM生成代码的跨切安全漏洞，并追溯提示级风险，可迁移到代码agent漏洞验证。
- [FlowArk: Boosting Agentic Data-flow Analysis for Android Apps via Context-Aware Knowledge Reuse](http://arxiv.org/abs/2607.11308v1) — 相关度 0.85：利用上下文感知知识复用来降低批量agentic数据流分析的API成本，是明确的代码agent效率优化方法。
- [Know Before Fix: QA-Driven Repository Knowledge Acquisition for Software Issue Resolution](http://arxiv.org/abs/2607.11111v1) — 相关度 0.85：提出QA驱动的仓库知识获取框架，将知识获取与补丁生成解耦，显著提升SWE-bench修复性能，对代码agent有清晰迁移价值。
- [Phantom Guardrails: When Self-Improving Agent Harnesses Fix Failures That Never Happened](http://arxiv.org/abs/2607.13083v1) — 相关度 0.85：研究自改进agent在harness优化中虚构故障的现象，并引入可复现的评测实验台，对agent可靠性机制有重要参考价值。
- [Predicting Acceptance and Review Effort in Human and Agent Pull Requests](http://arxiv.org/abs/2607.12057v1) — 相关度 0.85：从提交时特征预测agent/人类PR接受度与评审工作量，可用于triage与评估。
- [Retrieval-Oriented Code Representations in Agentic Bug Localization](http://arxiv.org/abs/2607.11046v1) — 相关度 0.85：系统比较代码表示对文件级bug定位的影响，并在Agentless管线中验证提升，可直接迁移到agent检索与定位设计。
- [TerraRepair: A Tool-Grounded LLM Agent for Infrastructure-as-Code Repair](http://arxiv.org/abs/2607.11390v1) — 相关度 0.85：工具增强的IaC修复agent，通过检索schema和重扫验证提升修复率，是代码agent在基础设施领域的直接应用。
- [AgentCheck: A Reproduce-Intervene-Mitigate Workbench for LLM Agents over MCP](http://arxiv.org/abs/2607.11098v3) — 相关度 0.80：针对MCP工具型agent的可复现故障注入与缓解验证工作台，提供修复前验证机制，可迁移到agent可靠性评测。
- [OpsMem: Dual-Memory Reasoning with Cross-Memory Resonance for Failure Diagnosis](http://arxiv.org/abs/2607.11357v1) — 相关度 0.80：双记忆LLM agent故障诊断框架，通过跨记忆共振提升诊断准确率，可直接迁移到agent记忆与推理设计。
- [Fail-Aware and Explainable Test Oracle Prediction](http://arxiv.org/abs/2607.11342v1) — 相关度 0.75：基于LLM的失败感知判别式测试预言预测，能补充fuzzing/测试生成，对agent测试有迁移价值。
- [Knowledge-Guided Synthetic Bug Feedback for LLM-Based Unit Test Generation](http://arxiv.org/abs/2607.11573v1) — 相关度 0.75：利用真实缺陷机制指导LLM生成单元测试的合成bug反馈，提升缺陷检测，对agent测试生成有参考价值。
- [Complexity Theory of Randomised Testing](http://arxiv.org/abs/2607.11811v1) — 相关度 0.65：为随机/属性测试提供复杂度理论，有助于设计agent测试生成器。
- [Fault Injection in OpenAPI Specifications for Evaluating Black-Box Testing Effectiveness](http://arxiv.org/abs/2607.12101v1) — 相关度 0.60：对OpenAPI规范注入故障评估黑盒测试工具，提供规范缺陷分类与评测方法，可借鉴于API测试agent验证。
- [Inference Economics of Enterprise Coding Agents: A Case Study of Cloud vs. On-Premise LLMs](http://arxiv.org/abs/2607.13080v1) — 相关度 0.60：云与本地LLM编码agent的成本-质量案例研究，揭示缓存成本与缺陷修复负担的权衡，对agent部署经济性有参考价值。
- [TestMiner: Software Testing Analysis for GitHub Repositories](http://arxiv.org/abs/2607.12223v1) — 相关度 0.55：提供GitHub项目测试实践的多维概览工具，可用于评估agent测试习惯，但迁移价值有限。
- [ThinkLog: Leveraging Reasoning for Log Statement Generation](http://arxiv.org/abs/2607.11615v1) — 相关度 0.55：利用推理链提升日志语句生成准确率并降低推理成本，其中的提示工程技巧可迁移到代码生成agent。
