...
- [SWE-Together: Evaluating Coding Agents in Interactive User Sessions](http://arxiv.org/abs/2606.29957v1) — 相关度 0.95：基于真实多轮人机会话的 coding agent benchmark，包含用户模拟器和纠错反馈评估，直接相关。
- [Loc2Repair: A Framework for Evaluating the Impact of File-Level Issue Localization in Repo-Level LLM Repair](http://arxiv.org/abs/2606.30963v1) — 相关度 0.92：提出仓库级LLM修复的模块化评测框架，将文件级定位与修复解耦并量化其影响，为SWE agent的定位-修复链路评测提供实用工具。
- [ATM: CID-Brokered Pre-Write Admission for Multi-Agent Code Co-Synthesis](http://arxiv.org/abs/2607.00041v1) — 相关度 0.85：为多 agent 代码协合提供并发写意图准入、原子化与验证链，直接针对代码 agent 的共享变更治理。
- [MicroAgent: Context-Augmented Multi-Agent Framework for Automatic Microservice Decomposition](http://arxiv.org/abs/2606.29742v1) — 相关度 0.80：多智能体框架将微服务分解任务委派给专门 agent，展示 agent 规划与上下文增强的软件工程应用。
- [SpreadsheetBench 2: Evaluating Agents on End-to-End Business Spreadsheet Workflows](http://arxiv.org/abs/2606.29955v1) — 相关度 0.80：端到端业务表格工作流级智能体 benchmark，覆盖多表跨表任务并提供失败分类，直接用于 agent 评测。
- [Towards Knowledge Alignment in Code LLMs: Contrastive Unlearning for Evolving APIs](http://arxiv.org/abs/2606.30810v1) — 相关度 0.78：针对代码LLM因API演进产生废弃调用的问题，提出对比去学习同时抑制旧API并促进正确替代，可迁移到代码生成agent的知识更新与修复。
- [MCP Server Architecture Patterns for LLM-Integrated Applications](http://arxiv.org/abs/2606.30317v1) — 相关度 0.75：总结 MCP server 架构模式与反模式，为 coding agent 工具集成和 tool selection 提供直接参考。
- [EvalSafetyGap: A Hybrid Survey and Conceptual Framework for LLM Evaluation-Safety Failures](http://arxiv.org/abs/2606.30219v2) — 相关度 0.70：提出 evaluation-safety gap 与对齐三难等分析框架，可迁移到代码 agent 评测可靠性与 reward hacking。
- [The Illusion of Agentic Complexity in README.md Generation: Evaluating Single-Agent vs. Multi-Agent RAG Systems](http://arxiv.org/abs/2606.30524v2) — 相关度 0.70：系统比较单/多 agent RAG 生成文档，给出架构复杂度与 token 成本权衡，对 agent 设计有参考价值。
- [How do Execution Features Improve Statistical Fault Localization? An Empirical Study](http://arxiv.org/abs/2606.30324v1) — 相关度 0.65：实验验证执行特征可增强统计故障定位，可帮助调试和修复 agent 提升定位精度并降低检查成本。
- [PyMETA: A Benchmark Dataset for Hierarchical Student Code Error Classification with Python-Interpreter-Based Labels](http://arxiv.org/abs/2606.30610v1) — 相关度 0.60：提供分层 Python 代码错误分类数据集与多错误标注，可用于代码 agent 调试和错误诊断评测。
- [A Taxonomy of Single-Turn Textual Prompt Patterns](http://arxiv.org/abs/2607.00043v1) — 相关度 0.55：系统梳理单轮文本提示模式的分类法，为代码agent的prompt设计与模式复用提供参考。
- [Bash-Commenter: Leveraging Syntax-Aware Preference Optimization to Reinforce Large Language Model for Bash Code Comment Generation](http://arxiv.org/abs/2606.29709v1) — 相关度 0.55：提出面向 Bash 的语法感知偏好优化用于代码注释生成，方法可迁移到代码模型的语义训练。
- [Rehearsed Multi-Agent Live Product Demonstrations with Real-Time Voice Question Answering](http://arxiv.org/abs/2606.30294v1) — 相关度 0.55：多 agent 演示系统的排练循环与 UI/源码跨模态定位机制，可迁移到软件工程 agent 的稳定执行。
