...
Selection reason: 端到端科学工作流agent评测基准，揭示完成声称与真实交付的差距，对agent评测机制有迁移价值。
...
Selection reason: 面向LLM的开源bug发现评测基准，与agent模糊测试和缺陷发现直接相关。
...
Selection reason: 上下文感知LLM近似的程序合成方法，有效改进枚举合成器，可迁移到代码生成与修复。
...
Selection reason: 提出多agent系统失败调试的重放与干预框架，直接针对agent可靠性与修复评估。
...
Selection reason: Secret MCP以证据限定和上下文隔离生成可审计设计规格，其合约与采样抽象可迁移到多参考代码生成agent。
...
### [32] [Evaluating and Preventing Security Smells in AI-Generated Ansible Code](https://arxiv.org/abs/2608.24962)
...
Selection reason: 评估和防止AI生成Ansible代码中的安全smell，通过CO-STAR提示框架实现合成阶段安全拦截，对代码生成质量有启示。
...
### [33] [Model-Based Agentic Software Engineering](https://arxiv.org/abs/2608.25174)
...
Selection reason: 提出面向编码agent的模型驱动治理框架，外部化知识、约束与验证门控，可直接迁移到agent工程。
...
### [34] [SPECMINE: A Large-Scale Corpus of Spec-Driven Development Artifacts](https://arxiv.org/abs/2608.25202)
...
Selection reason: 大规模spec-driven开发语料，关联spec与代码/PR，为研究AI编码agent工件提供宝贵数据集。
...
### [35] [A Few Pages of Markdown: Committed AI Configuration and Lower Quality Cost after Coding-Agent Adoption](https://arxiv.org/abs/2608.25241)
...
Selection reason: 研究编码agent采用后的质量与成熟度模型，对agent工程评测有参考价值。
...
### [36] [Metis: Typed Runtime Mediation for Tool-Using Software Agents](https://arxiv.org/abs/2608.25322)
*Jun Yu*
...
Selection reason: 工具使用agent的类型化运行时调解与权限验证，直接涉及coding agent的工具反馈与安全执行。
...
### [37] [RotDroid: Cross-Orientation State Equivalence Testing for Detecting GUI Rotation Bugs in Android Apps](https://arxiv.org/abs/2608.25425)
*Mengdi Qin,Bo Jiang*
...
Selection reason: 跨方向GUI状态等价测试与VLM测试oracle，可迁移到agent驱动的GUI测试场景。
...
### [38] [From General Agents to RCA Experts: A Self-Evolving Harness for Root Cause Analysis](https://arxiv.org/abs/2608.25661)
...
Selection reason: 自演化RCA harness，复用通用agent能力并加验证门控，对代码agent诊断与长期演化有直接借鉴。
...
### [39] [Answer Is Cheap, Show Me the Evidence! Augmenting Automated Vulnerability Assessment with Evidence](https://arxiv.org/abs/2608.25905)
...
Selection reason: 用LLM agents进行漏洞评估并提供证据，结合强化学习训练，可迁移到代码安全分析agent。
...
### [40] [XREPOTEST: Benchmarking Multilingual Repository-Level Unit Test Generation for Large Language Models](https://arxiv.org/abs/2608.25939)
...
Selection reason: 多语言仓库级单元测试生成基准，直接用于评测LLM的测试生成能力。
...
### [41] [Vulnerable Code Search: Transferable Attack for Code Language Models](https://arxiv.org/abs/2608.26031)
...
Selection reason: 对代码检索模型的迁移性对抗攻击，揭示代码检索脆弱性，对agent代码检索安全有参考。
