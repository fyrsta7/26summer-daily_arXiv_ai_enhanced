...
- [ConCovUp: Effective Agent-Based Test Driver Generation for Concurrency Testing](http://arxiv.org/abs/2605.09573v1) — 相关度 0.85：多agent与程序分析结合生成并发测试驱动，显著提升共享内存访问覆盖，对agent测试生成有直接价值。
- [Generating Complex Code Analyzers from Natural Language Questions](http://arxiv.org/abs/2605.09304v1) — 相关度 0.85：融合LLM与CodeQL生成代码分析器，具备RAG迭代与自测技术，直接可迁移到代码理解agent。
- [RubricRefine: Improving Tool-Use Agent Reliability with Training-Free Pre-Execution Refinement](http://arxiv.org/abs/2605.09730v3) — 相关度 0.85：通过执行前语义契约检查提升工具使用agent可靠性，零执行尝试且性能优越，直接适用于代码agent。
- [An Execution-Verified Multi-Language Benchmark for Code Semantic Reasoning](http://arxiv.org/abs/2605.11006v1) — 相关度 0.80：提供执行验证的多语言代码语义推理基准，填补程序结构理解评估空白。
- [An Executable Benchmarking Suite for Tool-Using Agents](http://arxiv.org/abs/2605.11030v2) — 相关度 0.75：可执行的工具使用agent基准套件，明确证据采纳契约，为agent评测提供标准化框架。
- [Evaluating Tool Cloning in Agentic-AI Ecosystems](http://arxiv.org/abs/2605.09817v2) — 相关度 0.75：针对 agent 工具生态的克隆检测与多样性评估，对代码 agent 工具数据集和 benchmark 构建有直接可迁移价值。
- [Your Simulation Runs but Solves the Wrong Physics: PDE-Grounded Intent Verification for LLM-Generated Multiphysics Simulation Code](http://arxiv.org/abs/2605.09360v1) — 相关度 0.75：基于PDE意图验证纠正LLM生成的仿真代码，区分可执行性与语义正确性，对代码验证有启示。
- [SmartEval: A Benchmark for Evaluating LLM-Generated Smart Contracts from Natural Language Specifications](http://arxiv.org/abs/2605.09610v1) — 相关度 0.70：面向智能合约生成的多维度评测基准，提供可复现的评估流程与失败模式分析。
- [Trajectory Supervision for Continual Tool-Use Learning in LLMs](http://arxiv.org/abs/2605.09734v1) — 相关度 0.65：轨迹监督用于持续工具学习，探索中间过程的保留价值，对agent学习方法有参考，但规模有限。
- [MACAA: Belief-Revision Multi-Agent Reasoning for Code Authorship Verification](http://arxiv.org/abs/2605.09421v3) — 相关度 0.60：基于信念修正的多agent推理框架可用于代码溯源验证，其协作推理机制可迁移至代码agent。
