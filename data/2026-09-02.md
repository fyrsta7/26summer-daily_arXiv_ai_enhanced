...
Selection reason: 长程依赖API工作流agent基准，分解工程能力、确定性验证与执行轨迹，对coding agent评测高度相关。
...
Selection reason: 小递归模型代码生成的评估方法（teacher-forcing vs free-running）对代码生成模型性能分析有参考价值。
...
Selection reason: 多轮生成UI维护基准，强调跨轮保留与外部状态同步，可迁移到代码agent长程维护任务。
...
Selection reason: 对rustc类型检查健全性缺陷的实证研究，涉及程序分析与验证，对代码agent正确性验证和bug检测有方法参考价值。
...
Selection reason: 系统研究coding agent验证工具集（linter、boot probe、shell、screenshot）对代码质量的影响，对agent工具设计和工作流优化直接相关。
...
Selection reason: 研究LLM生成UML类图的缺陷与修复策略，属于软件建模与设计生成，对代码生成agent有可迁移的评估与修复经验。
...
Selection reason: 提出面向vibe-coding应用的意图约束语言与确定性验证机制，用CodeQL检查行为正确性，对代码agent验证和反馈有明确迁移价值。
...
Selection reason: 工业界使用Claude Code迁移遗留系统的案例研究，实证了agent在真实场景下的效果与成本，对代码agent工程化有参考价值。
...
Selection reason: 发布GitHub Copilot云代理的大规模执行日志数据集，为研究agent行为、失败模式和协作提供关键资源，对代码agent评测和过程分析直接相关。
...
Selection reason: 提出跨代码库维护共享库的Super Library Agent，解决多应用生成中重复与结构退化问题，对代码agent的长程规划和代码复用有明确迁移价值。
...
Selection reason: 构造面向交互式编程问题的基准InteractBench，评估LLM生成代码在动态信息下的状态追踪与协议遵守能力，对代码agent评测与方法研究直接相关。
...
Selection reason: 研究将仓库探索阶段委托给低成本模型并系统评估对下游定位的影响，为模块化coding agent的成本与质量权衡提供直接方法。
...
Selection reason: 报告AI代理驱动VST验证liblzma内存安全的全流程与证明工程经验，对代码agent辅助形式化验证和正确性保证有显著迁移价值。
...
Selection reason: 首次针对LLM生成数据科学代码的执行效率构建基准，提出效率诊断与路由优化，对自动程序性能优化和代码生成agent有直接价值。
...
Selection reason: 提出依赖升级场景下隐藏破坏的基准DEPBENCH并评估主流coding agent，揭示了agent在真实维护任务中的能力差距，对agent鲁棒性评测直接相关。
...
Selection reason: 自动挖掘生态级API更新映射与客户端更新实例，为库更新和agent代码修复提供数据集与方法，对代码agent的依赖维护任务有迁移价值。
...
Selection reason: 调查开发者对软件能耗的态度与实践，指出现有指标盲区，对软件优化目标的设定和agent的优化策略有参考价值。
...
Selection reason: 提出操作LLM编码智能体的四阶段工作流与上下文管理策略，对软件工程agent的可靠执行有直接迁移价值。
...
Selection reason: 纵向研究LLM对话干预对开发者的影响，提供交互增强与评估方法，可迁移到代码agent交互设计。
...
Selection reason: 通过证明覆盖分析诊断规范质量，方法可迁移到agent生成代码的验证与测试环节。
...
Selection reason: 工业代码生成模型后训练的数据工程实践与评估，直接提升代码agent生成能力，有工程化迁移价值。
