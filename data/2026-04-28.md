...
- [Agentic Harness Engineering: Observability-Driven Automatic Evolution of Coding-Agent Harnesses](http://arxiv.org/abs/2604.25850v4) — 相关度 0.95：观测驱动自动演进 coding-agent harness，闭环提升成功率，直接相关。
- [CoRE: A Fine-Grained Code Reasoning Benchmark Beyond Output Prediction](http://arxiv.org/abs/2604.25399v1) — 相关度 0.90：提出代码推理benchmark CoRE，评估LLM对实现不变性和中间状态的推理，是代码智能评测的重要补充。
- [SWE-Edit: Rethinking Code Editing for Efficient SWE-Agent](http://arxiv.org/abs/2604.26102v2) — 相关度 0.90：将代码编辑接口分解为 Viewer/Editor，降低上下文耦合并提升 SWE-agent 效率，直接相关。
- [Symbolic Execution Meets Multi-LLM Orchestration: Detecting Memory Vulnerabilities in Incomplete Rust CVE Snippets](http://arxiv.org/abs/2605.00034v1) — 相关度 0.90：结合符号执行与多LLM智能体检测Rust不安全代码漏洞，解决不完整代码问题，是程序分析与agent协作的直接应用。
- [SAFEdit: Does Multi-Agent Decomposition Resolve the Reliability Challenges of Instructed Code Editing?](http://arxiv.org/abs/2604.25737v1) — 相关度 0.88：多 agent 分解+可执行验证+失败抽象层的代码编辑框架，直接提升指令编辑可靠性。
- [From Threads to Trajectories: A Multi-LLM Pipeline for Community Knowledge Extraction from GitHub Issue Discussions](http://arxiv.org/abs/2604.25880v1) — 相关度 0.82：多 LLM 管线从 issue 讨论提取结构化轨迹，可用于训练和引导 agent 推理。
- [R$^3$-SQL: Ranking Reward and Resampling for Text-to-SQL](http://arxiv.org/abs/2604.25325v1) — 相关度 0.80：提出Text-to-SQL的ranking与agentic resampling方法，可迁移到代码生成中的候选排序与自我校验。
- [RESTestBench: A Benchmark for Evaluating the Effectiveness of LLM-Generated REST API Test Cases from NL Requirements](http://arxiv.org/abs/2604.25862v1) — 相关度 0.80：提供 RESTful 测试生成基准与需求级突变测试指标，可评估 agent 测试有效性。
- [Bug-Report-Driven Fault Localization: Industrial Benchmarking and Lesson Learned at ABB Robotics](http://arxiv.org/abs/2604.25700v2) — 相关度 0.78：仅凭 bug 报告文本做故障定位的工业评估，对 issue 驱动 agent 有借鉴。
- [LLM-Guided Issue Generation from Uncovered Code Segments](http://arxiv.org/abs/2604.26118v2) — 相关度 0.78：结合覆盖率与 LLM 生成可复现 issue 报告，并验证有效性，可用于自动 bug 发现 agent。
- [MARD: A Multi-Agent Framework for Robust Android Malware Detection](http://arxiv.org/abs/2604.25264v1) — 相关度 0.75：提出多智能体Android恶意软件检测框架，利用LLM编排静态分析工具，多agent协作模式可迁移到代码分析agent。
- [PLMGH: What Matters in PLM-GNN Hybrids for Code Classification and Vulnerability Detection](http://arxiv.org/abs/2604.25599v1) — 相关度 0.75：受控实证揭示 PLM-GNN 混合模型在代码分类/漏洞检测中的设计要点，可指导代码分析 agent。
- [Using Large Language Models for Black-Box Testing of FMU-Based Simulations](http://arxiv.org/abs/2604.25650v1) — 相关度 0.72：用 LLM 生成结构化测试场景并自动执行验证，人机循环机制可迁移到测试 agent。
- [ImproBR: Bug Report Improver Using LLMs](http://arxiv.org/abs/2604.26142v1) — 相关度 0.70：使用LLM自动检测并改进bug报告结构，其混合检测器和RAG方法可迁移到coding agent对issue的理解与预处理。
- [Learning Generalizable Multimodal Representations for Software Vulnerability Detection](http://arxiv.org/abs/2604.25711v2) — 相关度 0.70：多模态对比学习提升漏洞检测泛化性，表征对齐方法可迁移至代码理解 agent。
- [Can Code Evaluation Metrics Detect Code Plagiarism?](http://arxiv.org/abs/2604.25778v1) — 相关度 0.65：系统比较代码评估指标与查重工具，揭示指标在检测修改代码中的能力边界，对代码评测有用。
- [Carbon-Taxed Transformers: A Green Compression Pipeline for Overgrown Language Models](http://arxiv.org/abs/2604.25903v1) — 相关度 0.65：压缩 pipeline 在 SE 任务上大幅降耗而保留精度，对部署代码 agent 有成本优化价值。
- [Commit-Aware Learning-Based Test Case Prioritization for Continuous Integration](http://arxiv.org/abs/2604.25363v2) — 相关度 0.65：基于commit结构的测试用例优先级排序，为CI回归测试提供学习型方法，可辅助agent的测试优化。
- [UCSC-NLP at SemEval-2026 Task 13: Multi-View Generalization and Diagnostic Analysis of Machine-Generated Code Detection](http://arxiv.org/abs/2604.26990v1) — 相关度 0.62：面向机器生成代码检测的多视图训练与类不平衡诊断，可用于 agent 输出鉴别与评测。
- [Scenario-based System Testing for Distributed Robotics Applications](http://arxiv.org/abs/2604.25772v1) — 相关度 0.60：提出场景规范语言与在线测试机制，支持动态配置，对分布式系统测试 agent 有参考价值。
- [AI Observability for Large Language Model Systems: A Multi-Layer Analysis of Monitoring Approaches from Confidence Calibration to Infrastructure Tracing](http://arxiv.org/abs/2604.26152v1) — 相关度 0.55：提出LLM系统多层可观测性分类，对构建可靠agent的监控与追踪有参考价值。
- [GenDetect: Generalizing Reactive Detection for Resilience Against Imitative DeFi Attack Cascade](http://arxiv.org/abs/2604.26094v1) — 相关度 0.55：从已知攻击自动抽象检测规则，思路可迁移至智能合约漏洞检测 agent。
