...
- [SkillOpt-Lite: Better and Faster Agent Self-evolution via One Line of Vibe](http://arxiv.org/abs/2607.03451v1) — 相关度 0.95：提出基于零阶优化的最小化agent技能自进化流程，直接提升代码agent能力并集成到生产环境。
- [Anchored Self-Play for Code Repair](http://arxiv.org/abs/2607.03523v1) — 相关度 0.90：针对代码修复的锚定自博弈训练方法，通过生成-修复循环提升修复率，可直接用于编码agent训练。
- [AtomicCommitBench: Can Coding Agents Reconstruct Commit Histories from Squashed Patches?](http://arxiv.org/abs/2607.03332v1) — 相关度 0.90：提出AtomicCommitBench基准，让编码agent从squashed补丁重构提交历史，直接评估agent生成代码的结构化能力。
- [BeSpec: Behavior-Level Specification Alignment for Code Generation](http://arxiv.org/abs/2607.02949v1) — 相关度 0.90：通过行为模型对齐解决代码生成中的规格歧义，显著提升 Pass@1。
- [CoACT: Action-Preserving Observation Compression for Coding Agents](http://arxiv.org/abs/2607.02911v1) — 相关度 0.90：面向 coding agent 的动作保持观测压缩方法，显著降低 token 消耗且保持任务效果。
- [Copper: Unifying Correctness and Performance Specification in Code Generation](http://arxiv.org/abs/2607.03130v1) — 相关度 0.90：结合形式验证与性能剖析生成既正确又高效的代码，直接契合自动程序优化与验证主题。
- [Diagnosis-Driven Automatic Repair for Agentic Workflow via Symbolic Inference](http://arxiv.org/abs/2607.02882v1) — 相关度 0.90：提出符号推理驱动的 agentic workflow 诊断修复框架，与软件工程 agent 修复直接相关。
- [GameEngineBench: Evaluating Coding Agents on Real C++ Runtime Environments](http://arxiv.org/abs/2607.03525v2) — 相关度 0.90：构建真实C++游戏引擎环境中的编码agent评测基准，提供复杂集成场景下的能力评估。
- [TATG: Tracking-Aware Testing Objective for LLM-based Test Generation](http://arxiv.org/abs/2607.03194v1) — 相关度 0.90：提出跟踪感知的LLM单元测试生成方法，统一覆盖率和变异得分目标，显著提升测试质量，直接相关于代码agent测试。
- [An Empirical Study of Downstream Adaptation for Agent Skills](http://arxiv.org/abs/2607.03238v1) — 相关度 0.85：首次实证研究下游开发者对agent技能的改编模式，为技能设计、演化和编排提供重要启示。
- [Compression, structure, and executor capability: a controlled real-cost decomposition of language-model agent skill optimisation](http://arxiv.org/abs/2607.03048v1) — 相关度 0.85：对agent技能优化进行受控成本分解，揭示执行器能力为主要杠杆，直接关系代码agent的性价比优化。
- [EvoOtter: Evolutionary Reproduction Test Generator](http://arxiv.org/abs/2607.02854v1) — 相关度 0.85：结合进化编程与 LLM 的高效 bug 复现测试生成器，直接面向软件测试 agent。
- [Is Agentic Code Review Helpful? Mining Developers' Feedback to CodeRabbit Reviews in the Wild](http://arxiv.org/abs/2607.03316v1) — 相关度 0.85：大规模实证分析CodeRabbit代理代码评审的开发者反馈，为代码审查agent的有效性改进提供数据支撑。
- [Scientific Code Search at Scale: A Multi-Domain Dataset and Benchmark](http://arxiv.org/abs/2607.05443v1) — 相关度 0.85：发布科学代码检索数据集与基准，覆盖多领域仓库与代码片段检索，可用于增强代码检索与RAG。
- [Which Algorithm Specification Formats Help Language Models Implement Machine Learning Algorithms?](http://arxiv.org/abs/2607.03158v1) — 相关度 0.85：系统比较算法规格格式对LLM实现准确率的影响，对代码生成评测与编写规范有直接指导意义。
- [Effectiveness of LLM-based Software Diversity for Reliability Improvement -- an Empirical Study](http://arxiv.org/abs/2607.03174v1) — 相关度 0.80：实证研究LLM生成代码的多样性对可靠性提升的作用，涉及代码生成与测试验证，具有迁移价值。
- [MOSAIC: Knowledge-Guided CLI Command Composition Attack in LLM Coding Agents](http://arxiv.org/abs/2607.02857v1) — 相关度 0.80：揭示 CLI 命令组合攻击面并构建黑盒评估框架，对 coding agent 安全评测重要。
- [Round-Trip Mutation Testing: Translating Code to Natural Language Intent and back](http://arxiv.org/abs/2607.03223v1) — 相关度 0.80：通过代码与自然语言意图的往返转换生成变异体用于变异测试，对LLM辅助测试和故障检测有明确贡献。
- [Determinants and Limits of LLM Security-Tool Orchestration: A Study with HexStrike-AI](http://arxiv.org/abs/2607.02873v1) — 相关度 0.75：系统评测 LLM 安全工具编排能力并诊断影响因素，提供 agent 评测方法论。
- [Flow-A11y: Flow-Aware Accessibility Testing](http://arxiv.org/abs/2607.03100v1) — 相关度 0.75：用浏览器agent执行自然语言场景并基于运行时轨迹进行可访问性测试，对agent驱动测试有可迁移价值。
- [CAGE-1: Control, Assurance, and Governance Evaluation for Enterprise Agentic AI](http://arxiv.org/abs/2607.03510v1) — 相关度 0.70：面向企业agent的评测框架，覆盖控制、保证与治理，可迁移至代码agent的可靠性评估。
- [AGL-1: The Enterprise AI Governance Layer as a Control Plane for Trusted Enterprise Intelligence](http://arxiv.org/abs/2607.03516v1) — 相关度 0.60：提出企业AI治理层参考模型，包含agent执行控制与审计机制，对agent工作流治理有参考价值。
- [Detecting Architectural Drift in Safety-Critical Firmware through Runtime Trace Analysis](http://arxiv.org/abs/2607.03135v1) — 相关度 0.60：利用运行时轨迹分析和确定性差异比对检测架构漂移，LLM报告仅用于支持审查，可迁移至软件工程分析工具。
- [LogSemFuse: Semantic Evidence Fusion for Explainable Log Anomaly Detection](http://arxiv.org/abs/2607.03599v1) — 相关度 0.60：提出日志异常检测的语义证据融合框架，可增强agent对系统状态的解释和调试反馈。
- [Execution Divergence Graphs:Effective Discovery of Control-Flows from Execution Traces as Fuzzing Feedback](http://arxiv.org/abs/2607.03396v1) — 相关度 0.55：基于执行轨迹构建控制流图指导模糊测试，是程序分析与测试的通用反馈机制，对agent测试反馈设计有参考价值。
- [Builder, Defender, Breaker: The Case Against Removing the Human from the AI-Driven Security Lifecycle](http://arxiv.org/abs/2607.03215v1) — 相关度 0.50：讨论AI安全生命周期中人类监督的必要性，对自动化代码生成和验证的可靠性设计具有参考意义。
- [Prior Bias in Vision Language Models on UML Diagram Interpretation](http://arxiv.org/abs/2607.02853v1) — 相关度 0.50：构建 UML 图理解的先验偏差基准，对多模态代码 agent 的鲁棒性评估有参考价值。
- [Towards Automatically Inferring Constraints to Identify Implicit Assumptions in Data Analysis](http://arxiv.org/abs/2607.03114v1) — 相关度 0.50：通过静态分析识别数据分析脚本中的隐式假设并转为约束，是面向程序分析的通用方法，可辅助代码agent理解代码。
