...
Selection reason: 工具-模式超图引导规划和执行，减少冗余调用并提升任务完成率，可迁移到代码 agent 规划。
...
Selection reason: 为 agentic 代码生成引入持久化状态、局部化修复与技能记忆，其修复机制对通用编码 agent 的反思和记忆设计有直接借鉴。
...
Selection reason: 提出 agentic 式 Web 生成评测框架，以多角色协作和自适应准则替代静态评测，为前端代码生成 agent 评测提供可扩展方法。
...
Selection reason: 自我进化技能系统反馈动态研究，对 agent 技能更新和反思机制有参考价值。
...
### [55] [Instruction Stacking Collapse: A Benchmark and the Capability-Dependent Value of Prompt Compilation](https://arxiv.org/abs/2608.02639)
...
Selection reason: 提出指令堆叠崩溃基准和 prompt 编译方法，可提升 LLM 代码 agent 的指令遵循能力与评测可靠性。
...
### [56] [IR2Solve: Structured Intermediate Representations for Cost-Efficient Optimization Autoformulation](https://arxiv.org/abs/2608.02641)
...
Selection reason: 用结构化中间表示和确定性验证改善 LLM 生成可靠性并降低推理成本，方法可迁移到代码 agent。
...
### [57] [CUADebug: Diagnosing and Repairing Computer-Use Agent Failures](https://arxiv.org/abs/2608.02643)
...
Selection reason: 提出 CUA 故障诊断与修复框架，含错误分类、基准和调试器，可迁移至 coding agent 的失败分析与修复。
...
### [58] [Verified Tool Calls Improve LLM Agent Reliability Under Non-Atomic Failures](https://arxiv.org/abs/2608.02645)
...
Selection reason: 验证工具调用与幂等机制显著提升 LLM agent 在非原子失败下的可靠性，适用于代码 agent 工具使用。
...
### [59] [When Policies Change Probabilities: Modular Decision-Making for LLM Code Review](https://arxiv.org/abs/2608.02677)
...
Selection reason: 分离风险估计与决策策略的模块化代码审查方法，提升 LLM 审查 agent 的可靠性和评测质量。
...
### [60] [TraceCompiler: Skill-Guided Mining and Compilation of LLM Agent Traces into Mostly Deterministic Workflows](https://arxiv.org/abs/2608.02680)
...
Selection reason: 将 agent 轨迹编译为确定性工作流，含证据验证和技能挖掘，直接提升代码 agent 的效率与可复现性。
...
### [61] [BulkPR-Bench: Benchmarking Queue-Level Governance of Interacting Pull Requests](https://arxiv.org/abs/2608.02685)
...
Selection reason: 面向 PR 队列治理的交互式基准，评估模型处理交互变更和顺序决策能力，是 coding agent 评测的重要新基准。
...
### [62] [PRWeaver: Evaluating LLM-Based Code Auditors against Long-Horizon Malicious Pull Requests](https://arxiv.org/abs/2608.02693)
...
Selection reason: 面向恶意 PR 的代码审计基准，揭示长程攻击和上下文干扰，为代码 agent 安全评测提供方法。
...
### [63] [Don't Regenerate, Debug: A Domain-Specific Agent for Repairing Near-Miss Hardware Operators](https://arxiv.org/abs/2608.02712)
...
Selection reason: 调试替代重新生成的领域专用 agent，大幅降低 token 消耗并提升算子修复率，与程序修复和性能优化高度相关。
...
### [64] [HyperFL: Query-Adaptive Representation Learning for Software Fault Localization](https://arxiv.org/abs/2608.02967)
...
Selection reason: 查询自适应表示学习改进软件故障定位的检索准确率，可迁移到代码 agent 的调试和检索。
...
### [65] [ConFL: Explainable Concurrent Fault Localization via Hierarchy-Guided LLM Reasoning](https://arxiv.org/abs/2608.02974)
...
Selection reason: 层次化知识库引导的并发故障定位，增强 LLM 推理可解释性和鲁棒性，适用于代码调试 agent。
...
### [66] [Efficient Grammar-Constrained Decoding via Parser Stack Classification](https://arxiv.org/abs/2608.03065)
...
Selection reason: 将语法约束解码的掩码计算简化为一次 parser stack 分类，显著降低 LLM 结构化输出/代码生成的延迟，可直接用于编码 agent 的推理加速。
...
### [67] [Cross-Ecosystem Bug Classification in Quantum Software](https://arxiv.org/abs/2608.03173)
...
Selection reason: 提供跨生态系统的量子软件 bug 分类框架与数据集，其可解释规则分类方法对测试、调试和 bug 分析有可迁移价值。
...
### [68] [Fail-Fast, Restart-Smart: Early Failure Prediction and Restart for SWE Agentic Tasks](https://arxiv.org/abs/2608.03222)
...
Selection reason: 针对 SWE agent 长程执行提出提前失败预测与重启恢复机制，直接提升 agent 执行效率并改善解决率。
...
### [69] [NotDec: WebAssembly Decompilation With Inter-Procedural Type Recovery](https://arxiv.org/abs/2608.03286)
...
Selection reason: 提出 WebAssembly 反编译与过程间类型恢复方法，为二进制程序理解和智能化修复提供可复用的程序分析能力。
...
### [70] [Evaluating LLM Trade-offs for Enterprise Automation: Lessons from Workflow Generation in a Production Enterprise Platform](https://arxiv.org/abs/2608.03311)
...
Selection reason: 在生产平台评估多种 LLM 工作流生成的成本/延迟/结构成功率，并提出分片生成流水线，可迁移到代码生成 agent 的工程设计。
...
### [71] [Assessing Behavioral Validation in UI Component Test Suites Using Inferred Metamorphic Relations](https://arxiv.org/abs/2608.03337)
...
Selection reason: 利用推断的 metamorphic relations 从行为层面评估 UI 测试套件并提出 MR 覆盖率指标，可迁移到 agent 测试生成与验证。
...
### [72] [Route-Align-Verify for Functional Correctness in Code Generation](https://arxiv.org/abs/2608.03341)
...
Selection reason: 通过路由、对齐 LoRA 与执行验证三个轻量阶段提升代码生成功能正确性，是可直接复用的编码 agent 流水线方法。
...
### [73] [Self-Evolving Coding Agents](https://arxiv.org/abs/2608.03392)
...
Selection reason: 系统梳理自我进化编码 agent 的演化对象、时机与证据，为构建自适应 coding agent 提供清晰 taxonomy 与设计框架。
...
### [74] [CodeAssay: A Multi-Metric Benchmark with Audited Ground Truth for LLM Code Generation](https://arxiv.org/abs/2608.03535)
...
Selection reason: 提出带审计 ground truth、隐藏测试与突变评分的多指标代码生成基准，显著提升 LLM 代码评测的可信度。
...
Selection reason: 结合差异式 profiling 与 LLM 推理定位仓库级时间低效修复点，并发布 RepoEffi-Bench，直接契合性能优化与自动修复。
...
### [76] [From Bug Reports to Browser-Executable Procedures: An LLM-Driven Agent for Web GUI Bug Reproduction](https://arxiv.org/abs/2608.03598)
...
Selection reason: 构建上下文感知 agent 从自然语言 bug report 复现 Web GUI bug，其状态建模与验证机制可迁移到通用软件测试 agent。
...
### [77] [Pattern over Pixels: Measuring Pattern Completion Bias in Multimodal Code Generation](https://arxiv.org/abs/2608.03691)
...
Selection reason: 首次系统度量多模态代码生成对重复 UI 模式的 pattern-completion bias，为评测和缓解代码生成偏差提供新基准与结论。
...
### [78] [We Must Have Missed This Comment: Detecting and Repairing Stale Function References in Linux Kernel Comments](https://arxiv.org/abs/2608.03734)
...
Selection reason: 结合符号检测、Git 演化追踪与 LLM 修复建议处理注释中的过期函数引用，是代码-注释一致性分析与自动修复的可复用方法。
...
### [79] [Can LLMs Test Terminal User Interfaces?](https://arxiv.org/abs/2608.03743)
...
Selection reason: 构建跨语言 TUI 测试基准并对比多 LLM 与随机探索，揭示行覆盖率与崩溃发现弱相关，对 agent 测试策略设计有直接价值。
