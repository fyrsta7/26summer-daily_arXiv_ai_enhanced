...
- [Exploring Pass-Rate Reward in Reinforcement Learning for Code Generation](http://arxiv.org/abs/2605.02944v1) — 相关度 0.90：直接研究代码生成RL中pass-rate与二元奖励的效果差异，对agent训练奖励设计有重要价值。
- [Themis: Training Robust Multilingual Code Reward Models for Flexible Multi-Criteria Scoring](http://arxiv.org/abs/2605.00754v3) — 相关度 0.90：构建多语言多准则代码奖励模型评测与训练集，直接支撑代码agent的后训练与评测。
- [Agent Capsules: Quality-Gated Granularity Control for Multi-Agent LLM Pipelines](http://arxiv.org/abs/2605.00410v1) — 相关度 0.85：多Agent LLM管线的质量门控粒度控制运行时，显著降低token消耗且保质量，可迁移至代码Agent工作流优化。
- [ClozeMaster: Fuzzing Rust Compiler by Harnessing LLMs for Infilling Masked Real Programs](http://arxiv.org/abs/2605.00413v1) — 相关度 0.85：基于LLM掩码填充的Rust编译器模糊测试，发现27个真实bug，是有效的测试生成方法。
- [Social Bias in LLM-Generated Code: Benchmark and Mitigation](http://arxiv.org/abs/2605.00382v3) — 相关度 0.82：构建LLM生成代码社会偏见基准并提出公平性监控Agent，直接相关代码生成Agent评测与改进。
- [Can Coding Agents Reproduce Findings in Computational Materials Science?](http://arxiv.org/abs/2605.00803v1) — 相关度 0.80：提出AutoMat基准评测coding agent复现材料科学工作流，并分析失败模式，对agent评估有参考价值。
- [Code World Model Preparedness Report](http://arxiv.org/abs/2605.00932v2) — 相关度 0.80：Meta代码世界模型准备度报告，面向代码生成与代码推理能力，对Coding Agent有直接参考价值。
- [Improving LLM Code Generation via Requirement-Aware Curriculum Reinforcement Learning](http://arxiv.org/abs/2605.00433v1) — 相关度 0.80：需求感知课程强化学习提升LLM代码生成，直接优化代码生成模型训练方法，有性能提升实证。
- [PPO guided Agentic Pipeline for Adaptive Prompt Selection and Test Case Generation](http://arxiv.org/abs/2605.00942v1) — 相关度 0.80：PPO驱动的Agentic流程自适应提示选择与测试用例生成，提升分支/行覆盖率，直接面向代码测试Agent。
- [GeoContra: From Fluent GIS Code to Verifiable Spatial Analysis with Geography-Grounded Repair](http://arxiv.org/abs/2605.00782v1) — 相关度 0.75：以可执行契约、静态/运行时验证和修复循环约束LLM代码生成，对agent可靠性验证有迁移价值。
- [Skills as Verifiable Artifacts: A Trust Schema and a Biconditional Correctness Criterion for Human-in-the-Loop Agent Runtimes](http://arxiv.org/abs/2605.00424v2) — 相关度 0.75：提出技能作为可验证工件、信任模型与双条件正确性准则，增强Agent运行时的可靠性与HITL机制。
- [Q-ARE: An Evaluation Dataset for Query Based API Recommendation](http://arxiv.org/abs/2605.00472v1) — 相关度 0.70：构建基于API推荐评测数据集及两个新指标，系统性评估现有方法与LLM，推进语义理解与代码检索。
- [RECAP: An End-to-End Platform for Capturing, Replaying, and Analyzing AI-Assisted Programming Interactions](http://arxiv.org/abs/2605.01104v1) — 相关度 0.70：提供捕获、回放与分析AI编程交互的平台，可直接支撑agent行为研究和评估。
- [Think Harder and Don't Overlook Your Options: Revisiting Issue-Commit Linking with LLM-Assisted Retrieval](http://arxiv.org/abs/2605.00447v2) — 相关度 0.70：系统评估多种检索与重排序方法用于issue-commit链接恢复，包含LLM对比，对代码检索与追踪有参考价值。
- [EnCoDe: Energy Estimation of Source Code At Design-Time](http://arxiv.org/abs/2605.00504v1) — 相关度 0.60：面向设计期的细粒度能耗建模与预测，可作profiling/能耗调优的参考方法。
- [The Productivity-Reliability Paradox: Specification-Driven Governance for AI-Augmented Software Development](http://arxiv.org/abs/2605.01160v1) — 相关度 0.55：提出AI辅助开发的效率-可靠性悖论与规格治理模型，对agent可靠性设计具有参考价值。
