...
- [Beyond Problem Solving: UOJ-Bench for Evaluating Code Generation, Hacking, and Repair in Competitive Programming](http://arxiv.org/abs/2606.12864v1) — 相关度 0.92：包含代码生成、破解与修复三任务的竞赛编程基准，直接评测 LLM 的编程与调试能力。
- [Knowledge-Based Zero-Replay Debugging of Multi-Agent LLM Traces](http://arxiv.org/abs/2606.14805v1) — 相关度 0.88：提出零重放预测多 agent 轨迹中关键事件的方法，显著提升调试定位效率，对 agent 运维有直接价值。
- [Toward Instructions-as-Code: Understanding the Impact of Instruction Files on Agentic Pull Requests](http://arxiv.org/abs/2606.13449v1) — 相关度 0.85：分析指令文件对 agentic PR 成功率的影响，发现长且结构化指令更有效，直接指导 rules 设计。
- [DIG: Oracle-Guided Directed Input Generation for One-Day Vulnerabilities](http://arxiv.org/abs/2606.13037v1) — 相关度 0.82：利用 LLM 和 oracle 引导生成漏洞利用输入，显著加速 PoC 构造，对安全分析与 agent 验证有借鉴。
- [Understanding the Rejection of Fixes Generated by Agentic Pull Requests -- Insights from the AIDev Dataset](http://arxiv.org/abs/2606.13468v1) — 相关度 0.82：定性分析 agentic PR 修复被拒绝的 14 种原因，为改进 agent 修复能力提供实证依据。
- [Bayesian-Calibrated Detection of Hallucinated Package Imports in AI-Assisted Code](http://arxiv.org/abs/2606.13918v1) — 相关度 0.80：为 LLM 生成代码中的幻觉包导入检测增加贝叶斯校准层，可提升代码 agent 的依赖可靠性。
- [Do programming languages still matter to your AI coding agent teammate? Evidence at scale from chess engines](http://arxiv.org/abs/2606.13763v1) — 相关度 0.75：大规模研究 coding agent 跨 17 种语言生成国际象棋引擎的能力与成本，揭示语言选择对性能的影响。
- [Mining Architectural Quality Under Agentic AI Adoption: A Causal Study of Java Repositories](http://arxiv.org/abs/2606.13298v1) — 相关度 0.70：对 agentic AI 采用前后架构质量进行因果分析，揭示密度指标的误导性，对智能体影响评估有参考。
- [An Empirical Study of Gemini 3 for Detecting Natural Language Test Smells in Manual Test Cases](http://arxiv.org/abs/2606.13804v1) — 相关度 0.60：使用大模型检测自然语言测试用例中的测试坏味道，相比小模型效果更优，可迁移到测试分析 agent。
- [ComAct: Reframing Professional Software Manipulation via COM-as-Action Paradigm](http://arxiv.org/abs/2606.13239v2) — 相关度 0.55：提出 COM-as-Action 范式，以确定性程序合成替代 GUI 控制，为 agent 操作软件提供新思路。
- [Faster Code, Deeper Debt? A Multivocal Literature Review on Technical Debt and Its Early Signs in LLM-Assisted Software Development](http://arxiv.org/abs/2606.14796v1) — 相关度 0.55：综述 LLM 辅助开发引入的技术债务及早期迹象，总结了策略与工具，对构建可靠 agent 工作流有参考。
