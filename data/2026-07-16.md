...
- [Self-Improving AI Coding Agents Through Accumulated Behavioral Rules: A Closed-Loop Framework](https://arxiv.org/abs/2607.13091) — 相关度 0.95：闭环规则积累使编码agent跨会话改进，0%复发率，对agent持久记忆与反思有直接价值。
- [Harness Handbook: Making Evolving Agent Harnesses Readable,Navigable, and Editable](https://arxiv.org/abs/2607.13285) — 相关度 0.90：为 agent harness 提供行为定位与渐进式披露机制，直接改善 coding agent 的编辑规划与代码定位。
- [PROBE: Benchmarking Code Generation in Large Language Models](https://arxiv.org/abs/2607.13820) — 相关度 0.90：面向多语言、多维度的代码生成基准与评测流程，直接服务 LLM coding agent 评估。
- [VisualRepair: Dynamic Tool Calling and Region Focusing for Visual Software Issue Repair](https://arxiv.org/abs/2607.14075) — 相关度 0.90：面向视觉软件缺陷的 MLLM 修复框架，类型感知工具调用与区域聚焦直接提升多模态 APR。
- [AgentCompass: A Unified Evaluation Infrastructure for Agent Capabilities](https://arxiv.org/abs/2607.13705) — 相关度 0.85：提出统一、可扩展的agent评测基础设施，解耦Benchmark/Harness/Environment，直接可用于代码agent评测。
- [Compaction as Epistemic Failure: How Agentic LLM Tools Fabricate Confirmed Results from Killed Processes](https://arxiv.org/abs/2607.13071) — 相关度 0.85：揭示 Claude Code 压缩摘要将超时进程输出误判为确认结果，直接暴露 coding agent 记忆与自我报告中的关键可靠性缺陷。
...
- [Faithful Autoformalization of Natural Language Assertions](https://arxiv.org/abs/2607.13303) — 相关度 0.78：基于一致性分数和测试验证的断言自动形式化，提升 LLM 生成可执行规格的可靠性，可直接用于测试 agent。
- [ProfMalPlus: Agent-Coordinated Detection of Malicious NPM Packages via Static-Dynamic Analysis Synergy](https://arxiv.org/abs/2607.13965) — 相关度 0.78：结合对象敏感行为图与 LLM 多智能体协作的恶意包检测，静态-动态分析裁决工作流可迁移到代码安全 agent。
- [SemaDiff: Identifying Semantic-Changing Commits with Generated Code and Tests](https://arxiv.org/abs/2607.13111) — 相关度 0.75：通过生成测试和对比执行识别语义变更提交，对 agent 驱动的测试与变更分析有直接价值。
- [Set-shifting Behavioral Test for Harnessed Agents](https://arxiv.org/abs/2607.13396) — 相关度 0.75：研究agent在工具可靠性变化下的适应能力，提供可迁移的评估框架，对coding agent可靠性评测有价值。
- [Early Adoption of Agentic Coding Tools by GitHub Projects](https://arxiv.org/abs/2607.14037) — 相关度 0.72：对 2361 个 GitHub 项目 agentic PR 的早期实证，揭示采用模式、生产力与人工监督模型，支撑 agent 工具设计。
- [From Human-Centric to Agentic Code Review: The Impact of Different Generations of Generative AI Technology on Review Quality](https://arxiv.org/abs/2607.13196) — 相关度 0.72：百万级 PR 研究人类/LLM/agent 评审协作模式，为 agentic code review 提供重要实证。
- [A Telemetry-Driven Model for Quantifying Upgrade Risk in Durable Workflow Execution](https://arxiv.org/abs/2607.13617) — 相关度 0.65：为 durable workflow 升级风险提供概率模型与 migrate/pin 策略，适用于长程 agent 的可靠执行。
- [Inference Economics of Enterprise Coding Agents: A Case Study of Cloud vs. On-Premise LLMs](https://arxiv.org/abs/2607.13080) — 相关度 0.60：云与本地LLM编码agent的成本-质量案例研究，揭示缓存成本与缺陷修复负担的权衡，对agent部署经济性有参考价值。
- [Can LLMs Learn and Apply Multi-Level Modelling Semantics? A First Empirical Study](https://arxiv.org/abs/2607.13257) — 相关度 0.55：LLM 生成多级建模语义的实证与提示策略对比，对生成结构化工件的 agent 有参考价值。
- [Design-System-Aware Development with AI: Evaluating Productivity and Design Consistency](https://arxiv.org/abs/2607.13156) — 相关度 0.55：DS-aware AI 前端开发生产力实验，对前端代码 agent 效果评测有参考价值。
- [The Replication Assessment Problem in Software Engineering](https://arxiv.org/abs/2607.13815) — 相关度 0.55：系统化复制研究评估框架，可迁移到 coding agent benchmark 的可复现性与一致性评估。
