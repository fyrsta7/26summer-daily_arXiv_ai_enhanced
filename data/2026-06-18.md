...
- [AutoPass: Evidence-Guided LLM Agents for Compiler Performance Tuning](http://arxiv.org/abs/2606.20373v1) — 相关度 1.00：LLM agent利用编译器和运行时证据调优LLVM编译选项，是SemOpt核心应用，直接相关。
- [Probe-and-Refine Tuning of Repository Guidance for Coding Agents](http://arxiv.org/abs/2606.20512v2) — 相关度 0.98：提出探针-精炼调优方法迭代改进仓库指引文件，显著提升SWE-bench Verified修复率，直接相关于coding agent优化。
- [Phoenix: Safe GitHub Issue Resolution via Multi-Agent LLMs](http://arxiv.org/abs/2606.20243v2) — 相关度 0.95：多agent LLM系统解决GitHub issue，包含安全控制与基线测试评估，直接相关于编码agent的工程化与评测。
- [The Correctness Illusion in LLM-Generated GPU Kernels](http://arxiv.org/abs/2606.20128v1) — 相关度 0.95：揭示LLM生成GPU kernel在固定shape allclose检查下的正确性幻觉，并用种子模糊+高精度参考验证，直接相关于代码生成正确性评估。
- [N-Version Programming with Coding Agents](http://arxiv.org/abs/2606.20158v1) — 相关度 0.90：将N版本编程应用于编码agent，评估多样性投票降低共因失败，对agent可靠性策略有直接借鉴。
- [When Lower Privileges Suffice: Investigating Over-Privileged Tool Selection in LLM Agents](http://arxiv.org/abs/2606.20023v2) — 相关度 0.90：研究LLM agent工具选择中的过度特权问题，提出评测集与权限感知后训练防御，可迁移到代码agent的工具反馈与安全评测。
- [AutoACSL: Synthesizing ACSL Specifications by Integrating LLMs with CPG-Based Static Analysis](http://arxiv.org/abs/2606.20969v1) — 相关度 0.85：将LLM与代码属性图静态分析结合，通过Frama-C验证反馈循环自动合成ACSL规范，直接适用于形式化验证agent。
- [Calibration Without Comprehension: Diagnosing the Limits of Fine-Tuning LLMs for Vulnerability Detection in Systems Software](http://arxiv.org/abs/2606.20502v1) — 相关度 0.85：构建漏洞检测LLM基准与诊断指标，揭示微调仅改变阈值而未改变决策策略，对代码安全agent评估有重要参考。
- [JAMER: Project-Level Code Framework Dataset and Benchmark on Professional Game Engines](http://arxiv.org/abs/2606.19830v2) — 相关度 0.80：首个专业游戏引擎上的项目级代码数据集与基准，扩展代码生成与agent评测范畴。
- [Library-Aware Doubles and Iterative Repair for Large Language Model-Generated Unit Tests in OpenSIL Firmware](http://arxiv.org/abs/2606.19725v2) — 相关度 0.80：LLM生成单元测试加迭代编译修复的流水线，直接匹配coding agent的测试生成与验证需求。
- [Beyond Static Endpoints: Tool Programs as an Interface for Flexible Agentic Web Services](http://arxiv.org/abs/2606.19992v1) — 相关度 0.75：以可执行tool program封装agent工具意图，降低长程工具调用延迟，可迁移到coding agent工具使用。
- [Repository-Level Solidity Code Generation with Large Language Models: From Prompting to Fine-Tuning](http://arxiv.org/abs/2606.19988v1) — 相关度 0.75：仓库级Solidity代码生成基准及多种调优策略对比，补充特定领域代码生成评测和方法。
- [Agentic Electronic Design Automation: A Handoff Perspective](http://arxiv.org/abs/2606.19795v1) — 相关度 0.70：以handoff有效性为原则综述agentic EDA，多阶段协调机制可迁移到SE agent工作流。
- [Formally Verified Code Synthesis for Structured Data Translation in a Medical Internet of Things](http://arxiv.org/abs/2606.20776v1) — 相关度 0.70：LLM代码合成结合形式化验证确保schema转换正确性，其验证集成方式可迁移到代码agent输出可靠性验证。
- [Qiskit Code Migration with LLMs](http://arxiv.org/abs/2606.20173v1) — 相关度 0.70：LLM+RAG的Qiskit代码迁移方法，虽然领域特定，但基于分类学的RAG结构化知识检索可迁移到一般代码迁移任务。
- [OxyMake: A Formally-Specified, Content-Addressable Workflow Engine](http://arxiv.org/abs/2606.20989v2) — 相关度 0.68：内容寻址缓存工作流引擎，用BLAKE3键替代mtime并给出TLA+形式化协议，可迁移到代码agent的执行缓存与可复现运行。
- [A Topology-Aware, Memory-Centric Architecture that Separates Root-Cause Derivation from Root-Cause Explanation](http://arxiv.org/abs/2606.20758v1) — 相关度 0.65：分离根因推导与解释的多agent运维架构，可迁移到agent的记忆、诊断与LLM集成设计。
- [PromptMark: A Prompt-Guided Iterative-Feedback Framework for Source Code Watermarking](http://arxiv.org/abs/2606.20835v1) — 相关度 0.60：黑盒提示引导源码水印与统计检测，可作为AI生成代码溯源/评测的辅助技术。
- [Grounded Inference: Principles for Deterministically Encapsulated Generative Models](http://arxiv.org/abs/2606.19753v1) — 相关度 0.55：提出生成模型的确定性封装原则，可参考用于构建可靠、可审计的agent系统。
- [The Hidden Environmental Cost of Poor Coding Practices in TensorFlow and Keras Applications: A Study on Resource Leaks and Carbon Emissions](http://arxiv.org/abs/2606.19799v1) — 相关度 0.55：量化ML代码资源泄漏对能耗和碳排放的影响，为代码优化目标提供实证参考。
- [The Substrate Collapse: AI Code Generation Invalidates Authorship-Based Knowledge Metrics](http://arxiv.org/abs/2606.20882v1) — 相关度 0.55：讨论AI生成代码使作者归属知识度量失效，并提出基于理解的度量与可证伪预测，对评估代码agent产物的长期可维护性有启发。
- [A Unified Framework for Runtime Verification and Model-Based Diagnosis in LOLA](http://arxiv.org/abs/2606.23720v1) — 相关度 0.50：统一运行时验证与模型诊断的流式语言框架，其诊断机制可参考用于代码agent行为监控与故障定位。
- [CoRaCommit: A VS Code Extension for Commit Message Generation with Exemplar Retrieval](http://arxiv.org/abs/2606.19814v1) — 相关度 0.50：检索增强的commit message生成工具，虽垂直但提供RAG+用户反馈的可迁移思路。
