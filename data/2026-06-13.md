...
- [ClayBuddy: A Framework, Evaluation, & Mitigation of Coding Agent Failures](http://arxiv.org/abs/2606.19380v3) — 相关度 0.92：系统研究coding agent三类失败机制并提出可修改harness的缓解方案，直接针对代码agent可靠性与评测。
- [The Hitchhiker's Guide to Program Analysis, Part III: Mostly Harmless LLMs](http://arxiv.org/abs/2606.15122v1) — 相关度 0.85：LLM仅负责构造分析harness，行为推理交给形式后端，这种分离LLM与形式验证的架构可迁移到代码agent的告警验证。
- [LogCopilot: Automating Log Aggregation Analysis through Large Language Models](http://arxiv.org/abs/2606.17094v1) — 相关度 0.75：用LLM检索知识并生成/执行LogQL查询来自动化日志聚合分析，其工具调用与知识库方法可迁移到agent的日志诊断。
- [AI-driven Software Development: A Pragmatic Path to Agentic Development Processes](http://arxiv.org/abs/2606.15283v1) — 相关度 0.70：提出从辅助AI到受控agentic开发流程的框架，强调harness、验证与治理机制，对代码agent工程化有参考价值。
- [Specifications for Humans, Agents, and Tooling](http://arxiv.org/abs/2606.15084v1) — 相关度 0.70：面向人/agent/工具的多语言规范生态，强调验证、测试生成与沙箱，对agentic软件工程的规范与验证有迁移价值。
