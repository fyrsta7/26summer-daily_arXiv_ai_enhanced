...
Selection reason: 为coding agent构建本体化项目记忆，通过MCP接口暴露知识图谱，对agent的长程上下文管理有直接价值。
...
Selection reason: 提出在agent行动和观察期间并行推理的Second Thought框架，减少主线程顺序解码，可迁移到coding agent的推理效率优化。
...
Selection reason: 三阶段微调Curriculum用于C转Rust翻译，结合agentic静态分析验证框架，对代码翻译agent训练与评测有参考。
...
Selection reason: 报告AI密集型软件开发成本测量的案例并修正方法论错误，对AI辅助开发成本评估和实证研究有参考价值。
...
Selection reason: 比较ISO标准化的NFR规格对LLM代码生成的影响，揭示内容比格式更重要，对prompt工程和agent需求规格化有参考。
...
Selection reason: 系统综述coding agent可靠性的评估与运维框架，包含可靠性记录目录和可复用技能，与用户研究方向直接相关。
...
Selection reason: 纵向分析AI辅助开发对PR指标的影响，虽偏实证，但提供的度量思路可迁移到agentic coding效果的工程评估。
...
Selection reason: 实证研究bug复杂度、故障定位和LLM成本效益对APR的影响，揭示低成本修复的边界，对agent修复策略有参考。
