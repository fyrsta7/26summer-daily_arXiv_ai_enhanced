...
- [Symbolon: Symbolic Execution by Learning Code Transformation](http://arxiv.org/abs/2606.29108v1) — 相关度 0.95：用agent学习代码变换提升符号执行，结合程序分析与LLM agent，高度相关且效果显著。
- [Diff-Based Code Corruption using LLMs for Large-Scale Bugfix Benchmarking](http://arxiv.org/abs/2606.29088v3) — 相关度 0.90：大规模bugfixing基准MegaBugFix，使用LLM diff注入bug，直接用于评估代码修复模型。
- [Reward-Free Code Alignment from Pretrained or Fine-Tuned LLM: Unpacking the Trade-offs for Code Generation](http://arxiv.org/abs/2606.28998v2) — 相关度 0.80：研究代码LLM的无奖励对齐路径，为代码生成模型的训练与部署提供实证洞察。
- [Understanding Binary Code Similarity for Real-World Vulnerability Detection: A Large-Scale Empirical Study](http://arxiv.org/abs/2606.28870v1) — 相关度 0.75：大规模二进制代码相似性实证研究，提出build-aware查询策略，对漏洞检测与程序分析有借鉴。
