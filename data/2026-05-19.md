...
- [Code Generation by Differential Test Time Scaling](http://arxiv.org/abs/2605.20473v1) — 相关度 0.95：提出基于差分分析和覆盖引导fuzzing的测试时扩展方法，无需额外LLM调用即可选择最优代码候选。
- [Prior Knowledge or Search? A Study of LLM Agents in Hardware-Aware Code Optimization](http://arxiv.org/abs/2605.19782v1) — 相关度 0.95：通过受控实验分析LLM agent在硬件感知代码优化中依赖先验而非反馈，直接关系SemOpt核心。
- [A Case for Agentic Tuning: From Documentation to Action in PostgreSQL](http://arxiv.org/abs/2605.19988v1) — 相关度 0.90：将专家调优方法转化为可执行技能，赋予LLM agent进行PostgreSQL配置优化，直接属于自动性能调优。
- [Does Code Cleanliness Affect Coding Agents? A Controlled Minimal-Pair Study](http://arxiv.org/abs/2605.20049v1) — 相关度 0.90：用最小对研究代码整洁度对coding agent行为的影响，为agent评测与代码维护提供可迁移方法。
- [Library Drift: Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries](http://arxiv.org/abs/2605.19576v2) — 相关度 0.90：诊断并修复自进化skill库的漂移问题，给出治理方案，直接适用于自进化coding agent。
- [Multi-agent Collaboration with State Management](http://arxiv.org/abs/2605.20563v1) — 相关度 0.90：面向多agent共享代码库的状态管理，能在写入时检测解决冲突，直接适用于coding agent协作
- [optimize_anything: A Universal API for Optimizing any Text Parameter](http://arxiv.org/abs/2605.19633v1) — 相关度 0.90：通用LLM文本优化API可优化agent架构与CUDA内核等代码，与SemOpt高度契合。
- [A Methodology for Selecting and Composing Runtime Architecture Patterns for Production LLM Agents](http://arxiv.org/abs/2605.20173v1) — 相关度 0.85：提出生产级LLM agent运行时架构模式选择方法，聚焦随机-确定边界与可靠性，可迁移至coding agent工作流。
- [AgentAtlas: Beyond Outcome Leaderboards for LLM Agents](http://arxiv.org/abs/2605.20530v2) — 相关度 0.85：提出agent评估的诊断词汇表和轨迹失败分类，可迁移至coding agent的评测与失败归因
- [Agentic Agile-V: From Vibe Coding to Verified Engineering in Software and Hardware Development](http://arxiv.org/abs/2605.20456v1) — 相关度 0.85：提出Agentic Agile-V流程框架，将agent代码生成约束为结构化工件与验收证据，对工程化agent有直接参考。
- [OpenComputer: Verifiable Software Worlds for Computer-Use Agents](http://arxiv.org/abs/2605.19769v1) — 相关度 0.85：面向computer-use agent的可验证软件世界框架，含状态验证器与部分奖励机制，可迁移到agent评测。
- [SCARA: A Semantics-Constrained Autonomous Remediation Agent for Opaque Industrial Software Vulnerabilities](http://arxiv.org/abs/2605.19668v1) — 相关度 0.85：面向无源码工业软件漏洞的自主修复agent，结合语义约束与验证流程，对代码修复agent有迁移价值。
- [MuMuTestUp: Mutation-based Multi-Agent Test Case Update](http://arxiv.org/abs/2605.19265v1) — 相关度 0.82：变异引导的多agent测试更新框架，针对断言充分性与语义检索，直接改进测试agent。
- [CriterAlign: Criterion-Centric Rationale Alignment for Code Preference Judging](http://arxiv.org/abs/2605.19665v2) — 相关度 0.80：提出面向代码偏好评估的criterion-centric pairwise评判框架，改进LLM judge的评估准确率。
- [MOCHA: Multi-Objective Chebyshev Annealing for Agent Skill Optimization](http://arxiv.org/abs/2605.19330v1) — 相关度 0.80：多目标Chebyshev退火优化agent skill，可迁移到coding agent的技能调优与Pareto探索。
- [When to Answer and When to Defer: A Decision Framework for Reliable Code Predictions](http://arxiv.org/abs/2605.19369v1) — 相关度 0.80：将不确定性估计、校准与工具调用结合的选择性预测框架，增强代码agent的风险感知与安全放弃。
- [On-the-Fly Input Adaptation for Reliable Code Intelligence](http://arxiv.org/abs/2605.19365v1) — 相关度 0.76：通过输入验证与适配减少代码模型误预测，无需重训，适合提升coding agent可靠性。
- [Characterizing Real-World Bugs in Tile Programs for Automated Bug Detection](http://arxiv.org/abs/2605.19652v1) — 相关度 0.75：系统研究tile程序代码生成bug，为编译器测试与自动化bug检测提供基础性洞见。
- [DRReduce: Enhancing Syntax-Guided Program Reduction with Dependency Reconstruction](http://arxiv.org/abs/2605.19412v1) — 相关度 0.75：通过依赖重建增强语法引导的程序化简，服务于调试与最小化测试用例，属程序分析通用方法。
- [Towards LLM-Assisted Architecture Recovery for Real-World ROS~2 Systems: An Agent-Based Multi-Level Approach to Hierarchical Structural Architecture Reconstruction](http://arxiv.org/abs/2605.20055v1) — 相关度 0.70：基于LLM agent的RO S2系统分层架构恢复方法，对软件架构逆向agent有参考价值。
- [Can LLMs Produce Better Object-Oriented Designs than Human-Involved Development?](http://arxiv.org/abs/2605.19901v1) — 相关度 0.65：比较LLM与人类参与项目的面向对象设计质量，为代码生成质量评估提供参考。
- [Combined Program Analysis Techniques: A Systematic Mapping Study](http://arxiv.org/abs/2605.20310v1) — 相关度 0.65：系统梳理程序分析技术组合的协同分类，为构建分析工具链提供结构化指导。
- [A Multi-Layer Testing Framework for Automated Data Quality Assurance in Cloud-Native ELT Pipelines](http://arxiv.org/abs/2605.20500v1) — 相关度 0.60：结合LLM生成语义测试的多层数据管道测试框架，对自动化测试生成和验证编排有一定参考价值。
- [Contract Based Verification of Non-functional Requirements for Embedded Automotive C Code](http://arxiv.org/abs/2605.21532v1) — 相关度 0.60：提出面向C模块的接口契约与Frama-C插件，可迁移至代码agent的合约验证与不可信代码检查
