...
- [SaaSBench: Exploring the Boundaries of Coding Agents in Long-Horizon Enterprise SaaS Engineering](http://arxiv.org/abs/2605.17526v1) — 相关度 0.92：首个面向企业SaaS长程工程的编码Agent基准，覆盖异构技术栈与系统级集成，直接用于评测coding agent能力边界。
- [ContraFix: Skill-Enhanced Contrastive Runtime Analysis for Vulnerability Repair](http://arxiv.org/abs/2605.17450v2) — 相关度 0.90：通过对比运行时分析构造修复证据，结合技能库复用与验证，在漏洞修复评测上大幅超越基线并提升语义正确性。
- [MemRepair: Hierarchical Memory for Agentic Repository-Level Vulnerability Repair](http://arxiv.org/abs/2605.17444v1) — 相关度 0.90：提出层次化记忆增强的 agentic 漏洞修复框架，利用历史修复/安全模式/失败轨迹记忆，显著提升多语言仓库级修复率。
- [Verifier-Guided Code Translation via Meta-Step Decoding](http://arxiv.org/abs/2605.17626v1) — 相关度 0.88：提出验证器引导的元步解码，在生成中插入结构边界检查与回滚，显著提升代码翻译正确率，对agent推理时优化有直接迁移价值。
- [ContractBench: Can LLM Agents Preserve Observation Contracts?](http://arxiv.org/abs/2605.17281v1) — 相关度 0.85：提出观察契约合规基准，评估 LLM agent 对 API 工件时效与完整性的保持能力，为 agent 可靠性与评测提供新维度。
- [From Runnable to Shippable: Multi-Agent Test-Driven Development for Generating Full-Stack Web Applications from Requirements](http://arxiv.org/abs/2605.17242v1) — 相关度 0.85：提出多 agent TDD 闭环框架，通过浏览器交互验证与结构化修复报告提升全栈应用生成质量，对 coding agent 实践有重要参考。
- [Firefly: Illuminating Large-Scale Verified Tool-Call Data Generation from Real APIs](http://arxiv.org/abs/2605.17558v1) — 相关度 0.84：从真实API反演生成可验证工具调用轨迹数据，保证标签正确性，适用于训练和评测工具调用agent，迁移价值高。
- [AgentModernize: Preserving Business Logic in Legacy Modernization with Multi-Agent LLMs and Behavioral Specification Graphs](http://arxiv.org/abs/2605.17535v1) — 相关度 0.82：多Agent遗留系统现代化框架，通过行为规格图显式保留业务逻辑，其提取-规范-生成-验证pipeline对代码迁移agent有直接借鉴价值。
- [DiagEval: Trajectory-Conditioned Diagnosis for Reliable Software Evaluation with GUI Agents](http://arxiv.org/abs/2605.17439v2) — 相关度 0.80：面向 GUI agent 评估的轨迹条件诊断协议，通过针对性探针区分评估器错误与软件缺陷，提升评估可靠性。
- [Rover: Context-aware Conflict Resolution with LLM](http://arxiv.org/abs/2605.17279v1) — 相关度 0.80：融合程序分析与 LLM 的代码合并冲突解决，提出 Multi-layer Code Property Graph 与上下文聚类，可迁移到代码 agent 的上下文建模。
- [Event-B Agent: Towards LLM Agent for Formal Model Synthesis and Repair](http://arxiv.org/abs/2605.17475v1) — 相关度 0.78：面向形式化模型合成与修复的LLM Agent，利用验证反馈迭代精化，其修复-细化循环可迁移至代码生成与验证场景。
- [Debug Like a Human: Scaling LLM-based Fault Localization to Processor Design via Block-Level Instruction-Oriented Slicing](http://arxiv.org/abs/2605.17290v1) — 相关度 0.75：基于 LLM 的处理器设计缺陷定位，提出块级指令导向切片以模拟人类调试推理，方法可迁移到大型代码库故障定位。
- [Automated Root-Cause Subclassification and No-Code Fix Generation for Invalid Bug Reports](http://arxiv.org/abs/2605.17561v2) — 相关度 0.72：针对无效bug报告的子类分类与无代码修复生成，结合RAG和agentic搜索，可迁移到代码agent的缺陷分类与修复建议。
- [Benchmarking Mythos-Linked Bug Rediscovery](http://arxiv.org/abs/2605.17416v1) — 相关度 0.70：控制变量实验评测 LLM 在给定目标文件下的 bug 复现能力，揭示提前承诺失败模式，对 agent 故障定位评测有参考。
- [Rethinking Code Review in the Age of AI: A Vision for Agentic Code Review](http://arxiv.org/abs/2605.17548v2) — 相关度 0.66：提出端到端agentic code review的愿景框架，将审查生命周期分阶段并保留人工控制，对代码审查agent设计有参考价值。
- [Bridging the Gap on AI-Assisted Scientific Software Development Through Transparency and Traceability](http://arxiv.org/abs/2605.17675v1) — 相关度 0.58：讨论AI辅助科学软件开发中的透明性与可追溯性治理框架，虽偏流程，但其验证与审计要求对agent工程化有参考意义。
- [A semantic mutation metric for metamorphic relation adequacy in scientific computing programs](http://arxiv.org/abs/2605.17437v2) — 相关度 0.55：提出面向科学计算程序的语义变异得分，与经典变异测试兼容，对测试充分性度量有方法贡献。
- [NOETHER: A Constructive Framework for Metamorphic Pattern Discovery from Operator Algebras](http://arxiv.org/abs/2605.17390v1) — 相关度 0.55：提出变形测试模式发现的构造性框架，提供算子代数与可判定性保证，对测试方法学有参考价值。
