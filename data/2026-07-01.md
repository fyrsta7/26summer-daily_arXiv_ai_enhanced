...
- [Are Performance-Optimization Benchmarks Reliably Measuring Coding Agents?](http://arxiv.org/abs/2607.01211v2) — 相关度 0.95：审计性能优化 benchmark 的可靠性，直接针对 SemOpt 核心评测问题。
- [SWE-Doctor: Guiding Software Engineering Agents with Runtime Diagnosis from Multi-Faceted Bug Reproduction Tests](http://arxiv.org/abs/2607.00990v1) — 相关度 0.95：面向软件工程 agent 的运行时诊断引导补丁生成，在 SWE-bench 上显著提升，直接相关。
- [AgentLTL: A Trace-Verification Framework for Measuring, Enforcing, and Training Procedural Compliance in Tool-Using LLM Agents](http://arxiv.org/abs/2607.02599v1) — 相关度 0.90：用FO-LTL对工具使用agent轨迹做程序性合规验证、拦截与指标化训练，可直接用于agent正确性保障。
- [Benchmarking Code Improvement with Progressive, Adaptive, and Interactive Feedback](http://arxiv.org/abs/2607.01360v1) — 相关度 0.90：提出渐进式自适应反馈的代码改进评测 benchmark，测量模型迭代改进能力。
- [ChainSWE: Benchmarking Coding Agents on Multi-Bug Software Maintenance](http://arxiv.org/abs/2607.02606v1) — 相关度 0.90：首个面向连续、依赖bug修复的coding agent基准，揭示长链维护中的性能衰减。
- [Knowledge-Enhanced Agentic Vulnerability Repair](http://arxiv.org/abs/2607.00820v1) — 相关度 0.90：知识增强的agentic漏洞修复，结合工具ReAct推理与闭环验证，显著提升C/C++漏洞修复率。
- [LLVM-Bench: Benchmarking and Advancing Large Language Models for LLVM Compiler Issue Resolution](http://arxiv.org/abs/2607.00700v1) — 相关度 0.90：首个LLM解决LLVM编译器问题的benchmark与自动化验证平台，直接服务编译优化与修复。
- [Rise From The Ashes: LLM-based Static Analysis for Deep Learning Framework Bugs](http://arxiv.org/abs/2607.00555v1) — 相关度 0.90：LLM多agent静态分析框架，以跨语言张量流SBIR检测DL框架bug，与代码agent直接相关。
- [Towards Better Linux Kernel Fault Localization: Leveraging Contrastive Reasoning and Hierarchical Context Analysis](http://arxiv.org/abs/2607.00562v1) — 相关度 0.90：面向Linux内核的LLM故障定位，对比推理+层次上下文分析，显著提升定位精度。
- [Better Together, in the Right Order: Classical-then-LLM Optimization for SE](http://arxiv.org/abs/2607.02583v1) — 相关度 0.85：提出经典优化器先导、LLM 后置的 SNAP2 组合方法，在 SE 配置任务上效果与成本皆优，对 SemOpt 有明确迁移价值。
- [Checked Program Recovery from Execution Video: A Sound Oracle for Untrusted Generators](http://arxiv.org/abs/2607.00635v1) — 相关度 0.85：从执行视频恢复程序并用sound oracle验证，为不可信生成器提供了可信任的正确性认证方案。
- [ClarifyCodeBench: Evaluating LLMs on Clarifying Ambiguous Requirements for Code Generation](http://arxiv.org/abs/2607.00711v1) — 相关度 0.85：评估LLM澄清不明确需求能力的交互式基准，对代码生成agent的交互迭代评测有直接价值。
- [RepoRescue: An Empirical Study of LLM Agents on Whole-Repository Compatibility Rescue](http://arxiv.org/abs/2607.01213v1) — 相关度 0.85：评测 LLM agents 修复仓库级兼容性问题，提出源-only 审计和运行时强制等评估方法。
- [Dependencies that Bundle Code and Execution](http://arxiv.org/abs/2607.02618v1) — 相关度 0.80：通过可分发执行缓存优化应用启动性能，属于运行时优化并有speedup验证。
- [TAG: A Lightweight Framework for Test-Driven Agentic Artifact Generation](http://arxiv.org/abs/2607.02615v2) — 相关度 0.80：测试驱动生成框架，结合确定性/LLM 测试和专家评判，可直接迁移到代码工件生成验证。
- [A Survey of LLM-Driven Penetration Testing: Taxonomy, Co-Evolution, and Open Challenges](http://arxiv.org/abs/2607.02605v1) — 相关度 0.75：系统综述LLM渗透测试agent的架构演进与RLVR训练范式，分类法和挑战可迁移至安全代码agent。
- [Attributing Structured-Output Gains in Function Calling: Interface Alignment versus Procedural Transfer](http://arxiv.org/abs/2607.02595v1) — 相关度 0.75：提出函数调用结构化增益归因协议，给出严格评测基线，可迁移至agent技能注入评估。
- [From Registry to Repository: How AI Agent Skills Are Written, Adapted, and Maintained](http://arxiv.org/abs/2607.00911v2) — 相关度 0.75：首次实证研究AI agent技能的编写、复用与维护模式，为技能库与工具链设计提供依据。
- [Large Language Models for Multi-Lingual Equivalent Mutant Detection: An Extended Empirical Study](http://arxiv.org/abs/2607.00511v1) — 相关度 0.75：系统研究 LLM 在多语言等价变异体检测上的效果与泛化能力，对软件测试和质量保障 agent 有直接应用价值。
- [Adoption and Impact of Command-Line AI Coding Agents: A Study of Microsoft's Early 2026 Rollout of Claude Code and GitHub Copilot CLI](http://arxiv.org/abs/2607.01418v1) — 相关度 0.70：对编码 agent 在大规模组织的采用与影响进行实证研究，对 agent 部署评测有参考价值。
- [Cheap Code, Costly Judgment: A Case Study on Governable Agentic Software Engineering](http://arxiv.org/abs/2607.01087v2) — 相关度 0.70：对 agentic 软件工程的治理机制进行案例研究，提出可迁移的 governance conversion 模型。
- [Delta Debugging in the Absence of Test Oracles Through Metamorphic Testing](http://arxiv.org/abs/2607.00929v1) — 相关度 0.70：利用蜕变测试替代oracle的delta debugging，为无oracle场景下的输入最小化提供可迁移方案。
- [Kani: A Model Checker for Rust](http://arxiv.org/abs/2607.01504v1) — 相关度 0.70：Rust模型检验器提供正确性验证能力，可作为代码agent的验证后端或正确性检查工具。
- [Petrify: Petri-net Based Analysis of Concurrency Properties in Java Bytecode](http://arxiv.org/abs/2607.00830v1) — 相关度 0.70：Petri网编码Java字节码分析并发属性，可迁移为agent生成代码的并发验证后端。
- [Skills Are Not Islands: Measuring Dependency and Risk in Agent Skill Supply Chains](http://arxiv.org/abs/2607.01136v1) — 相关度 0.70：引入 agent skill 供应链和依赖分析，对 agent 生态的可靠性和风险治理有迁移价值。
- [Auditing the Audit: Five Failure Modes in Benchmark-Validity Audits](http://arxiv.org/abs/2607.02586v1) — 相关度 0.65：归纳基准有效性审计的五大失败模式并提出尽职调查门控，对 agent 评测的可靠性与元审计有参考价值。
- [Risk Architecture for AI-Native Engineering Teams: An Organizational Framework for Agentic System Governance](http://arxiv.org/abs/2607.01421v1) — 相关度 0.65：提出 AI-native 团队的风险架构框架，对 agent 系统治理和可靠性设计有迁移价值。
- [SchedCheck: Schedule-Robustness Analysis for Event-Driven Block Programs](http://arxiv.org/abs/2607.00623v1) — 相关度 0.65：对事件驱动块程序做调度鲁棒性验证，为生成代码的并发正确性提供可自动化校验工具。
- [Theoria: Rewrite-Acceptability Verification over Informal Reasoning States](http://arxiv.org/abs/2607.01223v3) — 相关度 0.65：基于状态重写的可审计验证架构，可迁移到 agent 生成结果的验证与审计。
- [Auditing Empirical Comparisons in Quantum Software](http://arxiv.org/abs/2607.00516v1) — 相关度 0.60：提出审计经验比较声明的方法，可迁移到性能benchmark与正确性证据验证。
- [AutoRestTest at the SBFT 2026 Tool Competition](http://arxiv.org/abs/2607.01063v1) — 相关度 0.60：结合 LLM 与多智能体强化学习的 REST API 测试工具，对 agent 驱动的测试生成有参考价值。
- [From Anatomy to Smells: An Empirical Study of SKILL.md in Agent Skills](http://arxiv.org/abs/2607.01456v2) — 相关度 0.60：实证分析 SKILL.md 中的 skill smells，为 agent skill 质量改进提供检测方法。
- [Leveraging LLM-Based Agentic Systems to Generate Quantum Applications for Test Optimization](http://arxiv.org/abs/2607.00939v1) — 相关度 0.60：多智能体协作生成量子应用用于测试优化，方法可迁移到代码 agent 的生成与验证流程，但领域特定。
- [The Moving Target: A Longitudinal Audit of Trustworthiness Drift Across Twelve Checkpoints of Open-Source Chat LLMs](http://arxiv.org/abs/2607.02587v1) — 相关度 0.60：纵向审计开源模型检查点间的信任分数漂移，强调评测结果需注明日期，对 agent 模型评测的可复现性有借鉴意义。
- [BT-APE: A Computationally Light Backtracking Approach to Automatic Prompt Engineering for Requirements Classification](http://arxiv.org/abs/2607.00427v1) — 相关度 0.55：轻量级回溯式自动提示工程方法可迁移到代码 agent 的 prompt 优化，但应用场景为需求分类，相关度中等。
- [Registry-Governed Agent Lifecycle:Completing EDDOps with Evaluation-DrivenRegistration, Promotion, and Retirement on AWS AgentCore](http://arxiv.org/abs/2607.00345v1) — 相关度 0.55：提出评估驱动的 agent 生命周期治理框架并映射到 AWS AgentCore，对 coding agent 的评测与部署治理有参考价值。
- [SAGE: Structured Agentic Graph Editing for Software Diagrams](http://arxiv.org/abs/2607.01102v1) — 相关度 0.55：用 LLM 做结构化图表编辑，验证修复流程可迁移到 agent 的结构化输出编辑。
