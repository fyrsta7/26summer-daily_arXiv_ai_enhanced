...
- [Better Harnesses, Smaller Models: Building 90% Cheaper Agents via Automated Harness Adaptation](http://arxiv.org/abs/2607.08938v1) — 相关度 0.95：元 agent 自动适配 harness，使小模型达到大模型性能且成本降 90%，直接指导低成本 agent 部署。
- [SCATE: Learning to Supervise Coding Agents for Cost-Effective Test Generation](http://arxiv.org/abs/2607.08983v1) — 相关度 0.95：将 coding agent 监督建模为上下文 bandit，自适应选择测试动作，显著提升覆盖率与成本效率。
- [TTHE: Test-Time Harness Evolution](http://arxiv.org/abs/2607.08124v1) — 相关度 0.95：将 harness 视为测试时可进化程序，用执行轨迹无监督优化 agent 流程，直接面向代码 agent 的自适应改进。
- [The Patchwork Problem in LLM-Generated Code](http://arxiv.org/abs/2607.08981v1) — 相关度 0.90：系统化定义 LLM 生成代码的结构一致性不变量和失败分类，并提供验证框架，直接针对代码 agent 产物缺陷。
- [Tool-Making and Self-Evolving LLM Agents in Low-Latency Systems](http://arxiv.org/abs/2607.08010v1) — 相关度 0.90：agentic tool-making 将重复 SOP 步骤编译为工具，显著降低延迟与错误率，可直接迁移到低延迟代码 agent 设计。
- [From Prompts to Contracts: Harness Engineering for Auditable Enterprise LLM Agents](http://arxiv.org/abs/2607.08028v1) — 相关度 0.85：用 harness、契约、校验构件将 LLM agent 审计化，对工程化 agent 的可靠落地与模型替换有强参考价值。
- [ProjAgent: Procedural Similarity Retrieval for Repository-Level Code Generation](http://arxiv.org/abs/2607.08691v1) — 相关度 0.85：提出过程相似性检索与静态分析反馈修复，用于仓库级代码生成，直接提升 agent 代码生成能力。
- [SeedSmith: LLM-Driven Seed Synthesis for Directed Fuzzing](http://arxiv.org/abs/2607.08949v2) — 相关度 0.80：LLM agentic 流水线为定向 fuzzing 合成种子，大幅加速崩溃发现，与代码测试和 agent 验证直接相关。
- [Programmers Are Poor and Overconfident Judges of LLM-Generated Assertions](http://arxiv.org/abs/2607.08885v1) — 相关度 0.60：实验揭示程序员难以判断 LLM 生成断言的正确性，对构建可靠验证与代码审查 agent 有警示价值。
- [Toward Inferring Accurate Context-free Grammars for Big Languages in a Black-box Setting](http://arxiv.org/abs/2607.08959v4) — 相关度 0.60：黑盒上下文无关文法推断改进 fuzzing 与程序分析，可为代码 agent 的输入生成和漏洞发现提供底层能力。
- [Aleena: Alignment Agent for Research Software Engineering Collaborations](http://arxiv.org/abs/2607.08043v1) — 相关度 0.55：面向研究软件工程协作的生命周期对齐 agent，其项目状态追踪与决策连续性机制可迁移到多 agent 协作场景。
- [From Generic to Personalized: Exploring Persona-Aware Code Review Explanations](http://arxiv.org/abs/2607.08990v1) — 相关度 0.50：探索个性化代码审查解释，对构建适配开发者风格的 AI 审查 agent 有参考价值。
- [Loop-Based Slicing and Input-Driven Concretization: An Empirical Study of Termination and Non-Termination Analysis](http://arxiv.org/abs/2607.08988v1) — 相关度 0.50：循环切片与输入具体化预处理可增强终止性验证，作为程序分析 agent 的预处理策略有迁移价值。
- [Workflow as Knowledge: Semantic Persistence for LLM-Mediated Workflows](http://arxiv.org/abs/2607.08740v1) — 相关度 0.50：将工作流建模为可检查、可恢复的知识对象，为代码 agent 工作流持久化与审计提供概念框架。
